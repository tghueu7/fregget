...
public class SchemaGenerator extends AbstractGenerator {
  ...
  /** Flag indicating wether to follow type references within complex type definitions. */
  ...
  /**
   * @param complexTypes Flag indicating wether or not to process complex types in the supplied
   *     schema.
   */
  ...
  /**
   * @param simpleTypes Flag indicating wether or not to process complex types in the supplied
   *     schema.
   */
  ...
  /**
   * Add the explicit bindings of XSD types to fully-qualified class names. If a type has a binding,
   * it will be treated as non-complex and bound to the named class.
   *
   * @param typeBindings
   */
   ...
   *   <li>If B is complex and A is referenced from the type definition of B, then A appears in the
   *       list before B.
   * </ul>
   */
  ...
  protected AttributeType createType(XSDSimpleTypeDefinition xsdType, int depth) {
    ...
    AttributeType gtType =
        factory.createAttributeType(
            name(xsdType), Object.class, false, false, Collections.EMPTY_LIST, superType, null);
    types.put(xsdType, gtType);
  ...
  protected AttributeType createType(XSDComplexTypeDefinition xsdType, int depth) {
    ...
    if (followComplexTypes) {
      ...
      for (Iterator itr = atts.iterator(); itr.hasNext(); ) {
        ...
        AttributeDescriptor ad =
            factory.createAttributeDescriptor(gtType, name(attribute), 0, 1, true, null);
        properties.add(ad);
      }
    }
