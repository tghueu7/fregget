...
public class ContentState {
  ...
  /**
   * Even used for batch notification; used to collect the bounds and feature ids generated over the
   ...
   * FeatureEvent internally (and then only if listeners are interested). You may find it easier to
   * construct your own FeatureEvent using {{@link #hasListener()} to check if you need to fire
   * events at all.
   *
   ...
   *
   * <p>If this content state is used for Transaction.AUTO_COMMIT the notification will be passed to
  ...
  /**
   * Notifies all waiting listeners that a commit has been issued; this notification is also sent to
   * our
   */
   ...
   * Clears cached state.
   *
   * <p>This method does not affect any non-cached state. This method may be extended by subclasses,
   * but not overiden.
   */
  public void flush() {
    featureType = null;
