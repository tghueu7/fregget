...
class DirectoryTypeCache {
  ...
  /**
   * Checks if the feature type cache contents needs updating, does so in case. The code assumes the
   * caller already owns a read only lock that needs upgrading in case the information is stale.
   */
   ...
   * </ul>
   *
   * All of this should be done trying to avoid re-creating all of the datastores already loaded. We
   * assume a properly written datastore will be able to detect changes in its own feature type list
   * and feature type schemas on its own.
   *
   * @throws IOException
   */
  ...
  /**
   * Looks up in the registry data store factories that do look like file data store ones, that is,
   * they accept a File/URL and a namespace, and returns an adapter that can be used to build a
   * datastore given a File and a namespace.
   */
      ...
      for (Param param : params) {
        ...
        if (File.class.isAssignableFrom(type) || URL.class.isAssignableFrom(type))
          fileParam = param;
            ...
            && (String.class.isAssignableFrom(type) || URI.class.isAssignableFrom(type)))
          nsParam = param;
      }
