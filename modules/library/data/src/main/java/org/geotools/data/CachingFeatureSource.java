...
public class CachingFeatureSource implements SimpleFeatureSource {
  ...
  public CachingFeatureSource(SimpleFeatureSource original) throws IOException {
    ...
    if (originalBounds == null)
      originalBounds =
          new Envelope(-Double.MAX_VALUE, Double.MAX_VALUE, -Double.MAX_VALUE, Double.MAX_VALUE);
  }
  ...
  boolean isSubQuery(Query query) {
    ...
    if (cachedPropNames != Query.ALL_NAMES
        && (propNames == Query.ALL_NAMES
            || !Arrays.asList(cachedPropNames).containsAll(Arrays.asList(propNames)))) return false;
   ...
   * including some more feature in the result) and a residual component that we cannot address with
   * the spatial index
   *
   * @param query
   */
