...
public class MemoryDataStoreTest extends DataTestCase {
  ...
  /**
   * When a data store is loaded with a reader, it would be nice if the memory data store preserved
   * feature order, so that features are always rendered the same way (rendering is different if
   * order changes and features do overlap)
   */
  ...
  public void testFeatureEvents() throws Exception {
    SimpleFeatureStore store1 =
        (SimpleFeatureStore) data.getFeatureSource(roadFeatures[0].getFeatureType().getTypeName());
  ...
  public void testCallingAddFeaturesWithReaderTwiceAndExtentInitialCollection() throws IOException {
    ...
    FeatureReader<SimpleFeatureType, SimpleFeature> secondReader =
        DataUtilities.reader(new SimpleFeature[] {SimpleFeatureBuilder.template(roadType, null)});
