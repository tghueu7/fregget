...
public class Line implements Cloneable, Serializable {
  ...
  /**
   * Given a set of data points {@code x[0..ndata-1]}, {@code y[0..ndata-1]}, fit them to a straight
   * line <var>y</var>=<var>b</var>+<var>m</var><var>x</var> in a least-squares senses. This method
   * assume that the <var>x</var> values are precise and all uncertainty is in <var>y</var>.
   *
   * <p>Reference: <a
  ...
  /**
   * Computes <var>x</var>=<var>f</var><sup>-1</sup>(<var>y</var>). If the line is horizontal, then
   * this method returns an infinite value. This method is final for performance reason.
   *
  ...
  /**
   * Returns the intersection point between this line and the specified bounded line. If both lines
   * are parallel or if the specified {@code line} doesn't reach this line (since {@link Line2D} do
   * not extends toward infinities), then this method returns {@code null}.
   *
  ...
  /**
   * Computes the base of a isosceles triangle having the specified summit and side length. The base
   * will be colinear with this line. In other words, this method compute two points
   ...
   * @return The base of the isoscele triangle, colinear with this line, or {@code null} if the base
   *     can't be computed. If non-null, then the triangle is the figure formed by joining
