...
public class LineWriter extends FilterWriter {
   ...
   * written. If whitespaces are followed by a character, they are written to the underlying stream
   * before the character. Otherwise, if whitespaces are followed by a line separator, then they are
   * discarted. The buffer capacity will be expanded as needed.
   */
  ...
  /**
   * Changes the line separator. This is the string to insert in place of every occurences of {@code
   * "\r"}, {@code "\n"} or {@code "\r\n"}.
   *
  ...
  /**
   * Writes a portion of an array of characters. This portion must <strong>not</strong> contains any
   * line separator.
   */
  ...
  /**
   * Flushs the stream's content to the underlying stream. This method flush completly all internal
   * buffers, including any whitespace characters that should have been skipped if the next
   * non-blank character is a line separator.
   *
   * @throws IOException If an I/O error occurs.
   */
  @Override
  ...
  /**
   * Returns {@code true} if the specified character is a white space that can be ignored on end of
   * line. The default implementation returns {@link Character#isSpaceChar(char)}. Subclasses can
   * override this method in order to change the criterion.
   *
