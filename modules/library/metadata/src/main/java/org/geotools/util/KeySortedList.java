...
public class KeySortedList<K extends Comparable<K>, V> extends AbstractSequentialList<V>
  ...
  /**
   * Returns {@code true} if the list contains an element {@linkplain #add(Comparable,Object) added}
   * with the specified key. This is equivalent to testing <code>{@linkplain #count count}(key) != 0
   * </code>.
   */
  ...
  private final class Iter implements ListIterator<V> {
    ...
    /**
     * Inserts the specified element into the list. The element will have the same key than the one
     * from the previous call to {@link #next} or {@link #previous}.
     */
  ...
  /**
   * Returns a view of the portion of this list whose keys are strictly less than {@code toKey}. The
   * returned list is backed by this list, so changes in the returned list are reflected in this
   * list, and vice-versa.
   *
