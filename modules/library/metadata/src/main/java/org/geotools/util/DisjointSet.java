...
public class DisjointSet<E> extends AbstractSet<E> implements Serializable {
   ...
   *       </ul>
   *       If an other value was mapped to the key, the old value is discarted. This is equivalents
   *       to removing the element from an other {@code DisjointSet} prior to add it to this set (in
   *       other words, moving the element).
   * </ul>
   *
   ...
   *       DisjointSet}, then the element is assumed to belongs to an other {@code DisjointSet} and
   *       nothing is done.
   *   <li>Otherwise, puts the key-value pair with the {@code trash} value in the underlying {@link
   *       Map}. This is equivalent to moving the element from this set to the "trash" set. Note
   *       that if the operation is applied on the "trash" set itself or if this set doesn't have a
   *       trash ({@code trash==null}), then the element is effectively removed from the underlying
   *       map.
   * </ul>
   */
   ...
   *
   * <p>{@code DisjointSet}s constructed using this constructor has no trash. All remove operations
   * on this set really remove all references to the removed element, like a usual {@link Set}. This
   * is opposed to moving the element to a "trash" set, which is allowed by the {@code
   ...
   * {@code disjointSet} will also be mutually exclusive with the newly created set. If {@code
   * disjointSet} has a trash set, the newly created set will use the same trash (i.e. all {@code
   * remove} operations will really move the element to the trash set). Otherwise, the new {@code
   * DisjointSet} have no trash.
   *
  ...
  /**
   * Ensures that this collection contains the specified element. Adding an element to this set will
   * remove it from any mutually exclusive set.
   *
   ...
   * this {@code DisjointSet} has a trash set, all removed elements will be added to the trash set.
   *
  ...
  private final class Iter implements Iterator<E> {
    ...
    /**
     * Entry to remove if the {@link #remove} operation is invoked, or {@code null} if this iterator
     * is not in a legal state for element removing. An element can be removed only after a {@link
     * #next} call and only if this call still synchronized with the {@link Iterator#next} call on
     * the underlying map's iterator.
     */
    ...
    /**
     * Returns the {@link #prefetch} entry. If this entry was {@code null}, fetch the next entry. If
     * there is no more entries, returns {@code null}.
     */
    private Map.Entry<E, DisjointSet<E>> prefetch() {
      toRemove = null;
      if (prefetch == null) {
