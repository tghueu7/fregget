...
public class RecordTypeImpl implements RecordType {
  ...
  public RecordTypeImpl(
      final RecordSchema parent, final TypeName typeName, final Collection<MemberName> members) {
    this.parent = parent;
   ...
   * @todo Should we really provide this method? There is no garantee that the user-provided values
   *     are consistent with {@link MemberName#getAttributeType}.
   */
  public RecordTypeImpl(
   ...
   * {@linkplain RecordSchema record schema}, then the record type name should be a valid in the
   ...
   * are no attributes, this method returns the empty set. This method is functionally equivalent to
   * <code>{@linkplain #getAttributeTypes()}.{@linkplain Map#keySet() keySet()}</code>.
   */
   ...
   * name is not defined in this record type, then this method returns {@code null}. This method is
   * functionnaly equivalent to <code>
