...
public final class Classes {
   ...
   *   <li>{@code Set<? extends Number>} returns {@code Number.class} as well, since that collection
   *       can not (in theory) contain instances of super-classes; {@code Number} is the <cite>upper
   *       bound</cite>.
   ...
   * bounds of the parameterized type. Otherwise returns {@code null}. This method provides the same
   * semantic than {@link #boundOfParameterizedAttribute(Field)}, but works on a getter or setter
   * method rather then the field. See the javadoc of above methods for more details.
   *
   * <p>This method is typically used for fetching the type of elements in a collection. We do not
   * provide a method working from a {@link Class} instance because of the way parameterized types
   * are implemented in Java (by erasure).
  ...
  /**
   * Returns the most specific class implemented by the objects in the given collection. If no class
   * are {@linkplain Class#isAssignableFrom assignable} to all others, then this method returns the
  ...
  /**
   * Removes every classes in the specified collection which are assignable from an other class from
   * the same collection. As a result of this method call, the given collection should contains only
   * leaf classes.
   *
   ...
   * interfaces. Only interfaces assignable to {@code base} are compared. Declaration order doesn't
   * matter. For example in ISO 19111, different interfaces exist for different coordinate system
   * geometries ({@code CartesianCS}, {@code PolarCS}, etc.). We can check if two CS implementations
   * has the same geometry with the following code:
   *
   * <blockquote>
   *
   * <code>
   ...
   *
   * @param type The type to test (may be {@code null}).
   * @return {@code true} if {@code type} is the primitive of wrapper class of {@link Long}, {@link
   *     Integer}, {@link Short} or {@link Byte}.
   */
   ...
   * {@link Boolean}, {@link Character} or {@link String} according the specified type. This method
   * is intentionnaly restricted to primitive types, with the addition of {@code String} which can
   * be though as an identity operation. Other types like {@link java.io.File} are not the purpose
   * of this method.
   *
  ...
  /**
   * Returns a short class name for the specified class. This method will omit the package name. For
   * example, it will return "String" instead of "java.lang.String" for a {@link String} object. It
   * will also name array according Java language usage, for example "double[]" instead of "[D".
   *
   ...
   * For example, it will return "String" instead of "java.lang.String" for a {@link String} object.
   *
