...
public abstract class LoggerAdapter extends Logger {
   ...
   * default implementation returns {@link Level#FINER}, which is consistent with the value used in
   * the Java logging framework. Subclasses should override this method if a different debug level
   * is wanted.
   */
   ...
   * be compiled to the {@code tableswitch} bytecode rather than {@code lookupswitch} (see <a
   ...
   * {@link Logger}, this implementation bypass the level check in order to let the backing logging
   * framework do its own check.
   */
  @Override
  ...
  /**
   * Logs a method entry to the {@linkplain #getDebugLevel debug level} with one parameter. Compared
   * to the default {@link Logger}, this implementation bypass the level check in order to let the
   * backing logging framework do its own check.
   */
  @Override
   ...
   * Compared to the default {@link Logger}, this implementation bypass the level check in order to
   * let the backing logging framework do its own check.
   */
  @Override
  ...
  /**
   * Logs a record at the specified level. The default implementation delegates to one of the {@link
   * #severe(String) severe}, {@link #warning(String) warning}, {@link #info(String) info}, {@link
   * #config(String) config}, {@link #fine(String) fine}, {@link #finer(String) finer} or {@link
   * #finest(String) finest} methods according the supplied level.
   */
  @Override
  @SuppressWarnings("fallthrough")
   ...
   * immediately, then delegates to <code>{@linkplain #log(Level,String) log}(level, message)</code>
   * .
   */
  @Override
  ...
  /**
   * Do nothing since this logger adapter does not support arbitrary parents. More specifically, it
   * should not inherits any configuration from a parent logger using the Java logging framework.
   */
  @Override
   ...
   * sure that {@link Filter#isLoggable} is invoked only once even if a {@code log} call is cascaded
   * into many other {@code log} calls, and this test must works in multi-threads environment).
   */
  @Override
