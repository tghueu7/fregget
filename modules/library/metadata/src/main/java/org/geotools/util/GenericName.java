...
public abstract class GenericName implements org.opengis.util.GenericName, Serializable {
  ...
  /**
   * Ensures that the given name is a {@link String} or an {@link InternationalString}. This is used
   ...
   * not modifiable. The scope of a name determines where a name "starts". For instance, if a name
   * has a {@linkplain #depth depth} of two ({@code "util.GenericName"}) and is associated with a
   * {@linkplain NameSpace name space} having the name {@code "org.opengis"}, then the fully
   * qualified name would be {@code "org.opengis.util.GenericName"}.
   *
   ...
   *
   * <p>The depth is the length of the list returned by the {@link #getParsedNames} method. As such
   ...
   * element in this list is like a directory name in a file path name. The length of this sequence
   * is the generic name depth.
   *
  ...
  /**
   * Returns a view of this name as a fully-qualified name, or {@code null} if none. The {@linkplain
   * #scope scope} of a fully qualified name must be {@linkplain NameSpace#isGlobal global}.
   *
   ...
   * toString()} method for a {@linkplain ScopedName scoped name} will contains the scope, while the
   * {@code toString()} method for the {@linkplain LocalName local version} of the same name will
   * not contains the scope.
   *
   ...
   * {@linkplain InternationalString#toString(Locale) specified locale}. If no international string
   * is available, then this method should returns an implementation mapping to {@link #toString}
   * for all locales.
   *
  ...
  /**
   * Compares this name with the specified object for order. Returns a negative integer, zero, or a
   * positive integer as this name lexicographically precedes, is equals to, or follows the
   ...
   *   <li>If all elements in both names are lexicographically equal, then if this name has less or
   *       more elements than the specified name, returns a negative or a positive integer
   *       respectively.
