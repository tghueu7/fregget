...
public class LineFormat extends Format {
  ...
  /**
   * Array of formats to use for parsing a line. Each format object in this array match one column.
   * For example {@code data[4]} will be parsed with {@code format[4]}. If the {@link #data} array
   * is longer than {@link #format}, then the last format is reused for all remaining columns.
   */
   ...
   * the index of the first character read for {@code data[0]}, <cite>etc</cite>. This array length
   * must be equals to <code>{@linkplain #data}.length + 1</code>. The last element will be the line
   * length.
   */
  private int[] limits;
   ...
   * @throws IllegalArgumentException if {@code formats} is null or an element of {@code format} is
   *     null.
   */
  ...
  /**
   * Sets all values in the current line. The {@code values} argument must be an array, which may be
   * of primitive type.
   *
   ...
   * @throws ParseException If {@code array} was not null and its length is not equals to the number
   *     of elements parsed, or if at least one element can't be parsed.
   */
  ...
  /**
   * Ensures that the number of columns just parsed is equals to the number of columns expected. If
   * a mismatch is found, then an exception is thrown.
   *
  ...
  private ParseException notAnInteger(final int i) {
    return new ParseException(
        Errors.format(ErrorKeys.NOT_AN_INTEGER_$1, line.substring(limits[i], limits[i + 1])),
        limits[i]);
  }
  ...
  /**
   * Formats an object and appends the resulting text to a given string buffer. This method invokes
   * <code>{@linkplain #setValues setValues}(values)</code>, then formats all columns using the
   * {@link Format} object specified at construction time. Columns are separated by tabulation.
   *
   * @since 2.4
   */
