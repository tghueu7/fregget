...
public abstract class AbstractMetadata {
   ...
   * {@code source} metadata must implements the same metadata interface (defined by the {@linkplain
   * #getStandard standard}) than this class, but don't need to be the same implementation class.
   * The copy is performed using Java reflections.
   *
  ...
  /**
   * Returns {@code true} if this metadata is modifiable. The default implementation uses heuristic
   * rules which return {@code false} if and only if:
   *
   * <p>
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Otherwise, this method conservatively returns {@code true}. Subclasses should override this
   * method if they can provide a more rigorous analysis.
   */
   ...
   * immediately reflected in the map. The keys are the property names as determined by the list of
   * {@code get*()} methods declared in the {@linkplain #getInterface metadata interface}.
   *
   ...
   * {@link javax.swing.tree} package, it can be seen as a data structure independent of Swing. It
   * will not force class loading of Swing framework.
   *
   * <p>In current implementation, the tree is not live (i.e. changes in metadata are not reflected
   * in the tree). However it may be improved in a future Geotools implementation.
   *
  ...
  /**
   * Compares this metadata with the specified object for equality. The default implementation uses
   * Java reflection. Subclasses may override this method for better performances.
   *
   * <p>This method performs a <cite>deep</cite> comparaison (i.e. if this metadata contains other
   * metadata, the comparaison will walk through the other metadata content as well) providing that
   * every childs implement the {@link Object#equals} method as well. This is the case by default if
   * every childs are subclasses of {@code AbstractMetadata}.
   *
  ...
  /**
   * Computes a hash code value for this metadata using Java reflection. The hash code is defined as
   * the sum of hash code values of all non-null properties. This is the same contract than {@link
