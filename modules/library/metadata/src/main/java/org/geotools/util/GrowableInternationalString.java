...
public class GrowableInternationalString extends AbstractInternationalString
  ...
  /**
   * The set of locales created in this virtual machine through methods of this class. Used in order
   * to get a {@linkplain #unique unique} instance of {@link Locale} objects.
   */
  ...
  /**
   * An unmodifiable view of the entry set in {@link #localMap}. This is the set of locales defined
   * in this international string. Will be constructed only when first requested.
   */
  private transient Set<Locale> localSet;

  /**
   * Constructs an initially empty international string. Localized strings can been added using one
   * of {@link #add add(...)} methods.
   */
  ...
  /**
   * Constructs an international string initialized with the specified string. Additional localized
   * strings can been added using one of {@link #add add(...)} methods. The string specified to this
   * constructor is the one that will be returned if no localized string is found for the {@link
   * Locale} argument in a call to {@link #toString(Locale)}.
   *
  ...
  public boolean add(final String prefix, final String key, final String string)
      ...
      if (position == length) {
        final Locale locale =
            (i == 0)
                ...
                : unique(
                    new Locale(
                        parts[0] /* language */, parts[1] /* country  */, parts[2] /* variant  */));
        add(locale, string);
        return true;
      }
    ...
    throw new IllegalArgumentException(
        Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "locale", key.substring(prefix.length())));
  }
   ...
   * {@linkplain Locale#getCountry country} part. For example if the <code>"fr_CA"</code> locale was
   * requested but not found, then this method looks for the <code>"fr"</code> locale. The {@code
   * null} locale (which stand for unlocalized message) is tried last.
   *
   ...
   *   <li>
   *       <p>If {@code candidate} is an instance of {@link CharSequence}, then this method returns
   *       {@code true} if {@link #toString(Locale)} returns a string {@linkplain String#equals
   *       equals} to <code>candidate.{@linkplain CharSequence#toString()
   *       toString()}</code> for all locales.
   *   <li>
   *       <p>If {@code candidate} is an instance of {@link Map}, then this methods returns {@code
   *       true} if all <var>{@linkplain Locale locale}</var>-<var>{@linkplain String string}</var>
   *       pairs are contained into {@code candidate}.
   *   <li>
