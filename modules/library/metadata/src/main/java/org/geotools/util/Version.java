...
public class Version implements CharSequence, Comparable<Version>, Serializable {
   ...
   * {@link String} otherwise. If there is no minor version number, then this method returns {@code
   * null}.
   *
   ...
   * major.minor.revision} form, index 0 stands for the major version number, 1 stands for the minor
   * version number and 2 stands for the revision number.
   *
   ...
   * compares only the {@linkplain #getMajor major} version number. A limit of 2 compares the major
   * and {@linkplain #getMinor minor} version numbers, <cite>etc</cite>. The comparaisons are
   * performed as {@link Integer} object if possible, or as {@link String} otherwise.
   *
  ...
  /**
   * Compare this version string with the specified object for equality. Two version are considered
   * equal if <code>{@linkplain #compareTo(Object) compareTo}(other) == 0</code>.
   *
