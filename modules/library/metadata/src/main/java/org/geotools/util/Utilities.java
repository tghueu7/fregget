...
public final class Utilities {
  /**
   * An array of strings containing only white spaces. Strings' lengths are equal to their index in
   * the {@code spaces} array. For example, {@code spaces[4]}œcontains a string of length 4. Strings
  ...
  /**
   * The class for the {@link #EMPTY_QUEUE} instance. Defined as a named class rather than anonymous
   * in order to avoid serialization issue.
   */
  ...
  /**
   * Returns {@code true} if the given booleans are equals. This overloaded flavor is provided only
   * for allowing developper to invoke {@code equals} methods without consideration for the argument
   * type.
   *
  ...
  /**
   * Returns {@code true} if the given bytes are equals. This overloaded flavor is provided only for
   * allowing developper to invoke {@code equals} methods without consideration for the argument
   * type.
   *
   * @param o1 The first value to compare.
   * @param o2 The second value to compare.
  ...
  /**
   * Returns {@code true} if the given integers are equals. This overloaded flavor is provided only
   * for allowing developper to invoke {@code equals} methods without consideration for the argument
   * type.
   *
   * @param o1 The first value to compare.
  ...
  /**
   * Returns {@code true} if the given longs are equals. This overloaded flavor is provided only for
   * allowing developper to invoke {@code equals} methods without consideration for the argument
   * type.
   *
   * @param o1 The first value to compare.
  ...
  /**
   * Returns {@code true} if the given floats are equals. Positive and negative zero are considered
   * different, while a NaN value is considered equal to other NaN values.
   *
   * @param o1 The first value to compare.
   * @param o2 The second value to compare.
   * @return {@code true} if both values are equal.
  ...
  /**
   * Returns {@code true} if the given doubles are equals. Positive and negative zero are considered
   * different, while a NaN value is considered equal to other NaN values.
   *
   * @param o1 The first value to compare.
   * @param o2 The second value to compare.
  ...
  /**
   * Convenience method for testing two objects for equality. One or both objects may be null. This
   * method do <strong>not</strong> iterates recursively in array elements. If array needs to be
   * compared, use one of {@link Arrays} method or {@link #deepEquals deepEquals} instead.
   *
   * <p><b>Note on assertions:</b> There is no way to ensure at compile time that this method is not
   * invoked with array arguments, while doing so would usually be a program error. Performing a
   * systematic argument check would impose a useless overhead for correctly implemented {@link
   * Object#equals} methods. As a compromise we perform this check at runtime only if assertions are
   * enabled. Using assertions for argument check in a public API is usually a deprecated practice,
   * but we make an exception for this particular method.
   *
   ...
   * more efficient) methods are provided for every primitive types. This is true even when the two
   * arguments are different primitive type because of widening conversions. The only exception is
   * when a {@code boolean} argument is mixed with a different primitive type.
   *
   ...
   *
   * <p>This method may be useful when the objects may or may not be array. If they are known to be
   * arrays, consider using {@link Arrays#deepEquals(Object[],Object[])} or one of its primitive
   * counter-part instead.
   *
   * <p><strong>Rules for choosing an {@code equals} or {@code deepEquals} method</strong>
   *
   * <ul>
   *   <li>If <em>both</em> objects are declared as {@code Object[]} (not anything else like {@code
   *       String[]}), consider using {@link Arrays#deepEquals(Object[],Object[])} except if it is
   *       known that the array elements can never be other arrays.
   *   <li>Otherwise if both objects are arrays (e.g. {@code Expression[]}, {@code String[]}, {@code
   *       int[]}, <cite>etc.</cite>), use {@link Arrays#equals(Object[],Object[])}. This rule is
   *       applicable to arrays of primitive type too, since {@code Arrays.equals} is overriden with
   *       primitive counter-parts.
   ...
   *   <li>Otherwise if <em>both</em> objects are declared exactly as {@code Object} type and it is
   *       known that they could be arrays, only then invoke this {@code deepEquals} method. In such
   *       case, make sure that the hash code is computed using {@link #deepHashCode} for
   *       consistency.
   * </ul>
   *
   * @param object1 The first object to compare, or {@code null}.
   ...
   * @param seed The hash code value computed so far. If this method is invoked for the first field,
   *     then any arbitrary value (preferrably different for each class) is okay.
   ...
   * @param value The value whose hash code to compute.
   * @param seed The hash code value computed so far. If this method is invoked for the first field,
   *     then any arbitrary value (preferably different for each class) is okay.
   * @return An updated hash code value.
   */
  ...
  /**
   * Alters the given seed with the hash code value computed from the given value. {@code byte} and
   * {@code short} primitive types are handled by this method as well through implicit widening
   * conversion.
   *
   * @param value The value whose hash code to compute.
   * @param seed The hash code value computed so far. If this method is invoked for the first field,
  ...
  /**
   * Alters the given seed with the hash code value computed from the given value. The givan object
   * may be null. This method do <strong>not</strong> iterates recursively in array elements. If
   * array needs to be hashed, use one of {@link Arrays} method or {@link #deepHashCode
   * deepHashCode} instead.
   *
   * <p><b>Note on assertions:</b> There is no way to ensure at compile time that this method is not
   * invoked with an array argument, while doing so would usually be a program error. Performing a
   * systematic argument check would impose a useless overhead for correctly implemented {@link
   * Object#hashCode} methods. As a compromise we perform this check at runtime only if assertions
  ...
  /**
   * Returns a hash code for the specified object, which may be an array. This method returns one of
  ...
  /**
   * Returns a string of the specified length filled with white spaces. This method tries to return
   * a pre-allocated string if possible.
   *
  ...
  /**
   * Creates a stream with zero or one element, depending on whether the specified {@link Optional}
   * is empty or not.
   *
  ...
  /**
   * Collects stream elements into a {@code Map} whose values are the stream elements and whose keys
   * are those elements' fully qualified class names.
   *
   * <p>If the stream contains several instances of the same class, only one of them will be present
   * in the map. If the stream has an iteration order, the later elements win over earlier ones.
   *
   * @param <T> the type of elements in the stream
