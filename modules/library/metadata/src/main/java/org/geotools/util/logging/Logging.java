...
public final class Logging {
   ...
   *
   * <p>The plain array used there is not efficient for adding new items (an {@code ArrayList} would
   * be more efficient), but we assume that very few new items will be added. Furthermore a plain
   * array is efficient for reading, and the later is way more common than the former.
   */
  ...
  /**
   * {@code true} if every {@link Logging} instances use the same {@link LoggerFactory}. This is an
  ...
  static {
    ...
    try {
      ...
      if (imagingListener == null
        ...
        if (LOGGING_TRACE) {
          System.out.println(
              "Logging JAI messages: ImagingListener already in use: " + imagingListener);
        }
      }
  ...
  /**
   * Creates an instance for the root logger. This constructor should not be used for anything else
   * than {@link #ALL} construction; use {@link #getLogging} instead.
   */
  private Logging() {
    name = "";
  }
  ...
  /**
   * Returns the logger factory, or {@code null} if none. This method returns the logger set by the
   * last call to {@link #setLoggerFactory} on this {@code Logging} instance or on one of its
   * parent.
   */
    ...
    if (className == null) {
      ...
      if (!LoggerFactory.class.isAssignableFrom(factoryClass)) {
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.ILLEGAL_CLASS_$2, factoryClass, LoggerFactory.class));
      }
      try {
   ...
   * method replaces the simple formatter by an instance of {@link MonolineFormatter}. If no {@code
   * ConsoleHandler} are found, then a new one is created.
   *
  ...
  /**
   * Same as {@link #forceMonolineConsoleOutput()}, but additionnaly set an optional logging level.
   * If the specified level is non-null, then all {@link java.util.logging.Handler}s using the
   * monoline formatter will be set to the specified level.
   *
   * <p><b>Note:</b> Avoid this method as much as possible, since it overrides user's level setting.
   * A user trying to configure his logging properties may find confusing to see his setting
   * ignored.
   *
  ...
  /**
   * Invoked when an unexpected error occurs. This method logs a message at the {@link Level#WARNING
   * WARNING} level to the specified logger. The originating class name and method name are inferred
   * from the error stack trace, using the first {@linkplain StackTraceElement stack trace element}
   * for which the class name is inside a package or sub-package of the logger name. For example if
   * the logger name is {@code "org.geotools.image"}, then this method will uses the first stack
   * trace element where the fully qualified class name starts with {@code "org.geotools.image"} or
   * {@code "org.geotools.image.io"}, but not {@code "org.geotools.imageio"}.
   *
  ...
  /**
   * Invoked when an unexpected error occurs. This method logs a message at the {@link Level#WARNING
   * WARNING} level to the specified logger. The originating class name and method name can
   * optionnaly be specified. If any of them is {@code null}, then it will be inferred from the
   * error stack trace as in {@link #unexpectedException(Logger, Throwable)}.
   *
   * <p>Explicit value for class and method names are sometime preferred to automatic inference for
   ...
   *       really want to know about the private method, the stack trace is still available anyway.
   * </ul>
   *
   * @param logger Where to log the error.
  ...
  /**
   * Invoked when an unexpected error occurs. This method logs a message at the {@link Level#WARNING
   * WARNING} level to the logger for the specified package name. The originating class name and
   * method name can optionnaly be specified. If any of them is {@code null}, then it will be
   * inferred from the error stack trace as in {@link #unexpectedException(Logger, Throwable)}.
   *
   ...
   */
  public static boolean unexpectedException(
      final String paquet, final Class<?> classe, final String method, final Throwable error) {
   ...
   * #unexpectedException(Logger,Class,String,Throwable) unexpectedException} except that it doesn't
