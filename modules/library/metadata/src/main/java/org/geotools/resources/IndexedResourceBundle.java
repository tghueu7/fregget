...
public class IndexedResourceBundle extends ResourceBundle {
   ...
   * #summarize}. Resource strings are never cut to this length. However, text replacing "{0}" in a
   * string like "Parameter name is {0}." will be cut to this length.
   */
  private static final int MAX_STRING_LENGTH = 200;

  /**
   * The resource name of the binary file containing resources. It may be a file name or an entry in
   * a JAR file.
   */
  private final String filename;

  /**
   * The array of resources. Keys are an array index. For example, the value for key "14" is {@code
   * values[14]}. This array will be loaded only when first needed. We should not load it at
   * construction time, because some {@code ResourceBundle} objects will never ask for values. This
   * is particularly the case for ancestor classes of {@code Resources_fr_CA}, {@code Resources_en},
   * {@code Resources_de}, etc., which will only be used if a key has not been found in the
   * subclass.
   */
   ...
   *
   * @param key Key for the requested resource, or {@code null} if all resources are requested. This
   *     key is used mostly for constructing messages.
  ...
  private String[] ensureLoaded(final String key) throws MissingResourceException {
    ...
    try {
      ...
      final MissingResourceException error =
          new MissingResourceException(exception.getLocalizedMessage(), getClass().getName(), key);
   ...
   * removed). If {@code text} is longer, it will be cut somewhere in the middle. This method tries
   * to cut between two words and replace the missing words with "(...)". For example, the following
   * string:
   *
   * <blockquote>
   *
  ...
  /**
   * Returns {@code arguments} as an array. If {@code arguments} is already an array, this array or
   * a copy of this array will be returned. If {@code arguments} is not an array, it will be placed
   * in an array of length 1. In any case, all the array's elements will be checked for {@link
   * String} objects. Any strings of length greater than {@link #MAX_STRING_LENGTH} will be reduced
   * using the {@link #summarize} method.
   *
   ...
   * to {@link java.util.logging.Formatter#formatMessage}, except that the work will be delegated to
   * {@link #getString(int, Object)} if the {@linkplain LogRecord#getResourceBundle record resource
   * bundle} is an instance of {@code IndexedResourceBundle}.
   *
