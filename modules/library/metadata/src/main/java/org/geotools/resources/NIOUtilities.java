...
public final class NIOUtilities {
     ...
     * garbage collectors that might result in a JVM crash (failure to perform native memory 
     * allocation)
     *
     ...
     * value. The buffers are pooled, so remember to call {@link #clean(ByteBuffer, false)} to 
     * return
     ...
     * <ul><li>if the buffer is memory mapped (as per the specified parameter) the effect is the 
     * same as {@link #clean(ByteBuffer)}</li>
    ...
    private static synchronized void log(final Exception e, final ByteBuffer buffer) {
        warned = true;
        String message = "Error attempting to close a mapped byte buffer : " + buffer.getClass()
                .getName()
