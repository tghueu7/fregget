...
public class TableWriter extends FilterWriter {
  /**
   * A possible value for cell alignment. This specifies that the text is aligned to the left indent
   * and extra whitespace should be placed on the right.
   */
  ...
  /**
   * A possible value for cell alignment. This specifies that the text is aligned to the center and
   * extra whitespace should be placed equally on the left and right.
   */
   ...
   * null} value or a {@link Cell} object with <code>{@link Cell#text}==null</code> are move to the
   * next line.
   */
  ...
  /**
   * Column position of the cell currently being written. The field is incremented each time {@link
   * #nextColumn()} is invoked.
   */
  private int column;
  ...
  /**
   * Maximum width for each columns. This array's length must be equals to the number of columns in
   * this table.
   */
  ...
  /**
   * Tells if the next '\n' character must be ignored. This field is used in order to avoid writing
   * two EOL in place of {@code "\r\n"}.
   */
   ...
   *     underlying stream. If {@code out} is null, then the {@link #toString} method is the only
   *     way to get the table's content.
   * @param separator String to write between columns. Drawing box characters are treated specially.
   *     For example {@code " \\u2502 "} can be used for a single-line box.
   ...
   *       next row (i.e. tabulation and EOL are equivalent to {@link #nextColumn()} and {@link
  ...
  /**
   * Writes a portion of an array of characters. Tabulations and line separators are interpreted as
   * by {@link #write(int)}.
   *
  ...
  /**
   * Moves one column to the right. Next write operations will occur in a new cell on the same row.
   * This method fill every remaining space in the current cell with the specified character. For
   * example calling {@code nextColumn('*')} from the first character of a cell is a convenient way
   * to put a pad value in this cell.
   *
  ...
  /**
   * Moves to the first column on the next row. Next write operations will occur on a new row. This
   * method fill every remaining cell in the current row with the specified character. Calling
   * {@code nextLine('-')} from the first column of a row is a convenient way to fill this row with
   * a line separator.
   *
  ...
  /**
   * Flushs the table content to the underlying stream. This method should not be called before the
   * table is completed (otherwise, columns may have the wrong width).
   *
  ...
  /**
   * Écrit vers le flot spécifié toutes les cellules qui avaient été disposées dans le tableau. Ces
   * cellules seront automatiquement alignées en colonnes. Cette méthode peut être appelée plusieurs
   * fois pour écrire le même tableau par exemple vers plusieurs flots.
   *
