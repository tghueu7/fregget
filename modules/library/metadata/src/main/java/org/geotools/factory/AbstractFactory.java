...
public class AbstractFactory implements Factory, RegistrableFactory {
  ...
  /**
   * The maximum priority for a factory, which is {@value}. Factories with highest priority will be
   * preferred to any other factory in the same {@linkplain ServiceRegistry#getCategories category}.
   *
   * @see #priority
   * @see #onRegistration
   */
  ...
  /**
   * The {@linkplain Factory#getImplementationHints implementation hints}. This map should be filled
   * by subclasses at construction time. If possible, constructors should not copy blindly all
  ...
  /**
   * Returns the priority for this factory, as a number between {@link #MINIMUM_PRIORITY} and {@link
   * #MAXIMUM_PRIORITY} inclusive. Priorities are used by {@link FactoryRegistry} for selecting a
   * preferred factory when many are found for the same service. The default implementation returns
   * {@link #priority} with no change. Subclasses should override this method if they want to return
  ...
  /**
   * Called when this factory is added to the given {@code category} of the given {@code registry}.
   * The factory may already be registered under another category or categories.
   *
   * <p>This method is invoked automatically when this factory is registered as a plugin, and should
   * not be invoked directly by the user. The default implementation iterates through all services
   * under the same category that extends the {@code AbstractFactory} class, and set the ordering
   ...
   *
   * <p>This method is invoked automatically when this factory is no longer registered as a plugin,
  ...
  /**
   * Returns a hash value for this factory. The default implementation computes the hash value using
   * only immutable properties. This computation do <strong>not</strong> relies on {@linkplain
   * #getImplementationHints implementation hints}, since there is no garantee that they will not
   * change.
   *
   * @since 2.3
   */
   ...
   * implementation formats all {@linkplain #getImplementationHints implementation hints} as a tree.
   * If the implementation hints include some {@linkplain Factory factory} dependencies, then the
   * implementation hints for those dependencies will appears under a tree branch.
   *
   * @since 2.3
   */
  @Override
  ...
  /**
   * Returns a string representation of the specified hints. This is used by {@link Hints#toString}
   * in order to share the code provided in this class.
   */
  ...
  /**
   * Formats the specified hints. This method is just the starting point for {@link #format(Writer,
   * Map, String, Map)} below.
   */
