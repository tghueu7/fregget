...
public class LoggedFormat<T> extends Format {
    ...
    if (index < text.length()) {
      ...
      doLogWarning(
          Errors.getResources(getWarningLocale())
              .getLogRecord(level, ErrorKeys.ILLEGAL_CLASS_$2, value.getClass(), type));
      return null;
    }
  ...
  /**
   * Formats the specified object. This method delegates the work to the {@linkplain Format format}
  ...
  /**
   * Logs a warning. This method is invoked automatically by the {@link #parse parse} method when a
   * text can't be fully parsed. The default implementation logs the warning to the logger specified
   * by the last call to the {@link #setLogger setLogger} method. Subclasses may override this
   * method if they want to change the log record before the logging.
   *
   ...
   * {@link #formatUnparsable(String, int, int, Locale, Level) formatUnparsable(..., Level)}, except
   * that the result is returned as a {@link String} rather than a {@link LogRecord}. This is
   * provided as a convenience method for creating the message to give to an {@linkplain
  ...
  public static LogRecord formatUnparsable(
      final String text, final int index, final int errorIndex, final Locale locale, Level level) {
    if (level == null) {
