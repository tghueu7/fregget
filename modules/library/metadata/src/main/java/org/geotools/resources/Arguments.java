...
public class Arguments {
   ...
   *
   * @param args Command line arguments. Arguments {@code "-encoding"} and {@code "-locale"} will be
  ...
  private Locale getLocale(final String locale) throws IllegalArgumentException {
    if (locale != null) {
      ...
      switch (s.length) {
        ...
        default:
          illegalArgument(
              new IllegalArgumentException(Errors.format(ErrorKeys.BAD_LOCALE_$1, locale)));
      }
    }
   ...
   *
   * <p>Paramater may be instructions like "-encoding cp850" or "-encoding=cp850". Both forms (with
   * or without "=") are accepted. Spaces around the '=' character, if any, are ignored.
   *
  ...
  public String getRequiredString(final String name) {
    ...
    if (value == null) {
      illegalArgument(
          new IllegalArgumentException(
              Errors.getResources(locale).getString(ErrorKeys.MISSING_PARAMETER_$1, name)));
    }
    return value;
  }
   ...
   * {@link Double#parseDouble(String)} method, which means that the parsing is locale-insensitive.
   * Locale insensitive parsing is required in order to use arguments in portable scripts.
   *
   * @param name The parameter name. Name are case-insensitive.
   * @return The parameter value, of {@code null} if there is no parameter given for the specified
   *     name.
  ...
  /**
   * Returns an optional boolean value from the command line. The value, if defined, must be "true"
   * or "false".
   *
   * @param name The parameter name. Name are case-insensitive.
   * @return The parameter value, of {@code null} if there is no parameter given for the specified
  ...
  /**
   * Returns the list of unprocessed arguments. If the number of remaining arguments is greater than
   * the specified maximum, then this method invokes {@link #illegalArgument}.
   *
   ...
   * forbidden prefix is usually {@code '-'}, the character used for options as in "{@code
