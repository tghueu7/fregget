...
public class FactoryRegistry {
   ...
   * IllegalArgumentException}s if it encounters any. This factory registry relies on that behavior
   * and "outsources" a lot of its {@code null} checks this way.
   */
  ...
  /**
   * The set of category that need to be scanned for plugins. On initialization, all categories need
   * to be scanned for plugins. After a category has been first used, it is removed from this set so
  ...
  public <T> Iterator<T> getServiceProviders(
      final Class<T> category, final ServiceRegistry.Filter filter, final boolean useOrdering) {
  ...
  /**
   * Implementation of {@link #getFactories(Class, Predicate, Hints)} without the filtering applied
   * by the {@link #isAcceptable(Object, Class, Hints, Predicate)} method. If this filtering is not
   * already presents in the filter given to this method, then it must be applied on the elements
   * returned by the iterator. The later is preferable when:
   *
   * <p>
   *
   * <ul>
   ...
   * it is invoked. Except as a result of this scan, no new factory instance is created by the
   ...
   * of {@code hints} and the {@code candidateFilter}.
   *
  ...
  /**
   * Returns {@code true} if the specified {@code factory} meets the requirements specified by a map
   * of {@code hints}.
   *
   ...
   *
   * <p>Safely checks {@link OptionalFactory#isAvailable()} which can be used at runtime to allow a
   ...
   * authorities. If one or both factories are not currently registered, or if the desired ordering
   * is already set/unset, nothing happens and false is returned.
   *
