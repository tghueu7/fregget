...
public class MonolineFormatter extends Formatter {
  ...
  /**
   * The minimum amount of spaces to use for writting level and module name before the message. For
   * example if this value is 12, then a message from module "org.geotools.core" with level FINE
   * would be formatted as "<code>[core&nbsp;&nbsp;FINE]</code> <cite>the message</cite>" (i.e. the
   ...
   * {@link SimpleDateFormat}. For example, the pattern <code>"HH:mm:ss.SSS"</code> will display the
   * ellapsed time in hours, minutes, seconds and milliseconds.
   *
  ...
  /**
   * Returns the format for displaying ellapsed time. This is the pattern specified to the last call
   * to {@link #setTimeFormat}, or the patten specified in the {@code
   ...
   * replaced by a default value, while <code>"none"</code> means that the user explicitly requested
   * no source.
   *
   ...
   * {@link #setSourceFormat}, or the patten specified in the {@code
   * org.geotools.MonolineFormater.source} property in the {@code jre/lib/logging.properties} file.
   *
  ...
  /**
   * Setup a {@code MonolineFormatter} for the specified logger and its children. This method search
   * for all instances of {@link ConsoleHandler} using the {@link SimpleFormatter}. If such
   * instances are found, they are replaced by a single instance of {@code MonolineFormatter}. If no
   * such {@link ConsoleHandler} are found, then a new one is created with this {@code
   * MonolineFormatter}.
   *
   ...
   * @return The registered {@code MonolineFormatter} (never {@code null}). The formatter output can
   *     be configured using the {@link #setTimeFormat} and {@link #setSourceFormat} methods.
   */
