...
public class CommandLine {
  ...
  /**
   * Creates a new {@code CommandLine} instance from the given arguments. If this constructor fails
   * because of a programming error (for example a type not handled by {@link #parse parse} method),
   * then an exception is thrown like usual. If this constructor fails because of some user error
   * (e.g. if a mandatory argument is not provided) or some other external conditions (e.g. an
   * {@link IOException}), then it prints a short error message and invokes {@link System#exit} with
   * one the {@code EXIT_CODE} constants.
   *
   * @param args The command-line arguments.
  ...
  /**
   * Parses the given string as a value of the given type. This method is invoked automatically for
   * values that are not of one of the pre-defined types. The default implementation thrown an
  ...
  /**
   * Prints a description of all arguments to the {@linkplain #out standard output}. This method is
   * invoked automatically if the user provided the {@code --help} argument on the command line.
   * Subclasses can override this method in order to prints a summary before the option list.
   */
  protected void help() {
    ...
    optional.put(
        "encoding=S", "Set the input and output encoding. Examples: \"UTF-8\", \"ISO-8859-1\".");
