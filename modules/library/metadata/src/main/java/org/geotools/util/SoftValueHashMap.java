...
public class SoftValueHashMap<K, V> extends AbstractMap<K, V> {
   ...
   * last. Note that in a highly concurrent environments the exact total number of strong references
   * may differ slightly from {@link #hardReferencesCount}.
   */
   ...
   * already {@link #hardReferencesCount} hard references, then this method replaces the oldest hard
   * reference by a soft one.
   */
  @SuppressWarnings("unchecked")
  private void retainStrongly(final K key) {
    ...
    if (hardCache.size() > hardReferencesCount) {
      ...
      if (v
          instanceof
          Reference) // check if v is already a Reference: it can happen in concurrent environments
   ...
   * @return previous value associated with specified key, or {@code null} if there was no entry for
   *     key.
   */
  @Override
  ...
  private static final class Iter<K, V> implements Iterator<Map.Entry<K, V>> {
    ...
    /**
     * Set {@link #entry} to the next entry to iterate. Returns {@code true} if an entry has been
     * found, or {@code false} if the iteration is finished.
     */
    @SuppressWarnings("unchecked")
    private boolean findNext() {
      while (iterator.hasNext()) {
        ...
        if (value instanceof Reference) {
          ...
          entry =
              new MapEntry<K, V>((K) SoftValueHashMap.resolveNull(candidate.getKey()), (V) value);
          return true;
        }
        if (value != null) {
          entry =
    ...
    /**
     * Returns the next value. If some value were garbage collected after the iterator was created,
     * they will not be returned. A {@link ConcurrentModificationException} is not thrown since a
  ...
  private static final class Reference<K, V> extends SoftReference<V> {
    /**
     * A reference to the {@link SoftValueHashMap#hash} entries. We keep this reference instead than
     * a reference to {@link SoftValueHashMap} itself in order to avoid indirect retention of {@link
     * SoftValueHashMap#hardCache}, which is not needed for this reference.
     */
     ...
     * </ul>
     *
     * This is because this method is invoked when the entry should have already be removed, or is
     * about to be removed.
     */
  ...
  /**
   * A delegate that can be used to perform clean up operation, such as resource closing, before the
   * values cached in soft part of the cache gets disposed of
   *
