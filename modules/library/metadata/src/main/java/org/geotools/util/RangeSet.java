...
public class RangeSet<T extends Comparable<? super T>> extends AbstractSet<Range<T>>
  ...
  /**
   * The comparator for ranges. Defined only in order to comply to {@link #comparator} contract, but
   * not used for internal working in this class.
   */
  ...
  /**
   * Identical to {@code elementClass} except if the later is the wrapper of some primitive type. In
   * the later case this field is set to that primitive type. This is the type to be used in arrays.
   */
   ...
   * doublon.
   *
   * <p>La longueur de ce tableau est le double du nombre d'intervalles. Il aurait été plus efficace
   * d'utiliser une variable séparée (pour ne pas être obligé d'agrandir ce tableau à chaque ajout
   * d'un intervalle), mais malheureusement le J2SE 1.4 ne nous fournit pas de méthode {@code
   * Arrays.binarySearch} qui nous permettent de spécifier les limites du tableau (voir RFE #4306897
  ...
  /**
   * {@code true} if and only if the element class represents a primitive type. This is equivalents
   * to {@code primitiveType.isPrimitive()} and is computed once for ever for performance reason.
   */
  private final boolean isPrimitive;

  /**
   * {@code true} if we should invoke {@link ClassChanger#toNumber} before to store a value into the
   * array. It will be the case if the array {@code array} contains primitive elements and the type
   ...
   *
   * <p>Note: current version do not support open interval (i.e. {@code Range.is[Min/Max]Included()}
  ...
  /**
   * Returns a {@linkplain Range#getMinValue range's minimum value} as a {@code double}. The {@code
   * index} can be any value from 0 inclusive to the set's {@link #size size} exclusive. The
   * returned values always increase with {@code index}.
   *
  ...
  /**
   * Returns a {@linkplain Range#getMaxValue range's maximum value} as a {@code double}. The {@code
   * index} can be any value from 0 inclusive to the set's {@link #size size} exclusive. The
   * returned values always increase with {@code index}.
   *
   * @param index The range index, from 0 inclusive to {@link #size size} exclusive.
  ...
  /**
   * An iterator for iterating through ranges in a {@link RangeSet}. All elements are {@link Range}
   * objects.
   *
   * @version $Id$
   * @author Martin Desruisseaux (IRD)
