...
public final class XMath {
  ...
  /**
   * The sequence of prime numbers computed so far. Will be expanded as needed. We limit ourself to
   * 16 bits numbers because they are suffisient for computing divisors of any 32 bits number.
   */
  ...
  /**
   * Computes 10 raised to the power of <var>x</var>. This method tries to be slightly more accurate
   * than <code>{@linkplain Math#pow Math.pow}(10,x)</code>, sometime at the cost of performance.
   *
   ...
   * 10 is a special case since it is used for scaling axes or formatting human-readable output, in
   * which case the precision may matter.
   *
   * @param x The exponent.
   * @return 10 raised to the given exponent.
   */
   ...
   * rounded value is not greater than the specified amount of floating point units. This method can
   * be used for hiding floating point error likes 2.9999999996.
   *
   ...
   * specified value. This method tries small changes to {@code value}, by adding or substracting up
   * to {@code maxULP} (Unit in the Last Place). If there is no small change that remove at least
   * {@code n} fraction digits, then the value is returned unchanged. This method is used for hiding
   * rounding errors, like in conversions from radians to degrees.
   *
  ...
  /**
   * Counts the fraction digits in the string representation of the specified value. This method is
   * equivalent to a calling <code>{@linkplain Double#toString(double) Double.toString}(value)
   ...
   * fields ranging from {@code 0x7f800001} through {@code 0x7fffffff} or {@code 0xff800001} through
   * {@code 0xffffffff}. The standard {@link Float#NaN} has bit fields {@code 0x7fc00000}. See
   * {@link Float#intBitsToFloat} for more details on NaN bit values.
   *
