...
public class Range<T extends Comparable<? super T>> implements Serializable {
  ...
  /**
   * Creates a new range using the same element class than this range. This method will be overriden
   * by subclasses in order to create a range of a more specific type.
   */
  Range<T> create(
      final T minValue,
      final boolean isMinIncluded,
  ...
  /**
   * Returns {@code true} if this range contains the given value. A range never contains the {@code
   * null} value. This is consistent with the {@linkplain Range class javadoc} stating that null
   * {@linkplain #getMinValue minimum} or {@linkplain #getMaxValue maximum} values are exclusive.
   *
  ...
  /**
   * Implementation of {@link #intersects(Range)} to be invoked directly by subclasses. "NC" stands
   * for "No Cast" - this method do not try to cast the value to a compatible type.
   */
  ...
  /**
   * Compares the provided value with the {@linkplain #getMinValue minimum value}, taking in account
   * the included or excluded state.
   *
   ...
   * @return 0 if the given value is equal to the {@link #minValue} and both are included, negative
   *     if {@link #minValue} is lower, positive if {@link #minValue} is higher.
   */
  ...
  /**
   * Compares the provided value with the {@linkplain #getMaxValue maximum value}, taking in account
   * the included or excluded state.
   *
   * @param value The value to compare to this range {@linkplain #getMaxValue maximum}.
   * @param delta 0 if the value is inclusive, -1 if exclusive and the first inclusive value is
   *     lower, or +1 if exclusive and the first inclusive value is higher than the given one.
   * @return 0 if the given value is equal to the {@link #maxValue} and both are included, negative
   *     if {@link #maxValue} is lower, positive if {@link #maxValue} is higher.
   */
