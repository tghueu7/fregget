...
public class Range<T extends Comparable<? super T>> implements Serializable {
     ...
     * @param value        The minimal and maximum value (inclusive), or {@code null} for an 
     *                     unbounded range.
     */
     ...
     */
    static void ensureNonNull(final String name, final Object value) throws 
    ...
    public boolean contains(final Comparable<?> value) throws IllegalArgumentException {
        ...
        @SuppressWarnings("unchecked") final T c = (T) value;
        return containsNC(c);
    }
    ...
    final boolean containsNC(final Range<? extends T> range) {
        return (minValue == null || compareMinTo(range.minValue, range.isMinIncluded ? 0 : +1) <=
                0) &&
                (maxValue == null || compareMaxTo(range.maxValue, range.isMaxIncluded ? 0 : -1) 
                        >= 0);
    }
        ...
        if (!intersects(range)) {
            ...
            if (clipMin) {
                ...
                if (!clipMax) {
                    ...
                    array[0] = create(minValue, isMinIncluded, range.minValue, !range
                            .isMinIncluded);
                    array[1] = create(range.maxValue, !range.isMaxIncluded, maxValue, 
                            isMaxIncluded);
                    return array;
                }
     ...
     *              lower, or +1 if exclusive and the first inclusive value is higher than the 
     *              given one.
