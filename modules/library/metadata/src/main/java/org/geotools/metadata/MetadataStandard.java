...
public final class MetadataStandard {
   ...
   * HREF="http://geoapi.sourceforge.net">GeoAPI</A> interfaces in the {@link org.opengis.metadata}
  ...
  /**
   * Shared pool of {@link PropertyTree} instances, once for each thread (in order to avoid the need
  ...
  /**
   * Returns a view of the specified metadata object as a {@linkplain Map map}. The map is backed by
   * the metadata object using Java reflection, so changes in the underlying metadata object are
   ...
   * defined in the {@link javax.swing.tree} package, it can be seen as a data structure independent
   * of Swing. It will not force class loading of Swing framework.
   *
  ...
  /**
   * Returns {@code true} if this metadata is modifiable. This method is not public because it uses
