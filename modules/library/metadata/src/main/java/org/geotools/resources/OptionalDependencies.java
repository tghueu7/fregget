...
public final class OptionalDependencies {
   ...
   * @param object The user object to be returned by the tree node. May or may not be the same than
   *     {@code name}.
   ...
   *
   * <p>This method should not be defined here, since this class is about optional dependencies. It
   * should be defined in {@link org.geotools.gui.swing.tree.Trees} instead. However we put it here
   * (for now) because it is used in some module that don't want to depend on widgets.
   */
  ...
  /**
   * Returns a graphical representation of the specified tree. This representation can be printed to
   * the {@linkplain System#out standard output stream} (for example) if it uses a monospaced font
