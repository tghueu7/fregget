...
public final class XArray {
  ...
  /**
   * Returns an array containing the same elements as the given {@code array} but with the specified
   * {@code length}, truncating or padding with {@code null} if necessary.
   *
   * <ul>
   *   <li>
   *       <p>If the given {@code length} is longer than the length of the given {@code array}, then
   *       the returned array will contain all the elements of {@code array} at index <var>i</var>
   *       &lt; {@code array.length}. Elements at index <var>i</var> &gt;= {@code array.length} are
   ...
   *       {@code array} is returned unchanged. <strong>No copy</strong> is performed. This behavior
   *       is what make this method different than {@link Arrays#copyOf}.
   *
   ...
   * @param index Index where the first space should be inserted. All {@code array} elements having
   *     an index equal to or higher than {@code index} will be moved forward.
   ...
   * {@code NaN} values are unordered, they may appears anywhere in the array; they will be ignored.
  ...
  /**
   * Returns {@code true} if the specified array contains the specified value. This method should be
   * used only for very small arrays, or for search to be performed only once, because it performs a
   * linear search. If more than one search need to be done on the same array, consider using {@link
   * java.util.HashSet} instead.
   *
   ...
   * equals} to an element in the second array. The element doesn't need to be at the same index in
   * both array.
   *
   * <p>This method should be used only for very small arrays since it may be very slow. If the
   * arrays are large or if an array will be involved in more than one search, consider using {@link
   * java.util.HashSet} instead.
   */
