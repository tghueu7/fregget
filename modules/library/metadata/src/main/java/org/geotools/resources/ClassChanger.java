...
public abstract class ClassChanger<S extends Comparable<S>, T extends Number> {
  ...
  /**
   * Registers a new converter. All registered {@link ClassChanger} will be taken in account by the
   * {@link #toNumber} method. The example below register a conversion for the {@link Date} class:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * The source class is a {@link Comparable} subclass that will be specified as input to {@link
   * #convert}. The target class is a {@link Number} subclass that will be returned as output by
   * {@link #convert}. If no suitable mapping is found, then {@code source} is returned.
   */
   ...
   * toNumber(new&nbsp;Date())</code> returns the {@link Date#getTime()} value of the specified date
   * object as a {@link Long}.
   *
   * @param object Object to convert (may be null).
   * @return {@code null} if {@code object} was null; otherwise {@code object} if the supplied
   *     object is already an instance of {@link Number}; otherwise a new number with the numerical
   *     value.
  ...
  /**
   * Casts the number to the specified class. The class must by one of {@link Byte}, {@link Short},
  ...
  /**
   * Returns the class of the widest type. Numbers {@code n1} and {@code n2} must be instance of any
   * of {@link Byte}, {@link Short}, {@link Integer}, {@link Long}, {@link Float} or {@link Double}
