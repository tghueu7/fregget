...
public final class IndexedResourceCompiler implements Comparator<Object> {
  ...
  /**
   * Prefix for argument count in resource key names. For example, a resource expecting one argument
   * may have a key name like "HELLO_$1".
   */
   ...
   * {@code FooKeys.class} file. If one is found, integer keys are loaded in order to reuse the same
   * values.
   *
  ...
  /**
   * Compares two resource keys. Object {@code o1} and {@code o2} are usually {@link String} objects
   * representing resource keys (for example, "{@code MISMATCHED_DIMENSION}"), but may also be
