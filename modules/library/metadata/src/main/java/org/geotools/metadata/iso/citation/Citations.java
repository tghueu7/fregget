...
public final class Citations {
  ...
  /**
   * The <A HREF="http://www.opengeospatial.org">Open Geospatial consortium</A> organisation. "Open
   * Geospatial consortium" is the new name for "OpenGIS consortium". An {@linkplain
  ...
  /**
   * The <A HREF="http://www.opengis.org">OpenGIS consortium</A> organisation. "OpenGIS consortium"
   * is the old name for "Open Geospatial consortium". {@linkplain Citation#getAlternateTitles
   * Alternate titles} for this citation are "OpenGIS" and "OGC" (according ISO 19115, alternate
   * titles often contain abreviations).
   *
   ...
   * {@linkplain Citation#getAlternateTitles alternate title} for this citation is "JAI" (according
   * ISO 19115, alternate titles often contain abreviations).
   *
  ...
  /**
   * The <A HREF="http://www.epsg.org">European Petroleum Survey Group</A> authority. An {@linkplain
   * Citation#getAlternateTitles alternate title} for this citation is "EPSG" (according ISO 19115,
   * alternate titles often contain abreviations). In addition, this citation contains the "EPSG"
   * {@linkplain Citation#getIdentifiers identifier} for the "Authority name" {@linkplain
   * Citation#getIdentifierTypes identifier type}.
   *
   * <p>This citation is used as an authority for {@linkplain
   * org.opengis.referencing.crs.CoordinateReferenceSystem coordinate reference system} identifiers.
   * When searching an {@linkplain org.opengis.referencing.crs.CRSAuthorityFactory CRS authority
   * factory} on EPSG data, Geotools compares the {@code "EPSG"} string against the {@linkplain
   * Citation#getIdentifiers identifiers} (or against the {@linkplain Citation#getTitle title} and
   * {@linkplain Citation#getAlternateTitles alternate titles} if there is no identifier) using the
   * {@link #identifierMatches(Citation,String) identifierMatches} method.
   *
   ...
   * citation contains the "AUTO" {@linkplain Citation#getIdentifiers identifier} for the "Authority
   * name" {@linkplain Citation#getIdentifierTypes identifier type}.
   *
   ...
   * org.opengis.referencing.crs.CoordinateReferenceSystem coordinate reference system} identifiers.
   * When searching an {@linkplain org.opengis.referencing.crs.CRSAuthorityFactory CRS authority
   * factory} on AUTO data, Geotools compares the {@code "AUTO"} string against the {@linkplain
   * Citation#getIdentifiers identifiers} (or against the {@linkplain Citation#getTitle title} and
   * {@linkplain Citation#getAlternateTitles alternate titles} if there is no identifier) using the
   * {@link #identifierMatches(Citation,String) identifierMatches} method.
   *
   ...
   * org.opengis.referencing.crs.CoordinateReferenceSystem coordinate reference system} identifiers.
   * When searching an {@linkplain org.opengis.referencing.crs.CRSAuthorityFactory CRS authority
   * factory} on AUTO2 data, Geotools compares the {@code "AUTO2"} string against the {@linkplain
   * Citation#getIdentifiers identifiers} (or against the {@linkplain Citation#getTitle title} and
   * {@linkplain Citation#getAlternateTitles alternate titles} if there is no identifier) using the
   * {@link #identifierMatches(Citation,String) identifierMatches} method.
   *
  ...
  /**
   * Returns a citation of the given name. If the given name matches a {@linkplain Citation#getTitle
   * title} or an {@linkplain Citation#getAlternateTitles alternate titles} of one of the
   * pre-defined constants ({@link #EPSG}, {@link #GEOTIFF}, <cite>etc.</cite>), then this constant
   * is returned. Otherwise, a new citation is created with the specified name as the title.
   *
  ...
  /**
   * Returns {@code true} if at least one {@linkplain Citation#getIdentifiers identifier} in {@code
   * c1} is equals to an identifier in {@code c2}. The comparaison is case-insensitive and ignores
   * leading and trailing spaces. The identifier ordering is not significant.
   *
   * <p>If (and <em>only</em> if) the citations do not contains any identifier, then this method
   * fallback on titles comparaison using the {@link #titleMatches(Citation,Citation) titleMatches}
   * method. This fallback exists for compatibility with client codes using citation {@linkplain
   * Citation#getTitle titles} without identifiers.
   *
   * @param c1 The first citation to compare.
   * @param c2 the second citation to compare.
   ...
   * citation matches the given string. The comparaison is case-insensitive and ignores leading and
   * trailing spaces. If (and <em>only</em> if) the citation do not contains any identifier, then
   * this method fallback on titles comparaison using the {@link #titleMatches(Citation, String)
