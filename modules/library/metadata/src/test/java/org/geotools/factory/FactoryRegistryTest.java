...
public final class FactoryRegistryTest {
   ...
   * registry properly.
   *
   * <p>Factories are specified in arguments as {@link Factory} objects in order to avoid the {@link
   * DummyClass} to be initialized before {@link Hints}. This is not a problem for normal execution,
   * but Maven seems to mess with class loaders.
   *
  ...
  private FactoryRegistry getRegistry(
    ...
    assertTrue(
        registry.setOrdering(DummyFactory.class, (DummyFactory) factory1, (DummyFactory) factory2));
    assertTrue(
        registry.setOrdering(DummyFactory.class, (DummyFactory) factory2, (DummyFactory) factory3));
    assertTrue(
        registry.setOrdering(DummyFactory.class, (DummyFactory) factory1, (DummyFactory) factory3));
  ...
  /**
   * Tests the {@link FactoryRegistry#getProvider} method. Note that the tested method do not create
   * any new factory. If no registered factory matching the hints is found, an exception is
   * expected. <br>
   * <br>
   * Three factories are initially registered: factory #1, #2 and #3.
   *
   * <p>Factory #1 has no dependency. Factory #2 uses factory #1. Factory #3 uses factory #2, which
   * implies an indirect dependency to factory #1.
   *
   ...
   * expected to throws an exception in {@link #testGetProvider}. But now, those cases are expected
   * to creates automatically new factory instances instead of throwing an exception.
   */
  @Test
  ...
  public void testLookupWithSameFactoryInTwoClassLoaders()
    ...
    // code below was throwing ClassCastException (before java 7) prior to adding isAssignableFrom()
    // check (line 862)
    for (int i = 0; i < 2; i++) {
