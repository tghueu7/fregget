...
public abstract class OnlineTestCase extends TestCase {
  ...
  /**
   * A static map which tracks which fixtures are offline. This prevents continually trying to run a
   * test when an external resource is offline.
   */
  ...
  protected final void setUp() throws Exception {
    ...
    skipOnFailure =
        Boolean.parseBoolean(fixture.getProperty(SKIP_ON_FAILURE_KEY, SKIP_ON_FAILURE_DEFAULT));
   ...
   *
   * <p>Subclasses should do all initialization / connection here. In the event of a connection not
   * being available, this method should throw an exception to abort the test case.
   *
  ...
  /**
   * Allows tests to create an offline fixture in cases where the user has not specified an explicit
   * fixture for the test.
   *
   * <p>Note, that this should method should on be implemented if the test case is created of
   * creating a fixture which relies soley on embedded or offline resources. It should not reference
   * any external or online resources as it prevents the user from running offline.
   */
