...
public class GridToEnvelopeMapper {
   ...
   *     contains entirely all pixels, i.e. the envelope's upper left corner must coincide with the
   *     upper left corner of the first pixel and the envelope's lower right corner must coincide
  ...
  /**
   * Returns whatever the grid range maps {@linkplain PixelInCell#CELL_CENTER pixel center} or
   * {@linkplain PixelInCell#CELL_CORNER pixel corner}. The former is OGC convention while the later
   * is Java2D/JAI convention. The default is cell center (OGC convention).
   *
   ...
   * {@linkplain #isAutomatic isAutomatic}({@linkplain #SWAP_XY})</code> returns {@code true} (which
   * is the default), then this method make the following assumptions:
   *
   * <ul>
   *   <li>
   ...
   *       underlying image is (<var>column</var>, <var>row</var>) (which is the case for a majority
   *       of images), then the envelope should probably have a (<var>longitude</var>,
   ...
   * direction reversed. If <code>{@linkplain #isAutomatic isAutomatic}({@linkplain #REVERSE_AXIS})
   * </code> returns {@code true} (which is the default), then this method make the following
   * assumptions:
   *
   * <p>
   *
   ...
   *       the <var>y</var> axis on output device (usually the screen). This axis is reversed again
   *       in order to match the bottom direction often used with such devices.
   * </ul>
   *
  ...
  /**
   * Set which (if any) axis in <cite>user</cite> space (not grid space) should have their direction
   * reversed. Invoking this method force <code>
  ...
  /**
   * Reverses a single axis in user space. Invoking this methods <var>n</var> time is equivalent to
   * creating a boolean {@code reverse} array of the appropriate length, setting {@code
