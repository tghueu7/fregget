...
public class Envelope2D extends Rectangle2D.Double implements BoundingBox, Envelope, Cloneable {
   ...
   * AxisDirection#EAST East}, {@linkplain AxisDirection#NORTH North}). Those parameter names simply
   * match the {@linkplain #x x} and {@linkplain #y y} fields. The actual axis orientations are
   * determined by the specified CRS. See the {@linkplain Envelope2D class javadoc} for details.
   *
   * @param crs The coordinate reference system, or {@code null}.
  ...
  /**
   * A coordinate position consisting of all the minimal ordinates for each dimension for all points
   * within the {@code Envelope}.
   *
   * @return The lower corner.
   * @todo Change the return type to {@link DirectPosition2D} when we will be allowed to compile for
   *     J2SE 1.5.
   */
  ...
  /**
   * A coordinate position consisting of all the maximal ordinates for each dimension for all points
   * within the {@code Envelope}.
   *
  ...
  /**
   * Returns the envelope length along the specified dimension. This length is equals to the maximum
   * ordinate minus the minimal ordinate.
   *
   * @param dimension The dimension to query.
   ...
   * bounds.
   *
   * <p>Note this method conflicts with {@link Rectangle2D#intersects(Rectangle2D)} so you may need
