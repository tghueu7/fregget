...
class MapTriangulationFactory {
  ...
  /**
   * Calculate the destination position for the quad vertex as source position using the difference
   * between nearest source and destination point pair.
   *
   * @param x the original coordinate.
   * @param vertices List of the MappedPosition.
   * @return MappedPosition from the original and new coordinate, so the difference between them is
   *     the same as for the nearest one MappedPosition. It is used for calculating destination
   *     quad.
   */
  protected MappedPosition generateCoordFromNearestOne(
    ...
    DirectPosition dst =
        new DirectPosition2D(nearestOne.getTarget().getCoordinateReferenceSystem(), dstX, dstY);
