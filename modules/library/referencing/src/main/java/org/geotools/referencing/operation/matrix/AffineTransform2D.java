...
public class AffineTransform2D extends AffineTransform implements Matrix {
  ...
  /**
   * Creates a new matrix initialized to the same value than the specified one. The specified matrix
   * size must be {@value #SIZE}&times;{@value #SIZE}.
   */
  ...
  public double getElement(final int row, final int column) {
    ...
    throw new IndexOutOfBoundsException(Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "row", row));
  }
   ...
   * only 0 values except the last column which is set to 1. This method throws an exception if the
   * specified value is not the expected one.
   */
