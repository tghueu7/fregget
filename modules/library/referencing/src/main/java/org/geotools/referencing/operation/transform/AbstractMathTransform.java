...
public abstract class AbstractMathTransform extends Formattable implements MathTransform {
  ...
  /**
   * Returns the parameter values for this math transform, or {@code null} if unknow. This method is
   * similar to {@link Operation#getParameterValues}, except that {@code MathTransform} returns
   * parameters in standard units (usually {@linkplain SI#METER meters} or {@linkplain
   * NonSI#DEGREE_ANGLE decimal degrees}). Since this method returns a copy of the parameter values,
   * any change to a value will have no effect on this math transform.
   *
  ...
  /**
   * Transforms a list of coordinate point ordinal values. The default implementation invokes {@link
   * #transform(double[],int,double[],int,int)} using a temporary array of doubles.
   */
  ...
  /**
   * Transform the specified shape. The default implementation computes quadratic curves using three
   * points for each shape segments.
   *
   ...
   * The new object is usually a {@link GeneralPath}, but may also be a {@link Line2D} or a {@link
   ...
   * exception. Subclasses that implement the {@link MathTransform2D} interface should override this
   * method. Other subclasses should override {@link #derivative(DirectPosition)} instead.
   *
  ...
  /**
   * Gets the derivative of this transform at a point. The default implementation ensure that {@code
   * point} has a valid dimension. Next, it try to delegate the work to an other method:
   *
   * <ul>
   *   <li>If the input dimension is 2, then this method delegates the work to {@link
   *       #derivative(Point2D)}.
   *   <li>If this object is an instance of {@link MathTransform1D}, then this method delegates the
   *       work to {@link MathTransform1D#derivative(double) derivative(double)}.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>If {@code applyOtherFirst} is {@code true}, then transforming a point <var>p</var> by the
   *       combined transform is equivalent to first transforming <var>p</var> by {@code other} and
   *       then transforming the result by the original transform {@code this}.
  ...
  /**
   * Compares the specified object with this math transform for equality. The default implementation
   * checks if {@code object} is an instance of the same class than {@code this} and use the same
   * parameter descriptor. Subclasses should override this method in order to compare internal
   * fields.
   *
   ...
   * returned by {@link #getParameterValues}. The parameter group name is used as the math transform
   * name.
   *
   ...
   *   <LI>Coordinates are read and writen in shrunk. For example (longitude,latitude,height) values
   *       for one coordinate are read together, and the transformed (x,y,z) values are written
   *       together only after.
   * </UL>
   *
   * <p>However, this method does not assumes that source and target dimension are the same (in the
  ...
  /**
   * Ensures that the specified longitude stay within &plusmn;&pi; radians. This method is typically
   * invoked after geographic coordinates are transformed. This method may add or substract some
   * amount of 2&pi; radians to <var>x</var>.
   *
  ...
  /**
   * Wraps the specified matrix in a Geotools implementation of {@link Matrix}. If {@code matrix} is
   * already an instance of {@code XMatrix}, then it is returned unchanged. Otherwise, all elements
  ...
  /**
   * Wraps the specified matrix in a Geotools implementation of {@link Matrix}. If {@code matrix} is
   * already an instance of {@code GeneralMatrix}, then it is returned unchanged. Otherwise, all
   * elements are copied in a new {@code GeneralMatrix} object.
   *
  ...
  /**
   * Inverts the specified matrix in place. If the matrix can't be inverted (for example because of
   * a {@link SingularMatrixException}), then the exception is wrapped into a {@link
  ...
  static Matrix invert(final Matrix matrix) throws NoninvertibleTransformException {
    try {
      ...
      NoninvertibleTransformException e =
          new NoninvertibleTransformException(Errors.format(ErrorKeys.NONINVERTIBLE_TRANSFORM));
  ...
  protected abstract class Inverse extends AbstractMathTransform implements Serializable {
    /**
     * Serial number for interoperability with different versions. This serial number is especilly
     * important for inner classes, since the default {@code serialVersionUID} computation will not
     * produce consistent results across implementations of different Java compiler. This is because
     * different compilers may generate different names for synthetic members used in the
     * implementation of inner classes. See:
     *
     ...
     * behavior should not be changed since some implementation assume that the inverse of {@code
     * this} is always {@code AbstractMathTransform.this}.
     */
    @Override
    ...
    /**
     * Tests whether this transform does not move any points. The default implementation delegate
     * this tests to the enclosing math transform.
     */
    @Override
    public boolean isIdentity() {
     ...
     * implementation tests if {@code object} in an instance of the same class than {@code this},
     ...
     * Format the inner part of a <A
     * HREF="http://geoapi.sourceforge.net/snapshot/javadoc/org/opengis/referencing/doc-files/WKT.html"><cite>Well
     * Known Text</cite> (WKT)</A> element. If this inverse math transform has any parameter values,
     * then this method format the WKT as in the {@linkplain AbstractMathTransform#formatWKT
     * super-class method}. Otherwise this method format the math transform as an <code>"INVERSE_MT"
     * </code> entity.
     *
     * @param formatter The formatter to use.
     * @return The WKT element name, which is <code>"PARAM_MT"</code> or <code>"INVERSE_MT"</code>
