...
public class Formattable {
  ...
  /**
   * The indentation value to give to {@link #toWKT(int)} method for formatting the complete object
   * on a single line.
   *
   * @since 2.6
   */
   ...
   * string similar than {@link #toWKT()}, except that no exception is thrown if the string contains
   * non-standard keywords. For example the <A
   ...
   * @throws UnformattableObjectException If this object can't be formatted as WKT. A formatting may
   *     fails because an object is too complex for the WKT format capability (for example an
   ...
   * <tt>strict</tt> parameter is used to control whether the resulting WKT is tested for validity.
   * When set to <code>false</code> the check is not performed. When <code>true</code> and the WKT
   * is found to be invalid a {@link UnformattableObjectException} is thrown.
   *
   * <p>
   *
   * @param indentation The amount of spaces to use in indentation for WKT formatting, or {@link
  ...
  /**
   * Returns a WKT for this object using the specified indentation and authority. If {@code strict}
   * is true, then an exception is thrown if the WKT contains invalid keywords.
   */
   ...
   * Formatter#append(Formattable)}. Element name and authority code must not be formatted here. For
   * example for a {@code GEOGCS} element ({@link
   * org.geotools.referencing.crs.DefaultGeographicCRS}), the formatter will invokes this method for
