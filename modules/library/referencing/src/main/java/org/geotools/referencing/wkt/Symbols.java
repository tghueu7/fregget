...
public class Symbols {
  ...
  /**
   * The character used for space. Usually <code>'&nbsp;'</code>, but could be a no-break space too
   * if unicode is allowed.
   */
  final char space = ' ';
  ...
  /**
   * Returns {@code true} if the specified WKT contains at least one {@code AXIS[...]} element. This
   * method tries to make a quick checks taking in account a minimal set of WKT syntax rules.
   *
   * @since 2.4
   */
   ...
   * element must be followed (ignoring spaces) by an opening bracket. If found, this method returns
   * the index of the opening bracket after the element.
   *
