...
public class AuthorityFactoryAdapter extends AbstractAuthorityFactory implements OptionalFactory {
   ...
   * #csFactory}, {@link #crsFactory} or {@link #operationFactory}. Will be created only when first
   * needed.
   *
  ...
  /**
   * Creates a wrapper around no factory. This constructor should never be used except by subclasses
   * overriding the <code>get</code><var>Foo</var><code>AuthorityFactory</code> methods.
   *
  ...
  /**
   * Creates a wrapper around the specified factories. The {@link #priority priority} field will be
   * set to the highest priority found in the specified factories. Subclasses should override the
   * {@link #getPriority() getPriority()} method if they want to set a higher or lower priority for
   * this instance.
   *
  ...
  /**
   * Creates a wrappers around the default factories for the specified authority. The factories are
   * fetched using {@link ReferencingFactoryFinder}.
   *
   ...
   * Geotools design problem. Maybe... this is not a trivial issue. So we are better to not document
   * that in public API for now.
   *
   ...
   * FallbackAuthorityFactory#create(boolean,int,Iterator)} implementations. If there is no backing
   * store, returns {@code true} in order to take in account only the backing stores that are
   * assigned. This behavior match the need of the above-cited implementations.
   */
  ...
  /**
   * Replaces (if needed) the specified coordinate system by a new one. To be overridden with {@code
  ...
  /**
   * Replaces (if needed) the specified datum by a new one. To be overridden with {@code protected}
   * access by {@link TransformedAuthorityFactory}.
   */
  ...
  /**
   * Returns one of the underlying factories as an instance of the GeoTools implementation. If there
   * is none of them, then returns {@code null} or throws an exception if {@code caller} is not
   * null.
   */
  ...
  public ImageDatum createImageDatum(final String code) throws FactoryException {
    return (ImageDatum)
        replace(getDatumAuthorityFactory(code).createImageDatum(toBackingFactoryCode(code)));
  }
  ...
  public VerticalDatum createVerticalDatum(final String code) throws FactoryException {
    return (VerticalDatum)
        replace(getDatumAuthorityFactory(code).createVerticalDatum(toBackingFactoryCode(code)));
  }
  ...
  public TemporalDatum createTemporalDatum(final String code) throws FactoryException {
    return (TemporalDatum)
        replace(getDatumAuthorityFactory(code).createTemporalDatum(toBackingFactoryCode(code)));
  }
  ...
  public GeodeticDatum createGeodeticDatum(final String code) throws FactoryException {
    return (GeodeticDatum)
        replace(getDatumAuthorityFactory(code).createGeodeticDatum(toBackingFactoryCode(code)));
  }
  ...
  public CylindricalCS createCylindricalCS(final String code) throws FactoryException {
    return (CylindricalCS)
        replace(getCSAuthorityFactory(code).createCylindricalCS(toBackingFactoryCode(code)));
  }
  ...
  public EllipsoidalCS createEllipsoidalCS(final String code) throws FactoryException {
    return (EllipsoidalCS)
        replace(getCSAuthorityFactory(code).createEllipsoidalCS(toBackingFactoryCode(code)));
  }
  ...
  /**
   * Returns an arbitrary {@linkplain CoordinateReferenceSystem coordinate reference system} from a
   * code.
   *
   * @throws FactoryException if the object creation failed.
   * @see #createGeographicCRS
  ...
  public CoordinateReferenceSystem createCoordinateReferenceSystem(final String code)
      throws FactoryException {
    return replace(
        getCRSAuthorityFactory(code).createCoordinateReferenceSystem(toBackingFactoryCode(code)));
  }
  ...
  public GeographicCRS createGeographicCRS(final String code) throws FactoryException {
    return (GeographicCRS)
        replace(getCRSAuthorityFactory(code).createGeographicCRS(toBackingFactoryCode(code)));
  }
  ...
  public GeocentricCRS createGeocentricCRS(final String code) throws FactoryException {
    return (GeocentricCRS)
        replace(getCRSAuthorityFactory(code).createGeocentricCRS(toBackingFactoryCode(code)));
  }
  ...
  public ProjectedCRS createProjectedCRS(final String code) throws FactoryException {
    return (ProjectedCRS)
        replace(getCRSAuthorityFactory(code).createProjectedCRS(toBackingFactoryCode(code)));
  }
  ...
  /**
   * A {@link IdentifiedObjectFinder} which tests {@linkplain AuthorityFactoryAdapter#replaceObject
  ...
  class Finder extends IdentifiedObjectFinder.Adapter {
    ...
    protected IdentifiedObject deriveEquivalent(
        final IdentifiedObject candidate, final IdentifiedObject model) throws FactoryException {
   ...
   * catch the {@link FactoryException} for methods that don't allow it. The protected method should
   * be used instead when this exception is allowed.
   */
   ...
   * returns one of the factory specified at construction time. Subclasses can override this method
   * in order to select a different factory implementation depending on the code value.
   *
  ...
  /**
   * Returns the coordinate system factory to use for the specified code. The default implementation
   * always returns the factory specified at construction time. Subclasses can override this method
   * in order to select a different factory implementation depending on the code value.
   *
   * @param code The authority code given to this class. Note that the code to be given to the
   *     returned factory {@linkplain #toBackingFactoryCode may be different}.
   ...
   * information from the coordinate operation authority factory, or will returns the default one if
   * no explicit factory were found.
   */
   ...
   * TransformedAuthorityFactory} that need low-level access to factories. Do not change this method
   * into a public one; we would need a better API before to do such thing.
   */
  ...
  /**
   * Returns the code to be given to the wrapped factories. This method is automatically invoked by
   * all {@code create} methods before to forward the code to the {@linkplain
