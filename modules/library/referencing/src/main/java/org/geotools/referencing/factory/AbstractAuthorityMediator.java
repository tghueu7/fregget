...
public abstract class AbstractAuthorityMediator extends AbstractAuthorityFactory
  ...
  /**
   * Cache to be used for referencing objects defined by this authority. Please note that this cache
   * may be shared!
   *
   ...
   * if you are only planning on working with 50 CoordianteReferenceSystems please keep in mind that
   * you will need larger cache size in order to prevent a bottleneck.
   */
  ObjectCache cache;
  ...
  /**
   * Pool to hold workers which will be used to construct referencing objects which are not present
   * in the cache.
   */
   ...
   *
   * <p>The worker used is configured to store answers in a separate <code>findCache</code>, rather
   * that disrupt the regular <code>cached</code>(which is focused on retaining codes requested by
   * the user application). This is because hundred of objects may be created during a scan while
