...
public class BufferedAuthorityFactory extends AbstractAuthorityFactory implements BufferedFactory {
  ...
  /**
   * The underlying authority factory. This field may be {@code null} if this object was created by
   * the {@linkplain #BufferedAuthorityFactory(AbstractAuthorityFactory,int) package protected
  ...
  /**
   * Constructs an instance wrapping the specified factory with a default number of entries to keep
   * by strong reference.
   *
  ...
  /**
   * Constructs an instance wrapping the specified factory. The {@code maxStrongReferences} argument
   * specify the maximum number of objects to keep by strong reference. If a greater amount of
   * objects are created, then the strong references for the oldest ones are replaced by weak
   * references.
   *
   * <p>This constructor is protected because subclasses must declare which of the {@link
   * DatumAuthorityFactory}, {@link CSAuthorityFactory}, {@link CRSAuthorityFactory} and {@link
   ...
   * store creation time.
   *
   * <p>The backing store is of course an important dependency. This method gives a chance to {@link
   * org.geotools.factory.FactoryRegistry} to compare the user-requested hints (especially {@link
   * Hints#FORCE_LONGITUDE_FIRST_AXIS_ORDER}) against the backing store hints, by following the
  ...
  /**
   * Returns the organization or party responsible for definition and maintenance of the underlying
   * database.
   */
  ...
  /**
   * An implementation of {@link IdentifiedObjectFinder} which delegates the work to the underlying
   * backing store and caches the result.
   *
   ...
   * while only one will be typically retained. We don't want to overload the cache with every false
  ...
  private final class Finder extends IdentifiedObjectFinder.Adapter {
     ...
     * specified object. The default implementation performs the same lookup than the backing store
  ...
  /**
   * Put an element in the pool. This method is invoked everytime a {@code createFoo(...)} method is
   * invoked, even if an object was already in the pool for the given code, for the following
