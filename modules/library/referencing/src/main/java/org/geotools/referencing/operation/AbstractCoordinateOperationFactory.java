...
public abstract class AbstractCoordinateOperationFactory extends ReferencingFactory
  ...
  protected static final ReferenceIdentifier AXIS_CHANGES =
      new NamedIdentifier(
          Citations.GEOTOOLS, Vocabulary.formatInternational(VocabularyKeys.AXIS_CHANGES));
   ...
   * org.geotools.referencing.datum.BursaWolfParameters Bursa Wolf parameters}. Only the changes in
   * ellipsoid axis-length are taken in account. Such ellipsoid shifts are approximative and may
   * have 1 kilometer error. This transformation is allowed only if the factory was created with
  ...
  protected static final ReferenceIdentifier ELLIPSOID_SHIFT =
      new NamedIdentifier(
          Citations.GEOTOOLS, Vocabulary.formatInternational(VocabularyKeys.ELLIPSOID_SHIFT));
  ...
  protected static final ReferenceIdentifier INVERSE_OPERATION =
      new NamedIdentifier(
          Citations.GEOTOOLS, Vocabulary.formatInternational(VocabularyKeys.INVERSE_OPERATION));
   ...
   * strictly reserved for factory subclasses that are wrapper around an other factory, like {@link
  ...
  /**
   * Returns the implementation hints for this factory. The returned map contains values for {@link
   * Hints#CRS_FACTORY CRS}, {@link Hints#CS_FACTORY CS}, {@link Hints#DATUM_FACTORY DATUM} and
   * {@link Hints#MATH_TRANSFORM_FACTORY MATH_TRANSFORM} {@code FACTORY} hints. Other values may be
   * provided as well, at implementation choice.
   */
  @Override
  ...
  /**
   * Returns the specified identifier in a map to be given to coordinate operation constructors. In
   * the special case where the {@code name} identifier is {@link #DATUM_SHIFT} or {@link
   ...
   * the {@code name} identifier is {@link #DATUM_SHIFT} or {@link #ELLIPSOID_SHIFT}, the operation
   * will be an instance of {@link Transformation} instead of the usual {@link Conversion}.
   *
   ...
   * included as part of the second operation instead of creating an {@link ConcatenatedOperation}.
  ...
  protected CoordinateOperation concatenate(
      final CoordinateOperation step1, final CoordinateOperation step2) throws FactoryException {
   ...
   * operation instead of creating an {@link ConcatenatedOperation}. If a concatenated operation is
   * created, it will get an automatically generated name.
   *
  ...
  /**
   * Returns the dimension of the specified coordinate system, or {@code 0} if the coordinate system
   * is null.
   */
  ...
  private static final class TemporaryIdentifier extends NamedIdentifier {
    ...
    public TemporaryIdentifier(final ReferenceIdentifier parent) {
      this(
          parent,
          ((parent instanceof TemporaryIdentifier) ? ((TemporaryIdentifier) parent).count : 0) + 1);
    }
  ...
  /**
   * Returns the name of the GeoAPI interface implemented by the specified object. In addition, the
   * name may be added between brackets.
   */
  ...
  /**
   * Returns an error message for "No path found from sourceCRS to targetCRS". This is used for the
   * construction of {@link OperationNotFoundException}.
   *
