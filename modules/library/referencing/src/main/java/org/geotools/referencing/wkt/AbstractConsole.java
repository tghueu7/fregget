...
public abstract class AbstractConsole implements Runnable {
  ...
  /**
   * Creates a new console instance using {@linkplain System#in standard input stream}, {@linkplain
   * System#out standard output stream}, {@linkplain System#err error output stream} and the system
   ...
   * contains itself other definitions specified in some previous calls to this method. This method
   * do nothing if the {@linkplain #parser} is not an instance of {@link Preprocessor}.
   *
   ...
   * {@code name = wkt} (without the {@code SET} keyword). The result is the same than invoking the
   * {@code SET} instruction for each line in the specified stream. This method is used for loading
   * predefined objects like the database used by {@link
   ...
   * table is inferred from the values given to the {@link #addDefinition} method. This method print
   * nothing if the {@linkplain #parser} is not an instance of {@link Preprocessor}.
   *
  ...
  /**
   * Read the next line from the specified input stream. Empty lines and comment lines are skipped.
   * If there is no more line to read, then this method returns {@code null}.
   *
  ...
  /**
   * Process instructions from the {@linkplain #in input stream} specified at construction time. All
   * lines are read until the end of stream ({@code [Ctrl-Z]} for input from the keyboard), or until
   * {@link #stop()} is invoked. Non-empty and non-comment lines are given to the {@link #execute}
   * method. Errors are catched and printed to the {@linkplain #err error stream}.
   */
  public void run() {
    try {
      while (!stop) {
   ...
   * message includes the line number, and the column where the failure occured in the exception is
   * an instance of {@link ParseException}.
   *
