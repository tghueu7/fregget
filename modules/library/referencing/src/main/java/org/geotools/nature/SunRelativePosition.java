...
public class SunRelativePosition {
  ...
  /**
   * {@linkplain #getElevation Elevation angle} of nautical twilight, in degrees. Nautical twilight
   * is the time of morning or evening when the sun is 12° below the horizon (solar elevation angle
   * of -12°).
   */
  public static final double NAUTICAL_TWILIGHT = -12;

  /**
   * {@linkplain #getElevation Elevation angle} of civil twilight, in degrees. Civil twilight is the
   * time of morning or evening when the sun is 6° below the horizon (solar elevation angle of -6°).
   */
  ...
  /**
   * Heure à laquelle le soleil est au plus haut dans la journée en millisecondes écoulées depuis le
   * 1er janvier 1970.
   */
  ...
  /**
   * Calculate the right ascension of the sun. Similar to the angular system used to define latitude
   * and longitude on Earth's surface, right ascension is roughly analogous to longitude, and
   * defines an angular offset from the meridian of the vernal equinox.
   *
  ...
  /**
   * Calculate the declination of the sun. Declination is analogous to latitude on Earth's surface,
   * and measures an angular displacement north or south from the projection of Earth's equator on
   * the celestial sphere to the location of a celestial body.
   *
   * @param t number of Julian centuries since J2000.
   ...
   * year. Earth's elliptical orbit and Kepler's law of equal areas in equal times are the culprits
   * behind this phenomenon. See the <A
   * HREF="http://www.analemma.com/Pages/framesPage.html">Analemma page</A>. Below is a plot of the
   * equation of time versus the day of the year.
   *
   ...
   * pressure, humidity and other variables. Therefore the calculation is approximate. Errors can be
   * expected to increase the further away you are from the equator, because the sun rises and sets
   * at a very shallow angle. Small variations in the atmosphere can have a larger effect.
   *
  ...
  /**
   * Set the date and time when to compute the {@linkplain #getElevation elevation} and {@linkplain
   * #getAzimuth azimuth}.
   *
  ...
  /**
   * Set the sun's {@linkplain #getElevation elevation angle} at twilight, in degrees. Common values
   * are defined for the {@linkplain #ASTRONOMICAL_TWILIGHT astronomical twilight} (-18°),
   * {@linkplain #NAUTICAL_TWILIGHT nautical twilight} (-12°) and {@linkplain #CIVIL_TWILIGHT civil
   * twilight} (-6°). The {@linkplain #getElevation elevation} and {@linkplain #getAzimuth azimuth}
   * are set to {@linkplain Double#NaN NaN} when the sun elevation is below the twilight value (i.e.
   * during night). The default value is {@link #CIVIL_TWILIGHT}.
   *
   * @param twilight The new sun elevation at twilight, or {@link Double#NaN} if no twilight value
   *     should be taken in account.
   * @throws IllegalArgumentException if the twilight value is illegal.
   ...
   * équivalente à {@link #getNoonTime} mais inclue le jour de la date qui avait été spécifiée à la
   * méthode {@link #compute}.
   */
   ...
   *
   * où <var>date</var> est un argument optionel spécifiant la date et l'heure. Si cet argument est
   * omis, la date et heure actuelles seront utilisées.
   */
