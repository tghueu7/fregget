...
public abstract class AbstractAuthorityFactory extends ReferencingFactory
   ...
   * {@linkplain #getAuthorityCodes authority codes}, returns {@code true}. Example of such wrappers
   * are {@link BufferedAuthorityFactory} and {@link TransformedAuthorityFactory}.
   *
   * <p>This method should perform a cheap test. It is for {@link FallbackAuthorityFactory} internal
   * use only and should not be public.
   */
  ...
  /**
   * Returns a description of the underlying backing store, or {@code null} if unknown. This is for
   * example the database software used for storing the data. The default implementation returns
   * always {@code null}.
   *
   ...
   * override this method if they are capable to automatically detect the object type from its code.
   *
   ...
   * code. If the coordinate reference system type is know at compile time, it is recommended to
   ...
   * <code>&nbsp;createCoordinateReferenceSystem(code)&nbsp;</code> if the caller know he is asking
   * for a {@linkplain GeographicCRS geographic coordinate reference system}).
   *
   * @param code Value allocated by authority.
  ...
  /**
   * Creates an operation from coordinate reference system codes. The default implementation returns
   * an {@linkplain Collections#EMPTY_SET empty set}. We do not delegate to some kind of {@linkplain
   * CoordinateOperationFactory#createOperation(CoordinateReferenceSystem,
   * CoordinateReferenceSystem) coordinate operation factory method} because the usual contract for
   * this method is to extract the information from an authority database like <A
   ...
   * fully {@linkplain IdentifiedObject identified object} from an incomplete one, for example from
   * an object without identifier or "{@code AUTHORITY[...]}" element in <cite>Well Known
   ...
   *     unknown, use {@code IdentifiedObject.class}. A more accurate type may help to speed up the
   *     search, since it reduces the amount of tables to scan in some implementations like the
   *     factories backed by EPSG database.
  ...
  /**
   * Releases resources immediately instead of waiting for the garbage collector. Once a factory has
   * been disposed, further {@code create(...)} invocations may throw a {@link FactoryException}.
   * Disposing a previously-disposed factory, however, has no effect.
   *
   ...
   * and the specified code start with the "EPSG:" prefix, then the prefix is removed. Otherwise,
   * the string is returned unchanged (except for leading and trailing spaces).
   *
