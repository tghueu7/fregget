...
public final class CRSUtilities {
  ...
  /**
   * Returns the dimension within the coordinate system of the first occurrence of an axis colinear
   * with the specified axis. If an axis with the same {@linkplain CoordinateSystemAxis#getDirection
   * direction} or an {@linkplain AxisDirection#inverse opposite} direction than {@code axis} ocurs
   * in the coordinate system, then the dimension of the first such occurrence is returned. That is,
   * the a value <i>k</i> such that:
   *
   * <blockquote>
   *
   * <pre>
   ...
   *
   * <p>For example, {@code dimensionColinearWith(CoordinateSystemAxis.TIME)} returns the dimension
   * number of time axis.
   *
  ...
  /**
   * Returns the unit used for all axis in the specified coordinate system. If not all axis uses the
   * same unit, then this method returns {@code null}. This convenience method is often used for
  ...
  /**
   * Returns a two-dimensional coordinate reference system representing the two first dimensions of
   * the specified coordinate reference system. If {@code crs} is already a two-dimensional CRS,
   * then it is returned unchanged. Otherwise, if it is a {@link CompoundCRS}, then the head
   * coordinate reference system is examined.
   *
   * @param crs The coordinate system, or {@code null}.
   * @return A two-dimensional coordinate reference system that represents the two first dimensions
   *     of {@code crs}, or {@code null} if {@code crs} was {@code null}.
      ...
      while (crs.getCoordinateSystem().getDimension() != 2) {
        ...
        if (c == null) {
          throw new TransformException(
              Errors.format(ErrorKeys.CANT_REDUCE_TO_TWO_DIMENSIONS_$1, crs.getName()));
        }
        crs = c.get(0);
      }
    }
  ...
  /**
   * Changes the dimension declared in the name. For example if {@code name} is "WGS 84 (geographic
   * 3D)", {@code search} is "3D" and {@code replace} is "2D", then this method returns "WGS 84
   * (geographic 2D)". If the string to search is not found, then it is concatenated to the name.
   *
  ...
  /**
   * Returns the ellipsoid used by the specified coordinate reference system, providing that the two
   * first dimensions use an instance of {@link GeographicCRS}. Otherwise (i.e. if the two first
   * dimensions are not geographic), returns {@code null}.
   *
  ...
  /**
   * Derives a geographic CRS with (<var>longitude</var>, <var>latitude</var>) axis order in decimal
   * degrees, relative to Greenwich. If no such CRS can be obtained of created, returns {@link
