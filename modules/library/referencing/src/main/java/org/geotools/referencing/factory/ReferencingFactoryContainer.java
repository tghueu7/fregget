...
public class ReferencingFactoryContainer extends ReferencingFactory {
  ...
  /**
   * The {@linkplain Datum datum} factory. If null, then a default factory will be created only when
   * first needed.
   */
  ...
  /**
   * The {@linkplain CoordinateReferenceSystem coordinate reference system} factory. If null, then a
   * default factory will be created only when first needed.
   */
  private CRSFactory crsFactory;

  /**
   * The {@linkplain MathTransform math transform} factory. If null, then a default factory will be
  ...
  public static ReferencingFactoryContainer instance(final Hints hints) {
    ...
    synchronized (ReferencingFactoryFinder.class) {
      if (cache == null) {
        cache =
            new FactoryCreator(Arrays.asList(new Class<?>[] {ReferencingFactoryContainer.class}));
  ...
  /**
   * Forces the initialisation of all factories. Implementation note: we try to create the factories
   * in typical dependency order (CRS all because it has the greatest chances to depends on other
   * factories).
   */
  ...
  /**
   * Returns the hints to be used for lazy creation of <em>default</em> factories in various {@code
   * getFoo} methods. This is different from {@link #getImplementationHints} because the later may
   * returns non-default factories.
   */
  private Hints hints() {
   ...
   * DefaultMathTransformFactory Geotools implementation}, then this method just delegates the call
   * to it. Otherwise this method scans all operations registered in the math transform factory
  ...
  /**
   * Converts a 2D&nbsp;+&nbsp;1D compound CRS into a 3D CRS, if possible. More specifically, if the
   * specified {@linkplain CompoundCRS compound CRS} is made of a {@linkplain GeographicCRS
   ...
   * above the ellipsoid}, then this method converts the compound CRS in a single 3D CRS. Otherwise,
   * the {@code crs} argument is returned unchanged.
   *
  ...
  /**
   * Implementation of {@link #toGeodetic3D(CompoundCRS)} invoked after the horizontal and vertical
   * parts have been identified. This method may invokes itself recursively if the horizontal CRS is
   * a derived one.
   *
   ...
   * @param xyFirst {@code true} if the horizontal component appears before the vertical component,
   *     or {@code false} for the converse.
