...
public class MeteosatSG extends MapProjection {
   ...
   * responsible for the image "spread" in the NS and EW directions. They are calculated as follows:
   * CFAC = LFAC = 2^16 / delta with delta = 83.84333 micro Radian (size of one VIS/IR MSG pixel)
   ...
   * and stores the result in {@code ptDst} (pixel coordinates).
   *
  ...
  /**
   * Transforms the specified (<var>column</var>,<var>row</var>) coordinates (units in "normalized"
   * pixels) and stores the result in {@code ptDst}.
   */
   ...
   * provider} for an {@linkplain org.geotools.referencing.operation.projection.MeteosatSG Meteosat
   * Second Generation image} projection.
   *
   * @since 14.0
   * @version $Id$
   * @author Maciej Filocha (ICM)
