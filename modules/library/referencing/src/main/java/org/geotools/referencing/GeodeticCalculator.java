...
public class GeodeticCalculator {
  ...
  /**
   * The coordinate reference system for all methods working on {@link Position} objects. If {@code
   * null}, will be created the first time {@link #getCoordinateReferenceSystem} is invoked.
   */
  ...
  /**
   * The (<var>latitude</var>, <var>longitude</var>) coordinate of the destination point <strong>in
   * degrees</strong>. This point is set by {@link #setDestinationGeographicPoint}.
   */
  ...
  /**
   * Tell if the destination point is valid. {@code false} if {@link #long2} and {@link #lat2} need
   * to be computed.
   */
  private boolean destinationValid;

  /**
   * Tell if the azimuth and the distance are valids. {@code false} if {@link #distance} and {@link
   * #azimuth} need to be computed.
   */
  ...
  /**
   * Constructs a new geodetic calculator associated with the specified ellipsoid. All calculations
   * done by the new instance are referenced to this ellipsoid.
   *
  ...
  /**
   * Returns {@code true} if the specified axis is oriented toward the specified direction and uses
  ...
  /**
   * Checks the latidude validity. The argument {@code latidude} should be greater than or equal to
   * -90 degrees and less than or equal to +90 degrees.
   *
  ...
  private static void checkLongitude(final double longitude) throws IllegalArgumentException {
    if (!(Math.abs(longitude) <= Double.MAX_VALUE)) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "longitude", new Longitude(longitude)));
    }
  }
  ...
  private static void checkAzimuth(final double azimuth) throws IllegalArgumentException {
    if (!(Math.abs(azimuth) <= Double.MAX_VALUE)) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "azimuth", new Longitude(azimuth)));
    }
  }
  ...
  /**
   * Set the starting point in geographic coordinates. The <var>x</var> and <var>y</var> coordinates
   ...
   *
   * @return The destination point. The <var>x</var> and <var>y</var> coordinates are the longitude
   *     and latitude in decimal degrees, respectively.
  ...
  /**
   * Returns the destination position in user coordinates, which doesn't need to be geographic. The
   * coordinate reference system is the one specified to the {@linkplain
   * #GeodeticCalculator(CoordinateReferenceSystem) constructor}.
   *
   ...
   * @todo Current implementation will provides an innacurate value for antipodal points. For now a
   *     warning is logged in such case. In a future version (if we have volunter time) we should
   *     provides a solution (search Internet for "<cite>azimuth antipodal points</cite>").
   */
   ...
   * the last call to <code>{@linkplain #setDirection(double,double) setDirection}(azimuth,distance)
   * </code>, <strong>except</strong> if <code>
   * {@linkplain #setDestinationGeographicPoint(double,double)
   * setDestinationGeographicPoint}(...)</code> has been invoked after. In this later case, the
   * distance will be computed from the {@linkplain #getStartingGeographicPoint starting point} to
   * the destination point.
   *
   * @return The orthodromic distance, in the same units as the {@linkplain #getEllipsoid ellipsoid}
   *     axis.
   * @throws IllegalStateException if the destination point has not been set.
   */
  ...
  /**
   * Computes the azimuth and orthodromic distance from the {@linkplain #getStartingGeographicPoint
   * starting point} and the {@linkplain #getDestinationGeographicPoint destination point}.
   *
   * @throws IllegalStateException if the destination point has not been set.
   * @see #getAzimuth
   * @see #getOrthodromicDistance
   ...
   * geodetic curve. The geodetic curve is computed from the {@linkplain #getStartingGeographicPoint
   * starting point} to the {@linkplain #getDestinationGeographicPoint destination point}.
   *
  ...
  public List<Point2D> getGeodeticPath(int numPoints) {
    ...
    for (int i = 1; i <= numPoints + 1; i++) {
      GeodesicData g =
          line.Position(
              i * delta, GeodesicMask.LATITUDE | GeodesicMask.LONGITUDE | GeodesicMask.LONG_UNROLL);
  ...
  /**
   * Calculates the loxodromic curve between two points in the referenced ellipsoid. The loxodromic
   * curve between two points is a path that links together the two points with a constant azimuth.
   * The azimuth from every points of the loxodromic curve and the second point is constant.
   *
  ...
  private Shape getLoxodromicCurve() {
    ...
    /**
     * *********************************************************************************** * THE
     * FOLLOWING IS CHECKED FOR COMPILER ERROR, BUT EXCLUDED FROM THE .class FILE. ** * THIS CODE IS
     * WRONG: LOXODROMIC CURVES ARE STRAIGHT LINES IN MERCATOR PROJECTION, ** * NOT IT PLAIN
     * (longitude,latitude) SPACE. FURTHERMORE, THE "OUT OF BOUNDS" CHECK ** * IS UNFINISHED: WHEN
     * THE PATH CROSS THE 180° LONGITUDE, A +360° ADDITION NEED TO ** * BE PERFORMED ON ONE OF THE
     * SOURCE OR TARGET POINT BEFORE TO COMPUTE THE LINEAR ** * INTERPOLATION (OTHERWISE, THE SLOPE
     * VALUE IS WRONG). FORMULAS FOR COMPUTING MID- ** * POINT ON A LOXODROMIC CURVE ARE AVAILABLE
     * THERE: ** * ** * http://mathforum.org/discuss/sci.math/a/t/180912 ** * ** * LatM =
     * (Lat0+Lat1)/2 ** * ** * (Lon1-Lon0)log(f(LatM)) + Lon0 log(f(Lat1)) - Lon1 log(f(Lat0)) ** *
     * LonM = --------------------------------------------------------------- ** *
     * log(f(Lat1)/f(Lat0)) ** * ** * where log(f(x)) == log(sec(x)+tan(x)) is the inverse
