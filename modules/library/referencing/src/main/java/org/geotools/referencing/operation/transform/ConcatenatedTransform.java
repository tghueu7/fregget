...
public class ConcatenatedTransform extends AbstractMathTransform implements Serializable {
   ...
   * avoid too high values since allocating and initializing the array elements to zero have a cost.
   */
  ...
  protected ConcatenatedTransform(final MathTransform transform1, final MathTransform transform2) {
    ...
    if (!isValid()) {
      throw new IllegalArgumentException(
          Errors.format(
              ErrorKeys.CANT_CONCATENATE_TRANSFORMS_$2, getName(transform1), getName(transform2)));
    }
  }
  ...
  /**
   * Tests if one math transform is the inverse of the other. This implementation can't detect every
   * case. It just detect the case when {@code tr2} is an instance of {@link
   * AbstractMathTransform.Inverse}.
   *
   * @todo We could make this test more general (just compare with tr2.inverse(), no matter if it is
   *     an instance of AbstractMathTransform.Inverse or not, and catch the exception if one is
  ...
  /**
   * Constructs a concatenated transform. This factory method checks for step transforms dimension.
   * The returned transform will implements {@link MathTransform2D} if source and target dimensions
   * are equal to 2. Likewise, it will implements {@link MathTransform1D} if source and target
   * dimensions are equal to 1. {@link MathTransform} implementations are available in two version:
   * direct and non-direct. The "non-direct" version use an intermediate buffer when performing
   * transformations; they are slower and consume more memory. They are used only as a fallback when
   * a "direct" version can't be created.
   *
   * @param tr1 The first math transform.
   * @param tr2 The second math transform.
   * @return The concatenated transform.
   * @todo We could add one more optimisation: if one transform is a matrix and the other transform
   *     is a PassThroughTransform, and if the matrix as 0 elements for all rows matching the
   *     PassThrough sub-transform, then we can get ride of the whole PassThroughTransform object.
   */
  public static MathTransform create(MathTransform tr1, MathTransform tr2) {
    ...
    if (dim1 != dim2) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.CANT_CONCATENATE_TRANSFORMS_$2, getName(tr1), getName(tr2))
              + ' '
  ...
  /**
   * Tries to returns an optimized concatenation, for example by merging to affine transforms into a
   * single one. If no optimized cases has been found, returns {@code null}. In the later case, the
   * caller will need to create a more heavy {@link ConcatenatedTransform} instance.
   */
   ...
   * {@link #transform2} and finally the result is casted to {@code float} primitive type and stored
   * in the destination array. The use of {@code double} primitive type for intermediate results is
   * necesssary for reducing rounding errors.
   */
  @Override
  public void transform(
  ...
  /**
   * Tests whether this transform does not move any points. Default implementation check if the two
