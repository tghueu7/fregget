...
public class AbstractIdentifiedObject extends Formattable
  ...
  /**
   * An empty array of identifiers. This is usefull for fetching identifiers as an array, using the
   * following idiom:
   *
   * <blockquote>
   *
  ...
  /**
   * {@link #IDENTIFIER_COMPARATOR} implementation as a named class (rather than anonymous) for more
   * predictable serialization.
   */
   ...
   * case-insensitive, and leading and trailing spaces are ignored. The map given in argument shall
   * contains at least a {@code "name"} property. Other properties listed in the table below are
   * optional.
   *
   * <p>
   *
   ...
   * {@linkplain java.util.Locale#FRENCH French} and the {@code "remarks_fr_CA"} property stands for
   * remarks in {@linkplain java.util.Locale#CANADA_FRENCH French Canadian}.
   *
   * <p>Note that the {@code "authority"} and {@code "version"} properties are ignored if the {@code
   * "name"} property is already a {@link Citation} object instead of a {@link String}.
   *
  ...
  /**
   * Constructs an object from a set of properties and copy unrecognized properties in the specified
   * map. The {@code properties} argument is treated as in the {@linkplain
   * AbstractIdentifiedObject#AbstractIdentifiedObject(Map) one argument constructor}. All
   * properties unknow to this {@code AbstractIdentifiedObject} constructor are copied in the {@code
   * subProperties} map, after their key has been normalized (usually lower case, leading and
   ...
   * The returned map contains key such as {@link #NAME_KEY NAME_KEY}, and values from methods such
   * as {@link #getName}.
   *
  ...
  /**
   * Returns the properties to be given to an identified object derived from the specified one. This
   * method is typically used for creating a new CRS identical to an existing one except for axis
   * units. This method returns the same properties than the supplied argument (as of <code>
   ...
   * #getIdentifiers identifiers} in their iteration order. It returns the first identifier with an
   * {@linkplain ReferenceIdentifier#getAuthority authority} citation {@linkplain
   ...
   *       is found, then this method returns the {@linkplain ReferenceIdentifier#getCode identifier
   *       code} of this object.
   *   <li>
   *       <p>Otherwise, if the alias implements the {@link GenericName} interface, then this method
   *       compares the {@linkplain GenericName#getScope name scope} against the specified citation
   *       using the {@linkplain Citations#identifierMatches(Citation,String) identifierMatches}
   *       method. If a matching is found, then this method returns the {@linkplain
   *       GenericName#asLocalName local name} of this object.
   * </ul>
   *
  ...
  /**
   * Returns {@code true} if either the {@linkplain #getName() primary name} or at least one
   * {@linkplain #getAlias alias} matches the specified string. This method performs the same check
   * than the {@linkplain #nameMatches(String) non-static method} on arbitrary object implementing
   * the GeoAPI interface.
   *
   ...
   *
   * @param object The object to check.
   * @param alias The list of alias in {@code object} (may be {@code null}). This method will never
   *     modify this list. Consequently, it may be a direct reference to an internal array.
   * @param name The name.
   * @return {@code true} if the primary name of at least one alias matches the specified {@code
   *     name}.
   */
   ...
   * needed for computing transformations. In other words, {@code sourceCS.equals(targetCS, false)}
   * returns {@code true} only if the transformation from {@code sourceCS} to {@code targetCS} is
   * the identity transform, no matter what {@link #getName()} saids.
   *
   * <p>Some subclasses (especially {@link org.geotools.referencing.datum.AbstractDatum} and {@link
   * org.geotools.parameter.AbstractParameterDescriptor}) will test for the {@linkplain #getName()
   * name}, since objects with different name have completly different meaning. For example nothing
   * differentiate the {@code "semi_major"} and {@code "semi_minor"} parameters except the name. The
   * name comparaison may be loose however, i.e. we may accept a name matching an alias.
   *
  ...
  protected static boolean equals(
    ...
    return equals(
        (AbstractIdentifiedObject) object1, (AbstractIdentifiedObject) object2, compareMetadata);
  }
   ...
   * comparaison take order in account, which make it more appropriate for {@link java.util.List} or
   * {@link LinkedHashSet} comparaisons. This convenience method is provided for implementation of
   * {@code equals} method in subclasses.
   *
  ...
  /**
   * Compares two objects for order. Any object may be null. This method is used for implementation
   * of {@link #NAME_COMPARATOR} and its friends.
   */
   ...
   * #getIdentifiers identifiers} and {@linkplain #getRemarks remarks} are not taken in account. In
   * other words, two identified objects will return the same hash value if they are equal in the
   * sense of <code>{@linkplain
   * #equals(AbstractIdentifiedObject,boolean) equals}(AbstractIdentifiedObject,
   * <strong>false</strong>)</code>.
   *
   * @return The hash code value. This value doesn't need to be the same in past or future versions
   *     of this class.
   */
  @Override
  public int hashCode() {
  ...
  /**
   * Returns the specified array as an immutable set, or {@code null} if the array is empty or null.
   * This is a convenience method for sub-classes constructors.
   *
  ...
  protected static void ensureNonNull(final String name, final Object[] array, final int index)
    ...
    if (array[index] == null) {
      throw new InvalidParameterValueException(
          Errors.format(ErrorKeys.NULL_ARGUMENT_$1, name + '[' + index + ']'), name, array);
    }
  }

  /**
   * Makes sure that the specified unit is a temporal one. This is a convenience method for subclass
   * constructors.
   *
  ...
  /**
   * Makes sure that the specified unit is an angular one. This is a convenience method for subclass
   * constructors.
   *
   * @param unit Unit to check.
