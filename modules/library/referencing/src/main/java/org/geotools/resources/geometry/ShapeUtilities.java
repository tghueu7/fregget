...
public final class ShapeUtilities {
  ...
  /**
   * Constante pour les calculs de paraboles. Cette constante indique que l'axe des <var>x</var> de
   * la parabole doit être parallèle à la droite joignant les points P0 et P2.
   */
  public static final int PARALLEL = 0;

  /**
   * Constante pour les calculs de paraboles. Cette constante indique que l'axe des <var>x</var> de
   * la parabole doit être horizontale, quelle que soit la pente de la droite joignant les points P0
   * et P2.
   */
  ...
  /**
   * Retourne le point d'intersection de deux segments de droites. Cette méthode ne prolonge pas les
   * segments de droites à l'infini. Si les deux segments ne s'interceptent pas (soit par ce qu'ils
   * sont parallèles, ou soit parce qu'ils ne se prolongent pas assez loin pour se toucher), alors
   ...
   *   <li>La distance entre les points {@code result} et {@code point} est la plus courte distance
   *       possible pour les points qui respectent la condition précédente. Cette distance peut être
   *       calculée par {@code point.distance(result)}.
   * </ul>
   *
   ...
   * du point {@code (x,y)} spécifié. Appellons {@code result} le point retourné par cette méthode.
   * Il est garanti que {@code result} répond aux conditions suivantes (aux erreurs
   ...
   *   <li>La distance entre les points {@code result} et {@code (x,y)} est la plus courte distance
   *       possible pour les points qui respectent la condition précédente. Cette distance peut être
   *       calculée par <code>new&nbsp;Point2D.Double(x,y).distance(result)</code>.
   * </ul>
   *
   ...
   * cette méthode. Si {@code result} est non-nul, alors il est garanti qu'il répond aux conditions
   ...
   * </ul>
   *
   * Si aucun point ne peut répondre à ces conditions, alors cette méthode retourne {@code null}. Si
   * deux points peuvent répondre à ces conditions, alors par convention cette méthode retourne le
   * point le plus près du point {@code line.getP1()}.
   *
  ...
  /**
   * Retourne le point sur le segment de droite {@code (x1,y1)-(x2,y2)} qui se trouve à la distance
   * {@code distance} spécifiée du point {@code point}. Appellons {@code result} le point retourné
   * par cette méthode. Si {@code result} est non-nul, alors il est garantit qu'il répond aux
   ...
   * Si aucun point ne peut répondre à ces conditions, alors cette méthode retourne {@code null}. Si
   * deux points peuvent répondre à ces conditions, alors par convention cette méthode retourne le
   * point le plus près du point {@code (x1,y1)}.
   *
   * @param x1 <var>x</var> value of the first point on the line.
   * @param y1 <var>y</var> value of the first point on the line.
   * @param x2 <var>x</var> value of the last point on the line.
   ...
   * nécessairement horizontal. La direction de cet axe des <var>x</var> dépend du paramètre {@code
   * orientation} spécifié à cette méthode. La valeur {@link #HORIZONTAL} signifie que l'axe des
   * <var>x</var> de la parabole sera toujours horizontal. La courbe quadratique produite
   * ressemblera alors à une parabole classique telle qu'on en voit dans les ouvrages de
   ...
   *     point soit situé entre {@code P0} et {@code P1}. Toutefois, il ne doit pas être colinéaire
   *     avec {@code P0} et {@code P1}.
   ...
   *     {@link #HORIZONTAL}.
   * @return Une courbe quadratique passant par les trois points spécifiés. La courbe commencera au
   *     point {@code P0} et se terminera au point {@code P2}. Si deux points ont des coordonnées
   *     presque identiques, ou si les trois points sont colinéaires, alors cette méthode retourne
   *     {@code null}.
   * @throws IllegalArgumentException si l'argument {@code orientation} n'est pas une des constantes
   *     valides.
   */
   ...
   * ressemblera alors à une parabole classique telle qu'on en voit dans les ouvrages de
   * mathématiques élémentaires. La valeur {@link #PARALLEL} indique plutôt que l'axe des
   * <var>x</var> de la parabole doit être parallèle à la droite joignant les points {@code (x0,y0)}
   * et {@code (x2,y2)}. Ce dernier type produira le même résultat que {@link #HORIZONTAL} si {@code
   * y0==y2}.
   *
   ...
   *     {@link #HORIZONTAL}.
   * @return Une courbe quadratique passant par les trois points spécifiés. La courbe commencera au
   *     point {@code (x0,y0)} et se terminera au point {@code (x2,y2)}. Si deux points ont des
   ...
   * Il peut exister une infinité de courbes quadratiques passant par trois points. On peut voir les
   * choses en disant qu'une courbe quadratique correspond à une parabole produite par une équation
   * de la forme <code>y=ax²+bx+c</code>, mais que l'axe des <var>x</var> de cette équation n'est
   * pas nécessairement horizontal. La direction de cet axe des <var>x</var> dépend du paramètre
   * {@code orientation} spécifié à cette méthode. La valeur {@link #HORIZONTAL} signifie que l'axe
   * des <var>x</var> de la parabole sera toujours horizontal. La courbe quadratique produite
   * ressemblera alors à une parabole classique telle qu'on en voit dans les ouvrages de
   * mathématiques élémentaires. La valeur {@link #PARALLEL} indique plutôt que l'axe des
   * <var>x</var> de la parabole doit être parallèle à la droite joignant les points {@code (x0,y0)}
   * et {@code (x2,y2)}. Ce dernier type produira le même résultat que {@link #HORIZONTAL} si {@code
   ...
   *     La courbe commencera au point {@code (x0,y0)} et se terminera au point {@code (x2,y2)}. Si
   *     deux points ont des coordonnées presque identiques, ou si les trois points sont
  ...
  /**
   * Retourne la coordonnée centrale d'un cercle passant pas les trois points spécifiés. La distance
   * entre le point retourné et n'importe quel des points
