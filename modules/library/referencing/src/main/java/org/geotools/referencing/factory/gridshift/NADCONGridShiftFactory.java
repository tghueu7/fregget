...
public class NADCONGridShiftFactory extends ReferencingFactory implements BufferedFactory {
   ...
   * contains num. rows + 1 (for the header) records. The data records (with the grid shift values)
   * are all floats and have a 4 byte separator (0's) before the data. Row records are organized
   * from low y (latitude) to high and columns are orderd from low longitude to high. Everything is
   * written in low byte order.
   *
   ...
   * will be returned. Otherwise a generic channel will be obtained from the urls input stream. Code
   * swiped from ShapefileDataStore.
   *
   ...
   * of the shift data file contain the header, with the first being a description of the grid. The
   * second line contains 8 values separated by spaces: num. columns, num. rows, num. z, min x,
   * delta x, min y, delta y and angle. Shift data values follow this and are also separated by
   * spaces. Row records are organized from low y (latitude) to high and columns are orderd from low
   * longitude to high.
   *
  ...
  private NADConGridShift loadTextGrid(URL latGridUrl, URL longGridUrl)
    ...
    if (latSt.countTokens() != 8) {
      throw new FactoryException(
          Errors.format(
              ErrorKeys.HEADER_UNEXPECTED_LENGTH_$1, String.valueOf(latSt.countTokens())));
    }
    ...
    if (longSt.countTokens() != 8) {
      throw new FactoryException(
          Errors.format(
              ErrorKeys.HEADER_UNEXPECTED_LENGTH_$1, String.valueOf(longSt.countTokens())));
    }
