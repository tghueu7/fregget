...
public class DefaultEngineeringCRS extends AbstractSingleCRS implements EngineeringCRS {
  ...
  /**
   * Marks the CRS as a wildcard one. Wildcard CRS will transform to any other CRS with the identity
   * transform
   */
   ...
   * {@linkplain SI#METER metres}. By default, this CRS has no transformation path to any other CRS
   * (i.e. a map using this CS can't be reprojected to a {@linkplain DefaultGeographicCRS geographic
   ...
   * DefaultCoordinateSystemAxis#Z z} axis in {@linkplain SI#METER metres}. By default, this CRS has
   * no transformation path to any other CRS (i.e. a map using this CS can't be reprojected to a
   ...
   * {@linkplain DefaultCoordinateSystemAxis#Y y} axis in {@linkplain SI#METER metres}. At the
   * difference of {@link #CARTESIAN_2D}, this coordinate system is treated specially by the default
   * {@linkplain org.geotools.referencing.operation.DefaultCoordinateOperationFactory coordinate
   * operation factory} with loose transformation rules: if no transformation path were found (for
   * example through a {@linkplain DefaultDerivedCRS derived CRS}), then the transformation from
   * this CRS to any CRS with a compatible number of dimensions is assumed to be the identity
   ...
   * x}, {@linkplain DefaultCoordinateSystemAxis#Y y}, {@linkplain DefaultCoordinateSystemAxis#Z z}
   * axis in {@linkplain SI#METER metres}. At the difference of {@link #CARTESIAN_3D}, this
   ...
   * through a {@linkplain DefaultDerivedCRS derived CRS}), then the transformation from this CRS to
   * any CRS with a compatible number of dimensions is assumed to be the identity transform. This
   * CRS is usefull as a kind of wildcard when no CRS were explicitly specified.
   */
   ...
   * otherwise, {@code CARTESIAN_xD} and {@code GENERIC_xD} would be considered equals when metadata
   * are ignored.
   */
  @Override
  ...
  /**
   * Returns true if this is a wildcard CRS, that is, one that will transform from and to any other
   * CRS using the identity transformation
   *
   * @return
   */
