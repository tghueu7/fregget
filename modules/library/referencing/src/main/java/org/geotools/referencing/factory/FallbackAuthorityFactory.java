...
public class FallbackAuthorityFactory extends AuthorityFactoryAdapter {
   ...
   * @param type The interface to implement. Should be one of {@link DatumAuthorityFactory}, {@link
   *     CSAuthorityFactory}, {@link CRSAuthorityFactory} or {@link
   ...
   *
   * <p>Consider using <code>{@linkplain #create(Class, Collection) create}(type, factories)</code>
   ...
   * @param interfaceMask The value computed by {@link #interfaceMask(Class)} that describe the set
   *     of interfaces to be implemented by the returned factory.
  ...
  /**
   * Returns the direct dependencies. The returned list contains the backing store and the fallback
   * specified at construction time, or the exception if they can't be obtained.
   */
  @Override
   ...
   * implementation delegates the lookups to the primary factory, and fallback on the second one if
   * the primary factory can't find a match.
   *
   * @since 2.4
   */
  @Override
  ...
  /**
   * A {@link IdentifiedObjectFinder} which fallback to the second factory if the primary one can't
   * find a match.
   */
  private final class Finder extends AuthorityFactoryAdapter.Finder {
    ...
    private void ensureFallback() throws FactoryException {
      if (fallback == null) {
        fallback =
            FallbackAuthorityFactory.this.fallback.getIdentifiedObjectFinder(getProxy().getType());
      }
   ...
   * about the success of <cite>fallback</cite> factory. The default implementation log a message to
   * the {@link Level#FINE FINE} level.
   *
