...
public class XAffineTransform extends AffineTransform {
  ...
  /**
   * Constructs a new {@code XAffineTransform} from 6 values representing the 6 specifiable entries
   * of the 3&times;3 transformation matrix. Those values are given unchanged to the {@link
   * AffineTransform#AffineTransform(double,double,double,double,double,double) super class
   * constructor}.
   *
   * @since 2.5
   */
   ...
   * is defined to thrown an exception in all case, then this {@code XAffineTransform} is immutable.
   *
  ...
  protected void checkPermission() throws UnsupportedOperationException {
    throw new UnsupportedOperationException(Errors.format(ErrorKeys.UNMODIFIABLE_AFFINE_TRANSFORM));
  }
   ...
   * specified tolerance. This method is equivalent to computing the difference between this matrix
   * and an identity matrix (as created by {@link AffineTransform#AffineTransform() new
   * AffineTransform()}) and returning {@code true} if and only if all differences are smaller than
   ...
   * @param tolerance The tolerance value to use when checking for identity. return {@code true} if
   *     this tranformation is close enough to the identity, {@code false} otherwise.
   * @since 2.3.1
   */
   ...
   * @see org.geotools.referencing.CRS#transform( org.opengis.referencing.operation.MathTransform2D,
   ...
   * seems to swap axis to the (<var>y</var>, <var>x</var>) axis order, or {@code 0} if this method
  ...
  /**
   * Returns an estimation of the rotation angle in radians. This method assumes that the specified
   * affine transform is built from arbitrary translations, scales or rotations, but no shear. If a
   * flip has been applied, then this method assumes that the flipped axis is the <var>y</var> one
   * in <cite>source CRS</cite> space. For a <cite>grid to world CRS</cite> transform, this is the
   * row number in grid coordinates.
   *
   * @param tr The affine transform to inspect.
   * @return An estimation of the rotation angle in radians, or {@link Double#NaN NaN} if the angle
   ...
   * combines {@link #getScaleX0} and {@link #getScaleY0}. The way to compute such a "global" scale
   * is somewhat arbitrary and may change in a future version.
   */
   ...
   * example, for speeding up image displays. Above all, it is efficient when we know that a matrix
   * has a chance of being close to the similarity matrix.
   *
