...
public abstract class AbstractCRS extends AbstractReferenceSystem
  ...
  /**
   * Constructs a new coordinate reference system with the same values than the specified one. This
   ...
   * identifiers} and {@linkplain #getRemarks remarks} are not taken in account. In other words, two
   * CRS objects will return the same hash value if they are equal in the sense of <code>
   ...
   * Known Text</cite> (WKT)</A> element. The default implementation writes the following elements:
   *
   * <ul>
   *   <li>The {@linkplain #datum datum}, if any.
   *   <li>The unit if all axis use the same unit. Otherwise the unit is omitted and the WKT format
   *       is {@linkplain Formatter#isInvalidWKT flagged as invalid}.
