...
public final class ReferencingFactoryFinder extends FactoryFinder {
   ...
   * If more than one implementation is registered and an {@linkplain #setVendorOrdering ordering is
   * set}, then the preferred implementation is returned. Otherwise an arbitrary one is selected.
   *
   ...
   * hints. If no implementation matches, a new one is created if possible or an exception is thrown
   * otherwise. If more than one implementation is registered and an {@linkplain #setVendorOrdering
   * ordering is set}, then the preferred implementation is returned. Otherwise an arbitrary one is
   * selected.
   *
  ...
  /**
   * Returns the first implementation of {@link DatumAuthorityFactory} matching the specified hints.
  ...
  /**
   * Returns the first implementation of {@link CSAuthorityFactory} matching the specified hints. If
   ...
   * null)} returns only one of those, usually the official EPSG factory. If the union of those two
   * factories is wanted, then a chain of fallbacks is wanted. Consider using something like:
   *
   * <blockquote>
   *
   * <code>
   ...
   * specified hints. If no implementation matches, a new one is created if possible or an exception
   * is thrown otherwise. If more than one implementation is registered and an {@linkplain
  ...
  /**
   * Returns the first implementation of {@link MathTransformFactory} matching the specified hints.
   * If no implementation matches, a new one is created if possible or an exception is thrown
   * otherwise. If more than one implementation is registered and an {@linkplain #setVendorOrdering
   * ordering is set}, then the preferred implementation is returned. Otherwise an arbitrary one is
   * selected.
  ...
  /**
   * Sets a pairwise ordering between two authorities. If one or both authorities are not currently
   * registered, or if the desired ordering is already set, nothing happens and {@code false} is
   * returned.
   *
  ...
  /**
   * Returns {@code true} if the specified factory is registered. A factory may have been registered
   * by {@link #scanForPlugins()} if it was declared in a {@code META-INF/services} file, or it may
   * have been {@linkplain #addAuthorityFactory added programmatically}.
   *
   * @since 2.4
   */
  ...
  /**
   * List all available factory implementations in a tabular format. For each factory interface, the
   * first implementation listed is the default one. This method provides a way to check the state
   * of a system, usually for debugging purpose.
   *
  ...
  /**
   * Dump to the standard output stream a list of available factory implementations. This method can
   * be invoked from the command line. It provides a mean to verify if some implementations were
   * found in the classpath. The syntax is: <br>
   *
   * <BLOCKQUOTE>
   *
   * <CODE>
