...
public class DefaultMathTransformFactory extends ReferencingFactory
  ...
  /**
   * The object to use for parsing <cite>Well-Known Text</cite> (WKT) strings. Will be created only
   * when first needed.
   */
   ...
   * may be available for the same factory interface. The default implementation returns {@linkplain
   * Citations#GEOTOOLS Geotools}.
   *
   ...
   * known to the {@link #getDefaultParameters} method in this factory. The set of available methods
   ...
   *
   * @param name The case insensitive {@linkplain org.opengis.metadata.Identifier#getCode identifier
   *     code} of the operation method to search for (e.g. {@code "Transverse_Mercator"}).
   * @return The operation method.
   * @throws NoSuchIdentifierException if there is no operation method registered for the specified
   *     name.
   * @since 2.2
   */
  ...
  private MathTransformProvider getProvider(final String method) throws NoSuchIdentifierException {
    ...
    provider =
        registry
            .getFactories(MathTransformProvider.class, null, HINTS)
            .filter(prov -> prov.nameMatches(method))
            .findAny()
            .orElseThrow(
                () ->
                    new NoSuchIdentifierException(
                        Errors.format(ErrorKeys.NO_TRANSFORM_FOR_CLASSIFICATION_$1, method),
                        method));
   ...
   * </code>).
   *
   * <p>This method creates new parameter instances at every call. It is intented to be modified by
   * the user before to be passed to <code>{@linkplain #createParameterizedTransform
  ...
  /**
   * Creates a {@linkplain #createParameterizedTransform parameterized transform} from a base CRS to
   * a derived CS. If the {@code "semi_major"} and {@code "semi_minor"} parameters are not
   * explicitly specified, they will be inferred from the {@linkplain Ellipsoid ellipsoid} and added
   * to {@code parameters}. In addition, this method performs axis switch as needed.
   *
  ...
  /**
   * Creates an affine transform from a matrix. If the transform's input dimension is {@code M}, and
   * output dimension is {@code N}, then the matrix will have size {@code [N+1][M+1]}. The +1 in the
   * matrix dimensions allows the matrix to do a shift, as well as a rotation. The {@code [M][j]}
   * element of the matrix will be the j'th ordinate of the moved origin. The {@code [i][N]} element
   * of the matrix will be 0 for <var>i</var> less than {@code M}, and 1 for <var>i</var> equals
   * {@code M}.
   *
   ...
   * input space in the second transform. If you wish to concatenate more than two transforms, then
  ...
  public MathTransform createConcatenatedTransform(
      final MathTransform transform1, final MathTransform transform2) throws FactoryException {
    MathTransform tr;
    try {
   ...
   * (<var>Lat</var>,<var>Lon</var>,<var>Height</var>) coordinates, then you may wish to convert the
   * height values from meters to feet without affecting the (<var>Lat</var>,<var>Lon</var>) values.
   *
  ...
  /**
   * Creates a math transform object from a XML string. The default implementation always throws an
   * exception, since this method is not yet implemented.
   *
   ...
   * invoked from the command line. It provides a mean to verify which transforms were found in the
   * classpath. The syntax is: <br>
   *
   * <BLOCKQUOTE>
   *
   * <CODE>
   ...
   * "Affine"</CODE>, <CODE>"EPSG:9624"</CODE> or just <CODE>"9624"</CODE> for the affine transform
   * method).
   *
   ...
   * </blockquote>
   *
   * <p>The codepage number (850 in the previous example) can be obtained from the DOS commande line
   * using the "{@code chcp}" command with no arguments.
   *
