...
final class FactoryPrinter implements Comparator<Class<?>> {
  ...
  /**
   * Compares two factories for order. This is used for sorting out the factories before to display
   * them.
   */
  ...
  public void list(final FactoryRegistry registry, final Writer out, final Locale locale)
     ...
     */
    final List<Class<?>> categories = registry.streamCategories().sorted(this).collect(toList());
    /*
