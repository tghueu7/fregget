...
public class Preprocessor extends Format {
  ...
  /**
   * Formats the specified object. This method delegates the work to the {@linkplain #parser parser}
   ...
   *   <LI>A Well Know Text, which may contains itself shortcuts declared in previous call to {@code
   *       addDefinition}. This text is given to the underlying {@link #parser}.
   *   <LI>Any services provided by subclasses. For example a subclass way recognize some authority
   *       code like {@code EPSG:6326}.
   * </UL>
   *
  ...
  /**
   * Parses a WKT. This method delegates the work to the {@link #parser}, but catch the exception in
   * case of failure. The exception is rethrown with the {@linkplain ParseException#getErrorIndex
   * error index} adjusted in order to point to the character in the original text (before
   * substitutions).
   *
  ...
  private static final class Definition implements Serializable {
    /**
     * The definition as a string. This string should not contains anymore shortcut to substitute by
     * an other WKT (i.e. compound definitions must be resolved before to construct a {@code
     * Definition} object).
     */
