...
public class AngleFormat extends Format {
  ...
  /**
   * A constant for the symbol to appears before the degrees fields. Fields PREFIX, DEGREES, MINUTES
   * and SECONDS <strong>must</strong> have increasing values (-1, 0, +1, +2, +3).
   */
  private static final int PREFIX_FIELD = -1;

  /**
   * Constant for degrees field. When formatting a string, this value may be specified to the {@link
   * java.text.FieldPosition} constructor in order to get the bounding index where degrees have been
   * written.
   */
  public static final int DEGREES_FIELD = 0;

  /**
   * Constant for minutes field. When formatting a string, this value may be specified to the {@link
   * java.text.FieldPosition} constructor in order to get the bounding index where minutes have been
   * written.
   */
  public static final int MINUTES_FIELD = 1;

  /**
   * Constant for seconds field. When formatting a string, this value may be specified to the {@link
   * java.text.FieldPosition} constructor in order to get the bounding index where seconds have been
   * written.
   */
   ...
   * doivent être écrites ensembles (par exemple 34867 pour 34.867). La valeur par défaut est {@code
   * true}.
   */
  ...
  /**
   * Per-instance rounding method. If this differs to {@code defaultRoundingMethod} it will be used
   * instead.
   */
   ...
   *
   * <p>Note: this will not affect the rounding method being used by instances of this class created
   * previously.
   *
  ...
  /**
   * Set the rounding method for this instance to use when the last significant digit of a value is
   * 5. If the rounding method has previously been set globally with the static {@linkplain
   ...
   *     with one of the following constants: {@link #DEGREES_FIELD}, {@link #MINUTES_FIELD}, {@link
   *     #SECONDS_FIELD} or {@link #HEMISPHERE_FIELD}.
  ...
  public synchronized StringBuffer format(
    ...
    toAppendTo =
        formatField(
            degrees, toAppendTo, field == DEGREES_FIELD ? pos : null, width0, width1 == 0, suffix0);
    ...
    if (!Double.isNaN(secondes)) {
      toAppendTo =
          formatField(
              secondes, toAppendTo, field == SECONDS_FIELD ? pos : null, width2, true, suffix2);
    }
    return toAppendTo;
  }
   ...
   * @param pos Objet dans lequel mémoriser les index des premiers et derniers caractères écrits, ou
   *     {@code null} pour ne pas mémoriser ces index.
   ...
   * #applyPattern}. The argument {@code obj} shall be an {@link Angle} object or one of its derived
   * class ({@link Latitude}, {@link Longitude}). If {@code obj} is a {@link Latitude} object, then
   * a symbol "N" or "S" will be appended to the end of the string (the symbol will be choosen
   * according the angle's sign). Otherwise, if {@code obj} is a {@link Longitude} object, then a
   * symbol "E" or "W" will be appended to the end of the string. Otherwise, no hemisphere symbol
   * will be appended. <br>
   * <br>
   * Strictly speaking, formatting ordinary numbers is not the {@code AngleFormat}'s job.
   * Nevertheless, this method accept {@link Number} objects. This capability is provided only as a
   * convenient way to format altitude numbers together with longitude and latitude angles.
   *
   ...
   * @param pos En entré, le code du champs dont on désire les index ({@link #DEGREES_FIELD}, {@link
   *     #MINUTES_FIELD}, {@link #SECONDS_FIELD} ou {@link #HEMISPHERE_FIELD}). En sortie, les index
   *     du champs demandé. Ce paramètre peut être nul si cette information n'est pas désirée.
  ...
  /**
   * Procède à l'écriture d'un angle suivit d'un suffix 'N','S','E' ou 'W'. L'angle sera formaté en
   * utilisant comme modèle le patron spécifié lors du dernier appel de la méthode {@link
   * #applyPattern}.
   *
   ...
   * @param field Champs à vérifier de préférences. Par exemple la valeur 1 signifie que les suffix
   *     des minutes et des secondes devront être vérifiés avant celui des degrés.
   * @return Le numéro du champs correspondant au suffix qui vient d'être extrait: -1 pour le préfix
   *     de l'angle, 0 pour le suffix des degrés, 1 pour le suffix des minutes et 2 pour le suffix
   *     des secondes. Si le texte n'a pas été reconnu, retourne {@code SYMBOLS.length}.
   */
   ...
   * to the expected pattern. For example, this method will parse correctly string "<code>48°12.34'
   * </code>" even if the expected pattern was "{@code DDMM.mm}" (i.e. the string should have been
   * "{@code 4812.34}"). Spaces between degrees, minutes and secondes are ignored. If the string
   * ends with an hemisphere symbol "N" or "S", then this method returns an object of class {@link
   * Latitude}. Otherwise, if the string ends with an hemisphere symbol "E" or "W", then this method
   * returns an object of class {@link Longitude}. Otherwise, this method returns an object of class
   * {@link Angle}.
   *
  ...
  /**
   * Interprète une chaîne de caractères représentant un angle. Les règles d'interprétation de cette
   * méthode sont assez souples. Par exemple cettte méthode interprétera correctement la chaîne
   * "48°12.34'" même si le patron attendu était "DDMM.mm" (c'est-à-dire que la chaîne aurait du
   * être "4812.34"). Les espaces entre les degrés, minutes et secondes sont acceptés. Si l'angle
   * est suivit d'un symbole "N" ou "S", alors l'objet retourné sera de la classe {@link Latitude}.
   * S'il est plutot suivit d'un symbole "E" ou "W", alors l'objet retourné sera de la classe {@link
   * Longitude}. Sinon, il sera de la classe {@link Angle}.
   *
  ...
  /**
   * Interprète une chaîne de caractères qui devrait représenter un nombre. Cette méthode est utile
   * pour lire une altitude après les angles.
   *
