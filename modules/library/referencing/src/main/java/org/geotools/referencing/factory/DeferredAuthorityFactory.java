...
public abstract class DeferredAuthorityFactory extends BufferedAuthorityFactory
  ...
  /**
   * The task for disposing the backing store, or {@code null} if none. This task will be scheduled
   * for repeated execution by {@link #setTimeout}.
   */
  private TimerTask disposer;

  /**
   * {@code true} if the backing store was used since the last time the timer task was run. A value
   * of {@code true} means that the task must wait again. A value of {@code false} means that it can
   ...
   * returns {@code false} if no {@code createFoo} method has been invoked, if the backing store has
   * been automatically disposed after the {@linkplain #setTimeout timeout} or if this factoy has
   * been {@linkplain #dispose disposed}.
   */
   ...
   * used during that time. The default implementation always returns {@code true}. Subclasses
   * should override this method and returns {@code false} if they want to prevent the backing store
   * disposal under some circonstances.
   *
