...
public class GeocentricTransform extends AbstractMathTransform implements Serializable {
  ...
  /**
   * Maximal error tolerance in metres during assertions, in metres. If assertions are enabled (JDK
   * 1.4 only), then every coordinates transformed with {@link #inverseTransform} will be
  ...
  /**
   * {@code true} if geographic coordinates include an ellipsoidal height (i.e. are 3-D), or {@code
   * false} if they are strictly 2-D.
   */
   ...
   * @param hasHeight {@code true} if geographic coordinates include an ellipsoidal height (i.e. are
   *     3-D), or {@code false} if they are only 2-D.
   */
  ...
  /**
   * Converts geodetic coordinates (longitude, latitude, height) to geocentric coordinates (x, y, z)
  ...
  private void inverseTransform(
    ...
    while (--numPts >= 0) {
      ...
      if (computeHeight) {
        ...
        double distance;
        assert MAX_ERROR
                > (distance = checkTransform(new double[] {x, y, z, longitude, latitude, height}))
            : distance;
      }
    }
  }
  ...
  public static class Provider extends MathTransformProvider {
    ...
    /**
     * The operation parameter descriptor for the "semi_major" parameter value. Valid values range
     * from 0 to infinity.
     */
    public static final ParameterDescriptor<Double> SEMI_MAJOR =
        createDescriptor(
            new NamedIdentifier[] {
              ...
              new NamedIdentifier(
                  Citations.EPSG,
                  "semi-major axis") // epsg does not specifically define this parameter
            },
            Double.NaN,
            0,
    ...
    /**
     * The operation parameter descriptor for the "semi_minor" parameter value. Valid values range
     * from 0 to infinity.
     */
    public static final ParameterDescriptor<Double> SEMI_MINOR =
        createDescriptor(
            new NamedIdentifier[] {
              ...
              new NamedIdentifier(
                  Citations.EPSG,
                  "semi-minor axis") // epsg does not specifically define this parameter
            },
            Double.NaN,
            0,
            Double.POSITIVE_INFINITY,
   ...
   * {@linkplain org.geotools.referencing.crs.DefaultGeographicCRS geographic} coordinate reference
   * systems.
   *
   * @version $Id$
   * @author Martin Desruisseaux (IRD)
