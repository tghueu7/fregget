...
public class DefaultOperationMethod extends AbstractIdentifiedObject implements OperationMethod {
  ...
  /**
   * Convenience constructor that creates an operation method from a math transform. The information
   * provided in the newly created object are approximative, and usually acceptable only as a
   * fallback when no other information are available.
   *
  ...
  /**
   * Constructs an operation method from a set of properties and a descriptor group. The properties
   * given in argument follow the same rules than for the {@linkplain
   * AbstractIdentifiedObject#AbstractIdentifiedObject(Map) super-class constructor}. Additionally,
   * the following properties are understood by this construtor: <br>
   * <br>
   *
   * <table border='1'>
  ...
  /**
   * Returns the operation type. Current implementation returns {@code Projection.class} for proper
   * WKT formatting using an unknow implementation. But the {@link MathTransformProvider} subclass
   * (with protected access) will overrides this method with a more conservative default value.
   *
  ...
  /**
   * Returns {@code true} if the specified transform is likely to exists only for axis switch and/or
   * unit conversions. The heuristic rule checks if the transform is backed by a square matrix with
   * exactly one non-null value in each row and each column. This method is used for implementation
   * of the {@link #checkDimensions} method only.
   */
