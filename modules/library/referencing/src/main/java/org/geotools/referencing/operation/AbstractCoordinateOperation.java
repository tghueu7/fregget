...
public class AbstractCoordinateOperation extends AbstractIdentifiedObject
   ...
   * AbstractIdentifiedObject#AbstractIdentifiedObject(Map) super-class constructor}. Additionally,
   ...
   * that all of {@code transform}, {@code sourceCRS} and {@code targetCRS} are null together. Note
   * that null values are not allowed for transformations, so {@link DefaultTransformation} does not
   ...
   *       linear unit, then this accuracy estimate is converted to {@linkplain SI#METER meters} and
   *       returned.
   *   <li>Otherwise, if the operation is a {@linkplain Conversion conversion}, then returns 0 since
   *       a conversion is by definition accurates up to rounding errors.
   *   <li>Otherwise, if the operation is a {@linkplain Transformation transformation}, then checks
   *       if the datum shift were applied with the help of Bursa-Wolf parameters. This procedure
   *       looks for Geotools-specific {@link PositionalAccuracyImpl#DATUM_SHIFT_APPLIED
   *       DATUM_SHIFT_APPLIED} and {@link PositionalAccuracyImpl#DATUM_SHIFT_OMITTED
   *       DATUM_SHIFT_OMITTED} metadata. If a datum shift has been applied, returns 25 meters. If a
   *       datum shift should have been applied but has been omitted, returns 1000 meters. The 1000
   *       meters value is higher than the highest value (999 meters) found in the EPSG database
   *       version 6.7. The 25 meters value is the next highest value found in the EPSG database for
   *       a significant number of transformations.
   ...
   * #getAccuracy(CoordinateOperation)} directly since it would cause never-ending recursive calls.
   */
   ...
   * @return The most specific GeoAPI interface (e.g. <code>{@linkplain Transformation}.class</code>
   *     ).
   */
      ...
      // See comment in DefaultOperation.equals(...) about why we compare MathTransform.
      {
        if (compareMetadata) {
          if (!Utilities.equals(this.domainOfValidity, that.domainOfValidity)
              ...
              || !Utilities.equals(
                  this.coordinateOperationAccuracy, that.coordinateOperationAccuracy)) {
            return false;
          }
        }
        /*
