...
public class NamedIdentifier
  ...
  /**
   * Identifier of the version of the associated code space or code, as specified by the code space
   * or code authority. This version is included only when the {@linkplain #getCode code} uses
   * versions. When appropriate, the edition is identified by the effective date, coded using ISO
   * 8601 date format.
   */
  ...
  /**
   * Implementation of the constructor. The remarks in the {@code properties} will be parsed only if
   * the {@code standalone} argument is set to {@code true}, i.e. this identifier is being
   ...
   * Citation#getAlternateTitles alternate titles} often contains abreviation (for example "DCW" as
   * an alternative title for "Digital Chart of the World"). If no alternative title is found or if
   * the main title is yet shorter, then it is used.
   */
  ...
  /**
   * Returns a view of this object as a local name. The local name returned by this method will have
   * the same {@linkplain LocalName#getScope scope} than this generic name.
   *
   * @deprecated Replaced by {@link #tip()}.
   */
  @Deprecated
   ...
   * is available, then this method returns an implementation mapping to {@link #toString} for all
   * locales.
   */
