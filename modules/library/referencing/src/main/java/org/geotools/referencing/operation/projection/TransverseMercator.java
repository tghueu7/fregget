...
public class TransverseMercator extends MapProjection {
  ...
  /**
   * A derived quantity of excentricity, computed by <code>e'² = (a²-b²)/b² = es/(1-es)</code> where
   * <var>a</var> is the semi-major axis length and <var>b</bar> is the semi-minor axis length.
   */
  ...
  private static final class Spherical extends TransverseMercator {
    ...
    /**
     * Maximal error tolerated for assertions in the spherical case. When assertions are enabled,
     * every projection using spherical formulas is followed by a projection using the ellipsical
     * formulas, and the results are compared. If a distance greater than the tolerance level is
     * found, then an {@link AssertionError} will be thrown. Subclasses may override this method if
     * they need to relax the tolerance level.
     *
     * <p>The default implementation allows a larger tolerance when comparing spherical to
     * elliptical calculations when we are far from the central meridian (elliptical calculations
     * are not valid here).
     *
     * @param longitude The longitude standardized (in radians) with {@linkplain #centralMeridian}
   ...
   *
   * @param centralLongitudeZone1 Longitude in the middle of zone 1, in decimal degrees relative to
   *     Greenwich. Positive longitudes are toward east, and negative longitudes toward west.
   * @param zoneWidth Number of degrees of longitudes in one zone. A positive value means that zones
   *     are numbered from west to east (i.e. in the direction of positive longitudes). A negative
   *     value means that zones are numbered from east to west.
  ...
  private int getZone(final double centralLongitudeZone1, final double zoneWidth) {
    ...
    t =
        floor(
            t / zoneWidth
                + EPSILON); // Number of zones between the central longitude and longitude 1.
    t -=
        zoneCount
            * floor(t / zoneCount); // If negative, bring back to the interval 0 to (zoneCount-1).
    return ((int) t) + 1;
  }
   ...
   *
   * @return The zone number, using the scalefactor and false easting to decide if this is a UTM or
   *     MTM case. Returns 0 if the case of the projection cannot be determined.
   */
  public int getZone() {
    // UTM
  ...
  /**
   * The {@linkplain org.geotools.referencing.operation.MathTransformProvider math transform
   * provider} for a South Orientated {@linkplain TransverseMercator Transverse Mercator} projection
   * (EPSG code 9808). Note that at the contrary of what this class name suggest, the projected
   * coordinates are still increasing toward North. This is because all {@linkplain MapProjection
   * map projections} in Geotools must complies with standard axis orientations. The real axis flip
   * is performed by the CRS framework outside this package. See "<cite>Axis units and
   * orientation</cite>" in {@linkplain org.geotools.referencing.operation.projection package
   ...
   * (-<var>westing</var>,-<var>southing</var>). This is equivalent to a {@link TransverseMercator}
   * projection with {@link #falseEasting falseEasting} and {@link #falseNorthing falseNorthing}
   * sign reverted. This operation is implemented as a concatenation of a North-orientated
   * transverse mercator projection with an affine transform for (<var>false
