...
public class Formatter {
  ...
  /**
   * The unit for formatting measures, or {@code null} for the "natural" unit of each WKT element.
   * This value is set for example by "GEOGCS", which force its enclosing "PRIMEM" to take the same
   * units than itself.
   */
  ...
  /**
   * The buffer in which to format. Consider this field as private final; the only method to set the
   * buffer to a new value is {@link AbstractParser#format}.
   */
  StringBuffer buffer;
  ...
  /**
   * The amount of space to write on the left side of each line. This amount is increased by {@code
   * indentation} every time a {@link Formattable} object is appended in a new indentation level.
   */
  private int margin;

  /**
   * {@code true} if a new line were requested during the execution of {@link #append(Formattable)}.
   * This is used to determine if {@code UNIT} and {@code AUTHORITY} elements should appears on a
   * new line too.
   */
  ...
  /**
   * Set the colors using the specified ANSI escape. The color is ignored unless syntax coloring has
   * been explicitly enabled. The {@code color} should be a constant from {@link X364}.
   */
   ...
   * formattable.{@linkplain Formattable#formatWKT formatWKT}(this)</code> for completing the inner
   * part of the WKT.
   *
  ...
  /**
   * Append a floating point number. A comma (or any other element separator) will be written before
  ...
  /**
   * Returns the preferred identifier for the specified object. If the specified object contains an
   * identifier from the preferred authority (usually {@linkplain Citations#OGC Open Geospatial}),
  ...
  private boolean authorityMatches(final Citation citation) {
    ...
    return (citation != null)
        && authority.getTitle().toString(null).equalsIgnoreCase(citation.getTitle().toString(null));
  }
   ...
   * element. This value is set for example by "GEOGCS", which force its enclosing "PRIMEM" to take
   * the same units than itself.
   *
   * @return The unit for measure. Default value is {@code null}.
   */
   ...
   * specification</A>. This method returns {@code true} if {@link #setInvalidWKT} has been invoked
   * at least once. The action to take regarding invalid WKT is caller-dependant. For example {@link
   * Formattable#toString} will accepts loose WKT formatting and ignore this flag, while {@link
   * Formattable#toWKT} requires strict WKT formatting and will thrown an exception if this flag is
   * set.
   *
   ...
   * Formattable#toWKT()} when no indentation were explicitly requested. This method can be invoked
