...
final class LocalizationGridTransform2D extends AbstractMathTransform
   ...
   * throwing an exception if the transform do not converge. This is a temporary flag until we find
   * why the inverse transform fails to converge in some case.
   */
  ...
  /**
   * <var>y</var> (usually latitude) offset relative to an entry. Points are stored in {@link #grid}
   * as {@code (x,y)} pairs.
   */
  static final int Y_OFFSET = 1;

  /**
   * Length of an entry in the {@link #grid} array. This lenght is equals to the dimension of output
   * coordinate points.
   */
   ...
   *     cloned; this is the caller's responsability to ensure that it will not be modified as long
   *     as this transformation is strongly reachable.
  ...
  private void transform(
    ...
    while (--numPts >= 0) {
      ...
      final double x0 =
          linearInterpolation(
              col + 0, grid[offset00 + X_OFFSET], col + 1, grid[offset10 + X_OFFSET], xi);
      final double y0 =
          linearInterpolation(
              col + 0, grid[offset00 + Y_OFFSET], col + 1, grid[offset10 + Y_OFFSET], xi);
      final double x1 =
          linearInterpolation(
              col + 0, grid[offset01 + X_OFFSET], col + 1, grid[offset11 + X_OFFSET], xi);
      final double y1 =
          linearInterpolation(
              col + 0, grid[offset01 + Y_OFFSET], col + 1, grid[offset11 + Y_OFFSET], xi);
      /*
  ...
  private void getAffineTransform(double x, double y, final AffineTransform dest) {
    ...
    dest.setTransform(
        dxCol, dyCol, dxRow, dyRow, x - dxCol * col - dxRow * row, y - dyCol * col - dyRow * row);
    /*
  ...
  /**
   * Apply the inverse transform to a set of points. More specifically, this method transform "real
   * world" coordinates to grid coordinates. This method use an iterative algorithm for that
   ...
   *
   * <ul>
   *   <li>Transform the first point using a "global" affine transform (i.e. the affine transformed
   *       computed using the "least squares" method in LocalizationGrid). Other points will be
   *       transformed using the last successful affine transform, since we assume that the points
   *       to transform are close to each other.
   ...
   *     {@code source}!<strong>
   * @param tr In input, the affine transform to use for the first step. In output, the last affine
   *     transform used for the transformation.
   */
  ...
  private final class Inverse extends AbstractMathTransform.Inverse
     ...
     * purpose. A {@link TransformException} is thrown in the computation do not converge.
     *
   ...
   * @version $Id$
   * @author Martin Desruisseaux (IRD)
   * @todo Not yet fully implemented. Once it is implemented, we need to add a getParameterValues()
   *     method in LocalizationGridTransform2D.
   */
