...
public class ThreadedEpsgFactory extends DeferredAuthorityFactory
  ...
  /**
   * The default JDBC {@linkplain DataSource data source} name in JNDI. This is the name used if no
   * other name were specified through the {@link Hints#EPSG_DATA_SOURCE EPSG_DATA_SOURCE} hint.
   *
  ...
  /**
   * Constructs an authority factory using a set of factories created from the specified hints. This
   ...
   * FACTORY} hints, in addition of {@link Hints#EPSG_DATA_SOURCE EPSG_DATA_SOURCE}.
   *
  ...
  public ThreadedEpsgFactory(final Hints userHints, final int priority) {
    ...
    String defaultTimeout =
        System.getProperty("org.geotools.epsg.factory.timeout", String.valueOf(30 * 60 * 1000));
    try {
    ...
    if (timeout > 0) {
      LOGGER.log(
          Level.FINE,
          "Setting the EPSG factory " + getClass().getName() + " to a " + timeout + "ms timeout");
   ...
   * #getDataSource()} when no data source has been {@linkplain #setDataSource explicitly set}. The
   * default implementation searchs for a {@link DataSource} instance binded to the {@link
   ...
   *
   * <p>Subclasses override this method in order to initialize a default data source when none were
   * found with JNDI. For example {@code plugin/epsg-access} defines a default data source using the
   * JDBC-ODBC bridge, which expects an "{@code EPSG}" database registered as an ODBC data source
   * (see the {@linkplain org.geotools.referencing.factory.epsg package javadoc} for installation
   * instructions). Example for a PostgreSQL data source:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * instance of {@link AccessDialectEpsgFactory} or {@link AnsiDialectEpsgFactory}. Subclasses may
   * override this method in order to returns an instance tuned for the SQL syntax of the underlying
   * database. Example for a PostgreSQL data source:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * @return The {@linkplain DirectEpsgFactory EPSG factory} using SQL queries appropriate for this
   *     data source.
  ...
  private AbstractAuthorityFactory createBackingStore0() throws FactoryException, SQLException {
    ...
    if (ALLOW_REGISTRATION && context != null) {
      try {
        ...
        record =
            Loggings.format(Level.FINE, LoggingKeys.CREATED_DATASOURCE_ENTRY_$1, datasourceName);
      } catch (NamingException exception) {
        record =
            Loggings.format(Level.WARNING, LoggingKeys.CANT_BIND_DATASOURCE_$1, datasourceName);
   ...
   * automatically after the amount of time specified by {@link #setTimeout} if the factory were not
