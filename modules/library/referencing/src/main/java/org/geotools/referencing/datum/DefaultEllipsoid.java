...
public class DefaultEllipsoid extends AbstractIdentifiedObject implements Ellipsoid {
   ...
   * algorithm for {@linkplain #orthodromicDistance orthodromic distance computation}, which may be
  ...
  /**
   * The inverse of the flattening value, or {@link Double#POSITIVE_INFINITY} if the ellipsoid is a
   * sphere.
   *
   ...
   * unchanged to the {@linkplain AbstractIdentifiedObject#AbstractIdentifiedObject(Map) super-class
   * constructor}.
   *
   ...
   * supplied ellipsoid is already an instance of {@code DefaultEllipsoid} or is {@code null}, then
   ...
   * whenever asked. Other ellipsoids use the polar radius to calculate the IVF whenever asked. This
  ...
  /**
   * {@code true} if the ellipsoid is degenerate and is actually a sphere. The sphere is completely
   * defined by the {@linkplain #getSemiMajorAxis semi-major axis}, which is the radius of the
   * sphere.
   *
   ...
   * always on a great circle. This is different from the <cite>loxodromic distance</cite>, which is
   * a longer distance on a path with a constant direction on the compass.
   *
   ...
   * remarks} and the like are not taken in account. In other words, two ellipsoids will return the
   * same hash value if they are equal in the sense of <code>
