...
public abstract class MathTransformBuilder {
   ...
   * @throws MismatchedDimensionException if some points doesn't have the {@linkplain #getDimension
   ...
   * @param target {@code false} for extracting source points, or {@code true} for extracting target
   *     points.
   */
   ...
   *
   * <ul>
   *   <li>If at least one source points has a CRS, then this CRS is selected as the source one and
   *       returned.
   *   <li>If no source point has a CRS, then this method creates an {@linkplain EngineeringCRS
   *       engineering CRS} using the same {@linkplain CoordinateSystem coordinate system} than the
   *       one used by the {@linkplain #getTargetCRS target CRS}.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>If at least one target points has a CRS, then this CRS is selected as the target one and
   *       returned.
   *   <li>If no target point has a CRS, then this method creates an {@linkplain EngineeringCRS
   *       engineering CRS} using the same {@linkplain CoordinateSystem coordinate system} than the
   *       one used by the {@linkplain #getSourceCRS source CRS}.
   * </ul>
   *
   * @throws FactoryException if the CRS can't be created.
   */
  ...
  /**
   * Creates an engineering CRS using the same {@linkplain CoordinateSystem coordinate system} than
   * the existing CRS, and an area of validity determined from the specified points. This method is
   * used for creating a {@linkplain #getTargetCRS target CRS} from the {@linkplain #getSourceCRS
   * source CRS}, or conversely.
   *
   ...
   *
   * @param target {@code false} for the envelope of source points, or {@code true} for the envelope
   *     of target points.
   */
  ...
  /**
   * Returns a geographic extent that contains fully all the specified points. If the envelope can't
   * be calculated, then this method returns {@code null}.
   *
   * @param target {@code false} for the valid area of source points, or {@code true} for the valid
   *     area of target points.
   */
  ...
  private CoordinateReferenceSystem ensureValid(final DirectPosition[] points, final String label)
    ...
    if (points.length < necessaryNumber) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.INSUFFICIENT_POINTS_$2, points.length, necessaryNumber));
    }
    CoordinateReferenceSystem crs = null;
    final int dimension = getDimension();
    for (int i = 0; i < points.length; i++) {
  ...
  private boolean ensureValid(
      final DirectPosition[] points, final String label, final CoordinateReferenceSystem expected) {
   ...
   * {@linkplain #getSourcePoints source points} transformed by the math transform computed by this
   * {@code MathTransformBuilder}, and the {@linkplain #getTargetPoints target points}. Use {@link
   * Statistics#rms} for the <cite>Root Mean Squared error</cite>.
   *
