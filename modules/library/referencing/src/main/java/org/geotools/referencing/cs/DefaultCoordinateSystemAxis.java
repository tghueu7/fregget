...
public class DefaultCoordinateSystemAxis extends AbstractIdentifiedObject
  ...
  public static final DefaultCoordinateSystemAxis ALTITUDE =
      new DefaultCoordinateSystemAxis(VocabularyKeys.ALTITUDE, "h", AxisDirection.UP, SI.METER);
  ...
  public static final DefaultCoordinateSystemAxis DEPTH =
      new DefaultCoordinateSystemAxis(VocabularyKeys.DEPTH, "d", AxisDirection.DOWN, SI.METER);
  ...
  /**
   * Default axis info for Easting values in a {@linkplain org.opengis.referencing.crs.ProjectedCRS
   * projected CRS}.
   *
   * <p>Increasing ordinates values go {@linkplain AxisDirection#EAST East} and units are
   * {@linkplain SI#METER metres}.
  ...
  public static final DefaultCoordinateSystemAxis EASTING =
      new DefaultCoordinateSystemAxis(VocabularyKeys.EASTING, "E", AxisDirection.EAST, SI.METER);

  /**
   * Default axis info for Westing values in a {@linkplain org.opengis.referencing.crs.ProjectedCRS
   * projected CRS}.
   *
  ...
  public static final DefaultCoordinateSystemAxis WESTING =
      new DefaultCoordinateSystemAxis(VocabularyKeys.WESTING, "W", AxisDirection.WEST, SI.METER);
  ...
  /**
   * Default axis info for Northing values in a {@linkplain org.opengis.referencing.crs.ProjectedCRS
   * projected CRS}.
   *
   * <p>Increasing ordinates values go {@linkplain AxisDirection#NORTH North} and units are
   * {@linkplain SI#METER metres}.
  ...
  public static final DefaultCoordinateSystemAxis NORTHING =
      new DefaultCoordinateSystemAxis(VocabularyKeys.NORTHING, "N", AxisDirection.NORTH, SI.METER);

  /**
   * Default axis info for Southing values in a {@linkplain org.opengis.referencing.crs.ProjectedCRS
   * projected CRS}.
   *
  ...
  public static final DefaultCoordinateSystemAxis SOUTHING =
      new DefaultCoordinateSystemAxis(VocabularyKeys.SOUTHING, "S", AxisDirection.SOUTH, SI.METER);
   ...
   *
   * <p>Increasing time go toward {@linkplain AxisDirection#FUTURE future} and units are {@linkplain
   * NonSI#DAY days}.
   *
   * <p>The abbreviation is lower case "<var>t</var>".
   */
  public static final DefaultCoordinateSystemAxis TIME =
      new DefaultCoordinateSystemAxis(VocabularyKeys.TIME, "t", AxisDirection.FUTURE, NonSI.DAY);

  /**
   * A default axis for column indices in a {@linkplain org.opengis.coverage.grid.GridCoverage grid
   * coverage}. Increasing values go toward {@linkplain AxisDirection#COLUMN_POSITIVE positive
   * column number}.
   *
   ...
   * to distinguish between {@link CoordinateSystemAxis} instances. Those names are strictly defined
   * by ISO 19111 as "Geodetic latitude" and "Geodetic longitude" among others, but the legacy WKT
   ...
   * name is for. The actual axis instance doesn't matter (the algorithm using this map should work
   * for any axis instance); it is just a way to differentiate latitude and longitude.
   */
   ...
   * is the direction of this coordinate system axis locally.
   */
  ...
  /**
   * Constructs a new coordinate system axis with the same values than the specified one. This copy
  ...
  /**
   * Constructs an unbounded axis from a set of properties. The properties map is given unchanged to
   * the {@linkplain AbstractIdentifiedObject#AbstractIdentifiedObject(Map) super-class
  ...
  /**
   * Constructs an axis with a name as an {@linkplain InternationalString international string} and
   * an abbreviation. The {@linkplain #getName name of this identified object} is set to the
   ...
   *
   * <p>The direction argument is optional and can be used in order to resolve ambiguity like {@link
   * #X} and {@link #DISPLAY_X} axis. If this argument is {@code null}, then the first axis with a
   * matching name or abbreviation will be returned.
   *
  ...
  public static AxisDirection getDirection(String direction) throws NoSuchElementException {
    ...
    throw new NoSuchElementException(Errors.format(ErrorKeys.UNKNOW_AXIS_DIRECTION_$1, direction));
  }

  /**
   * Direction of this coordinate system axis. In the case of Cartesian projected coordinates, this
   * is the direction of this coordinate system axis locally. Examples: {@linkplain
   * AxisDirection#NORTH north} or {@linkplain AxisDirection#SOUTH south}, {@linkplain
   * AxisDirection#EAST east} or {@linkplain AxisDirection#WEST west}, {@linkplain AxisDirection#UP
   * up} or {@linkplain AxisDirection#DOWN down}.
   *
   ...
   * coordinate tuple shall be recorded using this unit of measure, whenever those coordinates use a
   ...
   * and {@linkplain #getMaximumValue maximum} values. This element shall be omitted when both
   * minimum and maximum values are omitted. It may be included when minimum and/or maximum values
   * are included. If this element is omitted when minimum or maximum values are included, the
   * meaning is unspecified.
   *
   * @since 2.3
   */
   ...
   *
   * <p>A positive angle denotes a right-handed system, while a negative angle denotes a left-handed
   * system. Example:
   *
   * <p>
   *
   ...
   *       WEST} is -90°
   *   <li>The angle from "<cite>North along 90 deg East</cite>" to "<cite>North along 0 deg</cite>"
   *       is 90°.
   * </ul>
   *
   ...
   * </ul>
   *
   * <p>The above special cases are needed in order to workaround a conflict in specifications: ISO
   * 19111 explicitly state that the latitude and longitude axis names shall be "Geodetic latitude"
   * and "Geodetic longitude", will legacy OGC 01-009 (where WKT is defined) said that the default
   * values shall be "Lat" and "Lon".
   *
   ...
   * Units should always be compared (they are not just metadata), except in the particular case of
   * {@link AbstractCS#axisColinearWith}, which is used as a first step toward units conversions
   * through {@link AbstractCS#swapAndScaleAxis}.
   */
