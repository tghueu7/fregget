...
public interface XMatrix extends Matrix {
   ...
   * method is equivalent to computing the difference between this matrix and an identity matrix of
   * identical size, and returning {@code true} if and only if all differences are smaller than or
   * equal to {@code tolerance}.
   *
   * @param tolerance The tolerance value.
   * @return {@code true} if this matrix is close enough to the identity matrix given the tolerance
   *     value.
   * @since 2.4
   */
  ...
  /**
   * Returns {@code true} if this matrix is an affine transform. A transform is affine if the matrix
   * is square and last row contains only zeros, except in the last column which contains 1.
   *
  ...
  /**
   * Sets the value of this matrix to the result of multiplying itself with the specified matrix. In
   * other words, performs {@code this} = {@code this} &times; {@code matrix}. In the context of
   * coordinate transformations, this is equivalent to <code>
