...
final class AuthorityCodes extends AbstractSet<String> implements Serializable {
  ...
  /**
   * The factory which is the owner of this set. One purpose of this field (even if it were not used
   * directly by this class) is to avoid garbage collection of the factory as long as this set is in
   * use. This is required because {@link DirectEpsgFactory#finalize} closes the JDBC connections.
   */
  ...
  /**
   * The SQL command to use for creating the {@code queryAll} statement. Used for iteration over all
   * codes.
   */
  final String sqlAll;
   ...
   * only when first needed. The special value -2 if set by {@link #isEmpty} if the size has not yet
   * been computed, but we know that the set is not empty.
   */
  private int size = -1;
  ...
  /**
   * Returns {@code true} if the code in the specified result set is acceptable. This method handle
  ...
  /**
   * Returns {@code true} if this collection contains no elements. This method fetch at most one row
  ...
  /**
   * Returns an iterator over the codes. The iterator is backed by a living {@link ResultSet}, which
   * will be closed as soon as the iterator reach the last element.
   */
  @Override
  ...
  /**
   * The iterator over the codes. This inner class must kept a reference toward the enclosing {@link
   * AuthorityCodes} in order to prevent a call to {@link AuthorityCodes#finalize} before the
