...
public class GeneralEnvelope extends AbstractEnvelope implements Cloneable, Serializable {
  ...
  /**
   * Minimum and maximum ordinate values. The first half contains minimum ordinates, while the last
   * half contains maximum ordinates. This layout is convenient for the creation of lower and upper
   * corner direct positions.
   *
  ...
  /**
   * Constructs an empty envelope of the specified dimension. All ordinates are initialized to 0 and
   * the coordinate reference system is undefined.
   *
   ...
   * @throws IllegalArgumentException if an ordinate value in the minimum point is not less than or
  ...
  /**
   * Constructs an empty envelope with the specified coordinate reference system. All ordinates are
   * initialized to 0.
   *
  ...
  /**
   * Converts the envelope to a general envelope, avoiding the construction of a new object in case
   * the input envelope is already a GeneralEnvelope
   *
   * @param envelope
   * @return
   */
   ...
   *
   * <p><b>Note:</b> The convention is specified as a {@link PixelInCell} code instead than the more
  ...
  public GeneralEnvelope(
    ...
    try {
      ...
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.BAD_TRANSFORM_$1, Classes.getClass(gridToCRS)), exception);
    }
  ...
  /**
   * Sets the coordinate reference system in which the coordinate are given. This method <strong>do
   * not</strong> reproject the envelope, and do not check if the envelope is contained in the new
   * domain of validity. The later can be enforced by a call to {@link #normalize}.
   *
   ...
   *             values greater than the {@linkplain CoordinateSystemAxis#getMaximumValue maximum
   *             value} are replaced by the maximum, and values smaller than the {@linkplain
   *             CoordinateSystemAxis#getMinimumValue minimum value} are replaced by the minimum.
   ...
   *             subtracted. If a value stay out of range after this correction, then the ordinates
   *             are set to the full [ {@linkplain CoordinateSystemAxis#getMinimumValue minimum} ...
   ...
   *             greater than the upper bounds. Reordering as [-175° ... 175°] would interchange the
   *             meaning of what is "inside" and "outside" the envelope. So this implementation
   *             conservatively expands the range to [-180° ... 180°] in order to ensure that the
   *             validated envelope fully contains the original envelope.
   *             </blockquote>
   *       </ul>
   *   <li>
  ...
  /**
   * A coordinate position consisting of all the {@linkplain #getMinimum minimal ordinates} for each
  ...
  /**
   * A coordinate position consisting of all the {@linkplain #getMaximum maximal ordinates} for each
   * dimension for all points within the {@code Envelope}.
   *
  ...
  /**
   * Returns the envelope span (typically width or height) along the specified dimension. The result
   * should be equals (minus rounding error) to <code>{@linkplain #getMaximum
  ...
  /**
   * Sets the envelope to the specified values, which must be the lower corner coordinates followed
   * by upper corner coordinates. The number of arguments provided shall be twice this {@linkplain
   * #getDimension envelope dimension}, and minimum shall not be greater than maximum.
   *
  ...
  public void setEnvelope(final double... ordinates) {
    ...
    if (dimension != check) {
      throw new MismatchedDimensionException(
          Errors.format(ErrorKeys.MISMATCHED_DIMENSION_$3, "ordinates", dimension, check));
    }
    checkCoordinates(ordinates);
   ...
   * @throws MismatchedDimensionException if the specified envelope doesn't have the expected number
   *     of dimensions.
   * @since 2.2
   */
  ...
  /**
   * Sets the lower corner to {@linkplain Double#NEGATIVE_INFINITY negative infinity} and the upper
   * corner to {@linkplain Double#POSITIVE_INFINITY positive infinity}. The {@linkplain
   ...
   * false} only after all dimensions have been initialized. More specifically, the following rules
   * apply:
   *
   * <p>
   *
   ...
   * greater than 0 along all dimensions. Note that a non-empty envelope is always non- {@linkplain
  ...
  /**
   * Returns {@code true} if at least one of the specified CRS is null, or both CRS are equals. This
   * special processing for {@code null} values is different from the usual contract of an {@code
   * equals} method, but allow to handle the case where the CRS is unknown.
   */
  ...
  /**
   * Adds an envelope object to this envelope. The resulting envelope is the union of the two {@code
   * Envelope} objects.
   *
  ...
  /**
   * Returns {@code true} if this envelope completly encloses the specified envelope. If one or more
   * edges from the specified envelope coincide with an edge from this envelope, then this method
   ...
   * from the specified envelope coincide with an edge from this envelope, then this method returns
   * {@code true} only if {@code edgesInclusive} is {@code true}.
   *
   * <p>This method assumes that the specified envelope uses the same CRS than this envelope. For
   * performance reason, it will no be verified unless J2SE assertions are enabled.
   *
   ...
   * length} along each dimension or can be an absolute value (as for example some ground resolution
   * of a {@linkplain org.opengis.coverage.grid.GridCoverage grid coverage}).
   *
   * <p>If {@code relativeToLength} is set to {@code true}, the actual tolerance value for a given
   * dimension <var>i</var> is {@code eps}&times;{@code length} where {@code length} is the maximum
   * of {@linkplain #getLength this envelope length} and the specified envelope length along
   * dimension <var>i</var>.
   *
   * <p>If {@code relativeToLength} is set to {@code false}, the actual tolerance value for a given
   * dimension <var>i</var> is {@code eps}.
   *
   * <p>Relative tolerance value (as opposed to absolute tolerance value) help to workaround the
   * fact that tolerance value are CRS dependent. For example the tolerance value need to be smaller
   ...
   * @param epsIsRelative {@code true} if the tolerance value should be relative to axis length, or
   *     {@code false} if it is an absolute value.
