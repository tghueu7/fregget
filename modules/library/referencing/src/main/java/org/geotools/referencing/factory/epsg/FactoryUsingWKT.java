...
public class FactoryUsingWKT extends DeferredAuthorityFactory implements CRSAuthorityFactory {
   ...
   * where to register the content of this file. The user-directory given by the system property may
   * contains other property files for other authorities, like {@code "esri.properties"}, but those
   * additional authorities are not handled by the default {@code FactoryUsingWKT} class.
   *
   ...
   *
   * @since 2.4
   * @deprecated We will try to replace the priority mechanism by a better one in a future Geotools
   *     version.
   */
  ...
  /**
   * Returns the authority. The default implementation returns the first citation returned by {@link
   * #getAuthorities()}, with the addition of identifiers from all additional authorities returned
   * by the above method.
   *
   * @see #getAuthorities
   */
   ...
   * PropertyAuthorityFactory#PropertyAuthorityFactory(ReferencingFactoryContainer, Citation[], URL)
  ...
  protected AbstractAuthorityFactory createBackingStore() throws FactoryException {
    try {
      ...
      final LogRecord record =
          Loggings.format(
              Level.CONFIG, LoggingKeys.USING_FILE_AS_FACTORY_$2, url.getPath(), authority);
  ...
  /**
   * Prints a list of codes that duplicate the ones provided in the {@link ThreadedEpsgFactory}. The
   * factory tested is the one registered in {@link ReferencingFactoryFinder}. By default, this is
   * this {@code FactoryUsingWKT} class backed by the {@value #FILENAME} property file. This method
   * can be invoked from the command line in order to check the content of the property file. Valid
   * arguments are:
   *
   * <p>
   *
