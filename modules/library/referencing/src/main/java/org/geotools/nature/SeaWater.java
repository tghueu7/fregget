...
public final class SeaWater {
  ...
  /**
   * Conductivity (in mS/cm) of a standard sea water sample. S is for <cite>Siemens</cite> (or Mho,
   * its the same...).
   */
   ...
   * is equivalent to the true density minus 1000&nbsp;kg/m³, and has typical values around 35. This
   * computation avoid some rouding errors occuring in the true density computation.
   *
   * @param S Salinity PSS-78 (0 to 42)
   * @param T Temperature ITS-68 (-2 to 40°C)
   * @param P Pressure in decibars (0 to 10<sup>5</sup> dbar), not including atmospheric pressure.
  ...
  public static double densitySigmaT(final double S, final double T, double P) {
    ...
    final double SR = Math.sqrt(S);
    final double RHO =
        (EOS80_D * S + polynome(T, EOS80_C) * SR + polynome(T, EOS80_B)) * S + polynome(T, EOS80_A);
   ...
   * inverse of density. This method is equivalent to <code>1/{@link #density density}(S,T,P)</code>
   * .
   *
   * @param S Salinity PSS-78 (0 to 42)
   * @param T Temperature ITS-68 (-2 to 40°C)
  ...
  public static double specificHeat(final double S, final double T, double P) {
    ...
    return (polynome(T, HEAT_CC)
        ...
        + ((((HEAT_J * SR + polynome(T, HEAT_H)) * S * P + (HEAT_G * SR + polynome(T, HEAT_F)) * S)
                    * P
  ...
  public static double soundVelocity(final double S, final double T, final double P) {
    // S^0 terms
    final double CW =
        ((polynome(T, SOUND_C3) * P + polynome(T, SOUND_C2)) * P + polynome(T, SOUND_C1)) * P
            + polynome(T, SOUND_C0);
    // S^1 terms
    final double A =
        ((polynome(T, SOUND_A3) * P + polynome(T, SOUND_A2)) * P + polynome(T, SOUND_A1)) * P
            + polynome(T, SOUND_A0);
    // S^3/2 terms
