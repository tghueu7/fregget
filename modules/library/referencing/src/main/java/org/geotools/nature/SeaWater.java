...
public final class SeaWater {
    ...
    private static final double
            EOS80_A[] = {-28.263737E+0, 6.793952E-2, -9.095290E-3, 1.001685E-4, -1.120083E-6, 
            6.536332E-9},
     ...
     */
    private static final double
            EOS80_At[] = {999.842594, 6.793952E-2, -9.095290E-3, 1.001685E-4, -1.120083E-6, 
            6.536332E-9},
    ...
    public static double density(final double S, final double T, double P) {
        ...
        final double RHO_S_T_0 = (EOS80_D * S + polynome(T, EOS80_C) * SR + polynome(T, EOS80_B))
                * S + RHO_0_T_0;

        // Compression terms
        final double K_S_T_0 = (polynome(T, EOS80_F) + polynome(T, EOS80_G) * SR) * S + polynome
                (T, EOS80_Et);
        final double K_S_T_P = K_S_T_0 + ((EOS80_J * SR + polynome(T, EOS80_I)) * S + polynome(T,
                EOS80_Ht) +
    ...
    public static double densitySigmaT(final double S, final double T, double P) {
        ...
        // Sea water density at atmospheric pressure
        final double SR = Math.sqrt(S);
        final double RHO = (EOS80_D * S + polynome(T, EOS80_C) * SR + polynome(T, EOS80_B)) * S +
                polynome(T, EOS80_A);
        ...
        final double K0 = (polynome(T, EOS80_F) + polynome(T, EOS80_G) * SR) * S + polynome(T, 
                EOS80_E);
    ...
    public static double volume(final double S, final double T, double P) {
        ...
        final double K_35_0_P = polynome(P, EOS80_N);
        final double V_S_T_0 = SVAN_S_T_0 + V_35_0_0;
        return (SVAN_S_T_0 + V_35_0_0) * (1.0 - P / K_35_0_P) + V_S_T_0 * P * DK / (K_35_0_P * 
                (K_35_0_P + DK));
    }
    ...
    public static double volumeAnomaly(final double S, final double T, double P) {
        ...
        final double K_35_0_P = polynome(P, EOS80_N);
        final double V_S_T_0 = SVAN_S_T_0 + V_35_0_0;
        return (SVAN_S_T_0 * (1.0 - P / K_35_0_P) + V_S_T_0 * P * DK / (K_35_0_P * (K_35_0_P + 
                DK)));
    }
    ...
    public static double salinity(double C, final double T, final double P) {
        ...
        if (!(C < 5E-4)) { // use '!' in order to accept NaN
            final double XR = Math.sqrt(C / (polynome(T, PSS78_C) * (1.0 + polynome(P, PSS78_E) *
                    P /
                    ((PSS78_D[1] * T + PSS78_D[0]) * T + 1.0 + (PSS78_D[3] * T + PSS78_D[2]) * C)
            )));
            final double S = sal(XR, T - 15.0);  // Do not use an 'assert' statement invoking 
            // 'cond'.
    ...
    public static double conductivity(final double S, final double T, final double P) {
        if (!(S < 0.02)) { // use '!' in order to accept NaN
            ...
            double SI = sal(RT, XT);
            for (int n = 0; n < 10; n++) {    // Iteration loop begin here with a maximum of 10 
                // cycles
                RT += (S - SI) / dsal(RT, XT);
                SI = sal(RT, XT);
    ...
    public static double specificHeat(final double S, final double T, double P) {
        ...
        return (polynome(T, HEAT_CC) + (polynome(T, HEAT_BB) * SR + polynome(T, HEAT_AA)) * S +
                ...
                ((((HEAT_J * SR + polynome(T, HEAT_H)) * S * P + (HEAT_G * SR + polynome(T, 
                        HEAT_F)) * S) * P +
