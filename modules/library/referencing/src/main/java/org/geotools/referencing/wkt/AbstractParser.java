...
public abstract class AbstractParser extends Format {
  /**
   * Set to {@code true} if parsing of number in scientific notation is allowed. The way to achieve
   * that is currently a hack, because {@link NumberFormat} has no API for managing that as of J2SE
   * 1.5.
   *
  ...
  /**
   * Returns the preferred authority for formatting WKT entities. The {@link #format format} methods
   ...
   * strings are read until the the end-of-stream, or until an unparsable WKT has been hit. In this
   * later case, an error message is formatted to the specified error stream.
   *
   * @param in The input stream.
   * @param out The output stream.
   * @param err The error stream.
   * @throws IOException if an error occured while reading from the input stream or writting to the
   *     output stream.
   */
  ...
  /**
   * If a warning occured during the last WKT {@linkplain #format formatting}, returns the warning.
