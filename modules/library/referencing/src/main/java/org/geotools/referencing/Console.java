...
public class Console extends AbstractConsole {
  ...
  /**
   * The number separator in vectors. Usually {@code ,}, but could also be {@code ;} if the coma is
   * already used as the decimal separator.
   */
  ...
  /**
   * The tolerance value. If non-null, the difference between the computed and the specified target
   * point will be compared against this tolerance threshold. If it is greater, a message will be
   * printed.
   */
  private double[] tolerance;

  /**
   * The last error thats occured while processing an instruction. Used in order to print the stack
   * trace on request.
   */
  ...
  /**
   * Run the console from the command line. Before to process all instructions from the {@linkplain
   * System#in standard input stream}, this method first process the following optional command-line
   * arguments:
   *
   * <p>
   *
        ...
        if (!keywords.hasMoreTokens()) {
          ...
          if (key1.equalsIgnoreCase("crs")) {
            if (key0.equalsIgnoreCase("source")) {
              sourceCRS =
                  (CoordinateReferenceSystem) parseObject(value, CoordinateReferenceSystem.class);
              transform = null;
              return;
            }
   ...
   * point with the expected target point. If a mismatch greater than the tolerance error is found,
   * an exception is thrown. Subclasses may overrides this method in order to performs more tests.
   *
   * @throws TransformException if the source point can't be transformed, or a mistmatch is found.
   * @throws MismatchedDimensionException if the transformed source point doesn't have the expected
   *     dimension.
   */
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (position.getIndex() != token.length()) {
        throw new ParseException(
            Errors.format(ErrorKeys.UNPARSABLE_NUMBER_$1, token), position.getErrorIndex());
      }
