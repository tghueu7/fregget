...
public class PropertyAuthorityFactory extends DirectAuthorityFactory
   ...
   * supposed to be immutable after factory construction. For now, this class do not allow addition
   * of elements.
   */
  private final Properties definitions = new Properties();

  /**
   * An unmodifiable view of the authority keys. This view is always up to date even if entries are
   * added or removed in the {@linkplain #definitions} map.
   */
  @SuppressWarnings("unchecked")
  private final Set<String> codes = Collections.unmodifiableSet((Set) definitions.keySet());

  /**
   * Views of {@link #codes} for different types. Views will be constructed only when first needed.
   * View are always up to date even if entries are added or removed in the {@linkplain
   * #definitions} map.
   */
   ...
   * factory for CRS defined by ESRI} uses the {@code "ESRI"} namespace, but also the {@code "EPSG"}
   * namespace because those CRS are used as extension of the EPSG database. Concequently, the same
   * CRS can be identified as {@code "ESRI:53001"} and {@code "EPSG:53001"}, where {@code "53001"}
   * is a unused code in the official EPSG database.
   *
   * @param factories The underlying factories used for objects creation.
   * @param authorities The organizations or party responsible for definition and maintenance of the
   *     database.
   * @param definitions URL to the definition file.
   * @throws IOException if the definitions can't be read.
   * @since 2.4
   ...
   * For example if this factory is an instance of CRSAuthorityFactory, then:
   *
   * <p>
   *
   * <ul>
  ...
  private static final class Codes extends DerivedSet<String, String> {
    ...
    public Codes(
        final Map<String, String> definitions, final Class<? extends IdentifiedObject> type) {
  ...
  /**
   * Returns a coordinate reference system from a code. If the object type is know at compile time,
  ...
  /**
   * The WKT parser for this authority factory. This parser add automatically the authority code if
   * it was not explicitly specified in the WKT.
   */
