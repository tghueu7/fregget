...
public class WarpBuilder {
    ...
    /**
     * Given a math transform and a source domain, return the number of rows
     * and cols by which the domain should be split to avoid transform linearity issues,
    ...
    public int[] getRowColsSplit(MathTransform2D mt, Rectangle2D.Double domain) {
        ...
        int[] rowCols;
        try {
            rowCols = computeOptimalDepths(mt, minx, maxx, miny, maxy, 0, 0, (minx1, maxx1, miny1, maxy1, rowDepth, colDepth) -> {
    ...
    interface DomainValidator {
        public void validateDomain(double minx,
                                  double maxx,
                                  double miny,
                                  double maxy,
                                  int rowDepth,
