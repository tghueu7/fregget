...
public class WarpBuilder {
  ...
  /**
   * The max distance tolerated between the actual projected point and the approximate version built
   * by the optimized warp transform
   */
  ...
  public Warp buildWarp(MathTransform2D mt, Rectangle domain) throws TransformException {
    ...
    if (rowCols[0] == 0 && rowCols[1] == 0) {
      ...
      LOGGER.log(
          Level.FINE, "Optimizing the warp into an grid warp {0} x {1}", new Object[] {rows, cols});
