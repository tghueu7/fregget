...
final class ModifiedParameterDescriptor extends DefaultParameterDescriptor {
   ...
   * {@link ModifiedParameterDescriptor} are unwrapped to their original descriptor. The drawback is
   * that this method is slower than {@code set.contains(descriptor)}, so it should be invoked only
   * if the former fails.
   */
