...
public class CoordinateFormat extends Format {
   ...
   * #getCoordinateReferenceSystem coordinate system}'s dimension. This array is never {@code null}.
   */
  ...
  /**
   * Conversions from temporal axis units to milliseconds. Non-null only if at least one ordinate is
   * a date.
   */
    ...
    if (crs != null) {
      if (dimension != formats.length) {
        throw new MismatchedDimensionException(
            Errors.format(ErrorKeys.MISMATCHED_DIMENSION_$3, "point", dimension, formats.length));
      }
