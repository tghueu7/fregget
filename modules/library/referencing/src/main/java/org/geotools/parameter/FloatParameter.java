...
public class FloatParameter extends AbstractParameter implements ParameterValue<Double> {
   ...
   * ParameterDescriptor#getValueClass() value class} must be <code>{@linkplain Double}.class</code>
   * .
   *
  ...
  public FloatParameter(final ParameterDescriptor<Double> descriptor) {
    ...
    if (!expected.equals(type) && !Double.TYPE.equals(type)) {
      throw new IllegalArgumentException(Errors.format(ErrorKeys.ILLEGAL_CLASS_$2, type, expected));
    }
  ...
  /**
   * Constructs a parameter from the specified descriptor and value. This convenience constructor is
   * equivalents to the one-argument constructor followed by a call to {@link #setValue(double)}.
   *
   * @param descriptor The abstract definition of this parameter.
