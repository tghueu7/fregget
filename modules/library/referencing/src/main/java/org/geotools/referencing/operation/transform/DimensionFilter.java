...
public class DimensionFilter {
  ...
  /**
   * Clears any {@linkplain #getSourceDimensions source} and {@linkplain #getTargetDimensions target
   * dimension} setting.
   */
  public void clear() {
  ...
  /**
   * Returns the input dimensions. This information is available only if at least one setter method
   * has been explicitly invoked for source dimensions.
   *
  ...
  /**
   * Add an output dimension to keep. The {@code dimension} applies to the target dimensions of the
   * transform to be given to <code>{@linkplain #separate separate}(transform)</code>. The number
   * must be in the range 0 inclusive to <code>
   * transform.{@linkplain MathTransform#getTargetDimensions getTargetDimensions()}</code>
   * exclusive.
   ...
   *       automatically from the {@linkplain #getSourceDimensions source dimensions} and the {@code
   *       transform}.
   * </ul>
   *
  ...
  /**
   * Separates the specified math transform. This method returns a math transform that uses only the
   * specified {@linkplain #getSourceDimensions source dimensions} and returns only the specified
   ...
   *       returned transform will expects all source dimensions as input but will produces only the
   *       specified {@linkplain #getTargetDimensions target dimensions} as output.
   *   <li>
  ...
  /**
   * Creates a transform which retains only a subset of an other transform's outputs. The number and
   * nature of inputs stay unchanged. For example if the supplied {@code transform} has
   ...
   * concatenated with {@code transform}. However, invoking {@code createFilterTransfom(...)} allows
   * the optimization of some common cases.
   *
   * @param transform The transform to reduces.
  ...
  /**
   * Adds the specified {@code dimension} to the specified sequence. Values are added in increasing
   * order. Duplicated values are not added.
   *
  ...
  /**
   * Adds the specified {@code dimensions} to the specified sequence. Values are added in increasing
   * order. Duplicated values are not added.
   *
   * @param sequence The {@link #sourceDimensions} or {@link #targetDimensions} sequence to update.
   */
  ...
  private static void ensureValidSeries(final int[] dimensions) throws IllegalArgumentException {
    ...
    for (int i = 0; i < dimensions.length; i++) {
      ...
      if (value <= last) {
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "dimensions[" + i + ']', value));
      }
      last = value;
    }
  }
