...
public class OrderedAxisAuthorityFactory extends TransformedAuthorityFactory
  ...
  /**
   * The rank to be given to each axis direction. The rank is stored at the indice corresponding to
   * the direction {@linkplain AxisDirection#ordinal ordinal} value.
   */
  private final int[] directionRanks;

  /**
   * {@code true} if this authority factory should also force the axis to their standard direction.
   * For example if {@code true}, then axis with increasing values toward South will be converted to
   * axis with increasing values toward North. The default value is {@code false}.
   *
  ...
  /**
   * Returns the rank for the specified axis. Any axis that were not specified at construction time
   * will ordered after all known axis.
   */
   ...
   * @return A negative integer if {@code axis1} should appears before {@code axis2}, or a positive
   *     number if {@code axis2} should appears before {@code axis1}, or 0 if the two axis are
   *     unordered one relative to the other.
   * @todo The argument type will be changed to {@link CoordinateSystemAxis} when we will be allowed
  ...
  /**
   * Replaces the specified unit, if applicable. This method is invoked automatically by the {@link
   * #replace(CoordinateSystem)} method. The default implementation replaces the unit only if the
   * {@link Hints#FORCE_STANDARD_AXIS_UNITS FORCE_STANDARD_AXIS_UNITS} hint was specified as {@link
   * Boolean#TRUE TRUE} at construction time. In such case, the default substitution table is:
   *
   * <p>
   *
   * <ul>
   ...
   * substitution table is as specified in the {@link AxisDirection#absolute} method. Subclasses may
   * override this method if they want to use a different substitution table.
   *
   * @since 2.3
   */
  @Override
