...
public abstract class AbstractEnvelope implements Envelope {
   ...
   * containing {@linkplain #getLowerCorner lower corner} coordinates first, followed by {@linkplain
   * #getUpperCorner upper corner} coordinates. Other informations like the CRS or class name may or
   * may not be presents at implementor choice.
   *
   * <p>This string is okay for occasional formatting (for example for debugging purpose). But if
   * there is a lot of envelopes to format, users will get more control by using their own instance
   * of {@link org.geotools.measure.CoordinateFormat}.
   */
  @Override
   ...
   * #getLowerCorner lower corner} coordinates first, followed by {@linkplain #getUpperCorner upper
   * corner} coordinates.
   */
  ...
  /**
   * Base class for direct position from an envelope. This class delegates its work to the enclosing
   * envelope.
   */
