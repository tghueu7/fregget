...
public class AbstractDatum extends AbstractIdentifiedObject implements Datum {
   ...
   * the Earth. Also known as the "origin", especially for Engineering and Image Datums.
   */
   ...
   * unlocalized name (usually in English locale), because the name is part of the elements compared
   * by the {@link #equals} method.
   */
   ...
   *       some cases, the "fundamental point" may consist of a number of points. In those cases,
   *       the parameters defining the geoid/ellipsoid relationship have then been averaged for
   *       these points, and the averages adopted as the datum definition.
   *   <li>For an engineering datum, the anchor point may be a physical point, or it may be a point
   *       with defined coordinates in another CRS.
   ...
   * in the legacy OGC 01-009 specification. In the new OGC 03-73 (ISO 19111) specification, datum
   * type is provided only for vertical datum. Nevertheless, we keep this method around since it is
   * needed for WKT formatting. Note that we returns the datum type ordinal value, not the code list
   * object.
   */
  ...
  public boolean equals(final AbstractIdentifiedObject object, final boolean compareMetadata) {
    if (super.equals(object, compareMetadata)) {
      if (!compareMetadata) {
         ...
         */
        return nameMatches(object.getName().getCode()) || object.nameMatches(getName().getCode());
      }
