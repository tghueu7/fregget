...
public class BursaWolfParameters extends Formattable implements Cloneable, Serializable {
  ...
  /**
   * Returns {@code true} if this Bursa Wolf parameters performs no operation. This is true when all
  ...
  public XMatrix getAffineTransform() {
    ...
    return new Matrix4(
        S, -ez * RS, +ey * RS, dx, +ez * RS, S, -ex * RS, dy, -ey * RS, +ex * RS, S, dz, 0, 0, 0,
        1);
  }
   ...
   * @param eps The tolerance error for the antisymmetric matrix test. Should be a small number like
   *     {@code 1E-4}.
