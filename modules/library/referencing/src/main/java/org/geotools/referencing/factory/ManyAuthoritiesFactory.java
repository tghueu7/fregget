...
public class ManyAuthoritiesFactory extends AuthorityFactoryAdapter
  ...
  /**
   * The types created by {@link #FACTORY_TYPES}. For each type {@code OBJECT_TYPES[i]}, the factory
   * to be used must be {@code FACTORY_TYPES[i]}.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Returns the factories. This method should not be public since it returns directly the internal
   * instance. This method is to be overriden by {@link AllAuthoritiesFactory} only.
   */
  ...
  /**
   * Sets the factories. This method is invoked by the {@link AllAuthoritiesFactory} subclass only.
   * No one else should invoke this method, since factories should be immutable.
   */
   ...
   * {@linkplain GenericName#DEFAULT_SEPARATOR default name separator} {@code ':'}, except if the
   * code looks like a URL (e.g. {@code "http://www.opengis.net/"}), in which case this method
   * returns {@code '/'}.
   *
   * <p>In the current implementation, "looks like a URL" means that the first non-{@linkplain
   * Character#isLetterOrDigit(char) aplhanumeric} characters are {@code "://"}. But this heuristic
   ...
   * <cite>code</cite>) pair at the specified index. The default implementation returns {@code true}
   * if the first non-whitespace character on the left and right side are valid Java identifiers.
   *
   * <p>The purpose of this method is to avoid considering the {@code "//"} part in {@code
   * "http://www.opengis.net/gml/srs/epsg.xml"} as separators. In case of failure to parse the code,
   * this restriction will produce and error message like "<cite>Unknown <code>
  ...
  /**
   * Returns the direct dependencies. Current implementation returns the internal structure because
   * we know that this package will not modifies it. But if the method become public, we will need
   * to returns a unmodifiable view.
   */
  @Override
  ...
  static class Finder extends IdentifiedObjectFinder {
    ...
    protected Finder(
        final ManyAuthoritiesFactory factory, final Class<? extends IdentifiedObject> type) {
      super(factory, type);
    }
      ...
      while (it.hasNext()) {
        ...
        if (factory instanceof AbstractAuthorityFactory) {
          final IdentifiedObjectFinder finder =
              ((AbstractAuthorityFactory) factory).getIdentifiedObjectFinder(getProxy().getType());
          if (finder != null) {
