...
public class ImagingParameterDescriptors extends DefaultParameterDescriptorGroup {
   ...
   * <code>{@linkplain GridCoverage}.class</code>) key-value pair. This is the default argument for
   * wrapping a JAI operation in the {@value
  ...
  /**
   * The JAI's operation descriptor, or {@code null} if none. This is usually an instance of {@link
   ...
   * {@linkplain #getName name for this parameter group} will be inferred from the {@linkplain
   * RegistryElementDescriptor#getName name of the supplied registry element} using the {@link
   * #properties properties} method.
   *
   * <p>The <cite>source type map</cite> default to a (<code>{@linkplain RenderedImage}.class</code>
  ...
  public ImagingParameterDescriptors(final RegistryElementDescriptor operation) {
    this(properties(operation), operation, DEFAULT_SOURCE_TYPE_MAP, RenderedRegistryMode.MODE_NAME);
  }
   ...
   * @param operation The JAI's operation descriptor, usually as an instance of {@link
   *     OperationDescriptor}.
   * @param extension Additional parameters to put in this descriptor, or {@code null} if none. If a
   *     parameter has the same name than an {@code operation} parameter, then the extension
   *     overrides the later.
   * @since 2.4
   */
   ...
   * is non-null, then {@code descriptor} should be derived from it. This constructor is private in
   ...
   * around for RFE #4093999 in Sun's bug database ("Relax constraint on placement of this()/super()
  ...
  /**
   * Creates a new instance of parameter value group. A JAI {@link javax.media.jai.ParameterList} is
   * created for holding parameter values, and wrapped into an {@link ImagingParameters} instance.
   *
