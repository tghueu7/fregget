...
public final class Element {
  ...
  /**
   * Returns {@code true} if the next non-whitespace character is the specified separator. Search is
   * performed in string {@code text} from position {@code position}. If the separator is found,
   * then the position is set to the first character after the separator. Otherwise, the position is
   * set on the first non-blank character.
   *
   * @param text The text to parse.
   * @param position In input, the position where to start parsing from. In output, the first
   *     character after the separator.
   ...
   * @param message The message explaining the cause of the failure, or {@code null} for reusing the
   *     same message than {@code cause}.
  ...
  /**
   * Returns a {@link ParseException} with a "Unparsable string" message. The error message is built
   * from the specified string starting at the specified position. Properties {@link
  ...
  private ParseException missingCharacter(final char c, final int position) {
    return trim(
        "missingCharacter",
        new ParseException(
            complete(Errors.format(ErrorKeys.MISSING_CHARACTER_$1, Character.valueOf(c))),
            position));
  }
  ...
  private ParseException missingParameter(final String key) {
    ...
    return trim(
        "missingParameter",
        new ParseException(complete(Errors.format(ErrorKeys.MISSING_PARAMETER_$1, key)), error));
  }
   ...
   * "GEOGCS["name", DATUM["name, ...]]"}, this is true for {@code "GEOGCS"} and false for all other
   * elements inside, like {@code "DATUM"}.
   *
      ...
      if (object instanceof Number) {
        ...
        if (number instanceof Float || number instanceof Double) {
          throw new ParseException(
              complete(Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, key, number)), offset);
        }
  ...
  /**
   * Removes and returns the next {@link Element} with no bracket. The key is used only for only for
