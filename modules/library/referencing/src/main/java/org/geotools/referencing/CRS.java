...
public final class CRS {
  ...
  /**
   * A factory for CRS creation with (<var>latitude</var>, <var>longitude</var>) axis order (unless
   * otherwise specified in system property). Will be created only when first needed.
   */
  private static CRSAuthorityFactory defaultFactory;

  /**
   * A factory for CRS creation with (<var>longitude</var>, <var>latitude</var>) axis order. Will be
   ...
   * org.geotools.referencing.factory.FallbackAuthorityFactory fallbacks} if there is more than one
   * {@linkplain ReferencingFactoryFinder#getCRSAuthorityFactories registered factory} for the same
   * authority.
   *
   * <p>This factory can be used as a kind of <cite>system-wide</cite> factory for all authorities.
   * However for more determinist behavior, consider using a more specific factory (as returned by
   ...
   * @throws FactoryRegistryException if no {@link CRSAuthorityFactory} implementation was found for
   ...
   *
   * <p>If there is more than one factory for the given authority, then this method merges the code
   * set of all of them. If a factory fails to provide a set of supported code, then this particular
   * factory is ignored. Please be aware of the following potential issues:
   *
   * <p>
   *
   * <ul>
  ...
  /**
   * Returns the set of the authority identifiers supported by registered authority factories. This
   * method search only for {@linkplain CRSAuthorityFactory CRS authority factories}.
   *
   * @param returnAliases If {@code true}, the set will contain all identifiers for each authority.
   *     If {@code false}, only the first one
   ...
   * org.geotools.referencing.factory.BufferedAuthorityFactory cached in a buffer} using {@linkplain
   * java.lang.ref.WeakReference weak references}. Subsequent calls to this method with the same
   * authority code should be fast, unless the CRS object has been garbage collected.
   *
   ...
   * validity} associated with the given CRS. Only {@linkplain GeographicExtent geographic extents}
   * of kind {@linkplain BoundingPolygon bounding polygon} are taken in account. If none are found,
   * then the {@linkplain #getGeographicBoundingBox geographic bounding boxes} are used as a
   * fallback.
   *
   ...
   * <p>This method fetchs the {@linkplain CoordinateReferenceSystem#getDomainOfValidity domain of
   * validity} associated with the given CRS. Only {@linkplain GeographicExtent geographic extents}
   * of kind {@linkplain GeographicBoundingBox geographic bounding box} are taken in account.
   *
   * @param crs The coordinate reference system, or {@code null}.
   ...
   * null} if there is none. A horizontal CRS is usually a two-dimensional {@linkplain GeographicCRS
   * geographic} or {@linkplain ProjectedCRS projected} CRS.
   *
   * @param crs The coordinate reference system, or {@code null}.
  ...
  /**
   * Returns the first temporal coordinate reference system found in the given CRS, or {@code null}
   * if there is none.
   *
   * @param crs The coordinate reference system, or {@code null}.
  ...
  /**
   * Returns the first ellipsoid found in a coordinate reference system, or {@code null} if there is
   * none.
   *
   * @param crs The coordinate reference system, or {@code null}.
  ...
  /**
   * Returns the <cite>Spatial Reference System</cite> identifier, or {@code null} if none. OGC Web
   * Services have the concept of a Spatial Reference System identifier used to communicate CRS
  ...
  public static String toSRS(final CoordinateReferenceSystem crs) {
    ...
    if (crs == DefaultGeographicCRS.WGS84) {
      // if( forcedLonLat ) return "EPSG:4326"; <-- this is a bad idea for interoperability WMS
      // 1.3.0
   ...
   *
   * <p>Some older web services are unable to deal with the full ogc:uri syntax, set simple to true
   * for force a very simple representation that is just based on the code portion.
   *
   * @param crs
   ...
   * object. If such object is found, then its identifier is returned. Otherwise this method returns
   * {@code null}.
   *
   * <p>This convenience method delegates its work to {@link IdentifiedObjectFinder}. Consider using
   * the later if more control are wanted, for example if the search shall be performed only on some
   * {@linkplain AuthorityFactory authority factories} instead of all registered onez, or if the
   * full {@linkplain IdentifiedObject identified object} is wanted instead of only its identifier.
   *
   * @param object The object (usually a {@linkplain CoordinateReferenceSystem coordinate reference
   *     system}) looked up.
   * @param fullScan If {@code true}, an exhaustive full scan against all registered objects will be
   *     performed (may be slow). Otherwise only a fast lookup based on embedded identifiers and
   ...
   * authority.
   *
   * <p>If the CRS does not have an {@linkplain ReferenceIdentifier identifier} which corresponds to
   * the {@linkplain Citations#EPSG EPSG} authority, then:
   *
   * <ul>
  ...
  /**
   * Looks up an EPSG code for the given {@linkplain CoordinateReferenceSystem coordinate reference
   * system}). This is a convenience method for <code>{@linkplain
   ...
   *
   * <p><strong>Don't use this method if there is many envelopes to transform.</strong> This method
   * is provided as a convenience when there is only one envelope to transform between CRS that
   * can't be known in advance. If there is many of them or if the CRS are restricted to known
   * values, get the {@linkplain CoordinateOperation coordinate operation} or {@linkplain
   * MathTransform math transform} once for ever and invoke one of the methods below instead (unless
  ...
  public static GeneralEnvelope transform(
      Envelope envelope, final CoordinateReferenceSystem targetCRS) throws TransformException {
   ...
   *
   * <p>Note that this method can not handle the case where the envelope contains the North or South
   ...
   * use {@link #transform(CoordinateOperation, Envelope)} instead.
   *
  ...
  /**
   * Implementation of {@link #transform(MathTransform, Envelope)} with the opportunity to save the
  ...
  private static GeneralEnvelope transform(
    ...
    if (envelope.getDimension() != sourceDim) {
      throw new MismatchedDimensionException(
          Errors.format(ErrorKeys.MISMATCHED_DIMENSION_$2, sourceDim, envelope.getDimension()));
    }
   ...
   * The transformation is only approximative. It may be bigger than the smallest possible bounding
   * box, but should not be smaller. Note that the returned envelope may not have the same number of
   ...
   */
  public static GeneralEnvelope transform(
      final CoordinateOperation operation, final Envelope envelope) throws TransformException {
    if (envelope == null) {
      return null;
    }
    ...
    if (sourceProjection instanceof PolarStereographic
      ...
      ParameterValue<?> fe =
          sourceProjection
              .getParameterValues()
              .parameter(MapProjection.AbstractProvider.FALSE_EASTING.getName().getCode());
      double originX = fe.doubleValue();
      ParameterValue<?> fn =
          sourceProjection
              .getParameterValues()
              .parameter(MapProjection.AbstractProvider.FALSE_NORTHING.getName().getCode());
   ...
   * South pole, or when it cross the &plusmn;180° longitude, because {@linkplain MathTransform math
   * transforms} do not carry suffisient informations. For a more robust rectangle transformation,
   * use {@link #transform(CoordinateOperation, Rectangle2D, Rectangle2D)} instead.
   *
   ...
   * @param destination The destination rectangle (may be {@code envelope}). If {@code null}, a new
   *     rectangle will be created and returned.
   * @return {@code destination}, or a new rectangle if {@code destination} was non-null and {@code
   *     envelope} was null.
   * @throws TransformException if a transform failed.
   * @since 2.4
  ...
  private static Rectangle2D transform(
    ...
    assert (destination == envelope
            ...
            || XRectangle2D.equalsEpsilon(
                destination, transform(transform, new Envelope2D(null, envelope)).toRectangle2D())
        : destination;
    return destination;
  }
  ...
  public static Rectangle2D transform(
    ...
    if (destination == null) {
      ...
      destination.setFrame(
          result.getMinimum(0), result.getMinimum(1), result.getSpan(0), result.getSpan(1));
      return destination;
    }
  }
  ...
  /**
   * Resets some aspects of the referencing system. The aspects to be reset are specified by a space
   * or comma delimited string, which may include any of the following elements:
   *
   * <p>
   *
   * <ul>
  ...
  /**
   * Prints to the {@linkplain System#out standard output stream} some information about {@linkplain
   * CoordinateReferenceSystem coordinate reference systems} specified by their authority codes.
   * This method can be invoked from the command line in order to test the {@linkplain
   * #getAuthorityFactory authority factory} content for some specific CRS.
   *
   ...
   * Uses the specified authority factory, for example {@code "EPSG"}. The authority name can be any
   * of the authorities listed by the {@code -factories} option. If this option is not specified,
   * then the default is all factories.
   *
   ...
   * most of the elements relevant to the {@link #equalsIgnoreMetadata equalsIgnoreMetadata} method,
   * with the addition of Bursa-Wolf parameters.
   *
   ...
   * may improve the output quality. This is not needed on Linux terminal using UTF-8 encoding (tip:
   * the <cite>terminus font</cite> gives good results).
   *
   ...
   * operations explicitly defined in a database like EPSG. There is sometime many such operations,
   * and sometime none (in which case this option prints nothing - it doesn't try to find an
   * operation by itself).
   *
   ...
   * "-operations"} option, this option pick up only one operation (usually the most accurate one),
   * inferring it if none were explicitly specified in the database.
   *
   * </blockquote>
   *
   ...
   * Prints the "Luxembourg 1930" CRS, the "WGS 84" CRS (from EPSG database), the ""WGS84" CRS (from
   * the <cite>Web Map Service</cite> specification) and a UTM projection in WKT format.
   *
   ...
   * Note that for this particular pair of CRS, there is close to 40 operations declared in the EPSG
   * database. This method prints only the ones that Geotools can handle.
   *
   * <p><b>{@code CRS -transform EPSG:4230 EPSG:4326}</b><br>
   * Prints the math transform that Geotools would use by default for coordinate transformation from
   * "ED50" to "WGS 84".
   *
   * </blockquote>
   *
