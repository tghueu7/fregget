...
public class ProjectiveTransformBuilder extends MathTransformBuilder {
   ...
   * Subclasses like {@linkplain AffineTransformBuilder affine transform builders} will reduce this
   * minimum.
   *
  ...
  protected void fillPMatrix() throws MissingInfoException {
    ...
    for (int i = 0; i < getMappedPositions().size(); i = i + 2) {
      if (Double.compare((((MappedPosition) getMappedPositions().get(i)).getAccuracy()), Double.NaN)
          == 0) {
  ...
  /**
   * Returns the matrix of parameters for Projective transformation. This method should by override
   * for the special cases like affine or similar transformation. The M matrix looks like this:
   *
   * <pre>
   *
   *  [  m00  m01  m02  ]
