...
final class URN_Parser extends URI_Parser {
   ...
   * All elements in the array are treated as synonymous. Those parts are up to, but do not include,
   * the type part ({@code "crs"}, {@code "cs"}, {@code "datum"}, <cite>etc.</cite>). They must
   * include a trailing (@value #URN_SEPARATOR} character.
   */
