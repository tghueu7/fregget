...
public abstract class MathTransformProvider extends DefaultOperationMethod {
   ...
   * DefaultOperationMethod#DefaultOperationMethod(Map,int,int,ParameterDescriptorGroup) super-class
   * constructor}.
   *
  ...
  /**
   * Constructs an optional parameter descriptor from a set of alias. The parameter is identified as
   * with {@link #createDescriptor}.
   *
   * @param identifiers The parameter identifiers. Most contains at least one entry.
   * @param minimum The minimum parameter value, or {@link Double#NEGATIVE_INFINITY} if none.
   * @param maximum The maximum parameter value, or {@link Double#POSITIVE_INFINITY} if none.
   ...
   * then an {@link InvalidParameterValueException} is thrown.
   *
   * @param values The parameters values to check.
   * @return The parameter values to use for {@linkplain MathTransform math transform} construction.
   *     May be different than the supplied {@code values} argument if some missing values needed to
  ...
  /**
   * Implementation of {@code ensureValidValues}, to be invoked recursively if the specified values
   * contains sub-groups of values. This method copy all values from the user-supplied parameter
   * values into the provider-supplied one. The provider one should understand alias, and performs
   * name conversion as well as argument checking on the fly.
   *
   ...
   * This class encapsulates a reference to the {@linkplain #method originating provider} as well as
   * the {@linkplain #transform created math transform}. This information is needed when a provider
   * delegates the work to an other provider according the parameter values. For example a generic
   * instance of {@link
   * org.geotools.referencing.operation.transform.ProjectiveTransform.ProviderAffine ProviderAffine}
   * may delegates the creation of an <cite>affine transform</cite> to an other {@code
   * ProviderAffine} instance with <cite>source</cite> and <cite>target</cite> dimensions matching
   * the supplied parameters, because those dimensions determine the set of legal <code>
