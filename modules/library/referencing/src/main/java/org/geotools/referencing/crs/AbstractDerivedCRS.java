...
public class AbstractDerivedCRS extends AbstractSingleCRS implements GeneralDerivedCRS {
  ...
  /**
   * Key for the <code>{@value}</code> property to be given to the constructor. The value should be
   * one of <code>{@linkplain org.opengis.referencing.operation.PlanarProjection}.class</code>,
   ...
   * projection} of proper type from a {@linkplain DefiningConversion defining conversion}. In many
   * cases, this hint is not needed since Geotools is often capable to infer it. This hint is used
   * mostly by advanced factories like the {@linkplain org.geotools.referencing.factory.epsg EPSG
   * backed} one.
   *
  ...
  /**
   * A lock for avoiding never-ending recursivity in the {@code equals} method. This field contains
   * a {@code boolean} flag set to {@code true} when a comparaison is in progress. This lock is
   * necessary because {@code AbstractDerivedCRS} objects contain a {@link #conversionFromBase}
   * field, which contains a {@link DefaultConversion#targetCRS} field set to this {@code
   * AbstractDerivedCRS} object.
   *
   * <p><STRONG>DO NOT USE THIS FIELD. It is strictly for internal use by {@link #equals} and {@link
   * org.geotools.referencing.operation.AbstractCoordinateOperation#equals} methods.</STRONG>
   *
   * @todo Hide this field from the javadoc. It is not possible to make it package-privated because
   *     {@link org.geotools.referencing.operation.AbstractCoordinateOperation} lives in a different
   *     package.
   */
  ...
  /**
   * Constructs a new derived CRS with the same values than the specified one. This copy constructor
  ...
  /**
   * Constructs a derived CRS from a set of properties. A {@linkplain DefaultOperationMethod default
   ...
   * specified with the <code>"conversion."</code> prefix added in front of property names (example:
   * <code>"conversion.remarks"</code>). The same applies for operation method, using the <code>
   * "method."</code> prefix.
   *
