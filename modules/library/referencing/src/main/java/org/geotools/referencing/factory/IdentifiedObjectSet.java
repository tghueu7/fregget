...
public class IdentifiedObjectSet extends AbstractSet implements Serializable {
  ...
  /**
   * Returns the number of objects available in this set. Note that this number may decrease during
   * the iteration process if the creation of some {@linkplain IdentifiedObject identified objects}
   * failed.
   */
  public int size() {
    return objects.size();
  ...
  /**
   * Returns an iterator over the objects in this set. If the iteration encounter any kind of {@link
   * FactoryException} other than {@link NoSuchIdentifierException}, then the exception will be
   * rethrown as an unchecked {@link BackingStoreException}.
   */
   ...
   * this method will preserve the corresponding {@linkplain IdentifiedObject identified object} if
   * it was already created. Other objects will be {@linkplain #createObject created} only when
   ...
   * iteration order on the basis of authority codes. If the specified {@code codes} array contains
   * the same elements than {@link #getAuthorityCodes} in a different order, then this method just
  ...
  /**
   * Returns the code to uses as a key for the specified object. The default implementation returns
   * the code of the first {@linkplain IdentifiedObject#getIdentifiers identifier}, if any, or the
   * code of the{@linkplain IdentifiedObject#getName primary name} otherwise. Subclasses may
   * overrides this method if they want to use a different key for this set.
   */
  ...
  /**
   * Creates an object for the specified authority code. This method is invoked during the iteration
   * process if an object was not already created. The default implementation invokes <code>
   * {@linkplain #factory}.{@link AuthorityFactory#createObject createObject}(code)</code>.
   ...
   * object. If this method returns {@code true} for the given exception, then the exception will be
   * logged in the {@linkplain AbstractAuthorityFactory#LOGGER Geotools factory logger} with the
   * {@link Level#FINE FINE} level. If this method returns {@code false}, then the exception will be
   * retrown as a {@link BackingStoreException}. The default implementation returns {@code true}
   * only for {@link NoSuchIdentifierException} (not to be confused with {@link
  ...
  /**
   * The iterator over the entries in the enclosing set. This iterator will creates the {@linkplain
   * IdentifiedObject identified objects} when first needed.
   *
   * @version $Id$
   * @author Martin Desruisseaux (IRD)
   */
