...
public class AbstractCS extends AbstractIdentifiedObject implements CoordinateSystem {
    ...
    for (int i = 0; i < axis.length; i++) {
      ...
      if (!isCompatibleDirection(direction)) {
        ...
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.ILLEGAL_AXIS_ORIENTATION_$2, direction.name(), getClass()));
      }
   ...
   * implementation returns {@code true} in all cases. Subclasses can override this method and check
   * for compatibility with {@linkplain SI#METER meter} or {@linkplain NonSI#DEGREE_ANGLE degree}
   * units.
   *
   ...
   * transforming from ({@linkplain AxisDirection#NORTH NORTH},{@linkplain AxisDirection#WEST WEST})
   * to ({@linkplain AxisDirection#EAST EAST},{@linkplain AxisDirection#NORTH NORTH} are taken in
   * account.
   *
   * <p><b>Example:</b> If coordinates in {@code sourceCS} are (<var>x</var>,<var>y</var>) pairs in
   * metres and coordinates in {@code targetCS} are (-<var>y</var>,<var>x</var>) pairs in
      ...
      for (int i = 0; i < sourceDim; i++) {
        ...
        if (!converter.isLinear()) {
          throw new ConversionException(
              Errors.format(ErrorKeys.NON_LINEAR_UNIT_CONVERSION_$2, sourceUnit, targetUnit));
        }
  ...
  /**
   * Returns a coordinate system with "standard" axis order and units. Most of the time, this method
   * returns one of the predefined constants with axis in (<var>longitude</var>,<var>latitude</var>)
   * or (<var>X</var>,<var>Y</var>) order, and units in degrees or metres. In some particular cases
   * like {@linkplain org.opengis.referencing.cs.CartesianCS Cartesian CS}, this method may create a
   * new instance on the fly. In every cases this method attempts to return a <A
  ...
  /**
   * Suggests an unit for measuring distances in this coordinate system. The default implementation
   * scans all {@linkplain CoordinateSystemAxis#getUnit axis units}, ignoring angular ones (this
   * also implies ignoring {@linkplain Unit#ONE dimensionless} ones). If more than one non-angular
   * unit is found, the default implementation returns the "largest" one (e.g. kilometers instead of
   * meters).
   *
  ...
  final void ensureDimensionMatch(final String name, final double[] coordinates)
    ...
    if (coordinates.length != axis.length) {
      throw new MismatchedDimensionException(
          Errors.format(ErrorKeys.MISMATCHED_DIMENSION_$3, name, coordinates.length, axis.length));
    }
  }

  /**
   * Computes the distance between two points. This method is not available for all coordinate
   * systems. For example, {@linkplain DefaultEllipsoidalCS ellipsoidal CS} doesn't have suffisient
   * information.
   *
   ...
   * DefaultCoordinateSystemAxis}. This allow usage of some methods specific to that implementation.
   */
   ...
   * this coordinate system. The comparaison is insensitive to axis order and units. What matter is
   * axis names (because they are fixed by ISO 19111 specification) and directions.
   *
   ...
   *
   * <p>This method should not be public because current implementation is not fully consistent for
   * every pair of CS. It tries to check the opposite direction in addition of the usual one, but
   * only a few pre-defined axis declare their opposite. This method should be okay when invoked on
   * pre-defined CS declared in this package. {@link PredefinedCS} uses this method only that way.
   */
