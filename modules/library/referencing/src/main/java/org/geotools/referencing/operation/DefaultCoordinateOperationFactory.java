...
public class DefaultCoordinateOperationFactory extends AbstractCoordinateOperationFactory {
    ...
    protected CoordinateOperation createOperationStep(final GeographicCRS sourceCRS,
        ...
        if (molodenskiMethod != null) {
            ...
            if (sourceDatum instanceof DefaultGeodeticDatum) {
                bursaWolf = ((DefaultGeodeticDatum) sourceDatum).getBursaWolfParameters
                        (targetDatum);
            }
            if (bursaWolf == null) {
                /*
                 ...
                 */
                final Matrix shift = DefaultGeodeticDatum.getAffineTransform(sourceDatum, 
                        targetDatum);
                if (shift != null) try {
            ...
            if (bursaWolf != null && bursaWolf.isTranslation()) {
                ...
                if (bursaWolf.isIdentity() && equalsIgnoreMetadata(sourceEllipsoid, 
                        targetEllipsoid)) {
                    final Matrix matrix = swapAndScaleAxis(sourceCS, targetCS, sourcePM, targetPM);
                ...
                if (sourceDim == targetDim) {
                    ...
                    step2 = createFromParameters(identifier, normSourceCRS, normTargetCRS, 
                            parameters);
    ...
    private CoordinateOperation createOperationStep(final CoordinateReferenceSystem sourceCRS,
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            if (lower != 0 || upper != dimensions) {
                ...
                mt = getMathTransformFactory().createPassThroughTransform(lower, mt, dimensions -
                        upper);
     ...
     * </ul>
     */
    private static boolean needsGeodetic3D(final List<SingleCRS> sourceCRS, final SingleCRS 
            targetCRS) {
