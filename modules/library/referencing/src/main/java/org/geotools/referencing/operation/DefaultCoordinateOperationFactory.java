...
public class DefaultCoordinateOperationFactory extends AbstractCoordinateOperationFactory {
   ...
   * If an operation exists, it is returned. If more than one operation exists, the default is
   * returned. If no operation exists, then the exception is thrown.
   *
   ...
   *     available operations. Use 1 to return just one operation. Currently, the behavior for other
   *     values of {@code limit} is undefined.
   ...
   * those conditions, then it is returned unchanged. Otherwise, a new normalized geographic CRS is
  ...
  /**
   * Returns an affine transform between two ellipsoidal coordinate systems. Only units, axis order
   * (e.g. transforming from (NORTH,WEST) to (EAST,NORTH)) and prime meridian are taken in account.
   * Other attributes (especially the datum) must be checked before invoking this method.
   *
   ...
   * ellipsoid}. More elaborated operation, like transformation from ellipsoidal to geoidal height,
  ...
  protected CoordinateOperation createOperationStep(
    ...
    if (VerticalDatumType.ELLIPSOIDAL.equals(targetCRS.getDatum().getVerticalDatumType())) {
      final Matrix matrix =
          swapAndScaleAxis(sourceCRS.getCoordinateSystem(), targetCRS.getCoordinateSystem());
      return createFromAffineTransform(AXIS_CHANGES, sourceCRS, targetCRS, matrix);
    }
    throw new OperationNotFoundException(getErrorMessage(sourceCRS, targetCRS));
  }
   ...
   * default implementation can adjust axis order and orientation. It also performs units conversion
   * if it is the only extra change needed. Otherwise, it performs three steps:
   *
   * <ul>
   *   <li>Unproject from {@code sourceCRS} to its base {@linkplain GeographicCRS geographic CRS}.
   *   <li>Convert the source to target base geographic CRS.
  ...
  /**
   * Creates an operation from a geographic to a projected coordinate reference system. The default
  ...
  /**
   * Returns all available operations from a geographic to a projected coordinate reference system.
  ...
  /**
   * Creates an operation from a projected to a geographic coordinate reference system. The default
   * implementation constructs the following operation chain:
   *
   * <blockquote>
   *
   ...
   *
   * <p><strong>NOTE:</strong> If there is a horizontal (geographic or projected) CRS together with
   * a vertical CRS, then we can't performs the transformation since the vertical value has an
   * impact on the horizontal value, and this impact is not taken in account if the horizontal and
   * vertical components are not together in a 3D geographic CRS. This case occurs when the vertical
   * CRS is not a height above the ellipsoid. It must be checked by the caller before this method is
   * invoked.
   *
   * @param sourceCRS Input coordinate reference system.
  ...
  private CoordinateOperation createOperationStep(
    ...
    if (!select.isIdentity()) {
      if (ordered.length == 1) {
        ...
        sourceStepCRS =
            factories.getCRSFactory().createCompoundCRS(getTemporaryName(sourceCRS), ordered);
      }
    ...
    for (int i = 0; i < targets.size(); i++) {
      ...
      if (i >= count) {
        ...
        targetStepCRS =
            factories.getCRSFactory().createCompoundCRS(getTemporaryName(target), ordered);
      }
      lower = upper;
      upper += getDimension(source);
   ...
   *       generic method above. An exception to this rule occurs when the target datum is used in a
   ...
   *
   * <p>The default implementation always returns {@code null}, since there is no database connected
   * to a {@code DefaultCoordinateOperationFactory} instance. In other words, the default
   * implementation is "standalone": it tries to figure out transformation paths by itself.
   * Subclasses should override this method if they can fetch a more accurate operation from some
   * database. The mean subclass doing so is {@link AuthorityBackedFactory}.
   *
   ...
   * operations). This method is <strong>not</strong> invoked for synthetic CRS generated by {@code
   * createOperationStep(...)}, since those temporary CRS are not expected to exist in a database.
   *
   * @param sourceCRS Input coordinate reference system.
   * @param targetCRS Output coordinate reference system.
   ...
   * connected to a {@code DefaultCoordinateOperationFactory} instance. In other words, the default
   * implementation is "standalone": it tries to figure out transformation paths by itself.
   * Subclasses should override this method if they can fetch a more accurate operation from some
   * database. The mean subclass doing so is {@link AuthorityBackedFactory}.
   *
   ...
   *     available operations. Use 1 to return just one operations. Currently, the behavior for
   *     other values of {@code limit} is undefined.
   * @return A set of coordinate operations from {@code sourceCRS} to {@code targetCRS} if and only
   *     if one is explicitly defined in some underlying database, or an empty {@link Set}
   *     otherwise.
   * @since 2.3
   */
