...
public class DatumAliases extends ReferencingFactory implements DatumFactory {
   ...
   * values are initially {@code String[]} objects. They are converted to {@code GenericName[]} only
   * when first needed.
   */
  private final Map<String, Object[]> aliasMap = new HashMap<String, Object[]>();

  /**
   * The authorities. This is the first line in the alias table. This array is constructed by {@link
   * #reload} when first needed.
   */
  private LocalName[] authorities;

  /**
   * The underlying datum factory. If {@code null}, a default factory will be fetch from {@link
   * ReferencingFactoryFinder} when first needed. A default value can't be set at construction time,
   * since all factories may not be registered at this time.
   */
  ...
  /**
   * Returns the backing datum factory. If no factory were explicitly specified by the user, selects
   * the first datum factory other than {@code this}.
   *
   ...
   * search them, and invoking this method in the constructor would prematurely ask an other {@link
   * DatumFactory} instance while the list is incomplete. Instead, we will invoke this method when
   * the first {@code createXXX} method is invoked, which typically occurs after all factories have
   * been initialized.
   *
  ...
  /**
   * Read again the "{@code DatumAliasesTable.txt}" file into {@link #aliasMap}. This method may be
   * invoked more than once in order to reload entries that have been discarted by {@link
  ...
  public synchronized ImageDatum createImageDatum(
      final Map<String, ?> properties, final PixelInCell pixelInCell) throws FactoryException {
  ...
  public synchronized Ellipsoid createFlattenedSphere(
      ...
      throws FactoryException {
    return getDatumFactory()
        .createFlattenedSphere(addAliases(properties), semiMajorAxis, inverseFlattening, unit);
  }
