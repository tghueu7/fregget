...
public class DefaultGeodeticDatum extends AbstractDatum implements GeodeticDatum {
  ...
  public DefaultGeodeticDatum(final GeodeticDatum datum) {
    ...
    bursaWolf =
        (datum instanceof DefaultGeodeticDatum) ? ((DefaultGeodeticDatum) datum).bursaWolf : null;
  }
  ...
  /**
   * Constructs a geodetic datum from a set of properties. The properties map is given unchanged to
   * the {@linkplain AbstractDatum#AbstractDatum(Map) super-class constructor}. Additionally, the
  ...
  /**
   * Returns Bursa Wolf parameters for a datum shift toward the specified target, or {@code null} if
   * none. This method search only for Bursa-Wolf parameters explicitly specified in the {@code
   * properties} map at construction time. This method doesn't try to infer a set of parameters from
   * indirect informations. For example it doesn't try to inverse the parameters specified in the
   * {@code target} datum if none were found in this datum. If such an elaborated search is wanted,
   * use {@link #getAffineTransform} instead.
   */
   ...
   * @param source The source datum.
   * @param target The target datum.
   * @param exclusion The set of datum to exclude from the search, or {@code null}. This is used in
   *     order to avoid never-ending recursivity.
   * @return An affine transform from {@code source} to {@code target}, or {@code null} if none.
   * @see BursaWolfParameters#getAffineTransform
   */
  ...
  /**
   * Returns {@code true} if the specified object is equals (at least on computation purpose) to the
   * {@link #WGS84} datum. This method may conservatively returns {@code false} if the specified
   * datum is uncertain (for example because it come from an other implementation).
   */
   ...
   * #getRemarks remarks} and the like are not taken in account. In other words, two geodetic datums
  ...
  public int hashCode() {
    int code =
        (int) serialVersionUID
            ^ 37
                * (super.hashCode()
                    ^ 37 * (ellipsoid.hashCode() ^ 37 * (primeMeridian.hashCode())));
    return code;
  }
