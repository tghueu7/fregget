...
public class DefaultCartesianCS extends DefaultAffineCS implements CartesianCS {
   ...
   * <var>{@linkplain DefaultCoordinateSystemAxis#Y y}</var>, <var>{@linkplain
   ...
   * display x}</var>, <var>{@linkplain DefaultCoordinateSystemAxis#DISPLAY_Y display y}</var> axis.
   *
   * @since 2.2
   */
      ...
      for (int j = i; ++j < dimension; ) {
        ...
        if (Math.abs(Math.abs(angle) - 90) > DirectionAlongMeridian.EPS) {
          throw new IllegalArgumentException(
              Errors.format(ErrorKeys.NON_PERPENDICULAR_AXIS_$2, axis0.name(), axis1.name()));
        }
      }
    }
  }
