...
public class AdvancedAffineBuilder extends MathTransformBuilder {
  ...
  /**
   * Sets constrain that {@code param} is equal to {@code value}. Be aware that the calculation may
   * diverge in the case you set some values that are not 'close' to approximate values. I the case
   * of divergence you can set approximate values using proper constructor
   *
  ...
  /**
   * Generates A matrix (Matrix of derivation of affine equation). Each column is derivation by each
   * transformation parameter (sx, sy, sxy, phi, tx,ty). The rows are derivations of fx and fy.
   *
   * @return A matrix
   */
  ...
  private GeneralMatrix createAB(GeneralMatrix ATA, GeneralMatrix B) {
    ...
    GeneralMatrix AAB =
        new GeneralMatrix(ATA.getNumRow() + B.getNumRow(), ATA.getNumCol() + BT.getNumCol());
   ...
   * iteration steps is still bigger than required then Exception is thrown. This is not the number
   * that was really needed for iteration.
   *
