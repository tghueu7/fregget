...
public class AdvancedAffineBuilder extends MathTransformBuilder {
    ...
    /**
     * Sets constrain that {@code param} is equal to {@code value}.  Be aware that the 
     * calculation may diverge in the case you set some values
     ...
     * each transformation parameter (sx, sy, sxy, phi, tx,ty). The rows are derivations of fx 
     * and fy.
     *
     * @return A matrix
     */
    ...
    private GeneralMatrix getDxMatrix(double tolerance, int maxSteps)
        ...
        do {
            ...
            if (i > maxSteps) { //&& oldDxMatrix.getElement(0, 0) < dxMatrix.getElement(0, 0)){   
                ...
                throw new FactoryException("Calculation of transformation is divergating - try to" +
                        " set proper aproximate values");
            }
