...
public class BufferedCoordinateOperationFactory extends AbstractCoordinateOperationFactory
  ...
  private static final class CRSPair {
     ...
     * {@code equalsIgnoreMetadata}, because metadata matter since they are attributes of the {@link
     * CoordinateOperation} object to be created.
     */
    @Override
   ...
   * initialize this field using {@link ReferencingFactoryFinder} from the no-argument constructor,
   * since this constructor is typically invoked while {@link ReferencingFactoryFinder} is still
   * iterating over the registered implementations.
   */
   ...
   * different for the same ({@code sourceCRS}, {@code targetCRS}) pair dependending of hint values
   * like {@link Hints#LENIENT_DATUM_SHIFT}.
   */
   ...
   * If an operation was already created and still in the cache, the cached operation is returned.
   * Otherwise the operation creation is delegated to the {@linkplain CoordinateOperationFactory
   * coordinate operation factory} specified at construction time and the result is cached.
   *
   ...
   * CoordinateOperationFactory coordinate operation factory} specified at construction time and the
  ...
  /**
   * Returns an operation for conversion or transformation between two coordinate reference systems
   * using the specified method. The current implementation delegates to the {@linkplain
   * CoordinateOperationFactory coordinate operation factory} specified at construction time with no
   * caching.
   *
