...
public class DefaultConversion extends DefaultOperation implements Conversion {
   ...
   * projection}. This type is inferred from the {@code conversion} argument when possible. However
   * the inferred type is not always the most accurate one, so an optional {@code typeHint} argument
   * may be specified in order to get a more specific subclass. This later argument is just a hint:
   * it may be {@code null} and will be ignored if it conflict with the automatically inferred type.
   *
   * @param definition The defining conversion.
   * @param sourceCRS The source CRS.
   * @param targetCRS The target CRS.
   ...
   *     {@linkplain CylindricalProjection}.class</code> or <code>{@linkplain ConicProjection}.class
   *     </code>, or {@code null}.
