...
public class GeometryBuilder {
  ...
  public PointArray createPointArray(double[] array) {
    return getPositionFactory()
        .createPointArray(array, 0, array.length / crs.getCoordinateSystem().getDimension());
  }
  ...
  public Curve createCurve(List segments)
    ...
    if (segments == null) throw new NullPointerException("Segments are required to create a curve");
  ...
  public Point createPoint(double[] ordinates) throws MismatchedDimensionException {
    if (ordinates == null) throw new NullPointerException("Ordinates required to create a point");
    int dimension = this.getCoordinateReferenceSystem().getCoordinateSystem().getDimension();
    if (ordinates.length != dimension)
      throw new MismatchedDimensionException(
          "Create point requires " + dimension + " ordinates (" + ordinates.length + " provided");
  ...
  public Point createPoint(Position position)
    ...
    DirectPosition copy =
        (DirectPosition)
            getPositionFactory().createDirectPosition(position.getDirectPosition().getCoordinate());
  ...
  public Ring createRing(List<OrientableCurve> orientableCurves)
      throws MismatchedReferenceSystemException, MismatchedDimensionException {
    /**
     * Creates a Ring from triple Array of DirectPositions (Array of arrays, which each represent a
     * future Curve. Each array contain an array of positions, which each represent a future
     * lineString)
     */
