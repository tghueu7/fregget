...
public class AuthorityBackedFactory extends DefaultCoordinateOperationFactory
  ...
  /**
   * Makes sure that every {@code FORCE_*} hints are set to false. We do that because we want {@link
   * CoordinateOperationAuthorityFactory#createFromCoordinateReferenceSystemCodes} to returns
   * coordinate operations straight from the EPSG database; we don't want an instance like {@link
   * org.geotools.referencing.factory.OrderedAxisAuthorityFactory}. Axis swapping are performed by
   * {@link #createFromDatabase} in this class <strong>after</strong> we invoked {@link
   * CoordinateOperationAuthorityFactory#createFromCoordinateReferenceSystemCodes}. An {@code
   * OrderedAxisAuthorityFactory} instance in this class would be in the way and cause an infinite
   * recursivity.
   *
   ...
   *
   * <p>Note that this method may be invoked recursively. For example no operation may be available
   * from the {@linkplain #getAuthorityFactory underlying authority factory} between two {@linkplain
   * org.opengis.referencing.crs.CompoundCRS compound CRS}, but an operation may be available
   * between two components of those compound CRS.
   *
   * @param sourceCRS Input coordinate reference system.
   * @param targetCRS Output coordinate reference system.
   * @return A coordinate operation from {@code sourceCRS} to {@code targetCRS}, or {@code null} if
   *     no such operation is explicitly defined in the underlying database.
   * @since 2.3
   */
  @Override
   ...
   * @return A set of coordinate operations from {@code sourceCRS} to {@code targetCRS}, or an empty
   *     {@code Set} if no operation is explicitly defined in the underlying database for that CRS
   *     pair.
   * @since 19
   */
  @Override
  protected Set<CoordinateOperation> findFromDatabase(
    ...
    try {
      operations =
          authorityFactory.createFromCoordinateReferenceSystemCodes(sourceCode, targetCode);
      ...
      if (inverse) {
         ...
         */
        operations =
            authorityFactory.createFromCoordinateReferenceSystemCodes(targetCode, sourceCode);
      }
   ...
   * code.
   *
   * <p>This method is used in order to change axis order when the user-specified CRS disagree with
   * the authority-supplied CRS.
   *
   ...
   * @return A new operation, or {@code operation} if {@code prepend} and {@code append} were nulls
   *     or identity transforms.
