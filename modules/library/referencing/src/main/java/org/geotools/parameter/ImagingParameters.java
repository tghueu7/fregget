...
public class ImagingParameters extends AbstractParameter implements ParameterValueGroup {
   ...
   * <strong>not</strong> including {@linkplain ParameterBlockJAI#getSources sources}) are actually
   * stored in this list.
   *
   * <p>If the {@linkplain ImagingParameterDescriptors#descriptor JAI descriptor} is an instance of
   * {@link OperationDescriptor}, then this parameter list is also an instance of {@link
  ...
  /**
   * The wrappers around each elements in {@link #parameters} as an immutable list. Will be created
   * by {@link #createElements} only when first needed. Note that while this list may be immutable,
   * <strong>elements</strong> in this list stay modifiable. The goal is to allows the following
   * idiom:
   *
   * <blockquote>
   *
   ...
   * descriptor. Note that the JAI descriptor is allowed to be less strict than the OGC one. This is
   * okay because {@link ImagingParameter} will keep a reference to the stricter OGC descriptor,
   * which can be used for performing a strict check if we wish.
   *
   ...
   * @param types The array returned by {@code listDescriptor.getParamClasses()}, obtained once for
   *     ever by the caller for efficienty.
   ...
   * inconditionnally and most not requires synchronization for proper working of the {@link #clone}
   * method.
   *
   * @return The array which is backing {@link #values}. This array is returned only in order to
   *     allow {@link #clone} to modify the values right after the clone. In other cases, this array
   *     should be discarted.
   */
        ...
        if (i != j) {
          ...
          if (AbstractIdentifiedObject.nameMatches(d, name)) {
            throw new InvalidParameterNameException(
                Errors.format(ErrorKeys.PARAMETER_NAME_CLASH_$4, d.getName().getCode(), j, name, i),
                name);
          }
        }
      }
   ...
   * parameters found in the {@linkplain #parameters underlying parameter list}. In addition, it may
   * contains sources found in the JAI's {@linkplain OperationDescriptor operation descriptor}.
   */
