...
final class ProjectionAnalyzer {
   ...
   * {@link MapProjection} implementation, this is the axis swapping and scaling needed in order to
   * get standard (<var>longitude</var>,<var>latitude</var>) axis in degrees. Can be {@code null} if
   * none.
   *
  ...
  /**
   * The affine transform applied on projected coordinates after the projection. In Geotools {@link
   * MapProjection} implementation, this is the axis swapping and scaling needed in order to get
   * standard (<var>x</var>,<var>y</var>) axis in metres. Can be {@code null} if none.
   */
   ...
   * the same projection than {@link #transform}. The normalized projection is a imaginary transform
   * (we don't have a {@link MathTransform} instance for it, but we don't need) with {@code "scale
   * factor"} == 1, {@code "false easting"} == 0 and {@code "false northing"} == 0. In other words,
  ...
  private XMatrix normalizedToProjection() {
    parameters =
        new LinkedList<GeneralParameterValue>(parameters); // Keep the original list unchanged.
    /*
    ...
    if (warning != null) {
      final LogRecord record =
          Loggings.format(
              Level.WARNING, LoggingKeys.APPLIED_UNIT_CONVERSION_$3, warning, unit, SI.METER);
  ...
  /**
   * Checks if the parameter in the two specified list contains the same values. The order parameter
   * order is irrelevant. The common parameters are removed from both lists.
   */
  ...
  /**
   * Returns a conversion from a source to target projected CRS, if this conversion is representable
   * as an affine transform. If no linear conversion has been found between the two CRS, then this
