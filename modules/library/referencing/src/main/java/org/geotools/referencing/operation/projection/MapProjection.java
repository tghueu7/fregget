...
public abstract class MapProjection extends AbstractMathTransform
   ...
   * enabled. This allows tests to work in "real world" conditions, where users often ask for points
   * that are way to far from the correct area of usage of projections
   */
  ...
  /**
   * Maximum difference allowed when comparing longitudes or latitudes in degrees. This tolerance do
   * not apply to angle in radians. A tolerance of 1E-4 is about 10 kilometers.
   */
  ...
  /**
   * The square of excentricity: e² = (a²-b²)/a² where <var>e</var> is the {@linkplain #excentricity
   * excentricity}, <var>a</var> is the {@linkplain #semiMajor semi major} axis length and
  ...
  /**
   * Length of semi-major axis, in metres. This is named '<var>a</var>' or '<var>R</var>' (Radius in
   * spherical cases) in Snyder.
   *
   * @see #excentricity
   * @see #semiMinor
   */
  ...
  /**
   * When different than {@link #globalRangeCheckSemaphore}, coordinate ranges will be checked and a
   * {@code WARNING} log will be issued if they are out of their natural ranges (-180/180&deg; for
   * longitude, -90/90&deg; for latitude).
   *
   ...
   * expected.contains(param)}. Some subclasses will override this method in order to handle {@link
  ...
  /**
   * Ensures that this projection has equals semi-major and semi-minor axis. This method is invoked
   * by constructors of classes implementing only spherical formulas.
   */
   ...
   * tolerance value. The expected value is usually either {@code 0} or {@code PI/2} (the equator or
   * a pole).
   *
  ...
  /**
   * Ensures that the longitue is within allowed limits (&plusmn;&pi;). This method is used to check
   * the validity of projection parameters, like {@link #centralMeridian}.
   *
  ...
  /**
   * Verifies if the given coordinates are in the range of geographic coordinates. If they are not,
   * then this method logs a warning and returns {@code true}. Otherwise this method does nothing
  ...
  private static boolean verifyGeographicRanges(
    ...
    yOut =
        (y < (Latitude.MIN_VALUE - ANGLE_TOLERANCE) || y > (Latitude.MAX_VALUE + ANGLE_TOLERANCE));
    if (!xOut && !yOut) {
      return false;
    }
   ...
   * #getParameterValues} implementation. Values are automatically converted from radians to decimal
   * degrees if needed.
   *
   * @param expected The value returned by {@code getParameterDescriptors().descriptors()}.
  ...
  /**
   * Check if the transform of {@code point} is close enough to {@code target}. "Close enough" means
   * that the two points are separated by a distance shorter than {@link
  ...
  /**
   * Checks if inverse transform using spherical formulas produces the same result than ellipsoidal
   * formulas. This method is invoked during assertions only.
   *
  ...
  /**
   * Compares two value for equality up to some tolerance threshold. This is used during assertions
   ...
   * "longitude" and "latitude"), then the value is assumed to be an angle in radians. This is used
   * for formatting an error message, if needed.
   */
  ...
  /**
   * Transforms the specified coordinate and stores the result in {@code ptDst}. This method returns
   * longitude as <var>x</var> values in the range {@code [-PI..PI]} and latitude as <var>y</var>
   * values in the range {@code [-PI/2..PI/2]}. It will be checked by the caller, so this method
   * doesn't need to performs this check.
   *
   * <p>Input coordinates have the {@link #falseEasting} and {@link #falseNorthing} removed and are
   ...
   * equations of Snyder, {@link #falseEasting}, {@link #falseNorthing} and {@link #scaleFactor} are
   * usually not given. When implementing these equations here, you will not need to add the {@link
   * #centralMeridian} to the output longitude or remove the {@link #semiMajor} (<var>a</var> or
   * <var>R</var>).
   *
   ...
   * @return the coordinate point after transforming {@code x}, {@code y} and storing the result in
   *     {@code ptDst}.
   ...
   * usually (but <strong>not</strong> guaranteed) to be invoked with values of <var>x</var> in the
   * range {@code [-PI..PI]} and values of <var>y</var> in the range {@code [-PI/2..PI/2]}. Values
   * outside those ranges are accepted (sometime with a warning logged) on the assumption that most
   * implementations use those values only in trigonometric functions like {@linkplain Math#sin sin}
   * and {@linkplain Math#cos cos}.
   *
   * <p>Coordinates have the {@link #centralMeridian} removed from <var>lambda</var> before this
   * method is invoked. After this method is invoked, the results in {@code ptDst} are multiplied by
   * {@link #globalScale}, and the {@link #falseEasting} and {@link #falseNorthing} are added. This
   * means that projections that implement this method are performed on an ellipse (or sphere) with
   * a semi-major axis of 1.
   *
   * <p>In <A HREF="http://www.remotesensing.org/proj/">PROJ.4</A>, the same standardization,
   ...
   * equations of Snyder, {@link #falseEasting}, {@link #falseNorthing} and {@link #scaleFactor} are
   * usually not given. When implementing these equations here, you will not need to remove the
   * {@link #centralMeridian} from <var>lambda</var> or apply the {@link #semiMajor} (<var>a</var>
   * or <var>R</var>).
   *
   ...
   * @param ptDst the specified coordinate point that stores the result of transforming {@code
   *     ptSrc}, or {@code null}. Ordinates will be in a dimensionless unit, as a linear distance on
   *     a unit sphere or ellipse.
   ...
   * {@link #globalScale} and adds the {@link #falseEasting} and {@link #falseNorthing} to the point
   * returned by the {@code transformNormalized(...)} call.
   *
   ...
   * @return the coordinate point after transforming {@code ptSrc} and storing the result in {@code
   *     ptDst}.
   * @throws ProjectionException if the point can't be transformed.
   */
  @Override
  public final Point2D transform(final Point2D ptSrc, Point2D ptDst) throws ProjectionException {
    ...
    ptDst =
        transformNormalized(
            ...
            ptDst);
    ptDst.setLocation(
        globalScale * ptDst.getX() + falseEasting, globalScale * ptDst.getY() + falseNorthing);
   ...
   *     every points even if some of them can't be transformed. Non-transformable points will have
   *     value {@link Double#NaN}. If more than one point can't be transformed, then this exception
   *     may be about an arbitrary point.
   */
   ...
   * @throws ProjectionException if a point can't be transformed. This method tries to transform
   *     every points even if some of them can't be transformed. Non-transformable points will have
   *     value {@link Float#NaN}. If more than one point can't be transformed, then this exception
   *     may be about an arbitrary point.
   */
  @Override
  public final void transform(
  ...
  /**
   * Inverse of a map projection. Will be created by {@link MapProjection#inverse()} only when first
   * required. Implementation of {@code transform(...)} methods are mostly identical to {@code
   * MapProjection.transform(...)}, except that they will invokes {@link
   * MapProjection#inverseTransformNormalized} instead of {@link MapProjection#transformNormalized}.
   *
   * @version $Id$
   * @author Martin Desruisseaux (PMO, IRD)
   */
  private final class Inverse extends AbstractMathTransform.Inverse implements MathTransform2D {
     ...
     * {@link #inverseTransformNormalized inverseTransformNormalized}(x, y, ptDst)</code>. It then
     * adds the {@link #centralMeridian} to the {@code x} of the point returned by the {@code
     * inverseTransformNormalized} call.
     *
    ...
    public final Point2D transform(final Point2D ptSrc, Point2D ptDst) throws ProjectionException {
      ...
      final double x =
          toDegrees(
              centralMeridian != 0 ? rollLongitude(ptDst.getX() + centralMeridian) : ptDst.getX());
   ...
   * every direct projection is followed by an inverse projection, and the result is compared to the
   * original coordinate. If a distance greater than the tolerance level is found, then an {@link
   * ProjectionException} will be thrown. Subclasses should override this method if they need to
   * relax the tolerance level.
   *
   ...
   * Calls to this method have immediate effect in the invoker's thread. The effect in other threads
   * may be delayed by some arbitrary amount of time. This method works only on a "best effort"
   * basis.
   *
  ...
  public abstract static class AbstractProvider extends MathTransformProvider {
    ...
    /**
     * The operation parameter descriptor for the longitude of center parameter value. Valid values
     * range is from -180 to 180°. Default value is 0.
     */
    ...
    /**
     * The operation parameter descriptor for the latitude of center parameter value. Valid values
     * range is from -90 to 90°. Default value is 0.
     */
    ...
    /**
     * The operation parameter descriptor for the standard parallel 1 parameter value. Valid values
     * range is from -90 to 90°. Default value is 0.
     */
    ...
    /**
     * The operation parameter descriptor for the standard parallel 2 parameter value. Valid values
     * range is from -90 to 90°. Default value is 0.
     */
    ...
    /**
     * The operation parameter descriptor for the {@link #scaleFactor scaleFactor} parameter value.
     * Valid values range is from 0 to infinity. Default value is 1.
     *
     * @todo Would like to start range from 0 <u>exclusive</u>.
     */
    ...
    /**
     * Returns the parameter value for the specified operation parameter in standard units. Values
     * are automatically converted into the standard units specified by the supplied {@code param}
     * argument, except {@link NonSI#DEGREE_ANGLE degrees} which are converted to {@link SI#RADIAN
     * radians}. This conversion is performed because the radians units are standard for all
     * internal computations in the map projection package. For example they are the standard units
     * for {@link MapProjection#latitudeOfOrigin latitudeOfOrigin} and {@link
     * MapProjection#centralMeridian centralMeridian} fields in the {@link MapProjection} class.
     *
     * @param param The parameter to look for.
     * @param group The parameter value group to search into.
