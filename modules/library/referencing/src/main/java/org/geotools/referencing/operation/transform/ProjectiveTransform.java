...
public class ProjectiveTransform extends AbstractMathTransform
  ...
  /**
   * Constructs a transform from the specified matrix. The matrix should be affine, but it will not
   * be verified.
   *
   ...
   * on the matrix size. Only matrix elements different from their default value will be included in
   * this group.
   *
  ...
  /**
   * Transforms an array of floating point coordinates by this matrix. Point coordinates must have a
   * dimension equals to <code>{@link Matrix#getNumCol}-1</code>. For example, for square matrix of
   * size 4&times;4, coordinate points are three-dimensional and stored in the arrays starting at
   * the specified offset ({@code srcOff}) in the order <code>
  ...
  /**
   * Gets the derivative of this transform at a point. For a matrix transform, the derivative is the
   * same everywhere.
   */
  @Override
    ...
    static {
      ...
      properties.put(
          ...
          new NamedIdentifier[] {
            ...
            new NamedIdentifier(
                Citations.GEOTOOLS, Vocabulary.formatInternational(VocabularyKeys.AFFINE_TRANSFORM))
          });
