...
public class MolodenskiTransform extends AbstractMathTransform implements Serializable {
  ...
  /**
   * Difference between the flattenings ({@code df = target f - source f}) of the target and source
   * ellipsoids.
   */
  private final double df;

  /**
   * Ratio of the Semi-major (<var>a</var>) semi-minor (<var>b/<var>) axis values ({@code a_b = a/b}
   * and {@code b_a = b/a}).
   */
  ...
  /**
   * A Molodenski transforms in 2D. This implementation is identical to {@link MolodenksiTransform}
   * except that it implements {@link MathTransform2D}.
   */
  ...
  public static class Provider extends MathTransformProvider {
    ...
    /**
     * The number of geographic dimension (2 or 3). This argument applies on both the source and the
     * target dimension. The default value is 2.
     */
    ...
    static final ParameterDescriptorGroup PARAMETERS =
        createDescriptorGroup(
            new NamedIdentifier[] {
              ...
              new NamedIdentifier(
                  Citations.GEOTOOLS,
                  Vocabulary.formatInternational(VocabularyKeys.MOLODENSKI_TRANSFORM))
            },
    ...
    protected MathTransform createMathTransform(final ParameterValueGroup values)
      ...
      if (!hasHeight) {
        ...
        return new Delegate(
            new MolodenskiTransform(abridged, a, b, hasHeight, ta, tb, hasHeight, dx, dy, dz),
            withHeight);
      }
    }
  ...
  /**
   * The provider for abridged {@link MolodenskiTransform}. This provider will construct transforms
   * from {@linkplain org.geotools.referencing.crs.DefaultGeographicCRS geographic} to {@linkplain
   * org.geotools.referencing.crs.DefaultGeographicCRS geographic} coordinate reference systems.
   *
   * <p><strong>Note:</strong> The EPSG does not use src_semi_major, etc. parameters and instead
   * uses "Semi-major axis length difference" and "Flattening difference".
