...
public class ReferencingObjectFactory extends ReferencingFactory
   ...
   * (at the contrary of what we usually do) because it is a little bit strange to declare that this
   * factory depends on an other {@link DatumFactory}. It is only a trick for getting the WKT paser
   * to work with aliases.
   *
   * @todo We should remove this trick if we can. Possible alternatives may be: make DatumAliases to
   *     implements CRSFactory with appropriate createWKT(String) method; move the createWKT(String)
   *     method out of CRSFactory interface.
   */
   ...
   * ReferencingFactoryFinder} must be null, since the non-null case should have been handled by the
   * constructor.
   *
   * @see #createParser
   */
  ...
  public CompoundCRS createCompoundCRS(
      Map<String, ?> properties, CoordinateReferenceSystem[] elements) throws FactoryException {
    CompoundCRS crs;
    try {
  ...
  public GeographicCRS createGeographicCRS(
      Map<String, ?> properties, GeodeticDatum datum, EllipsoidalCS cs) throws FactoryException {
    GeographicCRS crs;
    try {
   ...
   * conversions and change of axis order}, if needed. The {@link ReferencingFactoryContainer} class
   * provides conveniences methods for this task.
   *
   * @param properties Name and other properties to give to the new object.
   ...
   * @throws FactoryException if the object creation failed.
   * @deprecated Use {@link CoordinateOperationFactory#createDefiningConversion} followed by {@link
   *     #createDerivedCRS} instead.
   */
   ...
   * responsability to ensure that the conversion performs all required steps, including {@linkplain
   * AbstractCS#swapAndScaleAxis unit conversions and change of axis order}, if needed.
   *
   * @param properties Name and other properties to give to the new object.
