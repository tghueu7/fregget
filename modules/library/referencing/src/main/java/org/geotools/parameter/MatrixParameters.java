...
public class MatrixParameters extends ParameterGroup implements ParameterDescriptorGroup {
  ...
  /**
   * Returns the parameter in this group for the specified name. The name can be a matrix element if
  ...
  /**
   * Returns the value in this group for the specified name. The name can be a matrix element if it
   * uses the following syntax: <code>"elt_<var>row</var>_<var>col</var>"</code> where <code>"elt_"
   * </code> is the {@linkplain MatrixParameterDescriptors#prefix prefix} for all matrix elements,
   * and <var>row</var> and <var>col</var> are row and column indices respectively. For example
   * <code>"elt_2_1"</code> is the element name for the value at line 2 and row 1. The row and
  ...
  /**
   * Returns the parameters descriptors in this group. The amount of parameters depends on the value
   * of <code>"num_row"</code> and <code>"num_col"</code> parameters.
   */
   ...
   * <code>"num_row"</code> and <code>"num_col"</code> parameters. The parameter array will contains
   * only matrix elements which have been requested at least once by one of {@code parameter(...)}
   * methods. Never requested elements are left to their default value and omitted from the returned
   * array.
   */
  @Override
  ...
  /**
   * Sets all parameter values to the element value in the specified matrix. After this method call,
   * {@link #values} will returns only the elements different from the default value.
   *
  ...
  public boolean equals(final Object object) {
    ...
    if (super.equals(object)) {
      ...
      for (int j = 0; j < numRow; j++) {
        for (int i = 0; i < numCol; i++) {
          if (!Utilities.equals(
              this.parameter(j, i, numRow, numCol), that.parameter(j, i, numRow, numCol))) {
            return false;
          }
        }
      }
