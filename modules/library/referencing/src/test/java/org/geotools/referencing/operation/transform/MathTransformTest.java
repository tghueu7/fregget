...
public final class MathTransformTest {
  ...
  public void testAffineTransform() throws FactoryException, TransformException {
    for (int pass = 0; pass < 10; pass++) {
      ...
      transform.rotate(
          Math.PI * random.nextDouble(), 100 * random.nextDouble(), 100 * random.nextDouble());
  ...
  /**
   * Test various linear transformations. We test for many differents dimensions. The factory class
   * should have created specialized classes for 1D and 2D cases. This test is used in order to
   * ensure that specialized case produces the same results than general cases.
   */
  @Test
  ...
  public void testAffineTransformConcatenation() throws FactoryException, TransformException {
    ...
    for (int pass = 0; pass < 100; pass++) {
      ...
      for (int i = 0; i < transforms.length; i++) {
        ...
        String name =
            "transform[" + i + "](" + dimSource + " -> " + dimInterm + " -> " + dimTarget + ')';
  ...
  /**
   * Test the {@link ExponentialTransform1D} and {@link LogarithmicTransform1D} classes using simple
   * know values.
   */
  @Test
  ...
  /**
   * Test the concatenation of {@link LinearTransform1D}, {@link ExponentialTransform1D} and {@link
  ...
  /**
   * Compare the transformation performed by many math transforms. If some transforms don't have the
   * same number of input or output dimension, only the first input or output dimensions will be
   * taken in account.
   *
