...
public abstract class AbstractCRSTest extends OnlineTestCase {
   ...
   *
   * @todo Partially uncomment since we are allowed to compile for J2SE 1.5, but doesn't work as it
   *     did prior some changes in the referencing module. Need to investigate why.
   */
  ...
  public void testFind() throws FactoryException {
    ...
    if (supportsED50QuickScan()) {
      assertEquals(
          "With scan allowed, should find the CRS.", "EPSG:4230", CRS.lookupIdentifier(crs, false));
      assertEquals(Integer.valueOf(4230), CRS.lookupEpsgCode(crs, false));
    } else {
    ...
    assertEquals(
        "With scan allowed, should find the CRS.", "EPSG:4230", CRS.lookupIdentifier(crs, true));
  ...
  /**
   * Tests the number of CRS that can be created. This test will be executed only if this test suite
   * is run with the {@code -verbose} option provided on the command line.
   */
  ...
  public void testSRS() throws Exception {
    ...
    try {
      ...
      assertEquals(
          AxisOrder.NORTH_EAST, CRS.getAxisOrder(CRS.decode("urn:x-ogc:def:crs:EPSG::4326", true)));
    } finally {
      Hints.removeSystemDefault(Hints.FORCE_LONGITUDE_FIRST_AXIS_ORDER);
    }
    try {
