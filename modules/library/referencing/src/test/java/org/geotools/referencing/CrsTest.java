...
public final class CrsTest {
    ...
    public void testGetHorizontalCrs() {
        assertEquals(DefaultEngineeringCRS.GENERIC_2D, CRS.getHorizontalCRS(DefaultEngineeringCRS
                .GENERIC_2D));
    }
    ...
    public void testGetAxisOrder() throws FactoryException {
        ...
        assertEquals(AxisOrder.INAPPLICABLE, CRS.getAxisOrder(CRS.getHorizontalCRS
    ...
    public void parseWKT() throws Exception {
        String[] wkts = {
                "PROJCS[\"Wagner_IV\", GEOGCS[\"WGS84\",  DATUM[\"WGS84\",  SPHEROID[\"WGS84\",  " +
                        "6378137.0, 298.257223563]], PRIMEM[\"Greenwich\", 0.0], UNIT[\"degree\"," +
                        "0.017453292519943295], AXIS[\"Longitude\",EAST], AXIS[\"Latitude\"," +
                        "NORTH]], PROJECTION[\"Wagner_IV\"], UNIT[\"m\", 1.0], AXIS[\"Easting\", " +
                        "EAST], AXIS[\"Northing\", NORTH]]",
                "PROJCS[\"Wagner_V\", GEOGCS[\"WGS84\",  DATUM[\"WGS84\",  SPHEROID[\"WGS84\",  " +
                        "6378137.0, 298.257223563]], PRIMEM[\"Greenwich\", 0.0], UNIT[\"degree\"," +
                        "0.017453292519943295], AXIS[\"Longitude\",EAST], AXIS[\"Latitude\"," +
                        "NORTH]], PROJECTION[\"Wagner_V\"], UNIT[\"m\", 1.0], AXIS[\"Easting\", " +
                        "EAST], AXIS[\"Northing\", NORTH]]"
        };
        for (String wkt : wkts) {
    ...
    public void parseEsriWebMercator() throws Exception {
        ...
        assertEquals("Popular Visualisation Pseudo Mercator", esriCrs.getConversionFromBase()
                .getMethod().getName().getCode());
    ...
    public void testLambertParsing() throws FactoryException {
        String initialLambertWkt = "PROJCS[\"LAMBERT WKT\",GEOGCS[\"GCS_WGS_1984\"," +
                "DATUM[\"D_WGS_1984\"," +
    ...
    public void reprojectEnvelopeRotatedPole() throws Exception {
        try {
            ...
            String wkt = "PROJCS[\"WGS 84 / North Pole LAEA Alaska\", \n" +
                    ...
                    "    AXIS[\"Geodetic longitude\", EAST], \n" +
                    "    AUTHORITY[\"EPSG\",\"4326\"]], \n" +
                    "  PROJECTION[\"Lambert_Azimuthal_Equal_Area\", AUTHORITY[\"EPSG\"," +
                    "\"9820\"]], \n" +
            ...
            // back to wgs84
            GeneralEnvelope wgs84Envelope = CRS.transform(originalEnvelope, DefaultGeographicCRS
                    .WGS84);
