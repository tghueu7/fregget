...
public final class MathTransformBuilderTest {
  ...
  private List<MappedPosition> generateCoordsWithCRS(
    ...
    for (int i = 0; i < numberOfVertices; i++) {
      ...
      MappedPosition p =
          new MappedPosition(new DirectPosition2D(crs, xs, ys), new DirectPosition2D(crs, xd, yd));
      if (includeAccuracy) {
  ...
  /**
   * Implements the method for testing the calculation of least square method. The main requirement
   * of least square method is that A<sup>T<sup>PAx + A<sup>T<sup>PX = 0.
   *
   * @author jezekjas
   */
