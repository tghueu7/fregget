...
public final class CoordinateOperationFactoryTest extends TransformTestBase {
  ...
  /**
   * Make sure that <code>createOperation(sourceCRS, targetCRS)</code> returns an identity transform
   * when <code>sourceCRS</code> and <code>targetCRS</code> are identical, and tests the generic
   * CRS.
   */
  @Test
  public void testGenericTransform() throws FactoryException {
    ...
    assertTrue(
        opFactory.createOperation(CARTESIAN_2D, CARTESIAN_2D).getMathTransform().isIdentity());
    assertTrue(
        opFactory.createOperation(CARTESIAN_3D, CARTESIAN_3D).getMathTransform().isIdentity());
  ...
  public void testUnitConversion() throws Exception {
    ...
    assertEquals(
        "latitude_of_origin", 50.0, param.parameter("latitude_of_origin").doubleValue(), 1E-8);
    assertEquals(
  ...
  public void test4D_to_2D() throws Exception {
    ...
    sourceCRS =
        new DefaultCompoundCRS("Mercator 3D", sourceCRS, DefaultVerticalCRS.ELLIPSOIDAL_HEIGHT);
    sourceCRS =
        new DefaultCompoundCRS("Mercator 4D", sourceCRS, DefaultTemporalCRS.MODIFIED_JULIAN);
    final CoordinateOperation op = opFactory.createOperation(sourceCRS, targetCRS);
    final MathTransform mt = op.getMathTransform();
    if (usingDefaultFactory) {
      assertSame(sourceCRS, op.getSourceCRS());
