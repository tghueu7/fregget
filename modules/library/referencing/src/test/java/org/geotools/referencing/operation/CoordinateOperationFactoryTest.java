...
public final class CoordinateOperationFactoryTest extends TransformTestBase {
    ...
    public void testGenericTransform() throws FactoryException {
        ...
        assertTrue(opFactory.createOperation(CARTESIAN_2D, CARTESIAN_2D).getMathTransform()
                .isIdentity());
        assertTrue(opFactory.createOperation(CARTESIAN_3D, CARTESIAN_3D).getMathTransform()
                .isIdentity());
        assertTrue(opFactory.createOperation(GENERIC_2D, GENERIC_2D).getMathTransform()
                .isIdentity());
        assertTrue(opFactory.createOperation(GENERIC_2D, CARTESIAN_2D).getMathTransform()
                .isIdentity());
        assertTrue(opFactory.createOperation(CARTESIAN_2D, GENERIC_2D).getMathTransform()
                .isIdentity());
    ...
    public void testUnitConversion() throws Exception {
        ...
        assertEquals("latitude_of_origin", 50.0, param.parameter("latitude_of_origin")
                .doubleValue(), 1E-8);
...
//      assertTransformEquals2_2(transform, +180,  85.41283436546335, +180,  85.41283436548373);
    ...
    public void testDatumShift() throws Exception {
        ...
        if (usingDefaultFactory) {
            assertSame(sourceCRS, operation.getSourceCRS());
            assertSame(targetCRS, operation.getTargetCRS());
            assertTrue(operation.getCoordinateOperationAccuracy().contains(PositionalAccuracyImpl
                    .DATUM_SHIFT_APPLIED));
            assertFalse(operation.getCoordinateOperationAccuracy().contains
        ...
        assertFalse(lenient.getCoordinateOperationAccuracy().contains(PositionalAccuracyImpl
                .DATUM_SHIFT_APPLIED));
        assertTrue(lenient.getCoordinateOperationAccuracy().contains(PositionalAccuracyImpl
                .DATUM_SHIFT_OMITTED));
...
//      assertTransformEquals2_2(lenientTr, 20, -10, -6.66277083, 17.998143675921714);
    ...
    public void testDatumShift7Param() throws Exception {
        ...
        assertInterfaced(transform);
        assertTransformEquals2_2(transform, 168.1075, -21.597283333333, 822023.338884308, 
                7608648.67486555);
        // Note: Expected values above were computed with Geotools (not an external library).
    ...
    public void test4D_to_2D() throws Exception {
        ...
        sourceCRS = new DefaultCompoundCRS("Mercator 3D", sourceCRS, DefaultVerticalCRS
                .ELLIPSOIDAL_HEIGHT);
        sourceCRS = new DefaultCompoundCRS("Mercator 4D", sourceCRS, DefaultTemporalCRS
                .MODIFIED_JULIAN);
        final CoordinateOperation op = opFactory.createOperation(sourceCRS, targetCRS);
        final MathTransform mt = op.getMathTransform();
        if (usingDefaultFactory) {
