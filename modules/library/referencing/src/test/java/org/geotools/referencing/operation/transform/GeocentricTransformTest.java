...
public final class GeocentricTransformTest extends TransformTestBase {
   ...
   * algorithms used: one for the ellipsoidal model, and a simpler one for spherical model. We test
   * the ellipsoidal model using know values of nautical mile at different latitude. Then, we test
   * the spherical model with random values. If JDK 1.4 assertion is enabled, the spherical model
   * will compare its result with the ellipsoidal one.
   *
   * <p>Note about nautical mile:
   *
   * <p>"Le mille marin était, en principe, la longueur de la minute sexagésimale du méridien à la
   * latitude de 45°. Cette longueur dépendait donc des valeurs adoptées pour le rayon équatorial de
   * la terre et son aplatissement. En France, le décret du 3 mai 1961 sur les unités de mesure,
   * fixe à 1852 mètres la longueur du mille marin qui est également la valeur adoptée pour le mille
   * marin international."
   *
  ...
  public void testEllipsoid() throws FactoryException {
     ...
     */
    assertEquals(
        "Nautical mile at equator", 1842.78, e.orthodromicDistance(0, 00 - hm, 0, 00 + hm), 0.2);
    assertEquals(
        "Nautical mile at North pole", 1861.67, e.orthodromicDistance(0, 90 - 2 * hm, 0, 90), 0.2);
    assertEquals(
        "Nautical mile at South pole", 1861.67, e.orthodromicDistance(0, 2 * hm - 90, 0, -90), 0.2);
    assertEquals(
        "International nautical mile", 1852.00, e.orthodromicDistance(0, 45 - hm, 0, 45 + hm), 0.2);
