...
public final class GeocentricTransformTest extends TransformTestBase {
    ...
    public void testEllipsoid() throws FactoryException {
         ...
         */
        assertEquals("Nautical mile at equator", 1842.78, e.orthodromicDistance(0, 00 - hm, 0, 00
                + hm), 0.2);
        assertEquals("Nautical mile at North pole", 1861.67, e.orthodromicDistance(0, 90 - 2 * 
                hm, 0, 90), 0.2);
        assertEquals("Nautical mile at South pole", 1861.67, e.orthodromicDistance(0, 2 * hm - 
                90, 0, -90), 0.2);
        assertEquals("International nautical mile", 1852.00, e.orthodromicDistance(0, 45 - hm, 0,
                45 + hm), 0.2);
        ...
        final DefaultEllipsoid s = DefaultEllipsoid.createEllipsoid("Sphere", radius, radius, e
                .getAxisUnit());
        ...
        for (int i = 0; i < array0.length / 6; i++) {
            ...
            try {
                ...
                if (i < orthodromicDistance.length) {
                    assertEquals("Orthodromic distance[" + i + ']', orthodromicDistance[i], 
                            orthodromic, 0.1);
                }
