...
public interface Point extends Primitive, Position {
   ...
   * Primitive primitive} that cannot use {@linkplain Position positions} to represent its defining
   * geometry. A {@linkplain Position position} is either a {@linkplain DirectPosition direct
   * position} or a reference to a {@code Point} (from which a {@linkplain DirectPosition direct
   * position} may be obtained). By not allowing {@code Point} to use this technique, infinitely
   * recursive references are prevented.
   *
   ...
   * between this {@code Point} and a passed {@linkplain Position position}. The choice of the curve
   * type for defining the bearing is dependent on the {@linkplain
   * org.opengis.referencing.crs.CoordinateReferenceSystem coordinate reference system} in which
   * this {@code Point} is defined. For example, in the Mercator projection, the curve is the rhumb
   * line. In 3D, geocentric coordinate system, the curve may be the geodesic joining the two points
   * along the surface of the geoid or ellipsoid in use. Implementations that support this function
   * shall specify the nature of the curve to be used.
   *
