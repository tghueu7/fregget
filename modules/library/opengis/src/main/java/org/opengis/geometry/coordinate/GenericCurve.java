...
public interface GenericCurve {
  ...
  /**
   * Returns the tangent vector along this {@code GenericCurve} at the passed parameter value. This
   * vector approximates the derivative of the parameterization of the curve. The tangent shall be a
   * unit vector (have length 1.0), which is consistent with the parameterization by arc length.
   *
  ...
  /**
   * Indicates the parameter for the {@linkplain #getStartPoint start point}. The start parameter of
   * a {@linkplain org.opengis.geometry.primitive.Curve curve} shall always be 0. For {@linkplain
   * org.opengis.geometry.primitive.CurveSegment curve segments} within a {@linkplain
   ...
   * start parameter of any segment (except the first) shall be equal to the end parameter of the
   * previous segment.
   *
   ...
   * {@linkplain org.opengis.geometry.primitive.Curve curve}, the end parameters of the {@linkplain
   * org.opengis.geometry.primitive.CurveSegment curve segment} shall be equal to those of the
   ...
   * of calculation, and seldom have any simple relation to arc distances, which are defined as the
   * default parameterization. Normally, geometric constructions will use constructive parameters,
   * as the programmer deems reasonable, and calculate arc length parameters when queried. </font>
   *
   * </blockquote>
   *
  ...
  /**
   * Indicates the parameter used in the constructive paramerization for the end point. There is no
   * assumption that the {@code startConstructiveParam} is less than the {@code
   * endConstructiveParam}, but the parameterization must be strictly monotonic (strictly
   * increasing, or strictly decreasing).
   *
   * <blockquote>
   ...
   * restriction that the parameter represents the arc length of the curve, nor restrictions between
   * a {@linkplain org.opengis.geometry.primitive.Curve curve} and its component {@linkplain
   * org.opengis.geometry.primitive.CurveSegment curve segments}. The most common use of this
   ...
   * parameterization shall be by arc length, i.e. distance along the {@code GenericCurve} measured
   * from the start point and added to the start parameter.
   *
  ...
  /**
   * Returns the length between two points. The length of a piece of curvilinear geometry shall be a
   * numeric measure of its length in a coordinate reference system. Since length is an accumulation
   * of distance, its return value shall be in a unit of measure appropriate for measuring
   * distances. This method shall return the distance between the two points along the curve. The
   * default values of the two parameters shall be the start point and the end point, respectively.
   * If either of the points is not on the curve, then it shall be projected to the nearest
   * {@linkplain DirectPosition direct position} on the curve before the distance is calculated. If
   * the curve is not simple and passes through either of the two points more than once, the
   * distance shall be the minimal distance between the two points on this {@linkplain
   * org.opengis.geometry.primitive.Curve curve}.
   *
   * @param point1 The first point, or {@code null} for the {@linkplain #getStartPoint start point}.
   ...
   *
   * <p>Distances between direct positions determined by the default parameterization are simply the
   * difference of the parameter. The length function also allows for the conversion of the
   ...
   * between control points along the generated curve shall be not more than {@code maxSpacing}. If
   * {@code maxOffset} is given (not zero), the distance between generated curve at any point and
   * the original curve shall not be more than the {@code maxOffset}. If both parameters are set,
   * then both criteria shall be met. If the original control points of the curve lie on the curve,
   * then they shall be included in the returned {@linkplain LineString line string}'s control
   * points. If both parameters are set to zero, then the line string returned shall be constructed
   * from the control points of the original curve.
   *
   * <blockquote>
   *
   * <font size=2> <strong>NOTE:</strong> This function is useful in creating linear approximations
   * of the curve for simple actions such as display. It is often referred to as a "stroked curve".
   * For this purpose, the {@code maxOffset} version is useful in maintaining a minimal
   * representation of the curve appropriate for the display device being targeted. This function is
   * also useful in preparing to transform a curve from one coordinate reference system to another
   * by transforming its control points. In this case, the {@code maxSpacing} version is more
   * appropriate. Allowing both parameters to default to zero does not seem to have any useful
   * geographic nor geometric interpretation unless further information is known about how the
   * curves were constructed. </font>
   *
   * </blockquote>
   *
