...
public interface OffsetCurve extends CurveSegment {
   ...
   * system, positive distances are to be left of the basis curve, and negative distances are right
   * of the basis curve.
   *
   * @unitof Length
   */
  ...
  /**
   * Defines the vector direction of the offset curve from the basis curve. It can be omitted in the
   * 2D case, where the distance can be positive or negative. In that case, distance defines left
   * side (positive distance) or right side (negative distance) with respect to the tangent to the
   * basis curve. In 3D the basis curve shall have a well-defined tangent direction for every point.
   * The offset curve at any point (parameter) on the basis curve <var>c</var> is in the direction
   *
