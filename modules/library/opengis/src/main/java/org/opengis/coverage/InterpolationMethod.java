...
public class InterpolationMethod extends CodeList<InterpolationMethod> {
   ...
   * neighbour interpolation extends a discrete coverage to a step function defined on the convex
   * hull of the domain objects in the domain of the coverage. Nearest neighbour interpolation is
   * the only interpolation method described in ISO 19123 that can be used to interpolate attributes
   * that have nominal or ordinal values.
   *
   * <p><B>NOTE:</B> In the case of a discrete point coverage, the "steps" of the step function are
   ...
   * <var>v</var> is the value of a feature attribute at distance <var>x</var> along the curve from
   * the start. Three point value pairs are needed to provide control values for calculating the
  ...
  /**
   * Interpolation based on the assumption that feature attribute values vary as a cubic function of
   * distance along a value segment.
   *
   * <p>
   *
   ...
   * <p>where <var>a</var> is the value of a feature attribute at the start of a value segment and
   * <var>v</var> is the value of a feature attribute at distance <var>x</var> along the curve from
   * the start. Four point value pairs are needed to provide control values for calculating the
   * coefficients of the function.
   */
