...
public final class VerticalDatumType extends CodeList<VerticalDatumType> {
   ...
   * datum. Other variations to the above three vertical datum types may exist and are all included
   * in this type.
   */
   ...
   *     ellipsoidal height. More specifically {@linkplain org.opengis.referencing.crs.GeographicCRS
   *     Geographic CRS} with ellipsoidal height shall be backed by a three-dimensional {@linkplain
   *     org.opengis.referencing.cs.EllipsoidalCS Ellipsoidal CS}; they should never be built as
   *     {@linkplain org.opengis.referencing.crs.CompoundCRS Compound CRS}.
   ...
   *     org.opengis.referencing.crs.CoordinateReferenceSystem Coordinate Reference System} and let
   *     the user extracts the vertical component himself.
   *     <p>Furthermore the <A HREF="../doc-files/WKT.html">WKT format</A> still in wide use, and
   *     this format (defined before ISO 19111) does not treat ellipsoidal height in a special way.
   *     A WKT parser needs to get the vertical CRS separatly before to merge it with the geographic
   *     CRS.
   ...
   * which the associated vertical measurements are used for. For hydrographic charts, this is often
   * a predicted nominal sea surface (i.e., without waves or other wind and current effects) that
   * occurs at low tide. Depths are measured in the direction perpendicular (approximately) to the
   ...
   * coordinate system axis. These are approximations of orthometric heights obtained with the help
   * of a barometer or a barometric altimeter. These values are usually expressed in one of the
   * following units: meters, feet, millibars (used to measure pressure levels), or theta value
