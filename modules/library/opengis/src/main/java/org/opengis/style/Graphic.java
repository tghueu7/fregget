...
public interface Graphic {
   ...
   * default size of a format without an inherent size (such as SVG which are not specially marked)
   * is defined to be 16 pixels in height and the corresponding aspect in width. If a size is
   * specified, the height of the graphic will be scaled to that size and the corresponding aspect
   * will be used for the width. An expected common use case will be for image graphics to be on the
   * order of 200 pixels in linear size and to be scaled to lower sizes. On systems that can
   * resample these graphic images "smoothly," the results will be visually pleasing.
   *
   * @return Expression
   */
  @XmlParameter("Size")
   ...
   * dependent. If a format does not include an inherent rotation point, then the point of rotation
   ...
   * see 11.4.4) to use for anchoring the graphic to the main-geometry point. The coordinates are
   * given as two floating-point numbers in the AnchorPointX and AnchorPointY elements each with
   * values between 0.0 and 1.0 inclusive. The bounding box of the graphic/label to be rendered is
   * considered to be in a coordinate space from 0.0 (lower-left corner) to 1.0 (upper-right
   * corner), and the anchor position is specified as a point in this space. The default point is
   * X=0.5, Y=0.5, which is at the middle height and middle length of the graphic/label text. A
   * system may choose different anchor points to de-conflict graphics/labels.
   *
  ...
  /**
   * The Displacement gives the X and Y displacements from the "hot-spot" point. This element may be
   * used to avoid over-plotting of multiple graphic symbols used as part of the same point symbol.
   * The displacements are in units of measure above and to the right of the point. The default
