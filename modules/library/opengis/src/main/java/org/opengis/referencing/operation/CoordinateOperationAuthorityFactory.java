...
public interface CoordinateOperationAuthorityFactory extends AuthorityFactory {
   ...
   * authority}" and "{@linkplain Identifier#getCode code}" values of the created object will be set
   * to the authority of this object, and the code specified by the client, respectively. The other
   * metadata values may or may not be set.
   *
   ...
   * operations first. This method doesn't need to compute operations from {@code source} to {@code
   * target} CRS if no such operations were explicitly defined in the authority database.
