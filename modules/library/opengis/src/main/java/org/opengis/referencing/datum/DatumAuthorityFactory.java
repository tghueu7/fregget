...
public interface DatumAuthorityFactory extends AuthorityFactory {
  /**
   * Returns an arbitrary {@linkplain Datum datum} from a code. If the datum type is know at compile
   * time, it is recommended to invoke the most precise method instead of this one (for example
   * <code>&nbsp;{@linkplain #createGeodeticDatum createGeodeticDatum}(code)&nbsp;</code> instead of
   * <code>&nbsp;createDatum(code)&nbsp;</code> if the caller know he is asking for a {@linkplain
