...
public interface Coverage {
  ...
  /**
   * Return the value vector for a given point in the coverage. A value for each sample dimension is
   * included in the vector. The default interpolation type used when accessing grid values for
   ...
   *
   * <p><strong>WARNING:</strong> This method is inherited from the legacy OGC 01-004 specification
   * and may be deprecated in a future version. We are for more experience and feedbacks on the
  ...
  /**
   * Return a sequence of boolean values for a given point in the coverage. A value for each sample
   * dimension is included in the sequence. The default interpolation type used when accessing grid
   * values for points which fall between grid cells is nearest neighbor.
   *
   * <p>The coordinate reference system of the point is the same as the grid coverage coordinate
   * reference system (specified by the {@link #getCoordinateReferenceSystem} method).
   *
   ...
   *     null} if none.
   * @return A sequence of boolean values for a given point in the coverage. If {@code destination}
  ...
  /**
   * Return a sequence of integer values for a given point in the coverage. A value for each sample
   * dimension is included in the sequence. The default interpolation type used when accessing grid
   * values for points which fall between grid cells is nearest neighbor.
   *
   * <p>The coordinate reference system of the point is the same as the grid coverage coordinate
   ...
   * @param destination An optionally preallocated array in which to store the values, or {@code
   *     null} if none.
   * @return A sequence of integer values for a given point in the coverage. If {@code destination}
   *     was non-null, then it is returned. Otherwise, a new array is allocated and returned.
   * @throws PointOutsideCoverageException if the point is outside the coverage {@linkplain
   *     #getEnvelope envelope}.
   * @throws CannotEvaluateException if the point can't be evaluated for some othe reason.
   ...
   * interoperability with <A HREF="http://java.sun.com/products/java-media/2D/">Java2D</A>. If this
   * coverage is a {@link org.opengis.coverage.grid.GridCoverage} backed by a {@link
