...
public interface SurfacePatch extends GenericSurface {
   ...
   * <blockquote>
   *
   * <font size=2> <strong>NOTE:</strong> In the specification, surface patches do not appear except
   * in the context of a surface, and therefore this method should never returns {@code null} which
   * would preclude the use of surface patches except in this manner. While this would not affect
   * the specification, allowing {@code null} owner allows other standards based on ISO 19107 one to
   * use surface patches in a more open-ended manner. </font>
   *
   * </blockquote>
   *
   * @return The owner of this surface patch, or {@code null} if the association is not available or
   ...
   * for this patch. The default value of "0" means simple continuity, which is a mandatory minimum
   * level of continuity. This level is referred to as "C<sup>0</sup>" in mathematical texts. A
   * value of 1 means that the functions are continuous and differentiable at the appropriate end
   * point: "C<sup>1</sup>" continuity. A value of "n" for any integer means <var>n</var>-times
   ...
   * {@linkplain Curve curve} are also valid. In the normal case, {@code SurfacePatch}es will share
   * parts of their boundary with the aggregate {@linkplain Surface surface}, and other parts with
   * {@code SurfacePatch}es (not necessarily distinct).
   *
