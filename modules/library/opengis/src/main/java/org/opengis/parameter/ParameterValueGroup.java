...
public interface ParameterValueGroup extends GeneralParameterValue {
   ...
   * modification shall be reflected back into this {@code ParameterValueGroup}. More specifically:
   *
   * <UL>
   *   <LI>
   *       <p>If the list supports the {@link List#add(Object) add} operation, then it should ensure
   *       that the added {@linkplain GeneralParameterValue general parameter value} is valid and
   *       can be added to this group. An {@link InvalidParameterCardinalityException} (or any other
   *       appropriate exception) shall be thrown if it is not the case.
   *   <LI>
   ...
   *
   * <p>This convenience method provides a way to get and set parameter values by name. For example
   ...
   * </blockquote>
   *
   * This method do not search recursively in subgroups. This is because more than one subgroup may
   * exist for the same {@linkplain ParameterDescriptorGroup descriptor}. The user must {@linkplain
   * #groups query all subgroups} and select explicitly the appropriate one to use.
   *
   ...
   * {@linkplain ParameterDescriptor#getMinimumOccurs minimumOccurs} == 0</code>) and no value were
   * defined previously, then this method returns an empty set.
   *
   * @param name The case insensitive {@linkplain Identifier#getCode identifier code} of the
   ...
   *     ParameterDescriptorGroup#getMaximumOccurs maximum number of occurences} of subgroups of the
   *     given name.
   */
  @Extension
