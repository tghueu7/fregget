...
public interface MathTransform {
   ...
   * is {@code null}, a new {@link DirectPosition} object is allocated and then the result of the
   ...
   *     ptDst}, or a newly created point if {@code ptDst} was null.
   ...
   * dimensions corresponding to the source and target coordinate systems. If the input dimension is
   * <var>M</var>, and the output dimension is <var>N</var>, then the matrix will have size <code>
   * N&times;M</code>. The elements of the matrix <code>{e<sub>n,m</sub> : n=0..(N-1)}</code> form a
   * vector in the output space which is parallel to the displacement caused by a small change in
   * the <var>m</var>'th ordinate in the input space.
   *
   * <p>For example, if the input dimension is 4 and the output dimension is 3, then a small
   * displacement <code>(x<sub>0</sub>,&nbsp;x<sub>1</sub>,&nbsp;x<sub>2</sub>,&nbsp;x<sub>3</sub>)
   * </code> in the input space will result in a displacement <code>
   * (y<sub>0</sub>,&nbsp;y<sub>1</sub>,&nbsp;y<sub>2</sub>)</code> in the output space computed as
   * below (<code>e<sub>n,m</sub></code> are the matrix's elements):
   *
   * <pre>
   ...
   * </pre>
   *
   * @param point The coordinate point where to evaluate the derivative. Null value is accepted only
   *     if the derivative is the same everywhere. For example affine transform accept null value
   *     since they produces identical derivative no matter the coordinate value. But most map
  ...
  /**
   * Creates the inverse transform of this object. The target of the inverse transform is the source
   * of the original. The source of the inverse transform is the target of the original. Using the
   * original transform followed by the inverse's transform will result in an identity map on the
   * source coordinate space, when allowances for error are made. This method may fail if the
   * transform is not one to one. However, all cartographic projections should succeed.
   *
   ...
   * HREF="../doc-files/WKT.html">defined in extended Backus Naur form</A>. This operation may fails
   * if an object is too complex for the WKT format capability.
   *
