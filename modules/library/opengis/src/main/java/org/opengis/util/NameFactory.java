...
public interface NameFactory {
   ...
   *
   * <font size=-1><b>Implementation note:</b> despite the "create" name, implementations may return
   * existing instances. </font>
   *
   * </blockquote>
   *
   * @param name The name of the namespace to be returned. This argument can be created using <code>
   *     {@linkplain #createGenericName createGenericName}(null, parsedNames)</code>.
   ...
   *     typically {@code ":"}.
   * @param separator The separator to insert between {@linkplain GenericName#getParsedNames parsed
   *     names} in that namespace. For HTTP namespace, it is {@code "."}. For URN namespace, it is
   *     typically {@code ":"}.
  ...
  /**
   * Creates a local name from the given character sequence. The character sequence can be either a
   * {@link String} or an {@link InternationalString} instance. In the later case, implementations
   * can use an arbitrary {@linkplain Locale locale} (typically {@link Locale#ENGLISH ENGLISH}, but
   * not necessarly) for the unlocalized string to be returned by {@link LocalName#toString}.
   *
   ...
   * {@link String} or {@link InternationalString} instances. In the later case, implementations can
   * use an arbitrary {@linkplain Locale locale} (typically {@link Locale#ENGLISH ENGLISH}, but not
   * necessarly) for the unlocalized string to be returned by {@link GenericName#toString}.
   *
   * <p>If the length of the {@code parsedNames} array is 1, then this method returns an instance of
   * {@link LocalName}. If the length is 2 or more, then this method returns an instance of {@link
   * ScopedName}.
   *
   * @param scope The {@linkplain GenericName#scope scope} of the generic name to be created, or
   *     {@code null} for a global namespace.
   * @param parsedNames The local names as an array of strings or international strings. This array
   ...
   *
   * <p>For example if the {@code scope} argument is the namespace {@code "urn:ogc:def"} with {@code
   * ":"} as the separator, and if the {@code name} argument is the string {@code "crs:epsg:4326"},
   * then the result is a {@linkplain ScopedName scoped name} having a {@linkplain GenericName#depth
   * depth} of 3, which is the length of the list of {@linkplain GenericName#getParsedNames parsed
   * names} ({@code "crs"}, {@code "epsg"}, {@code "4326"}).
   *
   ...
   * LocalName#toString name}. The {@code scope} argument identifies the {@linkplain NameSpace name
   * space} in which the local name will be created. The {@code name} argument is taken verbatism as
   * the string representation of the local name.
   *
   * <p>This method
   *
   ...
   *     {@link #createLocalName(NameSpace,CharSequence) createLocalName} for the name and localized
   *     name arguments.
   */
  @Deprecated
   ...
   * ScopedName#toString name}. The {@code scope} argument identifies the {@linkplain NameSpace name
   * space} in which the scoped name will be created. The {@code name} argument will be parsed in
   * order to construct the list of {@linkplain ScopedName#getParsedNames parsed names}.
   *
   * @param scope The scope, or {@code null} for the global one.
   * @param name The unlocalized name.
   * @param localizedName A localized version of the name, or {@code null} if none.
