...
public interface GeometryFactory {
   ...
   * org.opengis.referencing.crs.CoordinateReferenceSystem coordinate reference system} being used,
   * and creates the appropriate geodesic string joining them.
   *
   * @throws MismatchedReferenceSystemException If geometric objects given in argument don't use
   *     compatible {@linkplain CoordinateReferenceSystem coordinate reference system}.
   * @throws MismatchedDimensionException If geometric objects given in argument don't have the
   ...
   * </blockquote>
   *
   * In 2D coordinate reference systems, the bulge can be given a sign and the normal can be assumed
   * to be the perpendicular to the line segment between the start and end point of the arc (the
   * chord of the arc), pointing left. For example if the two points are <var>P</var><sub>0</sub> =
   * (<var>x</var><sub>0</sub>,&nbsp;<var>y</var><sub>0</sub>) and <var>P</var><sub>1</sub> =
   * (<var>x</var><sub>1</sub>,&nbsp;<var>y</var><sub>1</sub>), and the bulge is <var>b</var>, then
   * the vector in the direction of <var>P</var><sub>1</sub> from <var>P</var><sub>0</sub> is:
   *
   * <blockquote>
   *
   ...
   * have a vector dot product of zero and a vector cross product of 1. By inspection, the leftward
   * normal to complete the pair is:
   *
   * <blockquote>
   *
  ...
  /**
   * Equivalents to the {@linkplain #createArc(Position,Position,double,double[]) second constructor
   * of arc}, except the bulge representation is maintained. The midpoint of the resulting arc is
   * given by:
   *
   * <blockquote>
   *
  ...
  /**
   * Equivalent to the {@linkplain #createArc(Position,Position,double,double[]) second constructor
   * of arc}, except the bulge representation is maintained internal to the object. The midpoints of
  ...
  /**
   * Constructs a B-spline curve. If the {@code knotSpec} is {@code null}, then the {@link KnotType}
   * is uniform and the knots are evenly spaced, and except for the first and last have multiplicity
   * = 1. At the ends the knots are of multiplicity = {@code degree}+1. If the {@code knotType} is
   * uniform they need not be specified.
   *
   ...
   * evenly spaced, and except for the first and last have multiplicity equal to degree. At the ends
   * the knots are of multiplicity = {@code degree}+1.
   *
   ...
   * multiplicity = 1. At the ends the knots are of multiplicity = degree+1. If the {@code knotType}
   ...
   * @param knotSpec The type of knot distribution used in defining this spline, or {@code null} if
   *     unspecified.
   * @throws MismatchedReferenceSystemException If geometric objects given in argument don't use
   *     compatible {@linkplain CoordinateReferenceSystem coordinate reference system}.
   * @throws MismatchedDimensionException If geometric objects given in argument don't have the
   ...
   */
  @UML(
    identifier = "GM_BSplineSurface(Sequence<GM_PointArray>,Integer,Sequence<GM_Knot>,GM_KnotType)",
    obligation = MANDATORY,
    specification = ISO_19107
  )
   ...
   *
   * <p><strong>NOTE:</strong> It is important that the boundary components be oriented properly for
   * this to work. It is often the case that in bounded manifolds, such as the sphere, there is an
   * ambiguity unless the orientation is properly used.
   *
   * @param boundary The surface boundary.
  ...
  /**
   * Constructs a restricted Delaunay network from triangle corners (posts), breaklines, stoplines,
   * and maximum length of a triangle side.
   *
  ...
  Tin createTin(
      Set<Position> post, Set<LineString> stopLines, Set<LineString> breakLines, double maxLength)
      throws MismatchedReferenceSystemException, MismatchedDimensionException;
