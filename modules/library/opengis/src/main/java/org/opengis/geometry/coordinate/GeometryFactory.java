...
public interface GeometryFactory {
     ...
     * @throws MismatchedDimensionException       If the coordinate position don't have 
     ...
     */
    @UML(identifier = "GM_LineSegment(GM_Position[2])", obligation = MANDATORY, specification = 
            ISO_19107)
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_LineString(GM_Position[2..n])", obligation = MANDATORY, specification =
            ISO_19107)
     ...
     * <p>
     * <blockquote><pre>midPoint = ((startPoint + endPoint)/2.0) + bulge&times;
     * normal</pre></blockquote>
     * <p>
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_Arc(GM_Position[2],Real,Vector)", obligation = MANDATORY, specification
            = ISO_19107)
    ...
    @UML(identifier = "GM_ArcByBulge(GM_Position[2],Real,Vector)", obligation = MANDATORY, 
            specification = ISO_19107)
    ArcByBulge createArcByBulge(Position startPoint, Position endPoint, double bulge, double[] 
            normal)
            throws MismatchedReferenceSystemException, MismatchedDimensionException;
     ...
     * <p>
     * <blockquote><code>
     * midPoint[<var>n</var>] = ((points[<var>n</var>] + points[<var>n</var>+1])/2.0) + (bulge * 
     * normal)
     * </code></blockquote>
     *
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_BSplineCurve(Integer,GM_PointArray,Sequence<GM_Knot>,GM_KnotType)", 
            obligation = MANDATORY, specification = ISO_19107)
    BSplineCurve createBSplineCurve(int degree, PointArray points, List<Knot> knots, KnotType 
            knotSpec)
            throws MismatchedReferenceSystemException, MismatchedDimensionException;
     ...
     * @param degree   The algebraic degree of the basis functions for the first and second 
     *                 parameter,
     *                 as an array of length 1 or 2. If only one value is given, then the two 
     *                 degrees
     *                 are equal.
     * @param knots    The sequence of knots as an array of length 2, or {@code null} if 
     *                 unspecified.
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_BSplineSurface(Sequence<GM_PointArray>,Integer,Sequence<GM_Knot>," +
            "GM_KnotType)", obligation = MANDATORY, specification = ISO_19107)
    BSplineSurface createBSplineSurface(List<PointArray> points, int[] degree, List<Knot>[] 
            knots, KnotType knotSpec)
            throws MismatchedReferenceSystemException, MismatchedDimensionException;
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_Polygon(GM_SurfaceBondary)", obligation = MANDATORY, specification = 
            ISO_19107)
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_Tin(Set<GM_Position>,Set<GM_LineString>,Set<GM_LineString>,Number)", 
            obligation = MANDATORY, specification = ISO_19107)
     ...
     *                                            the expected dimension.
     */
    @UML(identifier = "GM_PolyhedralSurace(GM_Polygon)", obligation = MANDATORY, specification = 
            ISO_19107)
