...
public interface DirectPosition extends Position {
   ...
   * {@linkplain CoordinateReferenceSystem coordinate reference system}. In this case, the cordinate
  ...
  /**
   * A <b>copy</b> of the ordinates presented as an array of double values. Please note that this is
   * only a copy (the real values may be stored in another format) so changes to the returned array
   * will not affect the source DirectPosition.
   *
   * <blockquote>
   *
   * <pre>
   ...
   *       (without modifying the original {@code DirectPosition}), or we want to protect the array
   *       from future {@code DirectPosition} changes.
   ...
   *
   * @return A copy of the coordinates. Changes in the returned array will not be reflected back in
   *     this {@code DirectPosition} object.
   */
  ...
  /**
   * Compares this direct position with the specified object for equality. Two direct positions are
   * considered equal if the following conditions are meet:
   *
   * <p>
   *
   * <ul>
