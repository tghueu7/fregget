...
public interface NameSpace {
   ...
   * {@linkplain LocalName local name}. If {@code false}, name should be a fully-qualified name
   * where <code>name.{@linkplain GenericName#scope() scope()}.{@linkplain #isGlobal} == true</code>
   * .
   *
   ...
   *             for the {@linkplain ScopedName#tail tail} must refer to a {@code NameSpace}.
   ...
   * @todo This method will put a significant burden on implementations (they will need to manage a
   *     list of names, probably through weak references, etc.). Is the ISO 19103 association really
   *     naviguable that way?
