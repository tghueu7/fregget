...
public interface Stroke {
  ...
  /**
   * If non-null, indicates that line should be drawn by tiling the (thin) area of the line with the
   * given graphic. Between {@code getGraphicFill()} and {@link #getGraphicStroke()}, only one may
   * return a non-null value since a {@code Stroke} can have a {@code GraphicFill} or a {@code
   ...
   * since a {@code Stroke} can have a {@code GraphicFill} or a {@code GraphicStroke}, but not both.
   *
   * @return Graphic
   */
   ...
   * intensity values, respectively, represented as two digit hexadecimal integers. The hexadecimal
   * digits between {@code A} and {@code F} may be in either uppercase or lowercase. If null, the
   * default color is {@code "#000000"}, black.
   *
   * @return Expression
   */
  @XmlParameter("stroke")
  ...
  /**
   * Indicates how the various segments of a (thick) line string should be joined. Valid values are
   * "miter", "round", and "bevel". If null, the default value is system dependent (probably
   ...
   * The first number represents the length of the first dash to draw. The second number represents
   * the length of space to leave. This continues to the end of the list then repeats. If the list
   * contains an odd number of values, then before rendering the list is enlarged by repeating the
   * last value. If this parameter is omitted, lines will be drawn as solid and unbroken.
   *
   * @return expression
   */
  ...
  /**
   * Indicates the distance offset into the dash array to begin drawing. If null, the default value
   * is zero.
   *
   * @return expression
   */
