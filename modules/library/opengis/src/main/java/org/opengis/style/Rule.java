...
public interface Rule {
  ...
  /**
   * Returns a name for this rule. This can be any string that uniquely identifies this rule within
   * a given canvas. It is not meant to be human-friendly. (The "title" property is meant to be
   * human friendly.)
   *
   ...
   *
   * <p>A nice user interface may want to present the user with a legend that indicates how features
   * of a given type are being portrayed. Through its {@code LegendGraphic} property, a {@code Rule}
  ...
  /**
   * Returns the filter that will limit the features for which this {@code Rule} will fire. This can
   * only be non-null if {@link #isElseFilter} returns false. If this value is null and {@code
  ...
  /**
   * Returns the minimum value (inclusive) in the denominator of the current map scale at which this
   * {@code Rule} will fire. If, for example, the {@code MinScaleDenominator} were 10000, then this
   * rule would only fire at scales of 1:X where X is greater than 10000. A value of zero indicates
  ...
  /**
   * Returns the maximum value (exclusive) in the denominator of the current map scale at which this
   * {@code Rule} will fire. If, for example, the {@code MaxScaleDenominator} were 98765, then this
   * rule would only fire at scales of 1:X where X is less than 98765. A value of {@link
   ...
   * redundant cast in the code. If you dont intend to use a special interface you can override this
   * method by : List<Symbolizer> symbolizers();
   *
  ...
  /**
   * It is common to have a style coming from a external xml file, this method provide a way to get
   * the original source if there is one. OGC SLD specification can use this method to know if a
   * style must be written completely or if writing the online resource path is enough.
   *
