...
public final class PixelOrientation extends CodeList<PixelOrientation> {
  ...
  /**
   * The corner in the pixel closest to the origin of the SRS; if two are at the same distance from
   * the origin, the one with the smallest x-value.
   *
   * @todo The sentence "<cite>closest to the origin of the SRS</cite> probably applies to positive
   *     coordinates only. For the general case including both positive and negative coordinates, we
   *     should probably read "in the direction of negative infinity". This interpretation should be
   *     clarified with ISO.
  ...
  /**
   * Returns the pixel orientation that matches the given string, or returns a new one if none match
   * it.
   *
