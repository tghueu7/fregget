...
public interface IdentifiedObject {
  /**
   * Key for the <code>{@value}</code> property to be given to the {@linkplain ObjectFactory object
   ...
   * Key for the <code>{@value}</code> property to be given to the {@linkplain ObjectFactory object
   * factory} <code>createFoo(&hellip;)</code> methods. This is used for setting the value to be
   * returned by {@link #getIdentifiers}.
   *
   * @see #getIdentifiers
   */
   ...
   * This operation may fails if an object is too complex for the WKT format capability (for example
   * an {@linkplain org.opengis.referencing.crs.EngineeringCRS engineering CRS} with different unit
   * for each axis).
   *
