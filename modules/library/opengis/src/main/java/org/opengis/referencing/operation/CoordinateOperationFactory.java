...
public interface CoordinateOperationFactory extends ObjectFactory {
   ...
   * CoordinateOperationAuthorityFactory#createFromCoordinateReferenceSystemCodes query an authority
   * factory} first, and compute the operation next if no operation from {@code source} to {@code
   * target} code was explicitly defined by the authority.
   *
   ...
   * org.opengis.datum.GeodeticDatum datum} requires a <cite>datum shift</cite>. Many methods exist
   * for this purpose, including interpolations in a grid, a scale/rotation/translation in
   ...
   * CRS}, and do not need to have a {@linkplain Conversion#getMathTransform math transform}. Their
   * sole purpose is to be given as an argument to {@linkplain
