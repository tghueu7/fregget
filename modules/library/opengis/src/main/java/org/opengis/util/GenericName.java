...
public interface GenericName extends Comparable<GenericName> {
  /**
   * Returns the scope (name space) in which this name is local. The scope is set on creation and is
   * not modifiable. The scope of a name determines where a name starts.
   *
   * <p><b>Example</b>: For a {@linkplain #toFullyQualifiedName fully qualified name} (a name having
   * a {@linkplain NameSpace#isGlobal global namespace}) {@code "org.opengis.util.Record"}, if this
   * instance is the name {@code "util.Record"}, then the scope of this instance has the {@linkplain
   * NameSpace#name name} {@code "org.opengis"}.
   *
   ...
   *
   * <p><b>Example</b>: If {@code this} name is {@code "org.opengis.util.Record"}, then this method
   * shall returns {@code 4}. If this name is {@code "util.Record"} in scope {@code "org.opengis"},
   * then this method shall returns {@code 2}.
   *
   ...
   * of this sequence is the {@linkplain #depth depth}. It does not include the {@linkplain #scope
   * scope}.
   *
   * <p>This method is similar in purpose to {@link javax.naming.Name#getAll()} from the <cite>Java
   * Naming and Directory Interface</cite>.
   *
   * <p><b>Example</b>: If {@code this} name is {@code "org.opengis.util.Record"}, then this method
   * shall returns a list containing {@code {"org", "opengis", "util", "Record"}} elements in that
   * iteration order. If this name is {@code "util.Record"} in scope {@code "org.opengis"}, then
   * this method shall returns a list containing only {@code {"util", "Record"}} elements.
   *
   * @return The local names making this generic name, without the {@linkplain #scope scope}. Shall
   *     never be {@code null} neither {@linkplain List#isEmpty empty}.
   */
  ...
  /**
   * Returns the last element in the sequence of {@linkplain #getParsedNames parsed names}. For any
   * {@link LocalName}, this is always {@code this}.
   *
   * <p>This method is similar in purpose to <code>{@linkplain javax.naming.Name#get(int)
  ...
  /**
   * Returns a view of this name as a fully-qualified name. The {@linkplain #scope scope} of a fully
   * qualified name must be {@linkplain NameSpace#isGlobal global}. If the scope of this name is
   * already global, then this method shall returns {@code this}.
   *
   * <p><b>Example</b>: If {@code this} name is {@code "util.Record"} ({@linkplain #depth depth} of
   * two) and its {@linkplain #scope scope} has the {@linkplain NameSpace#name name} {@code
   ...
   *
   * <ul>
   *   <li><code>push(</code><var>foo</var><code> : LocalName).{@linkplain #head}</code> {@linkplain
   *       #equals equals} <var>foo</var>
   ...
   *   <li><code>{@linkplain #name}.toString()</code> is garanteed to <strong>not</strong> contains
   *       any scope.
   * </ul>
   *
  ...
  /**
   * Returns a local-dependent string representation of this generic name. This string is similar to
   * the one returned by {@link #toString} except that each element has been localized in the
   * {@linkplain InternationalString#toString(java.util.Locale) specified locale}. If no
   * international string is available, then this method shall returns an implementation mapping to
   * {@link #toString} for all locales.
   *
   * <p><b>Example</b>: An implementation may want to localize the {@code "My Documents"} directory
   * name into {@code "Mes Documents"} on French installation of Windows operating system.
   *
