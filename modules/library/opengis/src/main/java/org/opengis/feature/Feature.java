...
public interface Feature extends ComplexAttribute {
   ...
   *
   * <p><code>getType().isIdentifiable()</code> must return <code>true</code> so this value must not
   * return <code>null</code>.
   *
   * <p>Generation of the identifier is dependent on the underlying data storage medium. Often this
   * identifier is not persistent. Mediums such shapefiles and database tables have "keys" built in
   * which map naturally to persistent feature identifiers. But other mediums do not have such keys
   * and may have to generate feature identifiers "on-the-fly". This means that client code being
   * able to depend on this value as a persistent entity is dependent on which storage medium or
   ...
   *
   * <p>In the case that the feature has no geometric attributes this method should return an empty
   * bounds, ie, <code>bounds.isEmpty() == true</code>. This method should never return <code>null
   * </code>.
   *
   ...
   * @throws IllegalArgumentException If the specified attribute is not already an attribute of the
   *     feature.
   */
