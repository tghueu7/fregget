...
public interface Geometry extends TransfiniteSet {
   ...
   * all {@linkplain org.opengis.geometry.primitive.Primitive primitive} elements and for all {@link
  ...
  /**
   * Returns a point value that is guaranteed to be on this {@code Geometry}. The default logic may
   * be to use the {@linkplain DirectPosition direct position} of the point returned by {@link
   ...
   * structure where appropriate. The finite set of {@code Geometry}s returned shall be in the same
   * coordinate reference system as this {@code Geometry}. If the {@code Geometry} is in a
   * {@linkplain Complex complex}, then the boundary {@code Geometry}s returned shall be in the same
   * {@linkplain Complex complex}. If the {@code Geometry} is not in any {@linkplain Complex
   * complex}, then the boundary {@code Geometry}s returned may have been constructed in response to
   * the operation. The elements of a boundary shall be smaller in dimension than the original
   * element.
   *
  ...
  /**
   * Returns a finite set of {@code Geometry}s containing all of the points on the boundary of this
   * {@code Geometry} and this object (the union of the object and its boundary). These object
   * collections shall have further internal structure where appropriate. The finite set of {@code
   * Geometry}s returned shall be in the same coordinate reference system as this {@code Geometry}.
   * If the {@code Geometry} is in a {@linkplain Complex complex}, then the boundary {@code
   * Geometry}s returned shall be in the same {@linkplain Complex complex}. If the {@code Geometry}
   * is not in any {@linkplain Complex complex}, then the boundary {@code Geometry}s returned may
   * have been constructed in response to the operation.
   *
   ...
   *
   * <p>Since most coordinate geometries are represented, either directly or indirectly by functions
   * from regions in Euclidean space of their topological dimension, the easiest test for simplicity
   * to require that a function exist that is one-to-one and bicontinuous (continuous in both
   * directions). Such a function is a topological isomorphism. This test does not work for "closed"
   * objects (that is, objects for which {@link #isCycle} returns {@code true}).
   *
   * @return {@code true} if this object has no interior point of self-intersection or selftangency.
   * @see #isCycle
   */
   ...
   * alternatively referred to as being "closed" as in a "closed curve." This creates some confusion
   * since there are two distinct and incompatible definitions for the word "closed". The use of the
   * word cycle is rarer (generally restricted to the field of algebraic topology), but leads to
   * less confusion. Essentially, an object is a cycle if it is isomorphic to a geometric object
   * that is the boundary of a region in some Euclidean space. Thus a curve is a cycle if it is
   * isomorphic to a circle (has the same start and end point). A surface is a cycle if it
   * isomorphic to the surface of a sphere, or some torus. A solid, with finite size, in a space of
   * dimension 3 is never a cycle.
   *
  ...
  /**
   * Returns the distance between this {@code Geometry} and another {@code Geometry}. This distance
   * is defined to be the greatest lower bound of the set of distances between all pairs of points
   * that include one each from each of the two {@code Geometry}s. A "distance" value shall be a
   * positive number associated to a distance unit such as meter or standard foot. If necessary, the
   * second geometric object shall be transformed into the same coordinate reference system as the
   * first before the distance is calculated.
   *
   ...
   * points (and therefore between geometric objects): map distance, geodesic distance, and terrain
   * distance.
   *
   * <ul>
   ...
   *       is usually accurate for small areas where scale functions have well-behaved derivatives.
   ...
   *       account. It is especially handy for air and sea navigation, although care should be taken
   *       to distinguish between rhumb line (curves of constant bearing) and geodesic curve
   *       distance.
   ...
   * the {@linkplain #getCoordinateDimension coordinate dimension}. The dimension of a collection of
   * geometric objects shall be the largest dimension of any of its pieces. Points are
   ...
   * point. Dimension is unambiguously defined only for {@linkplain DirectPosition direct positions}
   * interior to this {@code Geometry}. If the passed {@linkplain DirectPosition direct position} is
   * {@code null}, then the operation shall return the largest possible dimension for any
   * {@linkplain DirectPosition direct position} in this {@code Geometry}.
   *
   ...
   * same as the coordinate dimension of the coordinate reference system for this {@code Geometry}.
   *
  ...
  /**
   * Returns the set of maximal complexes within which this {@code Geometry} is contained. As a set
   * of primitives, a {@linkplain Complex complex} may be contained as a set in another larger
  ...
  /**
   * Returns the minimum bounding box for this {@code Geometry}. This shall be the coordinate region
   * spanning the minimum and maximum value for each ordinate taken on by {@linkplain DirectPosition
   * direct positions} in this {@code Geometry}. The simplest representation for an envelope
   * consists of two {@linkplain DirectPosition direct positions}, the first one containing all the
  ...
  /**
   * Returns the mathematical centroid for this {@code Geometry}. The result is not guaranteed to be
   * on the object. For heterogeneous collections of primitives, the centroid only takes into
   * account those of the largest dimension. For example, when calculating the centroid of surfaces,
   * an average is taken weighted by area. Since curves have no area they do not contribute to the
   * average.
   *
   * @return The centroid.
  ...
  /**
   * Returns a {@code Geometry} that represents the convex hull of this {@code Geometry}. Convexity
   * requires the use of "lines" or "curves of shortest length" and the use of different coordinate
   * systems may result in different versions of the convex hull of an object. Each implementation
   * shall decide on an appropriate solution to this ambiguity. For two reasonable coordinate
   * systems, a convex hull of an object in one will be very closely approximated by the transformed
   * image of the convex hull of the same object in the other.
   *
   ...
   * {@code Geometry} is normally the same as the coordinate dimension - a collection of {@linkplain
   * org.opengis.geometry.primitive.Surface surfaces} in 2D space and a collection of {@linkplain
   * org.opengis.geometry.primitive.Solid solids} in 3D space, but this may be application defined.
   *
   * @param distance The distance.
   * @return A geometry containing all points whose distance from this {@code Geometry} is less than
   ...
   * <UL>
   *   <LI>
   *       <p>If {@code false}, then this geometry is <cite>immutable</cite>. It is guarantee that a
   *       call to any {@code setFoo(...)} method will throws an {@link
   ...
   *       <cite>mutable</cite> geometry is not synonymous of <cite>modifiable</cite> geometry. The
   *       nuance lays in whatever the geometry may changes its state directly (as of user request)
   *       or indirectly:
   *       <p>
   *       <UL>
   *         <LI>
   ...
   *         <LI>
   *             <p>This geometry may still <cite>unmodifiable</cite>. User is not allowed to modify
   *             it himself and invoking any {@code setFoo(...)} method will throws an {@link
   *             UnmodifiableGeometryException}. However, the implementation may change the geometry
   *             itself (for example a time-varying geometry).
   *       </UL>
   * </UL>
   *
   ...
   * values will never change. Any attempts to change the values of the returned object will result
   * in a {@code UnmodifiableGeometryException}.
   *
   ...
   * are encouraged to share as much internal data as possible (as opposed to performing a real copy
   * of the data), while preserving the deep copy semantic.
   *
   * <p>Special cases:
   *
   * <UL>
   ...
   *       there is no need for cloning this object. This method may return {@code this} or returns
   *       a modifiable copy of this object, at implementation choice.
   *   <LI>
   ...
   * @throws CloneNotSupportedException if this object do not support clone. This exception is never
   *     throws if this object implements {@link Cloneable}.
   * @see Cloneable
   * @see #isMutable
   */
