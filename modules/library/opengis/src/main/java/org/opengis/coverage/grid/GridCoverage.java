...
public interface GridCoverage extends Coverage {
   ...
   * application can achieve better performance for a 2-D grid coverage by reading blocks of 128 by
   * 128 if the grid is tiled into blocks of this size. The sequence is ordered by dimension. If the
   * implementation does not have optimal sizes, the sequence will be {@code null}.
   *
   * @return The optimal size to use for each dimension when accessing grid values, or {@code null}
   *     if none.
   */
  ...
  /**
   * Returns a pre-calculated overview for a grid coverage. The overview indices are numbered from 0
   * to <code>{@linkplain #getNumOverviews numberOverviews}-1</code>. The overviews are ordered from
   * highest (index 0) to lowest (<code>{@linkplain #getNumOverviews numberOverviews}-1</code>)
   * resolution. Overview grid coverages will have overviews which are the overviews for the grid
   * coverage with lower resolution than the overview. For example, a 1 meter grid coverage with 3,
   * 9, and 27 meter overviews will be ordered as in the left side below. The 3 meter overview will
   * have 2 overviews as in the right side below:
   *
   * <blockquote>
   *
   * <table border=0>
  ...
  /**
   * Returns the sources data for a grid coverage. If the {@code GridCoverage} was produced from an
   * underlying dataset (by {@link GridCoverageReader#read read(...)} for instance), this method
   ...
   * coverages of the one used as input to {@code GridCoverageProcessor}. In general this method is
   * intended to return the original {@code GridCoverage} on which it depends.
   *
