...
public interface GridCoverageReader {
  ...
  /**
   * Returns the input source. It can be a {@link java.lang.String}, an {@link java.io.InputStream},
   * a {@link java.nio.channels.FileChannel}, whatever.
   */
  Object getSource();

  /**
   * Returns the list of metadata keywords associated with the {@linkplain #getSource input source}
   * as a whole (not associated with any particular grid coverage). If no metadata is available, the
   * array will be empty.
   *
  ...
  /**
   * Retrieve the list of grid coverages contained within the {@linkplain #getSource input source}.
   ...
   *     returned as paths? Explain what to return if the GridCoverage are accessible by index only.
   *     A proposal is to name them "grid1", "grid2", etc.
   * @deprecated use {@link #getGridCoverageNames()}
   */
  String[] listSubNames() throws IOException;

  /**
   * Retrieve the list of coverages contained within the {@linkplain #getSource input source}. Each
   * grid can have a different coordinate system, number of dimensions and grid geometry. For
   * example, a HDF-EOS file (GRID.HDF) contains 6 grid coverages each having a different
   * projection. An empty array will be returned if no sub names exist.
   *
   * @return The list of grid coverages contained within the input source.
  ...
  /**
   * Returns the name for the next grid coverage to be {@linkplain #read read} from the {@linkplain
   ...
   * @throws CannotCreateGridCoverageException if the coverage can't be created for a logical reason
   *     (for example an unsupported format, or an inconsistency found in the data).
  ...
  /**
   * Allows any resources held by this object to be released. The result of calling any other method
   * subsequent to a call to this method is undefined. It is important for applications to call this
   * method when they know they will no longer be using this {@code GridCoverageReader}. Otherwise,
   * the reader may continue to hold on to resources indefinitely.
   *
   * @throws IOException if an error occured while disposing resources (for example while closing a
   *     file).
   */
