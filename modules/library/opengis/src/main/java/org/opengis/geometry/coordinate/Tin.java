...
public interface Tin extends TriangulatedSurface {
   ...
   * result shall be a change of the surface boundary. The attribute {@code stopLines} contains all
   * these pathological segments as a set of line strings.
   */
  ...
  /**
   * Maximal length for retention. Areas of the surface where the data is not sufficiently dense to
   * assure reasonable calculations shall be removed by adding a retention criterion for triangles
   * based on the length of their sides. For any triangle sides exceeding maximum length, the
   ...
   * controlPoint} shall contain a set of the {@linkplain Position positions} used as posts for this
   * TIN. Since each TIN contains triangles, there must be at least 3 posts. The order in which
   * these points are given does not affect the surface that is represented. Application schemas may
   * add information based on the ordering of the control points to facilitate the reconstruction of
