...
public abstract class CodeList<E extends CodeList<E>> implements Comparable<E>, Serializable {
   ...
   * annotation or if the annotation contains an empty string. This field will be computed only when
   * first needed.
   */
   ...
   * give a static reference to an {@link java.util.ArrayList} for the {@code values} argument. This
   * list is used for {@code values()} method implementations.
   *
  ...
  /**
   * Returns the identifier declared in the {@link UML} annotation, or {@code null} if none. The UML
   * identifier shall be the ISO or OGC name for this code constant.
   *
  ...
  /**
   * Compares this code with the specified object for order. Returns a negative integer, zero, or a
