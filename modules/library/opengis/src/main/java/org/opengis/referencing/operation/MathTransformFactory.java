...
public interface MathTransformFactory extends Factory {
   ...
   * a derived CS. This convenience method {@linkplain #createConcatenatedTransform concatenates}
   ...
   *
   * <p>Cartographic projection transforms are used by {@linkplain ProjectedCRS projected coordinate
   * reference systems} to map geographic coordinates (e.g. <var>longitude</var> and
   * <var>latitude</var>) into (<var>x</var>,<var>y</var>) coordinates. These
   * (<var>x</var>,<var>y</var>) coordinates can be imagined to lie on a plane, such as a paper map
   * or a screen. All cartographic projection transforms created through this method will have the
   ...
   * projected CRSs use grads instead of degrees, and often the {@linkplain ProjectedCRS#getBaseCRS
   * base geographic CRS} is (<var>latitude</var>, <var>longitude</var>) instead of
   * (<var>longitude</var>, <var>latitude</var>). This means that the cartographic projected
   * transform is often used as a single step in a series of transforms, where the other steps
   ...
   * allows transforms to operate on a subset of ordinates. For example giving (<var>latitude</var>,
   * <var>longitude</var>, <var>height</var>) coordinates, a pass through transform can convert the
   * height values from meters to feet without affecting the (<var>latitude</var>,
   * <var>longitude</var>) values.
   *
