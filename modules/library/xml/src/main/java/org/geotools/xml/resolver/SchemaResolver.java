...
public class SchemaResolver {
  ...
  private static final Logger LOGGER =
      org.geotools.util.logging.Logging.getLogger(SchemaResolver.class.getPackage().getName());
   ...
   * original HTTP URL used to obtain it. This is required so that relative imports can be resolved
   * if they cross resolution boundaries. For example, an import ../../../om/.. used to import om in
   * a schema, where one is supplied locally and the other must be downloaded and cached. Another
   * example is when the schemas are in different jar files.
   */
   ...
   * @param context an absolute URL specifying the context schema of a relative location, or null if
   *     none
   ...
   * allows (for example) schema documents in jar files to be loaded from the classpath using their
   * canonical HTTP URLs.
   *
   * @param location
