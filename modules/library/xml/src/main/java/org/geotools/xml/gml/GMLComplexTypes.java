...
public class GMLComplexTypes {
    ...
    private static final String STREAM_FEATURE_NAME_HINT = "org.geotools.xml.gml" +
     ".STREAM_FEATURE_NAME_HINT";
        ...
        if (g.getUserData() != null) {
            ...
            if (g.getUserData() instanceof String) {
                ...
                try {
                    ...
                    logger.log(Level.WARNING, "Could not encode the srsName for geometry, no EPSG" +
                     " code found", ex);
                }
            }
        } else {
...
//            }
    ...
    static void encode(Element e, Polygon g, PrintHandler output)
        ...
        ((GMLSchema.getInstance()).getElements()[35]).getType().encode((GMLSchema.getInstance())
        .getElements()[35], g.getExteriorRing(), output, null);

        if (g.getNumInteriorRing() > 0) {
            for (int i = 0; i < g.getNumInteriorRing(); i++)
                ((GMLSchema.getInstance()).getElements()[36]).getType().encode((GMLSchema
                .getInstance()).getElements()[36], g.getInteriorRingN(i), output, null);
        }
    ...
    static void encodeCoords(Element e, Envelope env, PrintHandler output)
        ...
        output.characters(env.getMinX() + cs + env.getMinY() + ts + env.getMaxX() + cs + env
        .getMaxY());
    ...
    public static class LineStringType extends GMLComplexType {
        ...
        public Object getValue(Element element, ElementValue[] value,
            ...
            if (t instanceof Coordinate) {
                ...
                // TODO -- be forgiving
                if (c.size() == 1) {
                    c = DefaultCoordinateSequenceFactory.instance().create(new Coordinate[]{c
                    .getCoordinate(0), c.getCoordinate(0)});
                }
                p = gf.createLineString(c);
            }
    ...
    public static class AbstractFeatureType extends GMLComplexType {
        ...
        public Object getValue(Element element, ElementValue[] value,
            ...
            if ((nm != null) && nm.equals(element.getName())) {
                f = getFeature(element, value, attrs, hints, ((FCBuffer) hints.get
                (XMLHandlerHints.STREAM_HINT)).ft);
        ...
        public SimpleFeature getFeature(Element element, ElementValue[] value,
                                        Attributes attrs, Map hints, SimpleFeatureType ft) throws
                                         SAXException {
            if (ft == null)
        ...
        private void setAttribute(ElementValue[] value, SimpleFeatureType ft, Object[] values, 
        int i, int j) {
         ...
         */
        private void searchByType(ElementValue[] value, SimpleFeatureType ft, Object[] values, 
        int i) {
        ...
        private void assignValue(ElementValue[] value, Object[] values, AttributeDescriptor at, 
        int i, int k) {
        ...
        private int searchByName(ElementValue[] value, SimpleFeatureType ft, int i, int j) {
            for (int k = 0; k < ft.getAttributeCount() && j == -1; k++) {
                // TODO use equals
                if ((ft.getDescriptor(k).getLocalName() == null && value[i].getElement().getName
                () == null) ||
...
//                        Thread.yield();
        ...
        private SimpleFeatureType loadFeatureType(Element element,
                                                  ElementValue[] value, Attributes attrs) throws 
                                                  SAXException {
        ...
        public void encode(Element element, Object value, PrintHandler output,
            ...
            while (i.hasNext()) {
                ...
                if (e == null) { // first time
                    ...
                    ComplexType t = e.getType() instanceof ComplexType ? (ComplexType) e.getType
                    () : null;
                    while (t != null && t != AbstractFeatureType.getInstance())
                        t = t.getParent() instanceof ComplexType ? (ComplexType) t.getParent() : 
                        null;
                    if (t != AbstractFeatureType.getInstance()) {
                        ...
                        while (t != null && t != AbstractFeatureType.getInstance())
                            ...
                            () : null;
                        if (t != AbstractFeatureType.getInstance()) {
                            throw new OperationNotSupportedException("Could not find a correct " +
                             "Element for FeatureType " + f.getFeatureType().getTypeName());
                        }
                    }
                }
    ...
    public static class BoundingShapeType extends GMLComplexType {
        ...
        public Object getValue(Element element, ElementValue[] value,
            ...
            return value[0].getValue() instanceof Geometry ? (Geometry) value[0].getValue() : 
            NULL_ENV;
        }
...
//                BoxType.getInstance().encode(null, value, output, hints);
    ...
    public static class PointPropertyType extends GMLComplexType {
        ...
        public void encode(Element element, Object value, PrintHandler output,
                           Map hints) throws IOException, OperationNotSupportedException {
            if ((value == null) || !(value instanceof Point)) {
                throw new OperationNotSupportedException("Value is " + value == null ? "null" : 
...
//        }
        ...
        AttributeDescriptor[] attrs = (AttributeDescriptor[]) getAttributes(element.getName(), 
        child).toArray(new AttributeDescriptor[]{,});
...
//                    .equalsIgnoreCase(BoxType.getInstance().getName())) {
                            .equalsIgnoreCase(AbstractFeatureType.getInstance().getChildElements
                            ()[2].getName())) {
        ...
        for (AttributeDescriptor attributeDescriptor : attrs) {
            ...
            if ((geometryAttribute == null)
                ...
                if (!attributeDescriptor.getLocalName()
                        .equalsIgnoreCase(AbstractFeatureType.getInstance().getChildElements()[2]
                        .getName())) {
    ...
    private static AttributeDescriptor getAttribute(Element eg) {
        if (eg.getNamespace() == GMLSchema.NAMESPACE && (AbstractFeatureType.getInstance()
        .getChildElements()[0] == eg || AbstractFeatureType.getInstance().getChildElements()[1] 
        == eg || AbstractFeatureType.getInstance().getChildElements()[2] == eg))
            return null;
        ...
        if (eg.getType() != null) {
            if (eg.getType() instanceof SimpleType) {
                ...
                if (Object.class.equals(eg.getType().getInstanceType()) || Object[].class.equals
                (eg.getType().getInstanceType())) {
                    // some work now
...
//                        nillable);
    ...
    private static AttributeDescriptor getAttribute(String name, Choice eg) {
        ...
        // nillable should really be nillable, but in gt2.X nillable in an attribute is 
        // equivalent to minOccurs == 0 as well
        ...
        if (Geometry.class.isAssignableFrom(type)) {
            return new ChoiceGeometryTypeImpl(typeName, choices, type, nillable, 1, 1, null, 
            null, Collections.EMPTY_LIST);
        } else {
            return new ChoiceAttributeTypeImpl(typeName, choices, type, nillable, 1, 1, null, 
    ...
    /**
     * This is a bit of a hack to allow certain special cases.  This is required because the 
     * feature model does not
     * support choices and making a Choice (Polygon, MultiPolygon) a Geometry is terrible because
      * editing will allow all
