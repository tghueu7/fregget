...
public class GML {
  ...
  /**
   * {@link ContextCustomizer} setting up a {@link FeatureTypeCache} that will not cache dynamically
   * build feature types, thus allowing all features to have their own schema, instead of relying on
   * the schema of the first feature found
   */
   ...
   *
   * <p>Note that when working with GML2 you need to supply additional information prior to use (in
   ...
   * correct (something you can only tell when parsing the produced result!). Setting this value to
   * false will use the same GMLConfiguration employed when parsing and has less risk of producing
   * invalid content.
   *
   * @param legacy
   */
  ...
  /**
   * Decodes a feature collection from the stream provided. It assumes the features are uniform and
   * that all attributes are available in the first feature
   *
   * @param in
   * @return
   * @throws IOException
   ...
   *
   * @param in
   * @param computeFullFeatureType When true, all features are parsed and then a global feature type
   *     is determined that has attributes covering all feature needs, when false, the first feature
   *     attributes
   * @return
   * @throws IOException
   * @throws SAXException
   ...
   * schema is not available or correctly defined. The returned elements are wrapped up as a Feature
   * if needed. This mehtod can be used to retrive only the Geometry elements from a GML docuemnt.
   *
   * @param in InputStream used as a source of SimpleFeature content
   * @param xpath Optional xpath used to indicate simple feature element; the schema will be checked
   ...
   *
   * <p>This iterator is actually forgiving; and willing to "morph" content into a SimpleFeature if
   * needed.
   *
   * <ul>
