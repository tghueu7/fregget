...
public class SchemaHandler extends XSIElementHandler {
  ...
  public void startPrefixMapping(String pref, String uri1) {
    if (targetNamespace == null) {
      ...
      if (!pref.trim().equals("") || !prefixCache.containsKey(uri1)) prefixCache.put(uri1, pref);
  ...
  protected Schema compress(URI thisURI) throws SAXException {
    ...
    attributeGroups =
        attributes =
            complexTypes = simpleTypes = elements = groups = imports = includes = redefines = null;
   ...
   * resulting values (not placeholders). Making this evaluate place holders would cause the parser
   * to fail !!!
   *
   * @author dzwiers
   */
