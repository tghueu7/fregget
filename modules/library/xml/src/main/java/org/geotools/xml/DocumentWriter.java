...
public class DocumentWriter {
  ...
  /**
   * Element or String ... include a ref to an Element to be used, or a string representing the name
   * of the element
   */
  public static final String BASE_ELEMENT = "DocumentWriter_BASE_ELEMENT";

  /**
   * Schema[] or String[]... The order to search the schemas for a valid element, either an array of
   * ref to Schema instances or an Array or TargetNamespaces
   */
  ...
  public static void writeDocument(Object value, Schema schema, File f, Map hints)
    ...
    if ((hints != null) && hints.containsKey(WRITE_SCHEMA)) {
      ...
      File f2 =
          new File(f.getParentFile(), f.getName().substring(0, f.getName().indexOf(".")) + ".xsd");
  ...
  private static void writeSchema(Schema schema, PrintHandler ph, Map hints) throws IOException {
    ...
    ai.addAttribute(
        "", "xmlns:" + schema.getPrefix(), "", "anyUri", schema.getTargetNamespace().toString());
    ...
    if (element.getNamespace().equals(schema.getTargetNamespace())) {
      ...
      if (!found) {
        ...
        if (!element.getType().getNamespace().equals(schema.getTargetNamespace())) {
          ...
          ai.addAttribute(
              "", "type", "", "QName", s.getPrefix() + ":" + element.getType().getName());
        }
  ...
  private static void writeAttribute(Attribute attribute, Schema schema, PrintHandler ph, Map hints)
      throws IOException {
    AttributesImpl ai = new AttributesImpl();
    ...
    if (attribute.getNamespace().equals(schema.getTargetNamespace())) {
      ...
      if (!found) {
        ...
        if (!attribute.getSimpleType().getNamespace().equals(schema.getTargetNamespace())) {
          ...
          ai.addAttribute(
              "", "type", "", "QName", s.getPrefix() + ":" + attribute.getSimpleType().getName());
        }
  ...
  private static void writeAttributeGroup(
      AttributeGroup attributeGroup, Schema schema, PrintHandler ph, Map hints) throws IOException {
    AttributesImpl ai = new AttributesImpl();
  ...
  private static void writeSimpleType(
    ...
    if (simpleType.getFinal() != Schema.NONE) {
      ai.addAttribute(
          "", "final", "", "NMTOKENS", ComplexTypeHandler.writeFinal(simpleType.getFinal()));
    }
  ...
  private static void writeComplexType(
    ...
    if (complexType.getFinal() != Schema.NONE) {
      ai.addAttribute(
          "", "final", "", "NMTOKENS", ComplexTypeHandler.writeFinal(complexType.getFinal()));
    }

    if (complexType.getBlock() != Schema.NONE) {
      ai.addAttribute(
          "", "block", "", "NMTOKENS", ComplexTypeHandler.writeBlock(complexType.getBlock()));
    }
    ...
    // TODO determine if this complexType isDerived ... and make a complexContent or Simplecontent
    // to match
    if (egs != null) {
  ...
  private static class WriterContentHandler implements PrintHandler {
    ...
    private Map
        prefixMappings; // when the value is null it has not been included yet into the document ...
