...
public class FilterOpsComplexTypes {
      ...
      switch (Filters.getFilterType(lf)) {
        case COMPARE_EQUALS:
          BinaryComparisonOpType.getInstance()
              .encode(
                  new FilterElement(
                      "PropertyIsEqualTo", BinaryComparisonOpType.getInstance(), element),
                  value,
                  output,
                  hints);
        ...
        case COMPARE_LESS_THAN:
          BinaryComparisonOpType.getInstance()
              .encode(
                  new FilterElement(
                      "PropertyIsLessThan", BinaryComparisonOpType.getInstance(), element),
                  value,
                  output,
                  hints);
        ...
        case BETWEEN:
          PropertyIsBetweenType.getInstance()
              .encode(
                  new FilterElement(
                      "PropertyIsBetween", PropertyIsBetweenType.getInstance(), element),
                  value,
                  output,
                  hints);
      ...
      switch (Filters.getFilterType(lf)) {
        case GEOMETRY_BBOX:
          BBOXType.getInstance()
              .encode(
                  new FilterElement("BBOX", BBOXType.getInstance(), element), value, output, hints);
  ...
  public static class FilterType extends FilterComplexType
    ...
    /**
     * Note the assumption is that the comparison of this filter with the WFS capabilities document
  ...
  public static class FeatureIdType extends FilterComplexType {
    ...
    public void encode(Element element, Object value, PrintHandler output, Map hints)
      ...
      for (String fid : fids) {
        att.setAttribute(
            0, element.getNamespace().toString(), attrs[0].getName(), null, "anyUri", fid);
  ...
  public static class PropertyIsLikeType extends FilterComplexType {
    ...
    public void encode(Element element, Object value, PrintHandler output, Map hints)
      ...
      at.addAttribute(
          FilterSchema.NAMESPACE.toString(), "wildCard", null, "string", lf.getWildCard());
      at.addAttribute(
          FilterSchema.NAMESPACE.toString(), "singleChar", null, "string", lf.getSingleChar());
  ...
  public static class BinarySpatialOpType extends FilterComplexType {
    ...
    private static Sequence child =
        new SequenceGT(
            new ElementGrouping[] {elems[0], new ChoiceGT(new Element[] {elems[1], elems[2]})});
        ...
        if ((type2 == org.geotools.filter.ExpressionType.LITERAL_STRING)
          ...
          if (type1 == org.geotools.filter.ExpressionType.LITERAL_GEOMETRY) {
            elems[1]
                .getType()
                .encode(
                    elems[1], ((Literal) lf.getExpression1()).getValue(), output, hints); // geom
          } else {
            elems[2]
                .getType()
                .encode(
                    elems[2], ((Literal) lf.getExpression1()).getValue(), output, hints); // geom
          }
        } else {
          throw new OperationNotSupportedException(
  ...
  /**
   * The <BBOX> element is defined as a convenient and more compact way of encoding the very common
   * bounding box constraint based on the gml:Box geometry. It is equivalent to the spatial
  ...
  public static class BBOXType extends FilterComplexType {
    ...
    public Class getInstanceType() {
      return Filter
          .class; // was GeometryFilter.class but use of Disjoint.not() limits this to Filte
    }
