...
public abstract class XSIElementHandler implements Serializable {
   ...
   * parse + store all the information required to recreate the exact Schema document being parsed.
   * As a result, information such as annotations are ignored. When used, they method may be called
   * multiple times for one element. This means the implementor should keep this in mind when
   ...
   * expected to exist within this method, along with data logging for post-processing. This method
   * will directly affect the stack being used to complete the parse.
   *
   * @param namespaceURI
   * @param localName
  ...
  /**
   * Returns the LocalName for this element (ie this declaration in the Schema ... so ComplexType or
   * Sequence ...)
   *
