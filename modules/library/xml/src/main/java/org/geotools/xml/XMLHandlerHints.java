...
public class XMLHandlerHints implements Map<String, Object> {
  ...
  /**
   * Declares the schemas to use for parsing. Value must be a java.util.Map of <String,URI> objects
   ...
   *  </code></pre>
   *
   * <p><b>IMPORTANT:</b> If this compliance level is used and a non-strict FilterFactory is used to
   * create the filter then the original filter must be ran on the retrieved feature because this
   * hint will sometimes cause more features to be returned than is requested. Consider the
   * following filter:
   *
   * <p>not(fidFilter).
   *
