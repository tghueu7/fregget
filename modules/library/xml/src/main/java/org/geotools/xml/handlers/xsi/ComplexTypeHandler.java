...
public class ComplexTypeHandler extends XSIElementHandler {
  ...
  protected ComplexType compress(SchemaHandler parent) throws SAXException {
          ...
          if (ct != null && ext.getChild() != null) {
            ...
            dct.child =
                loadNewEG(
                    extensionBaseType,
                    extensionChild,
                    parent); // note should override element def only ... not spot
          } else {
    ...
    if (((cache.getChild() == null) && !abstracT) && !dct.simple) {
      logger.warning(
          getName() + " :: " + parent.getTargetNamespace() + " should have a real child: ");
      throw new NullPointerException();
    }

    if ((dct.parent == null) && dct.simple) {
      logger.warning(
          getName() + " :: " + parent.getTargetNamespace() + " should have a real parent: ");
      throw new NullPointerException();
    }
  ...
  private static class DefaultComplexType implements ComplexType {
     ...
     */
    public Object getValue(Element element, ElementValue[] value, final Attributes attrs, Map hints)
