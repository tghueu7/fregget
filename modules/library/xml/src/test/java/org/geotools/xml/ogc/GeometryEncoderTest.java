...
//        DocumentWriter.writeDocument(geom, schema, writer, new HashMap());
        ...
        PrintHandler output = new PrintHandler() {
            ...
            public void element(URI namespaceURI, String localName, Attributes attributes) throws
                    IOException {
            }
            ...
            public void startElement(URI namespaceURI, String localName, Attributes attributes) 
                ...
                if (attributes != null) {
                    for (int i = 0; i < attributes.getLength(); i++) {
                        writer.write(" " + attributes.getLocalName(i) + "=" + attributes.getValue
                                (i));
                    }
                }
                writer.write(">");
        ...
        String expected = "<GEOM><MultiPolygon srsName=EPSG:4326><polygonMember><Polygon " +
                "srsName=EPSG:4326><outerBoundaryIs><LinearRing><coordinates decimal=. cs=, ts= " +
                ">0.0,0.0 10.0,0.0 0.0,10.0 0.0," +
                "0.0</coordinates></LinearRing></outerBoundaryIs></Polygon></polygonMember" +
                "></MultiPolygon></GEOM>";
