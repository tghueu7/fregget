...
public final class ECQLGeoOperationTest extends CQLGeoOperationTest {
    ...
    public void IntersectsWithExpressions() throws CQLException {
        ...
        Filter resultFilter = CompilerUtil.parseFilter(language, "INTERSECTS(POLYGON((1 2, 2 2, 2" +
                " 3, 1 2)), POINT(1 2))");
    ...
    public void functionAsFirstArgument() throws CQLException {
        ...
        Filter resultFilter = CompilerUtil.parseFilter(language, "INTERSECTS(centroid(the_geom), " +
                "POINT(1 2))");
    ...
    public void functionAsSecondArgument() throws CQLException {
        ...
        Filter resultFilter = CompilerUtil.parseFilter(language, "INTERSECTS(the_geom, buffer" +
                "(POINT(1 2),10))");

        assertTrue("Intersects was expected", resultFilter instanceof Intersects);


        resultFilter = CompilerUtil.parseFilter(language, "INTERSECTS(the_geom, buffer(the_geom," +
                "10))");
    ...
    public void intersectsWithReferencedGeometry() throws CQLException, FactoryException {
        ...
        Filter resultFilter = CompilerUtil.parseFilter(language, "INTERSECTS(the_geom, SRID=4326;" +
                "POINT(1 2))");

        assertTrue("Intersects was expected", resultFilter instanceof Intersects);
        Intersects intersects = (Intersects) resultFilter;
        Literal geomLiteral = (Literal) intersects.getExpression2();
        assertEquals(CRS.decode("EPSG:4326", true), geomLiteral.evaluate(null, Geometry.class)
                .getUserData());
    }
