...
public abstract class AbstractFilterBuilder {
  ...
  public PropertyIsLike buildLikeFilter(boolean matchCase) throws CQLException {
    ...
    try {
      ...
      PropertyIsLike f =
          filterFactory.like(expr, pattern.toString(), WC_MULTI, WC_SINGLE, ESCAPE, matchCase);
  ...
  /**
   * Removes initial and final "'" from string. If some "''" is found it will be changed by a single
   * quote "'".
   *
   * @param source
  ...
  public BinarySpatialOperator buildSpatialEqualFilter() throws CQLException {
    ...
    FilterFactory2 ff =
        (FilterFactory2)
            filterFactory; // TODO this cast must be removed. It depends of Geometry implementation
  ...
  public And buildPropertyBetweenDates() throws CQLException {
    ...
    And filter =
        filterFactory.and(
            filterFactory.lessOrEqual(begin, property), filterFactory.lessOrEqual(property, end));
