...
public class CircularArcTest {
  ...
  public void testOutsideSequence() {
    ...
    double[] expected =
        circle.samplePoints(halfStep, halfStep * 2, halfStep * 3, halfStep * 4, halfStep * 5);
  ...
  public void testOutsideSequenceClockwise() {
    ...
    // linearize, we should get back the control points, plus the regular points in the middle
    double[] expected =
        circle.samplePoints(halfStep * 5, halfStep * 4, halfStep * 3, halfStep * 2, halfStep);
    assertArrayEquals(expected, arc.linearize(0.1), Circle.EPS);
    assertEquals(envelopeFrom(arc), arc.getEnvelope());
  }
  ...
  public void testOrientations() {
    ...
    // half circle up
    assertEquals(
        COUNTER_CLOCKWISE, getOrientationIndex(getLinearizedArc(circle, 0, Math.PI / 2, Math.PI)));
    ...
    assertEquals(
        CLOCKWISE, getOrientationIndex(getLinearizedArc(circle, 0, Math.PI * 3 / 2, Math.PI)));
    assertEquals(
        CLOCKWISE, getOrientationIndex(getLinearizedArc(circle, 0, -Math.PI / 2, -Math.PI)));
    ...
    assertEquals(
        CLOCKWISE,
        getOrientationIndex(getLinearizedArc(circle, Math.PI * 3 / 8, Math.PI / 4, Math.PI / 2)));
  }
