...
public interface FeatureTypeStyle extends org.opengis.style.FeatureTypeStyle {
   ...
   *       with the underlying map using color multiplication
   * </ul>
   *
   ...
   * rendering should be derived from an image that has an alpha channel and transparent background
   * (as most of the operations consider the transparency of the target surface in their math)
   */
   ...
   * sortByGroup will have their features globally ordered before painting, for example, in order to
   * respect their real world relationships. FeatureTypeStype are grouped only if they are adjacent
   * in the overall MapContent (even across layers). In case compositing is used in the same
   * FeatureTypeStyle, the first value in group will be used for the entire group.
   */
  ...
  /**
   * Only features with the type name returned by this method should be styled by this feature type
   * styler.
   *
   ...
   * community-controlled name(s), what the style is suitable to be used for. For example, a single
   * style may be suitable to use with many different feature types. The syntax of the
   ...
   * corresponding default geometry type (i.e., no feature properties are referenced in the feature
   * type style).
   *
   ...
   * contains conditions based on scale and feature attribute values. In addition, rules contain the
   * symbolizers which should be applied when the rule holds true.
   *
   * @version SLD 1.0
   ...
   * the original source if there is one.
   *
   ...
   * taking a feature collection or a grid coverage as the evaluation context and returns a feature
