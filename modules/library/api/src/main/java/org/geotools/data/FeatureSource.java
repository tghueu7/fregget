...
public interface FeatureSource<T extends FeatureType, F extends Feature> {
  ...
  /**
   * Returns the name of the features (strictly, the name of the {@code AttributeDescriptor} for the
   * features) accessible through this {@code FeatureSource}.
   *
   * <p>The value returned by this method can be different to that returned by {@code
   * featureSource.getSchema().getType().getName()}. This is because there is a distinction between
   * the name applied to features and the name of a feature type. When working with {@code
  ...
  /**
   * Returns information describing this {@code FeatureSource} which may include title, description
   * and spatial parameters. Note that in the returned {@code ResourceInfo} object, the distinction
   * between feature name and schema (feature type) name applies as discussed for {@linkplain
   * #getName()}.
   */
  ...
  /**
   * Enquire what what query capabilities this {@code FeatureSource} natively supports. For example,
   * whether queries can return sorted results.
   *
   ...
   *
   * @return The bounding envelope of the feature data; or {@code null} if the bounds are unknown or
   *     too costly to calculate.
   ...
   *
   * <p>Note: the existence of a specific hint does not guarantee that it will always be honored by
