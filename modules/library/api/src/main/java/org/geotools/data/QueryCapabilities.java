...
public class QueryCapabilities {
   ...
   * {@link SortBy#NATURAL_ORDER} shall be supported, and be the default order if a Query with
   ...
   *
   * <p>May include current feature type properties as well as <code>"@id"</code> for sorting on the
   * Feature ID. Note, however, that ability to sort by the fature id does not necessarily implies
   * the same ordering than SortBy.NATURAL_ORDER, though its probable they match for datastores
   * where the feature id is built up from a primary key.
   *
   ...
   *
   * @return whether the FeatureType this query capabilities refers to can be natively sorted by the
   *     provided list of attribtue/order pairs
   */
  ...
  /**
   * If true the datastore supports using the provided feature id in the data insertion workflow as
   * opposed to generating a new id. In that case it will look into the user data map ({@link
   ...
   * made, and the operation will fail of the key cannot be parsed into a valid storage identifier.
   *
   * @return
   */
