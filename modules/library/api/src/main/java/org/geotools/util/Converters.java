...
public final class Converters {
   ...
   * @return The converted value as an instance of target, or <code>null</code> if a converter could
   *     not be found
   * @since 2.4
   */
   ...
   * Converts an object of a particular type into an object of a different type.
   *
   * <p>This method uses the {@link ConverterFactory} extension point to find a converter capable of
   * performing the conversion. The first converter found is the one used. Using this class there is
   * no way to guarantee which converter will be used.
   *
   * @param source The object to convert.
   * @param target The type of the converted value.
  ...
  static ConverterFactory[] factories() {
    if (factories == null) {
      ...
      factories =
          (ConverterFactory[])
              factoryCollection.toArray(new ConverterFactory[factoryCollection.size()]);
    }
    return factories;
  }
}
