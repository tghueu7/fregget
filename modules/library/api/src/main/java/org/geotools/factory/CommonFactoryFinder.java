...
public final class CommonFactoryFinder extends FactoryFinder {
  ...
  public static synchronized Set<Function> getFunctions(Hints hints) {
    hints = mergeSystemHints(hints);
    return new LazySet<Function>(getServiceRegistry().getFactories(Function.class, null, hints));
  }
  ...
  /**
   * Returns the first implementation of {@link FeatureLockFactory} matching the specified hints. If
  ...
  /**
   * Returns the first implementation of {@link FeatureCollections} matching the specified hints. If
   * no implementation matches, a new one is created if possible or an exception is thrown
   * otherwise.
   *
   * @param hints An optional map of hints, or {@code null} if none.
  ...
  /**
   * Returns the first implementation of {@link FeatureCollections}. If no implementation matches, a
  ...
  /**
   * Returns the first implementation of {@link FilterFactory}. If no implementation matches, a new
   * one is created if possible or an exception is thrown otherwise.
   *
   ...
   * is a convenience method invoking {@link #getFilterFactory} with a hint value set for requerying
   * a {@link FactoryFilter2} implementation.
   *
   * @param hints An optional map of hints, or {@code null} if none.
   * @return The first filter factory that matches the supplied hints.
   * @throws FactoryRegistryException if no implementation was found or can be created for the
