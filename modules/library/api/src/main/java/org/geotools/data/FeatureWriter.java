...
public interface FeatureWriter<T extends FeatureType, F extends Feature> extends Closeable {
   ...
   * only be sent to FeatureSources operating on the same Transaction. When Transaction commit() is
   * called other FeatureSources will be informed of the modifications.
   *
   ...
   *       false</code>.
   *   <li>FeatureEvent.FEATURES_MODIFIED: when next has been called with hasNext() equal to <code>
   *       true</code> and the resulting Feature has indeed been modified.
   * </ul>
   *
   * <p>If this FeatureWriter is opperating against a Transaction the FEATURES_MODIFIED or
   * FEATURES_ADDED events should only be sent to FeatureSources opperating on the same Transaction.
   * When Transaction commit() is called other FeatureSources will be informed of the modifications.
   *
   ...
   * entries you do not want), than to force the creation of entire Features only to skip over them.
   *
   * <p>FeatureWriters that support append operations will allow calls to next, even when hasNext()
   ...
   *
   * @throws IOException if there there are problems releasing underlying resources, or possibly if
   *     close has been called (up to the implementation).
   * @throws IOException if an I/O error occurs
   */
