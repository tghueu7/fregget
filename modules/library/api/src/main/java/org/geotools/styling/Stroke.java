...
public interface Stroke extends org.opengis.style.Stroke {
   ...
   * totally transparent and 1.0 representing totally opaque. A linear scale of translucency is used
   ...
   * The first number gives the length in pixels of the dash to draw, the second gives the amount of
   * space to leave, and this pattern repeats.<br>
   * If an odd number of values is given, then the pattern is expanded by repeating it twice to give
  ...
  /**
   * A dash array need not start from the beginning. This method allows for an offset into the dash
   * array before starting it.
   */
   ...
   *
   * <p>Proper stroking with a linear graphic requires two "hot-spot" points within the space of the
   * graphic to indicate where the rendering line starts and stops. In the case of raster images
   * with no special mark-up, this line will be assumed to be the middle pixel row of the image,
   * starting from the first pixel column and ending at the last pixel column.
   *
