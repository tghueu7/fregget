...
public interface FeatureCollection<T extends FeatureType, F extends Feature> {
   ...
   * </code></pre>
   *
   * <p>GML Note: The contents of this iterator are considered to be defined by <b>featureMember</b>
   * tags (and/or the single allowed <b>FeatureMembers</b> tag). Please see getFeatureType for more
   * details.
   *
   ...
   *
   * <ul>
   *   <li>For a collection backed by a shapefiles (or database tables) the FeatureType returned by
   *       getSchema() will complete describe each and every child in the collection.
   ...
   *
   * <p>The contents of the returned SimpleFeatureCollection are determined by applying the provider
   * Filter to the entire contents of this FeatureCollection. The result is "live" and modifications
   * will be shared.
   *
   * <p>This method is used cut down on the number of filter based methods required for a useful
   * SimpleFeatureCollection construct. The FeatureCollections returned really should be considered
   * as a temporary "view" used to control the range of a removeAll, or modify operation.
   *
   * <p>Example Use:
   *
   * <pre><code>
