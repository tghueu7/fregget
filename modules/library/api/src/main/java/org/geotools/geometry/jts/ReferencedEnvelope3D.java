...
public class ReferencedEnvelope3D extends ReferencedEnvelope implements BoundingBox3D {
     ...
     *
     * @param x the value to lower the minimum x to or to raise the maximum x
     *          to
     * @param y the value to lower the minimum y to or to raise the maximum y
     *          to
     * @param z the value to lower the minimum z to or to raise the maximum z
     *          to
     */
     ...
     *
     * @param p the point which this <code>Envelope</code> is being checked
     *          for containing
     ...
     * <tt>contains</tt>, which would exclude the envelope boundary.
     *
     * @param x the x-coordinate of the point which this <code>Envelope</code>
     *          is being checked for containing
     * @param y the y-coordinate of the point which this <code>Envelope</code>
     *          is being checked for containing
     ...
     */
    public static ReferencedEnvelope3D EVERYTHING = new ReferencedEnvelope3D(Double
            .NEGATIVE_INFINITY,
    ...
    public ReferencedEnvelope3D(final double x1, final double x2, final double y1, final double y2,
                                final double z1, final double z2, final CoordinateReferenceSystem
    ...
    public ReferencedEnvelope3D(final BoundingBox3D bbox) throws MismatchedDimensionException {
        this(bbox.getMinX(), bbox.getMaxX(), bbox.getMinY(), bbox.getMaxY(), bbox.getMinZ(), bbox
                        .getMaxZ(),
                bbox.getCoordinateReferenceSystem());
    }
     ...
     * @throws MismatchedDimensionExceptionif the CRS dimension is not valid.
     */
    public ReferencedEnvelope3D(final ReferencedEnvelope3D envelope, final 
    ...
    public ReferencedEnvelope3D intersection(ReferencedEnvelope3D env) {
        ...
        return new ReferencedEnvelope3D(intMinX, intMaxX, intMinY, intMaxY, intMinZ, intMaxZ, env
     ...
     *
     * @param targetCRS                  The target coordinate reference system.
     * @param lenient                    {@code true} if datum shift should be applied even if 
     *                                               there is
     *                                   insuffisient information. Otherwise (if {@code false}), an
     *                                   exception is thrown in such case.
    ...
    public ReferencedEnvelope transform(final CoordinateReferenceSystem targetCRS,
        ...
        if (crs == null) {
            if (isEmpty()) {
                ...
                throw new NullPointerException("Unable to transform referenced envelope, crs has " +
        ...
        if (getDimension() != targetCRS.getCoordinateSystem().getDimension()) {
            if (lenient) {
                ...
                throw new MismatchedDimensionException(Errors.format(
                        ...
                        new Integer(getDimension()), new Integer(targetCRS.getCoordinateSystem()
                                .getDimension())));
            }
        }
        ...
        final CoordinateOperation operation = coordinateOperationFactory.createOperation(crs, 
                targetCRS);
