...
public final class JTS {
    ...
    public static final AffineTransformation Y_INVERSION = new AffineTransformation(1, 0, 0, 0, 
            -1, 0);
     ...
     */
    private static final Map<CoordinateReferenceSystem, GeodeticCalculator> CALCULATORS = new 
            HashMap<CoordinateReferenceSystem, GeodeticCalculator>();
     ...
     * @param targetEnvelope An envelope to expand with the transformation result, or {@code 
     * null} for
    ...
    public static Envelope transform(final Envelope sourceEnvelope, Envelope targetEnvelope,
                                     final MathTransform transform, int npoints) throws 
            TransformException {
     ...
     * @throws TransformException
     * @throws FactoryException           If operationis unavailable from source CRS to WGS84, to
     * from WGS84_3D to targetCRS
    ...
    public static ReferencedEnvelope3D transformTo3D(final ReferencedEnvelope sourceEnvelope,
                                                     CoordinateReferenceSystem targetCRS, boolean
                                                             lenient, int npoints)
     ...
     * @throws TransformException
     */
    private static DirectPosition transformTo3D(GeneralDirectPosition srcPosition, MathTransform 
            transformToWGS84,
                                                MathTransform transformFromWGS84_3D) throws 
            TransformException {
     ...
     * @throws TransformException
     */
    private static DirectPosition transformTo2D(GeneralDirectPosition srcPosition, MathTransform 
            transformToWGS84_3D,
                                                MathTransform transformFromWGS84) throws 
            TransformException {
        if (Double.isNaN(srcPosition.getOrdinate(2))) {
            srcPosition.setOrdinate(2, 0.0); // lazy add 3rd ordinate if not provided to prevent 
            // failure
        }
    ...
    public static Geometry transform(final Geometry geom, final MathTransform transform)
        ...
        final GeometryCoordinateSequenceTransformer transformer = new 
     ...
     * @throws TransformException If at least one coordinate can't be transformed.
     */
    public static Envelope toGeographic(final Envelope envelope, final CoordinateReferenceSystem 
            crs)
            throws TransformException {
     ...
     * @throws TransformException if the coordinates can't be transformed from the specified CRS 
     * to a
    ...
    public static Geometry toGeometry(final Shape shape, final GeometryFactory factory) {
        ...
        while (!iterator.isDone()) {
            switch (iterator.currentSegment(buffer)) {
                /*
                 * Close the polygon: the last point is equal to the first point, and a 
                 * LinearRing is
                 * created.
                 */
     ...
     * @throws MismatchedDimensionException if a two-dimensional envelope can't be created from 
    ...
    private static Geometry smoothMultiLineString(GeometryFactory factory,
                                                  GeometrySmoother smoother, Geometry geom, 
                                                  double fit) {
     ...
     */
    public static CoordinateSequence createCS(CoordinateSequenceFactory csFactory, int size, int 
            dimension) {
        ...
        if (cs.getDimension() != dimension) {
            ...
            throw new IllegalStateException("Unable to use" + csFactory + " to produce " +
                    "CoordinateSequence with dimension " + dimension);
        }
        return cs;
    }
     ...
     * {@link MultiPolygon} It will throw an exception if the
    ...
    /**
     * Removes collinear vertices from the provided {@link Geometry} if the number of point 
     * exceeds the requested minPoints.
     * <p>
     * <p>
     * For the moment this implementation only accepts, {@link Polygon}, {@link LineString} and 
     * {@link MultiPolygon} It will throw an exception if the
    ...
    public static Rectangle2D toRectangle2D(Envelope envelope) {
        ...
        return new Rectangle2D.Double(envelope.getMinX(), envelope.getMinY(), envelope.getWidth()
                , envelope.getHeight());
    }
    ...
    public static Envelope toEnvelope(Rectangle2D rectangle) {
        ...
        return new Envelope(rectangle.getMinX(), rectangle.getMaxX(), rectangle.getMinY(), 
                rectangle.getMaxY());
    }
