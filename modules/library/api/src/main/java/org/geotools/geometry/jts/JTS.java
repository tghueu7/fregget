...
public final class JTS {
  ...
  /**
   * Geodetic calculators already created for a given coordinate reference system. For use in {@link
   * #orthodromicDistance}.
   *
  ...
  /**
   * Transforms the densified envelope using the specified math transform. The envelope is densified
   * (extra points put around the outside edge) to provide a better new envelope for high deformed
   * situations.
   *
   * <p>If an optional target envelope is provided, this envelope will be {@linkplain
   * Envelope#expandToInclude expanded} with the transformation result. It will <strong>not</strong>
   * be {@linkplain Envelope#setToNull nullified} before the expansion.
   *
  ...
  public static Envelope transform(
    ...
    if (transform.getSourceDimensions() != transform.getTargetDimensions()
      ...
      throw new MismatchedDimensionException(
          Errors.format(ErrorKeys.BAD_TRANSFORM_$1, Classes.getShortClassName(transform)));
    }
   ...
   *
   * <p>This method transforms each ordinate into WGS84, manually converts this to WGS84_3D with the
   * addition of a Double.NaN, and then transforms to the final 3D position.
   *
   ...
   * @throws TransformException
   * @throws FactoryException If operationis unavailable from source CRS to WGS84, to from WGS84_3D
   *     to targetCRS
  ...
  /**
   * Transform the provided 2D direct position into 3D (0 Ellipsoidal height assumed when converting
   * from {@link DefaultGeographicCRS#WGS84} to {@link DefaultGeographicCRS#WGS84_3D}).
   *
   ...
   * manages a pool of pre-defined geodetic calculators for the given coordinate reference system in
   * order to avoid repetitive object creation. If a large amount of orthodromic distances need to
   * be computed, direct use of {@link GeodeticCalculator} provides better performance than this
   * convenience method.
   *
   ...
   * destination array can have any length. Only the relevant field of the source coordinate will be
   * copied. If the array length is greater than 3, then all extra dimensions will be set to {@link
   * Double#NaN NaN}.
   *
  ...
  public static Geometry toGeometry(final Shape shape, final GeometryFactory factory) {
    ...
    if (!coords.isEmpty()) {
      lines.add(
          factory.createLineString((Coordinate[]) coords.toArray(new Coordinate[coords.size()])));
    }
   ...
   * package.
   *
   * <p>If the provided envelope is a {@link ReferencedEnvelope} we check that the provided CRS and
  ...
  public static Envelope2D getEnvelope2D(
    ...
    if (crs2D == null)
      throw new MismatchedDimensionException(Errors.format(ErrorKeys.CANT_SEPARATE_CRS_$1, crs));

    return new Envelope2D(
        crs2D, envelope.getMinX(), envelope.getMinY(), envelope.getWidth(), envelope.getHeight());
  }
   ...
   * vertices which are positioned using Bezier splines. All vertices of the input Geometry will be
   * present in the output Geometry.
   *
   * <p>The {@code fit} parameter controls how tightly the smoothed lines conform to the input line
   * segments, with a value of 1 being tightest and 0 being loosest. Values outside this range will
   * be adjusted up or down as required.
   *
   * <p>The input Geometry can be a simple type (e.g. LineString, Polygon), a multi-type (e.g.
   * MultiLineString, MultiPolygon) or a GeometryCollection. The returned object will be of the same
   * type.
   *
        ...
        new CoordinateSequenceFilter() {
          ...
          public void filter(CoordinateSequence seq, int i) {
            ...
            p.add(
                new GeometryFactory()
                    .createLineString(
                        new Coordinate[] {seq.getCoordinate(i - 1), seq.getCoordinate(i)}));
          }
        });
    ...
    if (removeHoles) {
      for (int i = 0; i < result.size(); i++) {
        ...
        if (item.getNumInteriorRing() > 0) {
          ...
          Polygon noHoles = factory.createPolygon((LinearRing) item.getExteriorRing(), null);
