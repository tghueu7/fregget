...
public interface DataAccessFactory extends Factory {
   ...
   *
   * <p>Magic Params: the following params are magic and are honoured by convention by the GeoServer
   * and uDig application.
   *
   * <ul>
   *   <li>"user": is taken to be the user name
   *   <li>"passwd": is taken to be the password
   *   <li>"namespace": is taken to be the namespace prefix (and will be kept in sync with GeoServer
   ...
   *     values for the map include: url - location of a resource, used by file reading datasources.
   ...
   *     insufficent parameters were given. Note that canProcess() should have returned false if the
   ...
   * @return booean true if and only if this factory can process the resource indicated by the param
   *     set and all the required params are pressent.
   */
  boolean canProcess(java.util.Map<String, Serializable> params);

  /**
   * Test to see if the implementation is available for use. This method ensures all the appropriate
   * libraries to construct the DataAccess are available.
   *
   * <p>Most factories will simply return <code>true</code> as GeoTools will distribute the
   * appropriate libraries. Though it's not a bad idea for DataStoreFactories to check to make sure
   * that the libraries are there.
   *
   ...
   *
   * @return <tt>true</tt> if and only if this factory has all the appropriate jars on the classpath
   *     to create DataStores.
   */
  boolean isAvailable();
  ...
  public static class Param extends Parameter {
    ...
    public Param(
        ...
        Object sample) {
      super(
          key, type, new SimpleInternationalString(key), description, required, 1, 1, sample, null);
    }
     ...
     *
     * <p>Type conversion will occur if required, this may result in an IOException. An IOException
     * will be throw in the Param is required and the Map does not contain the Map.
     *
    ...
    public Object parse(String text) throws Throwable {
      ...
      try {
        ...
        throw new DataSourceException(
            "Could not create " + type.getName() + ": from '" + text + "'", instantiaionException);
      } catch (IllegalAccessException illegalAccessException) {
        throw new DataSourceException(
            "Could not create " + type.getName() + ": from '" + text + "'", illegalAccessException);
