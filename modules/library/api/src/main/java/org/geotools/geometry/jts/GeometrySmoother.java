...
class GeometrySmoother {
  ...
  /**
   * Default smoothing control. Specifies no minimum vertex distance and a constant number of points
  ...
  LineString smooth(LineString ls, double alpha) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if (dist < control.getMinLength()) {
        ...
        Coordinate[] segment =
            cubicBezier(
                coords[i], coords[i + 1], controlPoints[i][1], controlPoints[i + 1][0], smoothN);
  ...
  public Polygon smooth(Polygon p, double alpha) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (dist < control.getMinLength()) {
        ...
        Coordinate[] segment =
            cubicBezier(
                coords[i], coords[next], controlPoints[i][1], controlPoints[next][0], smoothN);
   ...
   * @return 2D array of {@code Coordinates} for positions of each pair of control points per input
   *     vertex
   */
