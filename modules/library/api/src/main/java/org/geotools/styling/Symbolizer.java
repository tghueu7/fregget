...
public interface Symbolizer extends org.opengis.style.Symbolizer {
   ...
   *
   * <p>Please consider getDescription().getTitle() as an alternative if presenting this symbolizer
   * in a user interface.
   *
   * @since SymbologyEncoding 1.1
   * @param name
   ...
   * PropertyName. In case the geometry expression is null, and in the case the geometry expression
   * is not a PropertyName, this method will return null.
   */
  String getGeometryPropertyName();

  /**
   * A shortcut to define the geometry expression as a {@link PropertyName} Typically, features only
   * have one geometry so, in general, the need to select one is not required. Note: this moves a
   * little away from the SLD spec which provides an XPath reference to a Geometry object, but does
   * follow it in spirit.
   */
   ...
   * The property is optional and if it is absent (null) then the "default" geometry property of the
   * feature should be used.<br>
   ...
   * make up geometries out of non geometric attributes it is advised that the Expression implements
