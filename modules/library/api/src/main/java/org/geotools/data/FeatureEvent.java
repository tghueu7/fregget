...
public class FeatureEvent extends EventObject {
   ...
   * the FeatureSource.getTransaction() for a match before sending notification to any listeners on
   * the FeatureSource.
   *
   ...
   * <p>If the FeatureWriter is opperating against a Transaction it will need ensure that to check
   * the FeatureSource.getTransaction() for a match before sending notification to any listeners on
   * the FeatureSource. All FeatureSources of the same typename will need to be informed of a <code>
   * commit</code>, except ones in the same Transaction, and only FeatureSources in the same
  ...
  public enum Type {
     ...
     *
     * <p>FeatureEvent.getFilter() lists the FeatureIds of the newly created features; please note
     * that these IDs may be changed during a commit.
     */
     ...
     *
     * <p>The FeatureEvent.getFilter() can be used to identify the removed features; often this is a
     * FidFilter. But it may be Filter.INCLUDES if we are unsure exactly what has been changed.
     */
  ...
  /**
   * Provides information on the type of change that has occurred. Possible types are: add, remove,
   * change
   *
   * @return Type
   */
