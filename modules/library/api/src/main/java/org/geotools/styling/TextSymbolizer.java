...
public interface TextSymbolizer extends org.opengis.style.TextSymbolizer, Symbolizer {
  ...
  /**
   * Minimum distance between two labels in the same label group. To be used when both displacement
   * and repeat are used to avoid having two labels too close to each other
   */
  ...
  /**
   * When true activates curved labels on linear geometries. The label will follow the shape of the
   * current line, as opposed to being drawn a tangent straight line
   */
  ...
  /**
   * When drawing curved labels, max allowed angle between two subsequent characters. Higher angles
   * may cause disconnected words or overlapping characters
   */
  public static String MAX_ANGLE_DELTA_KEY = "maxAngleDelta";

  /**
   * When label follows line, the max angle change between two subsequent characters, keeping it low
   * avoids chars overlaps. When the angle is exceeded the label placement will fail.
   */
  ...
  /**
   * Default behaviour is to not truncate labels on the border of the display area. Instead, labels
   * are hidden or moved to be fully displayed inside the area.
   */
  ...
  /**
   * Direct access to device independent Fonts used to render the label. Fonts are used in order of
   * priority given.
   *
  ...
  /**
   * Priority -- null = use the default labeling priority Expression = an expression that evaluates
   * to a number (ie. Integer, Long, Double...) Larger = more likely to be rendered
   */
