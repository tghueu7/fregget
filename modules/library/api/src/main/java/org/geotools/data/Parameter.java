...
public class Parameter<T> implements org.opengis.parameter.Parameter<T> {
  ...
  /**
   * What is the min and max number of this paramter there can be ( a value of -1 for min means 0 or
   * more, a value of -1 for max means any number greater than or equal to the min value )
   *
  ...
  /**
   * Boolean indicating whether the parameter shall be used as a password field, provides a hint for
   * UI's to mask text fields, configuration systems to encrypt content, etc
   */
  ...
  /**
   * "element": Class to use as the Element type for List<Element>. Please restrict your use of this
   * facility to simple types; for most higher order data structures multiplicity is already
  ...
  /**
   * Set parameter to deprecated - true, false ;can be used to conditional show the parameter based
   * on deprecated value
   */
   ...
   */
  public Parameter(
      String key, Class<T> type, String title, String description, Map<String, Object> metadata) {
    this(
        key,
        type,
        new SimpleInternationalString(title),
