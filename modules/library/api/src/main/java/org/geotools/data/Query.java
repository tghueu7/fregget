...
public class Query {
   ...
   *
   * <p>Datastores may implement adding all mandatory properties to the end result when this flag is
   * set to true. For example:
   *
   * <p>Object includeProps = query.getHints().get(Query.INCLUDE_MANDATORY_PROPS); if (includeProps
   * instanceof Boolean && ((Boolean)includeProps).booleanValue()) { query.setProperties
  ...
  /**
   * A constant (empty String array) that can be used with {@linkplain #setPropertyNames(String[])}
   * to indicate that no properties are to be retrieved.
   *
  ...
  /**
   * A constant (value {@code null}) that can be used with {@linkplain #setPropertyNames(String[])}
   * to indicate that all properties are to be retrieved.
   */
   ...
   * #setProperties(Collection<PropertyName>)} to indicate that all properties are to be retrieved.
   */
   ...
   * properties that are not part of the source's schema are requested an exception will be thrown.
   *
   ...
   * properties that are not part of the source's schema are requested an exception will be thrown.
   *
   * @param propNames the names of the properties to retrieve or one of {@linkplain #ALL_PROPERTIES}
   *     or {@linkplain #NO_PROPERTIES}.
   */
  public void setProperties(List<PropertyName> propNames) {
    this.properties =
        propNames == ALL_PROPERTIES ? ALL_PROPERTIES : new ArrayList<PropertyName>(propNames);
  }
   ...
   * @task REVISIT: This syntax is really obscure. Consider having an fid or featureID propertyName
   *     that datasource implementors look for instead of looking to see if the list size is 0.
   */
  ...
  /**
   * Set the index of the first feature to retrieve. This can be used in conjuction with {@linkplain
   * #setMaxFeatures(int) } to 'page' through a feature data source.
   *
  ...
  /**
   * Sets the filter to constrain the features that will be retrieved by this Query. If no filter is
   * set all features will be retrieved (taking into account any bounds set via {@linkplain
   ...
   *
   * <p>This denotes a request to <b>temporarily</b> override the coordinate system contained in the
   * feature data source being queried. The same coordinate values will be used, but the features
   * retrieved will appear in this Coordinate System.
   *
  ...
  /**
   * If reprojection has been requested, this returns the coordinate system that features retrieved
   * by this Query will be reprojected into.
   *
   ...
   *
   * <p>SortBy is part of the Filter 1.1 specification, it is referenced by WFS1.1 and Catalog 2.0.x
   * specifications and is used to organize results. The SortBy's are ment to be applied in order:
   *
   * <ul>
   ...
   * </code></pre>
   *
   * <p>SortBy should be considered at the same level of abstraction as Filter, and like Filter you
   * may sort using properties not listed in getPropertyNames.
   *
   ...
   *
   * <p>Each {@link Join} object specifies a feature type to join to. The join may only reference a
   * feature type from within the same datastore.
   *
   * @see Join
   */
