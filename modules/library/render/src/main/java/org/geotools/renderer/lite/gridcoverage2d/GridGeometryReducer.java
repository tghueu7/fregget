...
class GridGeometryReducer {
   ...
   * are reducing (if the grid to world does not contain rotation, they are the same, otherwise not)
   *
   * @author Andrea Aime - GeoSolutions
   */
  static enum Side {
  ...
  /**
   * Reduces the given grid geometry by at most one pixel on each side, in an attempt to make it fit
   * the
   *
   * @param gg
   * @return
  ...
  /**
   * Builds a cut envelope for the Crop operation. Since the crop internals will add back the pixels
   * we just removed due to numerical issues, we keep the envelope a bit on the safe side
   *
   * @param reduced
   * @return
   */
