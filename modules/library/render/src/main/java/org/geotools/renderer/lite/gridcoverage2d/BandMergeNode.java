    ...
    if (sources != null && !sources.isEmpty()) {
      ...
      if (size != 3 && size != 1) {
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.INVALID_NUMBER_OF_SOURCES_$1, Integer.valueOf(size)));
      }
        ...
        if (gridGeometry == null) {
          ...
          final ComponentColorModel cm =
              new ComponentColorModel(
                  ...
                  false,
                  transparency,
                  currentSourceCoverage.getRenderedImage().getSampleModel().getDataType());
      ...
      for (int i = 0; i < sd.length; i++)
        sd[i] =
            new GridSampleDimension(TypeMap.getColorInterpretation(op.getColorModel(), i).name());
