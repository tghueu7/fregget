...
public class DefaultMapContext extends MapContext {
  ...
  /**
   * Creates a default empty map context. The coordinate reference system for the map context should
   * be set explicitly, or implicitly via {@code addLayer} prior to using the context.
   */
   ...
   *
   * @param layers an array of MapLayer objects (may be empty or null) to be added to this context
   * @param title a title for this context (e.g. might be used by client-code that is displaying the
   ...
   *     empty string
   * @param contactInformation can be used, for example, to record the creators or custodians of the
   *     data that are, or will be, held by this context; may be null or an empty string
   ...
   * coordinate reference system has not been set for the context an attempt is made to retrieve one
   * from the new layer and use that as the context's CRS.
   *
  ...
  /**
   * Add a new layer, if not already present, to the end of the list of layers held by this context
   * and trigger a {@linkplain MapLayerListEvent} If a coordinate reference system has not been set
   * for the context an attempt is made to retrieve one from the new layer and use that as the
   * context's CRS.
   *
   * @param layer the map layer to add
   * @return true if the layer was added; false otherwise (layer was already present)
  ...
  /**
   * If a CRS has not been defined for this context, attempt to get one from this map layer and set
   * it as the context's CRS. Invoked by addLayer methods.
   *
  ...
  /**
   * If a CRS has not been defined for this context, attempt to get one from this featureSource and
   * set it as the context's CRS. Invoked by addLayer.
   *
  ...
  /**
   * Remove the layer at the given position in the list of layers held by this context. The position
   * must be valid or an IndexOutOfBoundsException will result. CAlling this method triggers a
