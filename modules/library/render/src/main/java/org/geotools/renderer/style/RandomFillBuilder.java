...
class RandomFillBuilder {
  ...
  BufferedImage buildRandomTilableImage(
    ...
    RotationRandomizer rotation =
        (RotationRandomizer)
            voParser.getEnumOption(symbolizer, "random-rotation", RotationRandomizer.NONE);
  ...
  static class GridBasedPositionGenerator implements PositionSequence {
    ...
    public Position getNextPosition() {
      ...
      // System.out.println("Grid position: " + c + ", " + r + ", deltas: " + deltaX + "," + deltaY
      // + " target area: " + targetArea);
      position.x =
          (int) Math.round(targetArea.getMinX() + c * deltaX + random.nextDouble() * deltaX);
      position.y =
          (int) Math.round(targetArea.getMinY() + r * deltaY + random.nextDouble() * deltaY);
      if (randomRotation) {
        position.rotation = random.nextDouble() * 360;
      }
