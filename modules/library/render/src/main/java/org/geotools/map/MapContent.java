...
public class MapContent {
   ...
   *
   * <p>While the map maintains one viewport internally to better reflect a map context document you
   * are free to maintain a separate viewport; or indeed construct many viewports representing tiles
   * to be rendered.
   */
  ...
  /**
   * Register interest in receiving a {@link LayerListEvent}. A <code>LayerListEvent</code> is sent
   * if a layer is added or removed, but not if the data within a layer changes.
   *
   ...
   *
   * @return The bounding box of the features or null if unknown and too expensive for the method to
   *     calculate.
   ...
   * which case a subsequent to {@linkplain #getViewport()} will return a new instance with default
   * settings.
   *
  ...
  /**
   * As an example it can be used to record contact details, map abstract, keywords and so forth for
   * an OGC "Open Web Service Context" document.
   *
  ...
  /**
   * Sets the CRS of the viewport, if one exists, based on the first Layer with a non-null CRS. This
   * is called when a new Layer is added to the Layer list. Does nothing if the viewport already has
   * a CRS set or if it has been set as non-editable.
   */
  ...
  private class LayerList extends CopyOnWriteArrayList<Layer> {
     ...
     *
     * @param element the layer to add
     * @return {@code true} if the layer was added; {@code false} if it was already present in this
     *     list
     */
    @Override
    ...
    /**
     * Adds all layers from the input collection that are not already present in this list, with the
     * first added layer taking position {@code index}.
     *
    ...
    /**
     * Removes the layer at position {@code index} from this list. Note: removing a layer causes its
     * {@code dispose} method to be called, so although a reference to the removed layer is returned
     * by this method it should not be used subsequently.
     *
    ...
    /**
     * Removes the specified element, which much be a Layer, from this list if present. This method
     * calls the layer's {@code dispose} method, so any external references to the layer should be
     * discarded.
     *
     ...
     * </code></pre>
     *
     * The same events will be sent to {@link MapLayerListListener} objects as if the above code had
     * been called.
     *
