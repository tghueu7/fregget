...
public class Drawer {
    ...
    public void drawFeature(BufferedImage bi, SimpleFeature feature, AffineTransform 
            worldToScreenTransform, boolean drawVertices, MathTransform mt) {
        if (feature == null)
            return;
    ...
    public void drawFeature(BufferedImage bi, SimpleFeature feature, AffineTransform 
            worldToScreenTransform, Style style) {
        if (feature == null)
            return;
        drawFeature(bi, feature, worldToScreenTransform, false, getSymbolizers(style), null);
    }

    public void drawFeature(BufferedImage bi, SimpleFeature feature, Style style, AffineTransform
            worldToScreenTransform) {
        if (feature == null)
            return;
    ...
    public static Symbolizer[] getSymbolizers(Class<? extends Geometry> type, Color baseColor, 
        ...
        if (Point.class.isAssignableFrom(type) ||
            ...
            point.getGraphic().setSize((Expression) CommonFactoryFinder.getFilterFactory(GeoTools
                    .getDefaultHints()).literal(10));
    ...
    public void drawFeature(BufferedImage bi, SimpleFeature feature, AffineTransform 
            worldToScreenTransform, boolean drawVertices, Symbolizer[] symbs, MathTransform mt) {
    ...
    public void drawFeature(BufferedImage bi, SimpleFeature feature, AffineTransform 
            worldToScreenTransform, boolean drawVertices,
    ...
    private void paint(BufferedImage bi, SimpleFeature feature, LiteShape shape, Symbolizer symb) {
        ...
        if (symb instanceof PolygonSymbolizer) {
            ...
            if (fill != null) {
                fill = new Color(fill.getRed(), fill.getGreen(), fill.getBlue(), (int) (255 * 
                        opacity));
                g.setColor(fill);
                g.fill(shape);
            }
        ...
        if (symb instanceof PointSymbolizer) {
            ...
            Style2D tmp = styleFactory.createStyle(feature, pointSymbolizer, NumberRange.create
     ...
     * new Drawing().schema("namespace.typename", "id:0,*geom:LineString,name:String,
     * *centroid:Point");
     * </code></pre>
     * <ul>
    ...
    static {
        try {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            lineSchema = DataUtilities.createType("generated:linestring", 
                    "*linestring:LineString"); //$NON-NLS-1$ //$NON-NLS-2$
            polygonSchema = DataUtilities.createType("generated:polygon", "*polygon:Polygon"); 
            //$NON-NLS-1$ //$NON-NLS-2$
            multipointSchema = DataUtilities.createType("generated:multipoint", 
                    "*multipoint:MultiPoint"); //$NON-NLS-1$ //$NON-NLS-2$
            multilineSchema = DataUtilities.createType("generated:multilinestring", 
                    "*multilinestring:MultiLineString"); //$NON-NLS-1$ //$NON-NLS-2$
            multipolygonSchema = DataUtilities.createType("generated:multipolygon", 
