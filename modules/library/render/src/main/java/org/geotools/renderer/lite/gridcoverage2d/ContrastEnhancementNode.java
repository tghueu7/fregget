...
class ContrastEnhancementNode extends StyleVisitorCoverageProcessingNodeAdapter
    ...
    if (sources != null && !sources.isEmpty()) {
      ...
      if ((!Double.isNaN(gammaValue)
        ...
        if (numbands > 1) {
          ...
          sChannel =
              new ImageWorker(IHS).setRenderingHints(hints).retainLastBand().getRenderedImage();
          hChannel =
              new ImageWorker(IHS)
                  .setRenderingHints(hints)
   ...
   * enhancement operations have been implemented in a way that is generic enough o handle all data
   * types.
   *
