...
public class GridCoverageReaderHelper {
  ...
  public GridCoverageReaderHelper(
    ...
    sameCRS =
        CRS.equalsIgnoreMetadata(
            mapExtent.getCoordinateReferenceSystem(), reader.getCoordinateReferenceSystem());
    paddingRequired =
        (!sameCRS || !(interpolation instanceof InterpolationNearest) || isMultiCRSReader(reader))
  ...
  /**
   * Returns true if the reader is advertising a single CRS, cannot fully perform a reproject to any
   * target CRS, but internally is working with several CRSs and could use some extra padding on the
   * requests
   *
   * @param reader
   * @return
   ...
   * multiple reads to manage reads across the date line, reducing the read area, splitting it into
   * parts to manage certain projections (e.g., conic) and so on
   */
  ...
  /**
   * Reads the data taking into account advanced projection handling in order to deal with date line
   * crossing, poles and other projection trouble areas. The result is a set of coverages that can
   * be either painted or reprojected safely
   */
  ...
  private GridCoverage2D cropCoverageOnRequestedEnvelope(GridCoverage2D readCoverage) {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINE, "Failed to crop coverage on the requested area, using the original one", e);
      return readCoverage;
    }
  }
    ...
    if (preProcessed != null && !preProcessed.isEmpty()) {
      if (coverageFootprint.equals(preProcessed)) {
        ...
        for (Polygon p : polygons) {
          ...
          cropEnvelope =
              new ReferencedEnvelope(cropEnvelope.intersection(coverageEnvelope), readerCRS);
          cropEnvelope =
              new ReferencedEnvelope(cropEnvelope.intersection(readingEnvelope), readerCRS);
          GridCoverage2D cropped = cropCoverage(coverage, cropEnvelope);
          if (cropped != null) {
  ...
  private GridGeometry2D computeReadingGeometry(
    ...
    if (sameCRS) {
      ...
      GridGeometry2D localGridGeometry =
          new GridGeometry2D(
              readingGridRange, gridToCRS2D, mapExtent.getCoordinateReferenceSystem());
      ...
      int width =
          (int) Math.max(1, Math.round(readEnvelope.getWidth() / Math.abs(readResolution[0])));
      int height =
          (int) Math.max(1, Math.round(readEnvelope.getHeight() / Math.abs(readResolution[1])));
