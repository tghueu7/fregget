...
public class MapLayer {
  ...
  /**
   * Wrap up a layer in a DefaultMapLayer; generally used to ensure layer is shown to the world as a
   * feature source even if it happens to be a WMS or something.
   */
  ...
  /**
   * Specify whether this layer is selected. A {@link LayerEvent} iw fired if the selected status is
   * changed.
   *
   * @param selected Set the layer as selected if <code>true</code> or as unselected if <code>false
   *     </code>
   */
   ...
   *
   * <p>IMPORTANT: only include attribute names in the query if you want them to be ALWAYS returned.
   * It is desirable to not include attributes at all but let the renderer decide which attributes
   * are actually needed to perform its required operation.
   *
   ...
   * @throws NullPointerException if no query is passed on. If you want to reset a definition query,
   *     pass it {@link Query.ALL} instead of <code>null</code>
