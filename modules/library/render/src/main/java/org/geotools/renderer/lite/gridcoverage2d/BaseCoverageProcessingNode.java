...
public abstract class BaseCoverageProcessingNode implements CoverageProcessingNode {
   ...
   *
   * @param maxSources maximum number of sources allowed for this node.
   * @param hints instance of {@link Hints} class to control creation of internal factories. It can
   *     be <code>null</code>.
   * @param description
   * @param name
   */
  ...
  private void checkExecuted() {
    ...
    if (disposed) {
      error =
          new CoverageProcessingException("Trying to process a disposed CoverageProcessingNode.");
      return;
    }
  ...
  /**
   * This method is responsible for triggering the execution of this {@link CoverageProcessingNode}
    ...
    if (maximumNumberOfSources != -1)
      if (this.sources.size() + sourcesToAdd > maximumNumberOfSources)
        throw new IllegalStateException(
            Errors.format(ErrorKeys.TOO_MANY_SOURCES_$1, Integer.valueOf(maximumNumberOfSources)));
  }
  ...
  /**
   * The {@link GridCoverageFactory} we will internally use for build intermediate and output {@link
   * GridCoverage2D} .
   *
  ...
  protected static void ensureSourceNotNull(final Object source, final String name) {
    if (source == null)
      throw new IllegalArgumentException(Errors.format(ErrorKeys.SOURCE_CANT_BE_NULL_$1, name));
  }
