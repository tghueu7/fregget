      ...
      for (int i = 0; i < splitted.length; i++) {
        ...
        LineBreakMeasurer lineMeasurer =
            new LineBreakMeasurer(
                iter, BreakIterator.getLineInstance(), graphics.getFontRenderContext());
        ...
        while (lineMeasurer.getPosition() < iter.getEndIndex()) {
          ...
          TextLayout layout =
              lineMeasurer.nextLayout(labelItem.getAutoWrap(), lineText.length(), true);
          ...
          for (FontRange range : lineRanges) {
            String extracted =
                lineText.substring(
                    Math.max(prevPosition, range.startChar), Math.min(newPosition, range.endChar));
  ...
  private List<LineComponent> buildLineComponents(
    ...
    if (text.trim().indexOf(' ') == -1 || wordSpacing <= 0) {
      ...
      LineComponent component =
          new LineComponent(text, layoutSentence(text, labelItem, graphics, font), layout);
      ...
      for (int i = 0; i < parts.length; i++) {
        ...
        if (i < parts.length - 1) {
          ...
          Font spacerFont =
              font.deriveFont(Collections.singletonMap(TextAttribute.TRACKING, tracking));
