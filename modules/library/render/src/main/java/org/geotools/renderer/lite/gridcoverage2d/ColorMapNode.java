...
class ColorMapNode extends StyleVisitorCoverageProcessingNodeAdapter
  ...
  /**
   * The {@link Domain1D} that we build while parsing the various {@link ColorMapEntry}s provided by
   * the {@link ColorMapTransform} we visit. This {@link LinearColorMap} we'll help us buld the
   * colormapped image.
   */
  ...
  public void visit(ColorMap colorMap) {
    ...
    if (cmEntries != null && cmEntries.length > 0) {
      ...
      builder
          .setExtendedColors(this.extendedColors)
      ...
      for (int i = 0; i < numBands; i++)
        sd[i] =
            new GridSampleDimension(
                TypeMap.getColorInterpretation(classified.getColorModel(), i).name());
