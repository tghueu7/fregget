...
/**
 * This           {@link CoverageProcessingNode}           is responsible for visiting the 
 * supplied          {@link ColorMapTransform}           and applying it to the source           
 * {@link GridCoverage2D}          . <p> <strong>What we support and how do we implement 
 * it</strong> <p> A ColorMapTransform is created in order to map categories to colors on a 
 * single band coverage (or on the visible band of multiband coverage). <p> In this 
 * implementation we allow users to use either 256 or 65536 colors via the creation of a paletted
 * image with s suitable palette derived from the single           {@link ColorMapEntry}         
 * that make up the           {@link ColorMapTransform}          .
 *
...
class ColorMapNode extends StyleVisitorCoverageProcessingNodeAdapter implements
    ...
    public void visit(ColorMap colorMap) {
        ...
        if (cmEntries != null && cmEntries.length > 0) {
            ...
            if (candidateNoDataValues != null && candidateNoDataValues.length > 0) {
                final LinearColorMapElement noDataCategories[] = new 
                        LinearColorMapElement[candidateNoDataValues.length];
    ...
    protected GridCoverage2D execute() {
        ...
        if (colorMapTransform != null) {
            ...
            GridCoverageRendererUtilities.ensureSourceNotNull(sourceImage, this.getName()
                    .toString());
            ...
            final RenderedOp classified = w.getRenderedOperation();//JAI.create
            ...
            for (int i = 0; i < numBands; i++)
                sd[i] = new GridSampleDimension(TypeMap.getColorInterpretation(classified
                        .getColorModel(), i).name());
