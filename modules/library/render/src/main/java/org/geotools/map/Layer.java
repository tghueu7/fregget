...
public abstract class Layer {
  ...
  /**
   * Sets whether the layer is to be shown or hidden when rendering. {@link LayerEvent} is fired if
  ...
  /**
   * Sets layer selection status. This can be used by clients such as {@code JMapPane} for selective
   ...
   *
   * <p>The returned bounds are a ReferencedEnvelope using the same CoordinateReferenceSystem as the
   * layers contents.
   *
  ...
  /**
   * The listener is notified when the layer information changes; or when the contents of the layer
   * changes. This is used by interactive map displays to detect when information is modified and
   * needs to be redrawn.
   *
  ...
  /**
   * Called in an interactive environment where a Layer is expected to listen to the data source on
   * behalf of the application.
   *
   * <p>This method is called as needed by addMapListener and removeMapListener to allow subclasses
   ...
   * as KML.
   *
   * <p>Please note that feature based renders can be very flexible; as an example raster content is
