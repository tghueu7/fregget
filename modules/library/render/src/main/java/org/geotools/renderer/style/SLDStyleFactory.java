...
public class SLDStyleFactory {
     ...
     */
    public static final String USE_LEGACY_ANCHOR_POINT_KEY = "org.geotools.renderer.style" +
            ".legacyAnchorPoint";
     ...
     */
    private static final java.util.Map<String, Integer> ALPHA_COMPOSITE_LOOKUP = new java.util
     ...
     * @param drawMe     The feature
     * @param symbolizer The SLD symbolizer
     * @param scaleRange The scale range in which the feature should be painted
    ...
    Style2D createLineStyle(Object feature, LineSymbolizer symbolizer,
        ...
        style.setPerpendicularOffset(evalToDouble(symbolizer.getPerpendicularOffset(), feature, 
                0d));
     ...
     * @return
     */
    Style2D createPointStyle(Object feature, Symbolizer symbolizer, Graphic sldGraphic, Range 
            scaleRange, boolean forceVector) {
        Style2D retval = null;
    ...
    MarkStyle2D createMarkStyle(Mark mark, Object feature, Symbolizer symbolizer, double size) {
        ...
        ms2d.setFillComposite(composite != null ? composite : getComposite(mark.getFill(), 
                feature));
    ...
    Style2D createTextStyle(Object feature, TextSymbolizer symbolizer,
        ...
        double anchorX = PointPlacement.DEFAULT_ANCHOR_POINT.getAnchorPointX().evaluate(null, 
                Double.class);
        double anchorY = PointPlacement.DEFAULT_ANCHOR_POINT.getAnchorPointY().evaluate(null, 
                Double.class);
        double rotation = 0;
        double dispX = 0;
        double dispY = 0;
        ...
        if (symbolizer instanceof TextSymbolizer2) {
            ...
            if (graphicShield != null) {
                Style2D shieldStyle = createPointStyle(feature, symbolizer, graphicShield, 
                        scaleRange, true);
     ...
     * @param geomName The name of the geometry to find: null if the default geometry
     *                 should be used.
     ...
     * @param fonts         An array of fonts dependent of the feature, the first that is
    ...
    private java.awt.Font applyKerning(java.awt.Font font) {
        return font.deriveFont(Collections.singletonMap(TextAttribute.KERNING, TextAttribute
                .KERNING_ON));
    }
    ...
    private static List<Float> dashArrayExpressionToFloatList(Expression expression, Object 
            feature) {
    ...
    TexturePaint getTexturePaint(org.geotools.styling.Graphic gr, Object feature,
        ...
        PositionRandomizer randomizer = (PositionRandomizer) voParser.getEnumOption(symbolizer, 
                "random", PositionRandomizer.NONE);
        ...
        if (randomizer != null && randomizer != PositionRandomizer.NONE) {
            ...
            image = randomFillBuilder.buildRandomTilableImage(symbolizer, gr, icon, mark, shape, 
                    graphicSize, feature);
        } else {
            ...
            if (margin != null) {
                ...
                BufferedImage imageWithMargin = new BufferedImage(image.getWidth() + extraX, 
                        image.getHeight() + extraY, type);
                int tx = margin[1];
                int ty = margin[0];
        ...
        Rectangle2D.Double rect = new Rectangle2D.Double(0.0, 0.0, image.getWidth(), image
                .getHeight());
    ...
    private BufferedImage markToTilableImage(org.geotools.styling.Graphic gr, Object feature,
        ...
        if ((sizeX * repeat) * (sizeY * repeat) > Integer.MAX_VALUE) {
            ...
            if (sizeX * sizeY > Integer.MAX_VALUE) {
                ...
                LOGGER.fine("Size of metatiled graphic (" + (repeat * sizeX) + " * " + (repeat * 
                        sizeY)
        ...
        double rotation = Math.toRadians(evalToDouble(gr.getRotation(), feature, 0.0)); // fix 
        // for GEOS-6217
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                if (eg.getFormat() != null) {
                    Expression formatExpression = ExpressionExtractor.extractCqlExpressions(eg
                            .getFormat());
        ...
        if (spec.indexOf(',') != -1) {
            ...
            if (invalidOpacity || opacity < 0 || opacity > 1) {
                throw new IllegalArgumentException(
                        "Invalid value for composite opacity, expecting a number between 0 and 1," +
                                " but got '"
                                + split[1] + "' instead");
            }
        } else {
