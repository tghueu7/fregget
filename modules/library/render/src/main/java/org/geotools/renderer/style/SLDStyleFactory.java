...
public class SLDStyleFactory {
  ...
  /**
   * Whether to turn on vector rendering or not. Normal behavior is to have it turned off, which is
   * faster but may not be the best thing when printing due to quality loss.
   */
  ...
  /**
   * Enabled by default, this optimization speeds up line rendering when the line width is less than
   * 1.5 pixels when antialiasing is disblaed. Unfortunately it also prevents fine line width
   * control when antialiasing is enabled. Given that the optimization has been hard coded for more
   * than six years, we give the user control on this one since turning this off will change the
   * rendering of all existing styles using thin line widths.
   */
  ...
  /**
   * Indicates whether vector rendering should be preferred when painting graphic fills (e.g., using
   * a Mark as stipple) or vector Graphic objects such as SVG ExternalGraphics. The default behavior
   * is to be disabled, meaning that graphic fills are painted as raster images using Java
   * TexturePaint, and SVGs are rendered to a BufferedImage prior to painting on the target
  ...
  void setPolygonStyleFill(
    ...
    if (fill.getGraphicFill() != null) {
      ...
      if (isVectorRenderingEnabled() || size > MAX_RASTERIZATION_SIZE) {
        ...
        // enabled
        Style2D style2DFill =
            createPointStyle(feature, symbolizer, fill.getGraphicFill(), scaleRange, false);
  ...
  /**
   * Turns a floating point style into a integer size useful to specify the size of a BufferedImage.
   * Will return 1 between 0 and 1 (0 excluded), will otherwise round the size to integer.
   *
   * @param size
   * @return
   */
   ...
   * @param geomName The name of the geometry to find: null if the default geometry should be used.
   ...
   * @param fonts An array of fonts dependent of the feature, the first that is found on the current
   *     machine is returned
   * @param vendorOptions
  ...
  private Style2D getGraphicStroke(
      Symbolizer symbolizer, org.geotools.styling.Stroke stroke, Object feature, Range scaleRange) {
