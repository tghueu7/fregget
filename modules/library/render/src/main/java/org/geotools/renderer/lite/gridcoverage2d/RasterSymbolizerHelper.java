...
//			return output;
        ...
        // Getting NoData
        Range nodata = CoverageUtilities.getNoDataProperty(output) != null ? CoverageUtilities
                .getNoDataProperty(output).getAsRange() : null;
        ...
        if (numBands > 4) {
            ...
            outputImage =
                    new ImageWorker(outputImage).setRenderingHints(this.getHints()).retainBands
                            (new int[]{visibleBand}).getRenderedImage();
            sd = new GridSampleDimension[]{(GridSampleDimension) output.getSampleDimension
                    (visibleBand)};
        } else {
        ...
        switch (dataType) {
            ...
            case DataBuffer.TYPE_SHORT:
                ...
                outputImage =
                        w.setROI(roiProp).setNoData(nodata).setRenderingHints(this.getHints())
                                .rescaleToBytes().getRenderedImage();
                roiProp = w.getROI();
                nodata = w.getNoData();
        }
        ...
        if (opacity < 1) {
            ...
            for (int i = 0; i < numBands; i++) {
                sd[i] = new GridSampleDimension(TypeMap.getColorInterpretation(finalImage
    ...
    public RasterSymbolizerHelper(GridCoverage2D sourceCoverage, Hints hints) {
        super(
                1,
                hints,
                SimpleInternationalString.wrap(Vocabulary.format(VocabularyKeys
