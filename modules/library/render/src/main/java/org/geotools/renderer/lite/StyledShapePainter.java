...
public class StyledShapePainter {
    ...
    private final static Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (StyledShapePainter.class.getName());
    /**
     * Whether icon centers should be matched to a pixel center, or not
     */
    public static boolean ROUND_ICON_COORDS = Boolean.parseBoolean(System.getProperty("org" +
            ".geotools.renderer.lite.roundIconCoords", "true"));

    /**
     * Whether to apply the new vector hatch fill optimization, or not (on by default, this is 
     * just a safeguard)
     */
    public static boolean OPTIMIZE_VECTOR_HATCH_FILLS = Boolean.parseBoolean(System.getProperty
    ...
    public void paint(final Graphics2D graphics, final LiteShape2 shape,
        ...
        if (style instanceof IconStyle2D) {
            ...
            if (style instanceof PolygonStyle2D && !optimizeOutFill((PolygonStyle2D) style, 
                    shape)) {
    ...
    void paintLineStyle(final Graphics2D graphics, final LiteShape2 shape,
                        final LineStyle2D ls2d, boolean isLabelObstacle, float 
        ...
        if (ls2d.getStroke() != null) {
            ...
            if (ls2d.getGraphicStroke() != null) {
                drawWithGraphicsStroke(graphics, dashShape(shape, ls2d.getStroke()), ls2d
                        .getGraphicStroke(), isLabelObstacle);
            } else {
                ...
                if (graphics
                        .getRenderingHint(RenderingHints.KEY_ANTIALIASING) == RenderingHints
                        .VALUE_ANTIALIAS_ON) {
    ...
    public void paint(final Graphics2D graphics, final LiteShape2 shape, final GraphicLegend legend,
        ...
        while (symbolIter.hasNext()) {
            ...
            if (symbol instanceof ExternalGraphic) {
                ...
                // Note: Converting to Radians here due to direct use of SLD Expressions which 
                // uses degrees
                double rotation = Math.toRadians(((Literal) legend.getRotation()).evaluate(null, 
                        Double.class));
                ...
                while (!(iter.isDone())) {
                    iter.currentSegment(coords);
                    try {
                        BufferedImage image = ImageIO.read(graphic.getOnlineResource().getLinkage
                                ().toURL());
                        if ((symbolScale > 0.0) && (symbolScale != 1.0)) {
                            int w = (int) (image.getWidth() / symbolScale);
                            int h = (int) (image.getHeight() / symbolScale);
                            int imageType = image.getType() == 0 ? BufferedImage.TYPE_4BYTE_ABGR 
                                    : image.getType();
                            ...
                            g.drawImage(image, 0, 0, w, h, 0, 0, image.getWidth(), image
                                    .getHeight(), null);
                            g.dispose();
                            image = rescaled;
                        }
                        ...
                        Logger.getLogger(StyledShapePainter.class.getName()).log(Level.SEVERE, 
                                null, ex);
                    }
                    iter.next();
                }
    ...
    private void renderImage(Graphics2D graphics, double x, double y, double dx, double dy,
                             BufferedImage image, double rotation, Composite composite, boolean 
                                     isLabelObstacle) {
        if (LOGGER.isLoggable(Level.FINEST)) {
    ...
    private void renderGraphicsStroke(Graphics2D graphics, double x, double y, Style2D style,
                                      double rotation, double graphicRotation, Composite 
                                              composite, boolean isLabelObstacle) {
        if (LOGGER.isLoggable(Level.FINEST)) {
        ...
        if (style instanceof GraphicStyle2D) {
            ...
            if (isLabelObstacle) {
                labelCache.put(new Rectangle2D.Double(x + dx, y + dy, icon.getIconWidth(), icon
                        .getIconHeight()));
            }
        }
    }
    ...
    void fillLiteShape(Graphics2D g, LiteShape2 shape) {
        if (shape.getGeometry() instanceof MultiPolygon && shape.getGeometry().getNumGeometries()
                > 1) {
            ...
            for (int i = 0; i < mp.getNumGeometries(); i++) {
                ...
                try {
                    ...
                    throw new RuntimeException("Unexpected error occurred while rendering a " +
                            "multipolygon", e);
                }
            }
        } else {
     ...
     * @throws FactoryException
     */
    protected void paintGraphicFill(Graphics2D graphics, Shape shape, Style2D graphicFill, double
            scale) {
        ...
        if (graphicFill instanceof MarkStyle2D) {
            ...
            double aspect = (boundsFill.getHeight() > 0 && boundsFill.getWidth() > 0) ? 
                    boundsFill.getWidth() / boundsFill.getHeight() : 1.0;
            ...
            if (OPTIMIZE_VECTOR_HATCH_FILLS) {
                final Shape rescaledStipple = AffineTransform.getScaleInstance(scaleFactor, 
                        scaleFactor).createTransformedShape(markShape);
        ...
        if (boundsClip.getMinX() > boundsShape.getMinX()) {
            fromX = (int) Math.floor((boundsClip.getMinX() - boundsShape.getMinX()) / stippleSize
                    .getWidth());
        }
        if (boundsClip.getMaxX() < boundsShape.getMaxX()) {
            toX -= (int) Math.floor((boundsShape.getMaxX() - boundsClip.getMaxX()) / stippleSize
                    .getWidth());
        }
        ...
        if (boundsClip.getMinY() > boundsShape.getMinY()) {
            fromY = (int) Math.floor((boundsClip.getMinY() - boundsShape.getMinY()) / stippleSize
                    .getHeight());
        }
        if (boundsClip.getMaxY() < boundsShape.getMaxY()) {
            toY -= (int) Math.floor((boundsShape.getMaxY() - boundsClip.getMaxY()) / stippleSize
                    .getHeight());
        }
        ...
        for (int i = fromX; i <= toX; i++) {
            for (int j = fromY; j <= toY; j++) {
                ...
                try {
                    stippleShape = new LiteShape2(stipplePoint, identityTransf, nullDecimator, 
                            false);
                } catch (Exception e) {
