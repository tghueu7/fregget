...
public class StyledShapePainter {
  ...
  /**
   * Whether to apply the new vector hatch fill optimization, or not (on by default, this is just a
   * safeguard)
   */
  public static boolean OPTIMIZE_VECTOR_HATCH_FILLS =
      Boolean.parseBoolean(
          System.getProperty("org.geotools.renderer.lite.optimizeVectorHatchFills", "true"));
  ...
  /**
   * Checks if the fill can simply be omitted because it's not going to be visible anyways. It takes
   * a style that has a solid outline and a width or height that's less than the stroke width
   *
   * @param style
   * @param shape
   * @return
  ...
  private void renderGraphicsStroke(
    ...
    if (style instanceof GraphicStyle2D) {
      ...
      if (isLabelObstacle) {
        labelCache.put(
            new Rectangle2D.Double(x + dx, y + dy, icon.getIconWidth(), icon.getIconHeight()));
      }
    }
  }
