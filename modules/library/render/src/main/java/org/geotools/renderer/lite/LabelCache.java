...
public interface LabelCache {
  ...
  /**
   * Called by renderer to indication the start of rendering a layer. Will add the layer to the set
   * of active layers.
   *
   ...
   *
   * <p>This facility is used to reserve an area so that labels do not end up overlapping on screen
   * constructs like scalebars or north arrows etc...
   *
  ...
  /**
   * Enable a layer after being disabled. If startLayer is called this does not need to be called as
   * start layer implicitely activates the layer.
   *
