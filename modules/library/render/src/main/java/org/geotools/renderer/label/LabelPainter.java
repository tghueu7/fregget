...
public class LabelPainter {
  ...
  /**
   * Sets the current label. The label will be laid out according to the label item settings (curved
   * lines, auto wrapping, curved line usage) and the painter will be ready to draw it.
   *
   * @param labelItem
   */
  public void setLabel(LabelCacheItem labelItem) {
    ...
    for (LineInfo info : lines) {
      ...
      double minX =
          (maxWidth - currBounds.getWidth()) * textStyle.getAnchorX() - currBounds.getMinX();
      info.setMinX(minX);
  ...
  /**
   * If, for any reason, a font size of 0 is provided to the renderer, resulting bounds will become
   * empty and this will ruin most geometric computations dealing with spacing and orientations.
   * Enlarge the envelope a tiny bit
   *
   * @param bounds
   */
  ...
  /**
   * Returns the line height for this label in pixels (for multiline labels, it's the height of the
   * first line)
   *
   * @return
   */
    ...
    try {
      for (LineComponent component : line.getComponents()) {
        ...
        for (int i = 0; i < numGlyphs; i++) {
          ...
          nextAdvance =
              i < numGlyphs - 1 ? glyphVector.getGlyphMetrics(i + 1).getAdvance() * 0.5f : 0;
  ...
  public double getLinePlacementYAnchor() {
    ...
    LineMetrics lm =
        textStyle.getFont().getLineMetrics(textStyle.getLabel(), graphics.getFontRenderContext());
   ...
   * defining upwards a label whose bottom to top direction is greater than zero, and less or equal
   * to 180 degrees.
   *
   * @param cursor
   * @return
