...
public class LabelPainter {
    ...
    public Rectangle2D getFullLabelBounds() {
        ...
        if (labelItem.getTextStyle().getGraphic() != null) {
            ...
            if (mode == GraphicResize.STRETCH) {
                ...
                shieldBounds = new Rectangle2D.Double(width / 2 + bounds.getMinX() - bounds
                        .getWidth() / 2,
        ...
        try {
            ...
            if (graphic != null) {
                ...
                LiteShape2 tempShape = new LiteShape2(gf.createPoint(new Coordinate(labelBounds
                        ...
                        labelBounds.getMinY() + lastLine.getBounds().getHeight() - 1.0 * 
                                labelBounds.getHeight() / 2.0 + offsetY)), null, null,
                        false, false);
    ...
    private Style2D resizeGraphic(Style2D graphic) {
        ...
        if (graphic instanceof MarkStyle2D) {
            ...
            if (mode == GraphicResize.PROPORTIONAL) {
                ...
                tss.shape = original;
                tss.setTransform(AffineTransform.getScaleInstance(width / bounds.getWidth(), 
     ...
     */
    private void drawStraightLabelUnderlineIfNeeded(java.awt.Shape outline, LineMetrics metrics, 
                                                    boolean drawingHalo) {
     ...
     * using the provided thickness and offset.
     */
    private void drawStraightLabelStrikethroughIfNeeded(java.awt.Shape outline, LineMetrics 
            metrics, boolean drawingHalo) {
    ...
    private void drawStraightLabelLine(java.awt.Shape outline, boolean drawingHalo, float 
            thickness, float offset) {
    ...
    private void configureHalo() {
        ...
        float haloRadius = labelItem.getTextStyle().getHaloFill() != null ? labelItem
                .getTextStyle().getHaloRadius() : 0;
        graphics.setStroke(new BasicStroke(2 * haloRadius, BasicStroke.CAP_ROUND, BasicStroke
                .JOIN_ROUND));
    }
     ...
     */
    private void drawCurvedUnderline(LineInfo line, LineStringCursor cursor, double 
            startOrdinate, boolean drawingHalo, LineMetrics metrics) {
     ...
     * Helper method that will draw the underline of a curved label using the context of the cursor.
     */
    private void drawCurvedStrikethrough(LineInfo line, LineStringCursor cursor, double 
            startOrdinate, boolean drawingHalo, LineMetrics metrics) {
    ...
    private void drawCurvedLine(LineInfo line, LineStringCursor cursor, double startOrdinate,
                                boolean drawingHalo, final float lineOffset, final float 
                                        lineThickness) {
        ...
        LineString labelLineString = cursor.getSubLineString(startOrdinate - advance, endOrdinate
                - advance);
     ...
     */
    private void drawOrFillOutlines(List<Shape[]> allOutlines, List<AffineTransform[]> 
            allTransforms, boolean fill) {
