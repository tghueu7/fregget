...
public class MapViewport {
  ...
  /**
   * Creates a new view port. Screen area and world bounds will be empty, and aspect ratio matching
   * will be disabled.
   */
  public MapViewport() {
    this(false);
  ...
  /**
   * Creates a new viewport based on an existing instance. The world bounds, screen area and aspect
   * ratio matching setting of {@code sourceViewport} are copied.
   *
   ...
   * non-editable viewport will not allow the value of any of its attributes to be changed and will
   * issue a log message (fine level) on any attempt to do so.
   *
  ...
  /**
   * Sets whether the value of this viewport's attributes can be changed. Viewports are editable by
   * default.
   *
   ...
   *
   * <p>Note Well: this only covers spatial extent; you may wish to use the user data map to record
   * the current viewport time or elevation.
   *
   ...
   *
   * @return a copy of the current screen to world transform or {@code null} if the transform is not
   *     set
   */
   ...
   *
   * @return a copy of the current world to screen transform or {@code null} if the transform is not
   *     set
   */
   ...
   * aspect ratio matching is enabled, the transforms transforms are calculated to centre the world
   * bounds in the screen area, after which the bounds will be adjusted if necessary to have the
   * same aspect ratio as the screen area. If aspect ratio matching is not enabled, basic transforms
   * are calculated without centering or bounds adjustment.
   *
  ...
  /**
   * Helper for setter methods which checkst that this viewport is editable and issues a log message
   * if not.
   */
