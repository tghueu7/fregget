...
/**
 * Builder facility for creating a        {@link LinearColorMap}        using elements from      
 * {@link RasterSymbolizer}               {@link ColorMapTransform}        element. <p> This 
 * class is not intended to be thread safe.
 *
...
public class SLDColorMapBuilder {
    ...
    /**
     * List of {@link LinearColorMapElement} we are putting together to create the final 
     * {@link LinearColorMap}
     **/
    private final List<LinearColorMapElement> colormapElements = new 
            ArrayList<LinearColorMapElement>();
    ...
    /**
     * Sets the default      {@link Color}      to use when a value falls outside the range of 
     * values for  provided color map elements. <p> Note that once the underlying colormap has 
     * been built this method will throw an      {@link IllegalStateException}      if invoked. 
     * <p> In case one would want to unset the default color, he should simply call this method 
     * with a <code>null</code> value.
     *
     * @param the default      {@link Color}      to use when a value falls outside the   
    ...
    /**
     * Retrieves the      {@link LinearColorMapType}      for this      
     * {@link SLDColorMapBuilder}     . <p> <code>-1</code> is returned in case the      
     * {@link LinearColorMapType}      is still unspecified.
     *
     * @return the      {@link LinearColorMapType}      for this      {@link SLDColorMapBuilder} 
     * or <code>-1</code>  case the      {@link LinearColorMapType}      is still unspecified.
     * @uml.property name="linearColorMapType"
     */
    ...
    public SLDColorMapBuilder addColorMapEntry(ColorMapEntry colorMapEntry) {
        ...
        ColorMapUtilities.ensureNonNull("colorMapEntry", colorMapEntry);
        if (this.numberColorMapEntries == -1 || linearColorMapType == -1 || numberColorMapEntries
                < colormapElements.size() + 1)
            throw new IllegalStateException(Errors.format(ErrorKeys.ILLEGAL_STATE));
            ...
            if (PiecewiseUtilities.compare(previousMax, q) != 0) {
                ...
                switch (linearColorMapType) {
                    case ColorMap.TYPE_RAMP:
                        ...
                        boolean minIncluded = previousColors[previousColors.length - 1].equals
                                (colors[0]);
                        ...
                        colormapElements.add(
                                LinearColorMapElement.create(label, colors, valueRange, 
                                        sampleRange));
                        break;
                    case ColorMap.TYPE_VALUES:
    ...
    private void init() {
        ...
        if (linearColorMapType == ColorMap.TYPE_RAMP) {
            ...
            //
            // //
            double colorsToDistribute = !extendedColors && numberColorMapEntries < 256 ? 256 : 
                    65536;
            //default color
            if (gapsColor != null)
                colorsToDistribute--;
            ...
            colorsPerColorMapElement = (int) ((colorsToDistribute) /
                    // we remove one since the first and last element use 1 color only, hence we 
                    // want to account only the internal ranges
    ...
    /**
     * Setter of the property <tt>extendedColors</tt> <p> Unless this property is set prior to 
     * start working with this      {@link SLDColorMapBuilder}     we will make use of only 256 
     * colors. If we use extended colors, then we'll be able to use  up to 65536 colors. <p> Note
     * that this imposes a limitation on the maximum number of      {@link ColorMapEntry}      we
     * can use.
     *
    ...
    public LinearColorMap buildLinearColorMap() {
        ...
        if (linearColorMapType == ColorMap.TYPE_RAMP) {
            ...
            last = LinearColorMapElement.create(
                    "ColorMapEntry" + this.colormapElements.size(), lastColorValue, RangeFactory
                            .create(
        ...
        //
        // /////////////////////////////////////////////////////////////////////
        final LinearColorMapElement preservedValuesElement[] = new 
                LinearColorMapElement[preservedValues
                .size()];
            ...
            preservedValuesElement[i] = LinearColorMapElement
                    .create(
                            org.geotools.resources.i18n.Vocabulary.format(
                                    org.geotools.resources.i18n.VocabularyKeys.NODATA) + Integer
                                    .toString(i + 1),
        ...
        this.colorMap = new LinearColorMap(name,
                (LinearColorMapElement[]) colormapElements.toArray(new 
                        LinearColorMapElement[colormapElements.size()]),
