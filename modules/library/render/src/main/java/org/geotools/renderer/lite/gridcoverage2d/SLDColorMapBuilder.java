...
public class SLDColorMapBuilder {
   ...
   *
   * <p>In case one would want to unset the default color, he should simply call this method with a
    ...
    if (linearColorMapType == ColorMap.TYPE_RAMP) {
      ...
      colorsPerColorMapElement =
          (int)
              ((colorsToDistribute)
                  /
                  // we remove one since the first and last element use 1 color only, hence we want
                  // to account only the internal ranges
   ...
   *
   * @return an array of double which represents the values that need to be preserved by the {@link
   *     ColorMapTransform} we will create.
   */
