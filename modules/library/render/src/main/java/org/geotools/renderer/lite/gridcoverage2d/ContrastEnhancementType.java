...
public enum ContrastEnhancementType {
    ...
    LookupTable createByteLookupTable(Map<String, Object> params) {
      ...
      for (int i = 1; i < lut.length; i++) {
        lut[i] =
            i < min
                ? (byte) MIN_BYTE
                : (i > max ? (byte) MAX_BYTE : ((byte) (scale * i + offset + 0.5d)));
      }
  ...
  /**
   * Main processing methods to be implemented by specific contrast enhancement algorithm to return
   * an enhanced image.
   *
   * @param inputWorker
   * @param hints
   ...
   *     algorithm may requires different parameters. As an instance, some implementations requires
   *     min and max values whilst some other may require an Histogram.
   * @return
   */
  PiecewiseTransform1D<DefaultPiecewiseTransform1DElement> generatePiecewise(
      Map<String, Object> params) {
  ...
  private static void checkParameters(
    ...
    if (parameters == null) {
      throw new IllegalArgumentException(Errors.format(ErrorKeys.NULL_ARGUMENT_$1, parameters));
    }
    if (parameters.isEmpty()) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_PARAMETERS_$2, parameter1, parameter2));
    }
  }
