...
public final class RendererUtilities {
    ...
    private final static Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (RendererUtilities.class.getName());

    /**
     * Enable unit correction in {@link #toMeters(double, CoordinateReferenceSystem)} calculation.
     * <p>
     * Toggle for a bug fix that will invalidate a good number of SLDs out there (and thus, we 
     * allow people to
     * turn off the fix).
     */
    static boolean SCALE_UNIT_COMPENSATION = Boolean.parseBoolean(System.getProperty("org" +
            ".geotoools.render.lite.scale.unitCompensation", "true"));
     ...
     *                      coordinates. No
     *                      assumptions are done on axis order as this is assumed to be 
     *                      pre-calculated. The
     *                      affine transform may specify an rotation, in case the envelope will 
     *                      encompass the
    ...
    public static ReferencedEnvelope createMapEnvelope(Rectangle paintArea,
                                                       AffineTransform worldToScreen, final 
     ...
     * @param hints            The hints used in calculation. if "dpi" key is present, it
     ...
     * <p>
     * In GeoTools 12 this method started to take into account units of measure, if this is not 
     * desirable
     * in your application you can set the system variable "org.geotoools.render.lite.scale
     * .unitCompensation"
     * to false.
     *
     * @param envelope
    ...
    /**
     * Method used by the OGC scale calculation to turn a given length in the specified CRS 
     * towards meters.
     * <p>
     * GeographicCRS uses {@link #OGC_DEGREE_TO_METERS} for conversion of lat/lon measures</li>
     * <p>
     * Otherwise the horizontal component of the CRS is assumed to have a uniform axis unit of 
     * measure
     * providing the Unit used for conversion. To ignore unit disable 
     * {@link #SCALE_UNIT_COMPENSATION} to
    ...
    private static double toMeters(double size, CoordinateReferenceSystem crs) {
        if (crs == null) {
            LOGGER.finer("toMeters: assuming the original size is in meters already, as crs is " +
                    "null");
            return size;
        }
        ...
        if (unit == null) {
            LOGGER.finer("toMeters: assuming the original size is in meters already, as the first" +
                    " crs axis unit is null. CRS is " + crs);
            return size;
        }
     ...
     * specification, page 26.
     *
     * @param CRS           the coordinate reference system. Used to check if we are operating in
     *                     degrees or
     *                      meters.
     ...
     * @return
     */
    public static double calculateOGCScaleAffine(CoordinateReferenceSystem crs, AffineTransform 
            worldToScreen, Map hints) {
     ...
     */
    public static double calculateScale(ReferencedEnvelope envelope, int imageWidth, int 
            imageHeight,
                                        Map hints)
            throws TransformException, FactoryException {
        ...
        if (hints != null && hints.containsKey("declaredScaleDenominator")) {
            ...
            if (scale.doubleValue() <= 0)
                throw new IllegalArgumentException("the declaredScaleDenominator must be greater " +
                        "than 0, was: " + scale.doubleValue());
    ...
    private static double geodeticDiagonalDistance(Envelope env) {
        if (env.getWidth() < 180 && env.getHeight() < 180) {
            return getGeodeticSegmentLength(env.getMinX(), env.getMinY(), env.getMaxX(), env
                    .getMaxY());
        } else {
            ...
            LineString ls = gf.createLineString(new Coordinate[]{new Coordinate(env.getMinX(), 
                    env.getMinY()),
            ...
            qMinX = Math.min(qMinX, (int) (Math.signum(env.getMinX()) * Math.ceil(Math.abs(env
                    .getMinX() / 180.0))));
            qMaxX = Math.max(qMaxX, (int) (Math.signum(env.getMaxX()) * Math.ceil(Math.abs(env
                    .getMaxX() / 180.0))));
            qMinY = Math.min(qMinY, (int) (Math.signum(env.getMinY()) * Math.ceil(Math.abs((env
                    .getMinY() + 90) / 180.0))));
            qMaxY = Math.max(qMaxY, (int) (Math.signum(env.getMaxY()) * Math.ceil(Math.abs((env
                    .getMaxY() + 90) / 180.0))));
            for (int i = qMinX; i < qMaxX; i++) {
                for (int j = qMinY; j < qMaxY; j++) {
                    ...
                    LinearRing ring = gf.createLinearRing(new Coordinate[]{new Coordinate(minX, 
                            minY),
                            new Coordinate(minX, maxY), new Coordinate(maxX, maxY), new 
    ...
    private static double getGeodeticSegmentLength(double minx, double miny, double maxx, double 
            maxy) {
     ...
     * @return FeatureCollection<SimpleFeatureType               ,                               
      * SimpleFeature> that produces results with the correct CRS
     */
    static FeatureCollection fixFeatureCollectionReferencing(FeatureCollection features, 
                                                             CoordinateReferenceSystem sourceCrs) {
        // this is the reader's CRS
        CoordinateReferenceSystem rCS = null;
        try {
            rCS = features.getSchema().getGeometryDescriptor().getType()
                    .getCoordinateReferenceSystem();
        ...
        if (rCS != sourceCrs && sourceCrs != null) {
            ...
            if ((rCS == null) || !CRS.equalsIgnoreMetadata(rCS, sourceCrs)) {
                ...
                try {
                    return new ForceCoordinateSystemFeatureResults((SimpleFeatureCollection) 
                            features, sourceCrs);
                } catch (Exception ee) {
     ...
     * <p>
     * Sampling occurs horizontally along the middle of the polygon obtained from the y 
     ...
     * @param pg       The prepared version of geom, can be null in which case it will be 
     *                 computed on demand.
     * @param gf       The geometry factory, can be null in which case the polygons factory will 
     *                 be used.
     * @return A central point that lies inside of the polygon, or null if one could not be found.
     */
    public static Point sampleForInternalPoint(Polygon geom, Point centroid, PreparedGeometry pg,
                                               GeometryFactory gf,
        ...
        if (stepSize > 0) {
            ...
            throw new IllegalArgumentException("One of stepSize or numSamples must be greater " +
                    "than zero");
        }
