...
public final class RendererUtilities {
  ...
  /**
   * Helber class for building affine transforms. We use one instance per thread, in order to avoid
   * the need for {@code synchronized} statements.
   */
   ...
   * servers. For geographic coordinate systems, which use angular units, the angular coverage of a
   * map should be converted to linear units for computation of scale by using the circumference of
   * the Earth at the equator and by assuming perfectly square linear units. For linear coordinate
   * systems, the size of the coordinate space should be used directly without compensating for
  ...
  public static double calculateScale(
    ...
    if (tempCRS == null)
      throw new TransformException(
          Errors.format(ErrorKeys.CANT_REDUCE_TO_TWO_DIMENSIONS_$1, coordinateReferenceSystem));
   ...
   * @param hints The hints used in calculation. if "dpi" key is present, it uses it's Integer value
   *     as the dpi of the current device. if not it uses 90 that is the OGC default value.
   ...
   * #calculateScale(Envelope, CoordinateReferenceSystem, int, int, double)}. If the hints contains
   * a DPI then that DPI is used otherwise 90 is used (the OGS default).
   */
  public static double calculateScale(
  ...
  private static double geodeticDiagonalDistance(Envelope env) {
    if (env.getWidth() < 180 && env.getHeight() < 180) {
      ...
      int qMinY = -1;
      int qMaxY = 1;
      // we must consider at least a pair of quadrants in each direction other wise lines which
      // don't cross both the equator and prime meridian are
      // measured as 0 length. But for some cases we need to consider still more hemispheres.
      qMinX =
          Math.min(
              qMinX,
              (int) (Math.signum(env.getMinX()) * Math.ceil(Math.abs(env.getMinX() / 180.0))));
      qMaxX =
          Math.max(
              qMaxX,
              (int) (Math.signum(env.getMaxX()) * Math.ceil(Math.abs(env.getMaxX() / 180.0))));
      qMinY =
          Math.min(
              qMinY,
              (int)
                  (Math.signum(env.getMinY()) * Math.ceil(Math.abs((env.getMinY() + 90) / 180.0))));
      qMaxY =
          Math.max(
              qMaxY,
              (int)
                  (Math.signum(env.getMaxY()) * Math.ceil(Math.abs((env.getMaxY() + 90) / 180.0))));
   ...
   * If the provided envelope does not carry along a crs the assumption that the map extent is in
   * the classic Lon,Lat form. In case the provided envelope is of type.
   *
   * <p>Note that this method takes into account also the OGC standard with respect to the relation
   * between pixels and sample.
   *
   ...
   * against feature sources generating feature collections without a CRS (which is fatal to the
   * reprojection handling later in the code)
   *
   * @param features
   * @param sourceCrs
   ...
   *
   * <p>Sampling occurs horizontally along the middle of the polygon obtained from the y coordinate
