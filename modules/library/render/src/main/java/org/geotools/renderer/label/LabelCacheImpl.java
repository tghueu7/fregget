...
public class LabelCacheImpl implements LabelCache {
  ...
  public enum LabelRenderingMode {
     ...
     * draw the straight labels. It's faster, straight and horizontal labels look better, diagonal
     * labels look worse, labels and halos are not perfectly centered
     */
    STRING,
    /**
     * Always extracts the outline from the {@link GlyphVector} and paints it as a shape. It's a bit
     * slower, generates more antialiasing, ensures labels and halos are perfectly Centred
     */
    OUTLINE,
    /**
     * Draws all diagonal lines in OUTLINE model, but horizontal ones in STRING mode. Gives the best
     * results when coupled with {@link RenderingHints#VALUE_FRACTIONALMETRICS_ON} for good
   ...
   * its missing --> DEFAULT_PRIORITY 2. if its a number, return that number 3. if its not a number,
   * convert to string and try to parse the number; return the number 4. otherwise, return
   * DEFAULT_PRIORITY
   *
   * @param symbolizer
   * @param feature
  ...
  private LabelCacheItem buildLabelCacheItem(
    ...
    item.setMinGroupDistance(
        voParser.getIntOption(symbolizer, MIN_GROUP_DISTANCE_KEY, DEFAULT_MIN_GROUP_DISTANCE));
    item.setRepeat(voParser.getIntOption(symbolizer, LABEL_REPEAT_KEY, DEFAULT_LABEL_REPEAT));
    item.setLabelAllGroup(
        voParser.getBooleanOption(symbolizer, LABEL_ALL_GROUP_KEY, DEFAULT_LABEL_ALL_GROUP));
    item.setRemoveGroupOverlaps(
        voParser.getBooleanOption(symbolizer, REMOVE_OVERLAPS_KEY, DEFAULT_REMOVE_OVERLAPS));
    ...
    item.setTextStrikethrough(
        voParser.getBooleanOption(symbolizer, STRIKETHROUGH_TEXT_KEY, DEFAULT_STRIKETHROUGH_TEXT));
    item.setWordSpacing(
  ...
  void paintLabels(Graphics2D graphics, Rectangle displayArea) {
    ...
    for (LabelCacheItem labelItem : items) {
      ...
      paintedLineLabels =
          paintLabel(graphics, displayArea, glyphs, paintedLineLabels, painter, labelItem);
    }
  ...
  private boolean paintLineLabelsWithLetterConflict(
    ...
    // ... use at least a 8 pixel step (curved processing is quite expensive), no matter what the
    // label length is
            ...
            if (labelItem.isFollowLineEnabled()) {
              ...
              if ((startOrdinate > 0 && endOrdinate <= cursor.getLineStringLength())) {
                if (maxAngleChange <= maxAngleDelta) {
                  ...
                  // if straight segment connecting the start and end ordinate is really close,
                  // paint as a straight label
                  if (maxAngleChange == 0
              ...
              // required
              if ((allowOverruns
                  || (startOrdinate > 0 && endOrdinate <= cursor.getLineStringLength()))) {
                painter.paintStraightLabel(tx);
                painted = true;
              }
            }
  ...
  /**
   * Gets a representative point and tries to place the label according to SLD. If a maxDisplacement
   * has been set and the default position does not work a search for a better position is tried on
   * concentric circles around the label up until the radius of the circle becomes bigger than the
   * max displacement
   */
   ...
   * polys) 2. choose the most "central" of the points METRIC - choose anyone TODO: change metric to
   * be "closest to the centoid of the possible points"
   *
