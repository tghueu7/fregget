...
public class MapContext extends MapContent {
   ...
   *
   * <p>This method is used to prevent duplication in classes supporting deprecated getMapContext()
   * methods.
   *
  ...
  /**
   * Add a new layer in the specified position and trigger a {@link LayerListEvent}. Layer won't be
   * added if it's already in the list.
   *
  ...
  /**
   * Transform the current area of interest for this context using the provided transform. This may
   * be useful for zooming and panning processes. Does nothing if no area of interest is set.
   *
    ...
    if (!(oldBounds == null || oldBounds.isEmpty())) {
      ...
      ReferencedEnvelope newBounds =
          new ReferencedEnvelope(
              coords[0], coords[2], coords[1], coords[3], oldBounds.getCoordinateReferenceSystem());
   ...
   * have been set. The array returned is a copy, changes to the returned array won't influence the
   * MapContextState
   *
