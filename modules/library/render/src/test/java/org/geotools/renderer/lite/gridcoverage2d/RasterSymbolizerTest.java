...
public class RasterSymbolizerTest extends org.junit.Assert {
  ...
  public void contrastEnhancementMethods() throws Exception {
    ...
    method_5.addOption(
        "algorithm",
        sldBuilder.literalExpression(ContrastEnhancementType.NORMALIZE_CLIP_TO_MINMAX_NAME));
    ...
    GridCoverage2D gc =
        CoverageFactoryFinder.getGridCoverageFactory(null)
            .create(
                "name",
                JAI.create(
                    "ImageRead", new File(TestData.url(this, "small_1band_Float32.tif").toURI())),
                envelope,
                new GridSampleDimension[] {new GridSampleDimension("test1BandByte_SLD")},
                null,
                null);
    ...
    gc =
        CoverageFactoryFinder.getGridCoverageFactory(null)
            .create(
                "name",
                JAI.create(
                    "ImageRead", new File(TestData.url(this, "small_4bands_UInt16.tif").toURI())),
                new GeneralEnvelope(new double[] {-90, -180}, new double[] {90, 180}));
    // the RasterSymbolizer Helper
    rsh = new RasterSymbolizerHelper(gc, null);
    // build the RasterSymbolizer
    ...
    GridCoverage2D gc =
        CoverageFactoryFinder.getGridCoverageFactory(null)
            .create(
                "name",
                JAI.create(
                    "ImageRead", new File(TestData.url(this, "small_3bands_Byte.tif").toURI())),
                envelope,
                gsd,
                null,
                null);
    ...
    GridCoverage2D gc =
        CoverageFactoryFinder.getGridCoverageFactory(null)
            .create(
                "name",
                JAI.create(
                    "ImageRead", new File(TestData.url(this, "small_1band_Byte.tif").toURI())),
                envelope,
                new GridSampleDimension[] {new GridSampleDimension("test1BandByte_SLD")},
                null,
                null);
            ...
            .create(
                ...
                new GridSampleDimension[] {
                  new GridSampleDimension(
                      "sd", new Category[] {new Category("", Color.BLACK, 0)}, null),
                  new GridSampleDimension(
                      "sd-alpha", new Category[] {new Category("", Color.BLACK, 0)}, null)
                },
                null,
                null);
    testColorMap(gc);
  ...
  private void testColorMap(GridCoverage2D gc) throws IOException, TransformerException {
    ...
    IndexColorModel icm1 =
        (IndexColorModel) ((GridCoverage2D) rsh.getOutput()).getRenderedImage().getColorModel();
    testRasterSymbolizerHelper(rsh);
  ...
  private static void testRasterSymbolizerHelper(
    ...
    if (TestData.isInteractiveTest()) {
      ImageIOUtilities.visualize(
          ((GridCoverage2D) rsh.getOutput()).getRenderedImage(), rsh.getName().toString());
  ...
  public void contrastEnhancementInChannelSelection() throws IOException {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      assertTrue(
          Integer.toString((i) + 1).equalsIgnoreCase(postBandSelectionChannel.getChannelName()));
