...
public class StreamingRendererCompositeTest {
  ...
  private static GridCoverage2D readCoverage(String imageFileName) throws Exception {
    ...
    Envelope2D envelope =
        new Envelope2D(DefaultEngineeringCRS.GENERIC_2D, 0, 0, bi.getWidth(), bi.getHeight());
    ...
    File reference =
        new File(
            "./src/test/resources/org/geotools/renderer/composite/test-data/blend1-"
                ...
                + ".png");
    ImageIO.write(
        blended, "PNG", new File(reference.getParent(), "blend1-" + composite + "-point.png"));
  ...
  private BufferedImage composeFts(GridCoverage2D first, GridCoverage2D second) {
    ...
    BufferedImage blended =
        new BufferedImage(
            referenceImage.getWidth(), referenceImage.getWidth(), BufferedImage.TYPE_4BYTE_ABGR);
  ...
  private BufferedImage composePolygon(GridCoverage2D first, Style polygonStyle)
    ...
    LinearRing shell =
        new LinearRing(new LiteCoordinateSequence(0, 0, 0, h, w, h, w, 0, 0, 0), GEOMETRY_FACTORY);
  ...
  private Style applyCompositeOnFirstSymbolizer(String styleName) throws IOException {
    ...
    Symbolizer symbolizer = style.featureTypeStyles().get(0).rules().get(0).symbolizers().get(0);
