...
public class RenderUtilitiesTest extends TestCase {
    ...
    public void testScaleGenericFeet() throws Exception {
        ReferencedEnvelope re = new ReferencedEnvelope(new Envelope(1587500, 1587510, 475000, 
                475010),
    ...
    public void testCreateMapEnvelope() throws Exception {
        ...
        assertEnvelopeEquals(new Envelope(offset, offset + 1600, offset, offset + 1200), env, 
                0.001);
        ...
        env = RendererUtilities.createMapEnvelope(paintArea, at);
        assertEnvelopeEquals(new Envelope(offset, offset + 1200, offset - 1600, offset), env, 
                0.0001);
    ...
    public void testHemisphereCrossing() throws Exception {
        ...
        ReferencedEnvelope re = new ReferencedEnvelope(new Envelope(-72.0, 132.0, -4.0, 70.0), 
                wgs84);
        double scale = RendererUtilities.calculateScale(re, 1000, 500, 75);
    ...
    public void testSampleForCentralPoint() throws Exception {
        ...
        Polygon g = (Polygon) new WKTReader().read("POLYGON ((-112.534433451864 43.8706532611928," +
                "-112.499157652296 44.7878240499628,-99.6587666095152 44.7878240499628," +
                "-99.7242788087131 43.2155312692142,-111.085391877449 43.099601544023," +
                "-110.744593363875 36.1862602686501,-98.6760836215473 35.9436771582516," +
                "-98.7415958207452 33.5197257879307,-111.77852346112 33.9783111823157," +
                "-111.758573671673 34.6566040234952,-113.088767445077 34.7644575726901," +
                "-113.023255245879 43.8706532611928,-112.534433451864 43.8706532611928))");
