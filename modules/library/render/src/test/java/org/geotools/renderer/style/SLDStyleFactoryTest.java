...
public class SLDStyleFactoryTest extends TestCase {
  ...
  public void testResizeGraphicFill() throws Exception {
    ...
    ExternalGraphic eg = sf.createExternalGraphic(url + "icon64.png", "image/png");
    Graphic g =
        sf.createGraphic(new ExternalGraphic[] {eg}, null, null, null, ff.literal(20), null);
  ...
  public void testSortByTwoPropertiesAscending() throws Exception {
    checkSortByParsing(
        "cat ,    name", ff.sort("cat", SortOrder.ASCENDING), ff.sort("name", SortOrder.ASCENDING));
  }
  ...
  public void testKerningOnByDefault() throws Exception {
    ...
    Font font =
        sf.createFont(
            ff.literal("Serif"), ff.literal("italic"), ff.literal("bold"), ff.literal(20));
    ts.setFont(font);
  ...
  /**
   * A very large symbol can cause an Integer overflow in the renderer. The SLDFactory should give a
   * warning instead and try again with out meta tiling.
   */
