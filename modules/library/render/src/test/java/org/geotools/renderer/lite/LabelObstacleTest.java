...
public class LabelObstacleTest {
    ...
    public void testExternalGraphicNoObstacle() throws Exception {
        BufferedImage labels = render(sources("roads", "points"), styles("label", 
                "grinNoObstacle"));
    ...
    ImageWorker intersectionExtrema(BufferedImage labels, BufferedImage obstacles) {
        ...
        //RenderedImage binaryLabel = w.binarize(1).getRenderedImage();//BinarizeDescriptor
        // .create(labelsCombine, 1.0, null);
        // get anything that is not fully white
        RenderedImage binaryObstacles = w1.binarize(250).getRenderedImage();//BinarizeDescriptor
        // .create(pointsCombine, 250.0, null);

        // combine the two, only pixels that are both black in both images will be black (0)
        w.or(binaryObstacles);
        //RenderedImage or = w.or(binaryObstacles).getRenderedImage();//OrDescriptor.create
        // (binaryObstacles, binaryLabel, null);
        // get the extrema
