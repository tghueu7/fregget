...
public class LabelObstacleTest {
  ...
  File file(String name) {
    return new File(
        "src/test/resources/org/geotools/renderer/lite/test-data/obstacles/" + name + ".png");
  }
  ...
  ImageWorker intersectionExtrema(BufferedImage labels, BufferedImage obstacles) {
    ...
    RenderedImage binaryObstacles =
        w1.binarize(250)
            .getRenderedImage(); // BinarizeDescriptor.create(pointsCombine, 250.0, null);

    // combine the two, only pixels that are both black in both images will be black (0)
    w.or(binaryObstacles);
    // RenderedImage or =
    // w.or(binaryObstacles).getRenderedImage();//OrDescriptor.create(binaryObstacles, binaryLabel,
    // null);
    // get the extrema
