...
public class InterpolateFunction implements Function {
  ...
  /**
   * Use as a PropertyName when defining a color map. The "Raterdata" is expected to apply to only a
   * single band;
   */
  ...
  static {
    ...
    Parameter<String> mode =
        new Parameter<String>(
            ...
            new KVP(
                Parameter.OPTIONS,
                Arrays.asList(new String[] {MODE_LINEAR, MODE_COSINE, MODE_CUBIC})));
  ...
  private <T> T linearInterpolate(
    ...
    if (method == Method.COLOR) {
      ...
      int r =
          (int)
              clamp(
                  Math.round(doLinear(lookupValue, data0, data1, color0.getRed(), color1.getRed())),
                  0,
                  255);
      int g =
          (int)
              clamp(
                  Math.round(
                      doLinear(lookupValue, data0, data1, color0.getGreen(), color1.getGreen())),
                  0,
                  255);
      int b =
          (int)
              clamp(
                  Math.round(
                      doLinear(lookupValue, data0, data1, color0.getBlue(), color1.getBlue())),
                  0,
                  255);
  ...
  private <T> T cosineInterpolate(
    ...
    if (method == Method.COLOR) {
      ...
      int r =
          (int)
              clamp(
                  Math.round(doCosine(lookupValue, data0, data1, color0.getRed(), color1.getRed())),
                  0,
                  255);
      int g =
          (int)
              clamp(
                  Math.round(
                      doCosine(lookupValue, data0, data1, color0.getGreen(), color1.getGreen())),
                  0,
                  255);
      int b =
          (int)
              clamp(
                  Math.round(
                      doCosine(lookupValue, data0, data1, color0.getBlue(), color1.getBlue())),
                  0,
                  255);
      return (T) new Color(r, g, b);
  ...
  private <T> T cubicInterpolate(
    ...
    if (segment == 1) {
      ...
      workingPoints.add(
          new InterpPoint(ff2.literal(2 * data0 - data1), workingPoints.get(segment).value));
    }
  ...
  /**
   * Find the interpolation segment containing the lookup value. The value returned is the index, in
   * the interpPoints list, of the higher point of the segment.
   *
   * @return segment index; or 0 if the lookup value is below the range of the interpolation points;
   *     or {@code max segment index + 1} if it is above the range
   */
   ...
   * value.
   *
   * @param x x ordinate of the point for which we seek an interpolated value and which lies between
   *     xi[1] and xi[2]
