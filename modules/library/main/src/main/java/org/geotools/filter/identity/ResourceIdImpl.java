...
public class ResourceIdImpl extends FeatureIdVersionedImpl implements ResourceId {
     ...
     * @param start lower end of the time range, inclusive, or {@code null} only if
     *              {@code end != null}
     * @param start upper end of the time range, inclusive, or {@code null} only if
     *              {@code start != null}
     */
    public ResourceIdImpl(String fid, Date start, Date end) {
        this(fid, (String) null, (Version) null);
        if (start == null && end == null) {
            throw new NullPointerException("At least one of start and end time are required for a" +
                    " lookup based on a date range");
        }
