...
public class QuantileFunction extends ClassificationFunction {
    ...
    private Object calculate(SimpleFeatureCollection featureCollection) {
        ...
        QuantileListVisitor quantileVisit = new QuantileListVisitor(getParameters().get(0), 
                getClasses());
        if (progress == null) progress = new NullProgressListener();
        try {
            featureCollection.accepts(quantileVisit, progress);
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE, "QuantileFunction calculate(SimpleFeatureCollection) " +
                    "failed", e);
            return null;
        }
    ...
    private Object calculateNumerical(List[] bin, Comparable globalMin, Comparable globalMax) {
        ...
        //globally consistent
        //double slotWidth = (((Number) globalMax).doubleValue() - ((Number) globalMin)
        // .doubleValue()) / classNum;
        for (int i = 0; i < classNum; i++) {
            ...
            //locally accurate
            double slotWidth = ((Number) localMax[i]).doubleValue() - ((Number) localMin[i])
                    .doubleValue();
            if (slotWidth == 0.0) { //use global value, as there is only 1 value in this set
                slotWidth = (((Number) globalMax).doubleValue() - ((Number) globalMin)
                        .doubleValue()) / classNum;
            }
