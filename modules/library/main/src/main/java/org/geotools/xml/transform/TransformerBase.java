...
public abstract class TransformerBase {
   ...
   * String. Calls transform(Object,Writer). <em>It should be noted the most efficient mechanism of
   * encoding is using the OutputStream or Writer methods</em>
   */
  ...
  /**
   * Should this transformer declare namespace prefixes in the first element it outputs? Defaults to
   * true.
   *
  ...
  public class Task implements Runnable {
    ...
    /**
     * Calls to the underlying translator to abort any calls to translation. Should return silently
    ...
    /**
     * Perform the translation. Exceptions are captured and can be obtained through the checkError
     * and getError methods.
     */
    public void run() {
      try {
  ...
  protected abstract static class TranslatorSupport implements Translator {
    ...
    /**
     * The queue of write operations pending for this translator. This should be empty if no mark is
     * set.
     */
    ...
    /**
     * The Comment class implements an Action corresponding to writing a comment block in the XML
     * output
     */
    ...
    /**
     * Set a mark() to which we can later "roll back" writes. After a call to mark(), the Translator
     * stores pending write operations in memory until commit() is called. The pending writes can be
     * discarded with the reset() method.
     *
     ...
     *
     * <p>This method is safe to call even if no mark is set - so it returns to a "known good" state
    ...
    /**
     * Commit pending write operations. After setting a mark, this method will commit the pending
     * writes.
     *
     * @see #mark()
   ...
   *     add a location to your nsURI. Or at least tie them more closely, so that you can only add a
    ...
    public void parse(InputSource in) throws SAXException {
      ...
      if (base.isNamespaceDeclartionEnabled()) {
        ...
        if (translator.getDefaultNamespace() != null) {
          ...
          atts.addAttribute(
              XMLNS_NAMESPACE, null, "xmlns", "CDATA", translator.getDefaultNamespace());
        ...
        if ((schemaLocSup != null) && !schemaLocSup.getSchemaLocation().equals("")) {
          ...
          atts.addAttribute(
              null, null, "xsi:schemaLocation", null, schemaLocSup.getSchemaLocation());
        }
      } else {
