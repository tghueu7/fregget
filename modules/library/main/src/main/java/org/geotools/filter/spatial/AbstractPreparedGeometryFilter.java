 ...
 * </p>
 * <ul>
 * <li><strong>NEITHER</strong> ({@link Literals#NEITHER) of the expressions are literal and is a
 * JTS Geometry (also non-null)</li>
 * <li><strong>BOTH</strong> ({@link Literals#BOTH}) expressions are literals and is a JTS 
 * Geometry (also non-null)</li>
 * <li>the <strong>LEFT</strong> ({@link Literals#LEFT}) expression (Expression1) is a literal 
 * and is a JTS Geometry (also non-null)</li>
 * <li>or the <strong>RIGHT</strong> ({@link Literals#RIGHT}) expression (Expression2) is a 
 * literal and is a JTS Geometry (also non-null)</li>
 * </ul>
 * <p>
 * If <strong>BOTH</strong> of the expressions are literals then a cached value is generated by 
 * calling {@link #basicEvaluate(Geometry, Geometry)}.
 * </p>
 * <p>
 * The method {@link #basicEvaluate(Geometry, Geometry)} is required to be implemented so that a 
 * cached value can be generated in the case
...
public abstract class AbstractPreparedGeometryFilter extends GeometryFilterImpl {
    ...
    protected enum Literals {
        ...
        private static Literals calculate(Expression expression1,
            ...
            boolean left = expression1 instanceof Literal && ((Literal) expression1).getValue() 
                    instanceof Geometry;
            boolean right = expression2 instanceof Literal && ((Literal) expression2).getValue() 
                    instanceof Geometry;
            if (left && right) {
                return BOTH;
            }
    ...
    protected AbstractPreparedGeometryFilter(Expression e1, Expression e2, MatchAction 
            matchAction) {
     ...
     * in the case that both geometries are Literals.  But in practice it is useful to extract 
     * this functionality
     * into its own method.
     *
