...
public class WKTWriter2 extends WKTWriter {
  ...
  /**
   * Creates a writer that writes {@link Geometry}s with the given output dimension (2 or 3). If the
   * specified output dimension is 3, the Z value of coordinates will be written if it is present
  ...
  /**
   * Converts a <code>Coordinate</code> to <code>&lt;Point&gt;</code> format, then appends it to the
   * writer.
   *
   * @param coordinate the <code>Coordinate</code> to process
   * @param writer the output writer to append to
  ...
  private void appendControlPointText(
      SingleCurvedGeometry cg, int level, boolean doIndent, Writer writer) throws IOException {
