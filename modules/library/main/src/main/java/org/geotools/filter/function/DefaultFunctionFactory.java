...
/**
 * Filter function factory that uses the service provider interface (SPI) lookup mechanism to 
 * create functions.
 * <p>
 * DefaultFunctionFactory is responsible for collection {@link FunctionExpression} 
...
//            }
    ...
    private FunctionName getFunctionName(Function function) {
        ...
        if (functionName == null) {
            ...
            if (!functionName.getName().equals(name)) {
                LOGGER.warning(function.getClass() + " FunctionName was null, used for " +
                        "etArgumentCount(): " + functionName);
            }
        } else {
            if (!functionName.getName().equals(name)) {
                LOGGER.warning(function.getClass() + " has name conflict betwee '" + name + "' " +
                        "and '" + functionName.getName() + "'");
            }
        }
        return functionName;
    }
    ...
    private Map<Name, FunctionDescriptor> loadFunctions() {
        ...
        for (Iterator<Function> i = functions.iterator(); i.hasNext(); ) {
            ...
            FunctionDescriptor fd = new FunctionDescriptor(functionName, (Class<Function>) 
                    function.getClass());
            ...
            if (functionMap.containsKey(key)) {
                ...
                LOGGER.warning("Function " + key + " clash between " + conflict.clazz
                        .getSimpleName() + " and " + function.getClass().getSimpleName());
            }
    ...
    static class FunctionDescriptor {
        ...
        Function newFunction(List<Expression> parameters, Literal fallback) throws Exception {
            ...
            Constructor<Function> constructor = clazz.getConstructor(new Class[]{List.class, 
                    Literal.class});
