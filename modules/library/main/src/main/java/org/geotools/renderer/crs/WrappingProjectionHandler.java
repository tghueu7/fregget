...
public class WrappingProjectionHandler extends ProjectionHandler {
  ...
  /**
   * Provides the strategy with the area we want to render and its CRS (the SPI lookup will do this
   * step)
   *
  ...
  public Geometry postProcess(MathTransform mt, Geometry geometry) {
    ...
    if (northEast) {
      ...
      lowLimit =
          Math.max(
              renderingEnvelope.getMinY(), renderingEnvelope.getMedian(1) - maxWraps * radius * 2);
      highLimit =
          Math.min(
              renderingEnvelope.getMaxY(), renderingEnvelope.getMedian(1) + maxWraps * radius * 2);
    } else {
      base = env.getMinX();
      curr = env.getMinX();
  ...
  /**
   * Adds the geometries into the collection by recursively splitting apart geometry collections, so
   * that geoms will contains only simple geometries.
   *
   * @param geoms
   * @param geometry
   * @param geomType
