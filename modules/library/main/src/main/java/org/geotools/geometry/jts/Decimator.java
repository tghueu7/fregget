...
public final class Decimator {
   ...
   * least pixelDistance away from each other when painted on the screen. Set pixelDistance to 0 if
   * you don't want any generalization (but just a transformation)
   *
   * @param screenToWorld
   * @param paintArea
   * @param pixelDistance
  ...
  /**
   * djb - noticed that the old way of finding out the decimation is based on the (0,0) location of
   * the image. This is often wildly unrepresentitive of the scale of the entire map.
   *
   * <p>A better thing to do is to decimate this on a per-shape basis (and use the shape's center).
   * Another option would be to sample the image at different locations (say 9) and choose the
   * smallest spanx/spany you find.
   *
   * <p>Also, if the xform is an affine Xform, you can be a bit more aggressive in the decimation.
   * If its not an affine xform (ie. its actually doing a CRS xform), you may find this is a bit too
   * aggressive due to any number of mathematical issues.
   *
   * <p>This is just a simple method that uses the centre of the given rectangle instead of (0,0).
   *
   * <p>NOTE: this could need more work based on CRS, but the rectangle is in pixels so it should be
   * fairly immune to all but crazy projections.
   *
   * @param screenToWorld
   * @param paintArea
   */
  ...
  public static double[] computeGeneralizationDistances(
    ...
    try {
      ...
      // search over a simple 3x3 grid for higher spans so that we perform a basic sampling of the
      // whole area
      ...
      // if we can't transform we went way out of the area of definition for the transform -> don't
      // generalize
  ...
  /**
   * @throws TransformException
   * @deprecated use the other constructor (with rectange) see javadox. This works fine, but it the
   *     results are often poor if you're also doing CRS xforms.
   */
  ...
  private final void decimateTransformGeneralize(
    ...
    if (DP_THRESHOLD > 0 && actualCoords > DP_THRESHOLD) {
      actualCoords =
          dpBasedGeneralize(actualCoords, coords, Math.min(spanx, spany) * Math.min(spanx, spany));
    }
