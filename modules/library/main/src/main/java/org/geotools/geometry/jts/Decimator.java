...
public final class Decimator {
    ...
    private static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger(Decimator
            .class);
    ...
    public Decimator(MathTransform screenToWorld, Rectangle paintArea, double pixelDistance) {
        if (screenToWorld != null && pixelDistance > 0) {
            try {
                ...
                throw new RuntimeException("Could not perform the generalization spans " +
                        "computation", e);
            }
        } else {
            this.spanx = 1;
     ...
     */
    public static double[] computeGeneralizationDistances(MathTransform screenToWorld, Rectangle 
            paintArea, double pixelDistance)
            throws TransformException {
        try {
            // init the spans with the upper left corner
            double[] spans = getGeneralizationSpans(paintArea.x, paintArea.y, screenToWorld);
            // search over a simple 3x3 grid for higher spans so that we perform a basic sampling
            // of the whole area
     ...
     * @throws TransformException
     */
    static double[] getGeneralizationSpans(double x, double y, MathTransform transform) throws 
            TransformException {
    ...
    public final Geometry decimateTransformGeneralize(Geometry geometry,
                                                      MathTransform transform) throws 
            TransformException {
    ...
    private final void decimateTransformGeneralize(LiteCoordinateSequence seq,
                                                   MathTransform transform, boolean ring, double 
                                                           spanx, double spany)
            throws TransformException {
        ...
        if (DP_THRESHOLD > 0 && actualCoords > DP_THRESHOLD) {
            actualCoords = dpBasedGeneralize(actualCoords, coords, Math.min(spanx, spany) * Math
                    .min(spanx, spany));
        }
    ...
    private int dpBasedGeneralize(int ncoords, double[] coords, double maxDistance) {
        while (coords[0] == coords[(ncoords - 1) * 2] && coords[1] == coords[2 * ncoords - 1] && 
                ncoords > 0) {
            ncoords--;
        }
        if (ncoords == 0) {
    ...
    private void dpSimplifySection(int first, int last, double[] coords, double 
            maxDistanceSquared) {
        if (last - 1 <= first) {
            return;
        }
