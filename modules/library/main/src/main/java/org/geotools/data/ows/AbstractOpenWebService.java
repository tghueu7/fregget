...
public abstract class AbstractOpenWebService<C extends Capabilities, R extends Object> {
   ...
   *   <li><b>3a) </b> If the client does not understand the new version number sent by the server,
   *       it may either cease communicating with the server or send a new request with a new
   *       version number that the client does understand but which is less than that sent by the
   *       server (if the server had responded with a lower version).
   *   <li><b>3b) </b> If the server had responded with a higher version (because the request was
   *       for a version lower than any known to the server), and the client does not understand the
   ...
   *
   * <p>Also, if we are unable to parse a given version for some reason, for example, malformed XML,
   * we will request a lower version until we have run out of versions to request with. Thus, a
   * server that does not play nicely may take some time to parse and might not even succeed.
   *
  ...
  protected C negotiateVersion() throws IOException, ServiceException {
    ...
    if (exception != null) {
      IOException e =
          new IOException(
              "Could not establish version neogitation: " + exception.getMessage(), exception);
      throw e;
    } else {
      throw new ServiceException(
          "Version negotiation unable to retrieve or parse Capabilities document.");
