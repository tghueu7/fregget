 ...
 * specification, available at <a href="http://www.opengeospatial.org">http://www.opengeospatial
 * .org</a>.
 * <p>
...
public abstract class AbstractOpenWebService<C extends Capabilities, R extends Object> {
     ...
     */
    public AbstractOpenWebService(C capabilties, URL serverURL) throws ServiceException, 
            IOException {
    ...
    public AbstractOpenWebService(final URL serverURL, final HTTPClient httpClient,
                                  final C capabilities, Map<String, Object> hints) throws 
        ...
        if (capabilities == null) {
            ...
            if (this.capabilities == null) {
                throw new ServiceException("Version negotiation unable to retrieve or parse " +
     ...
     * <ul>
     * <li><b>1) </b> If the server implements the requested version number, the server shall 
     * send that version.</li>
     * <li><b>2a) </b> If a version unknown to the server is requested, the server shall send the
     * highest version less
     * than the requested version.</li>
     * <li><b>2b) </b> If the client request is for a version lower than any of those known to 
     * the server, then the
     ...
     * server, it may either cease
     * communicating with the server or send a new request with a new version number that the 
     * client does understand but
     * which is less than that sent by the server (if the server had responded with a lower 
     * version).</li>
     * <li><b>3b) </b> If the server had responded with a higher version (because the request was
     * for a version lower
     * than any known to the server), and the client does not understand the proposed higher 
     * version, then the client
     ...
     * @throws IOException      if there is an error communicating with the server, or the XML 
     * cannot be parsed
    ...
    protected C negotiateVersion() throws IOException, ServiceException {
        ...
        while ((minClient <= test) && (test <= maxClient)) {
            ...
            GetCapabilitiesRequest request = tempSpecification.createGetCapabilitiesRequest
                    (serverURL);
