...
public class GeometryClipper {
   ...
   *
   * <p>This attempts to do a normal clip(). If it fails, it will try to do more to ensure the clip
   * works properly.
   *
   * <ol>
   ...
   * @param ensureValid If false there is no guarantee the polygons returned will be valid according
   *     to JTS rules (but should still be good enough to be used for pure rendering)
   * @return
   */
   ...
   * Dam. It's more efficient Sutherland-Hodgman version, but produces redundent turning vertices at
   * the corners of the clip region. This can make rendering as a series of triangles very awkward,
   * but it's fine of your underlying graphics mechanism has a forgiving drawPolygon routine. This
   * algorithm comes from http://www.longsteve.com/fixmybugs/?p=359, under a "DO WHAT THE FUCK YOU
   * WANT TO PUBLIC LICENSE" (no kidding!)
   */
  private LinearRing polygonClip(LinearRing ring) {
    ...
    // Parameter values of same, they are in [0,1] if the intersections are inside the segment, < 0
    // or > 1 otherwise
    ...
    if (gc.getNumGeometries() == 1) {
      ...
      if (gc instanceof MultiPoint) {
        ...
        return gc.getFactory()
            .createMultiLineString((LineString[]) result.toArray(new LineString[result.size()]));
      } else if (gc instanceof MultiPolygon) {
        return gc.getFactory()
            .createMultiPolygon((Polygon[]) result.toArray(new Polygon[result.size()]));
      } else {
        return gc.getFactory()
            .createGeometryCollection((Geometry[]) result.toArray(new Geometry[result.size()]));
      }
    }
  }
