...
public class DataUtilities {
   ...
   * #isMatch(AttributeDescriptor, AttributeDescriptor)} method is used to quickly confirm that the
   * local name and java binding are compatible.
   *
   * <p>Namespace is not considered in this opperations. You may still need to reType to get the
   * correct namesapce, or reorder.
   *
   * <p>Please note this method will not result in a stable sort if used in a {@link Comparator} as
   * -1 is used to indicate incompatiblity (rather than simply "before").
   *
   ...
   * </ul>
   *
   * This function is used recusively for (in order to handle complext features) no attempt is made
   * to detect cycles at this time so your milage may vary.
   *
   ...
   *
   * <p>This is usually done for use by the renderer; allowing it to query the feature collection as
   * required.
   *
   ...
   * @throws IllegalArgumentException If the provided collection is inconsistent (perhaps containing
   *     mixed feature types)
   */
  ...
  public static SimpleFeatureType simple(final FeatureType featureType) throws DataSourceException {
    ...
    final List<String> ignoreList =
        Arrays.asList(
            new String[] {"location", "metaDataProperty", "description", "name", "boundedBy"});
   ...
   *
   * <p>Please keep the use of this class to a minimum; if you are expecting a
   * FeatureLocking<SimpleFeatureType,SimpleFeature> please make use of SimpleFeatureLocking if you
   * possibly can.
   *
   * @since 2.7
   */
   ...
   *
   * <p>This method is here to work around the fact that JTS Geometry requires a specific method to
      ...
      if (Geometry.class.isAssignableFrom(clazz)) {
        GeometryType at =
            new GeometryTypeImpl(
                new NameImpl(name), clazz, crs, false, false, Collections.EMPTY_LIST, null, null);
        return new GeometryDescriptorImpl(at, new NameImpl(name), 0, 1, nillable, null);
      } else {
        AttributeType at =
            new AttributeTypeImpl(
                new NameImpl(name), clazz, false, false, Collections.EMPTY_LIST, null, null);
  ...
  /**
   * Manually count the number of features from the provided FeatureIterator. This implementation is
  ...
  /**
   * Manually calculate the bounds from the provided FeatureIteator. This implementation is intended
   * for FeatureCollection implementors and test case verification. Client code should always call
      ...
      while (!progress.isCanceled() && iterator.hasNext()) {
        ...
        try {
          ...
          throw new IOException(
              "Problem with " + collection.getID() + " visiting " + fid + ":" + erp, erp);
        }
      }
    } finally {
      progress.complete();
   ...
   * @throws IllegalArgumentException in case the specified path doesn't rely on a existing/readable
   *     directory.
   */
  ...
  /**
   * Returns a {@link IOFileFilter} obtained by excluding from the first input filter argument, the
