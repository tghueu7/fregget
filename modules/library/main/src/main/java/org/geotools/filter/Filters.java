...
public class Filters {
  ...
  /**
   * Private implementation used to handle static methods. Because this is a private instance we do
   * not have to override setFilterFactory; nobody will be messing with the factory and breaking
   * things for everyone.
   *
  ...
  /**
   * Safe version of FilterFactory *and* that is willing to combine filter1 and filter2 correctly in
   * the even either of them is already an And filter.
   *
   * @param ff
   * @param filter1
   * @param filter2
  ...
  /**
   * Safe version of FilterFactory *or* that is willing to combine filter1 and filter2 correctly in
   * the even either of them is already an Or filter.
   *
   * @param ff
   * @param filter1
   * @param filter2
   ...
   * performing instanceof checks. p> This utility method for those upgrading to a newer version of
   * GeoTools, instance of checks are preferred as they will take into account new kinds of
  ...
  /**
   * Convert filter to a constant for use in switch statements. This is an alternative to performing
   * instanceof checks.
   *
   ...
   *
   * <p>This will work even with dynamic expression that would normally require a feature. It works
   * especially well when the Expression is a Literal literal (which is usually the case).
   *
   ...
   * it is reall a small, lax, query language and Java objects need a but of help getting through.
   *
   * <p>A couple notes:
   *
   * <ul>
   *   <li>Usual trick of reflection for a Constructors that supports a String parameter is used as
   *       a last ditch effort.
   ...
   *       last item is removed from an Or statement then Filter.EXCLUDE is return If the last item
   *       is removed from an And statement then Filter.INCLUDE is returned
   * </ul>
   *
   * @param baseFilter
   * @param targetFilter
      ...
      DuplicatingFilterVisitor remove =
          new DuplicatingFilterVisitor() {
            ...
            private List<Filter> children(
                BinaryLogicOperator filter, final Filter targetFilter, Object extraData) {
   ...
   *
   * <p>The returned list is a mutable copy that can be used with filter factory to construct a new
   * filter when you are ready. To make that explicit I am returning an ArrayList so it is clear
   * that the result can be modified.
   *
   * @param filter
   ...
   * Any other filters will return false.
   *
   * <p>This represents the space covered by a number of the search functions, if *all* is true this
   * function will recursively search for additional child filters beyond those directly avaialble
   * from your filter.
   *
   * <p>The returned list is a mutable copy that can be used with filter factory to construct a new
   * filter when you are ready. To make that explicit I am returning an ArrayList so it is clear
   ...
   *
   * @param filter
   * @param all true to recurse into the filter and retrieve all children; false to only return the
   *     top level children
   * @return are belong to us
   */
  ...
  /**
   * Find the first child-filter (or the base filter itself) that is of the given type and uses the
   * specified property.
   *
   * @param filter
