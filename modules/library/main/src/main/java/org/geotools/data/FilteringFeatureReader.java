...
public class FilteringFeatureReader<T extends FeatureType, F extends Feature>
   ...
   *
   * <p>Please don't call this method with Filter.INCLUDE or Filter.EXCLUDE (consider not filtering
   * and EmptyFeatureReader instead)
   *
  ...
  public FilteringFeatureReader(FeatureReader<T, F> featureReader, Filter filter) {
    ...
    this.filter =
        (Filter) filter.accept(new BindingFilterVisitor(featureReader.getFeatureType()), null);
    next = null;
  }
   ...
   * looking at the attributes needed for comparison, whereas doing filtering here means we have to
   * create an entire feature each time."</i>
   *
