...
public class FeatureListenerManager {
    ...
    Map<FeatureSource<? extends FeatureType, ? extends Feature>, EventListenerList> listenerMap =
            new WeakHashMap<FeatureSource<? extends FeatureType, ? extends Feature>, 
                    EventListenerList>();
     ...
     */
    public void addFeatureListener(FeatureSource<? extends FeatureType, ? extends Feature> 
                                           featureSource,
     ...
     * @param featureListener
     */
    public void removeFeatureListener(FeatureSource<? extends FeatureType, ? extends Feature> 
                                              featureSource,
                                      FeatureListener featureListener) {
     ...
     * @return
     */
    private EventListenerList eventListenerList(FeatureSource<? extends FeatureType, ? extends 
    ...
    public void cleanListenerList(FeatureSource<? extends FeatureType, ? extends Feature> 
                                          featureSource) {
        synchronized (listenerMap) {
     ...
     */
    Map<SimpleFeatureSource, FeatureListener[]> getListeners(String typeName, Transaction 
            transaction) {
        Map<SimpleFeatureSource, FeatureListener[]> map = new HashMap<SimpleFeatureSource, 
                FeatureListener[]>();
            ...
            for (Map.Entry entry : map.entrySet()) {
                ...
                for (FeatureListener listener : listeners) {
                    try {
                        ...
                        LOGGER.log(Level.FINE, "Could not deliver " + event + " to " + listener +
                                ":" + t.getMessage(), t);
                    }
                }
            }
     ...
     * @param transaction
     */
    private void fireCommit(String typeName, Transaction transaction, int type, 
        ...
        for (Map.Entry entry : map.entrySet()) {
            featureSource = (FeatureSource<? extends FeatureType, ? extends Feature>) entry
                    .getKey();
     ...
     * @param bounds
     */
    private void fireEvent(String typeName, Transaction transaction, int type, ReferencedEnvelope
            bounds) {
        FeatureSource<? extends FeatureType, ? extends Feature> featureSource;
        FeatureListener[] listeners;
        FeatureEvent event;
