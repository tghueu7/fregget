...
public class FeatureListenerManager {
  ...
  /**
   * Returns a Map of FeatureListener[] by SimpleFeatureSource for all matches with featureType and
   * transaction.
   *
   ...
   * from an old API when {@link FeatureWriter#write()} didn't exist yet. It's a good idea though to
   * fire the event at FeatureWriter.write() instead of FeatureWriter.next() so there are actually
   * changes to notify for.
   *
      ...
      for (Map.Entry entry : map.entrySet()) {
        ...
        for (FeatureListener listener : listeners) {
          try {
            ...
            LOGGER.log(
                Level.FINE,
                "Could not deliver " + event + " to " + listener + ":" + t.getMessage(),
                t);
          }
        }
      }
   ...
   * @param typeName typeName being modified
   * @param transaction Transaction used for change
   * @param commit <code>true</code> for <code>commit</code>, <code>false</code> for <code>rollback
   *     </code>
   */
