...
public class FeatureTypes {
  ...
  static {
    ...
    try {
      featureType =
          FeatureTypes.newFeatureType(null, "Feature", new URI("http://www.opengis.net/gml"), true);
    } catch (Exception e) {
      // shold not happen
    }
   ...
   * knolwege.
   *
   * <p>This is a proper check, if the provided FeatureType matches the given namespace and typename
  ...
  /**
   * Exact equality based on typeNames, namespace, attributes and ancestors, including the user maps
   * contents
   */
  ...
  /**
   * Tolerant map comparison. Two maps are considered to be equal if they express the same content.
   * So for example two null maps are equal, but also a null and an empty one are
   */
