    ...
    public static org.opengis.filter.Filter parseFilter(Node root) {
        ...
        if (comparisions.containsKey(childName)) {
            ...
            try {
                ...
                if (type == AbstractFilter.FID) {
                    ...
                    while (sibling != null) {
                        ...
                        if (sibling.getNodeType() == Node.ELEMENT_NODE) {
                            ...
                            if (fidElementName.indexOf(':') != -1) {
                                //the DOM parser wasnt properly set to handle namespaces...
                                fidElementName = fidElementName.substring(fidElementName.indexOf
                                        (':') + 1);
                            }
                    ...
                    for (int i = 0; i < kids.getLength(); i++) {
                        ...
                        String kidName = (kid.getLocalName() != null) ? kid.getLocalName() : kid
                                .getNodeName();
                    ...
                    for (int i = 0; i < map.getLength(); i++) {
                        ...
                        String res = (kid.getLocalName() != null) ? kid.getLocalName() : kid
                                .getNodeName();
                        if (res.indexOf(':') != -1) {
                            //the DOM parser wasnt properly set to handle namespaces...
            ...
            try {
                ...
                String valueName = (value.getLocalName() != null) ? value.getLocalName() : value
                        .getNodeName();
                ...
                switch (type) {
                    ...
                    case FilterType.GEOMETRY_DWITHIN:
                        ...
                        if (value == null) {
                            throw new IllegalFilterException("DWithin is missing the Distance " +
                                    "element");
                        }
                        ...
                        if (!"Distance".equals(nodeName)) {
                            throw new IllegalFilterException("Parsing DWithin, was expecting to " +
                                    "find Distance but found " + value.getLocalName());
                        }
                    ...
                    case FilterType.GEOMETRY_BEYOND:
                        ...
                        if (value == null) {
                            throw new IllegalFilterException("Beyond is missing the Distance " +
                                    "element");
                        }
                        nodeName = value.getNodeName();
                        if (nodeName.indexOf(':') > 0) {
                        ...
                        if (!"Distance".equals(nodeName)) {
                            throw new IllegalFilterException("Parsing Beyond, was expecting to " +
                                    "find Distance but found " + value.getLocalName());
                        }
                        distance = Double.parseDouble(value.getTextContent());
                        if (value.getAttributes().getNamedItem("units") != null)
            ...
            try {
                List<org.opengis.filter.Filter> children = new ArrayList<org.opengis.filter
                        .Filter>();
                NodeList map = child.getChildNodes();
