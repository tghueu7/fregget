...
public class MultiRange<T extends Comparable<? super T>> {
    ...
    static final class RangeComparator<T extends Comparable<? super T>> implements 
            Comparator<Range<T>> {
    ...
    private boolean contiguous(Range r1, Range<T> r2) {
        if (r1.getMinValue() != null && r2.getMaxValue() != null && (r1.isMinIncluded() || 
                r2.isMaxIncluded())) {
            return r1.getMinValue().equals(r2.getMaxValue());
        } else if (r1.getMaxValue() != null && r2.getMinValue() != null && (r1.isMaxIncluded() ||
                r2.isMinIncluded())) {
    ...
    public Filter toFilter(FilterFactory ff, Expression variable) {
        ...
        } else if (ranges.size() == 1 && ranges.first().getMinValue() == null && ranges.first()
                .getMaxValue() == null) {
        ...
        for (int i = 0; i < rangeCount; ) {
            ...
            if (curr == range) {
                ...
                Range<T> union = new Range<T>(range.getElementClass(), (T) range.getMinValue(), 
                        range.isMinIncluded(),
    ...
    private Filter toFilter(FilterFactory ff, Expression variable, Range<T> range) {
        if (range.getMinValue() == null && range.getMaxValue() == null) {
            ...
            return ff.between(variable, ff.literal(range.getMinValue()), ff.literal(range
                    .getMaxValue()));
