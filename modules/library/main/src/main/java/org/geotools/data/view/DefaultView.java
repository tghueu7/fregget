...
public class DefaultView implements SimpleFeatureSource {
  ...
  public static SimpleFeatureSource create(SimpleFeatureSource source, Query query)
    ...
    if (source instanceof FeatureLocking) {
      //  return new GeoServerFeatureLocking((FeatureLocking<SimpleFeatureType, SimpleFeature>)
      // source,
   ...
   *
   * <p>Exposed attributes are those configured in the "attributes" element of the FeatureTypeInfo's
   * configuration
   *
    ...
    if (query.retrieveAllProperties()) {
      ...
      for (int i = 0; i < queriedAttCount; i++) {
        if (schema.getDescriptor(queriedAtts[i]) != null) {
          ...
          LOGGER.info(
              "queried a not allowed property: " + queriedAtts[i] + ". Ommitting it from query");
        }
      }
   ...
   *
   * <p>This method provides access to an optimized getBounds opperation. If no optimized opperation
   * is available <code>null</code> will be returned.
   *
   ...
   *
   * <p>This method provides access to an optimized getCount opperation. If no optimized opperation
