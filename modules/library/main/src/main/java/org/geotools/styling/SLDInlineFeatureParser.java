...
public class SLDInlineFeatureParser {
     ...
     **/
    private static Hashtable<Integer, CoordinateReferenceSystem> SRSLookup = new 
            Hashtable<Integer, CoordinateReferenceSystem>();
    ...
    public SLDInlineFeatureParser(Node root) throws Exception {
        ...
        if (featureType == null)
            throw new Exception("SLD InlineFeature Parser - couldnt determine a FeatureType.  See" +
                    " help for whats supported.");//shouldnt get here
    ...
    private void makeFeatures(Node root, boolean isCollection) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (child == null)
                throw new Exception("SLD inlineFeature Parser - couldnt extract a feature from " +
                        "the dom.");
     ...
     * @param featureType
     */
    private SimpleFeature parseFeature(Node feature, SimpleFeatureType featureType) throws 
            Exception {
    ...
    private Object getValue(Node root) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (child.getNodeType() == Node.TEXT_NODE) {
                strVal.append(child.getNodeValue());  //might get here multiple times -- see sax 
                // spec
            }
    ...
    private Geometry parseGeometry(Node root) throws Exception {
        ...
        ExpressionDOMParser parser = new ExpressionDOMParser(CommonFactoryFinder
    ...
    private boolean isSimple(Node root) throws Exception {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (childName.equalsIgnoreCase("FeatureCollection")) {
                ...
                if (!(childName.equalsIgnoreCase(featureName)))
                    throw new Exception("SLD inline feature parser  - it appear that there is >1 " +
                            "feature type present.  I got a " + childName + " when I was " +
                            "expecting a " + featureName + " tag");
            }
        }
        if (foundFC > 1)
            throw new Exception("SLD - UserLayer, inline feature parser - found >1 " +
                    "FeatureCollection.  Not supported");
        if ((foundFC > 0) && (foundFeature > 0))
            throw new Exception("SLD - UserLayer, inline feature parser - found  " +
                    "SimpleFeatureCollection and featureMembers.  Not supported");
        ...
        //otherwise decend into the SimpleFeatureCollection and check to make sure it only 
        // contains features
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (childName.equalsIgnoreCase("featureMember"))
                ...
                throw new Exception("SLD - UserLayer, inline feature parser - found a node of " +
                        "type FeatureCollection.  Expected a featureMember - dont support nested " +
                        "collections.");

            } else
                throw new Exception("SLD - UserLayer, inline feature parser - found a node of " +
                        "type '" + child.getLocalName() + "' and dont understand it.  Expected a " +
                        "featureMember.");
        }
    ...
    private void buildStore() {
        SimpleFeatureCollection collection = DataUtilities.collection(features.toArray(new 
                SimpleFeature[features.size()]));
     ...
     * 3. if that subtag contains a geometry, then it an attribute of type geometry, otherwise 
     * string
     * <p>
