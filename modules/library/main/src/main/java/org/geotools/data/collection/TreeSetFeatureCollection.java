...
public class TreeSetFeatureCollection implements SimpleFeatureCollection {
   ...
   * This constructor should not be used by client code.
   *
   * <p>Opportunistic reuse is encouraged, but only for the purposes of testing or other specialized
   * uses. Normal creation should occur through <code>
   * org.geotools.core.FeatureCollections.newCollection()</code> allowing applications to customize
   * any generated collections.
   *
   ...
   *
   * <p>Collections that support this operation may place limitations on what elements may be added
   * to this collection. In particular, some collections will refuse to add <tt>null</tt> elements,
  ...
  /**
   * Removes all of the elements from this collection (optional operation). This collection will be
   * empty after this method returns unless it throws an exception.
   */
  public void clear() {
   ...
   *
   * <p>If this collection fits in the specified array with room to spare (i.e., the array has more
   * elements than this collection), the element in the array immediately following the end of the
   * collection is set to <tt>null</tt>. This is useful in determining the length of this collection
   * <i>only</i> if the caller knows that this collection does not contain any <tt>null</tt>
   * elements.)
   *
   ...
   *
   * <p>Suppose <tt>l</tt> is a <tt>List</tt> known to contain only strings. The following code can
