        ...
        for (int i = 0; i < expr.size(); i++) {
            ...
            if (o == null) {
                if (expr.get(i).evaluate(obj) != null) {
                    //conversion error
                    throw new IllegalArgumentException(String.format("Failure converting value " +
                                    "for " +
                                    "argument %s. %s could not be converted to %s", arg.getName()
                            , obj.toString(),
                            arg.getType().getName()));
                }
            }
            if (prepped.containsKey(argName)) {
                if (arg.getMaxOccurs() == 1) {
                    throw new IllegalArgumentException(String.format("Multiple values specified " +
                            "for " +
