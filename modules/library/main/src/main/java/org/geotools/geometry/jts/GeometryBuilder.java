 ...
 *   Polygon polyhole = gb.polygon(gb.linearring(0,0, 0,10, 10,10, 10,0), gb.linearring(1,1, 1,9,
 *   9,9, 9,1))
 * </pre>
 *
...
public class GeometryBuilder {
    ...
    private CoordinateSequence createCS(double[] ord, int dim) {
        ...
        if (cs.getDimension() != dim) {
            ...
            throw new IllegalStateException("Unable to use" + csFact + " to produce " +
                    "CoordinateSequence with dimension " + dim);
        }
        for (int i = 0; i < n; i++) {
            for (int d = 0; d < dim; d++)
