...
public class CoordinateSequences extends com.vividsolutions.jts.geom.CoordinateSequences {
  ...
  public static boolean isCCW(CoordinateSequence ring) {
     ...
     *
     * <p>(1) is handled by checking if next is left of prev ==> CCW (2) will never happen if the
     * ring is valid, so don't check for it (Might want to assert this)
     */
    boolean isCCW = false;
    if (disc == 0) {
   ...
   * CoordinateArraySequence does not keep an accurate dimension - it always reports dim=3, even if
   * there is no Z ordinate (ie they are NaN). This method checks for that case and reports dim=2.
  ...
  /**
   * Returns true if the two geometries are equal in N dimensions (normal geometry equality is only
   * 2D)
   *
   * @param g1
   * @param g2
  ...
  public static boolean equalsND(Geometry g1, Geometry g2) {
    ...
    // ok, 2d equal, it means they have the same list of geometries and coordinate sequences, in the
    // same order
