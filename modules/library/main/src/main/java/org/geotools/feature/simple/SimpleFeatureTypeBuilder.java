...
public class SimpleFeatureTypeBuilder {
        ...
        if ((defaultGeometry != null && defaultGeometry.equals(name))
            ...
            if (!attributeBuilder.isCRSSet()) {
                if (defaultCrs == null && !defaultCrsSet) {
                    LOGGER.fine("Creating " + name + " with null CoordinateReferenceSystem - did " +
                            "you mean to setCRS?");
                }
    ...
    public AttributeDescriptor remove(String attributeName) {
        ...
        throw new IllegalArgumentException(attributeName + " is not an existing attribute " +
    ...
    public void set(AttributeDescriptor descriptor) {
        ...
        if (index == -1) {
            throw new IllegalArgumentException(descriptor.getLocalName() + " is not an existing " +
                    "attribute descriptor in this builder");
        }
            ...
            for (int i = 0; i < atts.size(); i++) {
                ...
                if (this.defaultGeometry.equals(att.getName().getLocalPart())) {
                    ...
                    if (!(att instanceof GeometryDescriptor)) {
                        LOGGER.warning("Default Geometry " + this.defaultGeometry + " was added " +
                                "as a geoemtry");
     ...
     */
    public static SimpleFeatureType retype(SimpleFeatureType original, CoordinateReferenceSystem 
            crs) {
        SimpleFeatureTypeBuilder b = new SimpleFeatureTypeBuilder();
