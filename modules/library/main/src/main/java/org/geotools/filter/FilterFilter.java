...
public class FilterFilter extends XMLFilterImpl implements GMLHandlerJTS {
  ...
  /**
   * collects element content on each call to {@link #characters(char[], int, int)} to be processed
   * by endElement
   */
  ...
  public void startElement(String namespaceURI, String localName, String qName, Attributes atts)
    ...
    if (localName.equals("Filter")) {
      ...
      // in all these cases, the correct (well, defined) thing to do is to ignore the tag - its
      // never used anywhere!
      if ((filterType == -1)
   ...
   * accumulated by the potentially many calls to {@link #characters(char[], int, int)} done by the
   * parser.
   *
   * @throws SAXException
   */
  ...
  /**
   * Converts the string representation of the expression to the AbstractFilter or DefaultExpression
   * short type.
   *
