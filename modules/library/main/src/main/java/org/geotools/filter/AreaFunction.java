...
public class AreaFunction extends FunctionExpressionImpl {
   ...
   * LineStrings the total line length is returned. GeometryCollections are iterated through so the
   * result is the sum of all Polygon and Line geometries anywhere within the collection. Any point
   * geometries return a value of 0;
   *
   * @param geometry The Geometry to calculate the area of.
   * @return The total area of the Geometry.
   */
  ...
  protected double getArea(Polygon polygon) {
    ...
    for (int i = 0; i < (numberOfExteriorRingCoordinates - 1); i++) {
      area +=
          (((exteriorRingCoordinates[i + 1].x - minx) - (exteriorRingCoordinates[i].x - minx))
              * (((exteriorRingCoordinates[i + 1].y - miny) + (exteriorRingCoordinates[i].y - miny))
                  / 2d));
    }
    area = Math.abs(area);
    ...
    for (int i = 0; i < numberOfInteriorRings; i++) {
      ...
      for (int j = 0; j < (numberOfInteriorRingCoordinates - 1); j++) {
        interiorArea +=
            (((interiorRingCoordinates[j + 1].x - minx) - (interiorRingCoordinates[j].x - minx))
