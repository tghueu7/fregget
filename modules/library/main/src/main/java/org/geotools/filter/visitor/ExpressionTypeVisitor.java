...
public class ExpressionTypeVisitor implements ExpressionVisitor {
      ...
      new HashMap<Class<?>, List<Class<?>>>() {
        {
          ...
          put(
              Long.class,
              Arrays.asList(
                  (Class<?>) Long.class, Double.class, BigInteger.class, BigDecimal.class));
  ...
  /**
   * Traverses the super-classes trying to find a common superclass. Won't consider interfaces (good
   * enough for the moment, all basic types we handle have a common superclass)
   *
   * @param c1
   * @param c2
   * @return
