 ...
 * An offset curve is not the same as a buffer, the line
 * is built only on one side of the original geometry, and will self intersect if the original 
...
public class OffsetCurveBuilder {
    ...
    /**
     * Creates an offset builder with the given offset and the default number of quadrant 
     * segments {@link QUADRANT_SEGMENTS_DEFAULT}
     *
     * @param offset The offset distance. The offset line will be on the left for positive 
     *               values, on the right otherwise
     */
     ...
     *
     * @param offset           The offset distance. The offset line will be on the left for 
     *                         positive values, on the right otherwise
    ...
    public OffsetCurveBuilder(double offset, int quadrantSegments) {
        ...
        if (quadrantSegments < 1) {
            throw new IllegalArgumentException("Invalid number of quadrantSegments, must be " +
                    "greater than zero: " + quadrantSegments);
        }
     ...
     * input geometries will be subject to offset, the
     * returned geometry will be either a LineString or a MultiLineString
     *
     * @param g The source geometry
     * @return The offset geometry (a single linestring for single line inputs, a 
     * multi-linestring for multi-line inputs),
    ...
    private GrowableOrdinateArray cleanupOrdinates(GrowableOrdinateArray ordinates, boolean 
            closed) {
        ...
        for (int i = 2; i < (max - 3); i += 2) {
            ...
            for (int j = i + 4; j < (max - 1); j += 2) {
                ...
                if (squaredDistance(c1, c3) > maxSearchDistanceSquared && squaredDistance(c1, c4)
                        > maxSearchDistanceSquared &&
                        squaredDistance(c2, c3) > maxSearchDistanceSquared && squaredDistance(c2,
                        c4) > maxSearchDistanceSquared) {
    ...
    private double computeJointAngle(double dx10, double dy10, double dx12, double dy12) {
        // see http://stackoverflow.com/questions/21483999/using-atan2-to-find-angle-between-two
        // -vectors
    ...
    /**
     * Appends to ordinates a point calculated as the perpendicular offset to the specified 
     * angle, from location x,y
     */
    ...
    private void appendInternalJoint(double x, double y, double angle01, double angle12,
                                     double dx10, double dy10, double dx12, double dy12, 
                                     GrowableOrdinateArray ordinates) {
        ...
        double maxAllowedDistanceSquared = Math.max(maxSearchDistanceSquared, Math.max
                (squaredDistance(dx10, dy10), squaredDistance(dx12, dy12)));
