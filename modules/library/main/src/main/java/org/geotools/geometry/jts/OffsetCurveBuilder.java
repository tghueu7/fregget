...
public class OffsetCurveBuilder {
   ...
   *
   * @param offset The offset distance. The offset line will be on the left for positive values, on
   *     the right otherwise
   */
  ...
  /**
   * Builds and return the perpendicular offset geometry. Only the linestring elements in the input
   * geometries will be subject to offset, the returned geometry will be either a LineString or a
   * MultiLineString
   *
   * @param g The source geometry
   * @return The offset geometry (a single linestring for single line inputs, a multi-linestring for
   *     multi-line inputs), or null if no offset geometry could be computed (e.g. the input
  ...
  private void addBulge(
      GrowableOrdinateArray ordinates, double c1x, double c1y, double angle10, double angle12) {
  ...
  /**
   * Appends to ordinates a point calculated as being the joining point of two segments with angles
   * angle01, angle02 and starting in x,y
   *
   * @param dx10
   * @param dy10
   * @param dx12
