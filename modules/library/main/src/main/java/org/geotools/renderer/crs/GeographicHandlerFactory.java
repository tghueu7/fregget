...
public class GeographicHandlerFactory implements ProjectionHandlerFactory {
    ...
    public ProjectionHandler getHandler(ReferencedEnvelope renderingEnvelope, 
        ...
        if (renderingEnvelope != null && crs instanceof GeographicCRS) {
            ...
            if (horizontalSourceCrs instanceof GeographicCRS && !CRS.equalsIgnoreMetadata
                    (horizontalSourceCrs, geogCrs)) {
                ...
                if (!CRS.equalsIgnoreMetadata(horizontalSourceCrs, geogCrs)) {
                    if (CRS.getAxisOrder(sourceCrs) == AxisOrder.NORTH_EAST) {
                        validArea = new ReferencedEnvelope(MIN_LATITUDE, MAX_LATITUDE, Float
                                .MAX_VALUE, -Float.MAX_VALUE, horizontalSourceCrs);
                    } else {
                        validArea = new ReferencedEnvelope(Float.MAX_VALUE, -Float.MAX_VALUE, 
                                MIN_LATITUDE, MAX_LATITUDE, horizontalSourceCrs);
                    }
                }
            }

            if (wrap && maxWraps > 0) {
                double centralMeridian = geogCrs.getDatum().getPrimeMeridian()
                ...
                handler.setDatelineWrappingCheckEnabled(!isWrappingException(crs, 
                        horizontalSourceCrs));
                return handler;
            } else {
