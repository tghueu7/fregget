...
public final class DataAccessFinder {
  ...
  static DataAccess<? extends FeatureType, ? extends Feature> getDataStore(
    ...
    while (ps.hasNext()) {
      ...
      try {
        ...
        LOGGER.log(
            Level.WARNING,
            "Problem asking " + fac.getDisplayName() + " if it can process request:" + t,
            t);
      ...
      if (canProcess) {
        ...
        try {
          ...
          LOGGER.log(
              Level.WARNING,
              "Difficulity checking if " + fac.getDisplayName() + " is available:" + t,
              t);
          // Protect against DataStores that don't carefully code
          // isAvailable
          continue;
  ...
  /**
   * Finds all implemtaions of DataAccessFactory which have registered using the services mechanism,
