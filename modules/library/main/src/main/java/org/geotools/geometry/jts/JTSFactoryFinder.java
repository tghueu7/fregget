...
public class JTSFactoryFinder extends FactoryFinder {
  ...
  /**
   * Returns the first implementation of {@link GeometryFactory} matching the specified hints. If no
  ...
  /**
   * Returns the first implementation of {@link PrecisionModel} matching the specified hints. If no
   * implementation matches, a new one is created if possible or an exception is thrown otherwise.
   *
   * @param hints An optional map of hints, or {@code null} if none.
   ...
   * {@link ServiceRegistry} for geotools object. However, JTS factories are an other story because
   * they don't know anything about the Geotools's factory plugin system. Consequently we need to
   * process JTS factories in a special way.
   */
  private static final class Registry extends FactoryCreator {
    ...
    public Registry() {
      super(
          Arrays.asList(
              new Class<?>[] {
                GeometryFactory.class, PrecisionModel.class, CoordinateSequenceFactory.class
              }));
    }
    ...
    protected Object createServiceProvider(
      ...
      if (GeometryFactory.class.isAssignableFrom(category)
        ...
        return new GeometryFactory(
            getPrecisionModel(hints), getSRID(hints), getCoordinateSequenceFactory(hints));
      }
     ...
     * to meets standard Geotools requirements.
     *
