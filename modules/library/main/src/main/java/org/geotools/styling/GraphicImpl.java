...
public class GraphicImpl implements Graphic, Cloneable {
   ...
   * only be used if no ExternalGraphic is provided, or if none of the external graphics formats are
   * supported.
   *
   * @return An array of marks to use when displaying this Graphic. By default, a "square" with 50%
   *     gray fill and black outline with a size of 6 pixels (unless a size is specified) is
   *     provided.
   */
  @Deprecated
   ...
   *
   * <p>This class operates as a "view" on getMarks() and getExternalGraphics() with the added magic
   * that if nothing has been set ever a single default MarkImpl will be provided. This default will
   * not effect the internal state it is only there as a sensible default for rendering.
   *
   ...
   * height and the corresponding aspect in width. If a size is specified, the height of the graphic
   * will be scaled to that size and the corresponding aspect will be used for the width.
   *
   ...
   *
   * <p>Two graphics are equal if and only if they both have the same geometry property name and the
   * same list of symbols and the same rotation, size and opacity.
   *
