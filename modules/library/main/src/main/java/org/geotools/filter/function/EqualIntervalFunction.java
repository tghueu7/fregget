...
public class EqualIntervalFunction extends ClassificationFunction {
  ...
  private RangedClassifier calculate(SimpleFeatureCollection featureCollection) {
    ...
    try {
      ...
      LOGGER.log(
          Level.SEVERE, "EqualIntervalFunction calculate(SimpleFeatureCollection) failed", e);
      return null;
  ...
  private RangedClassifier calculateNumerical(
    ...
    double slotWidth =
        (((Number) globalMax).doubleValue() - ((Number) globalMin).doubleValue()) / classNum;
    // size arrays
    ...
    for (int i = 0; i < classNum; i++) {
      ...
      localMax[i] =
          new Double(((Number) globalMax).doubleValue() - ((classNum - i - 1) * slotWidth));
      ...
      if (i == 0) {
        ...
        if (localMin[i].compareTo(new Double(((Number) globalMin).doubleValue())) < 0)
          localMin[i] =
              new Double(fixRound(((Number) localMin[i]).doubleValue(), decPlaces, false));
