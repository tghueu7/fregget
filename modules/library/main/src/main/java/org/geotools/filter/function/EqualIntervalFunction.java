...
public class EqualIntervalFunction extends ClassificationFunction {
    ...
    private RangedClassifier calculate(SimpleFeatureCollection featureCollection) {
        ...
        try {
            ...
            LOGGER.log(Level.SEVERE, "EqualIntervalFunction calculate(SimpleFeatureCollection) " +
                    "failed", e);
            return null;
    ...
    private RangedClassifier calculateNumerical(int classNum, Comparable globalMin, Comparable 
            globalMax) {
        double slotWidth = (((Number) globalMax).doubleValue() - ((Number) globalMin).doubleValue
                ()) / classNum;
        //size arrays
        ...
        for (int i = 0; i < classNum; i++) {
            ...
            localMax[i] = new Double(((Number) globalMax).doubleValue() - ((classNum - i - 1) * 
                    slotWidth));
            ...
            if (i == 0) {
                ...
                if (localMin[i].compareTo(new Double(((Number) globalMin).doubleValue())) < 0)
                    localMin[i] = new Double(fixRound(((Number) localMin[i]).doubleValue(), 
                            decPlaces, false));
                ...
                if (localMax[i].compareTo(new Double(((Number) globalMax).doubleValue())) > 0)
                    localMax[i] = new Double(fixRound(((Number) localMax[i]).doubleValue(), 
                            decPlaces, true));
            }
    ...
    private RangedClassifier calculateNonNumerical(int classNum, FeatureCollection<?, ?> 
        ...
        // this code currently implements option #2 (this is a quantile, why don't we use their 
        // code instead)
        ...
        for (int binIndex = 0; binIndex < classNum; binIndex++) {
            ...
            if (binIndex < localMin.length)
                localMin[binIndex] = (itemIndex < values.length ? values[itemIndex] : 
            ...
            if (binIndex == classNum - 1) {
                if (binIndex < localMax.length)
                    localMax[binIndex] = (itemIndex < values.length ? values[itemIndex] : 
                            values[values.length - 1]);
                else
                    localMax[localMax.length - 1] = (itemIndex < values.length ? 
                            values[itemIndex] : values[values.length - 1]);
            } else {
                if (binIndex < localMax.length)
                    localMax[binIndex] = (itemIndex + 1 < values.length ? values[itemIndex + 1] :
                            values[values.length - 1]);
                else
                    localMax[localMax.length - 1] = (itemIndex + 1 < values.length ? 
                            values[itemIndex + 1] : values[values.length - 1]);
            }
