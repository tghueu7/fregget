...
public class Types {
   ...
   * @throws IllegalAttributeException In the event that content violates any restrictions specified
   *     by the attribute.
   */
  ...
  public static Object parse(AttributeDescriptor descriptor, Object value)
    ...
    if (value == null) {
      ...
      if (!target.isAssignableFrom(value.getClass())) {
        ...
        //                else {
        //                    throw new IllegalArgumentException( descriptor.getLocalName()+ " could
        // not convert "+value+" into "+target);
        //                }
      }
    }
    return value;
  ...
  /**
   * FeatureType comparison indicating if the description provided by two FeatureTypes is similar to
   * the point data can be exchanged. This comparison is really very focused on the name / value
   * contract and is willing to overlook details like length restrictions.
   *
   ...
   *
   * <p>As an example databases traditionally support variable length strings with a limit of 32 k;
   * while a shapefile is limited to 256 characters. When working with data from both these data
   * sources you will need to make adjustments based on these abilities. If true is returned data
   * conforming to the expected FeatureType can be used with the actual FeatureType.
   *
   ...
   * another - specifically ensuring that the order / value is a reasonable match with the expected
   * number of attributes on each side and the values correctly assignable.
   *
   * <p>After assertOrderCovered returns without error the following code will work:
   *
   * <pre><code>
   ...
   * @return The name in which getLocalPart() == name and getNamespaceURI() == null. Or null if name
   *     == null.
   */
  ...
  /**
   * Takes a prefixed attribute name and returns an {@link Name} by looking which namespace belongs
   * the prefix to in {@link AppSchemaDataAccessDTO#getNamespaces()}.
   *
  ...
  public static List<PropertyDescriptor> descriptors(ComplexType type) {
    ...
    while (loopType != null) {
      ...
      loopType =
          loopType.getSuper() instanceof ComplexType ? (ComplexType) loopType.getSuper() : null;
    }
    return children;
  }
