...
public class LikeFilterImpl extends AbstractFilter implements PropertyIsLike {
   ...
   *
   * <p>NOTE; the SQL command is 'string LIKE pattern [ESCAPE escape-character]' We could re-define
   * the escape character, but I'm not doing to do that in this code since some databases will not
   * handle this case.
   *
   * <p>Method: 1.
   *
   ...
   *
   * <p>NOTE: we dont handle "'" as a 'special' character because it would be too confusing to have
   * a special char as another special char. Using this will throw an error
  ...
  public String getSQL92LikePattern() throws IllegalArgumentException {
    ...
    return LikeFilterImpl.convertToSQL92(
        escape.charAt(0), wildcardMulti.charAt(0), wildcardSingle.charAt(0), matchingCase, pattern);
  }
  ...
  /**
   * Compares this filter to the specified object. Returns true if the passed in object is the same
   * as this filter. Checks to make sure the filter types, the value, and the pattern are the same.
   * &
   *
  ...
  /**
   * Used by FilterVisitors to perform some action on this filter instance. Typicaly used by Filter
   * decoders, but may also be used by any thing which needs infomration from filter structure.
   * Implementations should always call: visitor.visit(this); It is importatant that this is not
