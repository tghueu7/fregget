...
public class GeometryTypeConverterFactory implements ConverterFactory {
    ...
    private static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (GeometryTypeConverterFactory.class);
    ...
    public Converter createConverter(Class<?> source, Class<?> target,
        ...
        if (Geometry.class.isAssignableFrom(source)
        ...
        } else if (Geometry.class.isAssignableFrom(source) && Geometry.class.isAssignableFrom
                (target)) {
            ...
            return new Converter() {
                 ...
                 * @throws Exception
                 */
                public <T> List<T> convertAll(GeometryCollection gc, Class<T> target) throws 
                        Exception {
                ...
                public Object convert(Object source, Class target)
                    ...
                    if (source instanceof Geometry) {
                        ...
                        // element
                        //  - if source is a GeometryCollection, we try to convert each element 
                        // to <geometry>
                        ...
                        if (MultiPoint.class.isAssignableFrom(target)) {
                            ...
                            if (sourceGeometry.isEmpty())
                                ...
                                points = this.convertAll((GeometryCollection) source, Point
                                        .class).toArray(new Point[]{});
                            else
                            ...
                            if (sourceGeometry.isEmpty())
                                ...
                                lineStrings = this.convertAll((GeometryCollection) source, 
                                        LineString.class).toArray(new LineString[]{});
                            else
                                lineStrings = new LineString[]{(LineString) this.convert(source, 
                                        LineString.class)};
                            ...
                            if (sourceGeometry.isEmpty())
                                ...
                                polygons = this.convertAll((GeometryCollection) source, Polygon
                                        .class).toArray(new Polygon[]{});
                            else
                                polygons = new Polygon[]{(Polygon) this.convert(source, Polygon
                                        .class)};
                        ...
                        // a new geometrycollection
                            ...
                            if (sourceGeometry.isEmpty())
                                ...
                                destGeometry = gFac.createGeometryCollection(new Geometry[]{
                                        (Geometry) source});
                        }
                        ...
                        // target is a point: we return the centroid of any complex geometry
                            ...
                            else if (source instanceof MultiPoint && sourceGeometry
                                    .getNumGeometries() == 1)
                                destGeometry = (Geometry) ((MultiPoint) source).getGeometryN(0)
                                        .clone();
                            else {
                                if (LOGGER.isLoggable(Level.FINE))
                                    LOGGER.fine("Converting Geometry " + source.toString() + " to" +
                                            " Point. This could be unsafe");
                        ...
                        // geometry coordinates
                            ...
                            else if (source instanceof MultiLineString && sourceGeometry
                                    .getNumGeometries() == 1)
                                destGeometry = (Geometry) ((MultiLineString) source).getGeometryN
                                        (0).clone();
                            else {
                                if (LOGGER.isLoggable(Level.FINE))
                                    LOGGER.fine("Converting Geometry " + source.toString() + " to" +
                                            " LineString. This could be unsafe");
                                destGeometry = gFac.createLineString(getLineStringCoordinates((
                                        (Geometry) source).getCoordinates()));
                            }
                        }
                        // target is a polygon: we return a polygon connecting all the 
                        // coordinates of the given geometry
                        else if (Polygon.class.isAssignableFrom(target)) {
                            // NC - Empty Geometry Support
                            if (sourceGeometry.isEmpty())
                                destGeometry = gFac.createLineString(new Coordinate[0]);
                            else if (source instanceof MultiPolygon && sourceGeometry
                                    .getNumGeometries() == 1)
                                destGeometry = (Geometry) ((MultiPolygon) source).getGeometryN(0)
                                        .clone();
                            else {
                                if (LOGGER.isLoggable(Level.FINE))
                                    LOGGER.fine("Converting Geometry " + source.toString() + " to" +
                                            " Polygon. This could be unsafe");
                                Coordinate[] coords = getPolygonCoordinates(((Geometry) source)
                                        .getCoordinates());
                                destGeometry = gFac.createPolygon(gFac.createLinearRing(coords), 
                                        new LinearRing[]{});
                            }
                        }
                ...
                private <T> T[] arrayCopy(T[] original, int length) {
                    ...
                    System.arraycopy(original, 0, copy, 0, original.length < length ? original
                            .length : length);
                    return copy;
                }
                ...
                private Coordinate[] getPolygonCoordinates(
                    ...
                    if (!coordinates[coordinates.length - 1].equals(coordinates[0])) {
                        Coordinate[] newCoordinates = arrayCopy(coordinates, coordinates.length +
                                1);
