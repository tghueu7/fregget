...
public abstract class LogicFilterImpl extends BinaryLogicAbstract {
   ...
   * as this filter. Checks to make sure the filter types are the same, and then checks that the
   * subFilters lists are the same size and that one list contains the other. This means that logic
   * filters with different internal orders of subfilters are equal.
   *
