...
public final class LiteShape2 implements Shape, Cloneable {
   ...
   * @param mathTransform - the transformation applied to the geometry in order to get to the shape
   *     points
   * @param decimator -
   ...
   * </ul>
   *
   * This means that this method might return <code>false</code> even though the <code>Shape</code>
   * contains the <code>Rectangle2D</code>. The <code>Area</code> class can be used to perform more
   * accurate computations of geometric intersection for any <code>Shape</code> object if a more
   ...
   *     contains the <code>Rectangle2D</code> and the <code>intersects</code> method returns <code>
   *     true</code> and the containment calculations would be too expensive to perform.
   ...
   * </code> for the entire rectanglar area to be considered contained within the <code>Shape</code>
   * .
   *
   ...
   *
   * This means that this method might return <code>false</code> even though the <code>Shape</code>
   * contains the rectangular area. The <code>Area</code> class can be used to perform more accurate
   * computations of geometric intersection for any <code>Shape</code> object if a more precise
   * answer is required.
   *
  ...
  /**
   * Returns an integer {@link Rectangle}that completely encloses the <code>Shape</code>. Note that
   * there is no guarantee that the returned <code>Rectangle</code> is the smallest bounding box
   * that encloses the <code>Shape</code>, only that the <code>Shape</code> lies entirely within the
   * indicated <code>Rectangle</code>. The returned <code>Rectangle</code> might also fail to
   * completely enclose the <code>Shape</code> if the <code>Shape</code> overflows the limited range
   ...
   * box returned by this method is usually tighter than that returned by the <code>getBounds</code>
   * method and never fails due to overflow problems since the return value can be an instance of
   ...
   * accordingly.
   *
   * <p>Each call to this method returns a fresh <code>PathIterator</code> object that traverses the
   * geometry of the <code>Shape</code> object independently from any other <code>PathIterator
   ...
   * rectangular area. The rectangular area is considered to intersect the <code>Shape</code> if any
   * point is contained in both the interior of the <code>Shape</code> and the specified rectangular
   * area.
   *
   ...
   * does not intersect the <code>Shape</code>. The {@link java.awt.geom.Area Area}class can be used
   * to perform more accurate computations of geometric intersection for any <code>Shape</code>
