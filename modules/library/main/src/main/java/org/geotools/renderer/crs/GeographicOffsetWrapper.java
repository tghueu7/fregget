...
class GeographicOffsetWrapper implements MathTransform {
    ...
    public void transform(double[] srcPts, int srcOff, double[] dstPts, int dstOff, int numPts)
        ...
        double offset = 0;
        for (int i = srcOff + srcDimension, j = dstOff + dstDimension, ptCount = 1; ptCount < 
                numPts; i += srcDimension, j += dstDimension, ptCount++) {
    ...
    public void transform(float[] srcPts, int srcOff, float[] dstPts, int dstOff, int numPts)
        ...
        double lastDstLon = dstPts[dstOff];
        float offset = 0;
        for (int i = srcOff + dimension, j = dstOff + dimension, ptCount = 0; ptCount < numPts; i
                += dimension, j += dimension, ptCount++) {
