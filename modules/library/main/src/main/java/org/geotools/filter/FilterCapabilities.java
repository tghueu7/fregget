...
public class FilterCapabilities {
  ...
  /**
   * Adds a new support type to capabilities. For 2.2 only function expression support is added this
   * way. As of geotools 2.3 this will be the supported way of adding to Filtercapabilities.
   *
   ...
   * nothing should be filtered than Filter.INCLUDE can be used. Embedded nulls can be a particular
   * source of problems, buried in logic filters.
   *
   * @param filter the filter to be tested.
   * @return true if all sub filters are supported, false otherwise.
   * @throws IllegalArgumentException If a null filter is passed in. As this function is recursive a
   *     null in a logic filter will also cause an error.
   */
