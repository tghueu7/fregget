 ...
 * DJB: okay, there's no where near enough comments in here to understand whats going on.  
 ...
 * Basically, the FilterFilter does most of the Filter parsing - but it hands most of the work 
 * off to the
 ...
 * </Filter>
 * <p>
 * The "PropertyIsEqualTo" is handled by another parser, so we dont have to worry about it here 
 * for the moment.
 * <p>
 ...
 * end ("OR")
 * create()                   // this creates an actual Filter [[ STATE_NAME = NY ] OR [ 
 * STATE_NAME = WA ]]
 * <p>
 ...
 * add([STATE_NAME = 'NY']) -- this is sent to the "AND" parser.  It then sends it to the "OR" 
 * parser.
 ...
 * "OR".create() -- makes a "[[ STATE_NAME = NY ] OR [ STATE_NAME = WA ]]" and "AND" remembers it
 * as a component
 * add ([ STATE_NAME = BC ]) --This is added as a component to the "AND" filter.
 * end ("AND")   --  the "AND" parser marks itself as complete
 * create()      --  the "AND" parser creates a FILTER [[[ STATE_NAME = NY ] OR [ STATE_NAME = WA
 * ]] AND [ STATE_NAME = BC ]]
 * <p>
 * <p>
 * Higher levels of nesting work the same way - each level will send the event down to the next 
 * level.
 * <p>
