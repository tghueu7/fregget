...
public class LogicSAXParser {
  ...
  /**
   * To be called by a parser to start the creation of a logic filter. Can start a nested or a base
   * logic filter.
   *
  ...
  public void start(short logicType) throws IllegalFilterException {
    ...
    if (this.logicType != -1) {
      // DJB: for GEOS-328 we need to keep the old parser around to handle multiple nestings of
      // logic operators.
