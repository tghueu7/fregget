...
public class SLDStyleTest extends TestCase {
  ...
  public void testParseAnchorDisplacement() throws IOException {
    ...
    Style[] styles = stylereader.readXML();
    PointSymbolizer ps =
        (PointSymbolizer) styles[0].featureTypeStyles().get(0).rules().get(0).getSymbolizers()[0];
  ...
  public void testParseRasterSymbolizerColorMap() throws IOException {
    ...
    String[] colors =
        new String[] {
          "#00ff00", "#00fa00", "#14f500", "#28f502", "#3cf505", "#50f50a", "#64f014", "#7deb32",
          "#78c818", "#38840c", "#2c4b04", "#ffff00", "#dcdc00", "#b47800", "#c85000", "#be4100",
          "#963000", "#3c0200", "#ffffff", "#ffffff"
        };
    int[] values =
        new int[] {
          -500, -417, -333, -250, -167, -83, -1, 0, 30, 105, 300, 400, 700, 1200, 1400, 1600, 2000,
          3000, 5000, 13000
        };
  ...
  private static void assertImagesEqual(BufferedImage expected, Icon icon) {
    ...
    for (int x = 0; x < w; ++x) {
      for (int y = 0; y < h; ++y) {
        assertEquals(
            "mismatch at (" + x + ", " + y + ")", actual.getRGB(x, y), expected.getRGB(x, y));
      }
    }
  }
}
