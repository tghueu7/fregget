...
public class DataUtilitiesTest extends DataTestCase {
    ...
    public void testSimpleCollectionList() {
        SimpleFeatureCollection featureCollection = DataUtilities.collection(Arrays.asList
                (roadFeatures));
        assertEquals(roadFeatures.length, featureCollection.size());
    }
    ...
    public void testCreate() throws Exception {
        SimpleFeatureType featureType = DataUtilities.createType("Contact", "id:Integer," +
                "party:String,geom:Geometry:srid=4326");
        SimpleFeature feature1 = DataUtilities.createFeature(featureType, "fid1=1|Jody " +
    ...
    public void testCreateSubType() throws Exception {
        ...
        // check that subtyping does not cause the geometry attribute structure to change
        before = DataUtilities.createType("cities", "the_geom:Point:srid=4326,name:String," +
                "population:Integer");
        after = DataUtilities.createSubType(before, new String[]{"the_geom"});
        assertEquals(before.getGeometryDescriptor(), after.getGeometryDescriptor());
    }
    ...
    public void testCreateSubTypePreservesDefaultGeometryProperty() throws Exception {
        ...
        SimpleFeatureType after = DataUtilities.createSubType(before, new String[]{"name", 
                "the_geom1", "the_geom3"});
        assertEquals(3, after.getAttributeCount());
        assertEquals("the_geom1", after.getGeometryDescriptor().getLocalName());

        after = DataUtilities.createSubType(before, new String[]{"name", "the_geom1", 
                "the_geom3"}, DefaultGeographicCRS.WGS84);
        assertEquals(3, after.getAttributeCount());
        assertEquals("the_geom1", after.getGeometryDescriptor().getLocalName());
        ...
        after = DataUtilities.createSubType(before, new String[]{"name", "the_geom1", 
                "the_geom2"}, DefaultGeographicCRS.WGS84);
        assertEquals(3, after.getAttributeCount());
        assertEquals("the_geom2", after.getGeometryDescriptor().getLocalName());
    }
    ...
    public void testCreateSubTypeWithPropertyNotMatchingAnAttributeDescriptor() throws Exception {
        ...
        SimpleFeatureType before = DataUtilities.createType("cities", "the_geom:Point:srid=4326," +
                "name:String");
        SimpleFeatureType after = DataUtilities.createSubType(before, new String[]{"the_geom", 
                "name", "not_existing"});
    ...
    public void testSpecNotIdentifiable() throws Exception {
        ...
        CoordinateReferenceSystem crsNoId = CRS
                .parseWKT("PROJCS[\"Geoscience Australia Standard National Scale Lambert " +
                        "Projection\",GEOGCS[\"WGS 84\",DATUM[\"WGS_1984\",SPHEROID[\"WGS_1978\"," +
                        "6378135,298.26],TOWGS84[0,0,0]],PRIMEM[\"Greenwich\",0]," +
                        "UNIT[\"Decimal_Degree\",0.0174532925199433]]," +
                        "PROJECTION[\"Lambert_Conformal_Conic_2SP\"]," +
                        "PARAMETER[\"central_meridian\",134.0],PARAMETER[\"latitude_of_origin\"," +
                        "0.0],PARAMETER[\"standard_parallel_1\",-18.0]," +
                        "PARAMETER[\"standard_parallel_2\",-36.0],UNIT[\"Meter\",1]]");
