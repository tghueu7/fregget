...
public class FilterFilterTest extends TestCase {
    ...
    public static void testWithoutFunction() throws Exception {
        String filter = "<wfs:GetFeature service=\"WFS\" version=\"1.0.0\" " +
                ...
                "\">-99.79800943339099,30.41833858217994 -99.79800943339099,30.71813913408305 " +
                "-99.49820888148788,30.71813913408305 -99.49820888148788,30.41833858217994 " +
                "-99.79800943339099,30.41833858217994</gml:coordinates>" +
                "</gml:LinearRing>" +
                "</gml:outerBoundaryIs>" +
                "</gml:Polygon>" +
                "</Intersects>" +
                ...
                "<gml:Box><gml:coordinates decimal=\".\" cs=\",\" ts=\" \">-124.731422,24.955967 " +
                "-66.969849,49.371735</gml:coordinates></gml:Box>" +
                "</BBOX>" +
                "</And>" +
                "</Filter>" +
...
//        assertEquals(contentHandler.filters.size(),1);
    ...
    public void testLargeFilter() throws Exception {
        ...
        String filter = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                "<GetFeature xmlns=\"http://www.opengis.net/wfs\" xmlns:gml=\"http://www.opengis" +
                ".net/gml\" xmlns:ogc=\"http://www.opengis.net/ogc\" version=\"1.0.0\" " +
                "service=\"WFS\" outputFormat=\"GML2\"><Query " +
                "typeName=\"topp:roadevent_pnt\"><ogc:PropertyName>roadeventid</ogc:PropertyName>" +
                "<ogc:Filter>" +
                "<ogc:Or>";
        for (int i = 0; i < filterCount; i++) {
            ...
            filter += "<ogc:PropertyIsEqualTo><ogc:PropertyName>" + attName + 
                    "</ogc:PropertyName>" +
...
//       instantiante parsers and content handlers
        ...
        int i = 0;
        for (Iterator<org.opengis.filter.Filter> filters = ((BinaryLogicOperator) f).getChildren
                ().iterator(); filters.hasNext(); i++) {
