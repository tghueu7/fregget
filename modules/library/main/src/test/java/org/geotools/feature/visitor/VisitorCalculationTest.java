...
public class VisitorCalculationTest extends DataTestCase {
  ...
  public void testUnique() throws IllegalFilterException, IOException {
    ...
    assertEquals(
        5, uniqueResult3.toSet().size()); // 3 and 3.0 are different, so there are actually 5
  ...
  public void testNearest() throws Exception {
    ...
    fc.add(
        SimpleFeatureBuilder.build(
            type,
            new Object[] {"abc", 10, 10.5, df.parse("2014-12-10"), new File("/tmp/test.txt")},
            null));
    fc.add(
        SimpleFeatureBuilder.build(
            type,
            new Object[] {"ade", 5, 3.5, df.parse("2012-11-10"), new File("/tmp/abc.txt")},
            null));
    fc.add(
        SimpleFeatureBuilder.build(
            type,
            new Object[] {"zaa", 2, 50.4, df.parse("2010-11-10"), new File("/tmp/zaa.txt")},
            null));
