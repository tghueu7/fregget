...
public class UomRescaleStyleVisitorTest {
  ...
  protected void visitPolygonSymbolizerTest(double scaleMetersToPixel, Unit<Length> uom) {
    try {
      ...
      String[] splitted =
          rescaledPolySymb.getOptions().get(TextSymbolizer.GRAPHIC_MARGIN_KEY).split("\\s+");
  ...
  protected void visitTextSymbolizerTest(double scaleMetersToPixel, Unit<Length> uom) {
    try {
      ...
      Font font =
          styleBuilder.createFont(new java.awt.Font("Verdana", java.awt.Font.PLAIN, fontSize));
      ...
      textSymb
          .getOptions()
          .put(TextSymbolizer.GRAPHIC_MARGIN_KEY, maxDisplacement + " " + maxDisplacement * 2);
      ...
      assertEquals(
          Math.round(expectedRescaledDisplacementXSize), Math.round(rescaledDisplacementXSize));
      assertEquals(
          Math.round(expectedRescaledDisplacementYSize), Math.round(rescaledDisplacementYSize));
  ...
  protected void visitLineSymbolizerTestDynamicDashArray(
    ...
    try {
      ...
      String rescaledDynamicDashArray =
          (String) ((Expression) rescaledLineSymb.getStroke().dashArray().get(0)).evaluate(null);
