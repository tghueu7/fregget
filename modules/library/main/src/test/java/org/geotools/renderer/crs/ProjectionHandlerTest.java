...
public class ProjectionHandlerTest {
  ...
  public void testCutGeometryLambertConformal() throws Exception {
    ...
    assertTrue(
        "Should have sliced the geometry in two parts", preProcessed instanceof MultiPolygon);
  }
  ...
  public void testWrapGeometrySmall() throws Exception {
    ...
    Geometry g =
        new WKTReader()
            .read("POLYGON ((2139122 5880020, 2139122 5880030, 2139922 5880030, 2139122 5880020))");
  ...
  public void testWrapAnctartica() throws Exception {
    ...
    // post process, this should wrap the geometry, make sure it's valid, and avoid large jumps in
    // its border
    Geometry postProcessed = handler.postProcess(prepared, reprojected);
    assertTrue(postProcessed instanceof MultiPolygon);
  ...
  public void testSkipInvalidGeometries() throws Exception {
    ...
    // Ensure the result is a Geometry collection which does not accept Geometries of the same type
    // of the Projection Handler one.
  ...
  public void testCutGeometryTouchingValidArea() throws Exception {
    ...
    // reference cutting
    // envelope (result of intersecting the valid area with the geometry own bbox, which results in
    // a "line" polygon
    ...
    ProjectionHandler ph =
        new ProjectionHandler(DefaultGeographicCRS.WGS84, validAreaBounds, renderingEnvelope);
