...
public class ProjectionHandlerTest {
    ...
    public void testWrapGeometrySmall() throws Exception {
        ...
        Geometry g = new WKTReader().read("POLYGON ((2139122 5880020, 2139122 5880030, 2139922 " +
                "5880030, 2139122 5880020))");
    ...
    public void testWrapGeometryLatLonMultipleTimes() throws Exception {
        ReferencedEnvelope renderingEnvelope = new ReferencedEnvelope(-90, 90, -580, 540, 
                ED50_LATLON);
        ...
        // make sure the geometry is not wrapped, but it is preserved
        ProjectionHandler handler = ProjectionHandlerFinder.getHandler(renderingEnvelope, WGS84, 
                true);
        assertTrue(handler.requiresProcessing(g));
        Geometry preProcessed = handler.preProcess(g);
    ...
    public void testWrapGeometryReprojectToLatLonED50() throws Exception {
        ...
        MathTransform prepared = handler.getRenderingTransform(CRS.findMathTransform(WGS84, 
                ED50_LATLON));
    ...
    public void testWrapAnctartica() throws Exception {
        ...
        // post process, this should wrap the geometry, make sure it's valid, and avoid large 
        // jumps in its border
        Geometry postProcessed = handler.postProcess(prepared, reprojected);
        assertTrue(postProcessed instanceof MultiPolygon);
    ...
    public void testWrapJumpLast() throws Exception {
        ...
        Geometry original = new WKTReader().read("POLYGON((-131 -73.5,0 -90,163 -60,174 -60,-131 " +
                "-73.5))");
        // make sure the geometry is not wrapped, but it is preserved
        ProjectionHandler handler = ProjectionHandlerFinder.getHandler(world, WGS84, true);
        assertTrue(handler.requiresProcessing(g));
    ...
    public void testWrapGeometryWGS84Duplicate() throws Exception {
        ...
        Geometry g = new WKTReader().read("POLYGON((-178 -90, -178 90, 178 90, 178 -90, -178 -90)" +
                ")");
        Geometry original = new WKTReader().read("POLYGON((-178 -90, -178 90, 178 90, 178 -90, " +
                "-178 -90))");
    ...
    public void testLimitExcessiveDuplication() throws Exception {
        ...
        ReferencedEnvelope renderingEnvelope = new ReferencedEnvelope(-1800000, 1800000, -50, 50,
                WGS84);
    ...
    public void testPolarStereographic() throws Exception {
        ...
        assertTrue(CRS.getMapProjection(envelope.getCoordinateReferenceSystem()) instanceof 
                PolarStereographic);
    }
    ...
    public void testSkipInvalidGeometries() throws Exception {
        ...
        Geometry g1 = new WKTReader()
                .read("POLYGON((150 40, 150 -90, 190 -90, 190 40, 175 40, 175 -87, 165 -87, 165 " +
                        "40, 150 40))");
        ...
        // Ensure the result is a Geometry collection which does not accept Geometries of the 
        // same type of the Projection Handler one.
    ...
    public void testWorldMeridian() throws Exception {
        ...
        // make sure the geometry is not wrapped, but it is preserved
        ProjectionHandler handler = ProjectionHandlerFinder.getHandler(requestWebMercator, WGS84,
                true);
        assertTrue(handler.requiresProcessing(g));
        Geometry preProcessed = handler.preProcess(g);
    ...
    public void testWrapPDCMercator() throws Exception {
        ...
        Geometry g = new WKTReader().read(
                "MULTIPOLYGON(((-73 60, -73 83, -11 83, -11 60, -73 60)),((-10 60, -10 61, -11 " +
                        "61, -11 60, -10 60)))");
        Geometry original = (Geometry) g.clone();
        //
        ProjectionHandler handler = ProjectionHandlerFinder.getHandler(world, WGS84, true);
    ...
    public void testReprojectBackwardsTo900913() throws Exception {
        ...
        String wkt = "PROJCS[\"WGS84 / Google Mercator\", GEOGCS[\"WGS 84\", DATUM[\"World " +
                ...
                "UNIT[\"m\", 1.0],  AUTHORITY[\"EPSG\",\"900913\"]]";
        CoordinateReferenceSystem epsg900913 = CRS.parseWKT(wkt);

        // assume we are rendering in WGS84
        ReferencedEnvelope renderingArea = new ReferencedEnvelope(-180, 0, 0, 90, 
        ...
        ReferencedEnvelope expected = new ReferencedEnvelope(-180, 0, 0, 85, DefaultGeographicCRS
                .WGS84).transform(epsg900913, true);
    ...
    public void testQueryEnvelopeOnInvalidArea() throws Exception {
        ...
        ReferencedEnvelope re = new ReferencedEnvelope(-130, -120, -40, 30, DefaultGeographicCRS
                .WGS84);
        ProjectionHandler ph = ProjectionHandlerFinder.getHandler(re, CRS.decode("EPSG:3003", 
                true), true);
        List<ReferencedEnvelope> queryEnvelopes = ph.getQueryEnvelopes();
    ...
    public void testQueryEnvelopeOnInvalidArea2() throws Exception {
        ...
        ProjectionHandler ph = ProjectionHandlerFinder.getHandler(re3857, CRS.decode("EPSG:3003",
                true), true);
        List<ReferencedEnvelope> queryEnvelopes = ph.getQueryEnvelopes();
        assertEquals(0, queryEnvelopes.size());
    }
    ...
    public void testCutGeometryTouchingValidArea() throws Exception {
        // for some reason JTS won't cut correctly this geometry with an zero height, but not 
        // empty, reference cutting
        // envelope (result of intersecting the valid area with the geometry own bbox, which 
        // results in a "line" polygon
        // Used a BASE64 encoded WKB as WKT is not precise enough to make it happen
        String wkb = "AAAAAAMAAAACAAAAJsBX4dme0ZBrwBBHY3zBphTAV9wm/XQjVMAN5aZ25n" +
                "+AwFfRnB4QLcDACkbY\n" +
        ...
        ReferencedEnvelope renderingEnvelope = new ReferencedEnvelope(-14542204.652543461, 
                15480411.404320458, -18705497.11355389, 11278026.995319324, lambertPolar);
        ReferencedEnvelope validAreaBounds = new ReferencedEnvelope(-180, 180, 0, 90, 
                DefaultGeographicCRS.WGS84);
        ProjectionHandler ph = new ProjectionHandler(DefaultGeographicCRS.WGS84, validAreaBounds,
                renderingEnvelope);
    ...
    public void testCutGeometryCrossingValidArea() throws Exception {
        ...
        String wkb = "AAAAAAMAAAABAAAAIMBX4dme0ZBrwBBHY3zBphTAV9GcHhAtwMAKRti7qZ" +
                "+AwFfQgOTrwUTACf2L\n" +
    ...
    private CoordinateReferenceSystem getLambertPolar() throws FactoryException {
        String crsWKT = "PROJCS[\"North_Pole_Lambert_Azimuthal_Equal_Area\"," +
                "GEOGCS[\"GCS_WGS_1984\",DATUM[\"D_WGS_1984\"," +
                "SPHEROID[\"WGS_1984\",6378137,298.257223563]],PRIMEM[\"Greenwich\",0]," +
                "UNIT[\"Degree\",0.017453292519943295]]," +
                "PROJECTION[\"Lambert_Azimuthal_Equal_Area\"],PARAMETER[\"False_Easting\",0]," +
                "PARAMETER[\"False_Northing\",0]," +
                "PARAMETER[\"Central_Meridian\",0],PARAMETER[\"Latitude_Of_Origin\",90]," +
                "UNIT[\"Meter\",1]]";
    ...
    public void testUTMDatelineWrapping() throws Exception {
        ...
        ReferencedEnvelope targetReferenceEnvelope = new ReferencedEnvelope(-180, 180, -90, 90, 
                WGS84);
        ProjectionHandler ph = ProjectionHandlerFinder.getHandler(targetReferenceEnvelope, crs, 
                true);
