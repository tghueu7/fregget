...
public class SimplifyingFilterVisitorTest extends TestCase {
    ...
    public void testNestedVolatile() {
        ...
        Function f = ff.function("pow", ff.function("random"), ff.function("env", ff.literal
                ("power")));
    ...
    public void testSimplifyStaticExclude() {
        ...
        assertEquals(Filter.EXCLUDE, simplify(ff.between(ff.literal(3), ff.literal(1), ff.literal
                (2))));
    }
    ...
    public void testSimplifyStaticInclude() {
        ...
        assertEquals(Filter.INCLUDE, simplify(ff.between(ff.literal(3), ff.literal(1), ff.literal
                (4))));
    }
    ...
    public void testDualFilterOr() {
        Or or = ff.or(Arrays.asList(ff.not(ff.equal(ff.property("a"), ff.literal(3), true)), ff
                .equal(ff.property("a"), ff.literal(3), true)));
        assertEquals(Filter.INCLUDE, or.accept(simpleVisitor, null));
    }

    public void testDualFilterAnd() {
        Filter original = ff.and(Arrays.asList(ff.not(ff.equal(ff.property("a"), ff.literal(3), 
                true)), ff.equal(ff.property("a"), ff.literal(3), true)));
        assertEquals(Filter.EXCLUDE, original.accept(simpleVisitor, null));
    }

    public void testDualFilterNullAnd() {
        Filter original = ff.and(Arrays.asList(ff.not(ff.isNull(ff.property("a"))), ff.isNull(ff
                .property("a"))));
        assertEquals(Filter.EXCLUDE, original.accept(simpleVisitor, null));
    }

    public void testDualFilterNullOr() {
        Filter original = ff.or(Arrays.asList(ff.not(ff.isNull(ff.property("a"))), ff.isNull(ff
                .property("a"))));
