...
public class MosaicTest extends GridProcessingTestBase {
  ...
  private GridCoverage2D simpleMosaic(GridCoverage2D coverage1, GridCoverage2D coverage2) {
    ...
    // Check that the final GridCoverage BoundingBox is equal to the union of the separate coverages
    // bounding box
    ...
    DirectPosition point =
        new DirectPosition2D(
            mosaic.getCoordinateReferenceSystem(), actual.getCenterX(), actual.getCenterY());
  ...
  public void testWorldFile() throws FileNotFoundException, IOException {
    ...
    GridCoverage2D mosaic =
        mosaic(
            sortCoverages(Arrays.asList(c1, shifted)), new GeneralEnvelope(reUnion), new Hints());
    ...
    DirectPosition point =
        new DirectPosition2D(
            mosaic.getCoordinateReferenceSystem(), expected.getCenterX(), expected.getCenterY());
    double nodata = 0;
  ...
  // Test which takes an input file, extracts 4 coverages from it, shifts them in order to created a
  // replication.
  @Test
  ...
  public void testPaletted() throws IOException {
    ...
    GridCoverage2D shifted =
        new GridCoverageFactory().create(world.getName(), world.getRenderedImage(), reShifted);
    sources.add(shifted);
    param.parameter("Sources").setValue(sources);
  ...
  private GridCoverage2D crop(GridCoverage2D gc, GeneralEnvelope envelope) {
    ...
    final ParameterValueGroup param =
        (ParameterValueGroup) processor.getOperation("CoverageCrop").getParameters().clone();
