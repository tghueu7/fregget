...
public final class ResampleTest extends GridProcessingTestBase {
  ...
  /**
   * An other source coverage initialized by {@link #setUp}. Contains indexed color model for a GIF
   * image, without categories.
   */
  ...
  /**
   * Projects the specified coverage to the same CRS without hints. The result will be displayed in
   * a window if {@link #SHOW} is set to {@code true}.
   *
  ...
  public void testCrop() {
    ...
    g1 =
        new GridGeometry2D(
            new GeneralGridEnvelope(new Rectangle(50, 50, 100, 100), 2), gridToCRS, null);
    g2 =
        new GridGeometry2D(
            new GeneralGridEnvelope(new Rectangle(50, 50, 200, 200), 2), gridToCRS, null);
  ...
  public void testReproject() throws NoSuchAuthorityCodeException, FactoryException {
    ...
    try {
      ...
      assertEquals(
          "Warp",
          showProjected(
              coverage, CRS.parseWKT(GOOGLE_MERCATOR_WKT), coverage.getGridGeometry(), null));
      Assert.assertTrue(
