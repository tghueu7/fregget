...
public final class ResampleTest extends GridProcessingTestBase {
    ...
    public void testCrop() {
        ...
        g1 = new GridGeometry2D(new GeneralGridEnvelope(new Rectangle(50, 50, 100, 100), 2), 
                gridToCRS, null);
        g2 = new GridGeometry2D(new GeneralGridEnvelope(new Rectangle(50, 50, 200, 200), 2), 
                gridToCRS, null);
    ...
    public void testReproject() throws NoSuchAuthorityCodeException, FactoryException {
        ...
        GridCoverage2D coverage_ = project(ushortCoverage, CRS.parseWKT(GOOGLE_MERCATOR_WKT), 
                null, "nearest", null);
        ...
        try {
            ...
            assertEquals("Warp", showProjected(coverage, CRS.parseWKT(GOOGLE_MERCATOR_WKT), 
                    coverage.getGridGeometry(), null));
            Assert.assertTrue("We should not be allowed to set different crs for target crs and " +
                    "target gg", false);
        } catch (Exception e) {
            // ok!
        }
    ...
    public void testGoogleWorld() throws Exception {
        ...
        GridCoverage2D gcCropWorld = (GridCoverage2D) Operations.DEFAULT.crop(gcFullWorld, 
                cropEnvelope);

        // resample
        Hints.putSystemDefault(Hints.RESAMPLE_TOLERANCE, 0d);
        GridCoverage2D gcResampled = (GridCoverage2D) Operations.DEFAULT.resample(gcCropWorld, 
                CRS.decode("EPSG:3857"),
                null, Interpolation.getInstance(Interpolation.INTERP_BILINEAR));

        File expected = new File("src/test/resources/org/geotools/image/test-data/google" +
                "-reproject.png");
    ...
    public void testWarpCompareGoogleWorld() throws Exception {
        ...
        GridCoverage2D gcResampledApprox = (GridCoverage2D) Operations.DEFAULT.resample
                (gcCropWorld, CRS.decode("EPSG:3857"),
                null, Interpolation.getInstance(Interpolation.INTERP_BILINEAR));
        ...
        // allow one row of difference
        ImageAssert.assertEquals(gcResampledAccurate.getRenderedImage(), gcResampledApprox
                .getRenderedImage(), 600);
    }
    ...
    public void testFlipTranslated() throws Exception {
        ...
        SampleModel sm = RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_BYTE, 
                256, 256, 3);
        ...
        GridCoverage2D coverage = factory.create("translated", ti, new Envelope2D(wgs84LatLon, 3,
                5, 6, 8));
        ...
        GridGeometry gg = new GridGeometry2D(new GridEnvelope2D(-10, -10, 5, 5), (Envelope) new 
                Envelope2D(wgs84LonLat, 5, 3, 8, 6));
    ...
    private void doTranslation(GridCoverage2D grid) throws NoninvertibleTransformException {
        ...
        final GridCoverageFactory factory = CoverageFactoryFinder.getGridCoverageFactory(null);
        grid = factory.create("Translated", image, grid.getEnvelope(), grid.getSampleDimensions()
        ...
        //account for the half pixel correction between the two spaces since we are talking 
        // raster here but the resample will talk model!
        final MathTransform correctedTransform = PixelTranslation.translate(tr, PixelInCell
                .CELL_CORNER, PixelInCell.CELL_CENTER);
        final GridGeometry2D geometry = new GridGeometry2D(null, correctedTransform, null);
        final GridCoverage2D newGrid = (GridCoverage2D) Operations.DEFAULT.resample(grid, grid
                .getCoordinateReferenceSystem(), geometry, null);
