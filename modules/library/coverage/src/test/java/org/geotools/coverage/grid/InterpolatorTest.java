...
public final class InterpolatorTest extends GridCoverageTestBase {
    ...
    public void testInterpolationAtEdges() {
        ...
        final double PIXEL_SIZE = XAffineTransform.getScale((AffineTransform) ((GridGeometry2D) 
                coverage.getGridGeometry()).getGridToCRS());
        final Interpolation interpolation = Interpolation.getInstance(Interpolation
                .INTERP_BILINEAR);
        ...
        Rectangle rectangle = PlanarImage.wrapRenderedImage(coverage.getRenderedImage())
                .getBounds();
        rectangle = new Rectangle(rectangle.x, rectangle.y, rectangle.width + interpolation
                .getWidth(), rectangle.height + interpolation.getHeight());
        final Raster data = PlanarImage.wrapRenderedImage(coverage.getRenderedImage())
    ...
    public void testInterpolationROINoData() {
        ...
        ROIShape roi = new ROIShape(new Rectangle(src.getMinX(), src.getMinY(), src.getWidth() / 
                2, src.getHeight() / 2));
        ...
        coverage = factory.create("Test2", src, coverage.getEnvelope(), coverage
                .getSampleDimensions(), null, properties);
        final double PIXEL_SIZE = XAffineTransform.getScale((AffineTransform) ((GridGeometry2D) 
                coverage.getGridGeometry()).getGridToCRS());
