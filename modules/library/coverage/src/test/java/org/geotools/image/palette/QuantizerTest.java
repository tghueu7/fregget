...
public class QuantizerTest {
  ...
  /**
   * Checks two images are visually equal given a certain maximum color distance. For a better tool
   * you might want to check out {@link ImageAssert}, but that works only with RGB images, this one
  ...
  private void assertImagesSimilar(
    ...
    for (int i = 0; i < bi1.getWidth(); i++) {
      for (int j = 0; j < bi1.getHeight(); j++) {
        ...
        int da = a1 - a2;
        double d =
            Math.sqrt((1.5 * dr * dr + 2 * dg * dg + db * db + 2 * da * da) / (1.5 + 2 + 1 + 2));
        assertTrue(
            "Color distance " + d + " excessive for pixels " + i + "," + j, d <= maxColorDistance);
      }
    }
  }
