...
public class GridSampleDimension implements SampleDimension, Serializable {
  ...
  /**
   * Decription for this sample dimension. Typically used as a way to perform a band select by using
   * human comprehensible descriptions instead of just numbers. Web Coverage Service (WCS) can use
   * this feature in order to perform band subsetting as directed from a user request.
   */
   ...
   *
   * @param description The sample dimension title or description, or {@code null} if none. This is
   *     the value to be returned by {@link #getDescription}.
   * @since 2.3
   */
   ...
   * expects only a sequence of category names for the values contained in a sample dimension. This
   * allows for names to be assigned to numerical values. The first entry in the sequence relates to
   * a cell value of zero. For example: [0]="Background", [1]="Water", [2]="Forest", [3]="Urban".
  ...
  /**
   * Constructs a sample dimension with a set of qualitative categories and colors. This constructor
   * expects a sequence of category names for the values contained in a sample dimension. This
   * allows for names to be assigned to numerical values. The first entry in the sequence relates to
   * a cell value of zero. For example: [0]="Background", [1]="Water", [2]="Forest", [3]="Urban".
   * The created sample dimension will have no unit and a default set of colors.
   *
   ...
   * general and provides less fine-grain control than the constructor expecting an array of {@link
   * Category} objects.
   *
   * @param description The sample dimension title or description, or {@code null} for the default
   *     (the name of what looks like the "main" category). This is the value to be returned by
   *     {@link #getDescription}.
   * @param type The grid value data type (which indicate the number of bits for the data type), or
   *     {@code null} for computing it automatically from the range {@code [minimum..maximum]}. This
   ...
   *     #getCategoryNames}.
   * @param nodata the values to indicate "no data", or {@code null} if none. This is the values to
   *     be returned by {@link #getNoDataValues}.
   * @param minimum The lower value, inclusive. The {@code [minimum..maximum]} range may or may not
   *     includes the {@code nodata} values; the range will be adjusted as needed. If {@code
   ...
   *     [minimum..maximum]} range may or may not includes the {@code nodata} values; the range will
   *     be adjusted as needed. If {@code categories} was non-null, then {@code maximum} is usually
   *     equals to {@code categories.length-1}. This is the value to be returned by {@link
   *     #getMaximumValue}.
   ...
   * quantitative and qualitative. It is possible to specify more than one quantitative categories,
   * providing that their sample value ranges do not overlap.
   *
   * @param description The sample dimension title or description, or {@code null} for the default
   *     (the name of what looks like the "main" category). This is the value to be returned by
   *     {@link #getDescription}.
   * @param categories The list of categories.
   * @param units The unit information for this sample dimension. May be {@code null} if no category
   *     has units.
   * @throws IllegalArgumentException if {@code categories} contains incompatible categories. If may
   *     be the case for example if two or more categories have overlapping ranges of sample values.
   * @since 2.3
   */
  ...
  /**
   * Constructs a new sample dimension with the same categories and units than the specified sample
   * dimension.
   *
   ...
   * allows for names to be assigned to numerical values. The first entry in the sequence relates to
   * a cell value of zero. For example:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * @throws IllegalStateException if a sequence can't be mapped because some category use negative
   *     or non-integer sample values.
   * @see #getCategories
   * @see #getCategory
   */
  ...
  /**
   * Returns all categories in this sample dimension. Note that a {@link Category} object may apply
   * to an arbitrary range of sample values. Consequently, the first element in this collection may
   * not be directly related to the sample value {@code 0}.
   *
  ...
  /**
   * Returns the values to indicate "no data" for this sample dimension. The default implementation
   * deduces the "no data" values from the list of categories supplied at construction time.
   *
   * @return The values to indicate no data values for this sample dimension, or {@code null} if not
   *     applicable.
  ...
  /**
   * Returns the range of values in this sample dimension. This is the union of the range of values
   * of every categories, excluding {@code NaN} values. A {@link NumberRange} object gives more
   * informations than {@link #getMinimumValue} and {@link #getMaximumValue} methods since it
  ...
  /**
   * Returns a string representation of a sample value. This method try to returns a representation
   * of the geophysics value; the transformation is automatically applied when necessary. More
   * specifically:
   *
   * <ul>
   *   <li>If {@code value} maps a qualitative category, then the category name is returned as of
   *       {@link Category#getName}.
   *   <li>Otherwise, if {@code value} maps a quantitative category, then the value is formatted as
   *       a number and the unit symbol is appened.
   * </ul>
   *
   ...
   * @todo What should we do when the value can't be formatted? {@code GridSampleDimension} returns
   *     {@code null} if there is no category or if an exception is thrown, but {@code CategoryList}
   *     returns "Untitled" if the value is an unknow NaN, and try to format the number anyway in
   *     other cases.
   */
  ...
  /**
   * Returns the value to add to grid values for this sample dimension. This attribute is typically
   * used when the sample dimension represents elevation data. The transformation equation is:
   *
   * <blockquote>
   *
  ...
  /**
   * Returns the value which is multiplied to grid values for this sample dimension. This attribute
   * is typically used when the sample dimension represents elevation data. The transformation
   * equation is:
   *
   * <blockquote>
   *
   ...
   * colors. See palette interpretation for meaning of the palette entries. If the grid coverage has
   * no color palette, {@code null} will be returned.
   *
   ...
   *     with values in the <code>{@link #getRange}</code> range. May be {@code null} if this sample
   ...
   *     will renderer only the {@code visibleBand} and ignore the others, but the existence of all
   *     {@code numBands} will be at least tolerated. Supplemental bands, even invisible, are useful
   ...
   *     SampleDimension} object for creating a multi-bands color model. Logically, we would expect
   *     as many {@code SampleDimension}s as bands.
   */
  ...
  /**
   * Returns a string representation of this sample dimension. This string is for debugging purpose
   * only and may change in future version. The default implementation format the sample value
   * range, then the list of categories. A "*" mark is put in front of what seems the "main"
   * category.
   */
  @Override
