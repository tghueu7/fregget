...
public class GridCoverage2D extends AbstractGridCoverage {
   ...
   * dimension is a band. The sample dimension information include such things as description, data
   * type of the value (bit, byte, integer...), the no data values, minimum and maximum values and a
   * color table if one is associated with the dimension. A coverage must have at least one sample
   * dimension.
   *
  ...
  /**
   * The preferred encoding to use for serialization using the {@code writeObject} method, or {@code
   * null} for the default encoding. This value is set by {@link GridCoverageFactory} according the
   * hints provided to the factory.
   */
   ...
   * (including the {@linkplain CoordinateReferenceSystem coordinate reference system}) is inferred
   * from the grid geometry.
   *
   * <p>This constructor accepts an optional set of properties. Keys are {@link String} objects
   * ({@link javax.media.jai.util.CaselessStringKey} are accepted as well), while values may be any
   * {@link Object}.
   *
  ...
  protected GridCoverage2D(
    ...
    if (!gridGeometry.isDefined(GridGeometry2D.GRID_RANGE_BITMASK)) {
      ...
      if (gridGeometry.isDefined(GridGeometry2D.GRID_TO_CRS_BITMASK)) {
        gridGeometry =
            new GridGeometry2D(
                r, PIXEL_IN_CELL, gridGeometry.getGridToCRS(PIXEL_IN_CELL), crs, hints);
      } else {
        /*
   ...
   * The returned envelope have at least two dimensions. It may have more dimensions if the coverage
   * has some extent in other dimensions (for example a depth, or a start and end time).
   */
  @Override
  ...
  /**
   * Retrieve sample dimension information for the coverage. For a grid coverage, a sample dimension
   * is a band. The sample dimension information include such things as description, data type of
   * the value (bit, byte, integer...), the no data values, minimum and maximum values and a color
   * table if one is associated with the dimension. A coverage must have at least one sample
   * dimension.
   */
   ...
   *     More specifically, {@link PointOutsideCoverageException} is thrown if the evaluation failed
   *     because the input point has invalid coordinates.
   */
  @Override
  ...
  public int[] evaluate(final GridCoordinates2D coord, final int[] dest) {
    if (image.getBounds().contains(coord.x, coord.y)) {
      return image
          .getTile(image.XToTileX(coord.x), image.YToTileY(coord.y))
  ...
  /**
   * Return sample dimension (band) values as an array of floats for the given <b>grid</b> location.
   * The range of valid grid coordinates can be retrieved as in this example:
   *
   * <pre><code>
   * GridEnvelope2D gridBounds = coverage.getGridGeometry2D().getGridRange();
   * </code></pre>
  ...
  private void writeObject(final ObjectOutputStream out) throws IOException {
    if (serializedImage == null) {
      ...
      if (source instanceof SerializableRenderedImage) {
        ...
        serializedImage =
            new SerializableRenderedImage(source, false, null, tileEncoding, null, null);
        final LogRecord record =
            Loggings.format(
                Level.FINE, LoggingKeys.CREATED_SERIALIZABLE_IMAGE_$2, getName(), tileEncoding);
   ...
   * method {@linkplain PlanarImage#dispose disposes} the {@linkplain #image} only if at least one
   * of the following conditions is true (otherwise this method do nothing):
   *
   * <p>
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>This safety check helps to prevent the disposal of an {@linkplain #image} that still used in
   * a JAI operation chain. It doesn't prevent the disposal in every cases however. When unsure
  ...
  /**
   * Returns a string representation of this grid coverage. This is mostly for debugging purpose and
