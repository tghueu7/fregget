...
final class ColorModelFactory {
   ...
   * {@linkplain java.lang.ref.SoftReference soft references} because the intend is not to cache the
   * values. The intend is to share existing instances in order to reduce memory usage. Rational:
   *
   * <ul>
   *   <li>{@link ColorModel} may consume a lot of memory. A 16 bits indexed color model can consume
   *       up to 256 kb. We don't want to retain such large objects longer than necessary. We want
   *       to share existing instances without preventing the garbage collector to collect them.
   ...
   *
   * @task TODO: The user may want to set explicitly the number of bits each pixel occupied. We need
   *     to think about an API to allows that.
   */
   ...
   * ColorModelKey}" instead. However, since this constructor is private, user doesn't need to know
   * that.
   */
  private ColorModelFactory(
    ...
    if (visibleBand < 0 || visibleBand >= numBands) {
      throw new IllegalArgumentException(Errors.format(ErrorKeys.BAD_BAND_NUMBER_$1, visibleBand));
    }
  }
