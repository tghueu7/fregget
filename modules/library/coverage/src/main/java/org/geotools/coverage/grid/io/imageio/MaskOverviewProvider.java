...
public class MaskOverviewProvider {
    ...
    if (overviewFile.exists() && overviewFile.canRead()) {
      ...
      try {
        ...
        if (LOGGER.isLoggable(Level.WARNING)) {
          LOGGER.log(
              Level.WARNING,
              "Unable to create a Reader for File: " + overviewFile.getCanonicalPath(),
              e);
        }
  ...
  /**
   * Returns a new {@link MaskInfo} instance containing all the parameters to set for accessing the
   * desired image index
   */
  ...
  public static class SpiHelper {
    ...
    public SpiHelper(File inputFile, ImageReaderSpi suggestedSPI) throws IOException {
      ...
      try {
        ...
        isMultidim =
            readerSpi != null
                && MULTIDIM_SERVICE_PROVIDERS.contains(readerSpi.getClass().getName());
      } catch (Exception e) {
        if (LOGGER.isLoggable(Level.WARNING)) {
          LOGGER.log(
              Level.WARNING,
