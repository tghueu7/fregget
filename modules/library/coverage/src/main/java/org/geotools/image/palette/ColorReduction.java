...
public class ColorReduction extends PointOpImage {
    ...
    private ImageLayout getImageLayout(RenderedImage image, RenderingHints hints) {
        ...
        layout.setSampleModel(paletteBuilder.getIndexColorModel().createCompatibleSampleModel
    ...
    public Raster computeTile(int tx, int ty) {
        ...
        for (int i = minx; i < maxx; i++)
            for (int j = miny; j < maxy; j++) {
                ...
                destRaster.setSample(i, j, 0, paletteBuilder.findNearestColorIndex(rgba, 
                        alphaBand));
            }
        return destRaster;
    }
