...
public final class GeoKeyEntry implements Comparable<GeoKeyEntry> {
  ...
  /**
   * "KeyID" gives the key-ID value of the Key (identical in function to TIFF tag ID, but completely
   * independent of TIFF tag-space)
   */
  private int keyID;

  /**
   * "TIFFTagLocation" indicates which TIFF tag contains the value(s) of the Key: if TIFFTagLocation
   * is 0, then the value is SHORT, and is contained in the "Value_Offset" entry. Otherwise, the
   * type (format) of the value is implied by the TIFF-Type of the tag containing the value.
   */
   ...
   * TIFFTagLocation, if it is nonzero. If TIFFTagLocation=0, then Value_Offset contains the actual
   * (SHORT) value of the Key, and Count=1 is implied. Note that the offset is not a byte-offset,
   ...
   * order. This is done to avoid forcing software to perform N-squared sort operations when reading
   * and writing tags.
   */
