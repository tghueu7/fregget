...
public interface GridCoverage2DReader extends GridCoverageReader {
  ...
  /**
   * Whether the elevation is expressed as a full domain or min/max/resolution (true if domain list
   * available)
   */
  ...
  /**
   * Name of a {@link GridCoverage2D} property that is a {@link URL} for the original source of the
   * coverage. This {@link URL} might be used to obtain further information on the coverage that is
   * not otherwise available through the {@link GridCoverage2D} API.
   */
  public static String SOURCE_URL_PROPERTY = "SourceUrl";

  /**
   * This property is present, and evaluates to "true", if the reader can do reprojection on its own
   * (that is, it is not backed by actual data, but by a remote service that can perform
   * reprojection for us).
   */
  ...
  /**
   * Retrieves the {@link CoordinateReferenceSystem} associated to this {@link GridCoverage2DReader}
   ...
   * coverage.
   *
   * @param parameters an array of {@link GeneralParameterValue} that uses a subset of the available
   *     read params for this {@link GridCoverage2DReader} as specified by the {@link Format}
   ...
   * resolution using the provided {@link OverviewPolicy}. This method works on the default coverage
  ...
  double[] getReadingResolutions(
      String coverageName, OverviewPolicy policy, double[] requestedResolution) throws IOException;
