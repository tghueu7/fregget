...
public class ImageComparator {
  ...
  public enum Mode {
    /**
     * Checks if the images are equal taking into account the full color and all pixels. Some light
     * difference between the two images is still being tolerated
     */
    IgnoreNothing,
    /**
     * Same as above, but if a pixel is found to be anti-aliased, only brightness will be compared,
     * instead of the full color component
     */
    IgnoreAntialiasing,
  ...
  private RenderedImage normalizeImage(RenderedImage image1) {
    image1 =
        new ImageWorker(image1).forceColorSpaceRGB().forceComponentColorModel().getRenderedImage();
