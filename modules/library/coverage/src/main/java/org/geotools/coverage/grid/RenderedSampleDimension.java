...
final class RenderedSampleDimension extends GridSampleDimension {
  ...
  /**
   * Creates a set of sample dimensions for the given image. The array length of both arguments must
   * matches the number of bands in the supplied {@code image}.
   *
   ...
   *     affect the sample dimensions to be created. The optional hint {@link
  ...
  private static void create(
    ...
    if (colors != null && colors.length != numBands) {
      throw new IllegalArgumentException(
          Errors.format(
              ErrorKeys.NUMBER_OF_BANDS_MISMATCH_$3, numBands, colors.length, "colors[i]"));
    }
    /*
