...
public abstract class AbstractGridCoverage extends AbstractCoverage implements GridCoverage {
  ...
  /**
   * Sources grid coverage, or {@code null} if none. This information is lost during serialization,
   * in order to avoid sending a too large amount of data over the network.
   */
   ...
   *     is not limited to) a {@link javax.media.jai.PlanarImage} or an other {@code
   ...
   * the source grid coverage of the one used as input to {@code GridCoverageProcessor}. In general
   * the {@code getSources()} method is intended to return the original {@code GridCoverage} on
   * which it depends. This is intended to allow applications to establish what {@code
   * GridCoverage}s will be affected when others are updated, as well as to trace back to the "raw
   * data".
   */
  @Override
  ...
  /**
   * Returns the number of predetermined overviews for the grid. The default implementation returns
   * 0.
   */
  ...
  /**
   * Returns a pre-calculated overview for a grid coverage. The default implementation always throws
  ...
  /**
   * Constructs an error message for a point that can not be evaluated. This is used for formatting
   * error messages.
   *
