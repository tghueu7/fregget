...
public final class FeatureUtilities {
    ...
    public static final FilterFactory2 DEFAULT_FILTER_FACTORY = CommonFactoryFinder
            .getFilterFactory2(GeoTools.getDefaultHints());
    public final static PropertyName GRID_PROPERTY_NAME = DEFAULT_FILTER_FACTORY.property("grid");
    public static final PropertyName PARAMS_PROPERTY_NAME = DEFAULT_FILTER_FACTORY.property
            ("params");


    final static Logger LOGGER = org.geotools.util.logging.Logging.getLogger(FeatureUtilities
            .class.toString());
    ...
    private static FeatureTypeFactory getTypeFactory() {
        if (typeFactory == null)
            typeFactory = new FeatureTypeFactoryImpl();  // CommonFactoryFinder
    ...
    public static boolean isWrappedCoverage(SimpleFeatureType featureType) {
        ...
        if (gridDescriptor == null || !GridCoverage.class.equals(gridDescriptor.getType()
                .getBinding()))
            return false;
     ...
     */
    @SuppressWarnings("unchecked")
    public static SimpleFeatureCollection wrapGridCoverageReader(final GridCoverage2DReader 
                                                                             gridCoverageReader,
        ...
        if (sourceCrs == null)
            throw new UnsupportedOperationException(
                    Errors.format(
                            ErrorKeys.CANT_SEPARATE_CRS_$1, gridCoverageReader
    ...
    public static boolean isWrappedCoverageReader(SimpleFeatureType featureType) {
        ...
        AttributeDescriptor gridDescriptor = featureType.getDescriptor("grid");
        if (gridDescriptor == null || !GridCoverage2DReader.class.equals(gridDescriptor.getType()
                .getBinding()))
            return false;

        AttributeDescriptor paramDescriptor = featureType.getDescriptor("params");
        if (paramDescriptor == null || !GeneralParameterValue[].class.equals(paramDescriptor
                .getType().getBinding()))
            return false;
     ...
     */
    public static Polygon getPolygon(final GeneralEnvelope env, final GeometryFactory gf) throws 
            IllegalStateException, MismatchedDimensionException {
        ...
        assert modelSpaceROI.getEnvelopeInternal().equals(new ReferencedEnvelope(rect, env
                .getCoordinateReferenceSystem()));
