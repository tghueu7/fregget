...
public final class Interpolator2D extends GridCoverage2D {
   ...
   *     {@link #view} method to work. Because the {@linkplain GridCoverage2D#image image} contained
   *     in the source is the same one than in this {@link Calculator2D}, there is few cost in
   *     keeping it.
   */
  ...
  /**
   * Transform from "real world" coordinates to grid coordinates. This transform maps coordinates to
   * pixel <em>centers</em>.
   */
  ...
  /**
   * The interpolation bounds. Interpolation will use pixel inside this rectangle. This rectangle is
   * passed as an argument to {@link RectIterFactory}.
   */
  private final Rectangle bounds;

  /**
   * Arrays to use for passing arguments to interpolation. This array will be constructed only when
   * first needed.
   */
  ...
  /**
   * Constructs a new interpolator for an interpolation and its fallbacks. The fallbacks are used if
   * the primary interpolation failed because of {@linkplain Float#NaN NaN} values in the
   ...
   *
   * @param coverage The coverage to interpolate.
   * @param interpolations The interpolations to use and its fallback (if any). This array must have
   *     at least 1 element.
  ...
  /**
   * Returns interpolations. The first array's element is the interpolation for this grid coverage.
   * Other elements (if any) are fallbacks.
   */
   ...
   * <strong>must</strong> have been initialized with {@code super.evaluate(...)} prior to invoking
   * this method.
   *
