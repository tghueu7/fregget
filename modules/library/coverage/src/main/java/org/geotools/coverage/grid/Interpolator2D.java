...
public final class Interpolator2D extends GridCoverage2D {
     ...
     */
    public static GridCoverage2D create(GridCoverage2D coverage, final Interpolation[] 
            interpolations) {
     ...
     */
    public static GridCoverage2D create(GridCoverage2D coverage, final Interpolation[] 
            interpolations, final BorderExtender be) {
    ...
    private Interpolator2D(final GridCoverage2D coverage,
        ...
        if (fallback != null && fallback != this) {
            ...
            final MathTransform2D transform = gridGeometry.getGridToCRS2D(PixelOrientation
                    .UPPER_LEFT);
    ...
    private synchronized double[] interpolate(final double x, final double y,
        ...
        bounds.x = ix - left;
        bounds.y = iy - top;
        final RectIter iter = RectIterFactory.create(image.getExtendedData(bounds, this
                .borderExtender), bounds);
