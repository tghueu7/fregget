...
public enum FootprintBehavior {
    ...
    Transparent(true) {
        ...
        public RenderedImage postProcessMosaic(RenderedImage mosaic, ROI overallROI, 
            ...
            if (imageWorker.getRenderedImage().getColorModel().hasAlpha()) {
                ...
                if (!alpha.getBounds().equals(imageWorker.getPlanarImage().getBounds())) {
                    ...
                    layout.setTileHeight(sampleModel.getWidth()).setTileWidth(sampleModel
                            .getHeight());
        ...
        @Override
        public RenderedImage postProcessBlankResponse(RenderedImage finalImage, RenderingHints 
                hints) {
            // force the current image in RGB or Gray
            ...
            if (!imageWorker.getRenderedImage().getColorModel().hasAlpha()) {
                ...
                RenderedImage alpha = imageWorker2.retainFirstBand().multiplyConst(new 
                        double[]{0.0}).getRenderedImage();
     ...
     * @return
     */
    public RenderedImage postProcessMosaic(RenderedImage mosaic, ROI overallROI, RenderingHints 
            hints) {
        return mosaic;
    }
