...
public class GeoTiffIIOMetadataEncoder {
    ...
    public void setModelPixelScale(double x, double y, double z) {
        if (isModelTransformationSet())
            throw new IllegalStateException(
                    "ModelTransformationTag already set. It is not possible to set the " +
                            "ModelPixelScale.");
    ...
    public void addGeoAscii(int keyID, String value) {
        ...
        if (currentEntry != null) {
            ...
            GeoKeyEntry newEntry = updateGeoAsciiParamsRef(keyID, newValue.length(), currentEntry
                    .getValueOffset());

            // +1 for the '|' character to be appended
            replaceAsciiParam(newValue.toString(), currentEntry.getValueOffset(), currentEntry
                    .getValueOffset() + currentLength);
        } else {
            // now add to the geoascii metatag
    ...
    public void assignTo(Element element) {
        ...
        if (ifd1 == null) {
            throw new IllegalArgumentException("Unable to find child " + GeoTiffConstants
                    .GEOTIFF_IFD_TAG);
        }

        final Element ifd2 = createIFD();
        ifd1.setAttribute(GeoTiffConstants.GEOTIFF_TAGSETS_ATT_NAME, ifd2.getAttributeValue
    ...
    private void checkParamTag(final int tag, final int expectedTag) {
        if (tag != expectedTag) {
            if (expectedTag == 0) {
                throw new IllegalArgumentException("invalid key access, not a GeoTIFF SHORT " +
                        "parameter");
            } else if (expectedTag == getGeoDoubleParamsTag().getNumber()) {
                throw new IllegalArgumentException("invalid key access, not a GeoTIFF DOUBLE " +
                        "parameter");
            } else if (expectedTag == getGeoAsciiParamsTag().getNumber()) {
                throw new IllegalArgumentException("invalid key access, not a GeoTIFF ASCII " +
                        "parameter");
            } else {
                throw new IllegalStateException();
            }
    ...
    private void addDoubleParam(double param) {
        ...
        if (numDoubleParams >= (geoTiffDoubleParams.length - 1)) {
            final double[] doubleParams = new double[numDoubleParams + GeoTiffConstants
    ...
    private GeoKeyEntry addGeoDoubleParamsRef(int keyID, int count) {
        return addGeoKeyEntry(keyID, getGeoDoubleParamsTag().getNumber(), count, 
                getCurrentGeoDoublesOffset());
    }

    private GeoKeyEntry addGeoAsciiParamsRef(int keyID, int length) {
        return addGeoKeyEntry(keyID, getGeoAsciiParamsTag().getNumber(), length, 
    ...
    private Element createIFD() {
        ...
        ifd.setAttribute(GeoTiffConstants.GEOTIFF_TAGSETS_ATT_NAME, BaselineTIFFTagSet.class
                .getName() + "," + GeoTIFFTagSet.class.getName());
    ...
    public void setModelTransformation(final AffineTransform rasterToModel) {
        if (modelPixelScale != null && modelPixelScale.isSet())
            throw new IllegalStateException("ModelPixelScaleTag already set. It is not possible " +
                    "to set the ModelTransformation.");
        if (isModelTiePointsSet())
            throw new IllegalStateException("ModelTiePointsTag already set. It is not possible to" +
                    " set the ModelTransformation.");
     ...
     *
     * @param name  is the Ascii TIFF Tag identifier. It can be a String representing:
     *              1) a simple Integer (referring to a tag ID) (in that case it will refer to 
     *              the BaselineTIFFTagSet
