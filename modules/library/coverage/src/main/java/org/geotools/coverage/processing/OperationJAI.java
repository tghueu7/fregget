...
public class OperationJAI extends Operation2D {
     ...
     * @param primarySourceType Tells if we have to change the "geo-view" for the provided 
     * {@link GridCoverage2D}.
     ...
     * @throws CannotReprojectException     if a grid coverage can't be resampled for some other 
     * reason.
     */
        ...
        for (int i = 0; i < sources.length; i++) {
            ...
            if (CRS.equalsIgnoreMetadata(crs2D, srcCrs2D)) {
                ...
                if (upperDim - lowerDim != srcCrs2D.getCoordinateSystem().getDimension()) {
                    // TODO: localize
                    throw new InvalidGridGeometryException("Unsupported CRS: " + sourceCRS
                            .getName().getCode());
                }
            ...
            if (CRS.equalsIgnoreMetadata(gridToCrs2D, toSource2D)) {
                ...
                final MathTransformFactory factory = ReferencingFactoryFinder
    ...
    /**
     * Returns the quantitative category for a single 
     ...
     *                   operations like {@code "add"} and {@code "multiply"}, this array has a 
     *                   length of 2.
     * @param parameters Parameters, rendering hints and coordinate reference system to use.
     ...
     * <p>
     * <blockquote><pre>
     * {@linkplain #getJAI getJAI}(hints).{@linkplain JAI#createNS createNS}
     * ({@linkplain #operation}.getName(), parameters, hints)
     * </pre></blockquote></li>
     * <p>
     ...
     * <p>
     * Notice that if you are using JAI, the new parameters will not be accepted by the 
     * {@link ParameterBlockJAI} instance.
     *
    ...
    /**
     * This method can be used for creating a property Map to set to the new coverage generated 
     * by the current operation.
     * Internally the method will search for ROI and NoData parameters set for the operation and 
     * will report them as
     * coverage properties
     *
     * @param properties
     ...
     * @return A {@link Map} containing all the coverage properties and also {@link ROI} and 
     * NoData if present
     */
    ...
    /**
     * This method can be used for merging input coverage properties (ROI and NoData) with the 
     * ones provided as input in the ParameterBlock instance.
     * If a ROI instance is already present as a coverage property, it will be intersected with 
     * an eventual ROI object defined as a parameter inside
     * the ParameterBlock. If no NoData Range is defined in the parameters but is defined as 
     * coverage property, it will be set in the input
     * ParameterBlock
     *
     * @param parameters
     * @param sourceCoverage
    ...
    protected static void handleROINoDataInternal(ParameterBlockJAI parameters,
                                                  GridCoverage2D sourceCoverage, String 
                                                          operationName, int roiIndex, int 
                                                          noDataIndex) {
