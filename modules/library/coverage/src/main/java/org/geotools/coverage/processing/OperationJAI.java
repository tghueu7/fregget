...
public class OperationJAI extends Operation2D {
   ...
   *   <li>Invokes {@link #deriveGridCoverage}. The sources in the {@code ParameterBlock} are {@link
   *       RenderedImage} objects obtained from {@link GridCoverage2D#getRenderedImage()}.
  ...
  /**
   * Returns a sub-coordinate reference system for the specified dimension range. This method is for
   * internal use by {@link #resampleToCommonGeometry}.
   *
   ...
   * geometry} before to apply the {@linkplain #operation}. This method is invoked automatically by
   * the {@link #doOperation doOperation} method. Only the two-dimensional part is reprojected
   ...
   * {@link javax.media.jai.PlanarImage} are. This is because the extra dimensions don't need to be
   * compatible for all operations. For example if a source image is a slice in a time series, a
   * second source image could be a slice in the frequency representation of this time series.
   *
   ...
   * system} different than the default ones. For example if a subclass wants to force all images to
   * be referenced in a {@linkplain org.geotools.referencing.crs.DefaultGeographicCRS#WGS84 WGS 84}
   * CRS, then it may overrides this method as below:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * </blockquote>
   *
   * @param sources The source grid coverages to resample. This array is updated in-place as needed
   *     (for example if a grid coverage is replaced by a projected one).
   * @param crs2D The target coordinate reference system to use, or {@code null} for a default one.
   * @param gridToCrs2D The target "grid to coordinate reference system" transform, or {@code null}
   *     for a default one.
                ...
                ReferencingFactoryFinder.getCRSFactory(hints)
                    .createCompoundCRS(
                        Collections.singletonMap(
                            IdentifiedObject.NAME_KEY, crs2D.getName().getCode()),
                        components);
   ...
   * @return a {@link Map} with the properties generated by this {@link OperationJAI} or null if we
   *     haven't any.
   * @since 2.4
   */
   ...
   * quantitative category. If {@code categories} contains 0, 2 or more quantitative category, then
   ...
   * #deriveGridCoverage deriveGridCoverage} with a {@code bandLists} argument initialized as below:
   *
   * <p>
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>This method shall returns an array with a length equals to the number of bands in the target
   ...
   * deriveCategory} and {@link #deriveUnit deriveUnit} methods for each of them. Subclasses should
   * override this method if they know a more accurate algorithm for determining sample dimensions.
   *
   ...
   * automatically by the {@link #deriveSampleDimension deriveSampleDimension} method for each band
   * in the target image. The default implementation creates a default category from the target
   * range of values returned by {@link #deriveRange deriveRange}.
   *
   ...
   * the {@link #deriveCategory deriveCategory} method for each band in the target image. Subclasses
   * should override this method in order to compute the target range of values. For example, the
   * {@code "add"} operation may implements this method as below:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * target {@linkplain GridCoverage2D grid coverage}. This method is invoked automatically by the
   * {@link #deriveSampleDimension deriveSampleDimension} method for each band in the target image.
   * Subclasses should override this method in order to compute the target units from the source
   * units. For example a {@code "multiply"} operation may implement this method as below:
   *
   * <blockquote>
   *
   * <pre>
   ...
   *     For binary operations like {@code "add"} and {@code "multiply"}, this array has a length of
   *     2.
   * @param parameters Parameters, rendering hints and coordinate reference system to use.
   ...
   * </blockquote>
   *
   * Subclasses may override this method in order to invokes a different JAI operation according the
   * parameters.
   *
  ...
  protected static final class Parameters {
    /**
     * The two dimensional coordinate reference system for all sources and the destination {@link
     * GridCoverage2D}. Sources coverages will be projected in this CRS as needed.
     */
    ...
    /**
     * The rendering hints to be given to the {@link JAI#createNS} method. The {@link JAI} instance
     * to use for the {@code createNS} call will be fetch from the {@link Hints#JAI_INSTANCE} key.
     */
  ...
  /**
   * This method can be used for creating a property Map to set to the new coverage generated by the
   * current operation. Internally the method will search for ROI and NoData parameters set for the
   * operation and will report them as coverage properties
   *
   * @param properties
   * @param parameters
   ...
   * @return A {@link Map} containing all the coverage properties and also {@link ROI} and NoData if
   *     present
   */
