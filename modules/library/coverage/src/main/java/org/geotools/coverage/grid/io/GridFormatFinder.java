...
public final class GridFormatFinder {
  ...
  /**
   * Finds all avalaible implementations of {@link GridFormatFactorySpi} which have registered using
   * the services mechanism, and that have the appropriate libraries on the classpath.
   *
   ...
   *
   * <p>It can be used toget basic information about all the available {@link GridCoverage} plugins.
   * Note that this method finds all the implemented plugins but returns only the availaible one.
   *
   ...
   * @return an unmodifiable {@link Set} comprising all the {@link Format} that can read the {@link
   *     Object} o.
   */
  ...
  /**
   * Returns a {@link Format} that is able to read a certain object. If no {@link Format} is able to
   * read such an {@link Object} we return an {@link UnknownFormat} object.
   *
   ...
   * @return an {@link AbstractGridFormat} that has stated to accept this {@link Object} o or <code>
   *     null</code> in no plugins was able to accept it.
   */
