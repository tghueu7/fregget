...
public class ImageWorker {
   ...
   *
   * <p>{@link PNGImageWriter} does not support {@link ImageWriteParam#setCompressionMode(int)} set
   * to {@link ImageWriteParam#MODE_EXPLICIT}, it only allows {@link ImageWriteParam#MODE_DEFAULT}.
   *
   * @author Simone Giannecchini
   ...
   * default is {@link Boolean#TRUE TRUE}. The {@code FALSE} value is sometime useful for exporting
   * images to some formats that doesn't support tiling (e.g. GIF).
   *
   ...
   * #invalidateStatistics} were invoked. The {@link #getComputedProperty} method will not search a
   * property pass this point.
   */
  ...
  /**
   * Returns the {@linkplain #getRenderedImage rendered image} after null operation. This operation
   * may be used for setting new ImageProperties or for applying new RenderingHints.
   *
   * @return The rendered operation.
   * @see #getRenderedImage
   * @see #getPlanarImage
   ...
   * estimation of its {@linkplain #intensity intensity}. Next, this method {@linkplain #binarize()
  ...
  /**
   * Returns the <cite>region of interest</cite> currently set, or {@code null} if none. The default
   * value is {@code null}.
   *
  ...
  /**
   * Returns the <cite>NoData Range</cite> currently set, or {@code null} if none. The default value
   * is {@code null}.
   *
   ...
   * applies only to the next images to be computed; images already computed before this method call
  ...
  /**
   * Removes a rendering hint. Note that invoking this method is <strong>not</strong> the same than
   * invoking <code>{@linkplain #setRenderingHint setRenderingHint}(key, null)</code>. This is
   * especially true for the {@linkplain javax.media.jai.TileCache tile cache} hint:
   *
   * <p>
   *
   * <ul>
   ...
   *       specified by a previous rendering hint. All images to be computed after this method call
   *       will save their tiles in the {@linkplain JAI#getTileCache JAI default tile cache}.
   * </ul>
   *
   * @return This ImageWorker
   */
   ...
   *
   * <ul>
   *   <li>An {@linkplain ImageLayout image layout} with tiles size computed automatically from the
   *       current {@linkplain #image} size.
   *   <li>Any additional hints specified through the {@link #setRenderingHint} method. If the user
   *       provided explicitly a {@link JAI#KEY_IMAGE_LAYOUT} , then the user layout has precedence
   *       over the automatic layout computed in previous step.
   * </ul>
   *
   ...
   * ComponentColorModel component color model} of the specified data type. The data type is changed
   * only if no color model was explicitly specified by the user through {@link
   * #getRenderingHints()}.
   *
  ...
  /**
   * If {@code false}, disables the tile cache. Invoking this method with value {@code true} cancel
   * the last invocation with value {@code false}. If this method was invoking many time with value
   * {@code false}, then this method must be invoked the same amount of time with the value {@code
   * true} for reenabling the cache.
   *
   * <p><strong>Note:</strong> This method name doesn't contain the usual {@code set} prefix because
   * it doesn't really set a flag. Instead it increments or decrements a counter.
   *
   * @return This ImageWorker
   */
  ...
  /**
   * Returns the minimums and maximums values found in the image. Those extremas are returned as an
   * array of the form {@code double[2][#bands]}.
   */
  ...
  /**
   * Returns {@code true} if the {@linkplain #image} is binary. Such image usually contains only two
   * values: 0 and 1.
   *
   * @see #binarize()
   * @see #binarize(double)
  ...
  /**
   * Returns {@code true} if the {@linkplain #image} uses an {@linkplain IndexColorModel index color
   * model}.
   *
   ...
   * Note that a RGB color space doesn't mean that pixel values are directly stored as RGB
   ...
   * space}. Note that a GrayScale color space doesn't mean that pixel values are directly stored as
   * GrayScale component. The image may be {@linkplain #isIndexed indexed} as well.
   *
   ...
   * for each band, {@linkplain RescaleDescriptor rescale} them in the range {@code [0 .. 255]} and
  ...
  public final ImageWorker rescaleToBytes() {
    ...
    for (int i = 0; i < length; i++) {
              ...
              || (extrema[0][i] < -1E-6))) // the minimum is smaller than 0
      {
        ...
        // we do not rescale explicitly bu in case we have to, we relay on the clamping capabilities
        // of the format operator
        scale[i] = 1;
        offset[i] = 0;
      }
    }
   ...
   *
   * <p>If an {@link IndexColorModel} with a single transparency index is enough for you, we advise
   * you to take a look at {@link #forceIndexColorModelForGIF(boolean)} methdo.
   *
   * @see #isIndexed
   * @see #forceBitmaskIndexColorModel
   * @see #forceIndexColorModelForGIF
   ...
   * for the final image.
   *
   * <p><strong>Tip:</strong> For optimizing writing GIF, we need to create the image untiled. This
  ...
  public final ImageWorker forceColorSpaceIHS() {
    if (!isColorSpaceIHS()) {
      ...
      final ColorSpace ihs =
          isJaiExtEnabled() ? IHSColorSpaceJAIExt.getInstance() : IHSColorSpace.getInstance();
      ;
   ...
   * @param before <code>true</code> if we want to use first the provided image, <code>false</code>
   *     otherwise.
   * @param addAlpha <code>true</code> if we want to set the last image as alpha, <code>false</code>
   *     otherwise.
   * @return this {@link ImageWorker}.
   */
   ...
   * @param addAlpha <code>true</code> if we want to set the last image as alpha, <code>false</code>
   *     otherwise.
   * @param transformationList List of AffineTransformation that can be applied to the input rasters
   *     in order to repoject them to the same CRS.
   * @return this {@link ImageWorker}.
   */
   ...
   * color space}. If the current color space is already of {@linkplain ColorSpace#TYPE_GRAY type},
   ...
   * color space}, then this method just {@linkplain #retainFirstBand retain the first band} without
   * any further processing. Otherwise, this method performs a simple {@linkplain
   * BandCombineDescriptor band combine} operation on the {@linkplain #image} in order to come up
   * with a simple estimation of the intensity of the image based on the average value of the color
  ...
  /**
   * Retains inconditionnaly the first {@code numBands} of {@linkplain #image}. All other bands (if
   * any) are discarted without any further processing. This method does nothing if the current
   * {@linkplain #image} does not have a greater amount of bands than {@code numBands}.
   *
  ...
  /**
   * Binarizes the {@linkplain #image}. If the image is multi-bands, then this method first computes
   * an estimation of its {@linkplain #intensity intensity}. Then, the threshold value is set
  ...
  /**
   * Applies the specified mask over the current {@linkplain #image}. The mask should be {@linkplain
   * #binarize() binarized} - if it is not, this method will do it itself. Then, for every pixels in
   * the mask with value equals to {@code maskValue}, the corresponding pixel in the {@linkplain
   * #image} will be set to the specified {@code newValue}.
   *
   ...
   * source image of the corresponding position and band. See JAI {@link AddDescriptor} for details.
   *
  ...
  /**
   * Takes two rendered or renderable source images, and myltiply form each pixel the related value
   * of the second image, each one from each source image of the corresponding position and band.
   ...
   * bit-wise logical "xor" between every pixel in the same band of the source and the constant from
   * the corresponding array entry. See JAI {@link XorConstDescriptor} for details.
   *
  ...
  /**
   * Takes two rendered or renderable source images, and subtract form each pixel the related value
   * of the second image, each one from each source image of the corresponding position and band.
  ...
  /**
   * Adds transparency to a preexisting image whose color model is {@linkplain IndexColorModel index
   * color model}. For all pixels with the value {@code false} in the specified transparency mask,
   * the corresponding pixel in the {@linkplain #image} is set to the transparent pixel value. All
  ...
  /**
   * Adds transparency to a preexisting image whose color model is {@linkplain IndexColorModel index
   * color model}. First, this method creates a new index color model with the specified {@code
   * transparent} pixel, if needed (this method may skip this step if the specified pixel is already
   * transparent. Then for all pixels with the value {@code false} in the specified transparency
   * mask, the corresponding pixel in the {@linkplain #image} is set to that transparent value. All
   * other pixels are left unchanged.
   *
   * @param alphaChannel The mask to apply as a {@linkplain #binarize() binarized} image.
   * @param translucent {@code true} if {@linkplain Transparency#TRANSLUCENT translucent} images are
   *     allowed, or {@code false} if the resulting images must be a {@linkplain
   ...
   * 'getRenderingHints()' failed to suggest a tile size. This method is for internal use by {@link
   * #write} methods only.
   *
   * @return this {@link ImageWorker}.
   */
  ...
  public ImageWorker applyOpacity(float opacity) {
    ...
    if (colorModel instanceof IndexColorModel) {
      ...
      if (!expanded.getColorModel().hasAlpha()) {
        ...
        pb.set(
            new Range[] {nodata, nodata == null ? null : RangeFactory.create(alpha - 1, alpha - 1)},
            0);
        if (isNoDataNeeded()) {
          if (background != null && background.length > 0) {
            double dest = background[0];
  ...
  /**
   * Builds a lookup table that is the identity on all bands but the alpha one, where the opacity is
   * applied
   *
   * @param opacity
   * @param bands
   ...
   *   <li>If the image writer accepts {@link File} objects as input, then the {@code file} argument
   *       is given directly without creating an {@link ImageOutputStream} object. This is important
   *       for some formats like HDF, which work <em>only</em> with files.
   *   <li>If the {@linkplain #image} is not tiled, then it is tiled prior to be written.
   *   <li>If some special processing is needed for a given format, then the corresponding method is
   *       invoked. Example: {@link #forceIndexColorModelForGIF}.
   * </ul>
   *
   * @return this {@link ImageWorker}.
   */
   ...
   * @return this {@link ImageWorker}.
   * @throws IOException In case an error occurs during the search for an {@link ImageOutputStream}
  ...
  public final void writePNG(
    ...
    if (paletted && !hasPalette) {
      ...
      if (!hasColorModel && !hasPalette) {
        ...
        // png supports gray, rgb, rgba and paletted 8 bit, but not, for example, double and float
        // values, or 16 bits palettes
        forceComponentColorModel();
      }
    }
    ...
    if (!nativeAcc || writer == null) {
      ...
      while (it.hasNext()) {
        ...
        // let me check if the native writer can encode this image (paranoiac checks this was
        // already performed by the ImageIO search
    ...
    LOGGER.fine(
        "Using ImageIO Writer with SPI: " + originatingProvider.getClass().getCanonicalName());
   ...
   * destination, compression and compression rate.
   *
   * <p>It is worth to point out that the only compressions algorithm availaible with the jdk {@link
   * GIFImageWriter} is "LZW" while the compression rates have to be confined between 0 and 1. AN
   * acceptable values is usally 0.75f.
   *
   * <p>The destination object can be anything providing that we have an {@link
   * ImageOutputStreamSpi} that recognizes it.
   *
  ...
  public final void writeJPEG(
    ...
    if (writer == null) {
      if (JDK_JPEG_IMAGE_WRITER_SPI == null) {
        throw new IllegalStateException(
            Errors.format(ErrorKeys.ILLEGAL_CLASS_$2, "Unable to find JDK JPEG Writer"));
      }
  ...
  /**
   * Performs an affine transform on the image, applying optimization such as affine removal in case
   * the affine is an identity, affine merging if the affine is applied on top of another affine,
   * and using optimized operations for integer translates
   *
   * @param tx
   * @param interpolation
   * @param bgValues
      ...
      if (WARP_REDUCTION_ENABLED
        ...
        try {
          ...
          MathTransform2D chained =
              (MathTransform2D)
                  factory.createConcatenatedTransform(affineMT.inverse(), originalTransform);
          ...
          if (warp instanceof WarpGrid
            ...
            // Boolean indicating if NoData are the same as for the source operation or are not
            // present
            Range oldNoData =
                (Range)
              ...
              if (roi != null) {
                ...
                try {
                  ...
                  if (LOGGER.isLoggable(Level.WARNING)) {
                    LOGGER.log(
                        Level.WARNING, "Unable to compute the inverse of the new ROI provided", e);
                  }
  ...
  private Color getBackgroundColor() {
    ...
    if (background.length == 3) {
      ...
      return new Color(
          (int) background[0], (int) background[1], (int) background[2], (int) background[3]);
    } else {
      return null;
    }
  }
    ...
    if (images != null && images.length > 0) {
      ...
      if (cmref instanceof IndexColorModel
        ...
        if (icm.hasAlpha()) {
          ...
          color =
              new Color(
                  icm.getRed(index), icm.getGreen(index), icm.getBlue(index), icm.getAlpha(index));
        }
  ...
  public ImageWorker scale(
    ...
    // getting the new ROI property
    PropertyGenerator gen =
        getOperationDescriptor("Scale").getPropertyGenerators(RenderedRegistryMode.MODE_NAME)[0];
    Object prop = gen.getProperty("roi", image);
    if (prop != null && prop instanceof ROI) {
      setROI((ROI) prop);
    } else {
  ...
  /**
   * Forces all NODATA pixels, as well as those outside of the ROI, to be transparent (expanding the
   * color model as needed in order to make it so). In case the image has no ROI or no nodata, the
   * method won't perform any change
   */
  public ImageWorker prepareForRendering() {
    ...
    if (cm instanceof IndexColorModel) {
      ...
      // we did not find the background color, well we have to expand to RGB and then tell Mosaic to
      // use the RGB(A) color as the
      // background
      if (bgColorIndex == -1) {
        ...
        // we found the background color in the original image palette therefore we set its index as
        // the bkg value.
        // The final Mosaic will use the IndexColorModel of this image anywa, therefore all we need
        // to do is to force
      ...
      if (ccm.getNumColorComponents() == 1) {
        if ((ccm.getTransferType() == DataBuffer.TYPE_DOUBLE
          ...
          if (hasAlpha) {
            ...
            image =
                iw.bandMerge(3).forceComponentColorModel().forceColorSpaceRGB().getRenderedImage();
          }
        } else {
      ...
      if (bgValues == null) {
        if (hasAlpha) {
          ...
          if (nodata != null) {
            ...
            if (numBands == 1) {
              alpha = new ImageWorker(this.image).rangeLookup(lookupTable).getRenderedImage();
            } else {
    ...
    double[][] thresholds =
        new double[][] {{ColorUtilities.getThreshold(image.getSampleModel().getDataType())}};
    // apply the mosaic
   ...
   * {@linkplain javax.swing.JTree tree}. This method is provided mostly for debugging purpose. This
   * method requires the {@code gt2-widgets-swing.jar} file in the classpath.
   *
   ...
   * reference to this image, the garbage collector discovers this, and finalize is called. This can
   * be used as a hint in situations where waiting for garbage collection would be overly
   * conservative.
   *
   * <p>Mind, this also results in disposing the JAI Image chain attached to the image the worker is
   * applied to, so don't call this method on image changes (full/partial) that you want to use.
   *
   * <p>{@link ImageWorker} defines this method to remove the image being disposed from the list of
   ...
   * method is mostly as a convenient way to test operation chains. This method can be invoked from
   * the command line. If an optional {@code -operation} argument is provided, the Java method (one
   * of the image operations provided in this class) immediately following it is executed. Example:
   *
   * <blockquote>
   *
   * <pre>
