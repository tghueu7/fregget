...
public class GeneralGridGeometry implements GridGeometry, Serializable {
   ...
   * {@link RenderedImage}. A grid with 512 cells can have a minimum coordinate of 0 and maximum of
  ...
  /**
   * The math transform (usually an affine transform), or {@code null} if none. This math transform
   * maps {@linkplain PixelInCell#CELL_CENTER pixel center} to "real world" coordinate using the
   * following line:
   *
   ...
   *     (pixel's <em>center</em>) to real world earth coordinates. May be {@code null}, but this is
   *     not recommanded.
   ...
   * using {@link CRSUtilities#transform(MathTransform, Envelope)}.
   */
   ...
   * </code> value for this parameter since we would not force to create a new raster that might be
   * slightly bigger than the original one, causing problems with black or nodata collars appearing
   * after a resample. In case we are trying to build a source raster area to be used at reading
   * time, in that case we may want to provide <code>true</code> for gridMaxInclusive since we may
   * want to read an area that is slightly bigger than what we need to be sure we actually reading
   * something.
   *
   * @param anchor {@link PixelInCell#CELL_CENTER CELL_CENTER} for OGC conventions or {@link
   *     PixelInCell#CELL_CORNER CELL_CORNER} for Java2D/JAI conventions.
  ...
  public GeneralGridGeometry(
    ...
    // making this inclusive may cause problems when we use for warping of something since we might
    ...
    if (preserveGridToCRS) {
      ...
      if (gridToCRS != null) {
        ...
        // all we need is a simple scale and translate to take care of the fact that we might have
        // approximated a bit the
        // raster area
   ...
   *     contains entirely all pixels, i.e. the rectangle's upper left corner must coincide with the
   *     upper left corner of the first pixel and the rectangle's lower right corner must coincide
   *     with the lower right corner of the last pixel.
   * @throws MismatchedDimensionException if the grid range and the envelope doesn't have consistent
   *     dimensions.
   * @since 2.2
   */
   ...
   *     {@linkplain #isDefined isDefined}({@linkplain #GRID_RANGE_BITMASK})</code> returned {@code
   *     false}).
   ...
   *     {@linkplain #isDefined isDefined}({@linkplain #GRID_TO_CRS_BITMASK})</code> returned {@code
   *     false}).
   ...
   * {@link #getGridToCRS()} except that the transform may maps other parts than {@linkplain
   ...
   * @throws IllegalArgumentException if the specified bitmask is not a combinaison of known masks.
   * @since 2.2
