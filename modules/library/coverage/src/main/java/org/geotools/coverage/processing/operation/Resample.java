...
public class Resample extends Operation2D {
  ...
  public Coverage doOperation(final ParameterValueGroup parameters, final Hints hints) {
    ...
    final Interpolation interpolation =
        ImageUtilities.toInterpolation(parameters.parameter("InterpolationType").getValue());
    CoordinateReferenceSystem targetCRS =
        (CoordinateReferenceSystem) parameters.parameter("CoordinateReferenceSystem").getValue();
    if (targetCRS == null) {
   ...
   * method for computing the {@link #GRID_GEOMETRY} argument of a {@code "resample"} operation from
   * an envelope. The target envelope may contains a different coordinate reference system, in which
   * case a reprojection will be performed.
   *
  ...
  public static GridGeometry computeGridGeometry(final GridCoverage source, final Envelope target)
    ...
    if (targetCRS == null || CRS.equalsIgnoreMetadata(reducedCRS, targetCRS)) {
      ...
      try {
        ...
        transformed =
            CRS.transform(
                CRS.getCoordinateOperationFactory(true).createOperation(targetCRS, reducedCRS),
                target);
