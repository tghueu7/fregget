...
public class GridCoverageFactory extends AbstractFactory {
  ...
  /**
   * Returns the default coordinate reference system to use when no CRS were explicitly specified by
   * the user. If a {@link Hints#DEFAULT_COORDINATE_REFERENCE_SYSTEM
   * DEFAULT_COORDINATE_REFERENCE_SYSTEM} hint were provided at factory construction time, then the
   * specified CRS is returned. Otherwise, the default implementation returns {@link
   ...
   * @param gridGeometry The grid geometry. The {@linkplain GridGeometry2D#getGridRange grid range}
   ...
   * @param raster The data (may be floating point numbers). {@linkplain Float#NaN NaN} values are
   *     mapped to a transparent color.
   * @param envelope The grid coverage cordinates and its CRS. This envelope must have at least two
   *     dimensions. The two first dimensions describe the image location along <var>x</var> and
   *     <var>y</var> axis. The other dimensions are optional and may be used to locate the image on
   *     a vertical axis or on the time axis.
   ...
   * @param colors The colors to use for values from {@code minValues} to {@code maxValues} for each
   *     bands, or {@code null} for a default color palette. If non-null, each arrays {@code
   *     colors[b]} may have any length; colors will be interpolated as needed.
   * @param hints An optional set of rendering hints, or {@code null} if none. Those hints will not
   *     affect the grid coverage to be created. The optional {@link Hints#SAMPLE_DIMENSION_TYPE
   *     SAMPLE_DIMENSION_TYPE} hint specifies the {@link SampleDimensionType} to be used at
   *     rendering time, which can be one of {@link SampleDimensionType#UNSIGNED_8BITS
   *     UNSIGNED_8BITS} or {@link SampleDimensionType#UNSIGNED_16BITS UNSIGNED_16BITS}.
   * @return The new grid coverage.
   * @since 2.2
   */
  public GridCoverage2D create(
   ...
   *     colors[b]} may have any length; colors will be interpolated as needed.
   * @param hints An optional set of rendering hints, or {@code null} if none. Those hints will not
   *     affect the grid coverage to be created. However, they may affect the grid coverage to be
   *     returned by <code>{@link GridCoverage2D#geophysics
   ...
   * describe the image location along <var>x</var> and <var>y</var> axis. The other dimensions are
  ...
  /**
   * Constructs a grid coverage from the specified {@linkplain RenderedImage image} and {@linkplain
   * Envelope envelope}. A default set of {@linkplain GridSampleDimension sample dimensions} is
   * used. The {@linkplain CoordinateReferenceSystem coordinate reference system} is inferred from
   * the supplied envelope.
   *
   * <p>The envelope must have at least two dimensions. The two first dimensions describe the image
   * location along <var>x</var> and <var>y</var> axis. The other dimensions are optional and may be
   * used to locate the image on a vertical axis or on the time axis.
   *
   * @param name The grid coverage name.
  ...
  /**
   * Constructs a grid coverage from the specified {@linkplain RenderedImage image} and {@linkplain
   * Envelope envelope}. An {@linkplain AffineTransform affine transform} will be computed
   * automatically from the specified envelope using heuristic rules described below.
   *
   * <p>This convenience constructor assumes that axis order in the supplied image matches exactly
   * axis order in the supplied envelope. In other words, in the usual case where axis order in the
   * image is (<var>column</var>, <var>row</var>), then the envelope should probably have a
   ...
   * EAST}|{@link AxisDirection#WEST WEST}). An example of such CRS is {@code EPSG:4326}. This
   * convenience constructor will interchange automatically the (<var>y</var>,<var>x</var>) axis for
   * such CRS.
   *
   ...
   * @param name The grid coverage name.
   * @param image The image.
   * @param envelope The grid coverage cordinates. This envelope must have at least two dimensions.
   *     The two first dimensions describe the image location along <var>x</var> and <var>y</var>
   *     axis. The other dimensions are optional and may be used to locate the image on a vertical
   *     axis or on the time axis.
   * @param bands Sample dimensions for each image band, or {@code null} for default sample
   *     dimensions. If non-null, then this array's length must matches the number of bands in
   *     {@code image}.
  ...
  public GridCoverage2D create(
    ...
    final GridGeometry2D gm =
        new GridGeometry2D(new GeneralGridEnvelope(image, envelope.getDimension()), envelope);
  ...
  /**
   * Constructs a grid coverage from the specified {@linkplain RenderedImage image} and {@linkplain
   * GridGeometry2D grid geometry}. The {@linkplain Envelope envelope} (including the {@linkplain
   * CoordinateReferenceSystem coordinate reference system}) is inferred from the grid geometry.
   *
   ...
   *     envelope} with its {@linkplain GridGeometry2D#getCoordinateReferenceSystem coordinate
   *     reference system} and a "{@linkplain GridGeometry2D#getGridToCoordinateSystem grid to CRS}"
   *     transform).
   * @param bands Sample dimensions for each image band, or {@code null} for default sample
   *     dimensions. If non-null, then this array's length must matches the number of bands in
   *     {@code image}.
