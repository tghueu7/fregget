...
public final class EfficientInverseColorMapComputation {
  ...
  /**
   * {@link EfficientInverseColorMapComputation} that allows us to specify the number of bits we are
   * going to save from the quantization.
   *
   * @param rgbColorMap
  ...
  public EfficientInverseColorMapComputation(byte[][] rgbColorMap, final int quantizationBits) {
    ...
    mapBuf =
        new byte[maximumQuantizationValue * maximumQuantizationValue * maximumQuantizationValue];
    if (mapBuf.length <= 0)
      throw new IllegalArgumentException("Illegal number of quantization colors");
    final int[] distBuf =
        new int[maximumQuantizationValue * maximumQuantizationValue * maximumQuantizationValue];
