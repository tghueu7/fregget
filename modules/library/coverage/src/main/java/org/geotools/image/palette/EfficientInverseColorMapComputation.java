...
public final class EfficientInverseColorMapComputation {
    ...
    public EfficientInverseColorMapComputation(byte[][] rgbColorMap,
        ...
        final int[] distBuf = new int[maximumQuantizationValue * maximumQuantizationValue * 
                maximumQuantizationValue];
        ...
        for (int i = 0; i < numberOfColors; ++i) {
            ...
            //
            // //
            for (int r = 0, rxx = rinc, rgbI = 0; r < maximumQuantizationValue; rdist += rxx, 
                    ++r, rxx += txsqr) {
                gdist = rdist;
                for (int g = 0, gxx = ginc; g < maximumQuantizationValue; gdist += gxx, ++g, gxx 
                        += txsqr) {
                    bdist = gdist;
                    for (int b = 0, bxx = binc; b < maximumQuantizationValue; bdist += bxx, ++b, 
                            ++rgbI, bxx += txsqr) {
