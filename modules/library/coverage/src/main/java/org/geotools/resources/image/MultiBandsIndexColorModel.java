...
final class MultiBandsIndexColorModel extends IndexColorModel {
   ...
   * are set to the same value. Replicating the pixel value is a somewhat arbitrary choice, but this
   * choice make this image appears as a gray scale image if the underlying {@link DataBuffer} were
   * displayed again with a RGB color model instead of this one. Such a gray scale image seems more
   * neutral than an image where only the Red component would vary.
   *
   * <p>All other {@code getDataElement} methods in this color model are ultimately defined in terms
