...
public abstract class AbstractCoverage extends PropertySourceImpl implements Coverage {
   ...
   * @param propertySource The source for this coverage, or {@code null} if none. Source may be (but
   *     is not limited to) a {@link PlanarImage} or an other {@code AbstractCoverage} object.
  ...
  /**
   * Returns the coverage name, or {@code null} if none. The default implementation returns the name
  ...
  /**
   * Returns the coordinate reference system to which the objects in its domain are referenced. This
   * is the CRS used when accessing a coverage or grid coverage with the {@code evaluate(...)}
   * methods. This coordinate reference system is usually different than coordinate system of the
   * grid. It is the target coordinate reference system of the {@link
   ...
   * associated coordinate reference system. For grid coverages, the grid cells are centered on each
   * grid coordinate. The envelope for a 2-D grid coverage includes the following corner positions.
   *
   * <blockquote>
   *
   * <pre>
  ...
  /**
   * Describes the range of the coverage. It consists of a list of attribute name/data type pairs. A
   * simple list is the most common form of range type, but {@code RecordType} can be used
   ...
   * field of the range type. If {@code list} is null, the operation shall return a value for every
   * field of the range type. Otherwise, it shall return a value for each field included in {@code
   * list}. If the direct position passed is not in the domain of the coverage, then an exception is
   * thrown. If the input direct position falls within two or more geometric objects within the
   * domain, the operation shall return records of feature attribute values computed according to
   * the {@linkplain #getCommonPointRule common point rule}.
   *
   ...
   * interpolation type used when accessing grid values for points which fall between grid cells is
   * {@linkplain javax.media.jai.InterpolationNearest nearest neighbor}, but it can be changed by
   * some {@linkplain org.geotools.coverage.grid.Interpolator2D subclasses}. The CRS of the point is
   * the same as the grid coverage {@linkplain #getCoordinateReferenceSystem coordinate reference
   * system}.
   *
   ...
   *     invalid coordinates.
   * @throws CannotEvaluateException if the values can't be computed at the specified coordinate for
   *     an other reason. It may be thrown if the coverage data type can't be converted to {@code
   *     boolean} by an identity or widening conversion. Subclasses may relax this constraint if
   *     appropriate.
   */
   ...
   * @throws CannotEvaluateException if the values can't be computed at the specified coordinate for
   *     an other reason. It may be thrown if the coverage data type can't be converted to {@code
   *     byte} by an identity or widening conversion. Subclasses may relax this constraint if
   *     appropriate.
   */
   ...
   * @throws CannotEvaluateException if the values can't be computed at the specified coordinate for
   *     an other reason. It may be thrown if the coverage data type can't be converted to {@code
   *     int} by an identity or widening conversion. Subclasses may relax this constraint if
   *     appropriate.
   */
   ...
   * @throws CannotEvaluateException if the values can't be computed at the specified coordinate for
   *     an other reason. It may be thrown if the coverage data type can't be converted to {@code
   *     float} by an identity or widening conversion. Subclasses may relax this constraint if
   *     appropriate.
   */
   ...
   * @throws CannotEvaluateException if the values can't be computed at the specified coordinate for
   *     an other reason. It may be thrown if the coverage data type can't be converted to {@code
   *     double} by an identity or widening conversion. Subclasses may relax this constraint if
   *     appropriate.
   */
  ...
  protected class Renderable extends PropertySourceImpl implements RenderableImage, ImageFunction {
     ...
     * {@linkplain AbstractCoverage#getDimension coverage's dimension}. The {@linkplain #xAxis x}
     * and {@link #yAxis y} ordinates will be ignored, since they will vary for each pixel to be
     * evaluated. Other ordinates, if any, should be set to a fixed value. For example a coverage
     * may be three-dimensional, where the third dimension is the time axis. In such case, {@code
     * coordinate.ord[2]} should be set to the point in time where to evaluate the coverage. By
     * default, all ordinates are initialized to 0. Subclasses should set the desired values in
    ...
    /**
     * Returns {@code true} if successive renderings with the same arguments may produce different
     * results. The default implementation returns {@code false}.
     *
     ...
     * {@linkplain AbstractCoverage#getEnvelope coverage's envelope} minimal value for the
     ...
     *
     * <p>The default implementation creates a render context with {@link #createRenderContext} and
     * invokes {@link #createRendering(RenderContext)}.
     *
     ...
     * ImageFunctionDescriptor ImageFunction}" operation if possible (i.e. if the area of interect
     * is rectangular and the affine transform contains only translation and scale coefficients).
     *
    ...
    /**
     * Initializes a render context with an affine transform that maps the coverage envelope to the
     * specified destination rectangle. The affine transform mays swap axis in order to normalize
     * their order (i.e. make them appear in the (<var>x</var>,<var>y</var>) order), so that the
     * image appears properly oriented when rendered.
     *
     * @param gridBounds The two-dimensional destination rectangle.
     * @param hints The rendering hints, or {@code null} if none.
     * @return A render context initialized with an affine transform from the coverage to the grid
     *     coordinate system. This transform is the inverse of {@link
    ...
    /**
     * Returns the number of elements per value at each position. This is the maximum value plus 1
     * allowed in {@code getElements(...)} methods invocation. The default implementation returns
     ...
     * rendered image is created using the "{@link ImageFunctionDescriptor ImageFunction}" operator
     * and the image type is not {@code double}. The default implementation invokes {@link
     * AbstractCoverage#evaluate(DirectPosition,float[])} recursively.
     */
     ...
     * automatically invoked at rendering time for populating an image tile, providing that the
     * rendered image is created using the "{@link ImageFunctionDescriptor ImageFunction}" operator
     * and the image type is {@code double}. The default implementation invokes {@link
  ...
  /**
   * Display this coverage in a windows. This convenience method is used for debugging purpose. The
   * exact appareance of the windows and the tools provided may changes in future versions.
   *
  ...
  /**
   * Displays this coverage in a windows. This convenience method is used for debugging purpose. The
   * exact appareance of the windows and the tools provided may changes in future versions.
   */
  public void show() {
    show(null);
  ...
  /**
   * Returns a string representation of this coverage. This string is for debugging purpose only and
  ...
  /**
   * Provides a hint that a coverage will no longer be accessed from a reference in user space. This
   ...
   * program that depended on the safety checks for its correctness. In case of doubt about whatever
   * this coverage still in use or not, it is safer to rely on the garbage collector.
   *
   ...
   * @return {@code true} if this method disposed at least some resources, or {@code false} if this
   *     method vetoed against the disposal.
