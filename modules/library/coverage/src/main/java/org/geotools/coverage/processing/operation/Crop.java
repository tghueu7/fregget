...
public class Crop extends Operation2D {
  ...
  /**
   * The parameter descriptor used to pass this operation the envelope to use when doing the spatial
   * crop.
   */
  ...
  /**
   * The parameter descriptor used to tell this operation to force the usage of a mosaic by avoiding
    ...
    try {
      ...
      if (cropROI != null) {
        ...
        try {
          ...
          // in case the intersection fail, accept using intersection(cropEnvelope, envelope(roi)),
          // as the
          // ROI will do the rest (we'll just carry around a larger image but pixels only get out
          // within the ROI)
      ...
      double[] background =
          destnodata != null ? destnodata : CoverageUtilities.getBackgroundValues(sourceCoverage);
      ...
      if (!isSimpleTransform || cropROI != null) {
        ...
        if (forceMosaic || cropROI != null || internalROI != null || nodata != null) {
          ...
          if (cropROI != null) {
            Geometry txROI =
                JTS.transform(cropROI, ProjectiveTransform.create(sourceWorldToGridTransform));
  ...
  /**
   * Stop gap measure to get a ROI that can scale up to massive images, until ROIGeometry gets fixed
   * to be a good ROIShape replacement
   *
   * @param theGeom
   * @return
   */
