...
public class Crop extends Operation2D {
     ...
     */
    public static final ParameterDescriptor<Envelope> CROP_ENVELOPE = new 
            DefaultParameterDescriptor<Envelope>(
     ...
     */
    public static final ParameterDescriptor<Geometry> CROP_ROI = new 
            DefaultParameterDescriptor<Geometry>(
     ...
     */
    public static final ParameterDescriptor<Double> ROI_OPTIMISATION_TOLERANCE = new 
     ...
     */
    public static final ParameterDescriptor<Boolean> FORCE_MOSAIC = new 
     ...
     */
    public static final ParameterDescriptor<double[]> DEST_NODATA = new 
            DefaultParameterDescriptor<double[]>(
     ...
     *
     * @see org.geotools.coverage.processing.AbstractOperation#doOperation(org.opengis.parameter
     * .ParameterValueGroup,
    ...
    public Coverage doOperation(ParameterValueGroup parameters, Hints hints) {
        ...
        double[] destnodata = (double[]) parameters.parameter(Crop.PARAMNAME_DEST_NODATA)
                .getValue();
        ...
        if (cropRoi != null && cropEnvelope == null) {
            Envelope e2d = JTS.getEnvelope2D(cropRoi.getEnvelopeInternal(), source
        ...
        // intersect
        if (cropRoi != null) {
            final Geometry jis = JTS.toGeometry((com.vividsolutions.jts.geom.Envelope) new 
        ...
        //
        // //
        final AffineTransform sourceCornerGridToWorld = (AffineTransform) ((GridGeometry2D) 
                source.getGridGeometry()).getGridToCRS(PixelInCell.CELL_CORNER);
        ...
        if (cropRoi != null || !intersectionEnvelope.equals(sourceEnvelope, tolerance / 2.0, 
                false)) {
    ...
    private static GridCoverage2D buildResult(
        ...
        try {
            ...
            if (cropROI != null) {
                ...
                // Remember that envelope(intersection(roi,cropEnvelope)) != intersection
                // (cropEnvelope, envelope(roi))
                try {

                    final Polygon modelSpaceROI = FeatureUtilities.getPolygon(cropEnvelope, 
                            GFACTORY);
                    ...
                    // envelope(roi)), as the
                    // ROI will do the rest (we'll just carry around a larger image but pixels 
                    // only get out within the ROI)
                    com.vividsolutions.jts.geom.Envelope cropROIEnvelope = cropROI
                            .getEnvelopeInternal();
                    com.vividsolutions.jts.geom.Envelope intersection = cropROIEnvelope
                            .intersection(ReferencedEnvelope.reference(cropEnvelope));
                    cropEnvelope.setEnvelope(new GeneralEnvelope(new ReferencedEnvelope
                            (intersection, cropEnvelope.getCoordinateReferenceSystem())));
                }
            }
            ...
            // //
            //
            // finalRasterArea will hold the smallest rectangular integer raster area that 
            // contains the floating point raster
            // area which we obtain when applying the world-to-grid transform to the cropEnvelope
            // . Note that we need to intersect
            // such an area with the area covered by the source coverage in order to be sure we 
            // do not try to crop outside the
            ...
            if (finalRasterArea.isEmpty()) {
                throw new EmptyIntersectionException("Crop envelope intersects in model space, " +
                        "but not in raster space");
            }
            ...
            double[] background = destnodata != null ? destnodata : CoverageUtilities
            ...
            if (!isSimpleTransform || cropROI != null) {
                ...
                if (forceMosaic || cropROI != null || internalROI != null || nodata != null) {
                    ...
                    ROI[] roiarr = null;
                    if (cropROI != null) {
                        Geometry txROI = JTS.transform(cropROI, ProjectiveTransform.create
                                (sourceWorldToGridTransform));
                    ...
                    // we do not have to crop in this case (should not really happen at
                    // this time)
                    if (!forceMosaic && bounds.getBounds().equals(sourceGridRange) && 
                            isSimpleTransform && nodata == null && roiarr == null)
                        return sourceCoverage;
                    ...
                    worker.mosaic(new RenderedImage[]{sourceImage}, MosaicDescriptor
                            .MOSAIC_TYPE_OVERLAY, null, roiarr, null, nodata != null ? new 
                            Range[]{nodata} : null);
                }
    ...
    /**
     * Initialize a layout object using the provided {@link RenderedImage} and the provided 
     * {@link Hints}.
     *
    ...
    private static ImageLayout initLayout(
        ...
        if ((layout.getValidMask() &
                (ImageLayout.TILE_WIDTH_MASK
                        ...
                        | ImageLayout.TILE_GRID_X_OFFSET_MASK | ImageLayout
                        .TILE_GRID_Y_OFFSET_MASK)) == 0) {
