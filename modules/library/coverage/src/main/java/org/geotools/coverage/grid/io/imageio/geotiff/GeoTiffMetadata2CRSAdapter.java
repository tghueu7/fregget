...
public final class GeoTiffMetadata2CRSAdapter {
  ...
  /**
   * This method creates a <code>CoordinateReferenceSystem</code> object from the metadata which has
   * been set earlier. If it cannot create the <code>CoordinateReferenceSystem</code>, then one of
   ...
   * @throws NullPointerException if the <code>csAuthorityFactory</code>, <code>datumFactory</code>,
   *     <code>crsFactory</code> or <code>metadata</code> are uninitialized
   * @throws UnsupportedOperationException if the coordinate system specified by the GeoTiff file is
   *     not supported.
   */
  public CoordinateReferenceSystem createCoordinateSystem(final GeoTiffIIOMetadataDecoder metadata)
      throws Exception {
   ...
   * @throws FactoryException
   */
  private ProjectedCRS createProjectedCoordinateReferenceSystem(GeoTiffIIOMetadataDecoder metadata)
      throws Exception {
  ...
  /**
   * Creation of a geographic coordinate reference system as specified in the GeoTiff specification.
   * User defined values are supported for all the possible levels of the above mentioned
   * specification.
   *
  ...
  private GeographicCRS createGeographicCoordinateReferenceSystem(
    ...
    if (tempCode == null || tempCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
      ...
      try {
        ...
        gcs =
            (GeographicCRS)
                allAuthoritiesFactory.createCoordinateReferenceSystem(geogCode.toString());
        if (angularUnit != null
  ...
  /**
   * Getting a specified geotiff geo key as a int. It is somehow tolerant in the sense that in case
   * such a key does not exist it retrieves 0.
   *
  ...
  /**
   * Create the grid to world (or raster to model) transformation for this source respecting ALWAYS
   * the OGC {@link PixelInCell#CELL_CENTER} convention for the {@link ImageDatum} of the underlying
   * {@link ImageCRS}.
   *
   * @see <a href="http://lists.maptools.org/pipermail/geotiff/2006-January/000213.html">this email
   *     post</a>
  ...
  public static MathTransform getRasterToModel(final GeoTiffIIOMetadataDecoder metadata)
    ...
    try {
      ...
      if (LOGGER.isLoggable(Level.FINE)) {
        LOGGER.log(
            Level.FINE, "Invalid model transformation found!\n" + e.getLocalizedMessage(), e);
      }
  ...
  /**
   * Creating a prime meridian for the gcs we are creating at an higher level. As usual this method
   * tries to follow the geotiff specification.
   *
        ...
        if (pmCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
          try {
            ...
            final IOException io =
                new GeoTiffException(metadata, "Invalid user-defined prime meridian spec.", nfe);
   ...
   * definition of the geodetic datum includes both an ellipsoid and a prime meridian, but the code
   * in the GeoTIFFWritingUtilities file does NOT include the prime meridian, as it is specified
   * separately. This code currently does not support user defined datum.
   *
   ...
   *
   * <p>I had no time to implement support for all of them therefore you will not find all of them.
   * If you want go ahead and add support for the missing ones. I have tested this code against some
   * geotiff files you can find on the geotiff website under the ftp sample directory but I can say
   * that they are a real mess! I am respecting the specification strictly while many of those
   * fields do not! I could make this method trickier and use workarounds in order to be less strict
   * but I will not do this, since I believe it is may lead us just on a very dangerous path.
   *
   * @param name
  ...
  /**
   * Getting the origin long with a minimum of tolerance with respect to the parameters name. I saw
   ...
   * ProjLinearUnitsGeoKey</code> and the <code>ProjLinearUnitSizeGeoKey</code>. The unit may either
   * be specified as a standard EPSG recognized unit, or may be user defined.
   *
   * @param key
