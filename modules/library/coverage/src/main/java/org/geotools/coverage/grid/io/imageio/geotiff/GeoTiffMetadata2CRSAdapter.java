...
public final class GeoTiffMetadata2CRSAdapter {
    ...
    private final static Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (GeoTiffMetadata2CRSAdapter.class);
     ...
     * @throws UnsupportedOperationException if the coordinate system specified by the GeoTiff 
     * file is not
     *                                       supported.
     */
    ...
    private ProjectedCRS createProjectedCoordinateReferenceSystem(
        ...
        try {
            ...
            final ProjectedCRS pcrs = (ProjectedCRS) allAuthoritiesFactory
                    .createCoordinateReferenceSystem(projCode.toString());
            // //
            //
            // We have nothing to do with the unit of measure
            //
            // //
            boolean isDefaultUnit = metadata.getGeoKey(GeoTiffPCSCodes.ProjLinearUnitsGeoKey) == 
                    null
            ...
            return new DefaultProjectedCRS(
                    java.util.Collections.singletonMap("name", DefaultEllipsoidalCS.getName(pcrs,
                            new CitationImpl("EPSG"))),
                    conversionFromBase,
                    pcrs.getBaseCRS(),
    ...
    private GeographicCRS createGeographicCoordinateReferenceSystem(
        ...
        try {
            angularUnit = createUnit(GeoTiffGCSCodes.GeogAngularUnitsGeoKey, GeoTiffGCSCodes
                    .GeogAngularUnitSizeGeoKey, SI.RADIAN, NonSI.DEGREE_ANGLE, metadata);
        } catch (GeoTiffException e) {
        ...
        try {
            linearUnit = createUnit(GeoTiffGCSCodes.GeogLinearUnitsGeoKey, GeoTiffGCSCodes
                    .GeogLinearUnitSizeGeoKey, SI.METER, SI.METER, metadata);
        } catch (GeoTiffException e) {
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.log(Level.FINE, e.getLocalizedMessage(), e);
            }
     ...
     * email post</a>
     */
    public static MathTransform getRasterToModel(final GeoTiffIIOMetadataDecoder metadata) throws
            GeoTiffException {
        ...
        if (hasTiePoints && hasPixelScales) {
            ...
            // "raster" space 
            final double tiePointColumn = tiePoints[0].getValueAt(0) + (rasterType == 
                    GeoTiffConstants.RasterPixelIsArea ? -0.5 : 0);

            // coordinates
            // (indicies)
            final double tiePointRow = tiePoints[0].getValueAt(1) + (rasterType == 
                    GeoTiffConstants.RasterPixelIsArea ? -0.5 : 0);
            ...
            gm.setElement(0, 2, tiePoints[0].getValueAt(3) - (scaleRaster2ModelLongitude * 
                    tiePointColumn));
            gm.setElement(1, 2, tiePoints[0].getValueAt(4) - (scaleRaster2ModelLatitude * 
                    tiePointRow));
     ...
     *
     * @param linearUnit is the UoM that this {@link ProjectedCRS} will use. It could
     *                   be null.
        ...
        if (projUserDefined) {
            ...
            if (parameters == null) {
                throw new GeoTiffException(
                        metadata,
                        "GeoTiffMetadata2CRSAdapter::createUserDefinedPCS:Projection is not " +
                                "supported.",
                        null);
            }
            ...
            conversionFromBase = (Conversion) this.allAuthoritiesFactory
                    .createCoordinateOperation(new StringBuilder("EPSG:").append(projCode)
                            .toString());
            ...
            if (pmCode != null) {
                if (pmCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
                    try {
                        ...
                        final String pmValue = metadata.getGeoKey(GeoTiffGCSCodes
    ...
    private GeodeticDatum createGeodeticDatum(final Unit<?> unit,
                                              final GeoTiffIIOMetadataDecoder metadata) throws 
            IOException {
    ...
    private Ellipsoid createEllipsoid(final Unit unit,
        ...
        if (ellipsoidKey.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
            ...
            return DefaultEllipsoid.createFlattenedSphere(name, semiMajorAxis, inverseFlattening,
                    unit);
        }
        ...
        try {
            ...
            //
            return this.allAuthoritiesFactory.createEllipsoid(new StringBuilder("EPSG:").append
                    (ellipsoidKey).toString());
        } catch (FactoryException fe) {
            final GeoTiffException ex = new GeoTiffException(metadata, fe.getLocalizedMessage(), 
                    fe);
            throw ex;
        }
    }
        ...
        if ((coordTrans == null)
            ...
            throw new GeoTiffException(
                    ...
                    "GeoTiffMetadata2CRSAdapter::createUserDefinedProjectionParameter(String " +
                            "name):User defined projections must specify"
     ...
     * @param coordTrans a {@link ParameterValueGroup} that can be used to trigger this
     *                   projection.
     * @return
     * @throws GeoTiffException
     */
    private ParameterValueGroup setParametersForProjection(String name,
                                                           ...
                                                           final GeoTiffIIOMetadataDecoder 
                                                                   metadata) throws 
            GeoTiffException {
        ...
        if (unitCode.equals(GeoTiffConstants.GTUserDefinedGeoKey_String)) {
            try {
                ...
                if (unitSize == null) {
                    throw new GeoTiffException(
                            metadata,
                            "GeoTiffMetadata2CRSAdapter::createUnit:Must define unit length when " +
                                    "using a user defined unit",
                            null);
                }
