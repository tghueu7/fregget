...
public class RecyclingTileFactory extends Observable
  ...
  /**
   * Cache of recycled arrays. The key in this mapping is a <code>Long</code> which is formed for a
   * given two-dimensional array as
   *
   * <pre>
   ...
   * The value corresponding to each key is an <code>ArrayList</code> of <code>SoftReferences</code>
   * to the internal data banks of <code>DataBuffer</code>s of tiles wherein the data bank array has
   * the type and dimensions implied by the key.
   */
  ...
  private Object getRecycledArray(int arrayType, long numBanks, long arrayLength) {
    ...
    if (DEBUG) {
      System.out.println(
          "Attempting to get array for: " + arrayType + " " + numBanks + " " + arrayLength);
