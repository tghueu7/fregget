...
public class GridCoverageBuilder {
   ...
   *
   * @throws IllegalArgumentException if the CRS is illegal for the {@linkplain #getEnvelope current
   *     envelope}.
   */
  ...
  /**
   * Returns a copy of current envelope. If no envelope has been {@linkplain #setEnvelope explicitly
   * defined}, then the default is inferred from the CRS (by default a geographic envelope from
   * 180°W to 180°E and 90°S to 90°N).
   */
   ...
   * was previously defined}, the envelope will be reprojected to that CRS. If no CRS was previously
   * defined, then the CRS will be set to the {@linkplain Envelope#getCoordinateReferenceSystem
   * envelope CRS}.
   *
   ...
   * by upper corner coordinates. The number of arguments provided shall be twice the envelope
   * dimension, and minimum shall not be greater than maximum.
   *
  ...
  /**
   * Returns the range of sample values. If no range has been {@linkplain #setSampleRange explicitly
   * defined}, then the default is a range from 0 inclusive to 256 exclusive.
   */
  ...
  /**
   * Returns the image size. If no size has been {@linkplain #setImageSize explicitly defined}, then
   * the default is 256&times;256 pixels.
   */
  ...
  /**
   * Sets the buffered image to a raster filled with random value using the specified random number
   * generator. This method can be used for testing purpose, or for adding noise to a coverage.
   */
  ...
  public class Variable {
    ...
    /**
     * The sample dimension. Will be created when first needed. May be reset to {@code null} after
     * creation if a new sample dimension need to be computed.
     */
