...
final class Resampler2D extends GridCoverage2D {
     ...
     *
     * @param sourceCoverage   The source grid coverage.
     * @param targetCRS        Coordinate reference system for the new grid coverage, or {@code 
     * null}.
     * @param targetGG         The target grid geometry, or {@code null} for default.
     * @param interpolation    The interpolation to use, or {@code null} if none.
     * @param hints            The rendering hints. This is usually provided by 
    ...
    public static GridCoverage2D reproject(GridCoverage2D sourceCoverage,
        ...
        if (targetCRS == null) {
            ...
            if (targetGG != null && targetGG.isDefined(GridGeometry2D.CRS_BITMASK)) {
                final CoordinateReferenceSystem targetGGCRS = targetGG
                        .getCoordinateReferenceSystem();
                if (!CRS.equalsIgnoreMetadata(targetCRS, targetGGCRS) && !CRS.findMathTransform
                        (targetCRS, targetGGCRS).isIdentity()) {
                    throw new IllegalArgumentException(Errors.format(ErrorKeys
                            .ILLEGAL_ARGUMENT_$1, "TargetCRS must be compatible with TargetGG " +
                            "CRS"));
                }
        ...
        if (!hints.containsKey(JAI.KEY_BORDER_EXTENDER)) {
            hints.put(JAI.KEY_BORDER_EXTENDER, BorderExtender.createInstance(BorderExtender
                    .BORDER_COPY));
        }
        ...
        if (CRS.equalsIgnoreMetadata(sourceCRS, targetCRS)) {
            ...
            if (!targetGG.isDefined(GridGeometry2D.GRID_TO_CRS_BITMASK)) {
                ...
                if (!targetGG.isDefined(GridGeometry2D.GRID_RANGE_BITMASK)) {
                    ...
                    targetGG = new GridGeometry2D(new GeneralGridEnvelope(gridRange, PixelInCell
                            .CELL_CORNER), targetGG.getGridToCRS(PixelInCell.CELL_CENTER), 
                            targetCRS);
                }
            }
        } else {
            ...
            step3 = (force2D ? sourceGG.getGridToCRS2D(CORNER) : sourceGG.getGridToCRS(CORNER))
                    .inverse();
            ...
            if (targetGG == null) {
                ...
                targetGR = force2D ? new GridEnvelope2D(sourceGG.getGridRange2D()) : sourceGG
                        .getGridRange();
        ...
        if (targetHints == null) {
            targetHints = new RenderingHints(JAI.KEY_INTERPOLATION, Interpolation.getInstance
                    (Interpolation.INTERP_NEAREST));
        } else {
            targetHints.put(JAI.KEY_INTERPOLATION, Interpolation.getInstance(Interpolation
                    .INTERP_NEAREST));
        }
        ...
        if (allSteps.isIdentity() || (allSteps instanceof AffineTransform &&
            ...
            if (sourceBB.contains(targetBB)) {
                ...
                w.setNoData(null);
                w.mosaic(new RenderedImage[]{sourceImage}, MosaicDescriptor.MOSAIC_TYPE_OVERLAY, 
                        null, new ROI[]{roi}, null, nodata != null ? new Range[]{nodata} : null);
                newROI = w.getROI();
                newNoData = w.getNoData();
                CoverageUtilities.setROIProperty(sourceProps, newROI);
                CoverageUtilities.setNoDataProperty(sourceProps, newNoData);
                        ...
                        case DataBuffer.TYPE_FLOAT: {
                            ...
                            if (!(new GeneralEnvelope(source).contains(target, true))) {
                                if (interpolation != null && !(interpolation instanceof 
                                    ...
                                    return reproject(sourceCoverage, targetCRS, targetGG, null, 
                                            hints, background);
                                } else {
        ...
        final RenderedOp targetImage = w.getRenderedOperation();//getJAI(hints).createNS
        // (operation, paramBlk, targetHints);
        ...
        if (!targetGR.equals(actualGR)) {
            targetGG = new GridGeometry2D(actualGR, targetGG.getGridToCRS(PixelInCell
                    .CELL_CENTER), targetCRS);
            if (!automaticGR) {
                log(Loggings.getResources(locale).getLogRecord(Level.FINE,
                        LoggingKeys.ADJUSTED_GRID_GEOMETRY_$1, sourceCoverage.getName().toString
                                (locale)));
            }
        }
        /*
        ...
        if (CoverageProcessor.LOGGER.isLoggable(LOGGING_LEVEL)) {
            log(Loggings.getResources(locale).getLogRecord(LOGGING_LEVEL,
                    LoggingKeys.APPLIED_RESAMPLE_$11, new Object[]{
                            ...
                            /*  {1} */ sourceCoverage.getCoordinateReferenceSystem().getName()
                            .getCode(),
                            /*  {2} */ sourceImage.getWidth(),
                            /*  {3} */ sourceImage.getHeight(),
                            /*  {4} */ targetCoverage.getCoordinateReferenceSystem().getName()
                            .getCode(),
    ...
    private static GridCoverage2D existingCoverage(GridCoverage2D coverage,
        ...
        while (!equivalent(coverage.getGridGeometry(), targetGG) ||
                (!CRS.equalsIgnoreMetadata(targetCRS, coverage.getCoordinateReferenceSystem()) &&
                        !CRS.equalsIgnoreMetadata(targetCRS, coverage
     ...
     * @param tile   {@code true} for testing tile bounds, or {@code false} for testing image 
     *                           bounds.
     */
     ...
     *                   dimension
     *                    provided that are not greater than {@code sourceCRS}, but in theory it 
     *                    is 2D.
     ...
     */
    private static boolean equivalent(final GridGeometry2D sourceGG, final GridGeometry2D 
            targetGG) {
     ...
     */
    private static Warp createWarp(final CharSequence name, final Rectangle sourceBB, final 
    Rectangle targetBB,
                                   final MathTransform2D allSteps2D, final MathTransformFactory 
                                           mtFactory, Hints hints)
            throws FactoryException, TransformException {
        ...
        do {
            ...
            if (actualBB != null) {
                ...
                final AffineTransform2D adjustment = new AffineTransform2D(
                        1 - scaleX * factor, 0, 0, 1 - scaleY * factor, translateX * factor, 
                        translateY * factor);
                transform = (MathTransform2D) mtFactory.createConcatenatedTransform(allSteps2D, 
                        adjustment);
            }
            /*
...
//                return warp;
//            }
//            // The loop below intentionally tries one more iteration than the constant in case 
// we need
//            // to apply slightly more than the above scale and translation because of rounding 
// errors.
