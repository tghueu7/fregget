...
final class Resampler2D extends GridCoverage2D {
   ...
   * maps to pixel center (as of OGC specification). In JAI, the transforms rather map to the upper
   * left corner.
   */
  private static final PixelOrientation CORNER = PixelOrientation.UPPER_LEFT; // UPPER_LEFT;

  /**
   * When an empirical adjustement of the Warp transform seems necessary, the amount of subdivisions
   * to try.
   */
  ...
  private static GridCoverage2D create(
    ...
    /**
     * Build up transformation info properties: this can be used by the client to decide whether it
     * wants to actually do the resample (which might take hours with WarpAdapter on very large
     * grids) or not
     */
   ...
   * geometry is supplied, only its {@linkplain GridGeometry2D#getRange grid range} and {@linkplain
   * GridGeometry2D#getGridToCRS grid to CRS} transform are taken in account.
   *
  ...
  public static GridCoverage2D reproject(
    ...
    final RenderedOp targetImage =
        w.getRenderedOperation(); // getJAI(hints).createNS(operation, paramBlk, targetHints);
    ...
    if (!targetGR.equals(actualGR)) {
      targetGG =
          new GridGeometry2D(actualGR, targetGG.getGridToCRS(PixelInCell.CELL_CENTER), targetCRS);
      if (!automaticGR) {
        log(
  ...
  /**
   * Checks if two geometries are equal, ignoring unspecified fields. If one or both geometries has
   * no "gridToCRS" transform, then this properties is not taken in account. Same apply for the grid
   * range.
   *
  ...
  private static Warp createWarp(
    ...
    do {
      ...
      //                return warp;
      //            }
      //            // The loop below intentionally tries one more iteration than the constant in
      // case we need
