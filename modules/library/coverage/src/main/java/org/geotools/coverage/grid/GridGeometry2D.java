...
public class GridGeometry2D extends GeneralGridGeometry {
  ...
  /**
   * Index of column ({@link #gridDimensionX}) and row ({@link #gridDimensionY}) ordinates in a grid
   * point. They are the index of the first two dimensions with a {@linkplain GridEnvelope#getSpan
   * span} greater than 1 in the {@linkplain #getGridRange grid range}. Their values are usually 0
   * and 1 respectively.
   *
   ...
   * are index of (<var>x</var>, <var>y</var>) ordinates in a direct position after the {@linkplain
   * #getGridToCoordinateSystem grid to CRS} transform.
   *
   * <p>There is no garantee that {@link #gridDimensionX} maps to {@link #axisDimensionX} and {@link
   * #gridDimensionY} maps to {@link #axisDimensionY}, since axis may be interchanged.
   *
   ...
   * serialized because it may be user-provided, in which case it is likely to be more accurate than
   * what we would compute. If {@code null}, will be computed when first needed.
   */
   ...
   * this constructor just defines the CRS; it does <strong>not</strong> reproject the envelope. For
   * this reason, this constructor should not be public. It is for internal use by {@link
  ...
  /**
   * Constructs a new grid geometry from a grid range and a math transform. The arguments are passed
   * unchanged to the {@linkplain GeneralGridGeometry#GeneralGridGeometry(GridEnvelope,
   * MathTransform, CoordinateReferenceSystem) super-class constructor}. However, they must obey to
   ...
   *     non-zero for arbitrary {@link java.awt.image.RenderedImage}. A grid with 512 cells can have
   *     a minimum coordinate of 0 and maximum of 512, with 511 as the highest valid index.
   * @param gridToCRS The math transform which allows for the transformations from grid coordinates
   *     (pixel's <em>center</em>) to real world earth coordinates.
   * @param crs The coordinate reference system for the "real world" coordinates, or {@code null} if
   *     unknown. This CRS is given to the {@linkplain #getEnvelope envelope}.
   ...
   * an optional set of hints giving more control on the {@link MathTransform2D} to be inferred from
   * the <var>n</var>-dimensional transform.
   *
   ...
   * PixelOrientation} argument, the translation (if any) applies to every dimensions, not just the
   * ones mapping the 2D part.
   *
   * @param gridRange The valid coordinate range of a grid coverage, or {@code null} if none.
   * @param anchor Whatever the {@code gridToCRS} transform maps {@linkplain PixelInCell#CELL_CENTER
   *     cell center} (OGC convention) or {@linkplain PixelInCell#CELL_CORNER cell corner}
   *     (Java2D/JAI convention).
   * @param gridToCRS The math transform which allows for the transformations from grid coordinates
   ...
   * the <var>n</var>-dimensional transform.
   *
   * <p>The {@code anchor} argument tells whatever the {@code gridToCRS} transform maps pixel center
   * or some corner. Use {@link PixelOrientation#CENTER CENTER} for OGC conventions or {@link
   * PixelOrientation#UPPER_LEFT UPPER_LEFT} for Java2D/JAI conventions. A translation (if needed)
   * is applied only on the {@link #gridDimensionX} and {@link #gridDimensionY} parts of the
   * transform - all other dimensions are assumed mapping pixel center.
   *
   * @param gridRange The valid coordinate range of a grid coverage, or {@code null} if none.
   ...
   * @param gridToCRS The math transform which allows for the transformations from grid coordinates
   *     to real world earth coordinates. May be {@code null}, but this is not recommended.
  ...
  /**
   * Constructs a new two-dimensional grid geometry. A math transform will be computed automatically
   * with an inverted <var>y</var> axis (i.e. {@code gridRange} and {@code userRange} are assumed to
   * have <var>y</var> axis in opposite direction).
   *
   * @param gridRange The valid coordinate range of a grid coverage. Increasing <var>x</var> values
   *     goes right and increasing <var>y</var> values goes <strong>down</strong>.
   * @param userRange The corresponding coordinate range in user coordinate. Increasing <var>x</var>
   *     values goes right and increasing <var>y</var> values goes <strong>up</strong>. This
   *     rectangle must contains entirely all pixels, i.e. the rectangle's upper left corner must
   *     coincide with the upper left corner of the first pixel and the rectangle's lower right
   *     corner must coincide with the lower right corner of the last pixel.
   */
  ...
  /**
   * Returns the given grid geometry as a {@code GridGeometry2D}. If the given object is already an
   * instance of {@code GridGeometry2D}, then it is returned unchanged. Otherwise a new {@code
    ...
    if (gridToCRS2D == null) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.BAD_TRANSFORM_$1, Classes.getClass(gridToCRS2D)), exception);
      }
  }

  /**
   * Constructs the two-dimensional CRS. This is usually identical to the user-supplied CRS.
   * However, the user is allowed to specify a wider CRS (for example a 3D one which includes a time
   * axis), in which case we infer which axis apply to the 2D image, and constructs a 2D CRS with
   * only those axis.
   *
   ...
   *
   * <p>The {@link Envelope#getCoordinateReferenceSystem coordinate reference system} of the source
   ...
   * @return A coordinate reference system with no more than 2 dimensions, or {@code null} if {@code
   *     crs} was null.
   ...
   * axis. Note that those axis are garanteed to appears in the same order than in the complete CRS.
   *
  ...
  /**
   * Returns the two-dimensional bounding box for the coverage domain in coordinate reference system
   * coordinates. If the coverage envelope has more than two dimensions, only the dimensions used in
   * the underlying rendered image are returned.
   *
  ...
  /**
   * Returns a math transform for the two dimensional part. This is a convenience method for working
   * on horizontal data while ignoring vertical or temporal dimensions.
   *
   * @return The transform which allows for the transformations from grid coordinates to real world
   *     earth coordinates, operating only on two dimensions. The returned transform is often an
   *     instance of {@link AffineTransform}, which make it convenient for interoperability with
   *     Java2D.
   ...
   *
   * @return The transform which allows for the transformations from real world earth coordinates to
   *     grid coordinates, operating only on two dimensions. The returned transform is often an
  ...
  /**
   * Returns a math transform mapping the specified pixel part. A translation (if needed) is applied
   * on the {@link #gridDimensionX} and {@link #gridDimensionY} parts of the transform; all other
   * dimensions are assumed mapping pixel center. For applying a translation on all dimensions, use
   * {@link #getGridToCRS(PixelInCell)} instead.
   *
   * @param orientation The pixel part to map. The default value is {@link PixelOrientation#CENTER
   *     CENTER}.
   * @return The transform which allows for the transformations from grid coordinates to real world
   ...
   * MathsTransform} provided by {@linkplain GridGeometry2D#getCRSToGrid2D(PixelOrientation) } which
   * is accessed via {@linkplain #getGridGeometry()}.
   *
   ...
   *
   * <p>The {@code GridEnvelope2D} returned contains the range of cells whose centers lie inside the
   * input {@code Envelope2D}
   *
   * <p>Users needing more control over the nature of the conversion can access the {@linkplain
   * MathsTransform} provided by {@linkplain GridGeometry2D#getCRSToGrid2D(PixelOrientation)} which
   * is accessed via {@linkplain #getGridGeometry()}.
  ...
  public final GridEnvelope2D worldToGrid(final Envelope2D envelope)
    ...
    if (sourceCRS != null) {
      ...
      if (!CRS.equalsIgnoreMetadata(sourceCRS, targetCRS)) {
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.ILLEGAL_COORDINATE_SYSTEM_FOR_CRS_$2, sourceCRS, targetCRS));
      }
    }
   ...
   * @since 2.6
   */
  public final DirectPosition gridToWorld(final GridCoordinates2D point) throws TransformException {
   ...
   * MathsTransform} provided by {@linkplain GridGeometry2D#getGridToCRS2D(PixelOrientation) } which
   * is accessed via {@linkplain #getGridGeometry()}.
   *
