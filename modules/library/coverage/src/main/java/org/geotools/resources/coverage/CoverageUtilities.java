...
public final class CoverageUtilities {
  ...
  public static final AffineTransform CENTER_TO_CORNER =
      AffineTransform.getTranslateInstance(
          ...
          PixelTranslation.getPixelTranslation(PixelInCell.CELL_CORNER));
  /**
   * {@link AffineTransform} that can be used to go from an image datum placed at the ULC corner of
   * pixels to one that is placed at center.
   */
  ...
  /**
   * Retrieves a best guess for the sample value to use for background, inspecting the categories of
   * the provided {@link GridCoverage2D}.
   *
  ...
  /**
   * Returns the visible band in the specified {@link RenderedImage} or {@link PropertySource}. This
   * method fetch the {@code "GC_VisibleBand"} property. If this property is undefined, then the
   * visible band default to the first one.
   *
  ...
  /**
   * Tries to estimate if the supplied affine transform is either a scale and translate transform or
   * if it contains a rotations which is an integer multiple of PI/2.
   *
  ...
  /**
   * Checks that the provided {@code dimensions} when intersected with the source region used by the
   * provided {@link ImageReadParam} instance does not result in an empty {@link Rectangle}.
  ...
  public static class UCUM {
    ...
    /**
     * Commonly used UCUM units. In case this set will grow too much, we may consider importing some
     * UCUM specialized library.
     */
