...
public class CoverageProcessor {
   ...
   *
   * @todo This is a temporary field, to be removed when a GeoAPI interfaces for coverage processing
   *     while be redesigned along the lines of ISO 19123.
   */
  ...
  static {
    ...
    if (cache.getMemoryCapacity() + (4 * 1024 * 1024) >= maxMemory) {
      final LogRecord record =
          Loggings.format(
              Level.SEVERE, LoggingKeys.EXCESSIVE_TILE_CACHE_$1, maxMemory / (1024 * 1024.0));
   ...
   * objects, the operation name are actually case-insensitive because of the comparator used in the
   * sorted map.
   */
   ...
   * added by subclasses with the {@link #addOperation} method. Rendering hints will be initialized
  ...
  /**
   * This method is called when the user has registered another {@link OperationDescriptor} and must
   * remove the old operation instance from the processors.
   *
  ...
  /**
   * Logs a message for an operation. The message will be logged only if the source grid coverage is
   * different from the result (i.e. if the operation did some work).
   *
  ...
  /**
   * Implementation of {@link #addOperation} method. Also used by {@link #scanForPlugins} instead of
   * the public method in order to avoid never-ending loop.
   */
  ...
  /**
   * Retrieves grid processing operations information. Each operation information contains the name
   * of the operation as well as a list of its parameters.
   */
  ...
  public Operation getOperation(String name) throws OperationNotFoundException {
    ...
    synchronized (operations) {
      ...
      throw new OperationNotFoundException(
          Errors.getResources(getLocale()).getString(ErrorKeys.OPERATION_NOT_FOUND_$1, name));
    }
  }
   ...
   * coverages use an interpolation, and then invokes {@link AbstractOperation#doOperation}. If all
  ...
  public void scanForPlugins() {
    synchronized (operations) {
      registry
          .getFactories(Operation.class, null, null)
   ...
   * default processor. If no argument is provided, then only a summary of operations is printed. If
   * arguments are provided, then the operation parameters are printed for all operation names given
   * in arguments. This method can been invoked from the command line. For example:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * </blockquote>
   *
   * <strong>Note for Windows users:</strong> If the output contains strange symbols, try to supply
   * an "{@code -encoding}" argument. Example:
   *
   * <blockquote>
   *
   * <pre>
