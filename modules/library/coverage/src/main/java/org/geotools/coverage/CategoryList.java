...
class CategoryList extends AbstractList<Category> implements Comparator<Category>, Serializable {
  ...
  /**
   * The category to use if {@link #getCategory(double)} is invoked with a sample value greater than
   * all sample ranges in this category list. This is usually a reference to the last category to
   * have a range of real values. A {@code null} value means that no fallback should be used. By
   * extension, a {@code null} value also means that {@link #getCategory} should not try to find any
   * fallback at all if the requested sample value do not falls in a category range.
   */
  private final Category overflowFallback;

  /**
   * The last used category. We assume that this category is the most likely to be requested in the
   * next {@code transform(...)} invocation.
   */
  private transient Category last;

  /**
   * {@code true} if there is gaps between categories, or {@code false} otherwise. A gap is found if
   * for example the range of value is [-9999 .. -9999] for the first category and [0 .. 1000] for
   * the second one.
   */
  ...
  /**
   * The unit information for all quantitative categories. It may be {@code null} if no category has
   * units.
   */
  private Unit<?> unit;
  ...
  /**
   * Compares {@link Category} objects according their {@link Category#minimum} value. This is used
   * for sorting the {@link #categories} array at construction time.
   */
  ...
  /**
   * Effectue une recherche bi-linéaire de la valeur spécifiée. Cette méthode est semblable à {@link
   * Arrays#binarySearch(double[],double)}, excepté qu'elle peut distinguer différentes valeurs de
   * NaN.
   *
   ...
   * with the geophysics value range. This is given to {@link GridSampleDimension} only if the user
   * did not specified explicitly a description.
   */
  ...
  /**
   * Returns the unit information for quantitative categories in this list. May returns {@code null}
   * if there is no quantitative categories in this list, or if there is no unit information.
   */
  ...
  /**
   * Returns the range of values in this category list. This is the union of the range of values of
   * every categories, excluding {@code NaN} values. A {@link NumberRange} object give more
   ...
   */
  StringBuffer format(
      final double value, final boolean writeUnits, final Locale locale, StringBuffer buffer) {
  ...
  /**
   * Formats a sample value. If {@code value} is a real number, then the value may be formatted with
   * the appropriate number of digits and the units symbol. Otherwise, if {@code value} is {@code
   * NaN}, then the category name is returned.
   *
