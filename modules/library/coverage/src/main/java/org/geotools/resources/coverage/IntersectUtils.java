...
public class IntersectUtils {
    ...
    public static Geometry intersection(Geometry g1, Geometry g2) {
        if (g1 instanceof GeometryCollection) {
            if (g2 instanceof GeometryCollection) {
                ...
                return g1.getFactory().createGeometryCollection(GeometryFactory.toGeometryArray
                        (ret));
            }
        } else {
            if (g2 instanceof GeometryCollection) {
     ...
     * <LI>GeometryCollection holding Polygons and MultiPolygons -> GeometryCollection of 
     * Polygons</LI>
     * </UL>
     *
     * @param geometry
    ...
    public static Geometry unrollGeometries(Geometry geometry)
        ...
        if (geometry instanceof com.vividsolutions.jts.geom.Polygon) {
            ...
            return geometry.getFactory().createMultiPolygon(unrollGeometries(mp).toArray(new 
                    Polygon[0]));
    ...
    private static List<com.vividsolutions.jts.geom.Polygon> unrollGeometries(MultiPolygon mp) {
        ...
        for (int i = 0; i < mp.getNumGeometries(); i++) {
            com.vividsolutions.jts.geom.Polygon g = (com.vividsolutions.jts.geom.Polygon) mp
                    .getGeometryN(i);
            ret.add(g);
        }
