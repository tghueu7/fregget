...
public class GeneralGridEnvelope implements GridEnvelope, Serializable {
   ...
   * later is the opposite of ISO specification. We store upper coordinates as exclusive values for
   ...
   *
   * @throws IllegalArgumentException if an ordinate value in the minimum index is not less than or
  ...
  /**
   * Constructs an initially empty grid envelope of the specified dimension. This is used by {@link
   * #getSubGridEnvelope} before a grid envelope goes public.
   */
   ...
   * inclusive ranges respectively. Extra dimensions (if any) are set to the [0..0] inclusive range.
   *
   * <p>Notice that this method ensure interoperability between {@link Raster} dimensions in Java2D
   * style and {@link GridEnvelope} dimensions in ISO 19123 style providing that the user remember
   * to add 1 to the {@link GridEnvelope#getHigh(int)} values.
   *
   ...
   * RenderedImage#getHeight height}-1] inclusive ranges respectively. Extra dimensions (if any) are
   * set to the [0..0] inclusive range.
   *
   * <p>Notice that this method ensure interoperability between {@link Raster} dimensions in Java2D
   * style and {@link GridEnvelope} dimensions in ISO 19123 style providing that the user remember
   * to add 1 to the {@link GridEnvelope#getHigh(int)} values.
   ...
   * arguments order can be confusing. Forcing usage of {@link Rectangle} in public API is probably
   * safer.
   */
   ...
   * org.opengis.coverage.grid.GridGeometry#getGridToCRS grid to CRS} transform. The floating point
   ...
   * maximum value since it is exclusive in a grid envelope). This offset is applied only if {@code
   * anchor} is {@link PixelInCell#CELL_CENTER}. Users who don't want such offset should specify
   ...
   *     {@link PixelInCell#CELL_CENTER} for OGC convention (an offset of 0.5 will be added to every
   *     envelope coordinate values), or {@link PixelInCell#CELL_CORNER} for Java2D/JAI convention
   ...
   * It would have been possible to round the {@linkplain Envelope#getMinimum minimal value} toward
   * {@linkplain Math#floor floor} and the {@linkplain Envelope#getMaximum maximal value} toward
   ...
   * range {@code [-0.25 ... 99.75]} (which is exactly 101 units wide) would be casted to {@code [-1
   * ... 100]}, which is 102 units wide. This leads to unexpected results when using grid envelope
   * with image operations like "{@link javax.media.jai.operator.AffineDescriptor Affine}". For
   * avoiding such changes in size, it is necessary to use the same rounding mode for both minimal
   * and maximal values. The selected rounding mode is {@linkplain Math#round nearest integer} in
   * this implementation.
   *
   * <p><b>Anchor</b><br>
   * According OpenGIS specification, {@linkplain org.opengis.coverage.grid.GridGeometry grid
   ...
   * Constructs a new grid envelope.
   *
   * <p>Notice that this constructor has been added for compatibility with JAVA2D, which means that
   * the high coords are intepreted as EXCLUSIVE
   *
   * @param low The valid minimum inclusive grid coordinate. The array contains a minimum value
   *     (inclusive) for each dimension of the grid coverage. The lowest valid grid coordinate is
   *     often zero, but this is not mandatory.
  ...
  /**
   * Returns the valid minimum inclusive grid coordinates. The sequence contains a minimum value for
  ...
  /**
   * Returns the valid maximum <strong>inclusive</strong> grid coordinates. The sequence contains a
  ...
  /**
   * Returns a {@link Rectangle} with the same bounds as this {@code GeneralGridEnvelope}. This is a
