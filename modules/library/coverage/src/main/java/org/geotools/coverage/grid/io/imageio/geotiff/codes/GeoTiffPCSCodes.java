...
public final class GeoTiffPCSCodes {
  ...
  /**
   * 6.3.3.1 Projected CS Type Codes Ranges: [ 1, 1000] = Obsolete EPSG/POSC Projection System Codes
   * [20000, 32760] = EPSG Projection System codes 32767 = user-defined [32768, 65535] = Private
   * User Implementations Special Ranges: 1. For PCS utilizing GeogCS with code in range 4201
   * through 4321: As far as is possible the PCS code will be of the format gggzz where ggg is
   * (geodetic datum code -4000) and zz is zone. 2. For PCS utilizing GeogCS with code out of range
   * 4201 through 4321 (i.e. geodetic datum code 6201 through 6319). PCS code 20xxx where xxx is a
   * sequential number. 3. Other: WGS72 / UTM northern hemisphere: 322zz where zz is UTM zone number
   * WGS72 / UTM southern hemisphere: 323zz where zz is UTM zone number WGS72BE / UTM northern
   * hemisphere: 324zz where zz is UTM zone number WGS72BE / UTM southern hemisphere: 325zz where zz
   * is UTM zone number WGS84 / UTM northern hemisphere: 326zz where zz is UTM zone number WGS84 /
   * UTM southern hemisphere: 327zz where zz is UTM zone number US State Plane (NAD27): 267xx/320xx
   * US State Plane (NAD83): 269xx/321xx Note: These are only a subset of the possible values
   */
  ...
  /**
   * ProjAzimuthAngleGeoKey Key ID = 3094 Type = DOUBLE Units: GeogAzimuthUnit Azimuth angle east of
   * true north of the central line passing through the projection center (for elliptical (Hotine)
   * Oblique Mercator). Note that this is the standard method of measuring azimuth, but is opposite
   * the usual mathematical convention of positive indicating counter-clockwise.
   */
  ...
  /**
   * ProjCenterLongGeoKey Key ID = 3088 Type = DOUBLE Units: GeogAngularUnit Longitude of Center of
   * Projection. Note that this is not necessarily the origin of the projection.
   */
   ...
   * coordinate of the center. This is NOT the False Northing. NOTE this value is incorrectly named
   * at http://www.remotesensing.org/geotiff/spec/geotiff2.7.html#2.7
   */
   ...
   * Transformations: PCSCitationGeoKey (additional parameter geokeys depending on the Coord. Trans.
   * specified).
   */
   ...
   * coordinate transformation method and projection zone parameters. Note : when associated with an
   * appropriate Geographic Coordinate System, this forms a Projected Coordinate System. GeoKeys
   * Required for "user-defined" Projections: PCSCitationGeoKey ProjCoordTransGeoKey
  ...
  /**
   * ProjFalseOriginLatGeoKey Key ID = 3085 Type = DOUBLE Units: GeogAngularUnit Gives the latitude
   * of the False origin.
   */
   ...
   * northing coordinate of the False origin. This is NOT the False Northing, which is the northing
   * attached to the Natural origin.
   */
  ...
  /**
   * ProjLinearUnitSizeGeoKey Key ID = 3077 Type = DOUBLE Units: meters Defines size of user-defined
  ...
  /**
   * ProjStraightVertPoleLongGeoKey Key ID = 3095 Type = DOUBLE Units: GeogAngularUnit Longitude at
   * Straight Vertical Pole. For polar stereographic.
   */
   ...
   * ModelTypeGeocentric = 3 Geocentric (X,Y,Z) Coordinate System Notes: 1. ModelTypeGeographic and
   * ModelTypeProjected correspond to the FGDC metadata Geographic and Planar-Projected coordinate
   * system types.
   */
