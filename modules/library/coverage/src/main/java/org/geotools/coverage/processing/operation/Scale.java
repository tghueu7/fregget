...
public class Scale extends BaseScaleOperationJAI {
    ...
    @Override
    protected RenderedImage createRenderedImage(ParameterBlockJAI parameters, RenderingHints 
            hints) {
        ...
        if (interpolation != null && !(interpolation instanceof InterpolationNearest)
                && (transferType == DataBuffer.TYPE_FLOAT || transferType == DataBuffer
                .TYPE_DOUBLE)) {
    ...
    protected Map<String, ?> getProperties(RenderedImage data, CoordinateReferenceSystem crs,
        ...
        if (data instanceof RenderedOp) {
            ...
            if (operationName.equalsIgnoreCase(SCALE) || operationName.equalsIgnoreCase
                    (TRANSLATE)) {
                propertyGenerator = getOperationDescriptor(operationName).getPropertyGenerators
