...
abstract class IndexColorOperation extends Operation2D {
    ...
    public Coverage doOperation(final ParameterValueGroup parameters, final Hints hints) {
        ...
        for (int i = 0; i < bands.length; i++) {
            ...
            final ColorModel candidate = (i == visibleBand) ? image.getColorModel() : band
                    .getColorModel();
        ...
        final RenderedImage newImage = new NullOpImage(image, layout, null, OpImage
                .OP_COMPUTE_BOUND);
        final GridCoverage2D target = CoverageFactoryFinder.getGridCoverageFactory(GeoTools
                .getDefaultHints()).create(
