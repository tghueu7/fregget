...
public class Category implements Serializable {
  ...
  /**
   * A default category for "no data" values. This default qualitative category use sample value 0,
   * which is mapped to geophysics value {@link Float#NaN} for those who work with floating point
   * images. The rendering color default to a fully transparent color and the name is "no data"
   ...
   */
  public Category(
      final CharSequence name, final Color color, final int sample, final boolean isQuantitative) {
   ...
   * @param name The category name as a {@link String} or {@link InternationalString} object.
   * @param color The category color, or {@code null} for a default color.
   * @param sampleValueRange The range of sample values for this category. Element class is usually
   ...
   *
   * @param name The category name as a {@link String} or {@link InternationalString} object.
   * @param colors A set of colors for this category. This array may have any length; colors will be
   *     interpolated as needed. An array of length 1 means that an uniform color should be used for
   *     all sample values. An array of length 0 or a {@code null} array means that some default
   *     colors should be used (usually a gradient from opaque black to opaque white).
   ...
   *     {@code scale} or {@code offset} are not real numbers.
   */
  public Category(final CharSequence name, final Color[] colors, final NumberRange sampleValueRange)
      throws IllegalArgumentException {
  ...
  private Category(
    ...
    if (!(minimum <= maximum) || Double.isInfinite(minimum) || Double.isInfinite(maximum)) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.BAD_RANGE_$2, range.getMinValue(), range.getMaxValue()));
    }
  }
  ...
  /**
   * Returns a {@code double} value for the specified number. If {@code direction} is non-zero, then
   * this method will returns the closest representable number of type {@code type} before or after
   * the double value.
   *
  ...
  /**
   * Returns ARGB values for the specified color. If {@code color} is null, a default ARGB code will
   * be returned.
   */
  ...
  /**
   * Returns the set of colors for this category. Change to the returned array will not affect this
   * category.
   *
  ...
  /**
   * Returns a category for the same range of sample values but a different color palette. The array
   * given in argument may have any length; colors will be interpolated as needed. An array of
   * length 1 means that an uniform color should be used for all sample values. An array of length 0
   * or a {@code null} array means that some default colors should be used (usually a gradient from
   * opaque black to opaque white).
   *
   * @param colors A set of colors for the new category.
   * @return A category with the new color palette, or {@code this} if the new colors are identical
   *     to the current ones.
  ...
  /**
   * Returns a hash value for this category. This value need not remain consistent between different
