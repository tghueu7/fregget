...
public final class GeoTiffConstants {
   ...
   * defines the general type of model Coordinate system used, and to which the raster space will be
   * transformed: unknown, Geocentric (rarely used), Geographic, Projected Coordinate System, or
   * user-defined. If the coordinate system is a PCS, then only the PCS code need be specified. If
   * the coordinate system does not fit into one of the standard registered PCS'S, but it uses one
   * of the standard projections and datums, then its should be documented as a PCS model with
  ...
  /**
   * GTRasterTypeGeoKey Key ID = 1025 Type = Section 6.3.1.2 codes This establishes the Raster Space
   * coordinate system used; there are currently only two, namely RasterPixelIsPoint and
   * RasterPixelIsArea.
   */
   ...
   * (GeoTIFFWritingUtilities Defined) [1024, 32766] = Reserved 32767 = user-defined [32768, 65535]=
   * Private User
   */
