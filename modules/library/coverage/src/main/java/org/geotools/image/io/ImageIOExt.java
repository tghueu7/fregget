...
public class ImageIOExt {
  ...
  /**
   * The threshold at which the class will flip from {@link MemoryCacheImageOutputStream} to {@link
   * FileCacheImageOutputStream}. If the in memory, uncompressed image size is lower than the
   * threshold a {@link MemoryCacheImageOutputStream} will be returned, otherwise a {@link
   ...
   * I/O extension for JAI provides native acceleration for PNG and JPEG. Unfortunatly, those native
   * codec has bug in their 1.0 version. Invoking this method will force the use of standard codec
   * provided in J2SE 1.4.
   *
   * <p><strong>Implementation note:</strong> the current implementation assume that JAI codec class
   * name start with "CLib". It work for Sun's 1.0 implementation, but may change in future
   * versions. If this method doesn't recognize the class name, it does nothing.
   *
   * @param format The format name (e.g. "png").
   * @param category {@code ImageReaderSpi.class} to set the reader, or {@code ImageWriterSpi.class}
   *     to set the writer.
   ...
   *     instance
   * @param streamCreationCheck if <code>true</code>, when a proper {@link ImageInputStreamSpi} have
   *     been found for the provided input, use it to try creating an {@link ImageInputStream} on
   *     top of the input.
   ...
   *
   * @return <code>false</code> in case the JAI/ImageIO native libs are not in the path, <code>true
   *     </code> otherwise.
   */
