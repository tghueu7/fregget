...
public abstract class AbstractGridFormat implements Format {
  ...
  /**
   * The Map object is used by the information methods(such as getName()) as a data source. The keys
   * in the Map object (for the associated method) are as follows: getName() key = "name" value
   * type=String getDescription() key = "description" value type=String getVendor() key = "vendor"
   * value type=String getDocURL() key = "docURL" value type=String getVersion() key = "version"
   * value type=String Naturally, any methods that are overridden need not have an entry in the Map
   */
  protected Map<String, String> mInfo;

  /**
   * {@link ParameterValueGroup} that controls the reading process for a {@link GridCoverageReader}
   * through the {@link GridCoverageReader#read(org.opengis.parameter.GeneralParameterValue[])}
   * method.
   */
  protected ParameterValueGroup readParameters;

  /**
   * {@link ParameterValueGroup} that controls the writing process for a {@link GridCoverageWriter}
  ...
  /**
   * This {@link GeneralParameterValue} cacn be provided to the {@link GridCoverageWriter}s through
   * the {@link GridCoverageWriter#write(org.opengis.coverage.grid.GridCoverage,
   ...
   * type of image read operation requested: using a JAI ImageRead operation (leveraging on Deferred
   * Execution Model, Tile Caching,...), or the direct {@code ImageReader}'s read methods.
   */
   ...
   * This {@link GeneralParameterValue} can be provided to the {@link GridCoverageReader}s through
   * the {@link GridCoverageReader#read(GeneralParameterValue[])} method in order to specify the
   * policy a reader should adopt when setting read parameters when evaluating a needed resolution.
   */
   ...
   * images. (Images with tiles Nx1) Value should be a String in the form of "W,H" (without quotes)
   * where W is a number representing the suggested tileWidth and H is a number representing the
   * suggested tileHeight.
   */
  public static final DefaultParameterDescriptor<String> SUGGESTED_TILE_SIZE =
      new DefaultParameterDescriptor<String>(SUGGESTED_TILESIZE, String.class, null, "512,512");

  /**
   * This {@link GeneralParameterValue} can be provided to the {@link GridCoverageReader}s through
   * the {@link GridCoverageReader#read(GeneralParameterValue[])} method to specify the band indices
   * of the input grid coverage that are going to be in the resulting coverage. The order of the
   * bands on the output coverage is the order of the indices in the parameter. Value should be an
   * integer array (int[]) containing the band indices in the desired order. Duplicate or multiple
   * appearances of the same band index are allowed.
   */
   ...
   * source</code> object.
   *
   * <p>In case this {@link Format} cannot reader the provided <code>source</code> object <code>null
   * </code> is returned.
   *
  ...
  /**
   * Returns an instance of {@link ImageWriteParam} that can be used to control a subsequent {@link
