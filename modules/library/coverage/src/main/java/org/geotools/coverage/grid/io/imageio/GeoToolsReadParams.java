...
public abstract class GeoToolsReadParams extends ImageReadParam {
  ...
  /**
   * Returns the width of each tile in an raster as it will be reader If tiling parameters have not
   ...
   * @exception IllegalArgumentException if the tile size is not within one of the allowable ranges
   ...
   * width and height. Together, each pair defines a valid range of sizes. To specify a fixed size,
   * use the same width and height for both elements. To specify an arbitrary range, a value of
   * <code>null</code> is used in place of an actual array of <code>Dimension</code>s.
   *
   * <p>If no array is specified on the constructor, but tiling is allowed, then this method returns
   * <code>null</code>.
   *
   * @exception UnsupportedOperationException if the plug-in does not support tiling.
   * @return an array of <code>Dimension</code>s with an even length of at least two, or <code>null
   *     </code>.
   */
