...
public class GridCoverage2DRIA extends GeometricOpImage {
     ...
     */
    public static GridCoverage2DRIA create(final GridCoverage2D src, final GridGeometry2D dst, 
                                           final double[] nodata) {
    ...
    public static GridCoverage2DRIA create(GridCoverage2D src, GridGeometry2D dst,
        ...
        imageLayout.setWidth(destinationRasterDimension.width).setHeight
                (destinationRasterDimension.height);
        ...
        if (hints != null && hints.containsKey(JAI.KEY_IMAGE_LAYOUT)) {
            ...
            if (l.isValid(ImageLayout.TILE_HEIGHT_MASK) && l.isValid(ImageLayout.TILE_WIDTH_MASK)) {
                imageLayout.setTileHeight(Math.min(imageLayout.getHeight(null), l.getTileHeight
                        (null)));
                imageLayout.setTileWidth(Math.min(imageLayout.getWidth(null), l.getTileWidth
                        (null)));
            }
        }
        ...
        //
        BorderExtender extender = new BorderExtenderConstant(new double[]{nodata != null ? 
                nodata[0] : 0d});
    ...
    public static GridCoverage2DRIA create(final GridCoverage2D src, final GridCoverage2D dst,
        ...
        return new GridCoverage2DRIA(src, dst.getGridGeometry(), vectorize(src.getRenderedImage()
        ), imageLayout,
                null, false, extender, Interpolation.getInstance(Interpolation.INTERP_NEAREST),
                nodata, roi, hints);
    }
    ...
    protected GridCoverage2DRIA(final GridCoverage2D src, final GridGeometry2D dst,
                                final Vector sources, final ImageLayout layout, final Map 
                                        configuration,
                                final boolean cobbleSources, final BorderExtender extender, final
     ...
     * @throws IllegalArgumentException if <code>sourceIndex</code> is negative or greater than 
    ...
    private void mapSrcPoint(double[] coords) throws TransformException {
        ...
        // sb.append("SRC[").append(coords[0]).append(',').append(coords[1]).append(']').append
        // ("--g2wd->");
        g2wd.transform(coords, 0, coords, 0, npoints);
        // sb.append("[").append(coords[0]).append(',').append(coords[1]).append(']').append
        // ("--d2sCRS->");
    ...
    protected void computeRect(PlanarImage[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        if (hasROI) {
            ...
            srcRectExpanded = new Rectangle((int) minX, (int) minY, (int) (maxX - minX) + 1, 
                    (int) (maxY - minY) + 1);
            ...
            srcRectExpanded.setRect(
                    ...
                    srcRectExpanded.getHeight() + interp.getBottomPadding() + interp
                            .getTopPadding());
    ...
    private void computeRectByte(PlanarImage src, RasterAccessor dst, RandomIter roiIter, boolean
            roiContainsTile) {
            ...
            if (ctable == null) { // source does not have IndexColorModel
                for (int h = 0; h < dstHeight; h++) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY || !inROI
                                (xint, yint, roiIter, roiContainsTile)) {
                    ...
                    for (int w = 0; w < dstWidth; w++) {
                        ...
                        if (xint < minX || xint >= maxX || yint < minY || yint >= maxY || !inROI
                            ...
                            for (int b = 0; b < dstBands; b++) {
                                ...
                                for (int j = 0; j < kheight; j++) {
                                    for (int i = 0; i < kwidth; i++) {
                                        samples[j][i] = t[iter.getSample(xint + i, yint + j, 0) &
                                                0xFF] & 0xFF;
                                    }
                                }
    ...
    private void computeRectUShort(PlanarImage src, RasterAccessor dst, RandomIter roiIter, 
                                   boolean roiContainsTile) {
        int lpad, rpad, tpad, bpad;
        if (interp != null) {
            lpad = interp.getLeftPadding();
    ...
    private void computeRectShort(PlanarImage src, RasterAccessor dst, RandomIter roiIter, 
                                  boolean roiContainsTile) {
        int lpad, rpad, tpad, bpad;
        if (interp != null) {
            lpad = interp.getLeftPadding();
    ...
    private void computeRectInt(PlanarImage src, RasterAccessor dst, RandomIter roiIter, boolean 
            roiContainsTile) {
        int lpad, rpad, tpad, bpad;
        if (interp != null) {
            lpad = interp.getLeftPadding();
    ...
    private void computeRectFloat(PlanarImage src, RasterAccessor dst, RandomIter roiIter, 
                                  boolean roiContainsTile) {
        int lpad, rpad, tpad, bpad;
        if (interp != null) {
            lpad = interp.getLeftPadding();
    ...
    private void computeRectDouble(PlanarImage src, RasterAccessor dst, RandomIter roiIter, 
                                   boolean roiContainsTile) {
        int lpad, rpad, tpad, bpad;
        if (interp != null) {
            lpad = interp.getLeftPadding();
    ...
    public float[] warpRect(int x, int y, int width, int height, float[] destRect) {
        if (destRect != null && destRect.length < (width * height * 2)) {
            throw new IllegalArgumentException("warpRect: bad destRect");// JaiI18N.getString
            // ("Warp0"));
        }
    ...
    private RandomIter getRandomIterator(final PlanarImage src, int leftPad, int rightPad,
        ...
        return ExtendedRandomIter.getRandomIterator(src, leftPad, rightPad, topPad, bottomPad, 
                extender);
    }
    ...
    static class GridCoverage2DRIAPropertyGenerator extends PropertyGeneratorImpl {
        ...
        public GridCoverage2DRIAPropertyGenerator() {
            super(new String[]{"ROI"}, new Class[]{ROI.class}, new Class[]{GridCoverage2DRIA
                    .class});
        }
        ...
        public Object getProperty(String name, Object opNode) {
            ...
            if (opNode instanceof GridCoverage2DRIA && name.equalsIgnoreCase("roi")) {
                ...
                InterpolationNearest interp = (InterpolationNearest) Interpolation.getInstance
                ...
                final PlanarImage constantImage = ConstantDescriptor.create(new Float(w), new 
                                Float(h),
                ...
                // Creating warped roi by the same way (Warp, Interpolation, source ROI) we 
                // warped the
                // input image.
