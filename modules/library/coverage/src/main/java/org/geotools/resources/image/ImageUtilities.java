...
public final class ImageUtilities {
    ...
    public final static BorderExtender DEFAULT_BORDER_EXTENDER = BorderExtender.createInstance
            (BorderExtender.BORDER_COPY);

    public final static RenderingHints BORDER_EXTENDER_HINTS = new RenderingHints(JAI
            .KEY_BORDER_EXTENDER, DEFAULT_BORDER_EXTENDER);

    public static final String DIRECT_KAKADU_PLUGIN = "it.geosolutions.imageio.plugins.jp2k" +
     ...
     * the image is null).
     */
    private static ImageLayout getImageLayout(final RenderedImage image, final boolean 
            initToImage) {
        if (image == null) {
            return null;
        }
     ...
     * than {@value #GEOTOOLS_MIN_TILE_SIZE}. If this method can't suggest a size, then it 
     * returns 0.
     *
     * @param imageSize The image size.
     * @param tileSize  The preferred tile size, which is often {@value 
     * #GEOTOOLS_DEFAULT_TILE_SIZE}.
     */
    private static int toTileSize(final int imageSize, final int tileSize) {
        ...
        /*
         * Inspects all tile sizes in the range [GEOTOOLS_MIN_TILE_SIZE .. MAX_TIME_SIZE]. We 
         * will begin
         ...
         * in the range [GEOTOOLS_MIN_TILE_SIZE .. MAX_TIME_SIZE] were tested. In this process, 
         * we remind
        ...
        /*
         * No dividor were found in the range [GEOTOOLS_MIN_TILE_SIZE .. MAX_TIME_SIZE]. At this 
         * point
     ...
     */
    public static ImageLayout createIntersection(final ImageLayout layout, final 
     ...
     *
     * @return <code>false</code> in case the JAI native libs are not in the path, 
     ...
     * @return
     */
    public static Number[] getBackgroundValues(final SampleModel sampleModel, final double[] 
            backgroundValues) {
        Number[] values = null;
        final int dataType = sampleModel != null ? sampleModel.getDataType() : DataBuffer
                .TYPE_DOUBLE;
        ...
        switch (dataType) {
            ...
            case DataBuffer.TYPE_BYTE:
                ...
                if (backgroundValues == null) {
                    ...
                    for (int i = 0; i < values.length; i++)
                        values[i] = i >= backgroundValues.length ? Byte.valueOf((byte) 
                                backgroundValues[0]) : Byte.valueOf((byte) backgroundValues[i]);
                }
                break;
            ...
            case DataBuffer.TYPE_USHORT:
                ...
                if (backgroundValues == null)
                    ...
                    //we have background values available
                    for (int i = 0; i < values.length; i++)
                        values[i] = i >= backgroundValues.length ? Short.valueOf((short) 
                                backgroundValues[0]) : Short.valueOf((short) backgroundValues[i]);
                }
                break;
            case DataBuffer.TYPE_INT:
                ...
                if (backgroundValues == null)
                    ...
                    // we have background values available
                    for (int i = 0; i < values.length; i++)
                        values[i] = i >= backgroundValues.length ? Integer.valueOf((int) 
                                backgroundValues[0]) : Integer.valueOf((int) backgroundValues[i]);
            ...
            case DataBuffer.TYPE_FLOAT:
                ...
                if (backgroundValues == null)
                    ...
                    //we have background values available
                    for (int i = 0; i < values.length; i++)
                        values[i] = i >= backgroundValues.length ? Float.valueOf((float) 
                                backgroundValues[0]) : Float.valueOf((float) backgroundValues[i]);
                }
                break;
            case DataBuffer.TYPE_DOUBLE:
                ...
                if (backgroundValues == null)
                    ...
                    //we have background values available
                    for (int i = 0; i < values.length; i++)
                        values[i] = i >= backgroundValues.length ? Double.valueOf((Double) 
                                backgroundValues[0]) : Double.valueOf((Double) backgroundValues[i]);
                }
                break;
        }
        return values;
    ...
    public static void disposeSinglePlanarImage(PlanarImage planarImage) {
        ...
        try {
            ...
            LOGGER.log(Level.FINE, "Failed to clear rendered image adapters field to null. "
                    + "Not a problem per se, but if the finalizer thread is not fast enough, this" +
                    " might result in a OOM", e);
        }
        planarImage.dispose();
    }
