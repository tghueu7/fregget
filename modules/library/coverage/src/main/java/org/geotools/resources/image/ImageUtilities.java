...
public final class ImageUtilities {
  ...
  /**
   * Maximum tile width or height before to consider a tile as a stripe. It tile width or height are
   * smaller or equals than this size, then the image will be retiled. That is done because there
   * are many formats that use stripes as an alternative to tiles, an example is tiff. A stripe can
   * be a performance black hole, users can have stripes as large as 20000 columns x 8 rows. If we
   * just want to see a chunk of 512x512, this is a lot of uneeded data to load.
   */
  private static final int STRIPE_SIZE = 64;

  /**
   * List of valid names. Note: the "Optimal" type is not implemented because currently not provided
   * by JAI.
   */
  ...
  /**
   * Suggests an {@link ImageLayout} for the specified image. All parameters are initially set equal
   * to those of the given {@link RenderedImage}, and then the {@linkplain #toTileSize tile size is
   * updated according the image size}. This method never returns {@code null}.
   */
  ...
  /**
   * Returns an {@link ImageLayout} for the specified image. If {@code initToImage} is {@code true},
   * then all parameters are initially set equal to those of the given {@link RenderedImage} and the
   * returned layout is never {@code null} (except if the image is null).
   */
  ...
  /**
   * Suggests a tile size for the specified image size. On input, {@code size} is the image's size.
   * On output, it is the tile size. This method write the result directly in the supplied object
   * and returns {@code size} for convenience.
   *
   * <p>This method it aimed to computing a tile size such that the tile grid would have overlapped
   ...
   * example.
   *
   * <p>This method inspects different tile sizes close to the {@linkplain JAI#getDefaultTileSize()
   * default tile size}. Lets {@code width} be the default tile width. Values are tried in the
   * following order: {@code width}, {@code width+1}, {@code width-1}, {@code width+2}, {@code
   * width-2}, {@code width+3}, {@code width-3}, <cite>etc.</cite> until one of the following
   * happen:
   *
   * <p>
   *
   ...
   *       this method <strong>may</strong> set the {@link Dimension} field to a value that maximize
   *       the remainder of <var>image size</var> / <var>tile size</var> (in other words, the size
   *       that left as few empty pixels as possible).
   * </ul>
   */
  ...
  /**
   * Suggests a tile size close to {@code tileSize} for the specified {@code imageSize}. This method
   * it aimed to computing a tile size such that the tile grid would have overlapped the image bound
   * in order to avoid having tiles crossing the image bounds and being therefore partially empty.
   * This method will never returns a tile size smaller than {@value #GEOTOOLS_MIN_TILE_SIZE}. If
   * this method can't suggest a size, then it returns 0.
   *
   ...
   * ImageLayout#getMinX minX}, {@link ImageLayout#getMinY minY}, {@link ImageLayout#getWidth width}
   * and {@link ImageLayout#getHeight height} properties are not defined in the {@code layout}, then
   * they will be inherited from the <strong>first</strong> source for consistency with {@link
   * OpImage} constructor.
   *
   ...
   *
   * @param Interpolation The interpolation object, or {@code null} for "nearest" (which is an other
   *     way to say "no interpolation").
   */
   ...
   *     already tiled, so it should not be a cause of tile size mismatch in an operation chain. The
   *     mean usage for a separated "tile" operation is to tile an image before to save it on disk
   *     in some format supporting tiling.
  ...
  /**
   * Sets the content of all banks in the given data buffer to the specified value. We do not allow
   * setting of different value for invidivual bank because the data buffer "banks" do not
  ...
  /**
   * Retrieves the dimensions of the {@link RenderedImage} at index <code>imageIndex</code> for the
   * provided {@link ImageReader} and {@link ImageInputStream}.
   *
   ...
   * by the provided {@link ImageReadParam} instance does not result in an empty {@link Rectangle}.
   *
