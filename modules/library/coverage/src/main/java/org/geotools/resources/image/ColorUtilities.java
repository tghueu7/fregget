...
public final class ColorUtilities {
   ...
   * @throws IllegalArgumentException if {@coder}, {@code g}, {@code b} or {@code a} are outside of
  ...
  /**
   * Returns a subarray of the specified color array. The {@code lower} and {@code upper} index will
   * be clamb into the {@code palette} range. If they are completly out of range, or if they would
   * result in an empty array, then {@code null} is returned.
   *
   * <p>This method is used by {@link org.geotools.cv.SampleDimension} as an heuristic approach for
   * distributing palette colors into a list of categories.
   *
   ...
   *
   * <p><b>Note:</b> Profiling shows that this method is a "hot spot". It needs to be fast, which is
   * why the implementation is not as straight-forward as it could.
   *
   ...
   * opaque. Otherwise, if the specified array has one and only one color with alpha value of 0, the
   * returned color model will have only this transparent color. Otherwise, the returned color model
   * will be translucent.
   *
  ...
  /**
   * Returns a tolerant index color model for the specified ARGB code. This color model accept image
   * with the specified number of bands.
   *
   * @param ARGB An array of ARGB values.
   * @param numBands The number of bands.
   * @param visibleBand The band to display.
   * @return An index color model for the specified array.
   * @todo Considerer caching previously created instances using weak references. Index color model
   *     may be big (up to 256 kb), so it may be worth to cache big instances. NOTE: IndexColorModel
   *     inherits a equals(Object) implementation from ColorModel, but do not override it, so the
  ...
  /**
   * Transforms a color from XYZ color space to LAB. The color are transformed in place. This method
   * returns {@code color} for convenience. Reference:
  ...
  public static float[] XYZtoLAB(final float[] color) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      color[i] =
          (float) ((c > 216 / 24389f) ? Math.pow(c, 1.0 / 3) : ((24389 / 27.0) * c + 16) / 116);
    }
   ...
   * alpha value, than the darkest one is returned. This method never returns a negative value (0 is
  ...
  /**
   * Tries to guess the number of bands from the specified color model. The recommanded approach is
   * to invoke {@link java.awt.image.SampleModel#getNumBands}. This method should be used only as a
   * fallback when the sample model is not available. This method uses some heuristic rules for
   * guessing the number of bands, so the return value may not be exact in all cases.
   */
  ...
  /**
   * Tells us if a specific {@link IndexColorModel} contains only gray color or not, ignoring alpha
   * information.
   *
   * @param icm {@link IndexColorModel} to be inspected.
   * @param ignoreTransparents {@code true} if the RGB values of fully transparent pixels (the ones
   *     with an {@linkplain IndexColorModel#getAlpha(int) alpha} value of 0) should not be taken in
   *     account during the check for gray color.
    ...
    switch (dataType) {
      ...
      default:
        throw new IllegalArgumentException(
            Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "DataType unknown:", dataType));
    }
  }
