...
public abstract class BasicSQLDialect extends SQLDialect {
   ...
   *
   * <p>Subclasses may wish to override or extend this method to handle specific types. This default
   ...
   *
   * <p>An implementations should serialize <tt>value</tt> into some exchange format which will then
   * be transported to the underlying database. For example, consider an implementation which
   ...
   *  </pre>
   *
   * <p>The <tt>srid</tt> parameter is the spatial reference system identifier of the geometry, or 0
   * if not known.
   *
   ...
   *
   * <p>The callback is provided with both the statement being executed and the database connection.
