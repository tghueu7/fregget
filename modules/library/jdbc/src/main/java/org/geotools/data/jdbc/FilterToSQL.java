...
public class FilterToSQL implements FilterVisitor, ExpressionVisitor {
  ...
  /**
   * Writes the SQL for the Like Filter. Assumes the current java implemented wildcards for the Like
   * Filter: . for multi and .? for single. And replaces them with the SQL % and _, respectively.
   *
   * @param filter the Like Filter to be visited.
   * @task REVISIT: Need to think through the escape char, so it works right when Java uses one, and
   *     escapes correctly with an '_'.
   */
    ...
    try {
      ...
      for (Iterator i = ids.iterator(); i.hasNext(); ) {
        ...
        for (int j = 0; j < attValues.length; j++) {
          ...
          out.write(" = '");
          out.write(
              attValues[j].toString()); // DJB: changed this to attValues[j] from attValues[i].
          out.write("'");
  ...
  /**
   * Writes out a non null, non geometry literal. The base class properly handles null, numeric and
   * booleans (true|false), and turns everything else into a string. Subclasses are expected to
   * override this shall they need a different treatment (e.g. for dates)
   *
   * @param literal
   * @throws IOException
   */
   ...
   * arguments provided to the GeoTools function, subclasses should override on a case by case basis
   * if this behavior is not the desired one.
   *
   ...
   * column names in an SQL statement to support mixed-case and non-English names. Without this, the
   * DBMS may assume a mixed-case name in the query should be treated as upper-case and an SQLCODE
   * of -204 or 206 may result if the name is not found.
   *
   ...
   * SPEAR.ARCHSITES If the column and table names were actually created using mixed-case, the query
   * needs to be specified as: SELECT "Geom" from "Spear"."ArchSites"
   *
  ...
  /**
   * Interface for customizing the encoding of Fields, irrespective of which subclass of FilterToSQL
   * we are using.
   */
  ...
  /**
   * Rough evaluation of distance in the units of the current SRID, assuming that the SRID maps to a
   * known EPSG code. Will use a rather imprecise transformation for distances over degrees, but
   * better than nothing.
   *
   * @param operator
   */
