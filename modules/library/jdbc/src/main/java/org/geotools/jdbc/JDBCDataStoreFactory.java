...
public abstract class JDBCDataStoreFactory implements DataStoreFactorySpi {
     ...
     */
    public static final Param NAMESPACE = new Param("namespace", String.class, "Namespace " +
            "prefix", false);

    /**
     * parameter for data source
     */
    public static final Param DATASOURCE = new Param("Data Source", DataSource.class, "Data " +
            "Source", false);
    ...
    /**
     * If different from one, the JDBCInsertFeatureWriter will buffer the features and insert 
     * them in batches
     */
    public static final Param BATCH_INSERT_SIZE = new Param("Batch insert size", Integer.class,
            "Number of records inserted in the same batch (default, 1). For optimal performance, " +
                    "set to 100.", false, 1);
    ...
    public static final Param MAXWAIT = new Param("Connection timeout", Integer.class,
            "number of seconds the connection pool will wait before timing out attempting to get " +
                    "a new connection (default, 20 seconds)", false, 20);
     ...
     **/
    public static final Param TIME_BETWEEN_EVICTOR_RUNS = new Param("Evictor run periodicity", 
            Integer.class,
     ...
     **/
    public static final Param MIN_EVICTABLE_TIME = new Param("Max connection idle time", Integer
            .class,
            "number of seconds a connection needs to stay idle for the evictor to consider " +
                    "closing it", false, 300);

    /**
     * Number of connections checked during a single evictor run
     **/
    public static final Param EVICTOR_TESTS_PER_RUN = new Param("Evictor tests per run", Integer
            .class,
            "number of connections checked by the idle connection evictor for each of its runs " +
                    "(defaults to 3)", false, 3);

    /**
     * Metadata table providing information about primary keys
     **/
    public static final Param PK_METADATA_TABLE = new Param("Primary key metadata table", String
            .class,
            "The optional table containing primary key structure and sequence associations. Can " +
                    "be expressed as 'schema.name' or just 'name'", false);

    /**
     * Number of prepared statements cached per connection (this param is exposed only by 
     * factories supporting prepared statements
     **/
    public static final Param MAX_OPEN_PREPARED_STATEMENTS = new Param("Max open prepared " +
            "statements", Integer.class,
            "Maximum number of prepared statements kept open and cached for each connection in " +
                    "the pool. " +
     ...
     */
    public static final Param EXPOSE_PK = new Param("Expose primary keys", Boolean.class, "Expose" +
    ...
    public final JDBCDataStore createDataStore(Map params)
        ...
        if (sqlOnBorrow != null || sqlOnRelease != null) {
            SessionCommandsListener listener = new SessionCommandsListener(sqlOnBorrow, 
                    sqlOnRelease);
    ...
    protected void setupParameters(Map parameters) {
        ...
        parameters.put(DBTYPE.key,
                new Param(DBTYPE.key, DBTYPE.type, DBTYPE.description, DBTYPE.required, 
                        getDatabaseID()));
    ...
    protected DataSource createDataSource(Map params, SQLDialect dialect) throws IOException {
        ...
        if (dialect instanceof PreparedStatementSQLDialect) {
            ...
            if (testMap.containsKey(MAX_OPEN_PREPARED_STATEMENTS.key)) {
                Integer maxPreparedStatements = (Integer) MAX_OPEN_PREPARED_STATEMENTS.lookUp
                        (params);
     ...
     * <p>
     * Will return {@link JDBCCallbackFactory#NULL} if the factory with the specified name can't 
     * be found.
     * </p>
     */
    static JDBCCallbackFactory findCallbackFactory(String factoryName) {
        if (factoryName != null) {
            try {
                Predicate<JDBCCallbackFactory> sameName = f -> factoryName.equalsIgnoreCase(f
                        .getName());
