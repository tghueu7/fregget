...
// }
    ...
    protected FeatureWriter<SimpleFeatureType, SimpleFeature> getWriterInternal(Query query, int 
            flags)
            throws IOException {
        ...
        try {
            ...
            if (getDataStore().getSQLDialect() instanceof PreparedStatementSQLDialect) {
                ...
                if ((flags | WRITER_UPDATE) == WRITER_UPDATE) {
                    ...
                    //update insert case
                    writer = new JDBCUpdateInsertFeatureWriter(ps, cx, delegate, query
                            .getPropertyNames(), query);
                }
            } else {
    ...
    public void modifyFeatures(Name[] names, Object[] values, Filter filter)
        ...
        if (postFilter != null && !Filter.INCLUDE.equals(postFilter)) {
            ...
            try {
                ...
                try {
                    getDataStore().ensureAuthorization(featureType, preFilter, getTransaction(), 
                            cx);
                ...
                if (state.hasListener()) {
                    ...
                    ReferencedEnvelope before = getBounds(new Query(getSchema().getTypeName(), 
                            preFilter));
