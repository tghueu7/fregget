...
public final class DataSourceFinder {
  ...
  /**
   * Checks each available datasource implementation in turn and returns the first one which claims
   * to support the resource identified by the params object.
   *
   * @param params A Map object which contains a defenition of the resource to connect to. for file
  ...
  public static synchronized UnWrapper getUnWrapper(Connection conn) throws IOException {
    ...
    while (ps.hasNext()) {
      ...
      try {
        ...
        /** The logger for the filter module. */
        LOGGER.log(
            Level.WARNING, "Could not test  " + uw + " for unwrapping abilities agaist " + conn, t);
        // Protect against DataStores that don't carefully
        // code canProcess
  ...
  public static synchronized UnWrapper getUnWrapper(Statement st) throws IOException {
    ...
    while (ps.hasNext()) {
      ...
      try {
        ...
        /** The logger for the filter module. */
        LOGGER.log(
            Level.WARNING, "Could not test  " + uw + " for unwrapping abilities agaist " + st, t);
        // Protect against DataStores that don't carefully
        // code canProcess
  ...
  /**
   * Finds all implementations of UnWrapper which have registered using the services mechanism, and
   * that have the appropriate libraries on the classpath.
   *
  ...
  private static FactoryRegistry getServiceRegistry() {
    ...
    if (registry == null) {
      registry =
          new FactoryCreator(
              Arrays.asList(new Class<?>[] {DataSourceFactorySpi.class, UnWrapper.class}));
    }
    return registry;
  }
