...
public abstract class JDBCJoinOnlineTest extends JDBCTestSupport {
        ...
        q.getJoins()
            .add(
                new Join(
                    tname("ftjoin"),
                    ff.equal(
                        ff.property(aname("stringProperty")), ff.property(aname("name")), true)));
  ...
  public void testSimpleJoinOnPrimaryKey() throws Exception {
    ...
    try (SimpleFeatureIterator ita =
      ...
      Join join =
          new Join(
              tname("ftjoin"),
              ff.equal(ff.property(aname("id")), ff.property(aname("ftjoin.id")), true));
  ...
  void doTestSpatialJoin(boolean exposePrimaryKeys) throws Exception {
    ...
    try (SimpleFeatureIterator it = features.features()) {
      ...
      assertEquals(2, ((Number) f.getAttribute(aname("intProperty"))).intValue());
      assertEquals(
          "two", ((SimpleFeature) f.getAttribute(tname("ftjoin"))).getAttribute(aname("name")));
    ...
    q.getJoins()
        .add(
            new Join(
                    tname("ft1"),
                    ff.equal(
                        ff.property(aname("name")), ff.property(aname("stringProperty")), true))
                .type(Type.OUTER));
