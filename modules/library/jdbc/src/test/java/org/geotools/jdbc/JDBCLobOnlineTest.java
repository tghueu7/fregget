...
public abstract class JDBCLobOnlineTest extends JDBCTestSupport {
  ...
  public void testRead() throws Exception {
    ...
    try (FeatureIterator<SimpleFeature> fi = fc.features()) {
      ...
      assertTrue(
          Arrays.equals(new byte[] {1, 2, 3, 4, 5}, (byte[]) f.getAttribute(aname(BLOB_FIELD))));
      assertTrue(
          Arrays.equals(new byte[] {6, 7, 8, 9, 10}, (byte[]) f.getAttribute(aname(RAW_FIELD))));
  ...
  public void testWrite() throws Exception {
    FeatureStore<SimpleFeatureType, SimpleFeature> fs =
        (FeatureStore<SimpleFeatureType, SimpleFeature>) dataStore.getFeatureSource(tname(TESTLOB));
