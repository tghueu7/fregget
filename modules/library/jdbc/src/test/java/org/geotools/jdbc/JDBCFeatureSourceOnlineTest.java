...
public abstract class JDBCFeatureSourceOnlineTest extends JDBCTestSupport {
    ...
    public void testGetFeaturesWithLogicFilter() throws Exception {
        FilterFactory ff = dataStore.getFilterFactory();
        PropertyIsEqualTo property = ff.equals(ff.property(aname("stringProperty")), ff.literal
                ("one"));
    ...
    public void testCaseInsensitiveFilter() throws Exception {
        FilterFactory ff = dataStore.getFilterFactory();
        PropertyIsEqualTo sensitive = ff.equal(ff.property(aname("stringProperty")), ff.literal
                ("OnE"), true);
        PropertyIsEqualTo insensitive = ff.equal(ff.property(aname("stringProperty")), ff.literal
                ("OnE"), false);
    ...
    public void testGetFeaturesWithMax() throws Exception {
        ...
        try (SimpleFeatureIterator it = features.features()) {
            int count = 0;
            ReferencedEnvelope env = new ReferencedEnvelope(features.getSchema()
    ...
    public void testGetFeaturesWithOffset() throws Exception {
        Query q = new Query(featureSource.getSchema().getTypeName());
        q.setSortBy(new SortBy[]{dataStore.getFilterFactory().sort(aname("intProperty"), 
                SortOrder.ASCENDING)});
        q.setStartIndex(2);
        SimpleFeatureCollection features = featureSource.getFeatures(q);
    ...
    public void testGetFeaturesWithOffsetLimitAndPostFilter() throws Exception {
        ...
        // no sorting, let's see if the database can use native one
        FilterFactory ff = dataStore.getFilterFactory();
        PropertyIsEqualTo filter = ff.equal(ff.literal("one"), ff.function("strToLowerCase", ff
                .property(aname("stringProperty"))), true);
        q.setFilter(filter);
        q.setStartIndex(0);
        q.setMaxFeatures(1);
        SimpleFeatureCollection features = featureSource.getFeatures(q);
    ...
    public void testRendererBehaviour() throws Exception {
        ...
        query.setHints(new Hints(new Hints(Hints.JTS_COORDINATE_SEQUENCE_FACTORY, new 
    ...
    public void testQueryCapabilitiesSort() throws Exception {
        ...
        assertTrue(caps.supportsSorting(new SortBy[]{ff.sort(aname("intProperty"), SortOrder
                .ASCENDING)}));
        assertTrue(caps.supportsSorting(new SortBy[]{ff.sort(aname("stringProperty"), SortOrder
                .DESCENDING)}));
        assertTrue(caps.supportsSorting(new SortBy[]{ff.sort(aname("doubleProperty"), SortOrder
                .ASCENDING)}));

        // but we cannot sort geometries
        assertFalse(caps.supportsSorting(new SortBy[]{ff.sort(aname("geometry"), SortOrder
                .ASCENDING)}));
    }
    ...
    public void testNotFilter() throws Exception {
        ...
        assertEquals(featureSource.getCount(Query.ALL) - 1, featureSource.getCount(new Query
                (null, f)));
    }
