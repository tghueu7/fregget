...
public abstract class JDBCTypeNamesTestSetup extends JDBCDelegatingTestSetup {
   ...
   * following schema:
   *
   * <p><code>create view ftnview as select id, geom from ft_table</code> In Addition to that, there
   * should be some database specific type structures like synonyms or aliases, if available.
   * (should be )
   */
