...
public abstract class JDBCViewOnlineTest extends JDBCTestSupport {
  ...
  /**
   * Whether the pk field in a view is nillable or not (it is for most databases, but not for Oracle
   * for example).
   *
   * @return
   */
  ...
  /**
   * Subclasses may want to override this in case the database has a native way, other than the pk,
   * to identify a row
   *
   * @throws Exception
   */
