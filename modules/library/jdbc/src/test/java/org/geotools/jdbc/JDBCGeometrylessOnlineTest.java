...
public abstract class JDBCGeometrylessOnlineTest extends JDBCTestSupport {
    ...
    protected void connect() throws Exception {
        ...
        personSchema = DataUtilities.createType(dataStore.getNamespaceURI() + "." + PERSON, ID + 
                ":0," + NAME + ":String," + AGE + ":0");
        zipCodeSchema = DataUtilities.createType(dataStore.getNamespaceURI() + "." + ZIPCODE, ID 
                + ":0," + CODE + ":String");
    }
    ...
    public void testWriteFeatures() throws Exception {
        try (FeatureWriter fw = dataStore.getFeatureWriterAppend(tname(PERSON), Transaction
                .AUTO_COMMIT)) {
