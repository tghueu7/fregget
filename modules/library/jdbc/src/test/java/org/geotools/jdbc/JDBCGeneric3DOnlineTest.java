...
public abstract class JDBCGeneric3DOnlineTest extends JDBCTestSupport {
  ...
  public void testWriteLine() throws Exception {
    ...
    LineString ls =
        gf.createLineString(new Coordinate[] {new Coordinate(0, 0, 0), new Coordinate(1, 1, 1)});
  ...
  public void testCreateSchemaAndInsertPolyTriangle() throws Exception {
    ...
    LinearRing shell =
        gf.createLinearRing(csf.create(new double[] {0, 0, 99, 1, 0, 33, 1, 1, 66, 0, 0, 99}, 3));
  ...
  public void testCreateSchemaAndInsertPolyRectangle() throws Exception {
    ...
    LinearRing shell =
        gf.createLinearRing(
            csf.create(new double[] {0, 0, 99, 1, 0, 33, 1, 1, 66, 0, 1, 33, 0, 0, 99}, 3));
    Polygon poly = gf.createPolygon(shell, null);
  ...
  public void testCreateSchemaAndInsertPolyRectangleWithHole() throws Exception {
    ...
    LinearRing shell =
        gf.createLinearRing(
            csf.create(new double[] {0, 0, 99, 10, 0, 33, 10, 10, 66, 0, 10, 66, 0, 0, 99}, 3));
    LinearRing hole =
        gf.createLinearRing(
            csf.create(new double[] {2, 2, 99, 3, 2, 44, 3, 3, 99, 2, 3, 99, 2, 2, 99}, 3));
  ...
  public void testCreateSchemaAndInsertPolyWithHoleCW() throws Exception {
    ...
    LinearRing shell =
        gf.createLinearRing(
            csf.create(new double[] {1, 1, 99, 10, 1, 33, 10, 10, 66, 1, 10, 66, 1, 1, 99}, 3));
    LinearRing hole =
        gf.createLinearRing(
            csf.create(new double[] {2, 2, 99, 8, 2, 44, 8, 8, 99, 2, 8, 99, 2, 2, 99}, 3));
    Polygon poly = gf.createPolygon(shell, new LinearRing[] {hole});
  ...
  /**
   * Creates the polygon schema, inserts a 3D geometry into the datastore, and retrieves it back to
   * make sure 3d data is preserved.
   *
   * @throws Exception
   */
  private void checkCreateSchemaAndInsert(Geometry poly) throws Exception {
    ...
    /**
     * Use a LiteCoordinateSequence, since this mimics GeoServer behaviour better, and it exposes
     * bugs in CoordinateSequence handling.
     */
   ...
   * locations. Requires that each geometry is internally location-consistent in Z; that is, if two
   * coordinates are identical in location, then the Z values are equal. This should always be the
   * case for valid data.
   *
   * @param g1
   * @param g2
  ...
  public void testRendererBehaviour() throws Exception {
    ...
    Hints hints =
        new Hints(Hints.JTS_COORDINATE_SEQUENCE_FACTORY, new LiteCoordinateSequenceFactory());
    q.setHints(hints);
