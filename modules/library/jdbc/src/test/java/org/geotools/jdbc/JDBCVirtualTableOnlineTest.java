...
public abstract class JDBCVirtualTableOnlineTest extends JDBCTestSupport {
    ...
    protected void connect() throws Exception {
        ...
        vt.addGeometryMetadatata(aname("geom"), LineString.class, 4326);
        vt.addParameter(new VirtualTableParameter("mul", "1", new RegexpValidator("[\\d\\" +
                ".e\\+-]+")));
    ...
    private void checkRiverReduced(SimpleFeatureType type) {
        ...
        assertEquals(4326, type.getGeometryDescriptor().getUserData().get(JDBCDataStore
                .JDBC_NATIVE_SRID));
    ...
    public void testWhereParam() throws Exception {
        ...
        dialect.encodeColumnName(aname("flow"), sb);
        sb.append(" > 4");
        q.setHints(new Hints(Hints.VIRTUAL_TABLE_PARAMETERS, Collections.singletonMap("where", sb
                .toString())));
    ...
    public void testMulParamValid() throws Exception {
        ...
        Query q = new Query(Query.ALL);
        q.setHints(new Hints(Hints.VIRTUAL_TABLE_PARAMETERS, Collections.singletonMap("mul", 
                "10")));
