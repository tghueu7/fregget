...
public abstract class JDBCVirtualTableOnlineTest extends JDBCTestSupport {
  ...
  private void checkRiverReduced(SimpleFeatureType type) {
    ...
    assertEquals(
        4326, type.getGeometryDescriptor().getUserData().get(JDBCDataStore.JDBC_NATIVE_SRID));
  ...
  public void testWhereParam() throws Exception {
    ...
    q.setHints(
        new Hints(
            Hints.VIRTUAL_TABLE_PARAMETERS, Collections.singletonMap("where", sb.toString())));
