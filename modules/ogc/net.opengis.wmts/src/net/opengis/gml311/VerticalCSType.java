/** */
package net.opengis.gml311;

/**
 *
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>Vertical CS Type</b></em>'.
 * <!-- end-user-doc -->
 * <!-- begin-model-doc -->
 * A one-dimensional coordinate system used to record the heights (or depths) of points. Such a
 * coordinate system is usually dependent on the Earth's gravity field, perhaps loosely as when
 * atmospheric pressure is the basis for the vertical coordinate system axis. A VerticalCS shall
 * have one usesAxis association.
 * <!-- end-model-doc -->
 *
 * @see net.opengis.gml311.Gml311Package#getVerticalCSType()
 * @model extendedMetaData="name='VerticalCSType' kind='elementOnly'"
 * @generated
 */
public interface VerticalCSType extends AbstractCoordinateSystemType {} // VerticalCSType
