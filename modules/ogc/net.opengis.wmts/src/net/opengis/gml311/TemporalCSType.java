/** */
package net.opengis.gml311;

/**
 *
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>Temporal CS Type</b></em>'.
 * <!-- end-user-doc -->
 * <!-- begin-model-doc -->
 * A one-dimensional coordinate system containing a single time axis, used to describe the temporal
 * position of a point in the specified time units from a specified time origin. A TemporalCS shall
 * have one usesAxis association.
 * <!-- end-model-doc -->
 *
 * @see net.opengis.gml311.Gml311Package#getTemporalCSType()
 * @model extendedMetaData="name='TemporalCSType' kind='elementOnly'"
 * @generated
 */
public interface TemporalCSType extends AbstractCoordinateSystemType {} // TemporalCSType
