 ...
 *
 * <p>Why? ArcSDEConnections are both expensive to set up and expensive in cache. We still use an
 * ObjectCache internally in order to control time out behavior.
 *
 * <p>The trick is this time that Transaction.AUTO_COMMIT is only treated as suggestion for read
 * only code. The system will allow one transaction to be underway at any point, and will hand out
 * that connection to code that knows how to ask.
 *
 * <p>This is an aggressive experiment designed to cut down the number of connections needed.
 *
 * <p>
 *
 * @author Jody Garnett
