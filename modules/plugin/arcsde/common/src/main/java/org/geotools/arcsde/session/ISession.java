...
public interface ISession {
  ...
  /**
   * Returns the live list of layers, not the cached ones, so it may pick up the differences in the
   * database.
   *
   * @return
   * @throws IOException
   ...
   *
   * <p>The point in returning an {@link SdeRow} instead of a plain {@link SeRow} is that the former
   * prefetches the row values and this can be freely used outside a {@link Command}. Otherwise the
   * SeRow should only be used inside a command as accessing its values implies using the
   * connection.
   *
   * @param query
   * @return
