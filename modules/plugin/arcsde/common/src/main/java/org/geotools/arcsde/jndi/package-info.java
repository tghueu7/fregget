 ...
 * <p>
 * The JNDI specification acts only at the API level, so how to configure a given resource to be 
 * accessible
 ...
 * <li>{@code commons-pool-1.3.jar} (or whatever commons-pool version the gt-arcsde-common 
 * depends on)
 * </ul>
 ...
 * </code>
 * </pre>
 * Where the values for description and res-ref-name are of your choice, res-ref-name being the 
 * path under
 * which the arcsde connection pool is configured on your JNDI container (see bellow for 
 * instructions on how
 ...
 * files to the Tomcat shared libs folder (either <catalina home>/lib or <catalina 
 * home>/common/lib folder,
 ...
 * </ul>
 * Finally, configure the {@link ArcSDEConnectionFactory ArcSDE Connection Factory} as explained 
 * in the
 * <i>Adding Custom Resource Factories</i> section of the Tomcat's
 * <a href="http://tomcat.apache.org/tomcat-6.0-doc/jndi-resources-howto.html">JNDI Resources 
 * HOW-TO</a>. That is,
 * add an entry as the following to the file {@code <catalina home>/conf/context.xml} as a child 
 * of the root
 * {@code <Context>} element:
 * <pre>
 * <code>
 *   &lt;Resource name="geotools/arcsde" auth="Container" type="org.geotools.arcsde.session
 *   .ISessionPool"
 ...
 * Disregard what the JBoss documentation says on its <a href="http://www.jboss
 * .org/file-access/default/members/jbossweb/freezone/docs/latest/jndi-resources-howto.html"
 ...
 * that configures the ArcSDE connection pool. You can call this file as you want, for example 
 * {@code geoserver-service.xml},
 ...
 * </p>
 * <p>
 * The contents of the {@code geoserver-service.xml} file shall be as following. For more 
 * information about this kind of
 * configuration consult the JBoss' 
 * <a href="http://www.jboss.org/community/wiki/JNDIBindingServiceMgr">JNDIBindingServiceMgr</a>
 * documentation.
 * <pre>
 * <code>
 ...
 * </p>
 * <p>
 * What we're doing here is to create a {@code java.util.Properties} object containing the 
 * connection parameters, and storing
 * it on the JNDI container at the {@code geotools/arcsdeGlobal} path. This is going to be a 
 * globally accessible resource that
 * the {@link ArcSDEConnectionFactory} will use to create the appropriate 
 * {@link org.geotools.arcsde.session.ISessionPool connection pool}.
 * </p>
 * <p>
 * But to get this resource visible to your web application, there's one more step missing. In 
 * addition to the resource-ref
 * entry in {@code WEB-INF/web.xml} mentioned above, JBoss requires an extra config file to 
 * manage the indirection between
 * your web application and the globally configured JNDI resource.
 * You'll need to create a file called {@code jboss-web.xml} as a sibling of {@code WEB-INF/web
 * .xml} with the following content:
 * <pre>
 * <code>
 * <?xml version="1.0" encoding="UTF-8"?>
 * <jboss-web>
 ...
 * </code>
 * </pre>
 * Where {@code geotools/arcsdeGlobal} is the name of the globally configured resource, and 
 * {@code geotools/arcsde}
 * is the JNDI name by which the resource will actually be accessible to your web application, 
 * and shall match
 * the name used for the resource-ref entry in {@code WEB-INF/web.xml}.
 * </p>
 * <p>
 * Now you're ready to select the <i>ArcSDE JNDI</i> DataStore factory and use the {@code 
 * "geotools/arcsde"}
 * (or whatever entry name of your choice you have configured) as the DataStore's JNDI resource 
 * path name.
 * </p>
 */
