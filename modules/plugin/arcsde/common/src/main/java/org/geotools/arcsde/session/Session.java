...
class Session implements ISession {
  ...
  /**
   * Keeps track of the number of references to this session (ie, how many times it has been {@link
   * #markActive() activated} so it's only actually {@link #dispose() disposed} when the reference
  ...
  public void dispose() throws IllegalStateException {
    ...
    if (refCount > 0) {
      ...
      if (LOGGER.isLoggable(Level.FINEST)) {
        LOGGER.finest(
            "---------> Ignoring disposal, ref count is still " + refCount + " for " + this);
      }
    ...
    if (autoCloseCounter >= AUTO_CLOSE_COUNTER_THRESHOLD) {
      LOGGER.warning(
          "Auto-closing " + this + " to avoid memory leak in ESRI Java API (see GEOT-3227)");
      this.destroy();
    }
    try {
  ...
  /**
   * @see ISession#createAndExecuteQuery(java.lang.String[], com.esri.sde.sdk.client.SeSqlConstruct)
   */
  ...
  private static final class CreateSeConnectionCommand extends Command<SeConnection> {
    ...
    /**
     * @param config
     * @param sessionId the session id the connection is to be created for. For exception reporting
     *     purposes only
     */
    ...
    public SeConnection execute(final ISession session, final SeConnection connection)
      ...
      try {
        for (int i = 0; i < 3; i++) {
          try {
            if (LOGGER.isLoggable(Level.FINE)) {
              LOGGER.fine(
                  "Creating connection for session #" + sessionId + "(try " + (i + 1) + " of 3)");
            }
        ...
        throw new ArcSdeException(
            "Can't create connection to " + serverName + " for Session #" + sessionId, e);
