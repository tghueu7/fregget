...
public class RasterTestData {
  ...
  public String getRasterTableName(final RasterCellType cellType, final int numBands)
    ...
    String testTableName = testData.getTempTableName() + "_" + cellType + "_" + numBands + "_BAND";
    return testTableName;
  }
  ...
  public String loadRGBColorMappedRaster() throws Exception {
    ...
    SeExtent imgExtent = new SeExtent(231000, 898000, 231000 + 500, 898000 + 500);
    SeCoordinateReference crs = getSeCRSFromPeProjectedCSId(PePCSDefs.PE_PCS_NAD_1983_HARN_MA_M);
    String rasterFilename = (String) testData.getConProps().get("sampledata.rgbraster-colormapped");
    ArcSDERasterProducer prod = new ArcSDERasterOneBytePerBandProducerImpl();
  ...
  public String loadOneByteGrayScaleRaster() throws Exception {
    ...
    SeExtent imgExtent = new SeExtent(231000, 898000, 231000 + 500, 898000 + 500);
    SeCoordinateReference crs = getSeCRSFromPeProjectedCSId(PePCSDefs.PE_PCS_NAD_1983_HARN_MA_M);
    String rasterFilename = (String) testData.getConProps().get("sampledata.onebyteonebandraster");
    ArcSDERasterProducer prod = new ArcSDERasterOneBytePerBandProducerImpl();
        ...
        new Command<Void>() {
          ...
          public Void execute(ISession session, SeConnection connection)
            ...
            BufferedImage sampleImage =
                ImageIO.read(org.geotools.test.TestData.getResource(null, rasterFilename));
            {
            ...
            if (indexCM != null) {
              attr =
                  getRasterAttributes(
                      connection, tableName, new Rectangle(0, 0, 0, 0), 0, new int[] {1});
              ...
              final int colorMapType =
                  numBanks == 3 ? SeRaster.SE_COLORMAP_RGB : SeRaster.SE_COLORMAP_RGBA;
   ...
   * @param colorModel the color model to apply to the raster, may be {@code null}. A non null value
   *     adds as precondition that {@code numberOfBands == 1}
   * @throws Exception
   */
   ...
   * @param skipLevelOne only relevant if {@code pyramiding == true}, {@code true} indicates not to
   *     create pyramid tiles for the first level, since its equal in dimension than the source
   *     raster
                  ...
                  public void run() {
                    try {
                      ...
                      for (int bandN = 0; bandN < numberOfBands; bandN++) {
                        ...
                        imgBandData =
                            sampler.getImgBandData(imageWidth, imageHeight, bandN, numberOfBands);
        ...
        new Command<Void>() {
          ...
          public Void execute(ISession session, SeConnection connection)
            ...
            if (colorModel != null) {
              try {
                SeQuery query =
                    new SeQuery(connection, new String[] {"RASTER"}, new SeSqlConstruct(tableName));
                query.prepareQuery();
                query.execute();
  ...
  public abstract static class PixelSampler {
    ...
    public static PixelSampler getSampler(final RasterCellType pixelType) {
      ...
      if (sampler == null) {
        throw new NoSuchElementException("no pixel sampler exists for pixel type " + pixelType);
      }
      return sampler;
    }
    ...
    /**
     * Returns the programatically generated test data for a band of {@code bandWidth x bandHeight}
     * for the band {@code bandN} of {@code numBand} total bands.
     *
     * @param bandWidth
     * @param bandHeight
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_8BIT_U}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_8BIT_S}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_16BIT_U}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_16BIT_S}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_32BIT_U}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
    ...
    /**
     * Pixel sampler for creating test data with pixel type {@link SeRaster#SE_PIXEL_TYPE_32BIT_S}
     *
     * @see PixelSampler#getImgBandData(int, int)
     */
          ...
          new Command<Void>() {
            ...
            public Void execute(ISession session, SeConnection connection)
              ...
              for (SeRasterColumn rasterCol : rasterCols) {
                ...
                SeQuery query =
                    new SeQuery(connection, new String[] {colName}, new SeSqlConstruct(tableQName));
                query.prepareQuery();
                query.execute();
                SeRow row = query.fetch();
                if (row == null) {
                ...
                for (SeRasterBand band : bands) {
                  ...
                  System.out.println(
                      "\t Band nº " + bandNumber + ": has color map: " + hasColorMap);
                  // if (hasColorMap) {
  ...
  public static RenderedImage createExpectedImage(
    ...
    SampleModel sm =
        new ComponentSampleModel(
            DataBuffer.TYPE_USHORT, width, height, pixelStride, scanLineStride, bandOffsets);
    DataBuffer db = new DataBufferUShort(width * height);
    WritableRaster wr = Raster.createWritableRaster(sm, db, null);
    ColorSpace cs = ColorSpace.getInstance(ColorSpace.CS_GRAY);
    ColorModel cm =
        new ComponentColorModel(cs, false, true, Transparency.OPAQUE, DataBuffer.TYPE_USHORT);
