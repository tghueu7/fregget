...
public class TestData {
  ...
  /**
   * creates an ArcSDEDataStore using {@code test-data/testparams.properties} as holder of datastore
   * parameters
   */
  ...
  /**
   * Creates an ArcSDE feature type names as <code>getTemp_table()</code> on the underlying database
   * and if <code>insertTestData == true</code> also inserts some sample values.
   *
  ...
  /**
   * Creates an ArcSDE feature type names as <code>getTemp_table()</code> on the underlying database
   * with no sample values. Sets flag to set an empty extent
   *
   * @throws Exception for any error
   */
    ...
    Command<SeColumnDefinition[]> createTableCmd =
        new Command<SeColumnDefinition[]>() {
          ...
          public SeColumnDefinition[] execute(ISession session, SeConnection connection)
            ...
            colDefs[0] =
                new SeColumnDefinition("ROW_ID", SeColumnDefinition.TYPE_INT32, 10, 0, false);

            colDefs[1] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[0], SeColumnDefinition.TYPE_INT32, 10, 0, false);
            colDefs[2] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[1], SeColumnDefinition.TYPE_INT16, 4, 0, isNullable);
            colDefs[3] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[2], SeColumnDefinition.TYPE_FLOAT32, 5, 2, isNullable);
            colDefs[4] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[3], SeColumnDefinition.TYPE_FLOAT64, 25, 4, isNullable);
            colDefs[5] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[4], SeColumnDefinition.TYPE_STRING, 25, 0, isNullable);

            colDefs[6] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[5], SeColumnDefinition.TYPE_NSTRING, 25, 0, isNullable);

            colDefs[7] =
                new SeColumnDefinition(
                    TEST_TABLE_COLS[6], SeColumnDefinition.TYPE_DATE, 1, 0, isNullable);
            // this is a blob one and should be ignored to all effects
            colDefs[8] =
                new SeColumnDefinition(
                    "SE_ANNO_CAD_DATA", SeColumnDefinition.TYPE_BLOB, 1000, 0, isNullable);
  ...
  public SimpleFeatureCollection createTestFeatures(
      Class<? extends Geometry> jtsGeomType, int numFeatures) throws IOException, SeException {
        ...
        new Command<Void>() {
          ...
          public Void execute(ISession session, SeConnection connection)
            ...
            colDefs[0] =
                new SeColumnDefinition("ROW_ID", SeColumnDefinition.TYPE_INT32, 10, 0, false);
            colDefs[1] =
                new SeColumnDefinition("INT_COL", SeColumnDefinition.TYPE_INT32, 10, 0, isNullable);
            colDefs[2] =
            ...
            colDefs[4] =
                new SeColumnDefinition(
                    "SE_ANNO_CAD_DATA", SeColumnDefinition.TYPE_BLOB, 4000, 0, isNullable);
        ...
        new Command<SeTable>() {
          ...
          public SeTable execute(ISession session, SeConnection connection)
            ...
            colDefs[0] =
                new SeColumnDefinition("ROW_ID", SeColumnDefinition.TYPE_INT32, 10, 0, false);
            colDefs[1] =
                new SeColumnDefinition("NAME", SeColumnDefinition.TYPE_STRING, 25, 0, isNullable);
  ...
  public void insertIntoVersionedTable(
      final ISession session, final SeState state, final String tableName, final String nameField)
      throws IOException {
