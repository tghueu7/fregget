...
public class SDEJavaApiJoinTest {
  ...
  public void testViewBounds() throws IOException {
    SelectBody select =
        ViewRegisteringFactoryHelper.parseSqlQuery(InProcessViewSupportTestData.masterChildSql);
    store.registerView(InProcessViewSupportTestData.typeName, (PlainSelect) select);
  ...
  public void testReadView() throws Exception {
    ...
    Query query =
        new Query(InProcessViewSupportTestData.typeName, Filter.INCLUDE, Query.ALL_PROPERTIES);
  ...
  public void testApiAlias() throws Exception {
    ...
    SeSqlConstruct sqlConstruct = new SeSqlConstruct();
    String[] tables = {
      InProcessViewSupportTestData.MASTER + " MASTER", InProcessViewSupportTestData.CHILD + " CHILD"
    };
    sqlConstruct.setTables(tables);
  ...
  public void testApiGroupBy() throws Exception {
    ...
    // tricky part is that SHAPE column must always be the last one
    String[] propertyNames = {
      InProcessViewSupportTestData.MASTER + ".ID", InProcessViewSupportTestData.CHILD + ".NAME" /*
                                                              * , MASTER + ".SHAPE"
                                                              */
    };
    ...
    queryInfo.setByClause(
        " GROUP BY " + groupBy + " ORDER BY " + InProcessViewSupportTestData.CHILD + ".NAME DESC");
