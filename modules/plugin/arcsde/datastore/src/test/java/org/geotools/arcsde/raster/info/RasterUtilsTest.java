...
public class RasterUtilsTest {
  ...
  private void assertColorModel(DataBuffer expected, IndexColorModel actual, int bitsPerSample) {
    ...
    for (int elem = 0; elem < size; elem++) {
      for (int bank = 0; bank < numBanks; bank++) {
        ...
        assertEquals(
            "at index " + elem + ", bank " + bank, expected.getElem(bank, elem), actualValue);
      }
    }
  }
  ...
  private void assertDetermineTargetCellType(
      RasterCellType nativeCellType, RasterCellType expectedTargetCellType, Number... noData) {
  ...
  public void testDeterminaNoDataValueNonColorMapped() {
    ...
    assertDetermineNoDataValue(
        ...
        TYPE_4BIT,
        16,
        "4BIT no-data should have been set to 16, " + "regardless of the dataset used values");
    ...
    assertDetermineNoDataValue(
        1, Integer.MIN_VALUE, Integer.MAX_VALUE, TYPE_32BIT_S, (long) Integer.MIN_VALUE - 1, null);
    assertDetermineNoDataValue(1, 1, Integer.MAX_VALUE, TYPE_32BIT_S, 0, null);
    assertDetermineNoDataValue(
        1, Integer.MIN_VALUE, Integer.MAX_VALUE - 1L, TYPE_32BIT_S, Integer.MAX_VALUE, null);
  ...
  private void assertDetermineNoDataValue(
    ...
    if (failureMsg == null) {
      ...
      assertEquals(failureMsg, expectedNoDataValue.doubleValue(), noDataValue.doubleValue(), 0);
    }
  }
}
