...
public class ArcSDEFeatureStoreTest {
  ...
  public void testFeatureEventsAndTransactionsForAdd() throws Exception {
    ...
    assertFalse(
        "confirm tempFid is not in the commit", id.getIdentifiers().contains(tempFeatureId));
              ...
              new Command<SeQuery>() {
                ...
                public SeQuery execute(ISession session, SeConnection connection)
                  ...
                  SeQuery seQuery =
                      new SeQuery(
                          connection,
                          new String[] {"ROW_ID", "INT32_COL", "STRING_COL"},
  ...
  public void testWriteAndUpdateNullShapes() throws Exception {
    ...
    try {
      ...
      feature = reader.next();
      LOGGER.info(
          "recovered geometry " + feature.getDefaultGeometry() + " from single inserted feature.");
   ...
   * another thread accesses the same FeatureStore with a query. Archetypical use case being a udig
   * addFeatures command sends calls addFeatures and the rendering thread does getFeatures.
   */
  @Test
