...
public class ArcSDEDataStoreFactoryTest {
  ...
  /**
   * Aset of datastore parameters that though valid (contains all the required parameters) point to
   * a non available server
   */
  ...
  public void testVersionParamCheck() throws IOException, UnavailableConnectionException {
    ...
    try {
      ...
      assertTrue(
          e.getMessage(), e.getMessage().startsWith("Specified ArcSDE version does not exist"));
    }
  }
