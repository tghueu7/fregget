...
public class ArcSDEFeatureSourceTest {
  ...
  /**
   * This method tests the feature reader by opening various simultaneous FeatureReaders using the 3
   * test tables.
   *
   ...
   * method is called or hasNext() returns flase, wich automatically closes the stream. If more than
   * 24 simultaneous streams are tryied to be opened upon a single SeConnection, an exception is
   * thrown by de Java ArcSDE API saying that a "NETWORK I/O OPERATION FAILED"
   */
  @Test
   ...
   */
  @Test
  public void testRestrictsAttributes() throws IOException, IllegalAttributeException, SeException {
    final String typeName = testData.getTempTableName();
  ...
  /**
   * Checks that arcsde datastore returns featuretypes whose attributes are exactly in the requested
   * order.
   */
  @Test
   ...
   *
   * <p>"Mixed queries" refers to mixing alphanumeric and geometry based filters, since that is the
  ...
  public void testMixedQueries() throws Exception {
    ...
    Not not =
        ff.not(ff.id(Collections.singleton(ff.featureId(testData.getTempTableName() + ".90000"))));
  ...
  /**
   * to expose GEOT-408, tests that queries in which only non spatial attributes are requested does
   * not fails due to the datastore trying to parse the geometry attribute.
   *
   * @throws Exception
   */
  @Test
  ...
  public void testFidFilters() throws Exception {
    ...
    try {
      while (iterator.hasNext()) {
        ...
        assertTrue(
            "a fid not included in query was returned: " + fid, fids.contains(ff.featureId(fid)));
      }
    } finally {
      iterator.close();
    }
  ...
  public void testQueryCapabilities() throws Exception {
    ...
    assertFalse(
        queryCapabilities.supportsSorting(new SortBy[] {ff.sort("nonExistent", ASCENDING)}));
    ...
    assertFalse(
        queryCapabilities.supportsSorting(new SortBy[] {ff.sort(geometryAttribute, ASCENDING)}));
