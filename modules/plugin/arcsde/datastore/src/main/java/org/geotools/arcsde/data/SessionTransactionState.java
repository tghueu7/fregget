...
final class SessionTransactionState implements Transaction.State {
   ...
   * @param pool connection pool where to grab a connection and hold it while there's a transaction
  ...
  /**
   * Commits the transaction and returns the connection to the pool. A new one will be grabbed when
   * needed.
   *
   * <p>Preconditions:
   *
   ...
   * @param transaction transaction information, <code>null</code> signals this state lifecycle end.
  ...
  /**
   * Used only within the package to provide access to a single connection on which this transaction
   * is being conducted.
   *
   ...
   *
   * @return the SessionTransactionState stored in the transaction with <code>connectionPool</code>
   *     as key.
   */
   ...
   * @version $Id$
   * @since 2.5.x
   * @source $URL: http://svn.geotools.org/trunk/modules/plugin/arcsde/datastore/src/main/java/org/
