...
final class NativeTileReader implements TileReader {
  ...
  /**
   * Creates a {@link TileReader} that reads tiles out of ArcSDE for the given {@code preparedQuery}
   * and {@code SeRow} using the given {@code session}, in the native raster format.
   *
  ...
  /**
   * How many random tiles requested (ie, not consecutive) before re executing the original request
   * as a rewind
   */
  ...
  public void dispose() {
    if (session != null) {
      ...
      if (LOGGER.isLoggable(Level.FINER)) {
        LOGGER.finer(
            "TileReader disposing " + session + " on Thread " + Thread.currentThread().getName());
      }
  ...
  /**
   * Disposes as to make sure the {@link ISession session} is returned to the pool even if a failing
   * or non careful client left this object hanging around
   *
   * @see #dispose()
