...
public class ArcSdeSimplifyingFilterVisitor extends SimplifyingFilterVisitor {
    ...
    // list)
    static final Set<Class<? extends BinarySpatialOperator>> SPATIAL_OPERATIONS = new 
            HashSet<Class<? extends BinarySpatialOperator>>();
    ...
    public Object visit(Or filter, Object extraData) {
        ...
        if (simplified instanceof Or) {
            ...
            Map<String, List<SpatialOperation>> spatialOps = new HashMap<String, 
                    List<SpatialOperation>>();
            ...
            for (Filter child : children) {
                ...
                if (child instanceof BinarySpatialOperator) {
                    ...
                    if (bso.getExpression1() instanceof PropertyName && bso.getExpression2() 
                            instanceof Literal) {
                        name = ((PropertyName) bso.getExpression1()).getPropertyName();
                        so = new SpatialOperation(bso);
                    } else if (bso.getExpression2() instanceof PropertyName && bso.getExpression1
                            () instanceof Literal) {
            ...
            for (String property : spatialOps.keySet()) {
                ...
                // we perform a reduction on the list of filters, trying to find groups that can 
                // be merged
                while (propertyFilters.size() > 0) {
                    ...
                    for (int j = 1; j < propertyFilters.size(); ) {
                        ...
                        if (secondary.operation == main.operation
                                ...
                                Intersects.class)
                                || (secondary.operation == Intersects.class && main.operation == 
                                BBOX.class)) {
    ...
    private Filter mergeOperations(String propertyName, List<SpatialOperation> ops) {
        ...
        GeometryCollection collection = geomArray[0].getFactory().createGeometryCollection
                (geomArray);
        ...
        if (BBOX.class.isAssignableFrom(operation) || Intersects.class.isAssignableFrom
                (operation)) {
    ...
    static class SpatialOperation {
        ...
        public SpatialOperation(BinarySpatialOperator op) {
            ...
            if (op.getExpression1() instanceof Literal) {
                ...
                throw new IllegalArgumentException("Cannot find literal geometry in the spatial " +
                        "filter");
            }
        }
    }
