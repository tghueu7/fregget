...
public final class RasterDatasetInfo {
  ...
  /**
   * raster column names on this raster. If there's more than one raster column (is this possible?)
  ...
  private List<GridSampleDimension> buildSampleDimensions() {
    ...
    for (RasterBandInfo band : bands) {
      ...
      if (geophysics) {
        ...
        Category nodataCat =
            new Category(
                Vocabulary.formatInternational(VocabularyKeys.NODATA), transparent, noDataValue);
    ...
    if (!this.renderedImageSpec.containsKey(Integer.valueOf(rasterIndex))) {
      synchronized (this) {
        if (!this.renderedImageSpec.containsKey(Integer.valueOf(rasterIndex))) {
          ...
          imageTypeSpecifier = RasterUtils.createFullImageTypeSpecifier(this, rasterIndex);
