...
final class ArcSDEAttributeReader implements AttributeReader {
  ...
  /**
   * lenght of the prefix string for creating string based feature ids, used to truncate the <code>
   * fidPrefix</code> and append it the SeShape's feature id number
   */
   ...
   * @param session the session the <code>query</code> is being ran over. This attribute reader will
  ...
  private Geometry adaptGeometry(final Geometry value, Class<? extends Geometry> targetType) {
    ...
    if (MultiPoint.class == targetType && Point.class == currentClass) {
      ...
      throw new IllegalArgumentException(
          "Don't know how to adapt " + currentClass.getName() + " to " + targetType.getName());
    }
    return adapted;
  }
