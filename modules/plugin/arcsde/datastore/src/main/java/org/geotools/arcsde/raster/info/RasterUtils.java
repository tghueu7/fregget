...
public class RasterUtils {
  ...
  /**
   * Returns the grid range specifying the matching tiles for a given pyramid level and grid extent
  ...
  private static GridEnvelope findMatchingTiles(
      final Dimension tileSize, int numTilesWide, int numTilesHigh, final GridEnvelope pixelRange) {
  ...
  private static GridEnvelope getTargetGridRange(
    ...
    int levelMaxPixelY;
    {
      ...
      levelOverlappingPixels = new GridEnvelope2D(levelMinPixelX, levelMinPixelY, width, height);
    }
  ...
  private static ImageTypeSpecifier createFourBitColorMappedImageSpec(
    ...
    IndexColorModel colorModel =
        new IndexColorModel(8, mapSize, cmap, 0, true, transparentPixel, DataBuffer.TYPE_BYTE);
  ...
  private static ImageTypeSpecifier createOneBitColorMappedImageSpec(
    ...
    int transparentPixel = noDataValue;
    IndexColorModel colorModel =
        new IndexColorModel(8, mapSize, cmap, 0, false, transparentPixel, DataBuffer.TYPE_BYTE);
  ...
  private static ImageTypeSpecifier createRGBAImageSpec(
    ...
    ColorModel colorModel =
        new ComponentColorModelJAI(
            colorSpace, nBits, hasAlpha, isAlphaPremultiplied, transparency, transferType);
  ...
  public static void fitRequestToRaster(
    ...
    try {
      ...
      Rectangle2D finalEnvelope =
          CRS.transform(rasterToModel, (Rectangle2D) resultGridRange, new Rectangle2D.Double());
   ...
   * transparent pixel whose index can be used as no-data value for colormapped rasters, even if the
   * returned IndexColorModel needs to be of a higher sample depth (ie, 16 instead of 8 bit) to
   * satisfy that.
   *
   * @param colorMap the raster's native color map the returned one will be based on
   * @return the same {@code colorMap} if it has a transparent pixel, another, possibly of a higher
   *     depth one if not, containing all the colors from {@code colorMap} and a newly allocated
   *     cell for the transparent pixel if necessary
   */
  ...
  /**
   * @param numBands number of bands in the raster dataset for the band whose nodata value is to be
   ...
   * @param statsMin the minimum sample value for the band as reported by the band's statistics, or
   *     {@code NaN}
   * @param statsMax the maximum sample value for the band as reported by the band's statistics, or
   *     {@code NaN}
  ...
  public static Number determineNoDataValue(
    ...
    if (nativeCellType == TYPE_32BIT_REAL) {
      ...
      LOGGER.fine(
          "1BIT images no-data value is set to 2," + " regardless of the raster statistics");
      return Double.valueOf(2);
    } else if (nativeCellType == TYPE_4BIT) {
      LOGGER.fine(
          "4BIT images no-data value is set to 16," + " regardless of the raster statistics");
    ...
    if (TYPE_32BIT_REAL == nativeCellType || TYPE_64BIT_REAL == nativeCellType) {
      ...
      for (Number nodata : noDataValues) {
        if (!Double.isNaN(nodata.doubleValue())) {
          throw new IllegalArgumentException(
              "no data values for float and " + "double cell types shall be NaN: " + nodata);
        }
      }
