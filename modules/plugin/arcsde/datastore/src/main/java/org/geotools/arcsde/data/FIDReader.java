...
public abstract class FIDReader {
   ...
   *
   * @param the feature type containing the properties the client code is interested in. May well be
   *     a subset of the full set of attributes in the SeLayer
  ...
  public static FIDReader getFidReader(
      final ISession session, final SeTable table, final SeLayer layer, final SeRegistration reg)
      throws IOException {
  ...
  public static class ShapeFidReader extends FIDReader {
    ...
    /**
     * Overrides to include the geometry column whether it is required by the {@code schema} or not,
     * since we need to get the fid from the geometry id.
     */
    @Override
    public String[] getPropertiesToFetch(SimpleFeatureType schema) throws IOException {
      List<String> attNames = new ArrayList<String>(schema.getAttributeCount() + 1);
