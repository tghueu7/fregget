...
public abstract class ArcSDEGeometryBuilder {
  ...
  /**
   * Takes an ArcSDE's <code>SeShape</code> and builds a JTS Geometry. The geometry type constructed
   * depends on this <code>ArcSDEGeometryBuilder</code> specialized subclass
   *
   * @param shape the ESRI's ArcSDE java api shape upon wich to create the new JTS geometry
   * @param geometryFactory
   * @return the type of JTS Geometry this subclass instance is specialized for or an empty geometry
   *     of the same class if <code>shape.isNil()</code>
   ...
   * @param seSrs Coordinate Reference System of the underlying <code>SeLayer</code> object for wich
  ...
  protected abstract Geometry newGeometry(
      final double[][][] coords, final GeometryFactory geometryFactory) throws DataSourceException;
   ...
   *
   * @param coordList array of coordinates of a single shape part to build a <code>Coordinate</code>
   *     from
   ...
   * @throws IllegalArgumentException if <code>jtsGeometryClass</code> is not a concrete JTS <code>
   *     Geometry</code> class (like <code>com.vividsolutions.jts.geom.MultiPoint.class</code> i.e.)
   */
  public static ArcSDEGeometryBuilder builderFor(Class<? extends Geometry> jtsGeometryClass)
    ...
    if (builder == null) {
      String msg =
          "no geometry builder is defined to construct " + jtsGeometryClass + " instances.";
  ...
  /**
   * <code>ArcSDEGeometryBuilder</code> which can create any type of JTS geometry from <code>SeShape
   * </code>'s and viceversa
   *
   * @author Gabriel Roldan, Axios Engineering
   * @version $Id$
   */
  private static class GenericGeometryBuilder extends ArcSDEGeometryBuilder {
    ...
    /**
     * Returns an instance of this geometry builder. Currently implemented as a singleton since it
    ...
    @Override
    protected Geometry newGeometry(final double[][][] coords, final GeometryFactory geometryFactory)
  ...
  /**
   * <code>ArcSDEGeometryBuilder</code> specialized in creating JTS <code>Point</code> s from <code>
   * SeShape</code> points and viceversa
   *
   * @author Gabriel Roldan, Axios Engineering
   * @version $Id$
   */
  private static class PointBuilder extends ArcSDEGeometryBuilder {
    ...
    /**
     * Returns an instance of this geometry builder for Point geometries. Currently implemented as a
     * singleton since it is completely thread safe.
     *
  ...
  private static class MultiPointBuilder extends ArcSDEGeometryBuilder {
    ...
    /**
     * Returns an instance of this geometry builder for MultiPoint geometries. Currently implemented
  ...
  private static class LineStringBuilder extends ArcSDEGeometryBuilder {
    ...
    /**
     * Returns an instance of this geometry builder for LineString geometries. Currently implemented
     * as a singleton since it is completely thread safe.
     *
  ...
  private static class PolygonBuilder extends ArcSDEGeometryBuilder {
    ...
    /**
     * Returns an instance of this geometry builder for Polygon geometries. Currently implemented as
     * a singleton since it is completely thread safe.
     *
    ...
    protected final Polygon buildPolygon(
        final double[] shellCoords, final double[][] holes, final GeometryFactory geometryFactory) {
      Polygon p = null;
    ...
    protected Polygon buildPolygon(final double[][] parts, final GeometryFactory geometryFactory) {
      ...
      final CoordinateSequenceFactory coordinateSequenceFactory =
          geometryFactory.getCoordinateSequenceFactory();
      LinearRing shell =
          geometryFactory.createLinearRing(toCoords(linearCoordArray, coordinateSequenceFactory));
  ...
  /**
   * <code>ArcSDEGeometryBuilder</code> specialized in creating JTS <code>MultiPolygon</code> s from
   * <code>SeShape</code> multipolygons and viceversa
   *
   * @author Gabriel Roldan, Axios Engineering
   * @version $Id$
   */
  private static class MultiPolygonBuilder extends PolygonBuilder {
     ...
     * @return a <code>MultiPolygon</code> constructed based on the SDE shape, or the empty geometry
     *     if the <code>shape == null ||
     *         shape.isNil()</code>
     * @throws DataSourceException if it is not possible to obtain the shape's coordinate arrays or
     *     an exception occurs while building the Geometry
     */
    @Override
    protected Geometry newGeometry(final double[][][] coords, final GeometryFactory geometryFactory)
        throws DataSourceException {
