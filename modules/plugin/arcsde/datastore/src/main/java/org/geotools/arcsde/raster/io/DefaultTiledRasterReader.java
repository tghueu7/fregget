...
class DefaultTiledRasterReader implements TiledRasterReader {
  ...
  /**
   * Creates an {@link DefaultTiledRasterReader} that uses the given connection to fetch raster data
   * for the given {@link RasterDatasetInfo rasterInfo}.
   *
   * <p>
   *
  ...
  public RenderedImage read(
    ...
    tileReader =
        TileReaderFactory.getInstance(sessionPool, rasterInfo, rasterId, pyramidLevel, tileRange);
  ...
  /**
   * Creates an image representing the whole pyramid level but with a tile reader ready to read only
   * the required tiles, the calling code is responsible for cropping it as needed
   *
   * @param tileReader
   * @param tileRange
   * @param rasterId
