...
public class ArcSDEAdapter {
  ...
  static {
    ...
    /**
     * By now keep using the deprecated constants (TYPE_INTEGER, etc.), switching directly to the
     * new ones gives problems with ArcSDE instances prior to version 9.0.
     */
  ...
  public static FeatureTypeInfo fetchSchema(
      final String typeName, final String namespace, final ISession session) throws IOException {
    ...
    FeatureTypeInfo typeInfo =
        new FeatureTypeInfo(featureType, fidStrategy, canDoTransactions, isMultiVersioned, isView);
    return typeInfo;
  }
   ...
   *
   * @param permissions the sde table permissions mask (as per SeTable.getPermissions())to check for
   *     write permissions
  ...
  private static SimpleFeatureType createSchema(
      final String typeName, final String namespace, final List<AttributeDescriptor> properties)
      throws IOException {
   ...
   * @return the actual CRS or null if <code>sdeLayer</code> does not defines its coordinate system.
   ...
   *     shape type (shouldnt happen as for the {@link SeShape#getType() types} defined in the esri
   *     arcsde java api 9.0)
   */
  ...
  public static long getNumericFid(Identifier id) throws IllegalArgumentException {
    if (!(id instanceof FeatureId))
      throw new IllegalArgumentException(
          "Only FeatureIds are supported when encoding id filters to SDE.  Not " + id.getClass());
  ...
  /**
   * Holds default values for the properties (size and scale) of a SeColumnDefinition, given by its
   * column type (SeColumnDefinition.SE_STRING, etc).
   *
  ...
  public static void createSchema(
      final SimpleFeatureType featureType, final Map<String, String> hints, final ISession session)
          ...
          public Void execute(ISession session, SeConnection connection)
            ...
            try {
              ...
              if (unqualifiedTypeName.indexOf('.') == -1) {
                ...
                qualifiedName =
                    connection.getUser() + "." + unqualifiedTypeName; // featureType.getTypeName();
  ...
  private static void createSeLayer(
      SeLayer layer, String qualifiedName, GeometryDescriptor geometryAtt) throws SeException {
