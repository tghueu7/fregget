...
public class RasterBandInfo {
   ...
   * channel regardless of whether the native on has alpha or not, to account for the no-data pixel
   * to be a transparent one where appropriate
   */
  IndexColorModel nativeColorMap;

  /**
   * The color map as it is going to be used by this library. May differ from the native one either
   * in the number of elements (when the native color map is not full and it has no transparent
   * pixel to be used as no-data value), or in the pixel depth (when the native colormap is full,
   * has no no-data pixel, and hence it needs to be promoted to a higher transfer type to make room
   * for a no-data index)
   */
