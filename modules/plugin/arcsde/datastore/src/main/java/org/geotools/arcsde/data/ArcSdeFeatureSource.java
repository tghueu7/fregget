...
public class ArcSdeFeatureSource implements SimpleFeatureSource {
  ...
  protected ReferencedEnvelope getBounds(final Query namedQuery, final ISession session)
    ...
    final ArcSdeVersionHandler versionHandler = dataStore.getVersionHandler(typeName, transaction);

    Envelope ev = ArcSDEQuery.calculateQueryExtent(session, typeInfo, namedQuery, versionHandler);
  ...
  /**
   * ArcSDE features are always "detached", so we return the FEATURE_DETACHED hint here, as well as
   * the JTS related ones.
   *
