...
public class FilterToSQLSDE extends FilterToSQL implements FilterVisitor {
        ...
        if (alias == null) {
          if (!(colDef.getExpression() instanceof Column)) {
            throw new RuntimeException(
                "if select item is not a plain column an alias should be provided: " + colDef);
          }
  ...
  /**
   * Writes the SQL for the attribute Expression. NOTE: If the feature type is the product of an in
   * process sql query, the attribute name encoded will be the actual one, not the alias (if any)
  ...
  /**
   * @see {@link FilterVisitor#visit(ExcludeFilter, Object)} Writes the SQL for the IncludeFilter by
   *     writing "FALSE".
  ...
  /**
   * @see {@link FilterVisitor#visit(IncludeFilter, Object)} Writes the SQL for the IncludeFilter by
   *     writing "TRUE".
   * @param the filter to be visited
   */
  @Override
