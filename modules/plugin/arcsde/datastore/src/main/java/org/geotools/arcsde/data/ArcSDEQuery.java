...
class ArcSDEQuery {
  ...
  /**
   * The connection to the ArcSDE server obtained when first created the SeQuery in <code>getSeQuery
   * </code>. It is retained until <code>close()</code> is called. Do not use it directly, but
   * through <code>getConnection()</code>.
   *
  ...
  /**
   * Holds the geotools Filter that originated this query from which can parse the sql where clause
   * and the set of spatial filters for the ArcSDE Java API
   */
  ...
  /**
   * creates an SeQuery with the filters provided to the constructor and returns it. Queries created
   * with this method can be used to execute and fetch results. They cannot be used for other
   * operations, such as calculating layer extents, or result count.
   *
   * <p>Difference with {@link #createSeQueryForFetch(Session, String[])} is tha this function tells
   ...
   *
   * @author $author$
   * @source $URL$ http://svn.osgeo.org/geotools/trunk/modules/plugin/arcsde/datastore/src/main/java
   *     /org/geotools/arcsde/data/ArcSDEQuery.java $
  ...
  public static class FilterSet {
    ...
    /**
     * @return the SeSqlConstruct corresponding to the given SeLayer and SQL based filter. Should
     *     never return null.
     * @throws DataSourceException if an error occurs encoding the sql filter to a SQL where clause,
     *     or creating the SeSqlConstruct for the given layer and where clause.
     */
     ...
     * geometry related filters included in the original <code>org.geotools.data.Query</code> passed
     * to the constructor.
     *
     ...
     *     implementation supports natively, or <code>Filter.INCLUDE</code> if the original Query
     *     does not contains non spatial filters that we can deal with at the ArcSDE Java API side.
     */
     ...
     *     supports natively, or <code>Filter.INCLUDE</code> if the original Query does not contains
     *     spatial filters that we can deal with at the ArcSDE Java API side.
     */
    ...
    private FilterToSQLSDE getSqlEncoder() {
      if (_sqlEncoder == null) {
        ...
        _sqlEncoder =
            new FilterToSQLSDE(layerName, fidColumn, featureType, layerSelectStatement, this.conn);
      }
      return _sqlEncoder;
    }
  }
