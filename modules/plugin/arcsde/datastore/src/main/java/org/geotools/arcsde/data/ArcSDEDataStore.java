...
public class ArcSDEDataStore implements DataStore {
   ...
   * @param versionName the name of the ArcSDE version to work upon, or {@code null} for the {@link
   *     SeVersion#SE_QUALIFIED_DEFAULT_VERSION_NAME DEFAULT} version
   * @param allowNonSpatialTables whether ArcSDE registered, non-spatial tables are to be published
   * @throws IOException
   */
   ...
   * @throws RuntimeException if an exception occurs while retrieving the list of registeres feature
   *     classes on the backend, or while obtaining the full qualified name of one of them
   */
  ...
  /**
   * Disposes this ArcSDEDataStore, which means disposing its session pool and hence closing all the
   * SeConnection objects held.
   *
  ...
  private FeatureReader<SimpleFeatureType, SimpleFeature> getFeatureReader(
    ...
    if (typeInfo.isInProcessView()) {
      ...
      sdeQuery =
          ArcSDEQuery.createQuery(session, completeSchema, query, fidStrategy, versionHandler);
    }
    ...
    if (!unsupportedFilter.equals(Filter.INCLUDE)) {
      ...
      reader = new FilteringFeatureReader<SimpleFeatureType, SimpleFeature>(reader, queryFilter);
      ...
      if (Transaction.AUTO_COMMIT == transaction) {
        writer =
            new AutoCommitFeatureWriter(
                fidReader, featureType, reader, session, listenerManager, versionHandler);
      } else {
        ...
        writer =
            new TransactionFeatureWriter(
                fidReader, featureType, reader, state, versionHandler, listenerManager);
      }
      return writer;
    } catch (IOException e) {
      try {
  ...
  /**
   * Returns the same list of names than {@link #getTypeNames()} meaning the returned Names have no
   * namespace set.
   *
   * @since 2.5
   ...
   *       feature type. In not present, <code>"DEFAULTS"</code> will be used.
   *   <li><b>rowid.column.name</b>: indicates the name of the table column to set up as the unique
   *       identifier, and thus to be used as feature id.
    ...
    try {
      ...
      FeatureTypeInfo typeInfo =
          ArcSDEAdapter.createInprocessViewSchema(
              session, typeName, typeInfoCache.getNamesapceURI(), qualifiedSelect, queryInfo);
   ...
   *
   * @param select
   * @throws UnsupportedOperationException if any of the unsupported constructs are found on <code>
   *     select</code>
   */
