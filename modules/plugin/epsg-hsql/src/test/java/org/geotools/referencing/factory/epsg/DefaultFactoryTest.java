...
public class DefaultFactoryTest {
  ...
  /**
   * Tests closing the factory after the timeout. <strong>IMPORTANT:</strong> This test must be run
   * after {@link #testCreation} and before any call to {@code getAuthorityCodes()}.
   *
   * @throws FactoryException if an error occured while querying the factory.
   */
  @Test
  ...
  public void testDescriptionText() throws FactoryException {
    assertEquals(
        "World Geodetic System 1984", factory.getDescriptionText("6326").toString(Locale.ENGLISH));
    assertEquals("Mean Sea Level", factory.getDescriptionText("5100").toString(Locale.ENGLISH));
    assertEquals(
        "NTF (Paris) / Nord France", factory.getDescriptionText("27591").toString(Locale.ENGLISH));
  ...
  public void testAuthorityCodes() throws FactoryException {
    ...
    // starting with EPSG 7.9 the following is no more true, for example EPSG:5214 is at the same
    // time
  ...
  public void testFind() throws FactoryException {
    ...
    assertEquals(
        "4326", AbstractIdentifiedObject.getIdentifier(find, factory.getAuthority()).getCode());
    finder.setFullScanAllowed(false);

    ReferenceIdentifier foundri =
        AbstractIdentifiedObject.getIdentifier(find, factory.getAuthority());

    // this is broken because, as we know from above, it is ambiguous, so it may not be EPSG:4326 in
    // the cache at all!
    ...
    assertTrue(
        ...
        CRS.equalsIgnoreMetadata(crs, find));
    assertEquals(
        "2442", AbstractIdentifiedObject.getIdentifier(find, factory.getAuthority()).getCode());
  ...
  /**
   * We are supposed to be able to get back identical {@link CoordinateReferenceSystem} objects when
   * we create using the same definition. This test case ensures that we do get back identical
   * objects when using an EPSG code and when using WKT.
   *
