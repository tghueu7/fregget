        ...
        if (!dataExists(directory)) {
            ...
            try {
                ...
                if (!dataExists(directory)) {
                    ...
                    ZipInputStream zin = new ZipInputStream(ThreadedHsqlEpsgFactory.class
                            .getResourceAsStream(ZIP_FILE));
                    ZipEntry ze = null;
                    byte[] buf = new byte[1024];
                    int read = 0;
                    while ((ze = zin.getNextEntry()) != null) {
                        FileOutputStream fout = new FileOutputStream(new File(directory, ze
                                .getName()));
                ...
                e.initCause(exception); // TODO: inline cause when we will be allowed to target 
                // Java 6.
                throw e;
            } finally {
                if (lock != null) {
