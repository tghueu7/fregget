...
public class PathTypeTest extends Assert {
    ...
    public void relative() throws FileNotFoundException, IOException {
        ...
        final URL temp = PathType.RELATIVE.resolvePath(TestData.url(this, ".").toExternalForm(), 
                "rgb/global_mosaic_0.pgw");
        assertNotNull(temp);
        temp.openStream().close();

        // test error checks
        LOGGER.info("Testing invalid path");
        final URL temp1 = PathType.RELATIVE.resolvePath(TestData.url(this, ".").toExternalForm(),
                "rgb/global_mosaic_0.pg");
        assertNull(temp1);
        try {
    ...
    public void absolute() throws FileNotFoundException, IOException {
        ...
        final URL temp = PathType.ABSOLUTE.resolvePath(TestData.url(this, ".").toExternalForm(), 
                testFile.toExternalForm());
        assertNotNull(temp);
        temp.openStream().close();
        ...
        final URL temp2 = PathType.ABSOLUTE.resolvePath(TestData.url(this, ".").toExternalForm(),
                "rgb/global_mosaic_0.pg");
        assertNull(temp2);
        try {
