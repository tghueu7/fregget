...
public class ImageMosaicFootprintsTest {
  ...
  public void testWkbSidecars() throws Exception {
    ...
    ShapefileDataStore ds =
        new ShapefileDataStore(URLs.fileToUrl(new File(footprintsSource, "footprints.shp")));
    ds.getFeatureSource()
        .getFeatures()
        .accepts(
            new FeatureVisitor() {
  ...
  /**
   * When the mosaic bounds don't match the requested image bounds, there's only one granule in the
   * requested bounds and FootprintBehavior is transparent a border is added to the image. This
   * actually only happens in very specific circumstances, like in the test data which is an L
   * shaped. In this case the footprint behavior was not being respected, resulting in a background
   * color even though the background should be transparent. Update: now the image mosaic uses only
   * the footprint of the intercepted tiles to build the result, so the bbox has been moved to hit
   * the internal corner of the L in order to still generate a transparent corner (with the original
   * bbox only the part intersecting the requested granule is returned)
   */
  @Test
  public void testFootprintWithBorderNeeded() throws IOException {
    ...
    ImageMosaicReader reader =
        (ImageMosaicReader) new ImageMosaicFormatFactory().createFormat().getReader(testFolder);
