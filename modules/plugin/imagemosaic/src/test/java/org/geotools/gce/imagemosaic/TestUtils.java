...
final class TestUtils extends Assert {
    ...
    static GridCoverage2D testCoverage(final ImageMosaicReader reader,
        ...
        if (values != null)
            for (GeneralParameterValue pv : values) {
                if (pv.getDescriptor().getName().equals(AbstractGridFormat.READ_GRIDGEOMETRY2D
                        .getName())) {

                    Parameter<GridGeometry2D> param = (Parameter<GridGeometry2D>) pv;
                    // check envelope if it has been requested

                    CoordinateReferenceSystem envCRS = param.getValue().getEnvelope()
                    ...
                    if (!equalsIgnoreMetadata) {
                        MathTransform destinationToSourceTransform = CRS.findMathTransform
                                (envCRS, coverageCRS, true);
                        equalsIgnoreMetadata = destinationToSourceTransform != null && 
     ...
     * @param value  that control the actions to take for creating a
     *               {@link GridCoverage2D}.
    ...
    static GridCoverage2D checkCoverage(final ImageMosaicReader reader,
                                        GeneralParameterValue[] values, String title) throws 
            IOException {
        return checkCoverage(reader, values, title, null);
    }

    static GridCoverage2D checkCoverage(final ImageMosaicReader reader,
                                        GeneralParameterValue[] values, String title, Rectangle 
     ...
     *
     * @param testURL points to a valid object to create an
    ...
    static ImageMosaicReader getReader(URL testURL,
                                       final AbstractGridFormat format) throws 
