...
public class ImageMosaicEgrTest {
  ...
  private GeneralParameterValue[] getFootprintReadParams(
    ...
    if (!foundGridGeometry) {
      ...
      geom.setValue(
          new GridGeometry2D(new GridEnvelope2D(0, 0, 300, 300), reader.getOriginalEnvelope()));
      params.add(geom);
    }
  ...
  public void testSingleRaster() throws Exception {
    createRasterFootprintsProperties(testMosaic);
    File sample =
        new File("src/test/resources/org/geotools/gce/imagemosaic/test-data/egr-red-rect.png");
    testSingle(sample);
  }
