...
public class SpatialRequestHelperTest extends Assert {
    ...
    public void testNoRequest() throws Exception {
        ...
        assertEquals(scale * XAffineTransform.getScaleX0(sourceGridToWorldTransform), 
                computedResolution[0], 1E-6);
        assertEquals(scale * XAffineTransform.getScaleY0(sourceGridToWorldTransform), 
                computedResolution[1], 1E-6);
    ...
    public void testBasic2() throws Exception {
        ...
        assertNotNull(computedG2W);
        double[] computedResolution = spatialRequestHelper.getComputedResolution();
        double scale = (1000.0 / 2) / 250; // we scaled down the original image but we also use a
        // portion of it
        assertNotNull(computedResolution);
        assertEquals(scale * XAffineTransform.getScaleX0(sourceGridToWorldTransform), 
                computedResolution[0], 1E-6);
    ...
    public void testBasic3() throws Exception {
        ...
        GridToEnvelopeMapper gridToEnvelopeMapper = new GridToEnvelopeMapper(
                ...
                requestedBBox);
        gridToEnvelopeMapper.setPixelAnchor(PixelInCell.CELL_CORNER);
        assertEquals(computedRasterArea, CRS.transform(gridToEnvelopeMapper.createTransform()
                .inverse(), computedBBox).toRectangle2D().getBounds());
    ...
    public void testBasic2Reproject() throws Exception {
        ...
        assertNotNull(computedG2W);
        double[] computedResolution = spatialRequestHelper.getComputedResolution();
        double scaleX = (1000.0 / 2) / 256; // we scaled down the original image but we also use 
        // a portion of it
        double scaleY = (1000.0 / 2) / 256; // we scaled down the original image but we also use 
        // a portion of it
        assertNotNull(computedResolution);
        assertEquals(scaleX * XAffineTransform.getScaleX0(sourceGridToWorldTransform), 
                computedResolution[0], 1E-6);
        assertEquals(scaleY * XAffineTransform.getScaleY0(sourceGridToWorldTransform), 
                computedResolution[1], 1E-6);
        ...
        assertNotEquals(computedResolution[1], computedResolution2[1], 1E-6);// high deformations
        // on latitude for large areas
