  ...
  protected void handleFile(final File fileBeingProcessed) throws IOException {
    ...
    try {
      ...
      eventHandler.fireFileEvent(
          ...
          fileBeingProcessed,
          false,
          "Exception occurred while processing file " + fileBeingProcessed + ": " + e.getMessage(),
    ...
    try {
      ...
      if (configHandler.getCachedReaderSPI() == null) {
        ...
        try {
          ...
          // Setting of the ImageReaderSpi to the ImageMosaicConfigHandler in order to set it inside
          // the indexer properties
      ...
      for (String cvName : coverageNames) {
        ...
        for (GranuleAcceptor acceptor : this.configHandler.getGranuleAcceptors()) {
          if (!acceptor.accepts(coverageReader, cvName, fileBeingProcessed, configHandler)) {
            shouldAccept = false;
            eventHandler.fireFileEvent(
                Level.FINE,
                fileBeingProcessed,
        ...
        if (shouldAccept) {
          configHandler.updateConfiguration(
              coverageReader, cvName, fileBeingProcessed, fileIndex, numFiles, transaction);
        }
