...
class ReprojectingSubmosaicProducer extends BaseSubmosaicProducer {
  ...
  public boolean accept(GranuleDescriptor granuleDescriptor) {
    ...
    boolean accepted =
        currentSubmosaicProducer != null && currentSubmosaicProducer.accept(granuleDescriptor);
    if (!accepted) {
      // either we have no producer, or the granule was rejected by the current one,
      // presumably because its CRS didn't match, we need to create a new one because we've moved on
      // to the next
      ...
      try {
        ...
        this.currentSubmosaicProducer =
            new CRSBoundMosaicProducer(transformedResponse, dryRun, targetCRS, granuleDescriptor);
  ...
  /**
   * Given a coverage in the mosaic target CRS generates an RenderedImage properly positioned in the
   * mosaic output raster space
   *
  ...
  private RenderedImage positionInOutputMosaic(GridCoverage2D resampledCoverage) {
    ...
    final AffineTransform finalRaster2Model =
        new AffineTransform((AffineTransform2D) resampledCoverage.getGridGeometry().getGridToCRS());
