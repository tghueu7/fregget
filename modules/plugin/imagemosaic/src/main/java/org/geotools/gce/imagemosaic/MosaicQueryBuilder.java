...
class MosaicQueryBuilder {
  ...
  /**
   * This method is responsible for creating the filters needed for addtional dimensions like TIME,
   * ELEVATION additional Domains
   *
  ...
  private void handleAdditionalFilters(Query query) {
    ...
    // handle elevation indexing first since we then combine this with the max in case we are asking
    // for current in time
    if (hasElevation) {
