    ...
    try {
      ...
      for (String typeName : catalog.getTypeNames()) {
        ...
        if (schema.getDescriptor(locationAttrName) == null) {
          ...
          LOGGER.fine(
              "Skipping feature type " + typeName + " as it does not have a footprint column");
          continue;
        }
        ...
        while (it.hasNext()) {
          ...
          if (locationAttrObj instanceof String) {
            ...
            if (Boolean.getBoolean(
                configHandler.getRunConfiguration().getParameter(Prop.ABSOLUTE_PATH))) {
              // absolute files
              file = new File(path);
            ...
            if (file == null) {
              ...
              file =
                  new File(
                      configHandler.getRunConfiguration().getParameter(Prop.ROOT_MOSAIC_DIR), path);
