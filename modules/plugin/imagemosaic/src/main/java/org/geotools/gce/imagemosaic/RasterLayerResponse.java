...
public class RasterLayerResponse {
  ...
  private class MosaicProducer implements GranuleCatalogVisitor {
    ...
    /**
     * {@link MosaicProducer} constructor. It can be used to specify that we want to perform a dry
     * run just to count the granules we would load with the specified query.
     *
     * <p>
     *
     ...
     * <p>
     *
     * <p>If not {@link MergeBehavior#STACK}ing is required, we collect them all together with an
     * include filter.
     */
    public void visit(GranuleDescriptor granuleDescriptor, SimpleFeature sf) {
      ...
      if (!footprintBehavior.handleFootprints()
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
          LOGGER.fine(
              "We rejected for non ROI inclusion the granule " + granuleDescriptor.toString());
        }
      }
    }
     ...
     * <p>
     *
     * <p>Depending on whether or not a {@link MergeBehavior#STACK}ing is required, we perform 1 or
     * 2 steps.
     *
     * <ol>
    ...
    private MosaicOutput produce() throws IOException {
      ...
      for (SubmosaicProducer collector : granuleCollectors) {
        ...
        if (preparedMosaic.size() > 0 && !preparedMosaic.stream().allMatch(p -> p == null)) {
   ...
   * GridCoverageFactory} to produce {@code GridCoverage}s and an {@code ImageReaderSpi} to be used
   * for instantiating an Image Reader for a read operation,
   *
    ...
    try {
      ...
      if (visitor.granulesNumber == 0) {
        ...
        if (dryRunVisitor.granulesNumber > 0) {
          ...
          // It means the previous lack of granule was due to a filter excluding all the results.
          // Then we return null
          return null;
        }
      }
    ...
    if (request.getExcessGranuleRemovalPolicy() == ExcessGranulePolicy.ROI) {
      ...
      excessGranuleRemover =
          new ROIExcessGranuleRemover(
              rasterBounds, tileWidth, tileHeight, rasterManager.getConfiguration().getCrs());
    }
  }
  ...
  /**
   * This method is responsible for creating a blank image as a reponse to the query as it seems we
   * got a no data area.
   *
  ...
  private MosaicOutput createBlankResponse() {
    ...
    if (ImageUtilities.isMediaLibAvailable()) {
      ...
      Assert.isTrue(
          il.isValid(
              ImageLayout.WIDTH_MASK | ImageLayout.HEIGHT_MASK | ImageLayout.SAMPLE_MODEL_MASK));
      ...
      w.mosaic(
          ...
          null,
          new double[][] {
            {CoverageUtilities.getMosaicThreshold(il.getSampleModel(null).getDataType())}
          },
    ...
    if (inputTransparentColor != null
      ...
      if (!hasAlpha) {
        // if the resulting image has no transparency (can happen with IndexColorModel then we need
        // to try component
        // color model
        finalImage =
            new ImageWorker(finalImage)
  ...
  private GridCoverage2D prepareCoverage(MosaicOutput mosaicOutput) throws IOException {
    ...
    for (int i = 0; i < numBands; i++) {
      ...
      if (noDataAsProperty != null) {
        ...
        // sometimes background values are not specified as 1 per each band, therefore we need to be
        // careful
