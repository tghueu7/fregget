...
public class ImageMosaicDirectoryWalker extends ImageMosaicWalker {
  ...
  final class MosaicDirectoryWalker extends DirectoryWalker {
    ...
    public MosaicDirectoryWalker(
        final List<String> indexingDirectories, final FileFilter filter, ImageMosaicWalker walker)
        throws IOException {
  ...
  private IOFileFilter createDefaultGranuleExclusionFilter() {
    ...
    filesFilter =
        FileFilterUtils.or(filesFilter, FileFilterUtils.nameFileFilter("indexer.properties"));
   ...
   * Default constructor.
   *
   * <p>Sets a filter that can reduce the file the mosaic walker will take into consideration (in a
   * more flexible way than the wildcards)
   *
   * @param filter
