...
public class IndexerUtils {
  ...
  /**
   * Return the parameter string value of the specified parameter name from the provided parameters
   * element
   *
   * @param params
   * @param parameterName
  ...
  public static void parseAdditionalDomains(String attributes, List<DomainType> domainList) {
    ...
    for (String domainAttributes : domainsAttributes) {
      ...
      if (domainAttributes.contains("(") && domainAttributes.contains(")")) {
        ...
        domainAttribs =
            domainAttribs.substring(domainAttribs.indexOf("(")).replace("(", "").replace(")", "");
      }
  ...
  /**
   * Get a {@link SchemaType} element for the specified {@link Coverage}. The {@link Indexer} object
   * will be used in case of an externally referenced schema.
   *
   * @param indexer the main {@link Indexer} instance
  ...
  public static Indexer createDefaultIndexer() {
    ...
    setParam(
        parameterList, Utils.Prop.ABSOLUTE_PATH, Boolean.toString(Utils.DEFAULT_PATH_BEHAVIOR));
    setParam(
        parameterList, Utils.Prop.RECURSIVE, Boolean.toString(Utils.DEFAULT_RECURSION_BEHAVIOR));
