...
public class SpatialRequestHelper {
  ...
  private void computeResolution() {
    ...
    try {
      ...
      computedResolution =
          calculator.computeRequestedResolution(ReferencedEnvelope.reference(computedBBox));
  ...
  /**
   * Compute the resolutions through a more accurate logic: Compute the resolution in 9 points, the
   * corners of the requested area and the middle points and take the best one. This will provide
   * better results for cases where there is a lot more deformation on a subregion
        ...
        try {
          this.computedBBox =
              Utils.reprojectEnvelope(
                  requestedBBox, coverageProperties.crs2D, coverageProperties.bbox);
        } catch (FactoryException e) {
          throw new DataSourceException(e);
        }
      } else {
