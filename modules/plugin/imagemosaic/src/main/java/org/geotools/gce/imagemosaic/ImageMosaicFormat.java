...
public final class ImageMosaicFormat extends AbstractGridFormat implements Format {
  ...
  public static final ParameterDescriptor<double[]> BACKGROUND_VALUES =
      new DefaultParameterDescriptor<double[]>("BackgroundValues", double[].class, null, null);
   ...
   * <p>
   *
   * <p>This parameter controls whether the mosaic will attempt to remove excess granules, that is,
   * granules not contributing pixels to the output, before performing the mosaicking. This is
    ...
    try {
      ...
      try {
        ...
        if (FilenameUtils.getName(sourceF.getAbsolutePath())
          ...
          for (Param p : paramsInfo) {
            ...
            if (properties.containsKey(p.key))
              params.put(
                  p.key, (Serializable) Converters.convert(properties.getProperty(p.key), p.type));
        ...
        if (schema.getDescriptor(locationAttributeName) == null
            && schema.getDescriptor(locationAttributeName.toUpperCase()) == null) return false;
