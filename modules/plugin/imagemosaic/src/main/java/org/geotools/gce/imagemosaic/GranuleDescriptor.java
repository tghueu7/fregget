...
public class GranuleDescriptor {
   ...
   *
   * <p>Notice that the internal transformations for the various levels are reffered to the corner,
      ...
      try {
        ...
        if (hints.get(Hints.REPOSITORY) != null
            && MethodUtils.getAccessibleMethod(
                    reader.getClass(), "setRepository", new Class[] {Repository.class})
                != null) {
   ...
   *     this granule.
   * @param pathType A {@link PathType} identifying if the granule location should be resolved as a
  ...
  public GranuleDescriptor(
    ...
    if (rasterFile == null) {
      throw new IllegalArgumentException(
          Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2, "granuleLocation", granuleLocation));
    }
        ...
        inStream =
            ovrProvider
                ...
                .createInputStreamInstance(
                    granuleURLUpdated, ImageIO.getUseCache(), ImageIO.getCacheDirectory());
        // get a reader and try to cache the relevant SPI
