...
public class ImageMosaicConfigHandler {
  ...
  /**
   * Proper way to stop a thread is not by calling Thread.stop() but by using a shared variable that
   * can be checked in order to notify a terminating condition.
   */
  ...
  private GranuleCatalog createCatalog(CatalogBuilderConfiguration runConfiguration, boolean create)
    ...
    if (Utils.checkFileReadable(datastoreProperties)) {
      ...
      final URL file =
          new File(parent, runConfiguration.getParameter(Prop.INDEX_NAME) + ".shp").toURI().toURL();
  ...
  /**
   * Get a {@link GranuleSource} related to a specific coverageName from an inputReader and put the
   * related granules into a {@link GranuleStore} related to the same coverageName of the
   * mosaicReader.
   *
          ...
          public void visit(Feature feature) {
            if (feature instanceof SimpleFeature) {
              ...
              for (Property prop : props) {
                Name geometryName =
                    sourceFeature.getFeatureType().getGeometryDescriptor().getName();
              ...
              updateAttributesFromCollectors(
                  destFeature, fileBeingProcessed, inputReader, propertiesCollectors);
  ...
  /**
   * Checks if the file system is case sensitive or not using File.exists (the only method that also
   * works on OSX too according to
  ...
  private static boolean isCaseSensitiveFileSystem(File fileBeingProcessed) {
    File loCase =
        new File(fileBeingProcessed.getParentFile(), fileBeingProcessed.getName().toLowerCase());
    File upCase =
        new File(fileBeingProcessed.getParentFile(), fileBeingProcessed.getName().toUpperCase());
  ...
  private static String prepareLocation(
    ...
    String relative =
        getRelativePath(
            targetPath,
            basePath,
            File.separator); // TODO: Remove this replace after fixing the quote escaping
    return relative;
  }
   ...
   *
   * <p>Make sure to call this method when you are not running the {@link ImageMosaicConfigHandler}
  ...
  private void createSampleImage(
    ...
    if (mosaicConfiguration.getSampleModel() != null
      ...
      filePath =
          baseName + (useName ? mosaicConfiguration.getName() : "") + Utils.SAMPLE_IMAGE_NAME;
      try {
  ...
  private void createPropertiesFiles(MosaicConfigurationBean mosaicConfiguration) {
    ...
    properties.setProperty(
        Utils.Prop.ABSOLUTE_PATH, Boolean.toString(catalogConfigurationBean.isAbsolutePath()));
    properties.setProperty(
    ...
    for (int k = 0; k < numberOfLevels; k++) {
      levels
          .append(Double.toString(resolutionLevels[k][0]))
          .append(",")
    ...
    properties.setProperty(
        Utils.Prop.HETEROGENEOUS, Boolean.toString(catalogConfigurationBean.isHeterogeneous()));
    properties.setProperty(
    ...
    if (mosaicConfiguration == null) {
      ...
      if (!useExistingSchema) {
        ...
        SimpleFeatureType indexSchema =
            createSchema(
                getRunConfiguration(), currentConfigurationBean.getName(), configBuilder.getCrs());
      ...
      resolutionLevels =
          getResolutionLevels(coverageReader, inputCoverageName, mosaicConfiguration.getCrs());
      numberOfLevels = resolutionLevels.length;
