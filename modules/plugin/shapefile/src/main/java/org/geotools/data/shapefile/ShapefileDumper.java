...
public class ShapefileDumper {
  ...
  private void writeToShapefile(
      SimpleFeature f, FeatureWriter<SimpleFeatureType, SimpleFeature> writer) throws IOException {
  ...
  private StoreWriter getStoreWriter(
      SimpleFeature f, Map<Class, StoreWriter> writers, boolean multiWriter) throws IOException {
  ...
  private StoreWriter getStoreWriter(
    ...
    if (storeWriter == null) {
      ...
      // we need to associate the geometry type to the file name only if we can have be multiple
      // types
      String fileName;
      if (multiWriter) {
