...
public class DbaseFileWriter {
    ...
    private boolean reportFieldSizeErrors = Boolean.getBoolean("org.geotools.shapefile" +
            ".reportFieldSizeErrors");
    ...
    public static class FieldFormatter {
        ...
        public String getFieldString(int size, int decimalPlaces, Number n) {
            ...
            if (n != null) {
                 ...
                 */
                /* However, the Double.toString method returns an ascii string, which is more 
                ESRI-friendly */
                if (Double.isNaN(dval) || Double.isInfinite(dval)) {
                    ...
                    if (fp.getBeginIndex() >= size) {
                        ...
                        if (buffer.length() > size) {
                            // we have a grevious problem -- the value does not fit in the 
                            // required size.
                            logger.logp(Level.WARNING, this.getClass().getName(), "getFieldString",
                                    "Writing DBF data, value {0} cannot be represented in size " +
                                            "{1,number}", new Object[]{n, size});
                            if (!swallowFieldSizeErrors) {
                                // rather than truncate, and corrupt the data, we throw a Runtime
                                throw new IllegalArgumentException("Value " + n + " cannot be " +
                                        "represented in size " + size);
                            }
                        }
                    }
                }
