...
public class ShapefileReader implements FileReader {
  ...
  /**
   * If there exists another record. Currently checks the stream for the presence of 8 more bytes,
   * the length of a record. If this is true and the record indicates the next logical record number
   * (if checkRecord == true), there exists more records.
   *
  ...
  /**
   * Sets the current location of the byteStream to offset and returns the next record. Usually used
   * in conjuctions with the shx file or some other index file. Mind that:
   *
   * <ul>
   *   <li>it's your responsibility to ensure the offset corresponds to the actual beginning of a
   *       shape struct
