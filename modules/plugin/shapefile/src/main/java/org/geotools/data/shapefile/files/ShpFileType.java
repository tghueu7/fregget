...
public enum ShpFileType {
  ...
  PRJ("prj"),
  /**
   * the .qix file, A quad tree spatial index of the shapefile. It is the same format the mapservers
   * shptree tool generates
   */
  QIX("qix"),
  /**
   * the .fix file, it contains all the Feature IDs for constant time lookup by fid also so that the
   * fids stay consistent across deletes and adds
   */
  FIX("fix"),
  ...
  /**
   * Returns the base of the file or null if the file passed in is not of the correct type (has the
   * correct extension.)
   *
