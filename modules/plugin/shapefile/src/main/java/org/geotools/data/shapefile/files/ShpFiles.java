...
public class ShpFiles {
   ...
   * @throws MalformedURLException if it isn't possible to create a URL from string. It will be used
   *     to create a file and create a URL from that if both fail this exception is thrown
   */
  ...
  /**
   * Acquire a File for read only purposes. It is recommended that get*Stream or get*Channel methods
   ...
   * @param requestor the object that is requesting the File. The same object must release the lock
   ...
   * file does not.
   *
   * <p>It is recommended that get*Stream or get*Channel methods are used when reading or writing to
   * the file is desired.
   *
   * @see #getInputStream(ShpFileType, FileReader)
   * @see #getReadChannel(ShpFileType, FileReader)
   ...
   * will be returned. Currently, this method will return a generic channel for remote urls, however
   * both shape and dbf writing can only occur with a local FileChannel channel.
   *
