...
public class IndexedFidWriter implements FileWriter {
  ...
  /**
   * Creates a new instance and writes the fids to a storage file which is replaces the original on
   * close().
   *
   ...
   * removed/deleted the following usage of the write should take place: next(); (move to feature 1)
   * next(); (move to feature 2) next(); (move to feature 3) remove();(delete feature 3) next();
   * (move to feature 4) // optional write(); (write feature 4) next(); (move to feature 5) write();
   * (write(feature 5)
   *
   * @throws IOException
   */
