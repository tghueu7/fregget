...
public class DbaseFileReader implements FileReader {
  ...
  public DbaseFileReader(
      final ShpFiles shapefileFiles, final boolean useMemoryMappedBuffer, final Charset charset)
      throws IOException {
    final ReadableByteChannel dbfChannel = shapefileFiles.getReadChannel(ShpFileType.DBF, this);
  ...
  private void bufferCheck() throws IOException {
    ...
    if (useMemoryMappedBuffer) {
      if (buffer.remaining() < header.getRecordLength()) {
        ...
        buffer =
            ((FileChannel) channel).map(MapMode.READ_ONLY, buffer.position(), Integer.MAX_VALUE);
      }
  ...
  /**
   * Reads the next record into memory. You need to use this directly when reading only a subset of
   * the fields using {@link #readField(int)}.
   *
   * @throws IOException
   */
        ...
        case '@':
          try {
            ...
            calendar.setTimeInMillis(
                days * MILLISECS_PER_DAY + DbaseFileHeader.MILLIS_SINCE_4713 + time);
  ...
  public static void main(final String[] args) throws Exception {
    final DbaseFileReader reader =
        new DbaseFileReader(new ShpFiles(args[0]), false, Charset.forName("ISO-8859-1"), null);
