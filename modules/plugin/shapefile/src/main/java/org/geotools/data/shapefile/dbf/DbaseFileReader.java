...
public class DbaseFileReader implements FileReader {
    ...
    public DbaseFileReader(final ShpFiles shapefileFiles,
                           final boolean useMemoryMappedBuffer, final Charset charset, final 
    ...
    public DbaseFileReader(final ShpFiles shapefileFiles,
                           final boolean useMemoryMappedBuffer, final Charset charset) throws 
            IOException {
        final ReadableByteChannel dbfChannel = shapefileFiles.getReadChannel(ShpFileType.DBF, this);
        init(dbfChannel, useMemoryMappedBuffer, charset, null);
    }

    public DbaseFileReader(final ReadableByteChannel readChannel, final boolean 
    ...
    private void init(final ReadableByteChannel dbfChannel, final boolean useMemoryMappedBuffer,
        ...
        if (channel instanceof FileChannel && this.useMemoryMappedBuffer) {
            ...
            // cacheable)
            //int size = (int) Math.pow(2, Math.ceil(Math.log(header.getRecordLength()) / Math
            // .log(2)));
    ...
    private void bufferCheck() throws IOException {
        ...
        if (useMemoryMappedBuffer) {
            if (buffer.remaining() < header.getRecordLength()) {
                ...
                buffer = ((FileChannel) channel).map(MapMode.READ_ONLY, buffer.position(), 
                        Integer.MAX_VALUE);
            }
    ...
    /**
     * Reads a single field from the current record and returns it. Remember to call 
     * {@link #read()} before
     * starting to read fields from the dbf, and call it every time you need to move to the next 
     * record.
     *
                ...
                case 'L':
                    ...
                    switch (c) {
                        ...
                        case 'n':
                            ...
                            break;
                        default:
                            // 0x20 should be interpreted as null, but we're going to be a bit 
                            // more lax
                            object = null;
                    }
                    break;
                ...
                case 'C':
                    ...
                    if (bytes[fieldOffset] != '\0') {
                        ...
                        if (oneBytePerChar) {
                            ...
                            object = new String(bytes, fieldOffset, fieldLen, stringCharset.name
                                    ()).trim();
                        }
                    }
                    break;
                ...
                case '@':
                    try {
                        ...
                        byte[] timestampBytes = {
                                ...
                                bytes[fieldOffset + 7], bytes[fieldOffset + 6], bytes[fieldOffset
                                + 5], bytes[fieldOffset + 4],
                                // Days, after reverse.
                                bytes[fieldOffset + 3], bytes[fieldOffset + 2], bytes[fieldOffset
                                + 1], bytes[fieldOffset]
                        };

                        ByteArrayInputStream i_bytes = new ByteArrayInputStream(timestampBytes);
                        DataInputStream i_stream = new DataInputStream(new BufferedInputStream
                                (i_bytes));

                        int time = i_stream.readInt();
                        int days = i_stream.readInt();

                        calendar.setTimeInMillis(days * MILLISECS_PER_DAY + DbaseFileHeader
                                .MILLIS_SINCE_4713 + time);
