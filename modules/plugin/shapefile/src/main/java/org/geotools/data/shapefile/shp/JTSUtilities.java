...
public class JTSUtilities {
   ...
   *
   * @param cs The array to search.
   * @param target array with at least two elements where to hold the min and max zvalues. target[0]
   *     will be filled with the minimum zvalue, target[1] with the maximum. The array current
   *     values, if not NaN, will be taken into acount in the computation.
   */
  ...
  public static final ShapeType getShapeType(Geometry geom, int shapeFileDimentions)
    ...
    if (type == null) {
      throw new ShapefileException(
          "Cannot handle geometry type : " + (geom == null ? "null" : geom.getClass().getName()));
    }
    return type;
  }
