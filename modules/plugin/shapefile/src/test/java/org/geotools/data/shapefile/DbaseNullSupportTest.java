...
public class DbaseNullSupportTest {
  ...
  /**
   * Creates some non-null values to mix in with the nulls so we have variety. Be sure to use powers
   * of two for the numbers to prevent any possible loss of precision when saving/reloading. Be sure
   * to use a Date with no time component since only the month/day/year are saved.
   */
  ...
  public void testNulls() throws IOException {
    ...
    assertTrue(
        "Number of records does not match", values.length == reader.getHeader().getNumRecords());
    for (int row = 0; row < values.length; row++) {
