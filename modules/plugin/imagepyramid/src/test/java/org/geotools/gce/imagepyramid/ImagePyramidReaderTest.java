...
public class ImagePyramidReaderTest extends ImageLevelsMapperTest {
  ...
  /**
   * Tests automatic building of all the mosaic and pyramid files from a gdal_retile like directory
   * structure
   *
   * @throws IOException
   */
  ...
  public void testCropHighestLevel()
    ...
    final GeneralEnvelope cropEnvelope =
        new GeneralEnvelope(
            new double[] {
              oldEnvelop.getLowerCorner().getOrdinate(0), oldEnvelop.getLowerCorner().getOrdinate(1)
            },
            new double[] {
    ...
    gg.setValue(
        new GridGeometry2D(new GridEnvelope2D(new Rectangle(0, 0, 125, 125)), cropEnvelope));
   ...
   * org.geotools.gce.imagepyramid.ImagePyramidReader#getMetadataValue(String, String)}. This method
   * is called by GeoServer.
   */
  @Test
  public void timePyramidForGeoserver()
    ...
    assertNotNull(timeMetadata);
    assertEquals(
        timeMetadata.split(",")[0], reader.getMetadataValue(coverageName, "TIME_DOMAIN_MINIMUM"));
    assertEquals(
        timeMetadata.split(",")[timeMetadata.split(",").length - 1],
