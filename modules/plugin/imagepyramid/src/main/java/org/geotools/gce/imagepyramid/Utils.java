...
class Utils {
  ...
  static URL checkSource(Object source, Hints hints) {
    ...
    try {
      ...
      LOGGER.log(
          Level.INFO, "We could not generate the pyramid property file " + sourceFile.getPath(), e);
      return null;
    } finally {
      if (os != null) {
    ...
    if (envelope.getCoordinateReferenceSystem() != null) {
      ...
      try {
        ...
        LOGGER.log(
            Level.INFO, "We could not write out the projection file " + prjFile.getPath(), e);
        return null;
      } finally {
        pw.close();
      }
