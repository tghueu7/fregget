...
public class TestSetup {
  ...
  public static void initialize() {
    ...
    try {
      ...
      while (reader.hasNext()) {
        ...
        POINTMAP
            .get(0.0)
            .put(stream.getID(), ((Geometry) stream.getDefaultGeometry()).getNumPoints());
  ...
  private static void addGeneralizedFeatureVertical(
    ...
    Geometry geomNew =
        TopologyPreservingSimplifier.simplify((Geometry) feature.getDefaultGeometry(), distance);
  ...
  private static void addGeneralizedFeatureMixed(
    ...
    Geometry geomNew =
        TopologyPreservingSimplifier.simplify((Geometry) feature.getDefaultGeometry(), distance1);
    feature_gen2.setAttribute("the_geom", geomNew);
    geomNew =
        TopologyPreservingSimplifier.simplify((Geometry) feature.getDefaultGeometry(), distance2);
  ...
  private static void createShapeFilePyramd() throws IOException {
    ...
    params.put(
        ShapefileDataStoreFactory.URLP.key, new File("target/0/streams.shp").toURI().toURL());
