...
public class GeneralizationInfo {
   ...
   * @return The proper Generalization for the requested distance, null if no proper distance found
   *     example: Given are generalizations for 10.0 and 20 0<= requestedDistance < 10 ---> return
   *     null 10<= requestedDistance < 20 ---> return distance info for 10.0 20<= requestedDistance
   *     ---> return distance info for 20.0
   */
   ...
   *
   * @throws IOException 1)if data source, feature name, base feature name or geometry property name
   *     is null 2) if the validation of generalizations fails
   */
