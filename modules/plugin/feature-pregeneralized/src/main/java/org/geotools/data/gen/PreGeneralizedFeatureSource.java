...
public class PreGeneralizedFeatureSource implements SimpleFeatureSource {
  ...
  private SimpleFeatureSource getBaseFeatureSource() throws IOException {
    ...
    DataStore ds =
        repository.dataStore(new NameImpl(info.getDataSourceNameSpace(), info.getDataSourceName()));
    ...
    int[] mapping =
        calculateIndexMapping(
            baseFeatureSource.getSchema(), info.getGeomPropertyName(), info.getGeomPropertyName());
  ...
  private SimpleFeatureSource getFeatureSourceFor(Generalization di) throws IOException {
    ...
    DataStore ds =
        repository.dataStore(new NameImpl(di.getDataSourceNameSpace(), di.getDataSourceName()));
    ...
    // calculate indexMapping
    int[] mapping =
        calculateIndexMapping(fs.getSchema(), info.getGeomPropertyName(), di.getGeomPropertyName());
