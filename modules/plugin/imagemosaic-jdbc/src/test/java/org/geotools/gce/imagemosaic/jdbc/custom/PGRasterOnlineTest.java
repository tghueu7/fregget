...
public class PGRasterOnlineTest extends AbstractTest {
    ...
    public void testCreate() {
        executeCreate(Connection, new String[]{"pgrasterfs.sql", "1/pgrasterfs.sql", 
                "2/pgrasterfs.sql"}, true);
    }

    public void testCreate2() {
        executeCreate(Connection, new String[]{"pgraster.sql", "1/pgraster.sql", "2/pgraster" +
                ".sql",}, false);
    }
    ...
    void executeCreate(java.sql.Connection con, String[] scriptNames, boolean outdb) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("insert into MOSAIC(NAME,TileTable) " +
                    "values (?,?)");
            ps.setString(1, "oek");
            ...
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable1\" (\"rid\" serial PRIMARY KEY,\"rast\" " +
                            "raster,\"filename\" text)").execute();
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable2\" (\"rid\" serial PRIMARY KEY,\"rast\" " +
                            "raster,\"filename\" text)").execute();
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable3\" (\"rid\" serial PRIMARY KEY,\"rast\" " +
                            "raster,\"filename\" text)").execute();
            

/*            
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable1\" (rid serial PRIMARY KEY, \"filename\" 
                    text)").execute();
 
            
            con.prepareStatement(
                    "SELECT AddRasterColumn('public','rtable1','rast',4326, ARRAY['8BUI','8BUI',
                    '8BUI','8BUI'], "+outdb+
                    ", false, null, 0.007547169811321, -0.005102040816327, null, null, null)")
                    .execute();
            
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable2\" (rid serial PRIMARY KEY, \"filename\" 
                    text)").execute();
            
            con.prepareStatement(
                    "SELECT AddRasterColumn('public','rtable2','rast',4326, ARRAY['8BUI','8BUI',
                    '8BUI','8BUI'], "+outdb+
                    ", false, null, 0.015094339622642, -0.010204081632653, null, null, null)")
                    .execute();
            
            con.prepareStatement(
                    "CREATE TABLE \"public\".\"rtable3\" (rid serial PRIMARY KEY, \"filename\" 
                    text)").execute();
            
            con.prepareStatement(
                    "SELECT AddRasterColumn('public','rtable3','rast',4326, ARRAY['8BUI','8BUI',
                    '8BUI','8BUI'], "+outdb+
                    ", false, null, 0.030188679245283, -0.020408163265306, null, null, null)")
                    .execute();
*/

//            con.prepareStatement("alter table raster1 drop constraint enforce_srid_rast")
// .execute();
//            con.prepareStatement("alter table raster2 drop constraint enforce_srid_rast")
// .execute();
//            con.prepareStatement("alter table raster3 drop constraint enforce_srid_rast")
// .execute(); 
            ...
            con.prepareStatement(
                    "CREATE INDEX \"rtable1_rast_gist_idx\" ON \"public\".\"rtable1\" USING GIST " +
                            "(st_convexhull(rast))").execute();
            con.prepareStatement(
                    "ANALYZE \"public\".\"rtable1\"").execute();
            con.prepareStatement(
                    "SELECT AddRasterConstraints('public','rtable1','rast',TRUE,TRUE,TRUE,TRUE," +
                            "TRUE,TRUE,FALSE,TRUE,TRUE,TRUE,TRUE,TRUE)").execute();

            con.prepareStatement(
                    "CREATE INDEX \"rtable2_rast_gist_idx\" ON \"public\".\"rtable2\" USING GIST " +
                            "(st_convexhull(rast))").execute();
            con.prepareStatement(
                    "ANALYZE \"public\".\"rtable2\"").execute();
            con.prepareStatement(
                    "SELECT AddRasterConstraints('public','rtable2','rast',TRUE,TRUE,TRUE,TRUE," +
                            "TRUE,TRUE,FALSE,TRUE,TRUE,TRUE,TRUE,TRUE)").execute();


            con.prepareStatement(
                    "CREATE INDEX \"rtable3_rast_gist_idx\" ON \"public\".\"rtable3\" USING GIST " +
                            "(st_convexhull(rast))").execute();
            con.prepareStatement(
                    "ANALYZE \"public\".\"rtable3\"").execute();
            con.prepareStatement(
                    "SELECT AddRasterConstraints('public','rtable3','rast',TRUE,TRUE,TRUE,TRUE," +
                            "TRUE,TRUE,FALSE,TRUE,TRUE,TRUE,TRUE,TRUE)").execute();
