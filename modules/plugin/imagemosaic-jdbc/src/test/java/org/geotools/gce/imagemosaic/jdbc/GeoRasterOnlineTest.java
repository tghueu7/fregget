    ...
    public void testCreate() {
        ...
        try {
            ...
            con.prepareStatement("INSERT INTO RASTER VALUES ('oek', sdo_geor.init('RASTER_RDT'))" +
                    "").execute();
            con.prepareStatement("CREATE TABLE blob_table (blob_col BLOB, blobid NUMBER unique, " +
                    "clob_col CLOB)").execute();
            ...
            String importString =
                    "DECLARE" +
                            ...
                            " lobd2 CLOB;" +
                            " BEGIN" +
                            " SELECT clob_col into lobd2 from blob_table WHERE blobid = 1 for " +
                            "update;" +
                            " SELECT blob_col into lobd1 from blob_table WHERE blobid = 1 for " +
                            "update;" +
