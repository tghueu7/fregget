...
class JDBCAccessOracle extends JDBCAccessBase {
  ...
  private void initStatementStrings(Config config) {
    ...
    allSelect =
        spatialSelectClause + ",s." + config.getBlobAttributeNameInTileTable() + " from {0} s";
    allSelectJoined =
        spatialSelectClause
            + ",t."
  ...
  protected String getRandomTileStatement(ImageLevelInfo li) {
    if (li.isImplementedAsTableSplit()) {
      return MessageFormat.format(
          allSelectJoined, new Object[] {li.getSpatialTableName(), li.getTileTableName()});
    } else {
      ...
      if (schema != null) {
        ...
        msg =
            MessageFormat.format(
                ...
                new Object[] {
                  li.getSpatialTableName(), config.getGeomAttributeNameInSpatialTable()
                });
      }
  ...
  protected CoordinateReferenceSystem getCRS(ImageLevelInfo li, Connection con) throws IOException {
    ...
    try {
      ...
      LOGGER.warning(
          "Cannot parse WKT defintion from db, srsid: " + li.getSrsId() + " : " + e.getMessage());
    }
