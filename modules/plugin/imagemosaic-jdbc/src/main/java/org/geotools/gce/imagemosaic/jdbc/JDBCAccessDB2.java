...
class JDBCAccessDB2 extends JDBCAccessBase {
    static String SRSSelect = "select srs_id,srs_name from db2gse.st_geometry_columns where " +
            "table_schema=? and table_name=? and column_name=? ";

    static String SRSSelectCurrentSchema = "select srs_id,srs_name from db2gse" +
            ".st_geometry_columns where table_schema=(select current schema from sysibm" +
            ".sysdummy1) and table_name=? and column_name=? ";

    static String CRSSelect = "select definition from db2gse.st_spatial_reference_systems where " +
            "srs_id=?";
    ...
    private void initStatementStrings(Config config) {
        ...
        String whereClause = "db2gse.st_mbrIntersects(s."
                + geomAttr
                + ","
                + "db2gse.ST_GEOMETRY(CAST(? AS BLOB(128)),CAST(? AS INT))) = 1 SELECTIVITY " +
                "0.0000001";
    ...
    protected void setGridSelectParams(PreparedStatement s,
                                       GeneralEnvelope envelope, ImageLevelInfo li) throws 
            SQLException {
