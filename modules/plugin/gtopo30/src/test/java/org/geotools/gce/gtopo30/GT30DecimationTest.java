...
public class GT30DecimationTest extends GT30TestBase {
  ...
  /**
   * Testing reader and writer for gtopo. This test first of all read an existing gtopo tessel into
   * a coverage object, therefore it writes it back onto the disk. Once the coverage is written
   * back\ it loads it again building a new coverage which is finally visualized.
   *
   * @throws Exception
   */
  public void test() throws Exception {
    ...
    if (format.accepts(statURL)) {
      ...
      params
          ...
          .setValue(
              new GridGeometry2D(
                  new GridEnvelope2D(new Rectangle(0, 0, 640, 480)), reader.getOriginalEnvelope()));
      gc =
          ((GridCoverage2D)
              reader.read(
