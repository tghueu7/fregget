...
public final class GTopo30Writer extends AbstractGridCoverageWriter implements GridCoverageWriter {
   ...
   *
   * @param dest The destination object can be a {@link File} (a directory actually), an {@link URL}
   *     to a directory, or a String representing a directory or an {@link URL} to a directory.
   ...
   * Creates a {@link GTopo30Writer}.
   *
   * @param dest The destination object can be a File (a directory actually), an URL to a directory,
   *     or a String representing a directory or an URL to a directory.
   * @throws DataSourceException
   */
  ...
  public void write(final GridCoverage coverage, final GeneralParameterValue[] params)
            ...
            new ZipOutputStream(
                new BufferedOutputStream(
                    new FileOutputStream(
                        new File(
                            (File) destination,
                            new StringBuffer(fileName).append(".zip").toString()))));
      } else {
    ...
    // if the caller has not asked us to compress explicitly it's usually a file ( a directory) or
    // it is already
    ...
    if (dest instanceof File) {
      ...
      final PrintWriter out =
          new PrintWriter(
              new BufferedOutputStream(
                  new FileOutputStream(
                      new File((File) dest, new StringBuffer(name).append(".HDR").toString()))));
    ...
    if (!(cm instanceof IndexColorModel)) {
      ...
      try {
        LookupTableJAI table =
            LookupTableFactory.create(sourceType, targetType, new MathTransform1D[] {transform});
  ...
  private void writePRJ(final GridCoverage2D gc, String name, Object dest) throws IOException {
    if (dest instanceof File) {
      ...
      final BufferedWriter fileWriter =
          new BufferedWriter(
              new FileWriter(
                  new File((File) dest, new StringBuffer(name).append(".PRJ").toString())));
      ...
      final ZipEntry e =
          new ZipEntry(new StringBuffer(gc.getName().toString()).append(".PRJ").toString());
      out.putNextEntry(e);
  ...
  private void writeStats(
    ...
    // /////////////////////////////////////////////////////////////////////
    final double[] Max =
        new double[] {Short.MAX_VALUE}; // we should encode more than the maximum short anyway
  ...
  private void writeWorldFile(final GridCoverage2D gc, String name, Object dest)
    ...
    /**
     * It is worth to point out that here I build the values using the axes order specified in the
     * CRs which can be either LAT,LON or LON,LAT. This is important since we ned to know how to
     * assocaite the underlying raster dimensions with the envelope which is in CRS values.
     */
  ...
  /**
   * This method has the objective of untiling the final image to write on disk since we do not want
   * to have tiles added to the file on disk causing failures when reading it back into memory.
   *
