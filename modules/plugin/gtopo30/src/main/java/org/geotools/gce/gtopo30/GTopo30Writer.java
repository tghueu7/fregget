...
final public class GTopo30Writer extends AbstractGridCoverageWriter implements
     ...
     *
     * @param dest The destination object can be a {@link File} (a directory
    ...
    public void write(final GridCoverage coverage,
        ...
        if (compressed) {
            ...
            if (destination instanceof File) {
                ...
                if (!(destination instanceof ZipOutputStream)) {
                    throw new IllegalArgumentException("Asking compression on a source that does " +
                            "not support it: "
        ...
        // if the caller has not asked us to compress explicitly it's usually a file ( a 
        // directory) or it is already
        ...
        if (!compressed &&
            ...
            throw new IllegalArgumentException("Asking to write, without compression, on a source" +
            ...
            if (compressed && !(destination instanceof ZipOutputStream)) {
                throw new IllegalArgumentException("Asking to write, with compression, on a " +
        ...
        //
        // /////////////////////////////////////////////////////////////////////
        final ParameterValueGroup pvg = PROCESSOR.getOperation("SelectSampleDimension")
                .getParameters();
        ...
        gc2D = (GridCoverage2D) ((SelectSampleDimension) PROCESSOR.getOperation
                ("SelectSampleDimension")).doOperation(pvg, hints);
     ...
     * format.
     *
     * @param gc2D source {@link GridCoverage2D} from which to take the input
     *             {@link PlanarImage}.
	...
	*/
    ...
    private void writeSRC(GridCoverage2D gc, final String name, Object dest)
        ...
        if (dest instanceof File) {
            final File file = new File((File) dest, new StringBuffer(name).append(".SRC")
                    .toString());
    ...
    private void writeGIF(final GridCoverage2D gc, final String name,
        ...
        if (!(cm instanceof IndexColorModel)) {
            ...
            sm = cm.createCompatibleSampleModel(layout.getTileWidth(image), layout.getTileHeight
                    (image));
            ...
            try {
                LookupTableJAI table = LookupTableFactory.create(sourceType, targetType, new 
    ...
    private GridCoverage2D rescaleCoverage(GridCoverage2D gc) {
        ...
        // new grid range
        final GeneralGridEnvelope newGridrange = new GeneralGridEnvelope(new int[]{0, 0}, new 
        ...
        return (GridCoverage2D) ((Resample) PROCESSOR.getOperation("Resample")).doOperation(pvg, 
                hints);
    ...
    private void writeStats(final PlanarImage image, String name, Object dest,
        ...
        //
        // /////////////////////////////////////////////////////////////////////
        final double[] Max = new double[]{Short.MAX_VALUE};//we should encode more than the 
        // maximum short anyway
     ...
     *
     * @param gc        The GridCoverage to write
     * @param worldFile The destination world file (can be a file or a
     *                  ZipOutputStream)
     * @throws IOException if the file could not be written
     */
