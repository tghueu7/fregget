...
public final class NoDataReplacerOpImage extends PointOpImage {
  ...
  private static ImageLayout getRightLayout(RenderedImage image) {
    ...
    if (sm instanceof ComponentSampleModel) {
      final ColorModel newCm =
          new ComponentColorModel(cm.getColorSpace(), false, false, cm.getTransparency(), dataType);
   ...
   *     <ul>
   *       <li>If source and destination are the same raster, then a single {@link WritableRectIter}
   *           object would be more efficient (the hard work is to detect if source and destination
   *           are the same).
   ...
   * transformed value are write back in the {@code iterator}. If a different destination raster is
   * wanted, a {@link org.geotools.resources.image.DualRectIter} may be used.
   *
  ...
  private static final class NoDataReplacerDescriptor extends OperationDescriptorImpl {
     ...
     * of bands in the source image is equals to the number of supplied sample dimensions, and that
     * all sample dimensions has categories.
     *
