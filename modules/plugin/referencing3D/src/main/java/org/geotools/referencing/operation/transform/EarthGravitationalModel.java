 ...
 * <code><a href="http://earth-info.nga.mil/GandG/wgs84/gravitymod/wgs84_180/clenqt.for">clenqt
 * .for</a></code>
 ...
 * <code
 * ><a href="http://earth-info.nima.mil/GandG/wgs84/gravitymod/wgs84_180/egm180.nor">egm180.nor
 * </a></code>
    ...
    protected void load(final String filename) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            try {
                ...
                exception.initCause(cause); // TODO: Inline when we will be allowed to target 
                // Java 6.
                throw exception;
            }
        }
    ...
    private final void initialize() {
        ...
        if (wgs84) {
            ...
            for (int i = 2; i < c2n.length; i++) {
                sign *= -1;
                esqi *= esq;
                c2n[i] = sign * (3 * esqi) / ((2 * i + 1) * (2 * i + 3)) * (1 - i + (5 * i * c2 /
                        esq));
            }
            /* all nmax */
        ...
        for (int i = 0; i <= nmax; i++) {
            for (int j = i + 1; j <= nmax; j++) {
                ...
                bClenshaw[ll] = Math.sqrt(n * (j + i - 1) * (j - i - 1) / (double) (ji * (2 * j -
                        3)));
            }
        }
    }
