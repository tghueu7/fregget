...
public final class EarthGravitationalModel extends VerticalTransform {
  ...
  /**
   * Cleanshaw coefficients needed for the selected gravimetric quantities that are computed. Those
   * arrays are computed by the {@link #initialize} method.
   */
  ...
  /**
   * Loads the coefficients from the specified ASCII file and initialize the internal <cite>clenshaw
   * arrays</cite>.
   *
   * <p><strong>Note:</strong> ASCII may looks like an unefficient format for binary distribution. A
   * binary file with coefficient values read by {@link java.io.DataInput#readDouble} would be more
   * compact than an <u>uncompressed</u> ASCII file. However, binary files are hard to compress by
   * the ZIP algorithm. Our experience show that a 675 kb uncompressed ASCII file is only 222 kb
   * after ZIP or JAR compression. The same data as a binary file is 257 kb uncompressed and 248 kb
   * compressed. So surprisingly, the ASCII file is more compact than the binary file after
   * compression. Since it is the primary format provided by the Earth-Info web site, we use it
   * directly in order to avoid a multiplication of formats.
   *
    ...
    while ((line = in.readLine()) != null) {
      ...
      try {
        ...
        final IOException exception =
            new IOException(
                Errors.format(ErrorKeys.BAD_LINE_IN_FILE_$2, filename, in.getLineNumber()));
   ...
   * this step in a separated method than {@link #from} in case we wish to read the coefficient from
   * an other source than an ASCII file in some future version.
   */
    ...
    public static final ParameterDescriptor<Integer> ORDER =
        DefaultParameterDescriptor.create(
            Collections.singletonMap(
                NAME_KEY,
                new NamedIdentifier(
                    Citations.GEOTOOLS, Vocabulary.formatInternational(VocabularyKeys.ORDER))),
            DEFAULT_ORDER,
            2,
            180,
            false);
