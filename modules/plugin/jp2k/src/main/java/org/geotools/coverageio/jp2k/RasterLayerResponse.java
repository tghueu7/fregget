...
class RasterLayerResponse {
    ...
    class GranuleLoader implements Callable<RenderedImage> {
        ...
        GranuleLoader(
            ...
            this.tilesDimension = tilesDimension != null ? (Dimension) tilesDimension.clone() : 
                    null;
        }
        ...
        public RenderedImage call() throws Exception {
            ...
            return granule.loadRaster(readParameters, imageIndex, cropBBox, worldToGrid, request,
                    tilesDimension);
        }
    ...
    class GranuleWorker {
        ...
        public void init(final ReferencedEnvelope aoi) {
            ...
            final GranuleLoader loader = new GranuleLoader(baseReadParameters, imageChoice, bbox,
                    finalWorldToGridCorner, granule, request.getTileDimensions());
            ...
            for (Future<RenderedImage> future : tasks) {
                ...
                try {
                    ...
                    if (LOGGER.isLoggable(Level.SEVERE))
                        LOGGER.log(Level.SEVERE, "Unable to load the raster for granule " + 
                                granuleIndex, e);
                    continue;
                    ...
                    if (LOGGER.isLoggable(Level.FINE))
                        LOGGER.fine("Loading image number " + granuleIndex + " failed, original " +
                                "request was " + request);
                    continue;
    ...
    private RenderedImage assembleGranules() throws DataSourceException {
        ...
        try {
            ...
            final AffineTransform2D adjustments = new AffineTransform2D(
                    (level.resolutionX / baseLevel.resolutionX) * baseReadParameters
                            .getSourceXSubsampling(),
                    0,
                    0,
                    (level.resolutionY / baseLevel.resolutionY) * baseReadParameters
    ...
    private GridCoverage2D prepareCoverage(
        ...
        return coverageFactory.create(rasterManager.getCoverageIdentifier(), image, new 
                GeneralEnvelope(bbox), bands, null, null);
