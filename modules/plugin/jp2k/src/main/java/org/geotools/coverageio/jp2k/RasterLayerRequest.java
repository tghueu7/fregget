...
class RasterLayerRequest {
  ...
  private void computeCropRasterArea() throws DataSourceException {
    ...
    if (destinationRasterArea.isEmpty()) {
      if (LOGGER.isLoggable(Level.FINE))
        LOGGER.log(
            Level.FINE, "Requested envelope too small resulting in empty cropped raster region");
  ...
  private void computeCropBBOX() throws DataSourceException {
    ...
    try {
      ...
      //
      // STEP 2: intersect requested BBox in native CRS with coverage native bbox to get the crop
      // bbox
      //
      ...
      if (!CRS.equalsIgnoreMetadata(
        ...
        CoordinateOperation op =
            factory.createOperation(
                requestCRS, rasterManager.spatialDomainManager.coverageGeographicCRS2D);
