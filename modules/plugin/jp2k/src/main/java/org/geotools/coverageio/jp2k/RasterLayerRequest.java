...
class RasterLayerRequest {
    ...
    private final static Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (RasterLayerRequest.class);

    private ReadType readType = AbstractGridFormat.USE_JAI_IMAGEREAD.getDefaultValue() ? ReadType
            .JAI_IMAGEREAD : ReadType.DIRECT_READ;
     ...
     *
     * @param params                   The {@code GeneralParameterValue}s to initialize this
     *                                 request
     * @param baseGridCoverage2DReader
     * @throws DataSourceException
     */
    public RasterLayerRequest(final GeneralParameterValue[] params, final RasterManager 
            rasterManager) throws DataSourceException {
    ...
    private void prepare() throws DataSourceException {
        ...
        if (requestedBBox == null) {
            ...
            requestedRasterArea = (Rectangle) rasterManager.spatialDomainManager
                    .coverageRasterArea.clone();
            destinationRasterArea = (Rectangle) rasterManager.spatialDomainManager
                    .coverageRasterArea.clone();
            ...
            requestedGridToWorld = (AffineTransform) rasterManager.spatialDomainManager
    ...
    private void inspectCoordinateReferenceSystems() throws DataSourceException {
        ...
        if (!CRS.equalsIgnoreMetadata(requestCRS, rasterManager.spatialDomainManager.coverageCRS2D))
            try {
                destinationToSourceTransform = CRS.findMathTransform(requestCRS, rasterManager
                        .spatialDomainManager.coverageCRS2D, true);
        ...
        if (destinationToSourceTransform == null || destinationToSourceTransform.isIdentity()) {
            ...
            try {
                destinationRasterArea = new GeneralGridEnvelope(CRS.transform
                        (requestedWorldToGrid, new GeneralEnvelope(cropBBox)), PixelInCell
                        .CELL_CORNER, false).toRectangle();
            ...
            try {
                ...
                destinationRasterArea = new GeneralGridEnvelope(CRS.transform
                        (requestedWorldToGrid, cropBBOXInRequestCRS), PixelInCell.CELL_CORNER, 
                        false).toRectangle();
                //intersect with the original requested raster space to be sure that we stay 
                // within the requested raster area
     ...
     * @param destinationRasterArea represents the area to load in raster space. This
     ...
     * @param requestedRasterArea   is the requested region where to load data of the
     *                              specified envelope.
    ...
    private void computeRequestedResolution() throws DataSourceException {
        ...
        try {
            ...
            if (requestedGridToWorld instanceof LinearTransform) {
                ...
                // same and the conversion is not , we can get the resolution from envelope + 
                // raster directly
                //
                if (destinationToSourceTransform != null && !destinationToSourceTransform
                        .isIdentity()) {
                    ...
                    final GridToEnvelopeMapper geMapper = new GridToEnvelopeMapper(new 
                            GridEnvelope2D(destinationRasterArea), cropBBox);
                ...
                // should not happen
                throw new UnsupportedOperationException(Errors.format(ErrorKeys
                        .UNSUPPORTED_OPERATION_$1, requestedGridToWorld.toString()));
        ...
        //
        LOGGER.log(Level.WARNING, "Unable to compute requested resolution, using highest " +
                "available");
    ...
    private void computeCropBBOX() throws DataSourceException {
        ...
        try {
            ...
            if (!CRS.equalsIgnoreMetadata(requestCRS, rasterManager.spatialDomainManager
                    .coverageCRS2D))
                destinationToSourceTransform = CRS.findMathTransform(requestCRS, rasterManager
                        .spatialDomainManager.coverageCRS2D, true);
            // now transform the requested envelope to source crs
            if (destinationToSourceTransform != null && !destinationToSourceTransform.isIdentity
                ...
                //we do not need to do anything, but we do this in order to aboid problems with 
                // the envelope checks
            ...
            //
            // STEP 2: intersect requested BBox in native CRS with coverage native bbox to get 
            // the crop bbox
            //
            // intersect the requested area with the bounds of this
            // layer in native crs
            if (!cropBBox.intersects((BoundingBox) rasterManager.spatialDomainManager
                    .coverageBBox)) {
                cropBBox = null;
                empty = true;
                return;
            ...
            cropBBox = new ReferencedEnvelope(((ReferencedEnvelope) cropBBox).intersection
                    (rasterManager.spatialDomainManager.coverageBBox), rasterManager
                    .spatialDomainManager.coverageCRS2D);
        ...
        try {
            ...
            if (!CRS.equalsIgnoreMetadata(rasterManager.spatialDomainManager
                    .coverageGeographicCRS2D, requestCRS)) {
                ...
                requestedBBOXInCoverageGeographicCRS
                        .setCoordinateReferenceSystem(rasterManager.spatialDomainManager
            ...
            if (!requestedBBOXInCoverageGeographicCRS.intersects(rasterManager
                    .spatialDomainManager.coverageGeographicBBox, true)) {
                cropBBox = null;
                empty = true;
                return;
            }
            ...
            requestedBBOXInCoverageGeographicCRS.intersect(rasterManager.spatialDomainManager
                    .coverageGeographicBBox);
            requestedBBOXInCoverageGeographicCRS.setCoordinateReferenceSystem(rasterManager
                    .spatialDomainManager.coverageGeographicCRS2D);
            ...
            approximateRequestedBBoInNativeCRS = CRS.transform(
                    requestedBBOXInCoverageGeographicCRS,
                    rasterManager.spatialDomainManager.coverageCRS2D);
            approximateRequestedBBoInNativeCRS.setCoordinateReferenceSystem(rasterManager
                    .spatialDomainManager.coverageCRS2D);
        ...
        LOGGER.log(Level.INFO, "We did not manage to crop the requested envelope, we fall back " +
                "onto loading the whole coverage.");
        cropBBox = null;
    }
