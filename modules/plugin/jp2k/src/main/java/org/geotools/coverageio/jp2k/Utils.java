...
class Utils {
  ...
  /** Logger. */
  static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger(Utils.class.toString());
  ...
  static ImageReader getReader(final ImageInputStream inStream) {
    ...
    try {
      if (inStream instanceof FileImageInputStreamExt) {
        ...
        if (FILEFILTER.accept(file)) return JP2KFormatFactory.getCachedSpi().createReaderInstance();
      }
      return null;
