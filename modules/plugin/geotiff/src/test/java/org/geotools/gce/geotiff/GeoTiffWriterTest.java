...
public class GeoTiffWriterTest extends Assert {
    private static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger
            (GeoTiffWriterTest.class.toString());
    ...
    public void testDefaultGeographicWGS84() throws Exception {
        ...
        ReferencedEnvelope envelope = new ReferencedEnvelope(0, 10, 0, 10, DefaultGeographicCRS
                .WGS84);
        ...
        assertTrue(CRS.findMathTransform(reader.getCrs(), DefaultGeographicCRS.WGS84, true)
...
//	    @Ignore
    public void testWriteCroppedCoverage() throws IllegalArgumentException,
        ...
        //
        // /////////////////////////////////////////////////////////////////////
        IIOMetadataDumper metadataDumper = new IIOMetadataDumper(((GeoTiffReader) reader)
                .getMetadata().getRootNode());
        if (TestData.isInteractiveTest()) {
        ...
        assertTrue(
                "The Grdi2World tranformations of the original and the cropped covearage do not " +
                        "match",
                sourceG2W.equals(croppedG2W));
        // check that the envelope is correct
        final GeneralEnvelope expectedEnvelope = new GeneralEnvelope(croppedGR, PixelInCell
                .CELL_CENTER, croppedG2W, cropped.getCoordinateReferenceSystem2D());
        assertTrue("Expected envelope is different from the computed one", expectedEnvelope
                .equals(croppedEnvelope, XAffineTransform.getScale((AffineTransform) croppedG2W) 
                        / 2.0, false));
        ...
        //
        // /////////////////////////////////////////////////////////////////////
        final File writeFile = new File(new StringBuilder(writedir.getAbsolutePath()).append(File
                .separatorChar).append(cropped.getName().toString()).append(".tiff").toString());
...
//    @Ignore
    public void testWriteGoogleMercator() throws Exception {
        final String google = "PROJCS[\"WGS84 / Google Mercator\", GEOGCS[\"WGS 84\", " +
        ...
        // get a writer
        final File mercator = new File(TestData.file(GeoTiffReaderTest.class, "."), 
                "wms_900913.tif");
        ...
        assertTrue(coverage.getEnvelope2D().getFrame().equals(coverageMercator.getEnvelope2D()
                .getFrame()));
        reader.dispose();
...
//	    @Ignore
    public void testWriteTFW() throws Exception {
        ...
        // get a writer
        final File noCrsTFW = new File(TestData.file(GeoTiffReaderTest.class, "."), "no_crs_tfw" +
                ".tif");
        ...
        final File finalTFW = new File(noCrsTFW.getParent(), noCrsTFW.getName().replace("tif", 
                "tfw"));
...
//    @Ignore
    public void testWriteWithMetadata() throws Exception {
        ...
        // get a writer
        final File output = new File(TestData.file(GeoTiffReaderTest.class, "."), "outMetadata" +
                ".tif");
        ...
        writer.setMetadataValue(TagSet.BASELINE + ":" + Integer.toString(BaselineTIFFTagSet
        ...
        String readSoftware = metadata.getAsciiTIFFTag(Integer.toString(BaselineTIFFTagSet
                .TAG_SOFTWARE));
        assertTrue(software.equalsIgnoreCase(readSoftware));
        String readCopyright = metadata.getAsciiTIFFTag(Integer.toString(BaselineTIFFTagSet
                .TAG_COPYRIGHT));
    ...
    public void testWriteLatLon() throws Exception {
        ...
        final GeneralEnvelope envelope = (GeneralEnvelope) CRS.transform(coverage.getEnvelope(), 
                latLon4267);
        ...
        assertTrue(((GeneralEnvelope) gc.getEnvelope()).equals(coverage.getEnvelope(), 
                XAffineTransform.getScaleX0(g2w) * 1E-1, false));
        reader.dispose();
    ...
    public void testWriteBigTiff() throws Exception {
        ...
        for (String file : files) {
            ...
            // get a writer
            final File output = new File(TestData.file(GeoTiffReaderTest.class, "."), "bigtiff" +
                    i + ".tif");
            GeoTiffWriter writer = new GeoTiffWriter(output);
