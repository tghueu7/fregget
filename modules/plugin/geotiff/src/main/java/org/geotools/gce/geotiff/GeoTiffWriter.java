...
public class GeoTiffWriter extends AbstractGridCoverageWriter implements
     ...
     *              TIFFTagSet:TIFFTagID. As an instance: "BaselineTIFFTagSet:305" in order to 
     ...
     *                        model space.
     * @param retainAxesOrder <code>true</code> in case we want to retain the axes order, 
     *                        <code>false</code> otherwise for lon-lat enforcing.
    ...
    private static void setGeoReference(final CoordinateReferenceSystem crs,
        ...
        //
        metadata.addGeoShortParam(GeoTiffConstants.GTRasterTypeGeoKey, GeoTiffConstants
                .RasterPixelIsArea);
        ...
        //
        boolean swapAxes = XAffineTransform.getSwapXY(modifiedRasterToModel) == -1 || CRS
                .getAxisOrder(crs).equals(AxisOrder.NORTH_EAST);
    ...
    private boolean writeImage(final RenderedImage image,
        ...
        //
        final TIFFImageWriter writer = (TIFFImageWriter) GeoTiffFormat.IMAGEIO_WRITER_FACTORY
        ...
        try {
            ...
            // listeners
            if (listener != null) {
                final GridCoverageWriterProgressAdapter progressAdapter = new 
                        GridCoverageWriterProgressAdapter(listener);
