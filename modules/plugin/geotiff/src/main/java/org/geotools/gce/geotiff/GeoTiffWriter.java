...
public class GeoTiffWriter extends AbstractGridCoverageWriter implements GridCoverageWriter {
   ...
   *     Integer (referring to a tag ID) (in that case it will refer to the BaselineTIFFTagSet 2) OR
   *     an identifier in the form: TIFFTagSet:TIFFTagID. As an instance: "BaselineTIFFTagSet:305"
   *     in order to add the Copyright info.
  ...
  public void write(final GridCoverage gc, final GeneralParameterValue[] params)
    ...
    //
    writeImage(
        ((GridCoverage2D) gc).getRenderedImage(), this.outStream, metadata, gtParams, listener);
   ...
   * writing or the ModelTransformation in case a more general {@link AffineTransform} is needed to
   * represent the raster space to model space transform.
   *
   ...
   *     space.
   * @param retainAxesOrder <code>true</code> in case we want to retain the axes order, <code>false
   *     </code> otherwise for lon-lat enforcing.
  ...
  private boolean writeImage(
    ...
    try {
      ...
      writer.write(
          writer.getDefaultStreamMetadata(params), new IIOImage(image, null, metadata), params);
