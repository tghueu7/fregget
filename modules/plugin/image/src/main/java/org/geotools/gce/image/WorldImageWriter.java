...
public final class WorldImageWriter extends AbstractGridCoverageWriter
  ...
  /**
   * Takes a GridCoverage and writes the image to the destination file. It then reads the format of
   * the file and writes an accompanying world file. It will throw a
  ...
  /**
   * This method is responsible for creating a projection file using the WKT representation of this
   * coverage's coordinate reference system. We can reuse this file in order to rebuild later the
   * crs.
   *
   * @param baseFile
  ...
  private static void encode(
    ...
    /**
     * For the moment we do not work with DirectColorModel but instead we switch to component color
     * model which is really easier to handle even if it much more memory expensive. Once we are in
     * component color model is really easy to go to Gif and similar.
     */
    ...
    if (extension.compareToIgnoreCase("gif") == 0) {
      ...
      /**
       * IndexColorModel with more than 8 bits for sample might be a problem because GIF allows only
       * 8 bits based palette therefore I prefere switching to component color model in order to
       * handle this properly. NOTE. The only transfert types avalaible for IndexColorModel are byte
       * and ushort.
       */
      if (image.getColorModel() instanceof IndexColorModel
      ...
      /**
       * component color model is not well digested by the gif encoder we need to go to indecolor
       * model somehow. This code for the moment remove transparency, but I am confident I will find
       * a way to add that.
       */
      ...
      /**
       * IndexColorModel with full transparency support is not suitable for gif images we need to go
       * to bitmask loosing some informations. we have only one full transparent color.
       */
      if (image.getColorModel() instanceof IndexColorModel) {
        worker.forceIndexColorModelForGIF(true);
        image = worker.getRenderedImage();
