...
public class PostGISJsonTestSetup extends JDBCDelegatingTestSetup {
  ...
  private void createTestJsonTable() throws Exception {
    ...
    // --------------------------------------
    // There are two JSON data types: json and jsonb. They accept almost identical sets of values as
    // input.
    // The major practical difference is one of efficiency. The json data type stores an exact copy
    // of the input text [...]
    ...
    // white space between tokens, as well as the order of keys within JSON objects. Also, if a JSON
    // object within
    // the value contains the same key more than once, all the key/value pairs are kept.
    // By contrast, jsonb does not preserve white space, does not preserve the order of object keys,
    // and does not keep
    // duplicate object keys. If duplicate keys are specified in the input, only the last value is
    // kept.
    // One semantically-insignificant detail worth noting is that in jsonb, numbers will be printed
    // according
    // to the behavior of the underlying numeric type. In practice this means that numbers entered
    // with E notation
