...
public class PostGISJsonTestSetup extends JDBCDelegatingTestSetup {
    ...
    private void createTestJsonTable() throws Exception {
        ...
        String sql = "CREATE TABLE \"jsontest\" ("
                + "\"id\" INT, \"name\" VARCHAR, \"jsonColumn\" JSON, \"jsonbColumn\" JSONB, " +
                "PRIMARY KEY(id))";
        run(sql);
        ...
        // white space between tokens, as well as the order of keys within JSON objects. Also, if
        // a JSON object within 
        // the value contains the same key more than once, all the key/value pairs are kept. 
        // By contrast, jsonb does not preserve white space, does not preserve the order of 
        // object keys, and does not keep 
        // duplicate object keys. If duplicate keys are specified in the input, only the last 
        // value is kept.
        ...
        sql = "INSERT INTO \"jsontest\" VALUES (0, 'numberEntry','{\"weight\": 1e-3 }', " +
                "'{\"weight\": 1e-3}');"
                + "INSERT INTO \"jsontest\" VALUES (1, 'entryWithSpaces','{\"title\"    :    " +
                "\"Title\" }',"
                + "'{\"title\"    :    \"Title\" }');"
                + "INSERT INTO \"jsontest\" VALUES (2, 'duppedKeyEntry', '{\"title\":\"Title1\", " +
                "\"title\":\"Title2\"}',"
