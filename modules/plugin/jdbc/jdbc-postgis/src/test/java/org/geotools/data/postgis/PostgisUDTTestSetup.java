...
public class PostgisUDTTestSetup extends JDBCUDTTestSetup {
    ...
    protected void createUdtTable() throws Exception {
        ...
        run("CREATE DOMAIN foo4 AS bigint CONSTRAINT posbigint_check CHECK ((VALUE >= (0)" +
                "::bigint));");
        run("CREATE DOMAIN foo5 AS boolean CONSTRAINT weirdboolean_check CHECK ((VALUE));");
        run("CREATE DOMAIN foo6 AS smallint CONSTRAINT posmallint_check CHECK ((VALUE >= (0)" +
                "::smallint));");
        run("CREATE DOMAIN foo7 AS float4 CONSTRAINT posfloat4_check CHECK ((VALUE >= (0)" +
                "::float4));");
        ...
        run("CREATE TABLE \"udt\" (id integer PRIMARY KEY, ut foo, ut2 foo2, ut3 foo3, ut4 foo4, " +
                "ut5 foo5, ut6 foo6, ut7 foo7, ut8 foo8, ut9 foo9, ut10 foo10, ut11 foo11, ut12 " +
                "foo12, ut13 foo13);");
        run("INSERT INTO \"udt\" VALUES (0, '12ab', 6, 6.6, 85748957, true, 3, 3.3, 2, " +
                "'14:30'::time, '15:30'::time, '2004-10-31 16:30'::timestamp, '2004-10-30 " +
                "17:30'::timestamp, '00000000-0000-0000-0000-000000000000'::uuid);");
    ...
    public void createDateUdtTable() throws Exception {
        ...
        run(format("CREATE TABLE \"date_udt\" (id serial PRIMARY KEY, bd %s, name varchar);", 
                BIGDATE_UDT));
