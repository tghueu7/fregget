...
public class PostGISDialect extends BasicSQLDialect {
    ...
    public Geometry decodeGeometryValue(GeometryDescriptor descriptor,
                                        ResultSet rs, int column, GeometryFactory factory, 
                                        Connection cx, Hints hints)
            throws IOException, SQLException {
        WKBAttributeIO reader = getWKBReader(factory);
    ...
    public Class<?> getMapping(ResultSet columnMetaData, Connection cx)
        ...
        if ("geometry".equalsIgnoreCase(typeName)) {
            ...
            gType = lookupGeometryType(columnMetaData, cx, "geography_columns", 
                    "f_geography_column");
        } else {
            return null;
        }
    ...
    public Integer getGeometrySRID(String schemaName, String tableName,
        ...
        try {
            ...
            if (supportsGeography(cx)) {
                try {
                    ...
                    LOGGER.log(Level.WARNING, "Failed to retrieve information about "
                            ...
                            + " from the geometry_columns table, checking geometry_columns " +
                            "instead", e);
                } finally {
                    dataStore.closeSafe(result);
                }
            ...
            try {
                ...
                LOGGER.log(Level.WARNING, "Failed to retrieve information about "
                        + schemaName + "." + tableName + "." + columnName
                        + " from the geometry_columns table, checking the first geometry " +
                        "instead", e);
            } finally {
                dataStore.closeSafe(result);
            }
        ...
        try {
            ...
            if (supportsGeography(cx)) {
                try {
                    ...
                    LOGGER.log(Level.WARNING, "Failed to retrieve information about "
                            + schemaName + "." + tableName + "." + columnName
                            + " from the geography_columns table, checking geometry_columns " +
                            "instead", e);
                } finally {
                    dataStore.closeSafe(result);
                }
    ...
    protected Integer getDimensionFromFirstGeo(String schemaName, String tableName, String 
            columnName,
                                               Connection cx) throws SQLException {
                ...
                if (att instanceof GeometryDescriptor) {
                    ...
                    if (getVersion(cx).compareTo(V_2_0_0) >= 0) {
                        ...
                        if (dimensions == 3) {
                            ...
                            throw new IllegalArgumentException("PostGIS only supports geometries " +
                                    "with 2, 3 and 4 dimensions, current value: " + dimensions);
                        }
    ...
    public void encodeGeometryValue(Geometry value, int dimension, int srid, StringBuffer sql)
        ...
        if (value == null || value.isEmpty()) {
            ...
            if (value instanceof LinearRing && !(value instanceof CurvedRing)) {
                ...
                value = value.getFactory().createLineString(((LinearRing) value)
    ...
    public Version getPostgreSQLVersion(Connection conn) throws SQLException {
        if (pgsqlVersion == null) {
            ...
            pgsqlVersion = new Version(
                    String.format("%d.%d", md.getDatabaseMajorVersion(), md
