...
public class WKBAttributeIO {
   ...
   *     wkb
   * @throws IOException if more than one geometry object was found in the WTB representation, or if
   *     the parser could not parse the WKB representation.
   */
  private Geometry wkb2Geometry(byte[] wkbBytes) throws IOException {
    if (wkbBytes
        == null) // DJB: null value from database --> null geometry (the same behavior as WKT).
      // NOTE: sending back a GEOMETRYCOLLECTION(EMPTY) is also a possibility, but this
