...
class FilterToSqlHelper {
    ...
    void visitComparisonSpatialOperator(BinarySpatialOperator filter,
        ...
        if (filter instanceof BBOX3D) {
            ...
            visitBinarySpatialOperator(filter, (Expression) property, (Expression) geometry, 
                    swapped, extraData);
        }
    }
            ...
            if (g != null) {
                ...
                if (Math.sqrt(env.getWidth() * env.getWidth() + env.getHeight() * env.getHeight()
                    ...
                    for (double lon = Math.floor(env.getMinX()); lon < env.getMaxX(); lon += 90) {
                        for (double lat = Math.floor(env.getMinY()); lat < env.getMaxY(); lat += 
                                90) {
                            Geometry quadrant = JTS.toGeometry(new Envelope(lon, lon + 90, lat, 
                                    lat + 90));
