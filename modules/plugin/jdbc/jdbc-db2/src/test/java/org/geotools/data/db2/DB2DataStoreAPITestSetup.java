...
public class DB2DataStoreAPITestSetup extends JDBCDataStoreAPITestSetup {
    ...
    protected void createRoadTable() throws Exception {
        ...
        con.prepareStatement("CREATE TABLE " + DB2TestUtil.SCHEMA_QUOTED + ".\"road\"(\"fid\" int" +
                "  PRIMARY KEY not null GENERATED ALWAYS AS IDENTITY, \"id\" int, "
        ...
        String insertClause = "INSERT INTO " + DB2TestUtil.SCHEMA_QUOTED + ".\"road\"(\"id\"," +
                "\"geom\",\"name\")";
        con.prepareStatement(insertClause + " VALUES (0,"
                + "db2gse.ST_LineFromText('LINESTRING(1 1, 2 2, 4 2, 5 1)'," + DB2TestUtil.SRID +
                ")," + "'r1')").execute();
        con.prepareStatement(insertClause + " VALUES ( 1,"
                + "db2gse.ST_LineFromText('LINESTRING(3 0, 3 2, 3 3, 3 4)'," + DB2TestUtil.SRID +
                ")," + "'r2')").execute();
        con.prepareStatement(insertClause + " VALUES ( 2,"
                + "db2gse.ST_LineFromText('LINESTRING(3 2, 4 2, 5 3)'," + DB2TestUtil.SRID + "),"
                + "'r3')").execute();

        // Configure for DBSQLDialect.getOptimizedBounds
        DB2Util.executeRegisterAndCalculateExtent(DB2TestUtil.SCHEMA, "road", "geom", DB2TestUtil
                .SRSNAME, con);
        con.close();
    }
    ...
    protected void createRiverTable() throws Exception {
        ...
        con.prepareStatement("CREATE TABLE " + DB2TestUtil.SCHEMA_QUOTED + ".\"river\"(\"fid\" " +
                "int  PRIMARY KEY not null GENERATED ALWAYS AS IDENTITY, \"id\" int, "
        ...
        String insertClause = "INSERT INTO " + DB2TestUtil.SCHEMA_QUOTED + ".\"river\"(\"id\"," +
                "\"geom\",\"river\",\"flow\")";
        con.prepareStatement(insertClause + "  VALUES ( 0,"
                + "db2gse.ST_MLineFromText('MULTILINESTRING((5 5, 7 4),(7 5, 9 7, 13 7),(7 5, 9 " +
                "3, 11 3))'," + DB2TestUtil.SRID + "),"
                + "'rv1', 4.5)").execute();
        con.prepareStatement(insertClause + " VALUES ( 1,"
                + "db2gse.ST_MLineFromText('MULTILINESTRING((4 6, 4 8, 6 10))'," + DB2TestUtil
                .SRID + ")," + "'rv2', 3.0)").execute();
        con.close();
    }

    protected void createLakeTable() throws Exception {
        Connection con = getDataSource().getConnection();
        con.prepareStatement("CREATE TABLE " + DB2TestUtil.SCHEMA_QUOTED + ".\"lake\"(\"fid\" int" +
                "  PRIMARY KEY not null GENERATED ALWAYS AS IDENTITY, \"id\" int, "
                + "\"geom\" db2gse.ST_POLYGON, \"name\" varchar(255) ) ").execute();

        String insertClause = "INSERT INTO " + DB2TestUtil.SCHEMA_QUOTED + ".\"lake\"(\"id\"," +
                "\"geom\",\"name\")";
        ...
        con.prepareStatement(insertClause + " VALUES ( 0,"
                ...
                DB2TestUtil.SRID + ")," + "'muddy')").execute();
        con.close();
    }
