...
public class DB2AggregateTestSetup extends JDBCAggregateTestSetup {
    ...
    protected void createAggregateTable() throws Exception {
        ...
        con.prepareStatement("CREATE TABLE " + DB2TestUtil.SCHEMA_QUOTED + ".\"aggregate\"" +
                "(\"fid\" int  PRIMARY KEY not null GENERATED ALWAYS AS IDENTITY, \"id\" int, "
                + "\"geom\" db2gse.ST_POLYGON, \"name\" varchar(255) ) ").execute();

        String insertClause = "INSERT INTO " + DB2TestUtil.SCHEMA_QUOTED + ".\"aggregate\"" +
                "(\"id\",\"geom\",\"name\")";
