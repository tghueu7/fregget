...
public class SqlServerBinaryReader {
    ...
    private Geometry decodeMultiLinestring(int shapeIndex) {
        ...
        for (int i = shapeIndex; i < binary.getShapes().length; i++) {
            if (binary.getShape(i).getParentOffset() == shapeIndex) {
                linestrings.add(gf.createLineString(binary.getSequence(binary.getShape(i)
                        .getFigureOffset())));
            }
        }
    ...
    private void readCoordinateSequences() {
        ...
        for (int i = 0; i < figures.length; i++) {
            ...
            int nextPointOffset = figures.length >= i + 2 ? figures[i + 1].getPointOffset() : 
                    binary.getCoordinates().length;
            Coordinate[] coordinates = Arrays.copyOfRange(binary.getCoordinates(), 
                    figurePointOffset, nextPointOffset);
            int attribute = figures[i].getAttribute();
            if ((attribute == 0 || attribute == 2) && !coordinates[0].equals
    ...
    private void parse(InStream is) throws IOException {
        ...
        if (version != 1) {
            throw new SqlServerBinaryParseException("Unsupported version (only supports version " +
                    "1): " + version);
        }
