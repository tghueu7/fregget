...
public class SQLServerDataStoreAPITestSetup extends JDBCDataStoreAPITestSetup {
    ...
    protected void createRoadTable() throws Exception {
        ...
        run("CREATE SPATIAL INDEX _road_geometry_index on road(geom) WITH (BOUNDING_BOX = (-10, " +
                "-10, 10, 10))");
    }
    ...
    protected void createRiverTable() throws Exception {
        ...
        run("INSERT INTO river (id,geom,river, flow)  VALUES ( 0,"
                + "geometry::STGeomFromText('MULTILINESTRING((5 5, 7 4),(7 5, 9 7, 13 7),(7 5, 9 " +
                "3, 11 3))',4326),"
                + "'rv1', 4.5)");
        run("INSERT INTO river (id,geom,river, flow) VALUES ( 1,"
                + "geometry::STGeomFromText('MULTILINESTRING((4 6, 4 8, 6 10))',4326)," + "'rv2'," +
                " 3.0)");

        run("CREATE SPATIAL INDEX _river_geometry_index on river(geom) WITH (BOUNDING_BOX = (-10," +
                " -10, 10, 10))");
    }
    ...
    protected void createLakeTable() throws Exception {
        ...
        run("CREATE SPATIAL INDEX _lake_geometry_index on lake(geom) WITH (BOUNDING_BOX = (-100, " +
                "-100, 100, 100))");
    }
