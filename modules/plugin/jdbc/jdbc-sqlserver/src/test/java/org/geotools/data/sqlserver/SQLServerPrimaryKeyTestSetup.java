...
public class SQLServerPrimaryKeyTestSetup extends JDBCPrimaryKeyTestSetup {
    ...
    protected void createAutoGeneratedPrimaryKeyTable() throws Exception {
        ...
        run("INSERT INTO auto (name,geom ) VALUES ('one',geometry::STGeomFromText('POINT(1 1)'," +
                "4326))");
        run("INSERT INTO auto (name,geom ) VALUES ('two',geometry::STGeomFromText('POINT(2 2)'," +
                "4326))");
        run("INSERT INTO auto (name,geom ) VALUES ('three',geometry::STGeomFromText('POINT(3 3)'," +
                "4326))");

        run("CREATE SPATIAL INDEX _auto_geometry_index on auto(geom) WITH (BOUNDING_BOX = (-10, " +
                "-10, 10, 10))");
    }
    ...
    protected void createNonIncrementingPrimaryKeyTable() throws Exception {
        ...
        run("CREATE SPATIAL INDEX _noninc_geometry_index on noninc(geom) WITH (BOUNDING_BOX = " +
                "(-10, -10, 10, 10))");
    }
    ...
    protected void createMultiColumnPrimaryKeyTable() throws Exception {
        ...
        run("INSERT INTO multi VALUES (2, 'y', 'two', geometry::STGeomFromText('POINT(2 2)',4326)" +
                ")");
        run("INSERT INTO multi VALUES (3, 'z', 'three', geometry::STGeomFromText('POINT(3 3)'," +
                "4326))");

        run("CREATE SPATIAL INDEX _multi_geometry_index on multi(geom) WITH (BOUNDING_BOX = (-10," +
                " -10, 10, 10))");
    }
    ...
    protected void createUniqueIndexTable() throws Exception {
        ...
        // run("CREATE SPATIAL INDEX _uniq_geometry_index on uniq(geom) WITH (BOUNDING_BOX = 
        // (-10, -10, 10, 10))");
    }

    @Override
    protected void createNonFirstColumnPrimaryKey() throws Exception {
        run("CREATE TABLE nonfirst (name VARCHAR(255), pkey int IDENTITY(1,1) PRIMARY KEY, geom " +
                "geometry)");

        run("INSERT INTO nonfirst (name,geom ) VALUES ('one',geometry::STGeomFromText('POINT(1 1)" +
                "',4326))");
        run("INSERT INTO nonfirst (name,geom ) VALUES ('two',geometry::STGeomFromText('POINT(2 2)" +
                "',4326))");
        run("INSERT INTO nonfirst (name,geom ) VALUES ('three',geometry::STGeomFromText('POINT(3 " +
                "3)',4326))");

        run("CREATE SPATIAL INDEX _auto_geometry_index on nonfirst(geom) WITH (BOUNDING_BOX = " +
                "(-10, -10, 10, 10))");
