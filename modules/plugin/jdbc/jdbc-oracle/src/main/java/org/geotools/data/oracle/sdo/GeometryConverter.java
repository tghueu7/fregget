...
public class GeometryConverter {
    ...
    public STRUCT toSDO(Geometry geom, int srid) throws SQLException {
        ...
        if (point == null) {
            ...
            if (env.getWidth() > 0 && env.getHeight() > 0 && !(geom instanceof 
                    GeometryCollection) && geom.isRectangle()) {
                ...
                double ordinates[];
                if (SDO.D(geom) == 2)
                    ordinates = new double[]{env.getMinX(), env.getMinY(), env.getMaxX(), env
                            .getMaxY()};
                else
                    ordinates = new double[]{env.getMinX(), env.getMinY(), 0, env.getMaxX(), env
                            .getMaxY(), 0};
