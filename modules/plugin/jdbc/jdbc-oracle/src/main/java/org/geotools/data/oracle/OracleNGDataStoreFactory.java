...
public class OracleNGDataStoreFactory extends JDBCDataStoreFactory {
    ...
    /**
     * parameter for getting bbox from MDSYS.USER_SDO_GEOM_METADATA or MDSYS
     * .ALL_SDO_GEOM_METADATA table
     */
    public static final Param METADATA_BBOX = new Param("Metadata bbox", Boolean.class, "Get data" +
            " bounds quickly from MDSYS.USER_SDO_GEOM_METADATA or MDSYS.ALL_SDO_GEOM_METADATA " +
    ...
    protected String getJDBCUrl(Map params) throws IOException {
        ...
        if (db.startsWith("(") || db.startsWith("ldap://"))
            ...
            // for Oracle RAC
            // (DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=[host])(PORT=[port, often 1521]))
            // (LOAD_BALANCE=YES)(CONNECT_DATA=(SERVICE_NAME=[oracle_service_name])))
            return JDBC_PATH + db;
            ...
            throw new IOException("Unable to properly compose the JDBC URL string, some " +
                    "parameters as host and port may be null !");
    }
