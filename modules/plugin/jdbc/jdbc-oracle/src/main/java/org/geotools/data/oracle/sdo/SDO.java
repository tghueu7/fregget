...
public final class SDO {
  ...
  private static void addElemInfo(List elemInfoList, MultiPoint points, final int STARTING_OFFSET) {
    addInt(elemInfoList, STARTING_OFFSET);
    addInt(elemInfoList, ETYPE.POINT);
  ...
  private static void addElemInfo(
      List elemInfoList, GeometryCollection geoms, final int STARTING_OFFSET, final int GTYPE) {
    Geometry geom;
    int offset = STARTING_OFFSET;
    int LEN = D(GTYPE) + L(GTYPE);
   ...
   *   <li><code>compound</code> elements are defined by a header triplet, and a series of triplets
   *       for the parts. Elements in a compound element share first and last points.
   ...
   * </code></pre>
   *
   * <p>When playing with circular arcs (SDO_INTERPRETATION==2) arcs are defined by three points. A
   * start point, any point on the arc and the end point. The last point of an arc is the start
   * point of the next. When used to describe a polygon (SDO_ETYPE==1003 or 2003) the first and last
   ...
   *   <li>d003 polygon/d007 multipolygon elements that are not SDO_ETYPE==3 unordered polygon lines
   *       or SDO_ETYPE==5 unorderd compound polygon ring are ignored
   * </ul>
   *
   ...
   *
   * <p>CoordinateSequence & CoordinateAccess wil be used to determine the dimension, and the number
   * of ordinates added.
   *
   ...
   *
   * <p>You should in sure that the provided <code>polygon</code> is a rectangle using isRectangle(
   * Polygon )
   *
   * <p>
   *
  ...
  */
  ...
  /**
   * We need to check if a <code>polygon</code> is defined with curves so we can produce the correct
   * encoding.
   *
   * <p>
   *
   ...
   *
   * <p>Works like assert exception IllegalArgumentException is thrown indicating this is a required
   * check.
   *
    ...
    if (!(min <= actual && actual <= max)) {
      String msg =
          MessageFormat.format(
              condition, new Object[] {new Integer(min), new Integer(actual), new Integer(max)});
   ...
   * </ul>
   *
   * <p>The number of ordinates per coordinate are taken to be D, and the number of ordinates should
   ...
   *
   * <p>For 3-dimensional coordinates we assume z to be the third ordinate. If the LRS measure value
   * is stored in the third ordinate (L=3) we assume a 2-dimensional coordinate.
   *
  ...
  public static CoordinateSequence coordinates(
    ...
    if (L != 0) {
      ...
      // thus we will get only 3 dimensions!
  ...
  public static Geometry create(
    ...
    switch (SDO.TT(GTYPE)) {
      ...
      default:
        LOGGER.warning(
            "Cannot represent provided SDO STRUCT (GTYPE =" + GTYPE + ") using JTS Geometry");
        return null;
    }
  }
   ...
   * infinite tolerance (the linearization will happen using the default base segments number set in
   * {@link CircularArc}
   *
   * @param gf
   * @return
  ...
  private static Point createPoint(
    ...
    Point point =
        gf.createPoint(
            subList(gf.getCoordinateSequenceFactory(), coords, GTYPE, elemInfo, element, false));
  ...
  private static LineString createLine(
    ...
    if (etype == ETYPE.LINE && INTERPRETATION == 1) {
      CoordinateSequence subList =
          subList(
              gf.getCoordinateSequenceFactory(), coords, GTYPE, elemInfo, triplet, compoundElement);
