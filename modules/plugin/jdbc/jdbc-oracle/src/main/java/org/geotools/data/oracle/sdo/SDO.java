 ...
 * @source $URL$
 * @see 
 * <a href="http://otn.oracle.com/pls/db10g/db10g.to_toc?pathname=appdev.101%2Fb10826%2Ftoc.htm&remark=portal+%28Unstructured+data%29">Spatial User's Guide (10.1)</a>
...
public final class SDO {
    ...
    */
    ...
    public static CoordinateSequence coordinates(CoordinateSequenceFactory f,
        ...
        final int LEN = D; // bugfix 20121231-BK: LEN = D instead of LEN = D + L as Oracle 
        // supports only one LRS ordinate!
     ...
     * @return DOCUMENT ME!
     * @deprecated bugfix 20121231-BK: Oracle supports only one LRS measure information! use 
     * {@link SDO#coordiantes(CoordinateSequenceFactory, OrdinateList, OrdinateList, OrdinateList, OrdinateList) coordiantes() with just a OrdinateList of measures}!
        ...
        if ((L == 0) && (TT == 01) && (point != null) && (elemInfo == null)) {
            ...
            if (etype == 0) {
                ...
                if (etype != -1) {
                    ...
                    System.arraycopy(ordinates, startpointCoordinates - 1, newOrdinates, 0, 
                    elemsToCopy);
    ...
    public static Geometry create(GeometryFactory gf, final int GTYPE,
        ...
        switch (SDO.TT(GTYPE)) {
            ...
            case TT.UNKNOWN:
            default:
                LOGGER.warning("Cannot represent provided SDO STRUCT (GTYPE =" + GTYPE + ") using" +
                 " JTS Geometry");
                return null;
        }
    }
    ...
    private static Point createPoint(GeometryFactory gf, final int GTYPE,
        ...
        if (etype != ETYPE.POINT)
            throw new IllegalArgumentException("ETYPE " + etype + " inconsistent with expected " +
             "POINT");
        if (INTERPRETATION != 1) {
            LOGGER.warning("Could not create JTS Point with INTERPRETATION " + INTERPRETATION + "" +
    ...
    private static Polygon createPolygon(CurvedGeometryFactory gf, final int GTYPE,
                                         ...
                                         CoordinateSequence coords) throws 
    ...
    private static LinearRing createLinearRing(CurvedGeometryFactory gf,
                                               final int GTYPE, final int SRID, final int[] 
                                               elemInfo,
        ...
        if (!(STARTING_OFFSET >= 1) || !(STARTING_OFFSET <= LENGTH))
            throw new IllegalArgumentException("ELEM_INFO STARTING_OFFSET " + STARTING_OFFSET + "" +
             " inconsistent with ORDINATES length " + coords.size());
        ...
        if (eTYPE == ETYPE.COMPOUND_POLYGON_EXTERIOR || eTYPE == ETYPE.COMPOUND_POLYGON_INTERIOR) {
            ...
            if (ext.size() != 3) {
                throw new IllegalArgumentException(
                        "The coordinate sequence for the circle creation must contain 3 points, " +
                         "the one at hand contains "
                                + ext.size() + " instead");
            }
    ...
    private static MultiPoint createMultiPoint(GeometryFactory gf,
        ...
        if (!(INTERPRETATION >= 1)) {
            LOGGER.warning("Could not create MultiPoint with INTERPRETATION " + INTERPRETATION + 
    ...
    private static MultiPolygon createMultiPolygon(CurvedGeometryFactory gf,
                                                   ...
                                                   elemInfo,
                                                   final int triplet, CoordinateSequence coords, 
                                                   final int N, boolean threeDimensional) {
        final int STARTING_OFFSET = STARTING_OFFSET(elemInfo, triplet);
        final int eTYPE = ETYPE(elemInfo, triplet);
        final int INTERPRETATION = INTERPRETATION(elemInfo, triplet);
        final int LENGTH = coords.size() * D(GTYPE);
        ...
        if (!(eTYPE == ETYPE.POLYGON) && !(eTYPE == ETYPE.POLYGON_EXTERIOR)
            ...
            throw new IllegalArgumentException("ETYPE " + eTYPE + " inconsistent with expected " +
             "POLYGON or POLYGON_EXTERIOR");
        if (!(eTYPE == ETYPE.COMPOUND_POLYGON) && !(eTYPE == ETYPE.COMPOUND_POLYGON_EXTERIOR)
                && INTERPRETATION != 1 && INTERPRETATION != 3) {
            LOGGER.warning("Could not create MultiPolygon with INTERPRETATION " + INTERPRETATION 
        ...
        for (int i = triplet;
            ...
            if ((etype == ETYPE.POLYGON) || (etype == ETYPE.POLYGON_EXTERIOR)
                ...
                list.add(poly);
            } else if (etype == ETYPE.COMPOUND_POLYGON_EXTERIOR || etype == ETYPE
            .COMPOUND_POLYGON) {
    ...
    private static GeometryCollection createCollection(CurvedGeometryFactory gf,
                                                       final int GTYPE, final int SRID, final 
                                                       int[] elemInfo,
                                                       final int triplet, CoordinateSequence 
                                                       coords, final int N) {
        final int STARTING_OFFSET = STARTING_OFFSET(elemInfo, triplet);
                ...
                case ETYPE.POLYGON_INTERIOR:
                    throw new IllegalArgumentException(
                            "ETYPE 2003 (Polygon Interior) no expected in a GeometryCollection"
                                    + "(2003 is used to represent polygon holes, in a 1003 " +
                                     "polygon exterior)");
