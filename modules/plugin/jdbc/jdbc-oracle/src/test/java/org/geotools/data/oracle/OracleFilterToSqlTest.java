...
public class OracleFilterToSqlTest extends TestCase {
  ...
  public void testBboxFilter() throws Exception {
    ...
    assertEquals(
        "WHERE SDO_RELATE(\"GEOM\", ?, 'mask=anyinteract querytype=WINDOW') = 'TRUE' ", encoded);
  }
  ...
  public void testLooseBboxFilter() throws Exception {
    ...
    String encoded = encoder.encodeToString(bbox);
    assertEquals(
        "WHERE SDO_FILTER(\"GEOM\", ?, 'mask=anyinteract querytype=WINDOW') = 'TRUE' ", encoded);
  }

  public void testContainsFilter() throws Exception {
    Contains contains =
        ff.contains(ff.property("SHAPE"), ff.literal(gf.createPoint(new Coordinate(10.0, -10.0))));
    String encoded = encoder.encodeToString(contains);
    assertEquals(
        "WHERE SDO_RELATE(\"SHAPE\", ?, 'mask=contains querytype=WINDOW') = 'TRUE' ", encoded);
  }
  ...
  public void testDWithinFilterWithUnit() throws Exception {
    ...
    DWithin dwithin =
        ff.dwithin(ff.property("GEOM"), ff.literal(gf.createPoint(coordinate)), 10.0, "kilometers");
