...
public class OracleGeometryTestSetup extends JDBCGeometryTestSetup {
    ...
    public void setUp() throws Exception {
        ...
        sql = "INSERT INTO USER_SDO_GEOM_METADATA (TABLE_NAME, COLUMN_NAME, DIMINFO, SRID ) " +
                "VALUES ('GTMETA','GEOMETRY',MDSYS.SDO_DIM_ARRAY(MDSYS.SDO_DIM_ELEMENT('X',-180," +
                "180,0.5), " +
                "MDSYS.SDO_DIM_ELEMENT('Y',-90,90,0.5)), 4326)";
        run(sql);

        sql = "CREATE INDEX GTMETA_GEOMETRY_IDX ON GTMETA(GEOMETRY) INDEXTYPE IS MDSYS" +
                ".SPATIAL_INDEX";
        run(sql);

        sql = "INSERT INTO GTMETA VALUES (0," +
                "MDSYS.SDO_GEOMETRY(2001,4326,SDO_POINT_TYPE(0.0,0.0,NULL),NULL,NULL), 0, 0.0," +
                "'zero')";
        run(sql);
    }
    ...
    public void setupGeometryColumns(JDBCDataStore dataStore) throws Exception {
        ...
        String sql = "CREATE TABLE GEOMETRY_COLUMNS(F_TABLE_SCHEMA VARCHAR(30), F_TABLE_NAME " +
                "VARCHAR(30), " +
                "F_GEOMETRY_COLUMN VARCHAR(30), COORD_DIMENSION INTEGER, SRID INTEGER, TYPE " +
                "VARCHAR(30))";
        run(sql);

        // register it in the override table with a different srs, so that we're sure it's 
        // getting read
        sql = "INSERT INTO GEOMETRY_COLUMNS (F_TABLE_SCHEMA, F_TABLE_NAME, F_GEOMETRY_COLUMN, " +
