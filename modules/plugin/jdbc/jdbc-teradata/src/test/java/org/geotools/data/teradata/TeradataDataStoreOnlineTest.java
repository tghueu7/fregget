...
public class TeradataDataStoreOnlineTest extends JDBCDataStoreOnlineTest {
        ...
        for (int i = 0; i < t.length; i++) {
            ...
            t[i] = new Thread(new Runnable() {
                ...
                public void run() {
                    for (int j = 0; j < 50; j++) {
                        try {
                            FeatureWriter w = dataStore.getFeatureWriter(tname("ft2"), 
    ...
    public void testCreateSchemaWithCaseSensitivity() throws Exception {
        ...
        PropertyIsEqualTo correct = ff.equal(ff.property(aname("stringProperty")), ff.literal
                ("one"), true);
        PropertyIsEqualTo incorrect = ff.equal(ff.property(aname("stringProperty")), ff.literal
                ("OnE"), true);

        assertEquals(1, dataStore.getFeatureSource("ft2").getCount(new Query(tname("ft2"), 
                correct)));
        assertEquals(0, dataStore.getFeatureSource("ft2").getCount(new Query(tname("ft2"), 
                incorrect)));
    }
}
