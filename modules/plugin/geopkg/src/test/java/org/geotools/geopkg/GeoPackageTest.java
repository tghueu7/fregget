...
public class GeoPackageTest {
    ...
    void assertDefaultSpatialReferencesExist() throws Exception {
        ...
        try {
            ResultSet rs = st.executeQuery("SELECT srs_id FROM gpkg_spatial_ref_sys WHERE srs_id " +
                    "= -1");
    ...
    boolean doesEntryExists(String table, Entry entry) throws Exception {
        ...
        try {
            ...
            SqlUtil.PreparedStatementBuilder psb = SqlUtil.prepare(cx, sql).set(entry
                    .getTableName());
    ...
    public void testSRS() throws Exception {
        ...
        Connection cx = geopkg.getDataSource().getConnection();
        try {
            String sql = String.format("SELECT srs_name FROM %s WHERE srs_id = ?", GeoPackage
                    .SPATIAL_REF_SYS);
    ...
    public void test3DGeometry() throws Exception {
        ...
        Point geom = new GeometryFactory(new PrecisionModel(), 4326).createPoint(new Coordinate
                (5, 3, 8));
        ...
        try {
            while (re.hasNext()) {
                ...
                ResultSet rs = st.executeQuery((String.format(
                        "SELECT ST_MinX(the_geom), ST_MinY(the_geom), ST_MaxX(the_geom), ST_MaxY" +
                                "(the_geom), ST_IsEmpty(the_geom) FROM bugsites WHERE ID=" + f
                                .getProperty("ID").getValue())));
                assertEquals(rs.getDouble(1), ((Geometry) f.getDefaultGeometry())
                        .getEnvelopeInternal().getMinX(), 0.0001);
                assertEquals(rs.getDouble(2), ((Geometry) f.getDefaultGeometry())
                        .getEnvelopeInternal().getMinY(), 0.0001);
                assertEquals(rs.getDouble(3), ((Geometry) f.getDefaultGeometry())
                        .getEnvelopeInternal().getMaxX(), 0.0001);
                assertEquals(rs.getDouble(4), ((Geometry) f.getDefaultGeometry())
                        .getEnvelopeInternal().getMaxY(), 0.0001);
    ...
    public void testSpatialIndexWriting() throws Exception {
        ...
        try (SimpleFeatureIterator it = coll.features()) {
            ...
            //some features
            try (Transaction tx = new DefaultTransaction(); SimpleFeatureWriter w = geopkg.writer
                    (entry, true, null, tx)) {
                for (int i = 0; i < 3; i++) {
        ...
        try (Connection cx = geopkg.getDataSource().getConnection(); Statement st = cx
                .createStatement()) {
     ...
     *
     * "The tile coordinate (0,0) always refers to the tile in the upper left corner of the tile 
     * matrix at any zoom
    ...
    public void testTopLeftTile() throws IOException, FactoryException {
        File sourceFile = GeoPackageFormat.getFileFromSource(getClass().getResource("Blue_Marble" +
                ".gpkg"));
        ...
        ImageAssert.assertEquals(URLs.urlToFile(getClass().getResource("bluemarble_0_0_0.jpeg")),
                tileImg, 250);

        //Render the GeoPackage at zoom level 0
        GeoPackageReader reader = new GeoPackageReader(getClass().getResource("Blue_Marble.gpkg")
                , null);

        GeneralParameterValue[] parameters = new GeneralParameterValue[1];
        GridGeometry2D gg = new GridGeometry2D(new GridEnvelope2D(new Rectangle(1536, 768)), new 
                ReferencedEnvelope(-180, 180, -90, 90, CRS.decode("EPSG:4326", true)));
    ...
    void assertFeatureEntry(FeatureEntry entry) throws Exception {
        ...
        try {
            ...
            assertEquals(entry.getGeometryType(), Geometries.getForName(rs.getString
                    ("geometry_type_name")));
            assertEquals(entry.getSrid().intValue(), rs.getInt("srs_id"));
