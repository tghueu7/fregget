...
public class GeoPackageTest {
  ...
  void assertLastChangedDateString(Calendar startTime, Calendar endTime) throws Exception {
    ...
    try (Connection cx = geopkg.getDataSource().getConnection();
        ...
        ResultSet rs =
            st.executeQuery(
                String.format("SELECT last_change FROM %s;", GeoPackage.GEOPACKAGE_CONTENTS))) {
  ...
  public void testSRS() throws Exception {
    ...
    try {
      String sql =
          String.format("SELECT srs_name FROM %s WHERE srs_id = ?", GeoPackage.SPATIAL_REF_SYS);
  ...
  public void test3DGeometry() throws Exception {
    ...
    Point geom =
        new GeometryFactory(new PrecisionModel(), 4326).createPoint(new Coordinate(5, 3, 8));
  ...
  void assertFeatureEntry(FeatureEntry entry) throws Exception {
    ...
    try {
      ...
      assertEquals(
          entry.getGeometryType(), Geometries.getForName(rs.getString("geometry_type_name")));
      assertEquals(entry.getSrid().intValue(), rs.getInt("srs_id"));
  ...
  URL setUpBoolShapefile() throws Exception {
    ...
    for (String ext : exts) {
      if ("prj".equals(ext)) {
        ...
        FileUtils.copyURLToFile(
            TestData.url("shapes/BooleanTest." + ext), new File(d, "BooleanTest." + ext));
      }
    }
