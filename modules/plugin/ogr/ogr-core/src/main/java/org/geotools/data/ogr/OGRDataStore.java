...
public class OGRDataStore extends ContentDataStore {
   ...
   * @param approximateFields if true, OGR will try to create fields that are approximations of the
   *     required ones when an exact match cannt be provided
   ...
   * possible to call createSchema() independently from a write, as the result will not contain the
   * schema definition without having data too. Also, in those formats, the output is writable only
   * so as long as it's empty, it's not possible to write against an existing GML file for example.
   *
   * @param schema the geotools schema
   * @param approximateFields if true, OGR will try to create fields that are approximations of the
   *     required ones when an exact match cannt be provided
