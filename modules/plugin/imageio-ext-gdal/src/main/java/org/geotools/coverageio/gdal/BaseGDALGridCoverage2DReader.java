...
public abstract class BaseGDALGridCoverage2DReader extends
    ...
    protected BaseGDALGridCoverage2DReader(Object input, final Hints hints,
                                           ...
                                           final ImageReaderSpi formatSpecificSpi) throws 
            DataSourceException {
    ...
    protected void setCoverageProperties(ImageReader reader) throws IOException {
        ...
        if (!(metadata instanceof GDALCommonIIOImageMetadata)) {
            throw new DataSourceException("Unexpected error! Metadata should be an instance of " +
                    "the expected class: GDALCommonIIOImageMetadata.");
        }
     ...
     *
     * @param metadata a {@link GDALCommonIIOImageMetadata} metadata instance
    ...
    private void parseCommonMetadata(final GDALCommonIIOImageMetadata metadata) {
        ...
        if (tempCRS != null) {
            ...
            // //
            parsePRJFile();
            // if there was not prj or the envelope could not be created easily, let's go with 
                ...
                if ((wkt != null) && !(wkt.equalsIgnoreCase(""))) {
                    try {
                        ...
                        if (LOGGER.isLoggable(Level.FINE)) {
                            LOGGER.log(Level.FINE, "Unable to get CRS from WKT contained in " +
                                    "metadata. Looking for a PRJ.");
                        }
                        //reset crs 
                        this.crs = null;
                    }
