...
class RasterLayerRequest {
  ...
  /**
   * Specify if a JAI ImageRead operation should use multithreading or not. Note that multithreading
   * is supported using a special JAI ImageReadMT operation
   */
  private boolean useMultithreading = false;

  /**
   * The imageRead parameters involved in the coverage request (source region, subsampling factors)
   * which will be used by a coverageResponse to read data.
   */
  ...
  /**
   * Set to {@code true} if this request will produce an empty result, and the coverageResponse will
   * produce a {@code null} coverage.
   */
  ...
  /**
   * Set to {@code true} if the read operation needed to request data is a JAI Image Read operation.
   * Set to {@code false} if the read operation is a direct {@code ImageReader.read(...)} call.
   */
  ...
  public synchronized void prepare() {
    try {
      ...
      if (requestedBBox != null && !requestedBBox.isEmpty()) {
        ...
        // Concatenating an adjustment to the native grid2world is requested since the requested
   ...
   * imageRead operation need to be performed or {@code false} if a simple read operation is needed.
   *
    ...
    try {
      ...
      if (requestedBBox != null) {
        ...
        XRectangle2D.intersect(
            coverageRequestedRasterArea, coverageRasterArea, coverageRequestedRasterArea);
  ...
  private void adjustRequestedBBox() throws TransformException, FactoryException {
    ...
    try {
      ...
      // XXX Optimize when referenced envelope has intersection method that actually retains the
      // CRS, this is the JTS one
      requestedBBox =
          new ReferencedEnvelope(
   ...
   *
   * @param reader a {@link BaseGridCoverage2DReader} from where to get basic coverage properties as
   *     well as basic parameters to be used by the incoming read operations.
   */
