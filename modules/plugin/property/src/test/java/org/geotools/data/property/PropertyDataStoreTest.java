...
public class PropertyDataStoreTest extends TestCase {
    ...
    public void testGetFeaturesFeatureTypeFilterTransaction1() throws Exception {
        ...
        FeatureReader<SimpleFeatureType, SimpleFeature> reader = store.getFeatureReader
                (roadQuery, Transaction.AUTO_COMMIT);
        int count = 0;
        try {
    ...
    public void testWriterSkipThrough() throws Exception {
        ...
        PropertyFeatureWriter writer = (PropertyFeatureWriter) featureStore.getWriterInternal
                (query, 0x01 << 0);
    ...
    public void testWriterAppendLastNull() throws Exception {
        FeatureWriter<SimpleFeatureType, SimpleFeature> writer = store.getFeatureWriterAppend
    ...
    public void testTransactionIndependence() throws Exception {
        ...
        assertEquals("client 2 after client 1 removes fid1", 5, roadFromClient2.getFeatures()
                .size());
        assertEquals("client 1 after client 1 removes fid1", 4, roadFromClient1.getCount(Query
                .ALL));
        assertEquals("client 2 after client 1 removes fid1", 5, roadFromClient2.getCount(Query
                .ALL));
        ...
        roadFromClient2.addFeatures(DataUtilities.collection(chrisFeature)); // road2 adds fid5 
        // on t2
        ...
        assertEquals("client 2 after client 1 removes fid1 and client 2 adds fid5", 6, 
                roadFromClient2.getFeatures().size());

        bounds = roadAuto.getFeatures().getBounds();
        client1Bounds = roadFromClient1.getFeatures().getBounds();
        client2Bounds = roadFromClient2.getFeatures().getBounds();
        assertFalse("client 1 after client 1 removes fid1 and client 2 adds fid5", bounds.equals
                (client1Bounds));
        assertFalse("client 2 after client 1 removes fid1 and client 2 adds fid5", bounds.equals
                (client2Bounds));
        ...
        client1Bounds = roadFromClient1.getFeatures().getBounds();
        client2Bounds = roadFromClient2.getFeatures().getBounds();
        assertTrue("client 1 after commiting removal of fid1 (client 2 has added fid5)", bounds
                .equals(client1Bounds));
        assertFalse("client 2 after client 1 commits removal of fid1 (client 2 has added fid5)", 
        ...
        client1Bounds = roadFromClient1.getFeatures().getBounds();
        client2Bounds = roadFromClient2.getFeatures().getBounds();
        assertTrue("client 1 after commiting addition of fid5 (fid1 previously removed)", bounds
                .equals(client1Bounds));
        assertTrue("client 2 after commiting addition of fid5 (fid1 previously removed)", bounds
                .equals(client2Bounds));
    }

    public void testUseExistingFid() throws Exception {
        SimpleFeatureType ROAD = store.getSchema("road");
        SimpleFeature chrisFeature = SimpleFeatureBuilder.build(ROAD, new Object[]{new Integer(5)
                , "chris"}, "fid5");
