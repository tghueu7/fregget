 ...
 * @since 3.0
 */
public class GrassCoverageReader extends AbstractGridCoverage2DReader implements 
    ...
    public GrassCoverageReader(Object input) {
        if (input instanceof File) {
            ...
            try {
                ...
                originalEnvelope = new GeneralEnvelope(new ReferencedEnvelope(env.getMinX(), env
                        .getMaxX(), env.getMinY(),
                        env.getMaxY(), crs));
                originalGridRange = new GridEnvelope2D(0, 0, fileRegion.getCols(), fileRegion
                        .getRows());
            } catch (Exception e) {
     ...
     *
     * @param cellAnchor               the object defining whether to assume the pixel value to be
     *                                 read in the grid's cell corner or center.
     * @param interpolation            the type of interpolation to be used in the case some scaling
     ...
     * @param castDoubleToFloating     a flag that gives the possibility to force the reading of a
     ...
     * @param monitor                  a {@link IHMProgressMonitor monitor} for logging purposes.
     *                                 This can be null, in which case a dummy one will be used.
     */
    public void setParams(PixelInCell cellAnchor, Interpolation interpolation, boolean 
            useSubSamplingAsColsRows,
     ...
     * <p>
     *
     * @param region the {@link JGrassRegion region}to read.
    ...
    public GridCoverage2D readRaster(JGrassRegion region) throws IOException {
        ...
        if (region != null) {
            ...
            if (!useSubSamplingAsRequestedColsRows) {
                ...
                subSamplingX = (int) Math.floor((double) tmpRegion.getCols() / (double) 
                        requestedCols);
                subSamplingY = (int) Math.floor((double) tmpRegion.getRows() / (double) 
                        requestedRows);
                if (subSamplingX == 0)
                    subSamplingX = 1;
                if (subSamplingY == 0)
                ...
                double sourceCols = (requestedEast - requestedWest) / (1 + (xPaddingSx / (xmax - 
                        xmin))) / requestedXres;
                double sourceRows = (requestedNorth - requestedSouth) / (1 + (yPaddingTop / (ymax
                        - ymin))) / requestedYres;
                /*
        ...
        BufferedImage image = imageReader.read(0, imageReadParam, 
                useSubSamplingAsRequestedColsRows, castDoubleToFloating,
                monitor);
        imageReader.dispose();
        ...
        if (requestedSouth < fileSouth || requestedWest < fileWest || requestedEast > fileEast ||
            ...
            if (requestedRows != imageHeight || requestedCols != imageWidth) {
                finalImage = JGrassUtilities.scaleJAIImage(requestedCols, requestedRows, 
                        translatedImage, interpolation);
            } else {
                finalImage = translatedImage;
            }
        } else if (requestedRows != imageHeight || requestedCols != imageWidth) {
            finalImage = JGrassUtilities.scaleJAIImage(requestedCols, requestedRows, image, 
                    interpolation);
        } else {
            finalImage = image;
        }
    ...
    private RenderedImage setPadding(int xPaddingSx, int xPaddingDx, int yPaddingTop, int 
            yPaddingBottom, RenderedImage image) {
        ImageWorker worker = new ImageWorker(image);
        worker.border(Math.abs(xPaddingSx), Math.abs(0), Math.abs(yPaddingTop), Math.abs(0), new 
                BorderExtenderConstant(new double[]{Double.NaN}));
    ...
    private GridSampleDimension createGridSampleDimension(HashMap<String, String> metaDataTable, 
        ...
        if (colorRulesSplit != null) {
            ...
            Category noData = new Category("novalue", new Color(Color.WHITE.getRed(), Color.WHITE
                    //$NON-NLS-1$
    ...
    // // String mapPath =
    // //
    // "/home/moovida/rcpdevelopment/WORKSPACES/eclipseGanimede/jai_tests/spearfish/PERMANENT
    // /cell/elevation.dem"
    // // ;
    //
    // /*
    ...
    // ("/Users/moovida/Desktop/test.png")); //$NON-NLS-1$
