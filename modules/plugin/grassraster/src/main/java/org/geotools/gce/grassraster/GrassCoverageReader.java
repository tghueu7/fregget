...
public class GrassCoverageReader extends AbstractGridCoverage2DReader
        ...
        originalEnvelope =
            new GeneralEnvelope(
                new ReferencedEnvelope(
                    env.getMinX(), env.getMaxX(), env.getMinY(), env.getMaxY(), crs));
   ...
   * @param monitor a {@link IHMProgressMonitor monitor} for logging purposes. This can be null, in
   *     which case a dummy one will be used.
   */
   ...
   *     <p>
   *
   * @param region the {@link JGrassRegion region}to read. If null, the map is read in its original
  ...
  public GridCoverage2D readRaster(JGrassRegion region) throws IOException {
    ...
    BufferedImage image =
        imageReader.read(
            0, imageReadParam, useSubSamplingAsRequestedColsRows, castDoubleToFloating, monitor);
    imageReader.dispose();
    ...
    if (requestedSouth < fileSouth
      ...
      finalImage =
          JGrassUtilities.scaleJAIImage(requestedCols, requestedRows, image, interpolation);
    } else {
      finalImage = image;
    }
  ...
  public GridCoverage2D read(GeneralParameterValue[] params)
    ...
    if (requestedEnvelope != null && dim != null) {
      ...
      JGrassRegion region =
          new JGrassRegion(
              westSouth[0], eastNorth[0], westSouth[1], eastNorth[1], dim.height, dim.width);
