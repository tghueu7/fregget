...
public class Utilities {
  ...
  /**
   * TODO: Define a contains method which allows to know if the extent of a CoverageSlice contains a
   * predefined extent. This would be useful to know which CoverageSlice to be used to access to a
   * specific temporal/vertical extent defined slice.
   */
  ...
  public static DefaultGeodeticDatum getDefaultGeodeticDatum(
      final String name, final double equatorialRadius, final double inverseFlattening, Unit unit) {
  ...
  public static CoordinateReferenceSystem getMercator2SPProjectedCRS(
    ...
    try {
      ...
      projectedCRS =
          fg.createProjectedCRS(props, sourceCRS, null, params, DefaultCartesianCS.PROJECTED);
  ...
  public static GeographicCRS getBaseCRS(
    ...
    final DefaultGeodeticDatum datum =
        Utilities.getDefaultGeodeticDatum("WGS84", equatorialRadius, inverseFlattening, SI.METER);
   ...
   * @return the value of the requested attribute as a {@code String}. Returns {@code null} in case
   *     of no attribute found.
   */
   ...
   * @return the resulting intersection of envelopes. In case of empty intersection, this method is
   *     allowed to return {@code null}
   * @throws TransformException
   * @throws FactoryException
  ...
  public static GeneralEnvelope getIntersection(
    ...
    try {
      ...
      final Rectangle2D ordinates =
          CRS.transform(readGridToWorld.inverse(), requestedEnvelopeCropped).toRectangle2D();
   ...
   * @param sourceRegion represents the area to load in raster space. This parameter cannot be null
   *     since it gets filled with whatever the crop region is depending on the <code>
  ...
  public static GridCoverage createCoverageFromImage(
    ...
    if (raster2Model != null) {
      ...
      gridCoverage =
          coverageFactory.create(coverageName, image, coverageEnvelope2D, bands, null, null);
  ...
  /**
   * This method creates the GridCoverage2D from the underlying file given a specified envelope, and
   ...
   *     ImageReader}.
   * @param useMultithreading specify if the underlying read process should use multithreading when
   *     a JAI ImageRead operation is requested
  ...
  /**
   * Returns a {@code PlanarImage} given a set of parameter specifying the type of read operation to
   * be performed.
   *
   * @param imageIndex
  ...
  // * In case the first argument is a period and the second one is an instant, check if the instant
  // is
