...
public interface CoverageSource {
  ...
  public abstract class SpatialDomain {
    ...
    /**
     * The first {@link BoundingBox} of this {@link List} should contain the overall bounding for
     * the underlying coverage in its native coordinate reference system. However, by setting the
     * <code>global</code> param to <code>false</code> we can request additional bounding boxes in
     * case the area covered by the mentioned coverage is poorly approximated by a single coverage,
     ...
     * coverage is unrectified this transformation maybe a georeferencing transformation of simply
     * the identity in case we do not have means to georeference the mentioned coverage.
     *
     * @param brief
     * @param listener
     * @return
    ...
    /**
     * The first {@link Rectangle} should describe the overall bidimensional raster range for the
     * underlying coverage. However, by setting the <code>
     * overall</code> param to true we can request additional raster ranges in case the area covered
     * by the mentioned coverage is poorly approximated by a single {@link Rectangle}, like it could
     * happen for a mosaic which has some holes.
     *
     * @param overall
     * @param listener
     * @return
     * @throws IOException
     * @todo should we consider {@link GridEnvelope}?? or RasterLayout which also contains tiling
     *     information??? This has also an impact on the {@link #getOptimalDataBlockSizes()} method,
     *     which may become useless
     */
  ...
  public abstract class TemporalDomain {
    ...
    /**
     * Describes the temporal domain for the underlying {@link RasterDataset} by returning a {@link
     * Set} of {@link DateRange} elements for it. Note that the {@link TemporalCRS} for the listed
     * {@link DateRange} objects can be obtained from the overall {@link CRS} for the underlying
     * coverage.
     *
     * @param listener
  ...
  public abstract class VerticalDomain {
    ...
    /**
     * A {@link Set} of {@link Envelope} element for the underlying coverage. Note that the {@link
     * CRS} for such envelope can be <code>null</code> in case the overall spatial {@link CRS} is a
     * non-separable 3D {@link CRS} like WGS84-3D. Otherwise, all the envelopes should share the
     * same {@link VerticalCRS}. Finally, note that the envelope should be 1-dimensional. In case of
     * single vertical value, the lower coordinate should match the upper coordinate while lower and
     * upper coordinates may be different to define vertical intervals.
     *
     * @param overall
     * @param listener
     * @return
   ...
   *   <li>schema - used to identify the type of resource; usually the format or data product being
   *       represented
   *       <ul>
   *
   ...
   * </code></pre>
   *
   * @return a {@link EnumSet} of CoverageCapabilities which can be used to discover capabilities of
   *     this {@link CoverageSource}.
   */
