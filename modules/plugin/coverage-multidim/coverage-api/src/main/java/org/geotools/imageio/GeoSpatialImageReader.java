...
public abstract class GeoSpatialImageReader extends ImageReader implements FileSetManager {
   ...
   * numRasters]. In case this constraint is not respected, an {@link IndexOutOfBoundsException} is
   * thrown.
   *
   * @param imageIndex the index to be checked
   * @throw {@link IndexOutOfBoundsException} in case the provided imageIndex is not in the range of
   *     supported ones.
   */
  ...
  /**
   * Return the list of imageIndex related to the feature in the slicesCatalog which result from the
   * specified query.
   *
