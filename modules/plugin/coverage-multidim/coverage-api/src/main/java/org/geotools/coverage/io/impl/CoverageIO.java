...
public class CoverageIO {
  ...
  public static boolean canConnect(java.util.Map<String, Serializable> params) {
    for (Driver driver : getAvailableDrivers()) {
      ...
      try {
        ...
        LOGGER.log(
            Level.WARNING, "Error asking " + driver.getTitle() + " if it can process request", t);
      ...
      if (canProcess) {
        ...
        try {
          ...
          LOGGER.log(
              Level.WARNING, "Error when checking if " + driver.getTitle() + " is available", t);
  ...
  /**
   * Finds all available implementations of {@link Driver} which have registered using the services
   * mechanism, and that have the appropriate libraries on the class-path.
   *
   * @return An unmodifiable {@link Set} of all discovered drivers which have registered factories,
   ...
   * re-scan. Thus this method need only be invoked by sophisticated applications which dynamically
   ...
   * plugins. Note that this method finds all the implemented plugins but returns only the available
   * one.
   *
   ...
   * @return an unmodifiable {@link Set} comprising all the {@link Driver} that can read the {@link
   *     URL} url.
   */
  ...
  /**
   * Returns a {@link Driver} that is able to read a certain object. If no {@link Driver} is able to
   * read such an {@link Object} we return an null object.
   *
