...
public class CoverageSlicesCatalog {
  ...
  /**
   * CoverageSlicesCatalog always used an hidden H2 DB to store granules index related to a specific
   * file.
   *
  ...
  public static class WrappedCoverageSlicesCatalog extends CoverageSlicesCatalog {
    ...
    public WrappedCoverageSlicesCatalog(
        DataStoreConfiguration config, File file, Repository repository) throws IOException {
  ...
  public CoverageSlicesCatalog(DataStoreConfiguration datastoreConfig, Repository repository) {
    ...
    try {
      ...
      if (isPostgis && wrapDatastore) {
        slicesIndexStore =
            new PostgisDatastoreWrapper(slicesIndexStore, parentLocation, HIDDEN_FOLDER);
      }
