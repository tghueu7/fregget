 ...
 * HREF="http://www.cfconventions.org/">CF Metadata conventions</A>), i.e. in (<var>t</var>,
 * <var>z</var>,<var>y</var>,<var>x</var>)
...
public class NetCDFImageReader extends GeoSpatialImageReader implements FileSetManager {
    ...
    /**
     * States whether any underlying time dimension should be indexed by "time" (instead of 
     * time1, time2, ...)
     ...
     **/
    private final SoftValueHashMap<String, VariableAdapter> coverageSourceDescriptorsCache = new 
            SoftValueHashMap<String, VariableAdapter>();
    ...
    protected int initIndex() throws InvalidRangeException, IOException {
        DefaultTransaction transaction = new DefaultTransaction("indexTransaction" + System
                .nanoTime());
        int numImages = 0;
        try {

            // init slice catalog
            DataStoreConfiguration datastoreConfig = ancillaryFileManager
            ...
            if (variables != null) {
                ...
                for (final Variable var_ : variables) {
                    if (var_ != null && var_ instanceof VariableDS) {
                        ...
                        final CoordinateSystem cs = NetCDFCRSUtilities.getCoordinateSystem
                                (variable);
                        ...
                        if (indexSchema == null) {
                            throw new IllegalStateException("Unable to created index schema for " +
                        ...
                        while (processedSlices < numberOfSlices) {
                            ...
                            processedSlices += vaAdapter.getFeatures(startPagingIndex, limit, 
                                    collection);
                            ...
                            if (features > 0) {
                                // adding granules to the catalog and updating the number of 
                                // written features
                                CoverageSlicesCatalog catalog = getCatalog();
                                if (catalog != null) {
                                    catalog.addGranules(indexSchema.getTypeName(), collection, 
                                            transaction);
                                }
                                collection.clear();
    ...
    private void updateFeaturesIndex(final ListFeatureCollection collection, final int offset, 
                                     boolean isShared) throws IOException {
    ...
    private void init() throws IOException {
        ...
        try {
            if (dataset != null) {
                ...
                uniqueTimeAttribute = ancillaryFileManager.getParameterAsBoolean(NetCDFUtilities
                ...
                if (slicesIndexFile != null) {
                    ...
                    if (slicesIndexFile.exists()) {
                        ...
                        if (!ignoreMetadata) {
                            ...
                            DataStoreConfiguration datastoreConfiguration = ancillaryFileManager
                                    .getDatastoreConfiguration();
    ...
    protected IIOException netcdfFailure(final Exception e) throws IOException {
        return new IIOException(new StringBuilder("Can't read file ").append(dataset.getLocation
                ()).toString(), e);
    }
    ...
    public VariableAdapter getCoverageDescriptor(Name name) {
        ...
        synchronized (coverageSourceDescriptorsCache) {
            ...
            VariableAdapter cd;
            try {
                String origName = ancillaryFileManager.variablesMap != null ? 
                        ancillaryFileManager.variablesMap.get(name) : null;
                if (origName == null) {
                    origName = name.getLocalPart();
                }
//                else {
//                    throw new IllegalArgumentException("Unable to locate descriptor for 
// Coverage: "+name);
//                }
    ...
    public BufferedImage read(int imageIndex, ImageReadParam param) throws IOException {
        ...
        MultipleBandsDimensionInfo multipleBands = ancillaryFileManager
        ...
        if (multipleBands != null) {
            ...
            if (bands != null) {
                ...
                if (maxSourceBand > numDstBands) {
                    throw new IllegalArgumentException("The provided source bands parameter is " +
                            "invalid.");
                }
        ...
        SampleModel sampleModel = new BandedSampleModel(wrapper.getSampleModel().getDataType(), 
                destWidth, destHeight, numDstBands);
        ...
        final BufferedImage image = new BufferedImage(colorModel, raster, colorModel
            ...
            if (flipYAxis) {
                ...
                switch (type) {
                    case DataBuffer.TYPE_DOUBLE: {
                        ...
                        raster.setSamples(xmin, ymin, destRegion.width, destRegion.height, 
                                dstBand, samples);
                        break;
                    }
                    case DataBuffer.TYPE_FLOAT:
    ...
    private synchronized Array readSection(VariableAdapter wrapper, Section section) throws 
    ...
    protected static void flipVertically(final ImageReadParam param, final int srcHeight,
         ...
         * a downward translation. We usually don't need to care
         * about if, except if the source region is very close to the bottom of the source image,
          * in which case the correction computed below may be
         * greater than the space left.
         *
         * We are done if there is no vertical subsampling. But if there is subsampling, then we 
         * need an adjustment. The flipping performed above must
         * be computed as if the source region had exactly the size needed for reading nothing 
         * more than the last line, i.e. 'srcRegion.height' must
         * be a multiple of 'sourceYSubsampling' plus 1. The "offset" correction is computed 
         * below accordingly.
         */
        if (param != null) {
    ...
    public SimpleFeatureType getIndexSchema(Name coverageName, CoordinateSystem cs) throws 
            Exception {
        return getIndexSchema(coverageName, cs, false);
    }


    public SimpleFeatureType getIndexSchema(Name coverageName, CoordinateSystem coordinateSystem,
        ...
        final SimpleFeatureType indexSchema = NetCDFUtilities.createFeatureType(coverage.getName
                (), schemaDef, crs);
     ...
     */
    String suggestSchemaFromCoordinateSystem(Coverage coverage, CoordinateSystem cs, boolean 
            isShared) throws SchemaException {

        // init with base
        String schemaAttributes = isShared ? CoverageSlice.Attributes.BASE_SCHEMA_LOCATION : 
    ...
    public IIOMetadata getImageMetadata(int imageIndex) throws IOException {
        ...
        SampleModel sampleModel = new BandedSampleModel(wrapper.getSampleModel().getDataType(), 
                width, height, wrapper.getNumBands());
        final ColorModel colorModel = ImageIOUtilities.createColorModel(sampleModel);
        NetCDFImageMetadata metadata = new NetCDFImageMetadata(variableName, sampleModel, 
                colorModel, crs);
