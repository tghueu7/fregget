...
class NetCDFFileResourceInfo extends DefaultResourceInfo implements FileResourceInfo {
   ...
   * file (records will be different in terms of time value, elevation value, and so on. Therefore,
   * we need to aggregate features related to the same file location by scanning the underlying
  ...
  class WrappedCoverageSlicesToFileGroupIterator extends SimpleCoverageSlicesToFileGroupIterator {
    ...
    public FileGroup next() {
      ...
      while (slicesIterator.hasNext()) {
        ...
        relevantSlices.add(next);
        String nextLocation =
            (String) next.getOriginator().getAttribute(CoverageSlice.Attributes.LOCATION);
    ...
    /**
     * Aggregate multipleFeatures related to the same file, on the same {@link FileGroup}. This is
     * usually needed when the underlying coverage isn't a simple 2D coverage but it has multiple
     * dimensions (as an instance, time, elevation, custom...)
     *
