...
public class NetCDFCRSUtilities {
   ...
   * is a temporary patch (will be removed when the final API for JSR-108: Units specification will
   * be available).
   */
      ...
      if (zAxis != null) {
        ...
        if (axisType == AxisType.RadialAzimuth
          ...
          if (!zAxis.getShortName().equalsIgnoreCase("height")) {
            v_datumType = "depth";
            v_crsName =
                new Identification("mean sea level depth", null, null, "EPSG:5715").getName();
          } else {
            v_datumType = "geoidal";
            v_crsName =
                new Identification("mean sea level height", null, null, "EPSG:5714").getName();
          }
        ...
        VerticalCS verticalCS =
            NetCDFCRSUtilities.FACTORY_CONTAINER
                .getCSFactory()
                .createVerticalCS(
                    csMap, getAxis(zAxis.getShortName(), getDirection(direction), units));
