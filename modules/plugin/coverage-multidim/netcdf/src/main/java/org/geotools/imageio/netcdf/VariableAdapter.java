...
public class VariableAdapter extends CoverageSourceDescriptor {
    ...
    for (Name key : keys) {
      ...
      if (origName.equalsIgnoreCase(varName)) {
        ...
        if (schema != null) {
          ...
          if (catalog != null) {
            // Current assumption is that we have a typeName for each coverage but we should keep on
            // working
            // with shared schemas
  ...
  private void initAdditionalDomain(CoordinateVariable<?> cv, List<DimensionDescriptor> dimensions)
    ...
    if (cv.getType().equals(Date.class)) {
      ...
      throw new IllegalArgumentException(
          "Unable to init domain from CoordinateVariable of type: " + cv.getType().getName());
    }
  ...
  private void initRange() {
    ...
    height =
        variableDS.getDimension(variableDS.getRank() - NetCDFUtilities.Y_DIMENSION).getLength();

    // computing the number of bands, according to COARDS convention ignored dimension are at the
    // beginning
    ...
    sampleModel =
        new BandedSampleModel(bufferType, width, height, multipleBands == null ? 1 : numBands);
    ...
    if (noData != null) {
      ...
      noDataCategory =
          new Category(Category.NODATA.getName(), new Color[] {new Color(0, 0, 0, 0)}, noDataRange);
  ...
  public Map<String, Integer> mapIndex(int[] splittedIndex) {
    ...
    for (int n = 0; n < splittedIndex.length; n++) {
      if (nDimensionIndex[n] != -1) {
        resultIndex.put(
            variableDS.getDimension(nDimensionIndex[n]).getFullName(), splittedIndex[n]);
      }
    }
    return resultIndex;
  }
  ...
  /**
   * Return features for that variable adapter, starting from slices with index = "startIndex", and
   * up to "limit" elements. This allows for paging. Put the created features inside the provided
   * collection
   *
  ...
  private SimpleFeature createFeature(
    ...
    if (getAdditionalDomains() != null) {
      for (int i = 0; i < getAdditionalDomains().size(); i++) {
        ...
        feature.setAttribute(
            reader.georeferencing.getDimensionMapper().getDimension(domain.getName().toUpperCase()),
            value);
      }
    }
  ...
  private <T> T getValueByIndex(int dimensionIndex, final Map<String, Integer> mappedIndex) {
    ...
    return (T)
        reader.georeferencing.getCoordinateVariable(dimension.getFullName()).read(mappedIndex);
  }
