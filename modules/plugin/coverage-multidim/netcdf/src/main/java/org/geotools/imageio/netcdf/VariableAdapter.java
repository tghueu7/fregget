...
public class VariableAdapter extends CoverageSourceDescriptor {
    ...
    public class UnidataTemporalDomain extends TemporalDomain {
        ...
        UnidataTemporalDomain(CoordinateVariable<?> adaptee) {
            if (!Date.class.isAssignableFrom(adaptee.getType())) {
                throw new IllegalArgumentException("Unable to wrap non temporal " +
                        "CoordinateVariable:" + adaptee.toString());
            }
        ...
        public SortedSet<? extends DateRange> getTemporalElements(boolean overall,
            ...
            if (overall) {
                ...
                final SortedSet<DateRange> extent = new TreeSet<DateRange>(new 
                        DateRangeComparator());
    ...
    public class UnidataVerticalDomain extends VerticalDomain {
        ...
        UnidataVerticalDomain(CoordinateVariable<?> cv) {
            if (!Number.class.isAssignableFrom(cv.getType())) {
                throw new IllegalArgumentException("Unable to wrap a non Number " +
                        "CoordinateVariable:" + cv.toString());
            }
    ...
    public class UnidataAdditionalDomain extends AdditionalDomain {
        ...
        private final Set<Object> globalDomainExtent = new TreeSet<Object>(new Comparator<Object>
            ...
            public int compare(Object o1, Object o2) {
                ...
                throw new ClassCastException("Incompatible range types: " + o1.getClass() + " is " +
                        "not the same as " + o2.getClass());
            }
        ...
        UnidataAdditionalDomain(CoordinateVariable<?> adaptee) throws IOException {
            ...
            if (Date.class.isAssignableFrom(type)) {
                ...
                throw new UnsupportedOperationException("Unsupported CoordinateVariable:" + 
                        adaptee.toString());
            }
    ...
    /**
     * Usual schema are the_geom, imageIndex, so the first attribute (time or elevation) will 
     * have index = 2
     */
     ...
     * In case the indexer.xml contains an explicit schema with different attributes for time and
     * elevation
     ...
     * @throws IOException
     */
    private void updateDimensions(List<DimensionDescriptor> dimensionDescriptors) throws 
            IOException {
        ...
        for (Name key : keys) {
            ...
            if (origName.equalsIgnoreCase(varName)) {
                ...
                final Coverage coverage = reader.ancillaryFileManager.coveragesMapping.get
                        (coverageName);
                ...
                if (schema != null) {
                    ...
                    if (catalog != null) {
                        ...
                        throw new IllegalStateException("Unable to find the table for this " +
    ...
    /**
     * Update the dimensionDescriptor attributes mapping by checking the actual attribute names 
     * from the schema
     *
     * @param indexSchema
     * @param descriptors
    ...
    public void updateMapping(SimpleFeatureType indexSchema, List<DimensionDescriptor> descriptors)
        ...
        if (dimensionNames == null || dimensionNames.isEmpty() || descriptors == null
                || descriptors.isEmpty() || indexSchema.getAttributeCount() <= 
    ...
    private boolean remapAttribute(final SimpleFeatureType indexSchema, final String currentDimName,
                                   final int indexAttribute, final List<DimensionDescriptor> 
                                           descriptors, DimensionMapper mapper) {
     ...
     */
    private void initCRS(List<DimensionDescriptor> dimensions) throws IllegalArgumentException, 
            RuntimeException,
         ...
         * dimensions in the rendered image. Typical NetCDF convention is
         * to put axis in the (time, depth, latitude, longitude) order, which typically maps to 
         * (longitude, latitude, depth, time) order in GeoTools
        ...
        for (CoordinateAxis axis : axesSorted) {
            ...
            CoordinateVariable<?> cv = reader.georeferencing.getCoordinateVariable(axis
                    .getShortName());
            if (cv == null) {
     ...
     * @throws IOException
     */
    private void initVerticalDomain(CoordinateVariable<?> cv, List<DimensionDescriptor> 
            dimensions) throws IOException {
        ...
        dimensions.add(new DefaultDimensionDescriptor(Utils.ELEVATION_DOMAIN,
                cv.getUnit(), CoverageUtilities.UCUM.ELEVATION_UNITS.getSymbol(), cv.getName(), 
                null));
    }
     ...
     * @throws IOException
     */
    private void initTemporalDomain(CoordinateVariable<?> cv, List<DimensionDescriptor> 
            dimensions) throws IOException {
        if (!cv.getType().equals(Date.class)) {
            throw new IllegalArgumentException("Unable to init temporal domain from " +
                    "CoordinateVariable that does not bind to Date");
        }
        ...
        dimensions.add(new DefaultDimensionDescriptor(Utils.TIME_DOMAIN,
                CoverageUtilities.UCUM.TIME_UNITS.getName(), CoverageUtilities.UCUM.TIME_UNITS
                .getSymbol(), timeAttribute, null));
    }
     ...
     * @throws IOException
     */
    private void initAdditionalDomain(CoordinateVariable<?> cv, List<DimensionDescriptor> 
            dimensions) throws IOException {
        ...
        if (cv.getType().equals(Date.class)) {
            ...
            dimensions.add(new DefaultDimensionDescriptor(cv.getName(),
                    CoverageUtilities.UCUM.TIME_UNITS.getName(), CoverageUtilities.UCUM
                    .TIME_UNITS.getSymbol(), cv.getName(), null));
        } else if (Number.class.isAssignableFrom(cv.getType())) {
            // TODO: Parse Units from axis and map them to UCUM units
            dimensions.add(new DefaultDimensionDescriptor(cv.getName(), cv.getUnit(), cv.getUnit
                    (), cv.getName(), null));
        } else {
            throw new IllegalArgumentException("Unable to init domain from CoordinateVariable of " +
                    "type: " + cv.getType().getName());
        }
    ...
    private void initRange() {
        ...
        width = variableDS.getDimension(variableDS.getRank() - NetCDFUtilities.X_DIMENSION)
                .getLength();
        height = variableDS.getDimension(variableDS.getRank() - NetCDFUtilities.Y_DIMENSION)
                .getLength();
        ...
        MultipleBandsDimensionInfo multipleBands = reader.ancillaryFileManager
                .getMultipleBandsDimensionInfo(candidateDimension);
        ...
        sampleModel = new BandedSampleModel(bufferType, width, height, multipleBands == null ? 1 
                : numBands);
        ...
        if (noData != null) {
            NumberRange noDataRange = NumberRange.create(noData.doubleValue(), true, noData
                    .doubleValue(), true);
            noDataCategory = new Category(Category.NODATA.getName(), new Color[]{new Color(0, 0, 
                    0, 0)}, noDataRange);
    ...
    protected GridGeometry2D getGridGeometry() throws IOException {
        ...
        for (CoordinateVariable<?> cv : reader.georeferencing.getCoordinatesVariables(variableDS
                .getShortName())) {
            if (!cv.isNumeric()) {
                continue;
            }
                ...
                case GeoX:
                    ...
                    if (cv.isRegular()) {
                        ...
                        if (!Double.isNaN(min) && !Double.isNaN(max)) {
                            ...
                            if (LOGGER.isLoggable(Level.FINE)) {
                                LOGGER.log(Level.FINE, "Axis values contains NaN; finding first " +
                                        "valid values");
                            }
                ...
                case GeoY:
                    ...
                    if (cv.isRegular()) {
                        ...
                        if (cv.getIncrement() > 0) {
                            // the latitude axis is increasing! This is a special case so we flip
                            // it around
    ...
    public int getNIndex(int n, int imageIndex) {
        if (n < nDimensionIndex.length && nDimensionIndex[n] >= 0) {
            ...
            return imageIndex % (NetCDFUtilities.getDimensionLength(variableDS, 
                    nDimensionIndex[n]) * factor)
                    / factor;
        }
        return -1;
    }
    ...
    public Map<String, Integer> mapIndex(int[] splittedIndex) {
        ...
        for (int n = 0; n < splittedIndex.length; n++) {
            if (nDimensionIndex[n] != -1) {
                resultIndex.put(variableDS.getDimension(nDimensionIndex[n]).getFullName(), 
                        splittedIndex[n]);
            }
        }
        return resultIndex;
     ...
     */
    public int getFeatures(final int startIndex, final int limit, final ListFeatureCollection 
            collection) {
        final SimpleFeatureType indexSchema = collection.getSchema();
        final int slicesNum = getNumberOfSlices();
        if (startIndex > slicesNum) {
            throw new IllegalArgumentException("The paging start index can't be higher than the " +
    ...
    private SimpleFeature createFeature(
        ...
        feature.setAttribute(CoverageSlice.Attributes.GEOMETRY,
                NetCDFCRSUtilities.GEOM_FACTORY.toGeometry(reader.georeferencing.getBoundingBox
                        (variableDS.getShortName())));
        ...
        if (nDimensionIndex[Z] >= 0) {
            ...
            feature.setAttribute(reader.georeferencing.getDimensionMapper().getDimension
                            (NetCDFUtilities.ELEVATION_DIM),
                    verticalValue);
        }
        ...
        if (getAdditionalDomains() != null) {
            for (int i = 0; i < getAdditionalDomains().size(); i++) {
                ...
                feature.setAttribute(reader.georeferencing.getDimensionMapper().getDimension
                        (domain.getName().toUpperCase()), value);
            }
        }
    ...
    private <T> T getValueByIndex(int dimensionIndex, final Map<String, Integer> mappedIndex) {
        ...
        return (T) reader.georeferencing.getCoordinateVariable(dimension.getFullName()).read
                (mappedIndex);
    }

    /**
     * Wrapper class used for setting the OSEQD dimension to Vertical, even if the 
     * {@link CoordinateSystem} does not handle it.
     *
