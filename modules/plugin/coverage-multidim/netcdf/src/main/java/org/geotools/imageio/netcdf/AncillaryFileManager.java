...
public class AncillaryFileManager implements FileSetManager {
  ...
  enum AuxiliaryFileType {
    INDEXER_XML {
      File lookup(String baseName, File parentDirectory, File destinationDirectory) {
        ...
        // Compose the path to an optional XML auxiliary file in the same directory of the input
        // file
        // (filename.xml)
  ...
  // contains information about dimensions that will produce multiple bands indexed by the dimension
  // name
  ...
  /**
   * Use different approaches to look for the specified file since it can be provided externally (as
   * from the imageMosaic sharing the same indexer between multiple NetCDF files), it can be
        ...
        for (Collector collector : collectorList) {
          ...
          if (selectedSPI == null) {
            if (LOGGER.isLoggable(Level.INFO)) {
              LOGGER.info(
                  "Unable to find a PropertyCollector for this INTERNAL_STORE_SPI: " + spiName);
            }
            continue;
          }
  ...
  /**
   * Create the {@link DataStoreConfiguration} using the external datastoreIndexFile if provided, or
   * the H2 based default.
   *
   * @return
   * @throws IOException
   */
  public DataStoreConfiguration getDatastoreConfiguration() throws IOException {
    ...
    if (datastoreIndexFile != null) {
      ...
      String database =
          FilenameUtils.removeExtension(FilenameUtils.getName(slicesIndexFile.getCanonicalPath()))
              .replace(".", "");
