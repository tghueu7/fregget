...
public class AncillaryFileManager implements FileSetManager {
    ...
    enum AuxiliaryFileType {
        INDEXER_XML {
            File lookup(String baseName, File parentDirectory,
                ...
                // Compose the path to an optional XML auxiliary file in the same directory of 
                // the input file
                // (filename.xml)
                ...
                if (!file.exists() || !file.canRead()) {
                    ...
                    // Compose the path to an optional XML auxiliary file inside a directory of 
                    // with the same
        ...
        INDEXER_DATASTORE {
            File lookup(String baseName, File parentDirectory, File destinationDirectory) {
                ...
                // CASE 1: side file (for backward compatibility)
                // Compose the path to an optional datastore file in the same directory of the 
                // input file
                String optionalAuxiliaryDatastorePath = parentDirectory.getAbsolutePath() + File
                        .separator
                ...
                if (!file.exists() || !file.canRead()) {
                    ...
                    optionalAuxiliaryDatastorePath = parentDirectory.getAbsolutePath() + File
                            .separator
    ...
    private static final Set<PropertiesCollectorSPI> pcSPIs = PropertiesCollectorFinder
    ...
    // dimension name
    private final Map<String, MultipleBandsDimensionInfo> multipleBandsDimensionsInfo = new 
            HashMap<>();
    ...
    public AncillaryFileManager(final File netcdfFile, final String indexFilePath) throws 
            IOException, JAXBException, NoSuchAlgorithmException {
        this(netcdfFile, indexFilePath, null);
    }

    public AncillaryFileManager(final File netcdfFile, final String indexFilePath, final String 
            datastoreFilePath) throws IOException, JAXBException, NoSuchAlgorithmException {
        ...
        String baseName = cutExtension(extension) ? FilenameUtils.removeExtension(mainName) : 
                mainName;
        ...
        // init
        initIndexer();
        datastoreIndexFile = lookupFile(datastoreFilePath, baseName, AuxiliaryFileType
                .INDEXER_DATASTORE);
    }
     ...
     * @throws IOException
     */
    private static void checkReset(final File mainFile, final File slicesIndexFile, final File 
        ...
        if (slicesIndexFile.exists()) {
            ...
            if (mainFileTime > indexTime) {
                ...
                final Collection<File> listedFiles = FileUtils.listFiles(destinationDir, null, 
                        true);
                for (File file : listedFiles) {

                    // Preserve summary file which contains mapping between coverages and 
     ...
     *
     * @throws IOException
     * @throws JAXBException TODO: Need to check for thread safety
     */
    ...
    private void initCoverages(Map<String, String> schemaMapping) {
        ...
        if (coverages != null) {
            ...
            for (Coverage coverageElement : coverageElements) {
                ...
                final Coverage coverage = createCoverate(coverageName, origName, 
                        schemaAttributes, schemaName);
                addCoverage(coverage);
            }
        }
    }
                ...
                for (Collector collector : collectorList) {
                    ...
                    if (selectedSPI == null) {
                        if (LOGGER.isLoggable(Level.INFO)) {
                            LOGGER.info("Unable to find a PropertyCollector for this " +
                                    "INTERNAL_STORE_SPI: " + spiName);
                        }
                        continue;
                    }

                    // property names
                    final String propertyNames[] = new String[]{mapped != null ? mapped : 
                            COVERAGE_NAME};

                    // create the PropertiesCollector
                    final PropertiesCollector pc = selectedSPI.create
                            (DefaultPropertiesCollectorSPI.REGEX_PREFIX + value, Arrays.asList
                                    (propertyNames));
                    if (pc != null) {
            ...
            if (properties != null) {
                ...
                if (storeName != null) {
                    ...
                    try {
                        ...
                        final DataStoreFactorySpi spi = (DataStoreFactorySpi) Class.forName
                                (SPIClass)
                                .newInstance();
     ...
     * @throws IOException
     */
    private void checkStoreWrapping(DataStoreConfiguration datastoreConfiguration) throws 
            IOException {
