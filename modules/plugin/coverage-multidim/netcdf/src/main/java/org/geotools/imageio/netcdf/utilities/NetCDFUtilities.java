...
public class NetCDFUtilities {
  ...
  /**
   * Number of bytes at the start of a file to search for a GRIB signature. Some GRIB files have WMO
   * headers prepended by a telecommunications gateway. NetCDF-Java Grib{1,2}RecordScanner look for
   * the header in this many bytes.
   */
  ...
  /**
   * The dimension <strong>relative to the rank</strong> in {@link #variable} to use as image width.
   * The actual dimension is {@code variable.getRank() - X_DIMENSION}. Is hard-coded because the
   * loop in the {@code read} method expects this order.
   */
  public static final int X_DIMENSION = 1;

  /**
   * The dimension <strong>relative to the rank</strong> in {@link #variable} to use as image
   * height. The actual dimension is {@code variable.getRank() - Y_DIMENSION}. Is hard-coded because
   * the loop in the {@code read} method expects this order.
   */
  ...
  /**
   * NetCDF files may contains a wide set of coverageDescriptorsCache. Some of them are unuseful for
   * our purposes. The method returns {@code true} if the specified variable is accepted.
   */
  ...
  /**
   * NetCDF files may contain a wide set of coverageDescriptorsCache. Some of them are unuseful for
   * our purposes. The method returns {@code true} if the specified variable is accepted.
   */
   ...
   * </ul>
   *
   * <p>The {@linkplain Locale#CANADA Canada locale} is used by default for most formats because it
   ...
   * coverageDescriptorsCache rather than some others. The method returns the type of check on which
   * we need to leverage to restrict the set of interesting coverageDescriptorsCache. The method
   * will check for some KEY/FLAGS/ATTRIBUTES within the input dataset in order to define the proper
   * check type to be performed.
   *
  ...
  public static SimpleFeatureType createFeatureType(
    ...
    try {
      ...
      indexSchema =
          DataUtilities.createSubType(indexSchema, DataUtilities.attributeNames(indexSchema), crs);
    } catch (Throwable e) {
  ...
  /**
   * Default parameter behavior properties TODO: better way of handling configuration settings, such
   * as read parameters.
   */
