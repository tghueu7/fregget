...
public class NetCDFProjection {
  ...
  /**
   * Subclasses override this if they wish to adjust OGC parameters after they are read from NetCDF.
   * This is the inverse of {@link #getNetcdfParameters(ParameterValueGroup)}.
   *
  ...
  static {
    ...
    LAMBERT_AZIMUTHAL_EQUAL_AREA =
        new NetCDFProjection(
            CF.LAMBERT_AZIMUTHAL_EQUAL_AREA, CF.LAMBERT_AZIMUTHAL_EQUAL_AREA, lazeq_mapping);
    ...
    POLAR_STEREOGRAPHIC =
        new NetCDFProjection(CF.POLAR_STEREOGRAPHIC, CF.POLAR_STEREOGRAPHIC, polarstereo_mapping);
    ...
    LAMBERT_CONFORMAL_CONIC_1SP =
        new NetCDFProjection(
            CF.LAMBERT_CONFORMAL_CONIC, CF.LAMBERT_CONFORMAL_CONIC + "_1SP", lcc_1sp_mapping);
    ...
    LAMBERT_CONFORMAL_CONIC_2SP =
        new NetCDFProjection(
            CF.LAMBERT_CONFORMAL_CONIC, CF.LAMBERT_CONFORMAL_CONIC + "_2SP", lcc_2sp_mapping);
    ...
    ROTATED_POLE =
        new NetCDFProjection(CF.ROTATED_LATITUDE_LONGITUDE, "Rotated_Pole", rotated_pole_mapping) {
          ...
          public ParameterValueGroup getOgcParameters(ParameterValueGroup netcdfParameters) {
            double lonNorthPole =
                (Double) netcdfParameters.parameter(NetCDFUtilities.CENTRAL_MERIDIAN).getValue();
            double latNorthPole =
                (Double) netcdfParameters.parameter(NetCDFUtilities.LATITUDE_OF_ORIGIN).getValue();
          ...
          public ParameterValueGroup getNetcdfParameters(ParameterValueGroup ogcParameters) {
            double lonOrigin =
                (Double) ogcParameters.parameter(NetCDFUtilities.CENTRAL_MERIDIAN).getValue();
            double latOrigin =
                (Double) ogcParameters.parameter(NetCDFUtilities.LATITUDE_OF_ORIGIN).getValue();
  ...
  /**
   * Get the NetCDF Attribute related to the specified OGC parameter and set the proper value within
   * the OGC parameters map.
   *
  ...
  /**
   * Adjust the mappingName if needed. This may happen for some projections where different standard
   * parallels may require _1SP or _2SP suffix.
   *
  ...
  private static String getProjectionName(String mappingName, Variable var) {
    ...
    if (mappingName.equalsIgnoreCase(CF.LAMBERT_CONFORMAL_CONIC)) {
      ...
      projectionName =
          CF.LAMBERT_CONFORMAL_CONIC + (standardParallel.getLength() == 1 ? "_1SP" : "_2SP");
  ...
  public static CoordinateReferenceSystem lookForDatasetCRS(NetcdfDataset dataset) {
    ...
    if (projection != null) {
      if (LOGGER.isLoggable(Level.FINE)) {
        LOGGER.fine(
            "Detected NetCDFProjection through spatial_ref attribute: " + projection.toWKT());
      }
    }
    return projection;
  }
