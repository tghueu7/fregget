...
public final class NetCDFStationsTest extends Assert {
   ...
   * bands should match the number of expected banks in the provided order. This testes are run for
   * the NetCDF reader and the ImageMosaicReader.
   */
  ...
  private void checkRasterData(Raster data, int[]... expected) {
    ...
    // we should have a number of bands equal to the number of banks abd equal to number of expected
    // results
  ...
  /**
   * Helper method that reads stationA data set using the NetCdf reader directly. Some basic checks
   * are also made.
   */
  private Raster readCoverageUsingNetCdfReader(GeneralParameterValue[] readParameters)
    ...
    try {
      ...
      // checking that we have four coverages (only the first station has values others stations
      // values are zero)
