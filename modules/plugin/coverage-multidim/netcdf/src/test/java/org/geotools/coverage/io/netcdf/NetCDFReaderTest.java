...
public class NetCDFReaderTest extends Assert {
  ...
  public void testScaleAndOffset() throws IOException, FactoryException, ParseException {
    ...
    float[] result =
        coverage.evaluate(
            (DirectPosition) new DirectPosition2D(DefaultGeographicCRS.WGS84, 5.0, 45.0),
            new float[1]);
    ...
    try {
      ...
      for (String coverageName : names) {
        ...
        assertEquals(
            ...
            reader.getMetadataValue(coverageName, "TIME_DOMAIN_MAXIMUM"));
        assertEquals(
            "java.util.Date", reader.getMetadataValue(coverageName, "TIME_DOMAIN_DATATYPE"));
  ...
  public void NetCDFTestOnClimatologicalTime()
    ...
    try {
      ...
      for (String coverageName : names) {
        ...
        assertNotNull(elevationMetadata);
        assertEquals(
            "0.0/0.0,10.0/10.0,20.0/20.0,30.0/30.0,50.0/50.0,75.0/75.0", elevationMetadata);
  ...
  /**
   * Test that {@link GridCoverage2DReader#SOURCE_URL_PROPERTY} is correctly set on a coverage read
   * from a NetCDF file.
   */
  @Test
