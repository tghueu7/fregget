...
public class NetCDFReaderTest extends Assert {
    ...
    public void testScaleAndOffset() throws IOException, FactoryException, ParseException {
        ...
        float[] result = coverage
                .evaluate((DirectPosition) new DirectPosition2D(DefaultGeographicCRS.WGS84, 5.0, 
                                45.0),
                        new float[1]);
    ...
    @Test
    public void NetCDFTestOn4Dcoverages() throws NoSuchAuthorityCodeException, FactoryException, 
        ...
        final Hints hints = new Hints(Hints.DEFAULT_COORDINATE_REFERENCE_SYSTEM, CRS.decode
                ("EPSG:4326", true));
        // Get format
        final AbstractGridFormat format = (AbstractGridFormat) GridFormatFinder.findFormat(file
                .toURI().toURL(), hints);
        ...
        try {
            ...
            for (String coverageName : names) {
                ...
                assertEquals("2012-04-01T00:00:00.000Z", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_MINIMUM"));
                assertEquals("2012-04-01T01:00:00.000Z", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_MAXIMUM"));

                assertEquals("true", reader.getMetadataValue(coverageName, "HAS_ELEVATION_DOMAIN"));
                final String elevationMetadata = reader.getMetadataValue(coverageName, 
                        "ELEVATION_DOMAIN");
                assertNotNull(elevationMetadata);
                assertEquals("10.0/10.0,450.0/450.0", elevationMetadata);
                assertEquals(2, elevationMetadata.split(",").length);
                assertEquals("10.0", reader.getMetadataValue(coverageName, 
                        "ELEVATION_DOMAIN_MINIMUM"));
                assertEquals("450.0", reader.getMetadataValue(coverageName, 
                ...
                assertEquals(CoverageUtilities.UCUM.TIME_UNITS.getSymbol(), descriptor
                        .getUnitSymbol());
                ...
                assertEquals(CoverageUtilities.UCUM.ELEVATION_UNITS.getSymbol(), descriptor
                        .getUnitSymbol());
                ...
                final SimpleDateFormat formatD = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss" +
                        ".SSS'Z'");
    ...
    @Test
    public void NetCDFTestOnFilter() throws NoSuchAuthorityCodeException, FactoryException, 
            IOException, ParseException {
    ...
    @Test
    public void NetCDFTestOn4DcoveragesWithDifferentSchemas() throws 
            NoSuchAuthorityCodeException, FactoryException, IOException, ParseException {
        File mosaic = new File(TestData.file(this, "."), 
                "NetCDFTestOn4DcoveragesWithDifferentSchemas");
        if (mosaic.exists()) {
            FileUtils.deleteDirectory(mosaic);
        }
        assertTrue(mosaic.mkdirs());
        ...
        try {
            ...
            for (String coverageName : names) {
                ...
                assertEquals("2012-04-01T01:00:00.000Z", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_MAXIMUM"));
                assertEquals("java.util.Date", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_DATATYPE"));

                if (coverageName.equalsIgnoreCase("O3")) {
                    assertEquals("true", reader.getMetadataValue(coverageName, 
                    ...
                    assertEquals("450.0", reader.getMetadataValue(coverageName, 
                            "ELEVATION_DOMAIN_MAXIMUM"));
                    assertEquals("java.lang.Double", reader.getMetadataValue(coverageName, 
                            "ELEVATION_DOMAIN_DATATYPE"));
                } else {
                    // Note that This sample doesn't have elevation for NO2
                    assertEquals("false", reader.getMetadataValue(coverageName, 
                            "HAS_ELEVATION_DOMAIN"));
                    final String elevationMetadata = reader.getMetadataValue(coverageName, 
                            "ELEVATION_DOMAIN");
                ...
                GeneralParameterValue[] values = coverageName.equalsIgnoreCase("O3") ? new 
                        GeneralParameterValue[]{gg, time, elevation} : new 
                        GeneralParameterValue[]{gg, time};
    ...
    @Test
    public void NetCDFTestOn4DcoveragesWithImposedSchemas() throws NoSuchAuthorityCodeException, 
            FactoryException, IOException, ParseException {
        File mosaic = new File(TestData.file(this, "."), 
                "NetCDFTestOn4DcoveragesWithImposedSchemas");
        if (mosaic.exists()) {
            FileUtils.deleteDirectory(mosaic);
        }
        assertTrue(mosaic.mkdirs());
        ...
        final Hints hints = new Hints(Hints.DEFAULT_COORDINATE_REFERENCE_SYSTEM, CRS.decode
                ("EPSG:4326", true));
        hints.put(Utils.AUXILIARY_FILES_PATH, new File(mosaic, "O3NO2-noZ.xml").getAbsolutePath()
        ); // impose def
    ...
    @Test
    public void NetCDFTestAscatL1() throws NoSuchAuthorityCodeException, FactoryException, 
            IOException, ParseException {
        ...
        try {
            ...
            for (String coverageName : names) {
                ...
                final String sigmaMetadata = reader.getMetadataValue(coverageName, 
                        "NUMSIGMA_DOMAIN");
    ...
    @Test
    public void NetCDFGOME2() throws NoSuchAuthorityCodeException, FactoryException, IOException,
            ParseException {
    ...
    @Test
    public void testFileInfo() throws NoSuchAuthorityCodeException, FactoryException, 
            IOException, ParseException {
    ...
    @Test
    public void NetCDFNoData() throws NoSuchAuthorityCodeException, FactoryException, 
            IOException, ParseException {
        File mosaic = new File(TestData.file(this, "."), "NetCDFGOME2");
        if (mosaic.exists()) {
            FileUtils.deleteDirectory(mosaic);
    ...
    @Test
    public void NetCDFTestOnClimatologicalTime() throws NoSuchAuthorityCodeException, 
            FactoryException, IOException, ParseException {
        ...
        FileUtils.copyFile(TestData.file(this, "climatological.zip"), new File(workDir, 
                "climatological.zip"));
        ...
        try {
            ...
            for (String coverageName : names) {
                ...
                final String timeMetadata = reader.getMetadataValue(coverageName, "TIME_DOMAIN");
                assertNotNull(timeMetadata);
                assertEquals("0001-01-16T00:00:00.000Z/0001-01-16T00:00:00.000Z," +
                                "0001-02-16T00:00:00.000Z/0001-02-16T00:00:00.000Z," +
                                "0001-03-16T00:00:00.000Z/0001-03-16T00:00:00.000Z," +
                                "0001-04-16T00:00:00.000Z/0001-04-16T00:00:00.000Z," +
                                "0001-05-16T00:00:00.000Z/0001-05-16T00:00:00.000Z," +
                                "0001-06-16T00:00:00.000Z/0001-06-16T00:00:00.000Z," +
                                "0001-07-16T00:00:00.000Z/0001-07-16T00:00:00.000Z," +
                                "0001-08-16T00:00:00.000Z/0001-08-16T00:00:00.000Z," +
                                "0001-09-16T00:00:00.000Z/0001-09-16T00:00:00.000Z," +
                                "0001-10-16T00:00:00.000Z/0001-10-16T00:00:00.000Z," +
                                "0001-11-16T00:00:00.000Z/0001-11-16T00:00:00.000Z," +
                                "0001-12-16T00:00:00.000Z/0001-12-16T00:00:00.000Z",
                        timeMetadata);
                assertEquals(12, timeMetadata.split(",").length);
                assertEquals("0001-01-16T00:00:00.000Z", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_MINIMUM"));
                assertEquals("0001-12-16T00:00:00.000Z", reader.getMetadataValue(coverageName, 
                        "TIME_DOMAIN_MAXIMUM"));
                ...
                final String elevationMetadata = reader.getMetadataValue(coverageName, 
                        "ELEVATION_DOMAIN");
                assertNotNull(elevationMetadata);
                assertEquals("0.0/0.0,10.0/10.0,20.0/20.0,30.0/30.0,50.0/50.0,75.0/75.0", 
                        elevationMetadata);
                assertEquals(6, elevationMetadata.split(",").length);
                assertEquals("0.0", reader.getMetadataValue(coverageName, 
                        "ELEVATION_DOMAIN_MINIMUM"));
                assertEquals("75.0", reader.getMetadataValue(coverageName, 
                        "ELEVATION_DOMAIN_MAXIMUM"));
    ...
    @Test
    public void NetCDFNoDataOperation() throws NoSuchAuthorityCodeException, FactoryException, 
            IOException, ParseException {
        File mosaic = new File(TestData.file(this, "."), "NetCDFGOME2");
        if (mosaic.exists()) {
            FileUtils.deleteDirectory(mosaic);
    ...
    /**
     * Test that {@link NetCDFReader#getOriginalEnvelope()}, a method for which no coverage name 
     * is specified, works for a NetCDF source containing
