...
public interface GetMapRequest extends Request {
  ...
  /**
   * From the Web Map Service Implementation Specification: "The required SRS parameter states which
   * Spatial Reference System applies to the values in the BBOX parameter. The value of the SRS
   * parameter shall be on of the values defined in the character data section of an &lt;SRS>
   ...
   * well-defined spatial reference system and should not be shown in conjunction with other layers.
   * The client shall specify SRS as "none" in the GetMap request and the Server may issue a Service
   * Exception otherwise."
   *
   ...
   * server has declared that a Layer is not subsettable, then the Client shall specify exactly the
   * declared Bounding Box values in the GetMap request and the Server may issue a Service Exception
   * otherwise."
   *
   * <p>NOTE: In WMS 1.3.0, the specification of "EPSG:4326" has the axis swapped, so a request made
   * in 1.1.1 using "minx,miny,maxx,maxy" would use "miny,minx,maxy,maxx" in 1.3.0. Only when using
   * EPSG:4326!
   *
   * <p>Currently it is up to the client to do this on there own. TODO Accept Envelopes and doubles
   * instead of Strings, and perform the 1.3.0 conversion automatically. Also note that not all
   * servers may implement this. Should provide an option to use 1.1.1 format even when using 1.3.0.
   *
   ...
   * the desired format of the response to an operation. Supported values for a GetMap request on a
   * WMS instance are listed in one or more &lt;Format> elements in the &;ltRequest>&lt;GetMap>
   ...
   * parameters specify the size in integer pixels of the map image to be produced. WIDTH specifies
   * the number of pixels to be used between the minimum and maximum X values (inclusive) in the
   * BBOX parameter, while HEIGHT specifies the number of pixels between the minimum and maximum Y
   * values. If the WMS server has declared that a Layer has fixed width and height, then the Client
   * shall specify exactly those WIDTH and HEIGHT values in the GetMap request and the Server may
