...
public class WMSLayer extends GridReaderLayer {
    ...
    public String getFeatureInfoAsText(DirectPosition2D pos, int featureCount) throws IOException {
        ...
        try {
            ...
            InputStream is = getReader().getFeatureInfo(pos, "text/plain", featureCount, 
                    mapRequest);
    ...
    public InputStream getFeatureInfo(ReferencedEnvelope bbox, int width, int height, int x, int y,
        ...
        try {
            ...
            // we need to convert x/y from the screen to the original coordinates, and then to 
            // the ones
            // that will be used to make the request
            AffineTransform at = RendererUtilities.worldToScreenTransform(bbox, new Rectangle
                    (width, height));
            ...
            MathTransform mt = CRS.findMathTransform(bbox.getCoordinateReferenceSystem(), 
                    getReader().requestCRS, true);
            ...
            throw (IOException) new IOException("Unexpected issue during GetFeatureInfo " +
                    "execution").initCause(t);
        }
    }
