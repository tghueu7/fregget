...
public class WMSLayer extends GridReaderLayer {
  ...
  /**
   * Retrieves the feature info as a generic input stream, it's the duty of the caller to interpret
   * the contents and ensure the stream is closed feature info format)
   *
   * @param pos the position to be checked, in real world coordinates
  ...
  public InputStream getFeatureInfo(
    ...
    try {
      ...
      MathTransform mt =
          CRS.findMathTransform(bbox.getCoordinateReferenceSystem(), getReader().requestCRS, true);
      ...
      throw (IOException)
          new IOException("Unexpected issue during GetFeatureInfo execution").initCause(t);
    }
  }
