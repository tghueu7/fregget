...
public class Layer implements Comparable<Layer> {
  ...
  //
  /**
   * The union of the layers's SRSs and the parent's SRSs. This cache is used to check if the SRS is
   * known to work for this layer
   */
  ...
  /**
   * Sets this layer's bounding boxes. The HashMap must have each BoundingBox's CRS/SRS value as its
   * key, and the <code>BoundingBox</code> object as its value.
   *
  ...
  /**
   * Direct access to the dimensions contributed by this Layer. For the complete list of Dimensions
   * applicable to the layer this value must be combined with any Dimensions supplied by the parent
   * - this work is done for you using the getDimensions() method.
   *
   * @see getDimensions()
   ...
   * ancestors. No duplicates are returned.
   *
   * <p>The List that is returned is of type List<org.opengis.layer.Style>. Before 2.2-RC0 it was of
  ...
  /**
   * Returns the LatLonBoundingBox for this layer. Implements inheritance: if this layer's bounding
   ...
   *
   * <p>Scale denominator is calculated based on the bounding box of the central pixel in a request
   ...
   * (ie not a scale based on real world size of the entire layer).
   *
   * <p>Some web map servers will refuse to render images at a scale greater than the value provided
   * here.
   *
   ...
   *
   * <p>Please note that the lookup is performed based on the SRS Name of the provided CRS which is
   * assumed to be one of its identifiers. This method returns the first envelope found; this may
   * not be valid for sparse data sets that indicate data location using multiple envelopes for a
   * provided CRS.
   *
   * @param crs
