...
public abstract class AbstractGetFeatureInfoRequest extends AbstractWMSRequest
  ...
  /**
   * GetMap request fills in the LAYERS and STYLES properties only when getFinalURL() is called, so
   * we force it to fill them in before copying the properties
   *
   * @param request
   * @return
   */
  ...
  public URL getFinalURL() {
    ...
    while (iter.hasNext()) {
      ...
      try {
        ...
        queryLayerString =
            queryLayerString + URLEncoder.encode(layer.getName(), "UTF-8").replaceAll("\\+", "%20");
        ; // $NON-NLS-1$
