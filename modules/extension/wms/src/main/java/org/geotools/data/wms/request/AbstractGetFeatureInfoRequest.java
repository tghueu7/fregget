 ...
 * @source $URL$
 */
public abstract class AbstractGetFeatureInfoRequest extends AbstractWMSRequest implements 
     ...
     * @param request        a previously configured GetMapRequest that the query will be 
     *                       executed on
     */
    ...
    public URL getFinalURL() {
        ...
        String initialQueryLayerString = properties.getProperty(QUERY_LAYERS) == null ? "" : 
                properties.getProperty(QUERY_LAYERS); //$NON-NLS-1$
        String queryLayerString = properties.getProperty(QUERY_LAYERS) == null ? "" : properties
                .getProperty(QUERY_LAYERS); //$NON-NLS-1$

        while (iter.hasNext()) {
            Layer layer = (Layer) iter.next();
            try {
                // spaces are converted to plus signs, but must be %20 for url calls [GEOT-4317]
                queryLayerString = queryLayerString + URLEncoder.encode(layer.getName(), "UTF-8")
                        .replaceAll("\\+", "%20");
                ; //$NON-NLS-1$
    ...
    /**
     * @see org.geotools.data.wms.request.GetFeatureInfoRequest#addQueryLayer(org.geotools.data
     * .ows.Layer)
     */
