...
public class WMSUtils {
   ...
   *
   * @return An array of Layers, each value has a it's name property set or an empty array if there
   *     are none. It will return null if there is no capabilities document
   */
  ...
  /**
   * Given a list of type Layer, return all EPSG codes that is supported by all of the layers. This
   * is an intersection of each layer's SRS set.
   *
   ...
   * will check the transform that exists between each EPSG code's CRS and the given CRS. If this is
   * the identity transform, meaning the CRS is equivalent to the EPSG code, the used EPSG code will
   * be returned. The first valid EPSG code found is returned, so it is possibly that multiple valid
   * codes exist.
   *
   * <p>If no such identity transform can be found, null will be returned.
   *
   * <p>If this method is succesful, the result is stored in a cache, which is called in subsequent
   * calls.
   *
