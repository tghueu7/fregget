...
public class BreadthFirstIteratorTest extends TestCase {
   ...
   * node should not be visited. After continue: 1. First node visited after continue should have id
   * = id + suspend node + 1 2. Every node should be visited.
   */
  public void test_1() {
    int nnodes = 100;
    Node[] ends = GraphTestUtil.buildNoBifurcations(builder(), nnodes);
  ...
  /**
   * Create a simple graph which has no bifurcations and do a kill branch at some intermediate node.
  ...
  /**
   * Create a balanced binary tree and do a traversal starting at root and kill branch at the first
   * node seen that is not the root, (should be left child). Then continue the traversal. <br>
   * <br>
  ...
  public void test_5() {
    ...
    final String id =
        (ln.getCount() < rn.getCount()) ? ln.getObject().toString() : rn.getObject().toString();
