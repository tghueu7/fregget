...
public class AutoClust {
    ...
    public static Graph runAutoClust(Graph d) {
        ...
        //Such a diagram can be obtained through org.geotools.graph.util.delaunay
        ...
        while (nodeIt.hasNext()) {
            ...
            while (anotherEdgeIt.hasNext()) {
                ...
                sumOfSquaredDiffs = sumOfSquaredDiffs + Math.pow((nextEdge.getEuclideanDistance()
                        - meanLength), 2);
            }
        ...
        while (anotherNodeIt.hasNext()) {
            ...
            while (edgeIt.hasNext()) {
                ...
                if (length < acd.getLocalMean() - meanStDev) {
                    ...
                    LOGGER.finer(nextEdge + ": length " + nextEdge.getEuclideanDistance() + " is " +
                            "short");
        ...
        while (nodeIt5.hasNext()) {
            ...
            List adjacentEdges = AutoClustUtils.findAdjacentEdges(next, edges); //yes, next
            // .getEdges() could work, but there's no guarantee that next's edge list is current 
            // anymore
            ...
            while (adjacentIt.hasNext()) {
                ...
                Node other = nextEdge.getOtherNode(next);
                List adjacentToOther = AutoClustUtils.findAdjacentEdges(other, edges);  //yes, 
                // other.getEdges() could work, but there's no guarantee that other's edge list 
                // is current anymore
    ...
    private static Graph getMyComponent(Node node, Vector components) {
        ...
        if (ret == null) {
            throw new RuntimeException("Couldn't find the graph component containing node: " + 
                    node);
        }
        return ret;
    }
