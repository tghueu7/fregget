...
public class BreadthFirstIterator extends SourceGraphIterator {
  ...
  /**
   * Sets the source of the traversal and places it in the node queue. The first call to this method
   * will result in the internal node queue being built. Subsequent calls to the method clear the
   * node queue and reset the iteration.
   *
  ...
  /**
   * Returns the next node from the node queue that has not yet been visited. It is possible for the
   * node queue to contain duplicate entries. To prevent the iteration returning the same node
  ...
  /**
   * Looks for nodes adjacent to the current node to place into the node queue. An adjacent node is
   * only placed into the node queue if its visited flag is unset.
   *
