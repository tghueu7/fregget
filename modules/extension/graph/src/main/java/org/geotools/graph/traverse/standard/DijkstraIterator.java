...
public class DijkstraIterator extends SourceGraphIterator {
   ...
   * infinite cost, then the node is not adjacent to any nodes in the set of nodes with known costs.
   * This situation will end the traversal every other node will also have infinite cost. This
   * usally is the result of a disconnected graph.
   *
  ...
  /**
   * Supplies a weight for each edge in the graph to be used by the iteration when calculating node
   * costs.
   *
   * @author Justin Deoliveira, Refractions Research Inc, jdeolive@refractions.net
   */
