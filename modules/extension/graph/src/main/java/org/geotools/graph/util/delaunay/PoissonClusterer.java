...
public class PoissonClusterer {
    ...
    public static Graph findClusters(Graph incoming, Expression base, Expression target, double 
            meanRate, int distance) {
        ...
        while (nodeIt.hasNext()) {
            ...
            if (!(baseObj instanceof Number)) {
                throw new RuntimeException("Expression " + base + " must evaluate to a number on " +
                        "feature " + nextFeature);
            }
            Object targetObj = target.evaluate(nextFeature);
            if (!(targetObj instanceof Number)) {
                throw new RuntimeException("Expression " + target + " must evaluate to a number " +
                        "on feature " + nextFeature);
            }
...
//                System.out.println("this node has " + newEdges.size() + " edges");
                ...
                while (edgeIt.hasNext()) {
                    ...
                    if (nextEdge.getEuclideanDistance() > 30) {
                        ...
                        if (neighbor == null) {
                            throw new RuntimeException("We have a problem.  " + next + " and " + 
                                    neighbor + " should be neighbors via " + nextEdge + ", but " +
                                    "aren't.");
                        }
                        ...
                        if (!(baseObj instanceof Number)) {
                            throw new RuntimeException("Expression " + base + " must evaluate to " +
                                    "a number on feature " + neighborFeature);
                        }
                        Object neighborsTargetObj = target.evaluate(nextFeature);
                        if (!(targetObj instanceof Number)) {
                            throw new RuntimeException("Expression " + target + " must evaluate " +
                                    "to a number on feature " + neighborFeature);
                        }
...
//                                System.out.println("adding node " + farNode + " and edge " + 
// nextEdge);
                            }
                        }
                    }
...
//                System.out.println(newEdges);
                ...
                while (newNodeIt.hasNext()) {
                    ...
                    if (!(baseObj instanceof Number)) {
                        throw new RuntimeException("Expression " + base + " must evaluate to a " +
                                "number on feature " + nextFeature2);
                    }
                    Object neighborsTargetObj = target.evaluate(nextFeature2);
                    if (!(targetObj instanceof Number)) {
                        throw new RuntimeException("Expression " + target + " must evaluate to a " +
                                "number on feature " + nextFeature2);
                    }
                    totalBase = totalBase + ((Number) baseObj).doubleValue();
                    totalTarget = totalTarget + ((Number) targetObj).doubleValue();
            ...
            double top = ((Math.pow(Math.E, (0 - expectedTarget)) * (Math.pow(expectedTarget, 
                    totalTarget))));
...
//            System.out.println("testing " + newEdges);
            System.out.println(next.getCoordinate().x + ", " + next.getCoordinate().y + ", " + 
                    totalTarget + ", " + expectedTarget + ", " + poissonProb);
