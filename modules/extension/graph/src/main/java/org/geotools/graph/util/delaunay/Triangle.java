...
public class Triangle {
    ...
    private void whinge(Edge e1, Edge e2, Edge e3) {
        throw new RuntimeException("You didn't give me a proper triangle.  " + e1 + ", " + e2 + 
                ", " + e3);
    }
    ...
    public int relate(XYNode n) {
        ...
        if ((!(node1 instanceof XYNode)) ||
            ...
            throw new RuntimeException("I can't perform a relate function on a non-spatial " +
                    "triangle");
        }
        LinearRing lr = fact.createLinearRing(new Coordinate[]{((XYNode) node1).getCoordinate(), 
                ((XYNode) node2).getCoordinate(), ((XYNode) node3).getCoordinate(), ((XYNode) 
                node1).getCoordinate()});
        Polygon poly = fact.createPolygon(lr, null);
        Point nPoint = fact.createPoint(n.getCoordinate());

        Line2D.Double line12 = new Line2D.Double(((XYNode) node1).getCoordinate().x, ((XYNode) 
                node1).getCoordinate().y, ((XYNode) node2).getCoordinate().x, ((XYNode) node2)
                .getCoordinate().y);
        Line2D.Double line13 = new Line2D.Double(((XYNode) node1).getCoordinate().x, ((XYNode) 
                node1).getCoordinate().y, ((XYNode) node3).getCoordinate().x, ((XYNode) node3)
                .getCoordinate().y);
        Line2D.Double line23 = new Line2D.Double(((XYNode) node2).getCoordinate().x, ((XYNode) 
                node2).getCoordinate().y, ((XYNode) node3).getCoordinate().x, ((XYNode) node3)
                .getCoordinate().y);
    ...
    public Edge getBoundaryEdge(XYNode n) {
        if (!(relate(n) == ON_EDGE)) {
            throw new RuntimeException("Can't get the boundary edge for a point that isn't on an " +
                    "edge.");
        }
        Point2D.Double point = new Point2D.Double(n.getCoordinate().x, n.getCoordinate().y);
        Line2D.Double line1 = new Line2D.Double(((XYNode) edge1.getNodeA()).getCoordinate().x, (
                (XYNode) edge1.getNodeA()).getCoordinate().y,
                ((XYNode) edge1.getNodeB()).getCoordinate().x, ((XYNode) edge1.getNodeB())
                .getCoordinate().y);
        Line2D.Double line2 = new Line2D.Double(((XYNode) edge2.getNodeA()).getCoordinate().x, (
                (XYNode) edge2.getNodeA()).getCoordinate().y,
                ((XYNode) edge2.getNodeB()).getCoordinate().x, ((XYNode) edge2.getNodeB())
                .getCoordinate().y);
        Line2D.Double line3 = new Line2D.Double(((XYNode) edge3.getNodeA()).getCoordinate().x, (
                (XYNode) edge3.getNodeA()).getCoordinate().y,
                ((XYNode) edge3.getNodeB()).getCoordinate().x, ((XYNode) edge3.getNodeB())
                .getCoordinate().y);
        Edge ret = null;
        if (line1.ptSegDist(point) <= TOLERANCE) {
            ...
            throw new RuntimeException("So...  node " + n + " is on an edge of " + this.toString
                    () + " but isn't on any of its edges: " + edge1 + ", " + edge2 + ", or " + 
                    edge3);
        }
        return ret;
    }
    ...
    public Node getThirdNode(Edge e) {
        if (e.getNodeA().equals(node1)) {
            if (e.getNodeB().equals(node2)) {
                ...
                throw new RuntimeException("Edge e must be in this triangle for Triangle" +
                        ".getThirdNode to work!");
            }
            ...
            throw new RuntimeException("Edge " + e + " must be in this triangle " + this.toString
    ...
    public Edge getOppositeEdge(Node n) {
        if ((edge1.getNodeA().equals(n) || (edge1.getNodeB().equals(n)))) {
            if ((edge2.getNodeA().equals(n) || (edge2.getNodeB().equals(n)))) {
                ...
                throw new RuntimeException("Node n must be in this triangle for Triangle" +
                        ".getOppositeEdge to work!");
            }
    ...
    public double getArea() {
        if ((node1 instanceof XYNode) ||
            ...
            throw new RuntimeException("I can't calculate the area if the triangle doesn't have " +
                    "XY coordinates.");
        }
    }
