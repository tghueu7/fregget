...
public class AutoClustUtils {
    ...
    private static void expandComponent(final Node node, final Collection edges, final Collection
            componentNodes, final Collection componentEdges) {
...
//            LOGGER.finer("Adding " + node + " to component");
            Vector adjacentEdges = findAdjacentEdges(node, edges);  //yes, I know node.getEdges()
            // should do this, but this method could be out of data by the time I use this in 
            // AutoClust
...
//                LOGGER.finer("its other node is " + additionalNode);
                if (additionalNode == null) {
                    throw new RuntimeException("I tried to get the other node of this edge " + 
                            next + " but it doesn't have " + node);
                }
