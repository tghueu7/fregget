...
public class NoBifurcationIterator extends SourceGraphIterator {
  ...
  /**
   * The next node in the iteration is the first node found adjacent to the current node that is non
  ...
  /**
   * Searches for the next node to be returned in the iteration. The next node is the first node (of
   * two) related to the current node that is non visited and of degree <= 2.
   *
  ...
  /**
   * Kills the current branch of the iteration by explicitly setting the next node to be returned to
   * null. This call always ends the traversal.
   *
