...
public class DelaunayTriangulator {
    ...
    public Graph getTriangulation() {
        ...
        DelaunayEdge e3 = new DelaunayEdge(tempNodes[nodes.length + 1], tempNodes[nodes.length + 
                2]);
    ...
    public Graph triangleListToGraph(Vector tList) {
        ...
        while (triangleIterator.hasNext()) {
            ...
            for (int i = 0; i < 3; i++) {
                if (!(((DelaunayEdge) edges[i]).hasEndPoint(temp1) ||      //this test ensures 
                        // that we don't
                        ((DelaunayEdge) edges[i]).hasEndPoint(temp2) ||      //add to the edge 
                        // list any edges referring                        
                        ((DelaunayEdge) edges[i]).hasEndPoint(temp3))) {      //to the temporary 
                    // nodes
    ...
    public Vector insertNode(DelaunayNode newNode, Vector tList) {
        //find triangle containing node or if node is on an edge, the two triangles bordering 
        // that edge.
        ...
        Triangle borderB = null;  //Note: assuming it's on the border of two triangles rather 
        // than at the intersection of 3 or more.
...
//                    System.out.println(newNode + " is on the border between " + borderA + " and
// " + borderB);
                    break;
                ...
                default:
                    throw new RuntimeException("So the point isn't inside, outside, or on the " +
        ...
        if (contains != null) {
            ...
            for (int i = 0; i < 3; i++) {
                if (((DelaunayEdge) triangleEdges[i]).hasEndPoint((DelaunayNode) 
                        triangleNodes[0])) {
                    if (((DelaunayEdge) triangleEdges[i]).hasEndPoint((DelaunayNode) 
                            triangleNodes[1])) {
...
//            System.out.println("after legalization, triangle list now is: " + triangleList);
            ...
            if (shared == null) {
                throw new RuntimeException("The two bordering triangles for a border case " +
                        "apparently don't share an edge(!)");
            }
...
//            System.out.println("switch internal edge");
            ...
            if (rejectSwap(t, otherFace, newTriangleP_A_4, newTriangleP_B_4)) { //Degenerate case
                // .  Explained in the method rejectSwap
...
//                System.out.println("Triangle list now is: " + triangleList);
    ...
    private boolean isIllegal(Triangle t, DelaunayEdge e, DelaunayNode p) {
        ...
        if ((farNode.equals(farNodeP_A)) || (farNode.equals(farNodeP_B))) {
            //Degenerate case.  There's already a line between p and farnode (p and k in the 
            // book) making either
        ...
        if (numTemporary == 0) {
            ...
            Point2D.Double point = new Point2D.Double(farNode.getCoordinate().x, farNode
                    .getCoordinate().y);
...
//                System.out.println("Illegal by case 4");
            ...
            throw new RuntimeException("Problem in DelaunayTriangulator.isIllegal--This " +
    ...
    private int whichSpecialNode(DelaunayNode a, DelaunayNode b) {
        if ((a.equals(temp1)) || (b.equals(temp1))) {
            ...
            throw new RuntimeException("I shouldn't be here.  Either node a or node b should be " +
                    "temporary.");
        }
    }

    private static Ellipse2D.Double constructCircle(DelaunayNode a, DelaunayNode b, DelaunayNode 
            c) {
        //center of this circle is the intersection of the perpendicular bisectors of the triangle

        Point2D.Double midPointA_B = new Point2D.Double((a.getCoordinate().x + b.getCoordinate()
                .x) / 2,
        ...
        Point2D.Double midPointA_C = new Point2D.Double((a.getCoordinate().x + c.getCoordinate()
                .x) / 2,
        ...
        int i = 1;
        do {
            bisectorA_C.setLine(new Point2D.Double((midPointA_C.getX() + Math.pow(100, i) * 
                            deltaYA_C),
    ...
    private boolean rejectSwap(Triangle oldFirst, Triangle oldSecond, Triangle newFirst, Triangle
            newSecond) {
        //I want to reject the swap if the new edge intersects any other edges in the graph, 
        // which can happen in
        //the case where A or B (i or j in the book) is one of the bounding triangle points.  
        // This (I think)
        //is equivalent to ensuring that the areas of the new triangles is the same as the areas 
