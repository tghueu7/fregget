...
public class DelaunayTriangulator {
  ...
  public Graph getTriangulation() {
    // this algorithm is from "Computational Geometry: Algorithms and Applications" by M. de Berg et
    // al.,
    // written in 1997 and printed by Springer-Verlag (New York).  Pseudocode from section 9.3 (pp.
    // 190-194).
    ...
    Triangle UNBOUNDED =
        new Triangle(new DelaunayEdge(U1, U2), new DelaunayEdge(U1, U3), new DelaunayEdge(U2, U3));
    ...
    while ((triangleIterator.hasNext()) && (notDone)) {
      ...
      switch (relation) {
        ...
        case Triangle.ON_EDGE:
          ...
          //                    System.out.println(newNode + " is on the border between " + borderA
          // + " and " + borderB);
          break;
  ...
  private boolean isIllegal(Triangle t, DelaunayEdge e, DelaunayNode p) {
    ...
    if ((farNode.equals(farNodeP_A)) || (farNode.equals(farNodeP_B))) {
      // Degenerate case.  There's already a line between p and farnode (p and k in the book) making
      // either
      // p_farnode_A or p_farNode_B a triangle already in the triangulation.  This will eventually
      // manifest
  ...
  private static Ellipse2D.Double constructCircle(DelaunayNode a, DelaunayNode b, DelaunayNode c) {
    ...
    bisectorA_B.setLine(
        new Point2D.Double(
            (midPointA_B.getX() + 100 * deltaYA_B), (midPointA_B.getY() - 100 * deltaXA_B)),
        new Point2D.Double(
            (midPointA_B.getX() - 100 * deltaYA_B), (midPointA_B.getY() + 100 * deltaXA_B)));
  ...
  private boolean rejectSwap(
    ...
    // I want to reject the swap if the new edge intersects any other edges in the graph, which can
    // happen in
    // the case where A or B (i or j in the book) is one of the bounding triangle points.  This (I
    // think)
    // is equivalent to ensuring that the areas of the new triangles is the same as the areas of the
    // old triangles.
