...
public class BasicDirectedNode extends BasicGraphable implements DirectedNode {
  ...
  /**
   * First searches for an in edge with an out node == this, and in node == other. If none is found
   * an edge with out node == other, and in node == this is searched for.
   *
   * @see Node#remove(Edge)
   */
  ...
  /**
   * Overides the default deserialization operation. The edge adjacency lists of a BasicDirectedNode
   * is not written out when the node is serialized so they must be recreated upon deserialization.
   *
