...
public class XPathUtilTest {
  ...
  public void testSteps() throws Exception {
    ...
    expected =
        Arrays.asList(
            new XPathUtil.Step[] {
              new XPathUtil.Step(new QName(rootQName.getNamespaceURI(), "measurement"), 5)
            });
  ...
  /**
   * Tests a location path of the form <code>"foo/bar/@baz"</code> gets built as a {@link StepList}
   * of attribute names <code>"foo/bar/baz"</code> (i.e. no distinction between what's a "property"
   * and what's an (xml) "attribute".
   *
   * @throws Exception
   */
  @Test
  ...
  public void testRootElementSteps() {
    ...
    FeatureType complexType =
        ComplexTestData.createExample05NoNamespaceURI(new UniqueNameFeatureTypeFactoryImpl());
    Name name = complexType.getName();
    AttributeDescriptor descriptor =
        new AttributeDescriptorImpl(complexType, name, 0, Integer.MAX_VALUE, true, null);
