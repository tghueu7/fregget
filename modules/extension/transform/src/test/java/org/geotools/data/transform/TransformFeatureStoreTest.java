...
public class TransformFeatureStoreTest extends AbstractTransformTest {
  ...
  public void testInsertOnSelection() throws Exception {
    ...
    assertEquals(
        1, STATES.getFeatures(new Query(null, CQL.toFilter("state_name = 'baloon'"))).size());
    assertEquals(
        1, transformed.getFeatures(new Query(null, CQL.toFilter("state_name = 'baloon'"))).size());
  }
  ...
  public void testInsertOnRename() throws Exception {
    ...
    assertEquals(
        1, STATES.getFeatures(new Query(null, CQL.toFilter("state_name = 'baloon'"))).size());
    assertEquals(
        1, transformed.getFeatures(new Query(null, CQL.toFilter("name = 'baloon'"))).size());
  }
