...
class TransformFeatureCollection extends AbstractFeatureCollection {
    ...
    public TransformFeatureCollection(SimpleFeatureSource source, Transformer transformer, Query 
            query) {
    ...
    protected Iterator<SimpleFeature> openIterator() {
        try {
            ...
            SimpleFeatureIterator transformed = new TransformFeatureIteratorWrapper(fi, 
                    transformer);

            // see if we have to apply sort
            if (query.getSortBy() != null && txQuery.getSortBy() == null) {
                transformed = new SortedFeatureIterator(transformed, getSchema(), query.getSortBy
                        (), -1);
            }
