...
public class GML2EncodingUtils {
   ...
   * Geometry#getUserData()} (creating it with{@link Geometry#getUserData()} if it does not already
   * exist). If the user data exists and is not a {@link Map}, this method has no effect.
   *
  ...
  /**
   * Returns the geometry dimension, either as forced in the configuration, or the geometry natural
   * one
   *
   * @param geometry
   * @param config
  ...
  public static Integer getGeometryDimension(Geometry geometry, Configuration config) {
    ...
    /**
     * For the dimension, use the actual dimension of the geometry. Using the dimension of the CRS
     * is not sufficient, since currently CRSes don't support 3D.
     */
