...
public abstract class FeatureCollectionEncoderDelegate implements EncoderDelegate {
  ...
  /**
   * Allows subclasses to generate a list of attributes for the property being encoded. The default
  ...
  final class FeatureTypeContext {
    ...
    private List<AttributeContext> setupAttributeContexts(
      ...
      for (Iterator p = properties.iterator(); p.hasNext(); ) {
        ...
        if (prefix != null) {
          contentName =
              QualifiedName.build(content.getTargetNamespace(), content.getName(), prefix);
        } else {
  ...
  /**
   * A cache for feature type contexts, to avoid rebuilding them in case a single feature collection
   * contains multiple feature types (wrong, but used in CompositeFeatureCollection in GeoServer)
   * and for joined results which makes us encode 2 or more features for each "member" in the result
   */
