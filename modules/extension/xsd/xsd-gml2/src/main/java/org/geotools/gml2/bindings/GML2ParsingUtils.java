...
public class GML2ParsingUtils {
  ...
  /**
   * Metadata key used to indicate if a feature type has been parsed from a XML schema, or reflected
   * out of a sample feature
   */
  ...
  public static SimpleFeature parseFeature(
    ...
    if (!decl.isAbstract()) {
      ...
      fType =
          ftCache.get(
              new NameImpl(node.getComponent().getNamespace(), node.getComponent().getName()));
  ...
  public static SimpleFeatureType featureType(
    ...
    for (Iterator itr = children.iterator(); itr.hasNext(); ) {
      ...
      if (bindings.isEmpty()) {
        ...
        LOGGER.fine(
            "Could not find binding for " + property.getQName() + ", using XSAnyTypeBinding.");
  ...
  static GeometryCollection GeometryCollectionType_parse(
    ...
    if (MultiPoint.class.isAssignableFrom(clazz)) {
      ...
      gc =
          gFactory.createGeometryCollection((Geometry[]) geoms.toArray(new Geometry[geoms.size()]));
    }
