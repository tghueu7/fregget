...
public class GetCapabilitiesTest extends TestCase {
   ...
   * <ows:Keyword>geospatial</ows:Keyword> <ows:Keyword>geoprocessing</ows:Keyword> </ows:Keywords>
   * <ows:ServiceType>WPS</ows:ServiceType> <ows:ServiceTypeVersion>1.0.0</ows:ServiceTypeVersion>
  ...
  void assertServiceIdentification(ServiceIdentificationType si) {
    ...
    assertEquals(
        "AAFC GDAS-based WPS server", ((LanguageStringType) si.getTitle().get(0)).getValue());
  ...
  void assertOperationsMetadata(OperationsMetadataType om) {
    ...
    assertEquals(
        "http://wms1.agr.gc.ca/GeoPS/GeoPS?", ((RequestMethodType) http.getGet().get(0)).getHref());
    ...
    assertEquals(
        "http://wms1.agr.gc.ca/GeoPS/GeoPS", ((RequestMethodType) http.getPost().get(0)).getHref());
