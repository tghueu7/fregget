...
public class GetCapabilitiesTest extends TestCase {
    ...
    public void testParse() throws Exception {
        ...
        Object o = parser.parse(getClass().getResourceAsStream("20_wpsGetCapabilities_response" +
                ".xml"));
    ...
    void assertServiceIdentification(ServiceIdentificationType si) {
        ...
        assertEquals("AAFC GDAS-based WPS server", ((LanguageStringType) si.getTitle().get(0))
                .getValue());

        assertEquals(1, si.getAbstract().size());
        assertEquals("AAFC GDAS-based WPS server developed for the OGC WPSie.", (
    ...
    void assertServiceProvider(ServiceProviderType sp) {
        ...
        assertEquals("Room 1135, Neatby Building, 960, Carling Avenue", a.getDeliveryPoint().get
                (0));
    ...
    void assertOperationsMetadata(OperationsMetadataType om) {
        ...
        assertEquals("http://wms1.agr.gc.ca/GeoPS/GeoPS?", ((RequestMethodType) http.getGet().get
                (0)).getHref());
        ...
        assertEquals("http://wms1.agr.gc.ca/GeoPS/GeoPS", ((RequestMethodType) http.getPost().get
                (0)).getHref());
     ...
     * distance, and output the results in GML.</ows:Abstract>
    ...
    void assertProcessOfferings(ProcessOfferingsType po) {
        ...
        assertEquals("Buffer  the polygon coordinates found in one GML stream by a given buffer " +
                "distance, and output the results in GML.", pb.getAbstract().getValue());
