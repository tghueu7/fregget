...
public class FilterParsingUtils {
    ...
    public static Object Filter_getProperty(Object object, QName qName) {
        ...
        if ("logicOps".equals(name) && (filter instanceof BinaryLogicOperator || filter 
                instanceof Not)) {
            return filter;
        }
    ...
    public static List<Filter> BinaryLogicOperator_getChildFilters(Node node, org.opengis.filter
    ...
    public static List<Filter> parseExtendedOperators(Node node, org.opengis.filter.FilterFactory
            factory) {
        ...
        if (node.hasChild(Expression.class)) {
            ...
            Filter extOp = lookupExtendedOperator(opName, Arrays.asList((Expression) n.getValue()
            ), factory);
            if (extOp != null) {
                extOps.add(extOp);
            }
            ...
            for (Node n : children) {
                Name opName = new NameImpl(n.getComponent().getNamespace(), n.getComponent()
                        .getName());
