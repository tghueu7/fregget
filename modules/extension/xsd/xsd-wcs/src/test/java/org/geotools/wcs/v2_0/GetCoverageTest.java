...
public class GetCoverageTest {
  ...
  private Map<String, Object> getExtensionsMap(GetCoverageType gc) {
    ...
    for (ExtensionItemType item : gc.getExtension().getContents()) {
      Object value =
          item.getSimpleContent() != null ? item.getSimpleContent() : item.getObjectContent();
  ...
  public void testParseGetCoverageScaleByFactor() throws Exception {
    ...
    ScalingType scaling =
        (ScalingType)
            extensions.get("http://www.opengis.net/WCS_service-extension_scaling/1.0:Scaling");
  ...
  public void testParseGetCoverageInterpolationLinear() throws Exception {
    ...
    assertEquals(
        "http://www.opengis.net/def/interpolation/OGC/1/linear", method.getInterpolationMethod());
  }
