 ...
 *          included for backwards compatibility with GML 2. Use
 *                          CurvePropertyType instead. A property that has a line string as its 
 *                          value domain can either be an appropriate geometry element encapsulated
 *                          in an element of this type or an XLink reference to a remote geometry
 ...
 *              xlinks.xsd). XLink is used in GML to reference remote resources
 *                                  (including those elsewhere in the same document). A simple 
 *                                  link element can be constructed by including a specific set 
 *                                  of XLink attributes.
 *                                  The XML Linking Language (XLink) is currently a Proposed 
 *                                  Recommendation of the World Wide Web Consortium. XLink allows
 *                                  elements to
 *                                  be inserted into XML documents so as to create sophisticated 
...
public class LineStringPropertyTypeBinding extends GeometryPropertyTypeBindingBase {
    ...
    public LineStringPropertyTypeBinding(GML3EncodingUtils encodingUtils, XSDIdRegistry 
            idRegistry) {
