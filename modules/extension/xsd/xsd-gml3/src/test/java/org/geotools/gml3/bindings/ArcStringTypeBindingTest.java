...
public class ArcStringTypeBindingTest extends GML3TestSupport {
  ...
  public void testEncodeSimple() throws Exception {
    LineString curve =
        new CurvedGeometryFactory(0.1)
            .createCurvedGeometry(
                new LiteCoordinateSequence(new double[] {1, 1, 2, 2, 3, 1, 5, 5, 7, 3}));
    ...
    assertEquals(
        1,
        xpath
            .getMatchingNodes(basePath + "[@interpolation='circularArc3Points']", dom)
            .getLength());
  ...
  public void testEncodeCompound() throws Exception {
    ...
    LineString curve =
        factory.createCurvedGeometry(new LiteCoordinateSequence(1, 1, 2, 2, 3, 1, 5, 5, 7, 3));
    ...
    assertEquals(
        1,
        xpath
            .getMatchingNodes(basePath1 + "[@interpolation='circularArc3Points']", dom)
            .getLength());
    ...
    assertEquals(
        1, xpath.getMatchingNodes(basePath2 + "[@interpolation='linear']", dom).getLength());
