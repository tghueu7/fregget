...
public class MultiCurveEncoderTest extends GeometryEncoderTestSupport {
  ...
  public void testEncodeMultiCompound() throws Exception {
    ...
    assertEquals(
        "100 100 120 120",
        xpath.evaluate("//gml:MultiCurve/gml:curveMember[1]/gml:LineString/gml:posList", doc));
    assertEquals(
        2,
        xpath
            .getMatchingNodes("//gml:MultiCurve/gml:curveMember[2]/gml:Curve/gml:segments/*", doc)
            .getLength());
  ...
  public void testEncodeMultiLineString() throws Exception {
    ...
    assertEquals(
        1, xpath.getMatchingNodes("//gml:MultiLineString/gml:lineStringMember", doc).getLength());
    assertEquals(
        "105 105 125 125",
        xpath.evaluate(
    ...
    assertEquals(
        "multi.1",
        xpath.evaluate("/gml:MultiLineString/gml:lineStringMember/gml:LineString/@gml:id", doc));
  }
}
