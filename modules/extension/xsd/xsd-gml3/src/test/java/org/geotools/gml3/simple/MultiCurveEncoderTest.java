...
public class MultiCurveEncoderTest extends GeometryEncoderTestSupport {
    ...
    public void testEncodeMultiCompound() throws Exception {
        MultiLineStringEncoder encoder = new MultiLineStringEncoder(gtEncoder, "gml", GML
                .NAMESPACE, true);
        Geometry geometry = new WKTReader2()
                .read("MULTICURVE((100 100, 120 120), COMPOUNDCURVE(CIRCULARSTRING(0 0, 2 0, 2 1," +
                        " 2 3, 4 3),(4 3, 4 5, 1 4, 0 0)))");
        ...
        assertEquals(
                "circularArc3Points",
                xpath.evaluate(
                        "/gml:MultiCurve/gml:curveMember[2]/gml:Curve/gml:segments/gml:ArcString" +
                                "/@interpolation",
                        doc));
        assertEquals(
                "0 0 2 0 2 1 2 3 4 3",
        ...
        assertEquals(
                "linear",
                xpath.evaluate(
                        "/gml:MultiCurve/gml:curveMember[2]/gml:Curve/gml:segments/gml" +
                                ":LineStringSegment/@interpolation",
                        doc));
        assertEquals(
                "4 3 4 5 1 4 0 0",
                xpath.evaluate(
                        "/gml:MultiCurve/gml:curveMember[2]/gml:Curve/gml:segments/gml" +
                                ":LineStringSegment/gml:posList",
                        doc));
    }
    ...
    public void testEncodeMultiCurve() throws Exception {
        MultiLineStringEncoder encoder = new MultiLineStringEncoder(gtEncoder, "gml", GML
                .NAMESPACE, true);
        Geometry geometry = new WKTReader2()
                .read("MULTICURVE((105 105, 125 125), CIRCULARSTRING(-10 0, -8 2, -6 0, -8 -2, " +
                        "-10 0))");
        Document doc = encode(encoder, geometry);
        // XMLTestSupport.print(doc);
        assertEquals(1, xpath.getMatchingNodes("//gml:MultiCurve", doc).getLength());
    ...
    public void testEncodeMultiLineString() throws Exception {
        ...
        assertEquals(1, xpath.getMatchingNodes("//gml:MultiLineString/gml:lineStringMember", doc)
                .getLength());
        assertEquals("105 105 125 125", xpath.evaluate(
        ...
        assertEquals("multi.1", xpath.evaluate
