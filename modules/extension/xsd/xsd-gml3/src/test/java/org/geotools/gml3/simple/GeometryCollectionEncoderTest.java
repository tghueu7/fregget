...
public class GeometryCollectionEncoderTest extends GeometryEncoderTestSupport {
  ...
  public void testGeometryCollectionEncoder() throws ParseException, Exception {
    ...
    assertEquals(
        "180 200 160 180",
        xpath.evaluate("//gml:MultiGeometry/gml:geometryMember/gml:LineString/gml:posList", doc));
    assertEquals(
        "19 19", xpath.evaluate("//gml:MultiGeometry/gml:geometryMember/gml:Point/gml:pos", doc));
    assertEquals("feature.1", xpath.evaluate("//gml:MultiGeometry/@gml:id", doc));
    assertEquals(
        "feature.1.1",
        xpath.evaluate("//gml:MultiGeometry/gml:geometryMember[1]/gml:LineString/@gml:id", doc));
    assertEquals(
        "feature.1.2",
