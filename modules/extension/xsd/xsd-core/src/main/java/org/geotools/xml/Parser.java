...
public class Parser {
  ...
  /**
   * Signals the parser to parse the entire instance document. The object returned from the parse is
   * the object which has been bound to the root element of the document. This method should only be
   * called once for a single instance document.
   *
   ...
   *
   * <p>The object returned from the parse is the object which has been bound to the root element of
   * the document. This method should only be called once for a single instance document.
   *
   * @return The object representation of the root element of the document.
   * @throws IOException
   * @throws SAXException
  ...
  public Object parse(Source source)
    ...
    return parse(
        new ByteArrayInputStream(((ByteArrayOutputStream) result.getOutputStream()).toByteArray()));
  }
   ...
   *
   * <p>When set to <code>true</code>, this will cause the parser to operate in a strict mode, which
   * means that xml being parsed must be exactly correct with respect to the schema it references.
   *
   * <p>Some examples of cases in which the parser will throw an exception while operating in strict
   * mode:
   *
   * <ul>
  ...
  /**
   * Flag that controls whether the parser will process mixed content in a way that preserves order
   * of child elements and text.
   *
   * <p>By default the parser will simply concatenate blindly all child text and not preserve order
   ...
   * inferred.
   *
   * <p>This method is used in cases where the element being parsed is not declared as global in the
   * schema.
   *
  ...
  public static interface Properties {
    /**
     * If set, the parser will continue to parse when it finds an element and cannot determine its
     * type.
     *
    ...
    /**
     * If set, the parser will continue to parse when it finds an attribute and cannot determine its
     * type.
     *
     * @deprecated use {@link Parser#setStrict(boolean)}
     */
    QName PARSE_UNKNOWN_ATTRIBUTES = new QName("http://www.geotools.org", "parseUnknownAttributes");
