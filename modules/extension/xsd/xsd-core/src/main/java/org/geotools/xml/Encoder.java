...
public class Encoder {
   ...
   *
   * <p>This encoding will determine the resulting character encoding for the XML content generated
   * by this Encoder and will be reflected in the XML declaration tag.
   *
   ...
   *
   * <p>When indenting, and only when indenting, long lines will be broken at space boundaries based
   * on this line width. No line wrapping occurs if this value is zero.
   *
   ...
   *
   * <p>Registering a schema location will include it on the "schemaLocation" attribute of the root
  ...
  /**
   * Helper method that checks if the complex feature we want to encode maps to a complex type that
   * respects the GML object-property model.
   */
  private boolean isNonStripedNestedElement(Object next, XSDElementDeclaration element) {
    ...
    // let's see if all the properties have the same type, and that the type is equal to the current
    // element type
    if (!nestedProperties
        .stream()
    ...
    for (XSDParticle childParticle :
        (List<XSDParticle>) Schemas.getChildElementParticles(element.getTypeDefinition(), true)) {
