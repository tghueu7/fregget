...
public class Encoder {
     ...
     * </p>
     *
     * @param indentSize the number, greater or equal than zero, of characters used to
    ...
    private boolean isNonStripedNestedElement(Object next, XSDElementDeclaration element) {
        ...
        if (!nestedProperties.stream().allMatch(property -> property.getType().getName().equals
                (complex.getType().getName()))) {
        ...
        for (XSDParticle childParticle : (List<XSDParticle>) Schemas.getChildElementParticles
            ...
            if (childElement.isElementDeclarationReference()) {
                ...
                if (childElement.getType().getName().equals(complex.getType().getName()
                        .getLocalPart())) {
        ...
        try {
            ...
            if (root == null) {
                ...
                if (typeDefintion == null) {
                    typeDefintion =
                            (QName) context.getComponentInstance("http://geotools" +
                                    ".org/typeDefinition");
                }
            ...
            while (!encoded.isEmpty()) {
                ...
                if (entry.encoding != null) {
                    ...
                    if (!entry.children.isEmpty()) {
                        ...
                        if (itr.hasNext()) {
                            ...
                            if (next instanceof EncoderDelegate) {
                                ...
                                if (next instanceof ComplexAttribute && relaxed && 
                                        isNonStripedNestedElement(next, element)) {
                                    for (Property property : ((ComplexAttribute) next)
                                            .getProperties()) {
                        ...
                        if (sub.size() > 0) {
                            ...
                            for (Iterator s = sub.iterator(); s.hasNext(); ) {
                                ...
                                if (binding.getType() == null) {
                                    logger.warning("Binding: " + binding.getTarget() + " returns " +
                                            "null type.");
                                    continue;
                                }
                            ...
                            //if one, we are gold
                            if (matches.size() == 1) {
                                entry.element = (XSDElementDeclaration) ((Object[]) matches.get
                                        (0))[0];
                            }
                    ...
                    entry.encoding = entry.parent != null ?
                            (Element) encode(entry.object, entry.element, entry.parent.element
                                    .getType()) :
                    ...
                    for (Iterator itr = attributes.iterator(); itr.hasNext(); ) {
                        ...
                        GetPropertyExecutor executor = new GetPropertyExecutor(entry.object, 
                                attribute);

                        BindingVisitorDispatch.walk(object, bindingWalker, entry.element, 
                                executor, context);
                    ...
                    for (Iterator pe = propertyExtractors.iterator(); pe.hasNext(); ) {
                        ...
                        if (propertyExtractor.canHandle(entry.object)) {
                            List extracted = propertyExtractor.properties(entry.object, entry
                                    .element);
                            O:
                            for (Iterator e = extracted.iterator(); e.hasNext(); ) {
                                ...
                                //check for a comment
                                if ((child != null)
                                        && (COMMENT.getNamespaceURI().equals(child
                                        .getTargetNamespace()))
                                ...
                                for (int i = 0; i < entry.encoding.getChildNodes().getLength(); 
                                     i++) {
                                ...
                                if (particle.isSetMaxOccurs()) {
                                    ...
                                    if (particle.eContainer() instanceof XSDModelGroup) {
                                        ...
                                        if (group.eContainer() instanceof XSDParticle) {
                                            XSDParticle cParticle = (XSDParticle) group
                                                    .eContainer();
                                ...
                                if ((maxOccurs == -1) || (maxOccurs > 1)) {
                                    ...
                                    entry.children.add(new Object[]{child, new SingleIterator
                                            (obj), obj});
                                }
                            }
                        }
     ...
     * </p>
     */
    public Document encodeAsDOM(Object object, QName name) throws IOException, SAXException, 
