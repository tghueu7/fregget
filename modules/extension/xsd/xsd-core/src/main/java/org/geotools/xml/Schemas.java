...
public class Schemas {
   ...
   *
   * @param location A uri pointing to the location of the schema.
   * @param locators An array of schema locator objects to be used when parsing imports/includes of
   *     the main schema.
  ...
  /**
   * Remove all references to a schema It is important to call this method for every dynamic schema
   * created that is not needed anymore, because references in the static schema's will otherwise
   ...
   * @return The particle representing the element declaration, or <code>null</code> if it could not
   *     be found.
   */
  ...
  /**
   * Method to visit xs:any element. The method differs from visitElements only by the fact that it
   * visits xs:any rather than <element>
   *
   ...
   * @throws IllegalArgumentException If the element declaration cannot be locaated withing the type
   *     definition.
   */
   ...
   *
   * <ol>
   *   <li>The immediate children of the specified element are examined, if a match is found, it is
   *       returned. If 1. does not match, global elements that derive from the immediate children
   *       are examined.
   * </ol>
   *
   ...
   * includes are removed. Two includes are considered equal if they have the same target namespace.
   *
  ...
  /**
   * Method for comparing the name of a schema component to a qualified name. The component name and
   * the qualified name match if both the namespaces match, and the local parts match. Prefixes are
   * ignored. Two strings will match if one of the following conditions hold.
   *
   * <ul>
   ...
   * @param container The container.
   * @param clazz The class.
   * @return A list of all adapters for components of class <code>clazz</code>, or the empty list if
   *     none found.
   */
  ...
  /**
   * Simple visitor interface for visiting elements which are part of complex types. This interface
   * is private api because there is probably a better way of visiting the contents of a type.
   *
