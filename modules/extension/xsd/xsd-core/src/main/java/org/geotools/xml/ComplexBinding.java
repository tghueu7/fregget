...
public interface ComplexBinding extends Binding {
   ...
   *
   * <p>This method is called when the leading edge of the associated element is reached, before any
   ...
   * @param value The result of the parse from another strategy in the type hierarchy. Could be null
   *     if this is the first strategy being executed.
   ...
   *
   * <p>Complex objects are encoded as elements in a document. The <param>value</param> parameter is
   * the encoded element, created by the parent binding. For the first binding in the execution
   ...
   *
   * <p>This method should just return null in the event that the object being encoded is an leaf in
   * its object model.
   *
   * <p>For multi-values properties ( maxOccurs > 0 ), this method may return an instance of {@link
   * java.util.Collection}, {@link java.util.Iterator}, or an array.
   *
   * @param object The object being encoded.
   ...
   *
   * <p>This method should only be implemented in the case where the encoder can not determine what
   * the properties of the object are from the schema.
   *
   * <p>An example would be an object which corresponds to an element in the schema which has a the
   * type <code>xs:anyType</code>. Since the content of this type can be anything the schema has no
   * way to determine what the properties are. So in this case this method must specify the
   * properties manually as a set of name, object tuples.
   *
   * <p>In the case of a multi-valued property, this method must return a tuple for each instance of
   * the property, and not a list, iterator, or array containing all of the instances.
   *
   * @param object the object being encoded.
