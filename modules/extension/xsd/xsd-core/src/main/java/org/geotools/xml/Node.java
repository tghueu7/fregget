...
public interface Node {
  ...
  /**
   * Returns all nodes corresponding child elements with the specified name. This method returns an
   * empty list if it finds no nodes corresponding to the specified name.
   *
   ...
   * @return the value of the child which is an instance of <code>clazz</code>, or <code>null</code>
   *     if no such child exists.
   */
  Object getChildValue(Class clazz);

  /**
   * Helper method for access to child's parsed contents by class, returning a <tt>defaultValue</tt>
   ...
   * @return A list of values representing the parsed values of the children, or an empty list of no
   *     such values exist.
   */
  ...
  /**
   * Returns all nodes corresponding to attributes which has a parsed values which are instances of
   * <code>clazz</code>.
   *
  ...
  /**
   * Helper method for access to the parsed value of the attribute whose parsed value is an instance
   * of <code>clazz</code>. In the event that the node contains multple attributes matching the
   * above criteria, the first encountered is returned, with no guaratnee of order. For all values
  ...
  /**
   * Helper method for access to the parsed value of the attribute whose parsed value is an instance
   * of <code>clazz</code>, returning a <code>defaultValue</code> when no such attribute is present.
   * In the event that the node contains multple attributes matching the above criteria, the first
