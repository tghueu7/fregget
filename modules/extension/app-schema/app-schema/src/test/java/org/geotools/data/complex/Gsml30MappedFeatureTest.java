  ...
  public void features() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(
          "First",
          ((ComplexAttribute)
                  featureMap.get("mf.1").getProperty(new NameImpl(GML.NAMESPACE, "name")))
              .getProperty(new NameImpl("simpleContent"))
              .getValue());
      Assert.assertEquals(
          "Second",
          ((ComplexAttribute)
                  featureMap.get("mf.2").getProperty(new NameImpl(GML.NAMESPACE, "name")))
              .getProperty(new NameImpl("simpleContent"))
              .getValue());
                    ...
                    ((ComplexAttribute)
                            ((ComplexAttribute)
                                    ((ComplexAttribute)
                                        ...
                                        .getProperty(
                                            new NameImpl(
                                                GMD.NAMESPACE, "MD_RepresentativeFraction")))
