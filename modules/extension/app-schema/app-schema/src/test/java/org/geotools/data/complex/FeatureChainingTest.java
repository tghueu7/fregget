...
public class FeatureChainingTest extends AppSchemaTestSupport {
   ...
   * multi valued. By making sure that both are nested inside geological unit feature, it's verified
   * that nesting multiple multi valued properties is possible.
   *
   * @throws Exception
   */
  @Test
    ...
    while (guIterator.hasNext()) {
      ...
      for (Property property : nestedTermValues) {
        ...
        for (Property nestedProperty : feature.getProperties("value")) {
          realValues.add(
              ((Property) ((Collection) nestedProperty.getValue()).iterator().next()).getValue());
        }
      }
  ...
  public void testFilters() throws Exception {
    ...
    property =
        ff.property("gsml:specification/gsml:GeologicUnit/gsml:occurrence/@xlink:href", namespaces);
   ...
   * Also making sure that a feature type can have multiple FEATURE_LINK to be referred by different
   * types.
   *
   * @throws Exception
   */
  ...
  /**
   * Test chaining multi-valued by reference (xlink:href). It should result with multiple attributes
   * with no nested attributes, but only client property with xlink:href.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * Tests that equivalent source data stores (i.e. with exactly the same configuration parameters)
   * are detected and only one instance of them is created and shared between all feature mappings
   * referencing it.
   *
   * <p>Note: the above holds true in the context of a single {@link AppSchemaDataAccess} instance,
   * not across data stores.
   *
   * @throws IOException
   */
  @Test
  ...
  private static void loadDataAccesses() throws Exception {
    ...
    /**
     * Non-feature types that are included in geologicUnit.xml should be loaded when geologic unit
     * data access is created
     */
    // Composition Part
