...
public class UnmappingFilterVisitor implements org.opengis.filter.FilterVisitor, ExpressionVisitor {
  ...
  public Object visit(PropertyIsGreaterThanOrEqualTo filter, Object arg1) {
    ...
    for (int i = 0; i < expressions.length; i++) {
      ...
      Expression right = expressions[i][1];
      Filter unrolled =
          ff.greaterOrEqual(left, right, filter.isMatchingCase(), filter.getMatchAction());
      combinedFilters.add(unrolled);
    }
  ...
  /**
   * @todo: support function arguments that map to more than one source expression. For example, if
   *     the argumen <code>gml:name</code> maps to source expressions <code>name</code> and <code>
   *     description</code> because the mapping has attribute mappings for both <code>
