...
public class XmlFeatureTypeMapping extends FeatureTypeMapping {
  ...
  /**
   * Find asXpath in a function, which might be the function itself or a parameter of the function,
   * and extract the xpath value including itemXpath and instancePath prefixes.
   *
   ...
   *
   * <p>If any step in <code>propertyName</code> has index greater than 1, any mapping for the same
   * property applies, regardless of the mapping. For example, if there are mappings for <code>
   * gml:name[1]</code>, <code>gml:name[2]</code> and <code>gml:name[3]</code>, but propertyName is
   * just <code>gml:name</code>, all three mappings apply.
   *
