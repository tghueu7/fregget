...
public class AppSchemaAttributeBuilder extends AttributeBuilder {
   ...
   *
   * <p>This method uses the type supplied in {@link #setType(AttributeType)} in order to determine
   * the attribute type.
   *
   * @param id the attribtue id
   * @param value The value of the attribute.
   * @param name The name of the attribute.
   * @param type the actual type of the attribute, which might be the same as the declared type for
   *     the given AttributeDescriptor or a derived type.
   */
  public Attribute add(
    ...
    if (!declaredType.equals(type)) {
      ...
      if (type instanceof GeometryType) {
        ...
        descriptor =
            new AttributeDescriptorImpl(type, name, minOccurs, maxOccurs, nillable, defaultValue);
      }
    }
    Attribute attribute;
