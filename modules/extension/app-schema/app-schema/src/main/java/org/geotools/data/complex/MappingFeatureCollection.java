...
public class MappingFeatureCollection implements FeatureCollection<FeatureType, Feature> {
   ...
   *
   * <p>Passing <code>null</code> is equivalent to calling {@link #features()} and lets the iterator
   * decide whether a new transaction should be created (and closed when the iterator is closed) or
   * not. Currently, a new transaction is created by {@link DataAccessMappingFeatureIterator} only
   * if a database backend is available and joining is enabled, to reduce the number of concurrent
