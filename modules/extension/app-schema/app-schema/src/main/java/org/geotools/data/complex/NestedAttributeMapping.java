...
public class NestedAttributeMapping extends AttributeMapping {
    ...
    public NestedAttributeMapping(Expression idExpression, Expression parentExpression,
            ...
            Expression> clientProperties,
                                  Expression sourceElement, StepList sourcePath, NamespaceSupport
                                          namespaces)
            throws IOException {
        super(idExpression, parentExpression, null, targetXPath, null, isMultiValued, 
                clientProperties);
    ...
    public List<Feature> getInputFeatures(Object caller, Object foreignKeyValue,
        ...
        if (source == null || isConditional) {
            ...
            if (featureTypeMapping == null) {
                LOGGER.info("FeatureTypeMapping for '" + featureTypeName + "' not found when " +
                        "evaluating filter!");
                return Collections.EMPTY_LIST;
            }
            ...
            if (source == null) {
                LOGGER.info("Feature source for '" + featureTypeName + "' not found when " +
                        "evaluating filter");
                return Collections.EMPTY_LIST;
            }
    ...
    public AttributeMapping getMapping(FeatureTypeMapping featureTypeMapping) {
        ...
        if (!ComplexFeatureConstants.FEATURE_CHAINING_LINK_STRING.equals(nestedTargetXPath.get
                (nestedTargetXPath.size() - 1).getName().getLocalPart())) {
     ...
     * @param isMultiple      true if the table is denormalised and multiple values are possible 
     *                        for the same id
     * @return list of built features
     * @throws IOException
     */
    private List<Feature> getFilteredFeatures(Object foreignKeyValue, boolean isMultiple) throws 
            IOException {
        ...
        try (FeatureIterator<Feature> it = fCollection.features()) {
            if (nestedIdExpression.equals(Expression.NIL)) {
                ...
                while (it.hasNext()) {
                    ...
                    matchingFeatures.add(f);
                    if (isMultiple) {
                        featureIds.add(Converters.convert(nestedIdExpression.evaluate(f), String
                                .class));
                    }
                }
    ...
    public List<Feature> getFeatures(Object foreignKeyValue,
                                     CoordinateReferenceSystem reprojection, Feature feature, int
                                             resolveDepth, Integer resolveTimeOut) throws 
            IOException {
        return getFeatures(null, foreignKeyValue, null, reprojection, feature, null, true, 
                resolveDepth, resolveTimeOut);
    }
