...
public class AttributeCreateOrderList {
    ...
    private Map<String, List<AttributeMapping>> childrenList = new HashMap<String, 
    ...
    class InnerClass implements Iterator<AttributeMapping> {
        ...
        public AttributeMapping next() {
            if (!isHasNextBeenCalled) {
                throw new IllegalStateException("next method called without hasNext being called " +
                        "first.");
            }
        ...
        private void getNextList() {
            if (returnedUnprocessedNodes.isEmpty()) {
                throw new IllegalStateException(
                        "Please check your mapping file. No attribute found for parentLabel: '"
                                + unprocessedTreeNodes.toString() + "' or root label has no " +
                                "matching children.");
            }
            ...
            if (!listFound) {
                throw new IllegalStateException("Error in tree structure.  No created elements " +
                        "link to unprocessed children elements");
            }
