...
public class XPath extends XPathUtil {
  ...
  /**
   * Sets the value of the attribute of <code>att</code> addressed by <code>xpath</code> and of type
   * <code>targetNodeType</code> to be <code>value</code> with id <code>id</code>.
   *
   ...
   * @param id the identifier of the attribute addressed by <code>xpath</code>, might be <code>null
   *     </code>
  ...
  public Attribute set(
    ...
    for (; stepsIterator.hasNext(); ) {
      ...
      if (_parentType.getName().equals(XSSchema.ANYTYPE_TYPE.getName())
        ...
        if (!isLastStep || targetNodeType == null) {
          if (null == attributeName.getNamespaceURI()) {
            currStepDescriptor =
                (AttributeDescriptor)
                    Types.findDescriptor(parentType, attributeName.getLocalPart());
          } else {
            currStepDescriptor =
                (AttributeDescriptor) Types.findDescriptor(parentType, attributeName);
          }
      ...
      if (isLastStep) {
        ...
        if (currStepDescriptor == null) {
          throw new IllegalArgumentException(
              currStep + " is not a valid location path for type " + _parentType.getName());
        }

        return setLeafAttribute(
            currStepDescriptor, currStep, id, value, parent, targetNodeType, isXlinkRef);
      } else {
  ...
  private Attribute setSimpleContentValue(Attribute attribute, Object value) {
    ...
    if (attribute instanceof ComplexAttribute) {
      simpleContent =
          ((ComplexAttribute) attribute).getProperty(ComplexFeatureConstants.SIMPLE_CONTENT);
    }
            ...
            if (isEmpty(convertedValue)) {
              ...
              if (index > -1) {
                ...
                for (Attribute stepValue : values) {
                  Object mappedIndex =
                      stepValue.getUserData().get(ComplexFeatureConstants.MAPPED_ATTRIBUTE_INDEX);
                  if (mappedIndex == null) {
              ...
              for (Attribute stepValue : values) {
                ...
                if (index > -1) {
                  if (stepValue
                      .getUserData()
                      .containsKey(ComplexFeatureConstants.MAPPED_ATTRIBUTE_INDEX)) {
                    sameIndex =
                        (index
                            == Integer.parseInt(
                                String.valueOf(
                                    stepValue
                                        .getUserData()
                                        .get(ComplexFeatureConstants.MAPPED_ATTRIBUTE_INDEX))));
                  }
                }
   ...
   * chaining is used for simple contents, such as gml:name.. therefore the iterator would create a
   * list of features containing the simple content attributes.
   *
   ...
   * attribute descriptor. If it is, then it is a feature chained feature with only simple content.
   *
   ...
   *
   * <p>Passed in value is converted to a string and then stored in the special <code>simpleContent
   * </code> attribute.
   *
   * @param type
   * @param value
  ...
  private Attribute buildSimpleContentInternal(AttributeType simpleContentType, Object value) {
    ...
    AttributeDescriptor descriptor =
        new AttributeDescriptorImpl(
            simpleContentType, ComplexFeatureConstants.SIMPLE_CONTENT, 1, 1, true, (Object) null);
