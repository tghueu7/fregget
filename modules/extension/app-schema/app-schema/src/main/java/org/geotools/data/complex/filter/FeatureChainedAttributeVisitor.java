...
public class FeatureChainedAttributeVisitor extends DefaultExpressionVisitor {
        ...
        if (nestedAttr.isConditional() && feature == null) {
          ...
          if (nestedType != null) {
            ...
            if (xpathContainsNestedType || hasSimpleContent) {
              ...
              int startIdx =
                  (xpathContainsNestedType) ? nestedTypeXPath.size() : currentXPath.size();
   ...
   *       feature types chain
   *   <li>the path of the attribute, relative to the last linked feature type in the chain (except
   *       when the last chaining is done by reference, in which case the path refers to the second
   *       last feature type)
   * </ol>
   *
   * @author Stefano Costa, GeoSolutions
  ...
  public static class FeatureChainedAttributeDescriptor {
     ...
     * chaining is done by reference (via an xlink:href attribute), in which case the second last
     * feature type is returned.
     *
     * @return
     */
  ...
  /**
   * Represents a single link in the "chain" of feature types that need to be linked to go from the
   * root type to a nested attribute.
   *
  ...
  public static class FeatureChainLink {
    ...
    /**
     * Gets the mapping configuration of the attribute holding the next nested feature in the chain,
     * cast to the specified {@link NestedAttributeMapping} subclass.
     *
     ...
     * chaining-by-reference mapping, i.e. nested feature is not fully encoded inline, only <code>
     * xlink:href</code> attribute is set.
     *
    ...
    /**
     * Returns <code>true</code> if this link refers to a nested feature type which in turn contains
     * another nested feature.
     *
     * @return <code>true</code> if there is another nested feature in the chain, <code>false</code>
     *     otherwise
     */
    ...
    /**
     * Unique identifier of a link in the chain; mainly useful when SQL encoding the feature chained
     * attribute.
     *
