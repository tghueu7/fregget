...
public abstract class AbstractMappingFeatureIterator implements IMappingFeatureIterator {
  ...
  /**
   * maxFeatures restriction value as provided by query. After the data query has run, *this* limit
   * is also applied to the result.
   */
  protected final int requestMaxFeatures;

  /**
   * maximum number of features to request when running the data(base?) query. For denormalised data
   * sources, this neesd to be be Query.DEFAULT_MAX to trigger a full table scan. In all other cases
   * it will be the same value as requestMaxFeatures
   */
   ...
   *     id by evaluating the corresponding <code>org.geotools.filter.Expression</code> from <code>
   *     sourceInstance</code>.
