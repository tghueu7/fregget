...
public class DataAccessMappingFeatureIterator extends AbstractMappingFeatureIterator {
    ...
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping mapping,
                                            Query query, boolean isFiltered, boolean 
                                                    removeQueryLimitIfDenormalised) throws 
            IOException {
    ...
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping mapping,
                                            Query query, boolean isFiltered, boolean 
                                                    removeQueryLimitIfDenormalised, boolean 
                                                    hasPostFilter) throws IOException {
        this(store, mapping, query, isFiltered, removeQueryLimitIfDenormalised, hasPostFilter, 
                null);
    }
    ...
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping mapping,
                                            ...
                                            boolean hasPostFilter, Transaction transaction) 
            throws IOException {
    ...
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping 
            mapping, Query query) throws IOException {
     ...
     */
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping mapping,
                                            Query query, Query unrolledQuery, boolean 
                                                    removeQueryLimitIfDenormalised) throws 
            IOException {
    ...
    public DataAccessMappingFeatureIterator(AppSchemaDataAccess store, FeatureTypeMapping mapping,
                                            Query query, Query unrolledQuery, boolean 
                                                    removeQueryLimitIfDenormalised, Transaction 
                                                    transaction) throws IOException {
        super(store, mapping, query, unrolledQuery, removeQueryLimitIfDenormalised, false, 
                transaction);
    }
    ...
    protected boolean checkForeignIdValues(List<Object> foreignIdValues, Feature next) {
        if (foreignIds != null) {
            for (int i = 0; i < foreignIds.size(); i++) {
                if (!peekValue(next, foreignIds.get(i)).toString().equals(foreignIdValues.get(i)
                        .toString())) {
                    return false;
                }
            }
        ...
        if (Expression.NIL.equals(idExpression) || idExpression instanceof Literal) {
            ...
            if (source instanceof Feature) {
                for (Property p : ((Feature) source).getProperties()) {
                    if (p.getName().getLocalPart().startsWith(JoiningJDBCFeatureSource
                            .PRIMARY_KEY)) {
                        ids.add(p.getValue());
                    }
                }
    ...
    protected void initialiseSourceFeatures(FeatureTypeMapping mapping, Query query,
        ...
        if (query instanceof JoiningQuery) {
            ...
            if (mappedSource instanceof JDBCFeatureSource) {
                ...
                throw new IllegalArgumentException("Joining queries are only supported on JDBC " +
                        "data stores");
            }

            // we have a database backend and Joining is enabled: make sure a transaction is 
            // available
            // to allow downstream iterators (i.e. iterators going through nested features) to 
            // share the
            ...
            if (this.transaction != null) {
                // transaction provided by calling code: iterator shall not consider itself owner
                // of the
        ...
        String version = (String) this.mapping.getTargetFeature().getType().getUserData().get
                ("targetVersion");
        ...
        if (targetCRS == null && version != null && !version.contains("wms")) {
            ...
            Object crsobject = this.mapping.getTargetFeature().getType().getUserData().get
                    ("targetCrs");
    ...
    protected Attribute setAttributeValue(Attribute target, String id, final Object source,
                                          final AttributeMapping attMapping, Object values, 
                                          StepList inputXpath, List<PropertyName> 
                                                  selectedProperties) throws IOException {
        ...
        if (attMapping.isNestedAttribute()) {
            ...
            if (mappingName != null) {
                if (nestedMapping.isSameSource() && mappingName instanceof Name) {
                    ...
                    return setPolymorphicValues((Name) mappingName, target, id, nestedMapping, 
                            source,
        ...
        if (isNestedFeature) {
            ...
            if (values instanceof Collection) {
                ...
                for (Object val : (Collection) values) {
                    ...
                    if (isHRefLink) {
                        ...
                        nestedFeatures.addAll(((NestedAttributeMapping) attMapping)
                                .getInputFeatures(this, val, getIdValues(source), source, 
                                        reprojection, selectedProperties, includeMandatory));
                    } else {
                        nestedFeatures.addAll(((NestedAttributeMapping) attMapping).getFeatures(
                                this, val, getIdValues(source), reprojection, source, 
                                selectedProperties, includeMandatory, newResolveDepth, 
                                resolveTimeOut));
                    }
                }
                ...
                // eg. gsml:GeologicUnit/gsml:occurence/gsml:MappedFeature
                // and gsml:MappedFeature/gsml:specification/gsml:GeologicUnit
                values = ((NestedAttributeMapping) attMapping).getInputFeatures(this, values, 
                        getIdValues(source), source, reprojection, selectedProperties, 
                        includeMandatory);
            } else {
                values = ((NestedAttributeMapping) attMapping).getFeatures(this, values, 
                        getIdValues(source), reprojection,
                        source, selectedProperties, includeMandatory, newResolveDepth, 
                        resolveTimeOut);
            }
            if (isHRefLink) {
        ...
        if (values instanceof Collection) {
            ...
            for (Object singleVal : (Collection) values) {
                ...
                if (singleVal instanceof Attribute) {
                    // copy client properties from input features if they're complex features
                    // wrapped in app-schema data access
                    Map<Name, Expression> valueProperties = getClientProperties((Attribute) 
                            singleVal);
                ...
                instance = setAttributeContent(target, xpath, valueList, id, targetNodeType, 
                        false, sourceExpression, source, clientPropsMappings, ignoreXlinkHref);
            }
        } else {
            ...
            instance = setAttributeContent(target, xpath, values, id, targetNodeType, false, 
                    sourceExpression, source, clientPropsMappings, ignoreXlinkHref);
    ...
    private Attribute setPolymorphicReference(String uri,
                                              ...
                                              Attribute target, StepList xpath,
    ...
    private Attribute setPolymorphicValues(Name mappingName, Attribute target, String id,
                                           NestedAttributeMapping nestedMapping, Object source, 
                                           StepList xpath,
                                           Map<Name, Expression> clientPropsMappings) throws 
            IOException {
        ...
        if (da instanceof AppSchemaDataAccess) {
            ...
            for (AttributeMapping mapping : polymorphicMappings) {
                ...
                setAttributeValue(instance, null, source, mapping, null, null, selectedProperties
                        .get(mapping));
            }
            return instance;
        }
    ...
    protected void setXlinkReference(Attribute target, Map<Name, Expression> clientPropsMappings,
        ...
        for (Object singleVal : (Collection) value) {
            ...
            Collection<Property> existingAttributes = getProperties((ComplexAttribute) target, 
                    xpath);
    ...
    protected List<Feature> setNextFeature(String fId, List<Object> foreignIdValues) throws 
            IOException {
        ...
        while (getSourceFeatureIterator().hasNext()) {
            ...
            if (extractIdForFeature(next).equals(fId) && checkForeignIdValues(foreignIdValues, 
                    next)) {
                // HACK HACK HACK
                // evaluate filter that applies to this list as we want a subset
                // instead of full result
    ...
    public void skipNestedMapping(AttributeMapping attMapping, List<Feature> sources) throws 
            IOException {
        if (attMapping instanceof JoiningNestedAttributeMapping) {

            for (Feature source : sources) {
                Object value = getValues(attMapping.isMultiValued(), attMapping
                        .getSourceExpression(), source);

                if (value instanceof Collection) {
                    for (Object val : (Collection) value) {
                        ((JoiningNestedAttributeMapping) attMapping).skip(this, val, getIdValues
                                (source));
                    }
                } else {
                    ((JoiningNestedAttributeMapping) attMapping).skip(this, value, getIdValues
                            (source));
                }
            }
        }
    ...
    private GeometryDescriptor reprojectGeometry(GeometryDescriptor descr) {
        ...
        GeometryType type = ftf.createGeometryType(descr.getType().getName(), descr.getType()
                .getBinding(), reprojection, descr.getType().isIdentified(), descr.getType()
                .isAbstract(), descr.getType().getRestrictions(), descr.getType().getSuper(), 
                descr.getType().getDescription());
        type.getUserData().putAll(descr.getType().getUserData());
        GeometryDescriptor gd = ftf.createGeometryDescriptor(type, descr.getName(), descr
                .getMinOccurs(), descr.getMaxOccurs(), descr.isNillable(), descr.getDefaultValue());
    ...
    private FeatureType reprojectType(FeatureType type) {
        ...
        if (type instanceof NonFeatureTypeProxy) {
            ft = new NonFeatureTypeProxy(((NonFeatureTypeProxy) type).getSubject(), mapping, 
                    schema);
        } else {
            ft = ftf.createFeatureType(type.getName(), schema, reprojectGeometry(type
                    .getGeometryDescriptor()), type.isAbstract(), type.getRestrictions(), type
    ...
    private AttributeDescriptor reprojectAttribute(AttributeDescriptor descr) {
        ...
        if (reprojection != null && descr.getType() instanceof FeatureType) {
            AttributeDescriptor ad = ftf.createAttributeDescriptor(reprojectType((FeatureType) 
                    descr.getType()), descr.getName(), descr.getMinOccurs(), descr.getMaxOccurs()
                    , descr.isNillable(), descr.getDefaultValue());
            ...
            try {
                ...
                if (attMapping.isList()) {
                    ...
                    setAttributeValue(target, null, sources.get(index), attMapping, null, null, 
                            selectedProperties.get(attMapping));
                    // When a feature is not multi-valued but still has multiple rows with the 
                    // same ID in
                    // a denormalised table, by default app-schema only takes the first row and 
                    // ignores
                    // the rest (see above). The following line is to make sure that the cursors 
                    // in the
                    // 'joining nested mappings'skip any extra rows that were linked to those 
                    // rows that are being ignored.
                    ...
                    // the first row). This never really occurs in practice I have noticed, but 
                    // it is a theoretic
                    // possibility, as there is no requirement for the foreign key to be unique 
                    // per id.
        ...
        if (sourceFeatures != null && getSourceFeatureIterator() != null) {
            ...
            if (transaction != null && isTransactionOwner) {
                try {
                    ...
                    LOGGER.log(Level.SEVERE, "Exception occurred closing transaction: " + e
                            .getMessage(), e);
                }
            }
        }
    ...
    private CoordinateReferenceSystem getDeclaredCrs(CoordinateReferenceSystem nativeCRS,
        ...
        try {
            ...
            if (wfsVersion.equals("1.0.0")) {
                ...
                //it's possible that we can't do the CRS -> code -> CRS conversion...so we'll 
                // just return what we have
                if (code == null) return nativeCRS;
                return CRS.decode("urn:x-ogc:def:crs:EPSG:6.11.2:" + code);
            }
        } catch (Exception e) {
            throw new UnsupportedOperationException("We have had issues trying to flip axis of " 
                    + nativeCRS, e);
        }
    }
