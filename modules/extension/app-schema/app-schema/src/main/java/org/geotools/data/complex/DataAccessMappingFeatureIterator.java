...
public class DataAccessMappingFeatureIterator extends AbstractMappingFeatureIterator {
  ...
  /**
   * Only used for Joining, to make sure that rows with different foreign id's aren't interpreted as
   * one feature and merged.
   */
  ...
  /**
   * Special handling for polymorphic mapping where the value of the attribute determines that this
   * attribute should be a placeholder for an xlink:href.
   *
  ...
  protected void setXlinkReference(
    ...
    for (Object singleVal : (Collection) value) {
      ...
      if (!exists) {
        Attribute instance =
            xpathAttributeBuilder.set(target, xpath, null, null, targetNodeType, true, null);
  ...
  private CoordinateReferenceSystem getDeclaredCrs(
    ...
    try {
      ...
      if (wfsVersion.equals("1.0.0")) {
        ...
        // it's possible that we can't do the CRS -> code -> CRS conversion...so we'll just return
        // what we have
