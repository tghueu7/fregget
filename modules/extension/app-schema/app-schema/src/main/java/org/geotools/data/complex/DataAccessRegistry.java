...
public class DataAccessRegistry implements Repository {
  ...
  private void disposeHiddenDataAccessInstances() throws IOException {
    // step 1: collect all hidden data access instances that are still referenced by some other data
    // access
          ...
          canSafelyRemove =
              canSafelyRemove
                  && reachOutToReferencedDataAccesses(
                      asda, stillReferencedHiddenDataAccesses, reachedDataAccesses);
      ...
      if (nestedMappings != null) {
        for (NestedAttributeMapping nestedAttr : nestedMappings) {
          ...
          if (!nestedAttr.isConditional()) {
            ...
            Name nestedTypeName =
                Types.degloseName(nestedTypeNameAsString, nestedAttr.getNamespaces());
            try {
              ...
              if (refDA instanceof AppSchemaDataAccess) {
                ...
                if (!reachedDataAccessInstances.contains(refASDA)) {
                  ...
                  if (!reachOutToReferencedDataAccesses(
                      refASDA, stillReferencedDataAccessInstances, reachedDataAccessInstances)) {
                    return false;
                  }
                }
              }
  ...
  /**
   * Dispose and unregister all data accesses in the registry. This is may be needed to prevent unit
   * tests from conflicting with data accesses with the same type name registered for other tests.
   */
   ...
   * mapping has mappingName, then it will be the key that is matched in the search. If it doesn't,
   * then it will match the targetElementName.
   *
  ...
  /**
   * Return true if a type name is mapped in one of the registered app-schema data accesses. If the
   * type mapping has mappingName, then it will be the key that is matched in the search. If it
   ...
   * Unregister * and dispose * all data accesses in the registry. This is may be needed to prevent
   * unit tests from conflicting with data accesses with the same type name registered for other
   * tests. @Deprecated use unregisterAndDisposeAll
   */
  @Deprecated
