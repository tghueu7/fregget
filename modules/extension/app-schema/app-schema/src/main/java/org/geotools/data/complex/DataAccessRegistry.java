...
public class DataAccessRegistry implements Repository {
     ...
     */
    protected List<DataAccess<FeatureType, Feature>> registry = new 
            ArrayList<DataAccess<FeatureType, Feature>>();
     ...
     * @throws IOException
     */
    public synchronized FeatureSource<FeatureType, Feature> featureSource(Name name) throws 
            IOException {
    ...
    public DataStore dataStore(Name name) {
        throw new UnsupportedOperationException("Simple feature DataStores not supported by " +
                "app-schema registry.");
    }
    ...
    // directive of some top-level app-schema data access) that are no longer needed (i.e. they 
    // are not referenced by any top-level data access).
    private void disposeHiddenDataAccessInstances() throws IOException {
        // step 1: collect all hidden data access instances that are still referenced by some 
        // other data access
        ...
        if (canSafelyRemove) {
            List<DataAccess<FeatureType, Feature>> copyRegistry = new 
                    ArrayList<DataAccess<FeatureType, Feature>>(
                    registry);
    ...
    private boolean reachOutToReferencedDataAccesses(AppSchemaDataAccess asda,
                                                     ...
                                                     Set<DataAccess<?, ?>> 
                                                             reachedDataAccessInstances) throws 
            IOException {
            ...
            if (nestedMappings != null) {
                for (NestedAttributeMapping nestedAttr : nestedMappings) {
                    ...
                    if (!nestedAttr.isConditional()) {
                        ...
                        LOGGER.finer("Polymorphic mapping found, disabling automatic disposal of " +
                                "hidden data accesses");
                        return false;
                    }
                }
            }
    ...
    public synchronized void disposeAndUnregisterAll() {
        List<DataAccess<FeatureType, Feature>> copyRegistry = new 
                ArrayList<DataAccess<FeatureType, Feature>>(registry);
        for (DataAccess<FeatureType, Feature> da : copyRegistry) {
            da.dispose();
        }
        registry.clear();
    ...
    public static DataAccess<FeatureType, Feature> getDataAccess(Name featureTypeName) throws 
            IOException {
