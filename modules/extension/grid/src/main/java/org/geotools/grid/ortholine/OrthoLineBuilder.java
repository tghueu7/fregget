...
public class OrthoLineBuilder {
   ...
   * into the provided {@link ListFeatureCollection}. Densified lines (lines strings with additional
  ...
  public void buildGrid(
    ...
    doBuildLineFeatures(
        horizontal, LineOrientation.HORIZONTAL, lineFeatureBuilder, densify, vertexSpacing, fc);
    doBuildLineFeatures(
    ...
    if (NDEFS > 0) {
      ...
      while (numActive > 0) {
        ...
        for (int i = 0; i < NDEFS; i++) {
          if (generate[i]) {
            OrthoLine element =
                new OrthoLine(gridBounds, orientation, pos[i], lineDefs.get(i).getLevel());
