...
public class Envelopes {
   ...
   * such that they are multiples of the specified resolution. For example, if {@code resolution} is
   * 100 then the min and max bounding coordinates of this envelope will set to mutliples of 100 by
   * rounding down the min values and rounding up the max values if required.
   *
   * <pre><code>
  ...
  /**
   * Helper method to round ordinate values up or down to a specified resolution. The returned value
   * will be a multiple of the specified resolution.
   *
   * <pre><code>
