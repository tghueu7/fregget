...
public class StyleGenerator {
  ...
  /**
   * Obtains the colour for the indexed rule. If an else rule is also to be created from the colour
   * palette, the appropriate offset is applied.
   *
   * @param index
   */
  ...
  public static FeatureTypeStyle createFeatureTypeStyle(
    ...
    if (elseMode != ELSEMODE_IGNORE) {
      Symbolizer symb =
          createSymbolizer(
              geometryAttrType, getElseColor(elseMode, colors), opacity, defaultStroke);
  ...
  private static Rule createRuleRanged(
    ...
    // create a symbolizer
    Symbolizer symb =
        createSymbolizer(geometryAttrType, getColor(elseMode, colors, i), opacity, defaultStroke);
  ...
  public static void modifyFTS(FeatureTypeStyle fts, int ruleIndex, String styleExpression)
    ...
    if (filter instanceof And) { // ranged expression
      ...
      thisRule.setFilter(
          filter); // style events don't handle filters yet, so fire the change event for filter
  ...
  public static Filter[] toFilter(
    ...
    for (int i = 0; i < styleExpression.length; i++) {
      ...
      if (isRangedExpr[i]) {
        ...
        filter[i] =
            toRangedFilter(
                styleExpression[i], featureType[i], attributeTypeName[i], upperBoundClosed);
      } else { // specific
