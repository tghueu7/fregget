...
public abstract class TileService {
  ...
  public Set<Tile> findTilesInExtent(
    ...
    Tile firstTile =
        tileFactory.findTileAtCoordinate(extent.getMinX(), extent.getMaxY(), zoomLevel, this);
    ...
    do {
      ...
      do {
        ...
        if (tileList.size() > maxNumberOfTiles) {
          LOGGER.warning(
              "Reached tile limit of " + maxNumberOfTiles + ". Returning an empty collection.");
          return Collections.emptySet();
        }
   ...
   *
   * <p>The cache used here is a soft cache, which has an un-controllable time to live (could last a
   * split seconds or 100 years).
   *
   ...
   * MinX: -180°
   *
   * <p>But cutExtentIntoTiles(..) requires an extent that looks like this: MaxY: 85° (or 90°) MinY:
   * -85° (or -90°) MaxX: 180° MinX: -180°
   *
   * @param envelope
   * @return
   */
