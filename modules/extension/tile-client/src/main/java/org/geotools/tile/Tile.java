...
public abstract class Tile implements ImageLoader {
  ...
  /**
   * These are the states of the tile. This state represents if the tile needs to be re-rendered or
   * not. A state of new or invalid means the tile should be re-rendered
   */
   ...
   * stack no longer matches the rendering stack the user has defined and the rendering stack needs
   * to be updated.
   */
   ...
   * needed synchronized (SWTLock) { // if the SWTImage is created once the lock is gained, exit if
   * (swtImage != null && !swtImage.isDisposed()) { return; } // otherwise try creating the SWTImage
   * now try { BufferedImage buffImage = getBufferedImage(); swtImage =
   ...
   *
   * <p>This is used with other information to determine if a tile can be disposed of. Valid values
   * include:
   *
   * <ul>
   ...
   *
   * <p>This is used in conjunction with the screen state to determine it a tile can be disposed of.
