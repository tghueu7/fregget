...
public final class BingTileUtil {
  ...
  public static int[] lonLatToPixelXY(double longitude, double latitude, int zoomLevel) {
    double _latitude =
        clip(latitude, WebMercatorTileService.MIN_LATITUDE, WebMercatorTileService.MAX_LATITUDE);
    double _longitude =
        clip(longitude, WebMercatorTileService.MIN_LONGITUDE, WebMercatorTileService.MAX_LONGITUDE);
  ...
  /**
   * <q>Converts a pixel from pixel XY coordinates at a specified level of detail [i.e. zoom level]
   * into latitude/longitude WGS-84 coordinates (in degrees).</q>
   *
  ...
  /**
   * <q>Converts pixel XY coordinates into tile XY coordinates of the tile containing the specified
   * pixel.</q>
   *
  ...
  public static ReferencedEnvelope getTileBoundingBox(double lon, double lat, int zoomLevel) {
    ...
    Envelope envelope =
        new Envelope(
            topLeftCoords[0], bottomRightCoords[0], topLeftCoords[1], bottomRightCoords[1]);
