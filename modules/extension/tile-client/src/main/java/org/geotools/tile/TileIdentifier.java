...
public abstract class TileIdentifier {
   ...
   * "/", etc. The id should be build from the code (which also uniquely identifies a tile, but, in
   * some service implementation may contain file-unfriendly characters (e.g. OpenStreetMap:
   * 5/16/10.png).
   *
   * <p>When building an id, you should use the service name as a prefix (e.g. for OpenStreetMap:
   * "Mapnik", "CycleMap"; Bing Maps: "Road", "Hybrid"; etc) and suffix the id with a file-friendly
   * string (e.g. OpenStreetMap: "Mapnik_X_Y_Z").
   *
   * @return
   */
   ...
   *
   * <p>The id is a string which uniquely identifies a tile. In some service implementations this is
   * a quadkey (e.g. Bing Maps: "03123") or the fragment of the tile image (e.g. OpenStreetMap:
