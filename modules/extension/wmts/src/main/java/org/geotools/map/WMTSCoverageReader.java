...
public class WMTSCoverageReader extends AbstractGridCoverage2DReader {
    ...
    if (srsName == null) { // initialize from first (unique) layer
      ...
      if (srsName == null) {
        if (owsLayer.getSrs().isEmpty()) {
          // force 4326
          if (LOGGER.isLoggable(Level.INFO)) LOGGER.info("adding default CRS to: " + srsName);
          srsName = "EPSG:4326";
  ...
  protected void renderTiles(
    ...
    for (Tile tile : tiles) {
      ...
      try {
        tileEnvViewport =
            nativeTileEnvelope.transform(viewportExtent.getCoordinateReferenceSystem(), true);
