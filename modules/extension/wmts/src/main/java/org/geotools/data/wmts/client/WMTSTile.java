...
class WMTSTile extends Tile {
  ...
  static {
    ...
    if (size != null) {
      try {
        ...
        LOGGER.info("Bad " + WMTS_TILE_CACHE_SIZE_PROPERTY_NAME + " property '" + size + "'");
      }
    }
  ...
  private URL getRESTurl(String baseUrl, TileIdentifier tileIdentifier) throws RuntimeException {
    ...
    baseUrl =
        replaceToken(baseUrl, "time", service.getDimensions().get(WMTSTileService.DIMENSION_TIME));

    if (LOGGER.isLoggable(Level.FINE)) LOGGER.fine("Requesting tile " + tileIdentifier.getCode());
  ...
  public BufferedImage doLoadImageTileImage(Tile tile) throws IOException {
    ...
    Map<String, String> headers =
        (Map<String, String>) this.service.getExtrainfo().get(WMTSTileService.EXTRA_HEADERS);
    InputStream is = null;
    try {
