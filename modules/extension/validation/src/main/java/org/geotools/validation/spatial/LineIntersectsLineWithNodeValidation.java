                        ...
                        if (envelope.contains(rLineGeom.getEnvelopeInternal())) {
                            if (LineString.class.isAssignableFrom(rLineGeom.getClass())) {
                                if (lineGeom.intersects(rLineGeom)) {
                                    if (!hasPair(((LineString) lineGeom).getCoordinateSequence(),
                                            ((LineString) rLineGeom).getCoordinateSequence())) {
                                        results.error(rLine, "Line does not intersect line at " +
                                                "node covered by the specified Line.");
                                        r = false;
                                    }
                                } else {
                                ...
                                results.warning(rLine, "Invalid type: this feature is not a " +
                                        "derivative of a LineString");
                            }
                        } else {
                            fcRLine.remove(rLine);
                        }
