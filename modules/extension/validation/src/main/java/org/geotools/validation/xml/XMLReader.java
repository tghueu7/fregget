...
public class XMLReader {
  ...
  public static PlugInDTO readPlugIn(Reader inputSource) throws ValidationException {
    ...
    try {
      ...
      try {
        dto.setName(
            ReaderUtils.getElementText(ReaderUtils.getChildElement(elem, "name", true), true));
      ...
      try {
        dto.setClassName(
            ReaderUtils.getElementText(ReaderUtils.getChildElement(elem, "class", true), true));
      } catch (SAXException e) {
        throw new ValidationException(
      ...
      if ((nl == null) || (nl.getLength() == 0)) {
        ...
        for (int i = 0; i < nl.getLength(); i++) {
          try {
            ...
            throw new ValidationException(
                "An error occured loading a test in " + dto.getName() + " test suite.", e);
          } catch (Throwable t) {
            throw new ValidationException(
                "Could not load test suite " + dto.getName() + ":" + t.getMessage(), t);
          }
        }
      }
    } catch (IOException e) {
