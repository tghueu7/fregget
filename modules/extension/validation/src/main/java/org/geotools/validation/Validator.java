...
public class Validator {
  ...
  /**
   * Integrity validation will iterate over all data stores passed in through the stores map and run
   * the tests associated with each store. Author: bowens<br>
   * Created on: Jun 26, 2004<br>
   *
  ...
  public void integrityValidation(
    ...
    for (Iterator i = typeRefs.iterator(); i.hasNext(); ) {
       ...
       * already if we are in a transaction operation. If this is for the "do it" button, there will
       * be no feature stores already loaded and thus we always hit the 'else' statement.
       */
