...
public interface Validation {
   ...
   * <p>This is a Priority Hint to Validation Processor based on computational expense.
   *
   * <p>Used for simple attribute or geometry validations, basically anything that is Order(N) like
   * a Bounds check.
   */
   ...
   * O(N^2).
   *
   * <p>Any integrity tests involving network code, were we have to build the network and then walk
   * it, is an example of PRIORITY_INVOLVED.
   */
   ...
   *
   * <p>Used to denote that all FeatureTypes should be tested. (If this does not Pan out we may have
   * to define the wild-card character "" or support Regex based typeName matching.
   */
   ...
   *
   * <p>If this list is empty the ValidationProcess will run the test against all FeatureTypes. The
   * ValidationProcess expects these names to be in the format dataStoreId:typeName.
   *
