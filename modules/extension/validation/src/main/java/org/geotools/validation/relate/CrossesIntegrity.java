...
public class CrossesIntegrity extends RelationIntegrity {
    ...
    /* (non-Javadoc)
     * @see org.geotools.validation.IntegrityValidation#validate(java.util.Map, com
     * .vividsolutions.jts.geom.Envelope, org.geotools.validation.ValidationResults)
     */
    public boolean validate(Map layers, ReferencedEnvelope envelope,
        ...
        if (typeRef2 == EMPTY || typeRef1.equals(typeRef2))
            ...
            return validateMultipleLayers(geomSource1, geomSource2, isExpected(), results, 
                    envelope);
        }
     ...
     * It creates iterators over both filtered FeatureSources. It calls overlaps() and contains()
     * using the
            ...
            while (fr1.hasNext()) {
                ...
                while (fr2 != null && fr2.hasNext()) {
                    ...
                    if (g1.overlaps(g2) != expected || g1.contains(g2) != expected) {
                        results.error(f1, ((Geometry) f1.getDefaultGeometry()).getGeometryType() 
                                + " " + getGeomTypeRefA() + " overlapped " + getGeomTypeRefB() + 
                                "(" + f2.getID() + "), Result was not " + expected);
                        success = false;
                    }
                }
            }
     ...
     * It creates iterators over the filtered FeatureSource. It calls overlaps() and contains() 
     * using the
...
//				System.out.println("env1 = " + bBox);
                ...
                Geometry g1 = (Geometry) f1.getDefaultGeometry();
                Filter filter2 = filterBBox(ReferencedEnvelope.reference(g1.getEnvelope()
                        .getEnvelopeInternal()), ft);
                ...
                while (fr2 != null && fr2.hasNext()) {
                    ...
                    if (!usedIDs.contains(f2.getID())) {
                        ...
                        if (!f1.getID().equals(f2.getID()))    // if they are the same feature, 
                            // move onto the next one
                        {
                            if (g1.crosses(g2) != expected) {
                                ...
                                // "+expected );
                                results.error(f1, getGeomTypeRefA() + "(" + f1.getID() + ")" + " " +
                                        "crossed " + getGeomTypeRefA() + "(" + f2.getID() + ")");
                                System.out.println(((Geometry) f1.getDefaultGeometry())
                                        .getGeometryType() + " " + getGeomTypeRefA() + "(" + 
                                        f1.getID() + ")" + " crossed " + getGeomTypeRefA() + "(" 
                                        + f2.getID() + "), Result was not " + expected);
                                success = false;
                            }
                        }
