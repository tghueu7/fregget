...
public class ValidationProcessor {
   ...
   * addToLookup
   *
   * <p>Description: Add the validation test to the map for every featureType that it validates. If
   * the FeatureTypes array is ALL, then the validation is added to the ANYTYPENAME entry.
   *
   * @param validation
   */
   ...
   * FeatureCollection. The first validation test lookup checks to see if there are any validations
   * that are to be performed on every FeatureTypeInfo. An example of this could be an isValid()
   * test on all geometries in all FeatureTypes. Once those tests have been gathered, a lookup is
   * performed on the TypeName of the FeatureTypeInfo to check for specific FeatureTypeInfo
   * validation tests. A list of validation tests is returned from each lookup, if any exist. When
   * all the validation tests have been gathered, each test is iterated through then run on each
   * Feature, with the ValidationResults coming along for the ride, collecting error information.
   * Parameter "SimpleFeatureCollection collection" should be changed later to take in a
  ...
  public void runFeatureTests(
      String dsID, SimpleFeatureCollection collection, ValidationResults results) throws Exception {
   ...
   * FeatureTypeInfo. An example of this could be a uniqueID() test on a unique column value in all
   * FeatureTypes. Once those tests have been gathered, a lookup is performed on the TypeName of the
   * FeatureTypeInfo to check for specific Integrity validation tests. A list of validation tests is
   * returned from each lookup, if any exist. When all the validation tests have been gathered, each
   * test is iterated through then run on each Feature, with the ValidationResults coming along for
   * the ride, collecting error information.
   *
   ...
   *
   * <p>This is mostly useful for testing, you may want to write your own load method with enhanced
   * error reporting.
   *
   ...
   *
   * <p>This method is useful for testing, it is not forgiving and will error out if things go bad.
   *
   * @param plugInDTOs
   * @param testSuiteDTOs
   * @throws Exception
    ...
    for (Iterator i = plugInNames.iterator(); i.hasNext(); ) {
      ...
      if (plugInClass == null) {
        throw new ClassNotFoundException(
            "Could class for " + plugInName + ": class " + dto.getClassName() + " not found");
      }
