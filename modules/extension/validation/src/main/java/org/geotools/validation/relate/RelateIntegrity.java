...
public class RelateIntegrity extends RelationIntegrity {
     ...
     * match the elements in intersectionPattern  , which may be:
     * 0
     * 1
     * 2
     * T ( = 0, 1 or 2)
                ...
                while (fr2 != null && fr2.hasNext()) {
                    ...
                    if (g1.relate(g2, de9im) != expected) {
                        results.error(f1, ((Geometry) f1.getDefaultGeometry()).getGeometryType() 
                                + " " + getGeomTypeRefA() + " failed RELATE on " + 
                                getGeomTypeRefB() + "(" + f2.getID() + "), Result was not " + 
                                expected);
                        success = false;
                    }
                }
                ...
                while (fr2 != null && fr2.hasNext()) {
                        ...
                        // onto the next one
                    {
                        if (g1.overlaps(g2) != expected || g1.contains(g2) != expected) {
                            results.error(f1, ((Geometry) f1.getDefaultGeometry())
                                    .getGeometryType() + " " + getGeomTypeRefA() + " overlapped "
