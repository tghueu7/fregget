...
public class TransformTest extends TestCase {
  ...
  private void assertEquals(GeometryImpl geom1, GeometryImpl geom2, double epsilon) {
    ...
    if (geom1 instanceof PointImpl && geom2 instanceof PointImpl) {
      ...
      for (int i = 0; i < point1.getPosition().getCoordinate().length; i++) {
        assertEquals(
            point1.getPosition().getOrdinate(i), point2.getPosition().getOrdinate(i), epsilon);
      }
  ...
  public void testPoint() throws Exception {
    ...
    PointImpl point1 =
        (PointImpl)
            primitiveFactory.createPoint(new double[] {-123.47009555832284, 48.543261561072285});
    ...
    PointImpl expectedPoint2 =
        (PointImpl) expectedPrimF2.createPoint(new double[] {1187128.000000001, 395268.0000000004});
  ...
  public void testRing() throws Exception {
    ...
    points1.add(
        primitiveFactory.createPoint(new double[] {-123.47009555832284, 48.543261561072285}));
    points1.add(
        primitiveFactory.createPoint(new double[] {-123.46972894676578, 48.55009592117936}));
    points1.add(
        primitiveFactory.createPoint(new double[] {-123.45463828850829, 48.54973520267305}));
  ...
  /**
   * We need to ensure that an IdentityTransform is recognized and the result is *equals* (although
   * perhaps not *equalsExact*.
   *
   * <p>Note I am using GeometryBuilder here as I am interested in testing functionality rather than
   * factories.
   *
