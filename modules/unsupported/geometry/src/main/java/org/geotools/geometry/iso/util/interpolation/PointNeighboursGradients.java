...
public class PointNeighboursGradients {
    ...
    public void setAkimaGradient() {
        ...
        for (int i = 0; i < nnbr; ++i) {
            Point2D dp0 = new Point2D.Double(this.mNbrs[i].x - this.mP.x, this.mNbrs[i].y - this
                    .mP.y);
            double dz0 = this.mNbrs[i].z - this.mP.z;
            for (int j = i + 1; j <= this.mNbrs.length - 1; ++j) {
                Point2D dp1 = new Point2D.Double(this.mNbrs[j].x - this.mP.x, this.mNbrs[j].y - 
                        this.mP.y);
    ...
    public void setClosestPoints(ArrayList<Point3d> pIn, int nnbr) {
        ...
        for (int i = 0, j = 0; i < pIn.size(); ++i) {
            ...
            if (this.mP != p1) {
                double dist = (this.mP.x - p1.x) * (this.mP.x - p1.x) + (this.mP.y - p1.y) * 
                        (this.mP.y - p1.y);
                if (j < nnbr) {
                    nbrDist[j] = dist;
                    nbrs.add(p1);
