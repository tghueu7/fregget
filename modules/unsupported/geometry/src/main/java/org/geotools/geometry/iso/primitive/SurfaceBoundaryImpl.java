...
public class SurfaceBoundaryImpl extends PrimitiveBoundaryImpl implements SurfaceBoundary {
  ...
  /**
   * A SurfaceBoundary consists of some number of Rings, corresponding to the various components of
   * its boundary. In the normal 2D case, one of these rings is distinguished as being the exterior
   * boundary. In a general manifold this is not always possible, in which case all boundaries shall
   ...
   * linguistic metaphor that uses the same linguistic constructs for the concept of being inside an
   * object to being inside a container. In normal mathematical terms, the exterior boundary is the
   * one that appears in the Jordan Separation Theorem (Jordan Curve Theorem extended beyond 2D).
   * The exterior boundary is the one that separates the surface (or solid in 3D) from infinite
   * space. The interior boundaries separate the object at hand from other bounded objects. The
   * uniqueness of the exterior comes from the uniqueness of unbounded space. Essentially, the
   ...
   * standard is restricted to at most 3 dimensions.
   *
   * <p>EXAMPLE 1 If the underlying manifold is an infinite cylinder, then two transverse cuts of
   * the cylinder define a compact surface between the cuts, and two separate unbounded portions of
   * the cylinders. In this case, either cut could reasonably be called exterior. In cases of such
   * ambiguity, the standard chooses to list all boundaries in the "interior" set. The only
   * guarantee of an exterior boundary being unique is in the 2-dimensional plane, E2.
   *
   * <p>EXAMPLE 2 Taking the equator of a sphere, and generating a 1 meter buffer, we have a surface
   * with two isomorphic boundary components. There is no unbiased manner to distinguish one of
   * these as an exterior.
   */
