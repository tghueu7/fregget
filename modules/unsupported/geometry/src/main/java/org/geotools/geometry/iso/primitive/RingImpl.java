...
public class RingImpl extends RingImplUnsafe implements Ring {
    ...
    private void checkConsistency(List<OrientableCurve> aGenerator) {
        ...
        if (!oc0.getStartPoint().equals(oc1.getEndPoint()))
            throw new IllegalArgumentException("Start point of first element has to be at the " +
                    "same position as end point of last element"); //$NON-NLS-1$
        ...
        if (!isSimple.isSimple(newCurve))
            throw new IllegalArgumentException("The curve segments are not simple, but " +
                    "intersect"); //$NON-NLS-1$
