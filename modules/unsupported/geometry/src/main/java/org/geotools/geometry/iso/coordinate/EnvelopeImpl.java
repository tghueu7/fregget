...
public class EnvelopeImpl implements Envelope, Serializable {
  ...
  public void add(double[] coord) {
    ...
    for (int i = 0; i < this.getDimension(); ++i) {
      ...
      if (!Double.isNaN(ci) && ((ci < cmini) || Double.isNaN(cmini))) this.pMin.setOrdinate(i, ci);
      if (!Double.isNaN(ci) && ((ci > cmaxi) || Double.isNaN(cmaxi))) this.pMax.setOrdinate(i, ci);
    }
  }
    ...
    if (D == DimensionModel.TWO_DIMENSIONIAL) {
      ...
      rDP =
          new DirectPositionImpl(
              crs,
              new double[] {
                this.pMax.getOrdinate(X), this.pMin.getOrdinate(Y), this.pMin.getOrdinate(Z)
              });
    } else {
    ...
    if (D == DimensionModel.TWO_DIMENSIONIAL) {
      ...
      rDP =
          new DirectPositionImpl(
              crs,
              new double[] {
                this.pMin.getOrdinate(X), this.pMax.getOrdinate(Y), this.pMin.getOrdinate(Z)
              });
    } else {
      throw new UnsupportedDimensionException("3d not supported.");
    }
