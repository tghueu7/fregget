...
public class ITP_Interpolation {
    ...
    private static ArrayList<PointNeighboursGradients> akimaGradient(ArrayList<Point3d> pIn, int 
            nnbr) {
        // transform each node into a node, neighbours and gradients
        ArrayList<PointNeighboursGradients> result = new ArrayList<PointNeighboursGradients>(pIn
                .size());
    ...
    private static void shepardModified(ArrayList<PointNeighboursGradients> pngColl, double q, 
                                        Point3d p_eval) {
        ...
        for (int i = 0; i < pngColl.size(); ++i) {
            ...
            sum0 += (png0.mP.z + AlgoPoint2D.scalar(AlgoPoint2D.subtract(p0, p1), png0.mGradXY)) 
                    * prod;
            sum1 += prod;
        }
        p_eval.z = (sum0 / sum1);
