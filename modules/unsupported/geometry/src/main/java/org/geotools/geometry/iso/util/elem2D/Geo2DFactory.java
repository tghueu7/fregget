...
//		return null;
    ...
    public static Edge2D createEdge(Node2D nodeFrom, Node2D nodeTo, Simplex2D rightSimplex, 
    ...
    public static boolean connectEdgeSimplex(Edge2D edge, Simplex2D simplex, int edgeSide) {
        if (edge == null || simplex == null || (edgeSide != Edge2D.RIGHTSIDE && edgeSide != 
                Edge2D.LEFTSIDE))
            return false;
     ...
     * @return
     */
    public static Triangle2D createTriangleAndNodes(double x0, double y0, double x1, double y1, 
                                                    double x2, double y2) {
        Node2D n0 = new Node2D(x0, y0);
        Node2D n1 = new Node2D(x1, y1);
