...
public class PolygonImpl extends SurfacePatchImpl implements Polygon {
   ...
   *
   * <p>NOTE The spanning surface should have no boundary components that intersect the boundary of
   * the polygon, and there should be no ambiguity as to which portion of the surface is described
   * by the bounding curves for the polygon. The most common spanning surface is an elevation model,
   * which is not directly described in this standard, although Tins and gridded surfaces are often
   * used in this role.
   */
   ...
   * set of boundary curves (organized into a SurfaceBoundary) which shall be defined using coplanar
   * Positions as controlPoints. Polygon::Polygon(boundary : SurfaceBondary) : Polygon
   *
   ...
   * There is no restriction of the types of interpolation used by the composite curves used in the
   * SurfaceBoundary, but they must all be lie on the "spanningSurface" for the process to succeed.
   * Polygon(boundary : SurfaceBondary, spanSurf : Surface) : Polygon
   *
   * <p>NOTE It is important that the boundary components be oriented properly for this to work. It
   * is often the case that in bounded manifolds, such as the sphere, there is an ambiguity unless
   * the orientation is properly used.
   *
   * @param boundary
