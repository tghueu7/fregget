...
public class CurveImpl extends OrientableCurveImpl implements Curve {
    ...
    public CurveImpl(LineSegment edge) {
        this(edge.getControlPoints().getCoordinateReferenceSystem(), Collections.singletonList
                (edge));
    }
    ...
    private void initialize(List<? extends CurveSegment> segments) {
        ...
        while (it.hasNext()) {
            ...
            if (!cs0.getEndPoint().equals(cs1.getStartPoint())) {
                throw new IllegalArgumentException(
                        "Curvesegments are not continuous. Following curve segments are " +
                                "disjoint:" //$NON-NLS-1$
    ...
    public CurveImpl clone() throws CloneNotSupportedException {
        ...
        //return (CurveImpl) this.getFeatGeometryFactory().getPrimitiveFactory()
        // .createCurveByDirectPositions(newDPList);
    }
    ...
    public ParamForPoint getParamForPoint(DirectPosition p) {
        ...
        double minDistanceSquare = AlgoPointND.getDistanceSquare(p.getCoordinate(), 
                paramForPoints.getPosition().getCoordinate());
        //double minDistanceSquare = ((DirectPositionImpl) p).distanceSquare(paramForPoints
        // .getDirectPosition());
        ...
        for (int i = 1; i < this.curveSegments.size(); i++) {
            ...
            actDistanceSquare = AlgoPointND.getDistanceSquare(p.getCoordinate(), 
                    paramForPoints1.getPosition().getCoordinate());
            //actDistanceSquare = ((DirectPositionImpl) p).distanceSquare
            // (paramForPoints1.getDirectPosition());
...
//	}
     ...
     * (non-Javadoc)
     *
     * @see org.geotools.geometry.featgeom.root.GeometryImpl#getDimension(org.opengis.geometry
     * .coordinate.DirectPosition)
     */
...
//		}
    ...
    public double length(Position point1, Position point2) {
         ...
         */
        double minDistance = Math.abs(DoubleOperation.subtract(startParams.getDistance(), 
                endParams.getDistance()));
     ...
     * There will be no more references to positions or lists of the input curves, all values are
     * copied.
     *
     * @param other
     * @return
