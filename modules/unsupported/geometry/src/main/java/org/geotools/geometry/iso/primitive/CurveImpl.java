...
public class CurveImpl extends OrientableCurveImpl implements Curve {
   ...
   *
   * <p>The field type AbstractSequentialList<CurveSegmentImpl> is implemented mainly outside of the
   * feature geometry packages. The curve implementation iterates through segment, which could
  ...
  private void initialize(List<? extends CurveSegment> segments) {
    if ((segments == null) || segments.isEmpty())
      throw new IllegalArgumentException("The list of CurveSegments ist empty."); // $NON-NLS-1$
  ...
  public CurveImpl clone() throws CloneNotSupportedException {
    // Test OK
    // Create a new Curve by cloning the direct positions which define the control points of this
    // curve
   ...
   * DirectPosition. If the DirectPosition is not on the curve, the nearest point on the curve shall
   * be used.
   *
   ...
   * DirectPosition on the GenericCurve at the same minimal distance from the passed "p", the return
  ...
  public double length(double par1, double par2) {
    if (par1 < 0 || par2 > this.length())
      throw new IllegalArgumentException("Parameter out of parametrisation range."); // $NON-NLS-1$
   ...
   * point and the original curve is be more than the "maxOffset". If both parameters are set, then
   * both criteria are met. If the original control points of the curve lie on the curve, then they
   * are included in the returned LineString's controlPoints. If both parameters are set to zero,
   * then the line string returned is constructed from the control points of the original curve.
   *
   ...
   * "maxOffset" version is useful in maintaining a minimal representation of the curve appropriate
   * for the display device being targeted. This function is also useful in preparing to transform a
   * curve from one coordinate reference system to another by transforming its control points. In
   * this case, the "maxSpacing" version is more appropriate. Allowing both parameters to default to
   * zero does not seem to have any useful geographic nor geometric interpretation unless further
   * information is known about how the curves were constructed.
   *
  ...
  /**
   * The method <code>dimension</code> returns the inherent dimension of this Object, which is less
   * than or equal to the coordinate dimension. The dimension of a collection of geometric objects
   * is the largest dimension of any of its pieces. Points are 0-dimensional, curves are
   * 1-dimensional, surfaces are 2-dimensional, and solids are 3-dimensional. Locally, the dimension
   * of a geometric object at a point is the dimension of a local neighborhood of the point - that
   * is the dimension of any coordinate neighborhood of the point. Dimension is unambiguously
   * defined only for DirectPositions interior to this Object. If the passed DirectPosition2D is
   * NULL, then the method returns the largest possible dimension for any DirectPosition2D in this
   * Object.
   *
  ...
  public double length(Position point1, Position point2) {
     ...
     */
    double minDistance =
        Math.abs(DoubleOperation.subtract(startParams.getDistance(), endParams.getDistance()));
