...
public class SurfaceImpl extends OrientableSurfaceImpl implements Surface {
   ...
   * joined together to form this Surface. Depending on the interpolation method, the set of patches
   * may require significant additional structure. In general, the form of the patches shall be
   ...
   * SurfaceBoundary. In 3D coordinate spaces, this second version of the constructor shall require
   * all of the defining boundary Curve instances to be coplanar (lie in a single plane) which will
  ...
  public Geometry transform(CoordinateReferenceSystem newCRS, MathTransform transform)
    ...
    SurfaceBoundaryImpl surfaceBoundary =
        (SurfaceBoundaryImpl) primitiveFactory.createSurfaceBoundary(newExterior, newInteriors);
