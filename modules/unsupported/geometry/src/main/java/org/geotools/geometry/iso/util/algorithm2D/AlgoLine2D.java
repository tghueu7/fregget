...
//		while(i < lines.size()){
//			Line2D line = (Line2D)lines.get(i);
//			if (line == lines.get(lines.size()-1)&& (AlgoLine.length(line) < minLength)&& (line
// .getP1().lineSize()< 3)){
...
//				j=i+1;
//				while((AlgoLine.length(line) < minLength) && (j<=(lines.size()-1))&&  (((AlgoLine)
// lines.get(j)).getP1().lineSize()< 3)){
...
//	}
    ...
    public static boolean isParallel(Point2D p0, Point2D p1, Point2D q0, Point2D q1) {
        return Math.abs((double) AlgoPoint2D.cross(AlgoPoint2D.subtract(p1, p0), AlgoPoint2D
                .subtract(q1, q0))) <= AlgoPoint2D.EPSILON;
    }
    ...
    public static int sameSegment(Point2D pa0, Point2D pa1, Point2D pb0, Point2D pb1) {
        return (pa0.equals(pb0) && pa1.equals(pb1)) ? EQUALSEGMENT : (pa0.equals(pb1) && 
                pa1.equals(pb0) ? OPPOSITESEGMENT : DIFFERENTSEGMENT);
    }
