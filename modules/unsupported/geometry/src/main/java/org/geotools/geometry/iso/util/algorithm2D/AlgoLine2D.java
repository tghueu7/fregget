...
public class AlgoLine2D {
  ...
  public static boolean isParallel(Point2D p0, Point2D p1, Point2D q0, Point2D q1) {
    return Math.abs(
            (double) AlgoPoint2D.cross(AlgoPoint2D.subtract(p1, p0), AlgoPoint2D.subtract(q1, q0)))
        <= AlgoPoint2D.EPSILON;
  }
  ...
  public static boolean rightSide(Line2D line, Point2D p) {
    /**
     * A return value of 1 relativeCCW indicates that the line segment must turn in the direction
     * that takes the positive X axis towards the negative Y axis. In the default coordinate system
     * used by Java 2D, this direction is counterclockwise, in our coordinate system it is clockwise
     * (right side)
     */
   ...
   * pa0.equals(pb0) && pa1.equals(pb1) 2 if it is inverted, i.e. pa0.equals(pb1) && pa1.equals(pb0)
   *
   * @return
   */
