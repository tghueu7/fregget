...
public class PointArrayImpl extends ArrayList<Position> implements PointArray {
    ...
    public PointArrayImpl(PointArray aPointArray) {
        if (aPointArray.isEmpty()) {
            throw new IllegalArgumentException("Parameter PointArray is empty. Cannot create " +
                    "empty PointArray as we need the CRS");
        }
...
//				.getDefaultCoordinateFactory(coordDim);
        ...
        // Wie in deinem Vorschlag von unserem Telefonat am 04/10 hole ich die CoordFactory über 
        // ein DP
    ...
    public PointArrayImpl(List<Position> positions) {
        super(positions);
        if (positions.size() == 0) {
            throw new IllegalArgumentException("Parameter positions is empty. Cannot create empty" +
                    " PointArray as we need the CRS");
        }
        crs = getDirectPosition(0).getDirectPosition().getCoordinateReferenceSystem();
    }
    ...
    /**
     * Returns the Feature Geometry Factory Instance based on the reference of the 
     * DirectPositions of this PointArray
...
//	}
    ...
    /* (non-Javadoc)
     * @see org.opengis.geometry.coordinate.PointArray#get(int, org.opengis.geometry.coordinate
     * .DirectPosition)
     */
    ...
    /* (non-Javadoc)
     * @see org.opengis.geometry.coordinate.PointArray#set(int, org.opengis.geometry.coordinate
     * .DirectPosition)
     */
        ...
        public LineSegmentImpl get(int arg0) {
            ...
            for (int i = 1; i <= arg0; i++) {
                startParam = DoubleOperation.add(
                        startParam,
                        AlgoPointND.getDistance(this.getStartCoordinate(i - 1), this
                                .getEndCoordinate(i - 1))
                );
                //startParam += AlgoPointND.getDistance(this.getStartCoordinate(i-1), this
                // .getEndCoordinate(i-1));
            }
            LineSegmentImpl rSeg = new LineSegmentImpl(pointArray.getCoordinateReferenceSystem(),
                    p0, p1, startParam);
...
//							p0, p1, this.length);
            LineSegmentImpl ls = new LineSegmentImpl(pointArray.getCoordinateReferenceSystem(), 
                    p0, p1, this.length);
    ...
    public DirectPosition getDirectPosition(int index, DirectPosition dest) throws 
    ...
    public void setDirectPosition(int index, DirectPosition position) throws 
