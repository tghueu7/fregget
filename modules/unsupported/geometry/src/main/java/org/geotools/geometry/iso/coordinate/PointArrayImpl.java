...
public class PointArrayImpl extends ArrayList<Position> implements PointArray {
  ...
  // Wie in deinem Vorschlag von unserem Telefonat am 04/10 hole ich die CoordFactory über ein DP
  /**
   * Returns the Feature Geometry Factory Instance based on the reference of the DirectPositions of
   * this PointArray
   *
  ...
  /**
   * Creates a LineSegment connecting the Positins p0 and p1, whereas p0 is the position at arg0 and
   * p1 is the position at (arg0+1). If (arg0+1) == size() then returns the LineSegment connecting
   * the last position with the first one
   *
  ...
  public class LineSegmentsSequence extends AbstractList<LineSegmentImpl> {
        ...
        startParam =
            DoubleOperation.add(
                ...
                AlgoPointND.getDistance(
                    this.getStartCoordinate(i - 1), this.getEndCoordinate(i - 1)));
        // startParam += AlgoPointND.getDistance(this.getStartCoordinate(i-1),
        // this.getEndCoordinate(i-1));
      }
      LineSegmentImpl rSeg =
          new LineSegmentImpl(pointArray.getCoordinateReferenceSystem(), p0, p1, startParam);
    ...
    public LineSegmentImpl next() {
      ...
      //							p0, p1, this.length);
      LineSegmentImpl ls =
          new LineSegmentImpl(pointArray.getCoordinateReferenceSystem(), p0, p1, this.length);
