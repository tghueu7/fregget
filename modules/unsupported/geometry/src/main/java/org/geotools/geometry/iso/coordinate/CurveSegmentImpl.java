...
public abstract class CurveSegmentImpl implements CurveSegment, Serializable {
   ...
   * Segmentation association, so that the startParam of any segment (except the first) is equal to
   * the endParam of the previous segment. If a GenericCurve is used for other purposes, there shall
   * be a restriction that the two parameters must differ by the arc length of the GenericCurve.
   */
   ...
   * components (CurveSegments) of Curve, each of which defines the direct position of points along
   * a portion of the curve. The order of the CurveSegments is the order in which they are used to
   * trace the Curve.
   *
   ...
   * {self.startParam() < self.endParam()}; {s : Distance (startParam() <= s <= endParam()) implies
   ...
   * constructive parameter interval, which will roughly correspond to the point on the curve where
   * it passes closest that the corresponding controlPoint. These points, the values of the curve at
   * the maxima of the weight functions, will be the sample points for the curve segment.
   *
   * @return
   */
   ...
   *
   * <p>NOTE The above CurveBoundary will almost always be two distinct positions, but, like Curves,
   * CurveSegments can be cycles in themselves. The most likely scenario is that all of the points
   * used will be transients (constructed to support the return value), except for the startPoint
   * and endPoint of the aggregated Curve. These two positions, in the case where the Curve is
   * involved in a Complex, will be represented as Points in the same Complex.
   *
   * @return
   */
  ...
  /**
   * The method "isSimple" returns TRUE if this Object has no interior point of self-intersection or
   * self-tangency. In mathematical formalisms, this means that every point in the interior of the
   * object must have a metric neighborhood whose intersection with the object is isomorphic to an
   * n-sphere, where n is the dimension of this Object. Since most coordinate geometries are
   ...
   * which the isCycle method returns TRUE). While Complexes contain only simple Objects, non-simple
   * Objects are often used in "spaghetti" data sets.
   *
