...
public class SolidBoundaryImpl extends PrimitiveBoundaryImpl implements SolidBoundary {
  ...
  /**
   * SolidBoundaries are similar to SurfaceBoundaries. In normal 3-dimensional Euclidean space, one
   * shell is distinguished as the exterior. In the more general case, this is not always possible.
   *
   * <p>SolidBoundary::exterior[0,1] : Shell; SolidBoundary::interior[0..n] : Shell;
   *
   * <p>NOTE An alternative use of solids with no external shell would be to define "complements" of
   * finite solids. These infinite solids would have only interior boundaries. If this standard is
   * extended to 4D Euclidean space, or if 3D compact manifolds are used (probably not in geographic
