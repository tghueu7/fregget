...
public class GeometryFactoryImpl implements Factory, GeometryFactory {
   ...
   *
   * <p>FactoryRegistry will check to make sure that there are no duplicates created (so there will
   * be only a "single" PositionFactory created with this configuration).
   */
  ...
  public DirectPosition createDirectPosition(double[] coord) {
    // Test ok
    if (coord == null) throw new IllegalArgumentException("Parameter coord is null"); // $NON-NLS-1$
  ...
  public TriangleImpl createTriangle(
    ...
    return new TriangleImpl(
        triangleBoundary, ts, new PositionImpl(p1), new PositionImpl(p2), new PositionImpl(p3));
  }
