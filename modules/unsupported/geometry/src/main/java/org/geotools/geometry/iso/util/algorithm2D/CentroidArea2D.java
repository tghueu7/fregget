...
public class CentroidArea2D {
  ...
  /**
   * Adds the area defined by a Geometry to the centroid total. If the geometry has no area it does
  ...
  public void add(GeometryImpl geom) {
    if (geom instanceof SurfaceImpl) {
      ...
      this.setBasePoint(
          ((CurveImpl) sb.getExterior().getGenerators().iterator().next()).getStartPoint());
      this.addSurface(sb);
  ...
  /**
   * Returns twice the signed area of the triangle p1-p2-p3, positive if a,b,c are oriented ccw, and
   * negative if cw.
   */
