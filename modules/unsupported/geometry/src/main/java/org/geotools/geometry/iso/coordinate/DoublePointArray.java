...
public class DoublePointArray extends AbstractList<Position> implements PointArray, Serializable {
  ...
  */
  ...
  public void setPosition(int index, DirectPosition position)
    ...
    // note: tempting to use System arraycopy on position.getCoordinates() but that would make an
    // temporary extra array
    int D = getDimension();
    for (int i = 0; i < D; i++) {
  ...
  public boolean equals(DoublePointArray dpArray, double tol) {
    ...
    for (int x = 0; x < dpArray.length(); x++) {
      ...
      for (int i = 0; i < D; ++i) {
        if (Math.abs(DoubleOperation.subtract(ddPos.getOrdinate(i), thisddPos.getOrdinate(i)))
            > tol) return false;
      }
    }
    return true;
