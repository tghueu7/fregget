...
public abstract class OrientablePrimitiveImpl extends PrimitiveImpl implements OrientablePrimitive {
  ...
  /**
   * Oriented Association (from Specification of OrientablePrimitive) Each Primitive of dimension 1
   * or 2 is associated to two OrientablePrimitives, one for each possible orientation.
   *
  ...
  /**
   * Returns an array with two orientable primitives, whereas the first one is "this" object and the
   * second one the field proxy
   *
   * @return an array OrientablePrimitive[2] with the positive and the negative orientable primitive
   */
   ...
   * (curve or surface). The sign datatype is defined in ISO 19103. If "c" is a curve, then "<+, c>"
   * is its positive orientable curve and "<-, c>" is its negative orientable curve. In most cases,
   * leaving out the syntax for record "< , >" does not lead to confusion, so "<+, c>" may be
   * written as "+c" or simply "c", and "<-, c>" as "-c". Curve space arithmetic can be performed if
