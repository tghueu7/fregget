...
public class DirectPositionImpl implements DirectPosition, Cloneable, Serializable {
    ...
    public void setOrdinate(int dimension, double value)
        ...
        if (dimension >= this.coordinate.length || dimension < 0) {
            throw new IndexOutOfBoundsException("Index " + dimension + " out of coordinate range " +
                    "(max " + coordinate.length + ")"); //$NON-NLS-1$
        }
    ...
    public void setCoordinate(double[] coord) {
        if (coord.length != this.getDimension()) {
            throw new IllegalArgumentException("Index " + coord.length + " out of coordinate " +
                    "range (expected " + getDimension() + ")"); //$NON-NLS-1$
        }
        this.coordinate = coord;
    }
    ...
    public boolean equals(DirectPosition position, double tol) {
        ...
        if (!CRS.equalsIgnoreMetadata(getCoordinateReferenceSystem(), position
...
//	}
     ...
     *
     * @param values Array of doubles. values[0] will be added to the X ordinate, values[1] to 
     *               the Y value and an optional values[2] to the Z value.
     */
    ...
    public void add(DirectPositionImpl otherDP) {
        ...
        for (int i = 0; i < this.coordinate.length; i++) {
            if (Double.compare(this.coordinate[i], Double.NaN) == 0) {
                ...
                this.coordinate[i] = DoubleOperation.add(this.coordinate[i], otherDP.getOrdinate
                        (i));
            }
        }
    }
...
//	}
    ...
    // TODO JR: Wenn du nichts dagegen hast, den folgenden teil bitte rausloeschen. solche 
    // methoden werden in robuster form in jts angeboten.
