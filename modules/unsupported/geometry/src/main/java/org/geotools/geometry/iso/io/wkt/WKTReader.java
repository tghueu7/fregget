...
public class WKTReader {
   ...
   *
   * @param wellKnownText one or more <Geometry Tagged Text>strings (see the OpenGIS Simple Features
   ...
   *     returned by the stream should be L_PAREN (the beginning of "(x1 y1, x2 y2, ..., xn yn)") or
   *     EMPTY.
   * @return the next array of <code>Coordinate</code>s in the stream, or an empty array if EMPTY is
   *     the next element returned by the stream.
  ...
  private void parseError(String expected) throws ParseException {
    ...
    if (tokenizer.ttype == StreamTokenizer.TT_EOL) throw new ParseException("Unexpected EOL token");
  ...
  private Surface readPolygonText() throws IOException, ParseException {
    ...
    if (nextToken.equals(EMPTY)) {
      return new SurfaceImpl(
          (SurfaceBoundary) null); // this.primitiveFactory.createSurface((SurfaceBoundary) null);
    }
    ...
    SurfaceBoundary sfb =
        new SurfaceBoundaryImpl(
            crs, shell, holes); // this.primitiveFactory.createSurfaceBoundary(shell, holes);
    ...
    for (int i = 0; i < aCoords.length; i++) {
      points.add(
          this.positionFactory.createPosition(
              this.positionFactory.createDirectPosition((aCoords[i].getCoordinates()))));
    }
    // Create List of CurveSegment´s (LineString´s)
    LineString lineString =
        new LineStringImpl(
            new PointArrayImpl(points), 0.0); // this.geometryFactory.createLineString(points);
