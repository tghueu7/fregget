...
public abstract class SurfacePatchImpl implements SurfacePatch, Serializable {
   ...
   * mathematical texts. A value of 1 means that the functions are continuous and differentiable at
   * the appropriate end point: "C1" continuity. A value of "n" for any integer means n-times
   ...
   *
   * <p>If the GM_Surface.coordinateDimension is 2, then the entire GM_Surface is one logical patch
   * defined by linear interpolation from the boundary.
   *
   * <p>NOTE In this standard, surface patches do not appear except in the context of a surface, and
  ...
  protected SurfacePatchImpl(SurfaceBoundaryImpl boundary) {
    ...
    // TODO This behaviour must be reviewed again: Probably there are cases when we are not able to
    // create a SurfacePatch by the boundary, for example when creating Triangles which are defined
    // by three positions.
    if (boundary == null)
