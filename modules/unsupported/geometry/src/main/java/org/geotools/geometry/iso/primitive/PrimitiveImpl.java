...
public abstract class PrimitiveImpl extends GeometryImpl implements Primitive {
  ...
  /**
   * The "Interior to" association associates Primitives which are by definition coincident with one
   * another. This allows applications to override the Set<DirectPosition> interpretation and its
   * associated computational geometry, and declare one Primitive to be "interior to" another. This
   * association should normally be empty when the Primitives are within a Complex, since in that
   * case the boundary information is sufficient for most cases.
   *
   * <p>Primitive::coincidentSubelement [0..n] : Reference<Primitive> Primitive::superElement [0..n]
   * : Reference<Primitive>
   *
   * <p>This association is constrained by the set theory operators and dimension operators defined
   * at Object.
   *
  ...
  /**
   * As a set of primitives, a Complex may be contained as a set in another larger Complex, referred
   * to as a "super complex" of the original. A Complex is maximal if there is no such larger super
   * complex. The operation "maximalComplex" shall return the set of maximal Complexes within which
   * this Object is contained.
   *
   * <p>Object::maximalComplex() : Set<Complex>
   *
   * <p>If the application schema used does not include Complex, then this operation shall return a
   * NULL value.
   *
