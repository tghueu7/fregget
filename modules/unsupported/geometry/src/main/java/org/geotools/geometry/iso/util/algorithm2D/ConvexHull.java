...
public class ConvexHull {
  ...
  private static Coordinate[] extractCoordinates(GeometryImpl geom) {
    ...
    if (geom instanceof PointImpl) {
      ...
      positions = new ArrayList<DirectPositionImpl>();
      positions =
          ((RingImplUnsafe) ((SurfaceImpl) geom).getBoundary().getExterior()).asDirectPositions();
      ...
      while (surfaceIter.hasNext()) {
        positions.addAll(
            ((RingImplUnsafe) ((SurfaceImpl) surfaceIter.next()).getBoundary().getExterior())
                .asDirectPositions());
      }
      ...
      while (iterator.hasNext()) {
        ...
        if (prim instanceof PointImpl) {
          ...
          if (hull instanceof CurveImpl) {
            ...
            positions.addAll(
                ((RingImplUnsafe) ((SurfaceImpl) surface).getBoundary().getExterior())
                    .asDirectPositions());
          }
  ...
  private Geometry lineOrPolygon(Coordinate[] coordinates) {
    ...
    if (coordinates.length == 3) {
      ...
      segments.add(lineString);
      return new CurveImpl(this.crs, segments);
      // return this.geomFactory.getPrimitiveFactory().createCurveByDirectPositions((List<Position>)
      // positions);
    }
    ...
    // return
    // this.geomFactory.getPrimitiveFactory().createSurfaceByDirectPositions((List<DirectPosition>)
    // positions);
  }
