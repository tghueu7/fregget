...
public class PointImpl extends PrimitiveImpl implements Point {
   ...
   *
   * <p>Point is the only subclass of Primitive that cannot use Positions to represent its defining
   * geometry. A Position is either a DirectPostion or a reference to a Point (from which a
   * DirectPosition2D may be obtained).
   *
   * <p>By not allowing Point to use this technique, infinitely recursive references are prevented.
   * Applications may choose another mechanism to prevent this logical problem.
   *
   * <p>NOTE In most cases, the state of a Point is fully determined by its position attribute. The
   * only exception to this is if the Point has been subclassed to provide additional non-geometric
   * information such as symbology.
   */
   ...
   * this Point is defined. For example, in the Mercator projection, the curve is the rhumb line. In
   * 3D, geocentric coordinate system, the curve may be the geodesic joining the two points along
   * the surface of the geoid or ellipsoid in use. Implementations that support this function shall
   * specify the nature of the curve to be used.
   *
  ...
  /**
   * The method <code>getEnvelope</code> returns the minimum bounding box for this Object. There are
   * cases for which the min and max positions would be outside the domain of validity of the
