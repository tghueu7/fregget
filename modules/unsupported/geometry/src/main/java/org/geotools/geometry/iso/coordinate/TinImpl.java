...
public class TinImpl extends TriangulatedSurfaceImpl {
   ...
   * result shall be a change of the surface boundary. The attribute "stopLines" contains all these
  ...
  /**
   * Areas of the surface where the data is not sufficiently dense to assure reasonable calculations
   * shall be removed by adding a retention criterion for triangles based on the length of their
   * sides. For any triangle sides exceeding maximum length, the adjacent triangles to that triangle
   * side shall be removed from the surface.
   *
   ...
   * given does not affect the surface that is represented. Application schemas may add information
   * based on the ordering of the control points to facilitate the reconstruction of the TIN from
   * the controlPoints.
   *
  ...
  public TinImpl(
    ...
    if (post.length >= 3) {
      ...
      throw new IllegalArgumentException("At least three control points expected."); // $NON-NLS-1$
    }
