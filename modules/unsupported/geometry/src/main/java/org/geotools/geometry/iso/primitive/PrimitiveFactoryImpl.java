...
public class PrimitiveFactoryImpl implements Serializable, Factory, PrimitiveFactory {
    ...
    public PrimitiveFactoryImpl(Hints hints) {
        if (hints == null) {
            ...
            if (crs == null) {
                throw new NullPointerException("A CRS Hint is required in order to use " +
    ...
    public PrimitiveFactoryImpl(CoordinateReferenceSystem crs, PositionFactory positionFactory) {
        this.crs = crs;
        if (crs == null) {
            throw new NullPointerException("A non null crs is required in order to use " +
                    "PrimitiveFactoryImpl");
        }
     ...
     * (non-Javadoc)
     *
     * @see org.opengis.geometry.primitive.PrimitiveFactory#createPoint(org.opengis.geometry
     * .coordinate.Position)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.opengis.geometry.primitive.PrimitiveFactory#createSolid(org.opengis.geometry
     * .primitive.SolidBoundary)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.opengis.geometry.primitive.PrimitiveFactory#createPrimitive(org.opengis.geometry
     * .coordinate.Envelope)
     */
...
//				DirectPositionImpl min1 = (DirectPositionImpl) positionFactory
// .createDirectPosition(null);//new DirectPositionImpl(crs);
//				min1.setOrdinate(d, min);
//				DirectPositionImpl max1 = (DirectPositionImpl) positionFactory
// .createDirectPosition(null);//new DirectPositionImpl(crs);
...
//					DirectPositionImpl maxN = (DirectPositionImpl) positionFactory
// .createDirectPosition(position.getCoordinate()); //new DirectPositionImpl(position);
...
//	}
    ...
    private PrimitiveImpl processSegmentToPrimitive(Envelope bounds, LineSegment segment, int 
            dimension) {
    ...
    public LineSegment processBoundsToSegment(Envelope bounds) {
        ...
        DirectPosition positionA = positionFactory.createDirectPosition(null); //new 
        // DirectPositionImpl(crs);
        DirectPosition positionB = positionFactory.createDirectPosition(null); //new 
        // DirectPositionImpl(crs);		
        if (axis.getDirection() != AxisDirection.OTHER) {
    ...
    public Ring processBoundsToRing(Envelope bounds, LineSegment segment, final int D) {
        ...
        DirectPosition three = positionFactory.createDirectPosition(two.getCoordinate()); //new 
        // DirectPositionImpl( two );
        three.setOrdinate(D, bounds.getMaximum(D));

        DirectPosition four = positionFactory.createDirectPosition(one.getCoordinate()); //new 
        // DirectPositionImpl( one );
     ...
     *
     * @param positions List of positions, the last positions must be equal to the
     *                  first position
