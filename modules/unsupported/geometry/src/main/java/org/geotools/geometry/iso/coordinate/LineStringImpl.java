...
public class LineStringImpl extends CurveSegmentImpl implements LineString {
   ...
   * linearly interpolated. The first position in the sequence is the startPoint of the LineString,
   * and the last point in the sequence is the endPoint of the LineString. LineString::controlPoint
   * : PointArray
   */
  ...
  /**
   * Constructor by another LineString The constructor for LineString takes a sequence of points and
   * constructs a LineString with those points as controlPoints. The constructor of a LineString
  ...
  /**
   * Merges this LineString with another LineString, forming a new LineString The input LineStrings
   * will not be modified by this operation. The used Lists of the pointArray will be cloned. The
   * Positions which represents the control points will not be cloned.
   *
  ...
  public LineStringImpl asLineString(double maxSpacing, double maxOffset) {
    ...
    while (actualParam < this.endParam) {
      ...
      do {
        ...
        // || (segments[firstSegment].endParam() > actualParam && segments[firstSegment].endParam()
        // < actualParam+newSpacing)) {
        // TO DO numerical precision
        // assert false;
        double EPSILON = 0.00001;
        if ((segments.get(firstSegment).getStartParam() > actualParam
                && segments.get(firstSegment).getStartParam() + EPSILON < actualParam + newSpacing)
            || (segments.get(firstSegment).getEndParam() > actualParam
                && segments.get(firstSegment).getEndParam() + EPSILON < actualParam + newSpacing)) {
          ...
          if (maxDistance > maxOffset) {
            // System.out.println("Testausgabe: Musste den Abstand verkleinern. Neuer Abstand: " +
            // (newSpacing/2));
            newSpacing /= 2;
          }
