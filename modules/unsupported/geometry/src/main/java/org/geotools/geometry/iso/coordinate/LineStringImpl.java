...
public class LineStringImpl extends CurveSegmentImpl implements LineString {
    ...
    public LineStringImpl asLineString(double maxSpacing, double maxOffset) {
        ...
        /* The newSpacing represents the distance between the Posistion at the actualParam and 
        the next Position;
        ...
        while (actualParam < this.endParam) {
            ...
            /* The maximum distance between the the line (actualParam to actualParam+newSpacing) 
            and the Segments (between firstSegment and lastSegment) will be calculated after */
            maxDistance = 0;
            ...
            do {
                ...
                // || (segments[firstSegment].endParam() > actualParam && segments[firstSegment]
                // .endParam() < actualParam+newSpacing)) {
                ...
                if ((segments.get(firstSegment).getStartParam() > actualParam &&
                    ...
                    i = firstSegment;
                    while ((i < segments.size()) && (segments.get(i).getEndParam() <= actualParam
                            + newSpacing)) {
                        i++;
                    }
                    lastSegment = i - 1;
                    DirectPosition p1 = this.forParam(actualParam);
                    DirectPosition p2 = new DirectPositionImpl(this.forParam(actualParam + 
                            newSpacing));
                    LineSegmentImpl seg = new LineSegmentImpl(p1, p2, 0);
                    //(LineSegmentImpl)cf.createLineSegment(cf.createPosition(this.forParam
                    // (actualParam)),cf.createPosition(this.forParam(actualParam+newSpacing)));

                    /* Search maximum of all distances */
                    CurveImpl curve = new CurveImpl(seg);
                    maxDistance = curve.distance(new PointImpl(segments.get(firstSegment)
                            .getStartPoint()));
