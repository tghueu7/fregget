...
public class LineSegmentImpl extends LineStringImpl implements LineSegment {
  ...
  /**
   * A LineSegment consists of two distinct DirectPositions (the startPoint and endPoint) joined by
   * a straight line. Thus its interpolation attribute is "linear". The default
   ...
   *
   * <p>NOTE The linear interpolation, given using a constructive parameter t, 0<=t<=1.0, where c(o)
   * = c.startPoint() and c(1)=c. endPoint(), is: c(t) = c(0)(1-t) + c(1)t
   *
  ...
  public LineSegmentImpl(
    ...
    this(
        new PointArrayImpl(new DirectPositionImpl(crs, from), new DirectPositionImpl(crs, to)),
        startParam);
  }
   ...
   * parameter value. This vector approximates the derivative of the parameterization of the curve.
