...
public class XmlComplexFeatureParser extends XmlFeatureParser<FeatureType, Feature> {
   ...
   * id'd attributes so that we can refer back to them if they're referred to further down the input
   * stream by a href.
   */
  ...
  /**
   * Register the target of any hrefs' with the target id specified, to the attribute provided. This
   * has two effects: it puts the id and value in the discoveredComplexAttributes object so that if
   * we come across a href pointing to this id in the future, we'll be able to just use the
   * attribute provided as the value for that href. Secondly: it loops over any
  ...
  /**
   * Given a href and an expected type, return either the actual manifestation of that href's target
   * or a placeholder object. The real instance will be returned if it's already been parsed,
   * otherwise the placeholder will be returned. The placeholder will automatically be replaced upon
          ...
          while ((innerAttribute = parseNextAttribute((ComplexType) type)) != null) {
            ...
            if (ComplexAttribute.class.isAssignableFrom(innerAttribute.value.getClass())) {
              ...
              attributeBuilder.add(
                  innerAttribute.id, getValue(innerAttribute), innerAttribute.name);
            }
          }
