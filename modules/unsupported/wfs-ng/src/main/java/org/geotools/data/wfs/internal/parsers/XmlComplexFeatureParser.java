...
public class XmlComplexFeatureParser extends
     ...
     */
    private Map<String, ArrayList<Attribute>> placeholderComplexAttributes = new HashMap<String, 
            ArrayList<Attribute>>();
     ...
     */
    public XmlComplexFeatureParser(InputStream getFeatureResponseStream,
                                   FeatureType targetType, QName featureDescriptorName, Filter 
                                           filter)
            throws IOException {
        super(getFeatureResponseStream, targetType, featureDescriptorName);
        this.featureBuilder = new ComplexFeatureBuilder(this.targetType);
        ...
        try {
            ...
            while ((nextAttribute = parseNextAttribute(this.targetType)) != null) {
                if (!Property.class.isAssignableFrom(nextAttribute.value.getClass())) {
                    featureBuilder.append(nextAttribute.name, new AttributeImpl(nextAttribute
                            .value, (AttributeDescriptor) this.targetType.getDescriptor
                            (nextAttribute.name), null));
                } else {
     ...
     * for this target, if their are any they will have their values changed from the placeholder
     * value
     ...
     * @return An attribute of the type specified, either the actual attribute or a
     * placeholder.
     */
     ...
     * GML Id and its name.
     *
     * @param complexType The complex type that the CALLER is trying to build. NB: this
        ...
        if (tagType == XmlPullParser.START_TAG) {
            ...
            if (descriptor != null) {
                ...
                // ComplexType or an AttributeType.
                    ...
                    if (type.getBinding() == Collection.class
                        ...
                        if (value != null || descriptor.isNillable()) {// add
                            ...
                            AttributeDescriptor simpleContentDescriptor = new 
                ...
                throw new RuntimeException(
                        String.format(
                                "WFS response structure unexpected. Could not find descriptor in " +
                                        "type '%s' for '%s'.",
