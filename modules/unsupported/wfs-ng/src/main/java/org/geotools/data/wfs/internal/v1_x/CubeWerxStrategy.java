...
public class CubeWerxStrategy extends StrictWFS_1_x_Strategy {
  ...
  /**
   * @return {@code true} only if resultType == results, CubeWerx throws a service exception if the
   *     resultType parameter is set on a POST request, no matter it's value, and on a GET request
   *     it's just ignored; also the returned feature collection does not contain the number of
   *     features matched.
   */
  @Override
