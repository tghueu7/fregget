...
class WFSFeatureSource extends ContentFeatureSource {
  ...
  private void invertAxisInFilterIfNeeded(Query query) {
    boolean invertXY =
        WFSConfig.invertAxisNeeded(client.getAxisOrderFilter(), query.getCoordinateSystem());
    if (invertXY) {
  ...
  /**
   * Returns the feature type that shall result of issueing the given request, adapting the original
   * feature type for the request's type name in terms of the query CRS and requested attributes.
   *
   * @param query
   * @return
   * @throws IOException
  ...
  SimpleFeatureType getQueryType(final Query query, SimpleFeatureType featureType)
    ...
    if (coordinateSystemReproject != null) {
      try {
        queryType =
            DataUtilities.createSubType(queryType, propertyNames, coordinateSystemReproject);
      } catch (SchemaException e) {
        throw new DataSourceException(e);
      }
    }
