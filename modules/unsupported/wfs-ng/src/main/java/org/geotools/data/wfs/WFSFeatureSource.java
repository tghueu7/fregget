...
class WFSFeatureSource extends ContentFeatureSource {
    ...
    private void invertAxisInFilterIfNeeded(Query query) {
        boolean invertXY = WFSConfig.invertAxisNeeded(client.getAxisOrderFilter(), query
                .getCoordinateSystem());
        if (invertXY) {
    ...
    protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(Query localQuery)
        ...
        final SimpleFeatureType contentType = getQueryType(localQuery, (SimpleFeatureType) 
                request.getFullType());
        ...
        if (request.getUnsupportedFilter() != null && request.getUnsupportedFilter() != Filter
                .INCLUDE) {
            reader = new FilteringFeatureReader<SimpleFeatureType, SimpleFeature>(reader, request
    ...
    protected String getSupportedSrsName(GetFeatureRequest request, Query query) {
        ...
        Set<String> supported = request.getStrategy().getSupportedCRSIdentifiers(request
                .getTypeName());
    ...
    protected FeatureReader<SimpleFeatureType, SimpleFeature> applyReprojectionDecorator
        ...
        if (query.getCoordinateSystem() != null
                && !query.getCoordinateSystem().equals(reader.getFeatureType()
                .getCoordinateReferenceSystem())) {
            if (request.getSrsName() != null) {
                try {
                    reader = new ForceCoordinateSystemFeatureReader(reader, query
                            .getCoordinateSystem());
     ...
     * @throws IOException
     */
    SimpleFeatureType getQueryType(final Query query, SimpleFeatureType featureType) throws 
            IOException {
