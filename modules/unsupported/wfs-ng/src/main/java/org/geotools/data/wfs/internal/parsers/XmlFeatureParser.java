...
public abstract class XmlFeatureParser<FT extends FeatureType, F extends Feature>
  ...
  public XmlFeatureParser(
      final InputStream getFeatureResponseStream, final FT targetType, QName featureDescriptorName)
      throws IOException {
   ...
   *
   * <p>Postcondition: parser positioned at the {@link GML#MultiLineString MultiLineString} end tag
   * of the starting tag
   *
   * @throws IOException
   * @throws XmlPullParserException
   ...
   *
   * <p>Postcondition: parser positioned at the {@link GML#MultiSurface MultiSurface} end tag of the
   * starting tag
   */
  ...
  private Coordinate[] parseCoordinates(int dimension) throws XmlPullParserException, IOException {
    ...
    String rawTextValue = parser.nextText();
    Coordinate[] coords =
        toCoordList(rawTextValue, decimalSeparator, coordSeparator, tupleSeparator, dimension);
