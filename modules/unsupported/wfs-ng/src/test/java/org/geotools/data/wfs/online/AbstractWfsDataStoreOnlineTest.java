...
public abstract class AbstractWfsDataStoreOnlineTest {
  ...
  /**
   * Performs a FeatureSource.getCount(Query) with the constructor provided fid filter if the filter
   * is not null.
   *
   * @throws IOException
   */
  ...
  public void testSingleType() throws IOException, NoSuchElementException {
    ...
    Query query =
        new Query(testType.FEATURETYPENAME, Filter.INCLUDE, 20, Query.ALL_NAMES, "work already");
    features = source.getFeatures(query);
    features.size();
