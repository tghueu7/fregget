...
public abstract class AbstractIntegrationTest {
  ...
  private void completeTestDataType(TestDataType test) throws IOException {
    ...
    test.feat12Filter =
        ff.id(ff.featureId(test.features[0].getID()), ff.featureId(test.features[1].getID()));
  ...
  /**
   * This function is stolen from DefaultFeature equals method. We want to check for equality except
   * for featureId which we expect to be different.
   *
  ...
  public void testGetFeatureStoreSetFeatures() throws IOException {
    ...
    assertEquals(
        1, count(data.getFeatureReader(new Query(first.typeName), Transaction.AUTO_COMMIT)));
