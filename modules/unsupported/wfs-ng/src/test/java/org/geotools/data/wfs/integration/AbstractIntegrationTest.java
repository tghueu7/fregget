...
public abstract class AbstractIntegrationTest {
    ...
    private void completeTestDataType(TestDataType test) throws IOException {
        ...
        test.bounds = new ReferencedEnvelope(test.features[0].getFeatureType()
                .getCoordinateReferenceSystem());
        for (int i = 0; i < test.numberOfFeatures; i++) {
            test.bounds.expandToInclude(new ReferencedEnvelope(test.features[i].getBounds()));
        }

        test.feat12Bounds = new ReferencedEnvelope(test.features[0].getFeatureType()
                .getCoordinateReferenceSystem());
    ...
    public void testLockFeatureInteraction() throws IOException {
        ...
        try (Transaction t1 = new DefaultTransaction(); Transaction t2 = new DefaultTransaction()) {
...

            {
                SimpleFeatureSource source = data.getFeatureSource(first.typeName);
                if (!(source instanceof FeatureLocking)) {
                    LOGGER.info("testLockFeatureInteraction ignored, store does not support " +
                            "locking");
                    return;
                }
     ...
     * </p>
     */
    protected static int count(FeatureReader<SimpleFeatureType, SimpleFeature> reader) throws 
            IOException {
        if (reader == null) {
            return -1;
        }
