...
public class WFSTestData {
  ...
  public static class TestDataType {
    ...
    /**
     * @param folder the folder name under {@code test-data} where the test files for this feature
     *     type are stored
     ...
     */
    TestDataType(
        final String folder, final QName qName, final String featureTypeName, final String crs) {
  ...
  /**
   * Creates the test {@link #wfs} with the provided connection factory that parses the capabilities
   * object from the test xml file pointed out by {@code capabilitiesURL}
   *
   * <p>Tests methods call this one to set up a protocolHandler to test
   *
  ...
  public static class TestWFSClient extends WFSClient {
    ...
    public DescribeFeatureTypeResponse issueRequest(DescribeFeatureTypeRequest request)
      ...
      HTTPResponse response =
          new TestHttpResponse(request.getOutputFormat(), "UTF-8", describeFeatureTypeUrlOverride);
      try {
