...
//        this.baseGridToWorld=new AffineTransform((AffineTransform) rasterManager
// .spatialDomainManager.coverageGridToWorld2D);
...
//                                    1,
//                                    1,
//                                    new RasterLayout(0, 0, width, height,reader
// .getTileGridXOffset(i),reader.getTileGridYOffset(i),reader.getTileWidth(i),reader
// .getTileHeight(i))                                    
//                                    )
//                            );
//                }
...
//                
//                // get a stream
//                inStream = rasterManager.parent.ovrInStreamSPI.createInputStreamInstance
// (rasterManager.parent.ovrSource, ImageIO.getUseCache(), ImageIO.getCacheDirectory());
//                if (inStream == null) {
//                    throw new IllegalArgumentException(
//                            "Unable to get an input stream for the provided file "
//                                    + granuleFile.toString());
...
//                    this.levels.add(
//                            new RasterLevelDescriptor(
//                                    ImageReaderSource.wrapFile(k,rasterManager.parent
// .ovrSource, rasterManager.parent.ovrInStreamSPI, GeoTiffUtils.TIFFREADERFACTORY),
//                                    scaleX,
//                                    scaleY,
//                                    new RasterLayout(0, 0, width, height,reader
// .getTileGridXOffset(k),reader.getTileGridYOffset(k),reader.getTileWidth(k),reader
// .getTileHeight(k))
//                                    )
//                            );                    
//                }
...
//    }
    ...
    public RasterDescriptor(final List<RasterSlice> slices) {
        ...
        this.levels.add(new RasterLevelDescriptor(fullRes.source, 1.0, 1.0, fullRes
                .rasterDimensions));

        final int baseLevelWidth = fullRes.rasterDimensions.getWidth(), baseLevelHeight = fullRes
                .rasterDimensions.getHeight();
        ...
        for (int i = 1; i < size; i++) {
            ...
            this.levels.add(new RasterLevelDescriptor(slice.source, scaleX, scaleY, slice
                    .rasterDimensions));
    ...
    public RasterLoadingResult loadRaster(
        ...
        try {
            ...
            inStream = selectedlevel.source.getInputStreamSPI().createInputStreamInstance(input, 
            ...
            final MathTransform2D cropGridToWorldCorner = (MathTransform2D) ProjectiveTransform
                    .create(selectedlevel.gridToWorldTransformCorner);
            ...
            // it.
            final Rectangle sourceArea = CRS.transform(cropWorldToGrid, new GeneralEnvelope
                    (intersection)).toRectangle2D().getBounds();
            XRectangle2D.intersect(sourceArea, selectedlevel.rasterDimensions.getBounds(), 
                    sourceArea);
            ...
            if (sourceArea.isEmpty()) {
                ...
                LOGGER.fine("Loading level " + overviewIndex + " with sourceFile region " + 
                        sourceArea);
            }
            ...
            RenderedImage raster = request.getReadType().read(readParameters, imageChoice, (File)
                    input, selectedlevel.rasterDimensions.getBounds(), tileDimension);
            if (raster == null) {
                return null;
            }
            try {
            ...
            if (!XAffineTransform.isIdentity(backToBaseLevelScaleTransform, GeoTiffUtils
                    .AFFINE_IDENTITY_EPS))
                finalRaster2Model.concatenate(backToBaseLevelScaleTransform);
            if (!XAffineTransform.isIdentity(afterDecimationTranslateTranform, GeoTiffUtils
                    .AFFINE_IDENTITY_EPS))
                finalRaster2Model.concatenate(afterDecimationTranslateTranform);
            if (!XAffineTransform.isIdentity(decimationScaleTranform, GeoTiffUtils
                    .AFFINE_IDENTITY_EPS))
...
//    private RasterLevelDescriptor getLevel(ImageReader reader, RasterLayerRequest request,  int
// imageChoice, int overviewIndex) {
//        synchronized (levels) {
...
//                    //
//                    // get selected level and base level dimensions
//                    final Rectangle levelDimension =  new Rectangle(0, 0, reader.getWidth
// (imageChoice), reader.getHeight(imageChoice));
//
