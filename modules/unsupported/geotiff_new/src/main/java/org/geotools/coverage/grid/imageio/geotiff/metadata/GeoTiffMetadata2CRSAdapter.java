...
import org.geotools.coverage.grid.imageio.geotiff.metadata.codes
...
public final class GeoTiffMetadata2CRSAdapter {
     ...
     * objects.
     */
    private final static MathTransformFactory mtFactory = ReferencingFactoryFinder
     ...
     *                          convention for the {@link ImageDatum} of the underlying 
     *                          {@link ImageCRS}.
     * @throws GeoTiffException
     */
    public static MathTransform getRasterToModel(
        ...
        if (hasTiePoints && hasPixelScales) {
            ...
            final double tiePointColumn = tiePoints[0].getValueAt(0)
                    + ((forceToCellCenter || rasterType == GeoTiffConstants.RasterPixelIsArea) ? 
                    -0.5 : 0);
            ...
            try {
                ...
                if (unitSize == null) {
                    throw new GeoTiffException(
                            metadata,
                            new StringBuffer(
                                    "GeoTiffMetadata2CRSAdapter::createUnit:Must define unit " +
                                            "length when using a user ")
