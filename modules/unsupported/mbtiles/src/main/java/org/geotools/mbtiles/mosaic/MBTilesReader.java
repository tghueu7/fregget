...
public class MBTilesReader extends AbstractGridCoverage2DReader {
    ...
    protected final static ReferencedEnvelope WORLD_ENVELOPE = new ReferencedEnvelope
    ...
    public MBTilesReader(Object source, Hints hints) throws IOException {
        ...
        try {
            bounds = ReferencedEnvelope.create(metadata.getBounds(), WGS_84).transform
        ...
        highestRes = new double[]{WORLD_ENVELOPE.getSpan(0) / size, WORLD_ENVELOPE.getSpan(1) / 
                size};
    ...
    public GridCoverage2D read(GeneralParameterValue[] parameters) throws 
        ...
        if (parameters != null) {
            for (int i = 0; i < parameters.length; i++) {
                ...
                if (name.equals(AbstractGridFormat.READ_GRIDGEOMETRY2D.getName())) {
                    ...
                    try {
                        requestedEnvelope = ReferencedEnvelope.create(gg.getEnvelope(), gg
                                .getCoordinateReferenceSystem()).transform(SPHERICAL_MERCATOR, 
                                true);
                        ;
                    } catch (Exception e) {
        ...
        if (requestedEnvelope != null && dim != null) {
            ...
            //proportion of total width that is being requested
            double propWidth = dim.getWidth() / ratioWidth; //this is the width in pixels that 
            // the whole world would have in the requested resolution
            zoomLevel = Math.round(Math.log(propWidth / DEFAULT_TILE_SIZE) / Math.log
                    (ZOOM_LEVEL_BASE));
            //the closest zoom level to the resolution, based on the formula width = 
            // zoom_base^zoom_level * tile_size -> zoom_level = log(width / tile_size)/log
            // (zoom_base)
        }
        ...
        long numberOfTiles = Math.round(Math.pow(ZOOM_LEVEL_BASE, zoomLevel)); //number of tile 
        // columns/rows for chosen zoom level
        ...
        if (requestedEnvelope != null) { //crop tiles to requested envelope                   
            leftTile = Math.max(leftTile, Math.round(Math.floor((requestedEnvelope.getMinimum(0) 
                    - offsetX) / resX)));
            bottomTile = Math.max(bottomTile, Math.round(Math.floor((requestedEnvelope.getMinimum
                    (1) - offsetY) / resY)));
            rightTile = Math.max(leftTile, Math.min(rightTile, Math.round(Math.floor(
                    (requestedEnvelope.getMaximum(0) - offsetX) / resX))));
            topTile = Math.max(bottomTile, Math.min(topTile, Math.round(Math.floor(
        ...
        ReferencedEnvelope resultEnvelope = new ReferencedEnvelope(offsetX + leftTile * resX, 
                offsetX + (rightTile + 1) * resX, offsetY + bottomTile * resY, offsetY + (topTile
                + 1) * resY, SPHERICAL_MERCATOR);
        ...
        while (it.hasNext()) {
            ...
            BufferedImage tileImage = readImage(tile.getData(), metadata.getFormatStr() == null ?
                    "png" : metadata.getFormatStr());
        ...
        return coverageFactory.create(metadata.getName() == null ? "nameless mbtiles" : metadata
                .getName(), image, resultEnvelope);
    }
