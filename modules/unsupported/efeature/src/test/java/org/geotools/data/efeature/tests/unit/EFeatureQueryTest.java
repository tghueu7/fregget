...
public class EFeatureQueryTest extends AbstractResourceTest {
    ...
    public void testFeatureGeometryCQL() {
        try {
            ...
            int count = 0;
            TreeIterator<EObject> eObjects = eResource.getAllContents();
            EFeatureQuery eQuery = EFeatureUtils.toEFeatureQuery(eFeatureDataInfo, eObjects, 
                    filter);
            EFeatureIterator it = eQuery.iterator();
            while (it.hasNext()) {
                EObject eObject = it.next();
            ...
            assertEquals("Unexpected number of EFeatureData instances found", GEOMETRIES.length, 
                    count);
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, e.getMessage(), e);
            fail(e);
    ...
    protected void doSetUp() throws Exception {
        ...
        eFeatureCompatibleDataInfo = eContextHelper.eGetFeatureInfo("efeature" +
