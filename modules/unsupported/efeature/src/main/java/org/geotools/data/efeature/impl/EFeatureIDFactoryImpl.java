...
public class EFeatureIDFactoryImpl implements EFeatureIDFactory {
     ...
     */
    protected Map<URI, WeakHashMap<EObject, Long>> eInverseIDMap = new HashMap<URI, 
            WeakHashMap<EObject, Long>>();
     ...
     * garbage collection.
     */
    protected Map<URI, Map<Long, WeakHashSet<EObject>>> eIDMap = new HashMap<URI, Map<Long, 
            WeakHashSet<EObject>>>();
     ...
     *                                  more {@link EAttribute#getEAttributeType()}s are not 
     *                                  {@link String}.
...
//        verify("eURI",eURI,true);
    ...
    protected String eCacheID(URI eURI, EObject eImpl, Long uID) {
        ...
        if (eImpl instanceof EFeatureDelegate) {
            throw new IllegalArgumentException("IDs can not be associated with EFeatureDelegate " +
                    "instances");
        }
        //
    ...
    protected void verify(String eType, EObject eObject, boolean isNonNull, boolean isValid, 
                          boolean isLoaded) {
        ...
        if (isLoaded && eObject.eResource() == null) {
            throw new IllegalStateException(eType + " " + getName(eObject) + " is not loaded into" +
                    " a resource");
        }
    }
