...
public final class GeometryUtils {
    ...
    public static Envelope getWholeWorld() {
        if (WHOLE_WORLD == null) {
            ...
            final DirectPosition lowerCorner = geometryFactory.createDirectPosition(new 
                    double[]{-90, -180});
            final DirectPosition upperCorner = geometryFactory.createDirectPosition(new 
                    double[]{90, 180});
    ...
    public static boolean intersects(final Envelope envelope1, final Envelope envelope2) {
        ...
        if (!crs.equals(bot1.getCoordinateReferenceSystem())
            ...
            throw new IllegalArgumentException(
                    "Current implementation of GeoemtryUtils.intersect requires that the corners " +
                            "of both Envelopes have the same CRS");
        }
    ...
    public static double[] getPoints(
        ...
        for (int i = 0; i < positions.length; i++) {
            ...
            for (int j = 0; j < axisIndices.length; j++) {
                returnable[(i * dimension) + j] = converters[j].convert(positions[i].getOrdinate
                        (axisIndices[j]));
            }
        }
    ...
    public static DirectPosition ensureWGS84(DirectPosition dp) {
        ...
        DirectPosition dp2 = BasicFactories.getDefault().getGeometryFactory(wgs84crs)
        ...
        */
        ...
        try {
        	...
        	if (dp2.getCoordinateReferenceSystem() != null) {
        		...
        		getLog().warn(
        			"Attempted to convert coordinate CRS, transform method returned DirectPosition
        			 with null CRS, using original ordinates",
     ...
     *                                  For the present version, only Curves that wrap only 
     *                                  LineStrings are convertible.
     */
    ...
    public static DirectPosition[] getDirectPositions(final Ring ring) {
        ...
        if (directPositionList.size() > 0) {
            return (DirectPosition[]) directPositionList.toArray(new 
    ...
    private static Curve createCurve(
        ...
        final BasicFactories commonFactory = BasicFactories.getDefault();
        final GeometryFactory geometryFactory = commonFactory.getGeometryFactory(primitiveFactory
                .getCoordinateReferenceSystem());

        final List curveSegmentList = Collections.singletonList(createLineString(geometryFactory,
                points));
    ...
    private static LineString createLineString(
        ...
        final LineString lineString = geometryFactory.createLineString(new ArrayList(Arrays
                .asList(points)));
        return lineString;
    }
    ...
    /*public static void main(String[] args) {
        ...
        System.out.println("directions "+targetDirections[0]+", "+targetDirections[1]+", 
        "+targetDirections[2]);
    ...
    public static void ensureDimensionMatch(
        ...
        if (dimension != expectedDimension) {
            throw new MismatchedDimensionException(name + " does not have " + dimension + 
                    "dimension(s)"
                    /*
                     * Resources.format(
                     * ResourceKeys.ERROR_MISMATCHED_DIMENSION_$3,
