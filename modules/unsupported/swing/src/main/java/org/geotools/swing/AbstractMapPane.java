...
public abstract class AbstractMapPane extends JPanel
   ...
   * DefaultRenderingExecutor} will be set on the next call to {@linkplain #getRenderingExecutor()}.
   *
   ...
   * avoid flickering and redundant rendering when drag-resizing the pane or panning the map image.
   *
    ...
    if (mapContent != null && !mapContent.layers().isEmpty()) {
      try {
        ...
        if (fullExtent == null) {
          ...
          fullExtent =
              new ReferencedEnvelope(-1, 1, -1, 1, mapContent.getCoordinateReferenceSystem());
          ...
          fullExtent =
              new ReferencedEnvelope(
                  xmin, xmax, ymin, ymax, mapContent.getCoordinateReferenceSystem());
        }
