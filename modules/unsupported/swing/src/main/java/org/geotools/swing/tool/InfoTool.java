...
public class InfoTool extends CursorTool implements TextReporterListener {
  ...
  /**
   * Respond to a mouse click by querying each of the {@code Layers}. The details of features lying
   * within the threshold distance of the mouse position are reported on screen using a {@code
   * JTextReporter} dialog.
   *
   * <p><b>Implementation note:</b> An instance of {@code InfoToolHelper} is created and cached for
   * each of the {@code Layers}. The helpers are created using reflection to avoid direct references
   * to grid coverage classes here that would required JAI (Java Advanced Imaging) to be on the
   * classpath even when only vector layers are being used.
   *
   ...
   * reporter dialog is not currently displayed. The connection should not be stored because it will
   * expire when the reporter dialog is closed.
   *
