...
public class MapPaneKeyHandler extends KeyAdapter {
  ...
  static {
    ...
    defaultBindings.put(
        new KeyInfo(KeyEvent.VK_DOWN, KeyEvent.SHIFT_DOWN_MASK, "Shift+Down"), Action.ZOOM_OUT);
  ...
  /**
   * Sets the bindings to those specified in {@code newBindings}. This method differs to {@linkplain
   * #setBindings(java.util.Map)} in that any actions which do not appear in the input map are
   * disabled.
   *
   * @param newBindings new key bindings
   * @throws IllegalArgumentException if {@code newBindings} is {@code null}
