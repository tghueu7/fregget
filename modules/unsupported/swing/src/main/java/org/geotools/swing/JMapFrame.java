...
public class JMapFrame extends JFrame {
   ...
   * MapContent}; and displays the frame. If {@linkplain MapContent#getTitle()} returns a non-empty
   * string, this is used as the frame's title.
   *
  ...
  /**
   * Sets whether to display the default toolbar (default is false). Calling this with state == true
   * is equivalent to calling {@link #enableTool} with all {@link JMapFrame.Tool} constants.
   *
  ...
  /**
   * Get the map content associated with this frame. Returns {@code null} if no map content has been
   * set explicitly with the constructor or {@link #setMapContent}.
   *
