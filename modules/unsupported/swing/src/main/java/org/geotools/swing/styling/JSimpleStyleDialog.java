...
public class JSimpleStyleDialog extends JDialog {
   ...
   * preferences to use with the given {@code StyleLayer}. The layer's existing style, if any, will
   * be used to initialize the dialog.
   *
  ...
  /**
   * Gets the geometry type of the selected feature type. Returns {@code null} if the user cancelled
   * the dialog.
   *
  ...
  private void initComponents() {
    ...
    pointSymbolCBox.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            pointSymbolName =
                WELL_KNOWN_SYMBOL_NAMES[pointSymbolCBox.getSelectedIndex()].toString();
          }
        });
