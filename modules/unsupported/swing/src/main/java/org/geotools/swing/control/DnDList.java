...
public class DnDList<T> extends JList
   ...
   * state. The getDropAction() method returns the operation that the drop site selected to apply to
   * the Drop operation. Once this method is complete, the current DragSourceContext and associated
   ...
   * Transferable returned by DropTargetDropEvent.getTransferable() is guaranteed to succeed only if
   * the data transfer is local; that is, only if DropTargetDropEvent.isLocalTransfer() returns
