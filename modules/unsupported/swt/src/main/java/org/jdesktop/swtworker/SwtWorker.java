...
public abstract class SwtWorker<T, V> implements Future<T>, Runnable {
   ...
   * perform completion actions on the <i>Event Dispatch Thread</i>. Note that you can query status
   * inside the implementation of this method to determine the result of this task or whether this
   ...
   *
   * <p>Because {@code PropertyChangeListener}s are notified asynchronously on the <i>Event Dispatch
   * Thread</i> multiple invocations to the {@code setProgress} method might occur before any {@code
   * PropertyChangeListeners} are invoked. For performance purposes all these invocations are
  ...
  /**
   * Schedules this {@code SwingWorker} for execution on a <i>worker</i> thread. There are a number
   * of <i>worker</i> threads available. In the event all <i>worker</i> threads are busy handling
   * other {@code SwingWorkers} this {@code SwingWorker} is placed in a waiting queue.
   *
  ...
  /**
   * Adds a {@code PropertyChangeListener} to the listener list. The listener is registered for all
   * properties. The same listener object may be added more than once, and will be called as many
   * times as it is added. If {@code listener} is {@code null}, no exception is thrown and no action
   * is taken.
   *
   ...
   * If {@code listener} is {@code null}, or was never added, no exception is thrown and no action
   * is taken.
   *
   * <p>Note: This is merely a convenience wrapper. All work is delegated to {@code
   * PropertyChangeSupport} from {@link #getPropertyChangeSupport}.
   ...
   *
   * <p>returns the service stored in the appContext or creates it if necessary. If the last one it
   * triggers autoShutdown thread to get started.
   *
