...
public class SLDs extends SLD {
    ...
    for (GraphicalSymbol gs : graphic.graphicalSymbols()) {
      if ((gs != null) && (gs instanceof Mark)) {
        ...
        if (fill != null) {
          ...
          colour =
              new Color(
                  colour.getRed() / 255f, colour.getGreen() / 255f, colour.getBlue() / 255f, alpha);
          if (colour != null) {
            return colour;
          }
        }
  ...
  /**
   * Converts the type name of all FeatureTypeStyles to Feature so that the all apply to any feature
   * type. This is admittedly dangerous but is extremely useful because it means that the style can
   * be used with any feature type.
   *
   * @param ftStyles
   */
