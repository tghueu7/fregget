...
public final class ExceptionMonitor {
  ...
  /**
   * Displays an error message for the specified exception. Note that this method can be called from
   * any thread (not necessarily the <cite>Swing</cite> thread).
   *
  ...
  public static void show(final Shell parent, final Throwable exception, final String message) {
    if (Display.getCurrent() != null) {
      Status status =
          new Status(IStatus.ERROR, "My Plug-in ID", 0, "Status Error Message", exception);
