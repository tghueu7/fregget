...
public class InfoTool extends CursorTool {
  ...
  /**
   * Constructs a new info tool. To activate the tool only on certain mouse events provide a single
   ...
   * lying within the threshold distance of the mouse position are reported on screen using a {@code
   * JTextReporter} dialog.
   *
   * <p><b>Implementation note:</b> An instance of {@code InfoToolHelper} is created and cached for
   * each of the {@code MapLayers}. The helpers are created using reflection to avoid direct
   * references to grid coverage classes here that would required JAI (Java Advanced Imaging) to be
   * on the classpath even when only vector layers are being used.
   *
    ...
    for (Layer layer : context.layers()) {
      if (layer.isSelected()) {
        ...
        if (helper instanceof VectorLayerHelper) {
          ...
          double searchWidth =
              DEFAULT_DISTANCE_FRACTION * (mapEnv.getWidth() + mapEnv.getHeight()) / 2;
          try {
  ...
  private void createReporter() {
    if (reporter == null || reporter.getShell() == null || reporter.getShell().isDisposed()) {
      Shell shell =
          new Shell(Display.getCurrent(), SWT.TITLE | SWT.CLOSE | SWT.BORDER | SWT.MODELESS);
