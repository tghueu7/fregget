...
public abstract class InfoToolHelper<T> {
   ...
   * @param dataCRS the coordinate reference system of the feature data that will be queried by this
   *     helper
   */
   ...
   *
   * <p>Helpers only hold a {@linkplain WeakReference} to the map context and layer with which they
   * are working to avoid blocking garbage collection when layers are discarded. If this method
   * returns {@code false} the helper should be re-created.
   *
   * <pre><code>
   * //
  ...
  /**
   * Get the {@code MapContent} associated with this helper. The helper maintains only a {@linkplain
   * WeakReference} to the context.
   *
