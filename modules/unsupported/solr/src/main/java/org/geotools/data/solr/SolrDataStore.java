...
public class SolrDataStore extends ContentDataStore {
  ...
  public SolrDataStore(URL url, SolrLayerMapper layerMapper) {
    ...
    this.solrServer =
        new HttpSolrClient.Builder().withBaseSolrUrl(url.toString()).allowCompression(true).build();
   ...
   * static kinds. <br>
   * For each discovered field a SOLR request is needed to verify if the field has no values in the
   * actual type, this information will be stored in {@link SolrAttribute#setEmpty}. <br>
   * SolrJ not extracts information about uniqueKey so custom class {@link ExtendedFieldSchemaInfo}
   * is used. <br>
