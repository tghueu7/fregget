...
public final class TestsSolrUtils {
  ...
  /**
   * Instantiates a new Apache Solr client using the provided base URL. The provided base URL should
   * point to a specific Solr core.
   *
  ...
  public static HttpSolrClient instantiateClient(String baseUrl) {
    HttpSolrClient client =
        new HttpSolrClient.Builder().withBaseSolrUrl(baseUrl).allowCompression(true).build();
  ...
  /**
   * Performs an update request extracting the necessary Solr documents from the provided stream. It
   * is expected that the stream provides Solr documents encoded in the XML format:
   *
   * <p>
   *
   * <pre>{@code
  ...
  /**
   * Create field type named bbox capable of handling envelopes. If a field type named bbox already
  ...
  /**
   * Adds a new wkt field type to the schema, i.e. a field capable of handling WKT geometries. If a
  ...
  /**
   * Helper method that creates a new field type using the provided attributes. If a field type with
   * the same name already exists its definition will be replaced.
   *
   * @param client Sorl client to use, should already point to the desired core
  ...
  private static void createFieldType(HttpSolrClient client, Map<String, Object> attributes) {
    ...
    if (replaceResponse.hasErrors()) {
      // trying to replace the field type failed, let's throw an exception with all the messages
      // errors
  ...
  /**
   * Helper method that creates a new field using the provided name and type (field type name). If a
   * field with the same name already exists itd definition will be replaced.
   *
   * @param client Sorl client to use, should already point to the desired core
   * @param name name of the field
