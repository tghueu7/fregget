...
public class BilinearInterpolator {
  ...
  /**
   * Interpolates across an edge grid cell, which has 1 or more NO_DATA values. Grid cells with 2 or
   * or NO_DATA values still receive the value NO_DATA. Otherwise, the cell is interpolated across
   * the triangle defined by the 3 valid corner values. This produces a much smoother edge
  ...
  private float interpolateBoundaryCell(
     ...
     * normalized to put noData in NE. This is done by flipping the cell across the X or Y axis, or
     * both (and transforming the point offsets likewise)
     */
    if (v00 == noDataValue)
   ...
   * corner (<tt>v11</tt>), and valid data values in the three other corners. The interpolated value
   * is computed using linear interpolation across the 3D triangle defined by the three valid
   * corners.
   *
   * @param xfrac the fractional x location of the interpolation point within the cell
   * @param yfrac the fractional y location of the interpolation point within the cell
