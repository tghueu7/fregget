 ...
 * @source $URL$
 */
@DescribeProcess(title = "Aggregate", description = "Computes one or more aggregation functions " +
        "on a feature attribute. Functions include Count, Average, Max, Median, Min, StdDev, and " +
        "Sum.")
public class AggregateProcess implements VectorProcess {
     ...
     */
    public static Results process(SimpleFeatureCollection features, String aggAttribute, 
                                  Set<AggregationFunction> functions, Boolean singlePass, 
    ...
    public static Results process(SimpleFeatureCollection features, String aggAttribute, 
                                  ...
                                  List<String> groupByAttributes, Boolean singlePass, 
                                  ProgressListener progressListener) throws ProcessException, 
            IOException {
        AggregateProcess process = new AggregateProcess();
        return process.execute(features, aggAttribute, functions, singlePass, groupByAttributes, 
                progressListener);
    }

    public Results execute(SimpleFeatureCollection features, String aggAttribute, 
                           Set<AggregationFunction> functions,
                           boolean singlePass, ProgressListener progressListener) throws 
            ProcessException, IOException {
        return execute(features, aggAttribute, functions, singlePass, null, progressListener);
    }

    @DescribeResult(name = "result", description = "Aggregation results (one value for each " +
            "function computed)")
    public Results execute(
            @DescribeParameter(name = "features", description = "Input feature collection") 
                    SimpleFeatureCollection features,
            @DescribeParameter(name = "aggregationAttribute", min = 0, description = "Attribute " +
                    "on which to perform aggregation") String aggAttribute,
            @DescribeParameter(name = "function", description = "An aggregate function to compute" +
                    ". Functions include Count, Average, Max, Median, Min, StdDev, Sum and " +
                    "SumArea.", collectionType = AggregationFunction.class) 
                    Set<AggregationFunction> functions,
            @DescribeParameter(name = "singlePass", description = "If True computes all " +
                    "aggregation values in a single pass (this will defeat DBMS-specific " +
                    "optimizations)", defaultValue = "false") boolean singlePass,
            @DescribeParameter(name = "groupByAttributes", min = 0, description = "List of group " +
                    "by attributes", collectionType = String.class) List<String> groupByAttributes,
        ...
        if (groupByAttributes != null && !groupByAttributes.isEmpty()) {
            ...
            return handleGroupByVisitor(features, aggAttribute, functions, groupByAttributes, 
                    progressListener);
        }
        ...
        EnumMap<AggregationFunction, Number> results = new EnumMap<AggregationFunction, Number>
    ...
    /**
     * Helper method that handle requests that have group by attributes by wrapping the functions
     * in group by visitors.
     */
    private Results handleGroupByVisitor(SimpleFeatureCollection features, String aggAttribute, 
                                         Set<AggregationFunction> functions,
                                         List<String> rawGroupByAttributes, ProgressListener 
                                                 progressListener) throws IOException {
        // building a group by visitor for every aggregate function

        FilterFactory factory = CommonFactoryFinder.getFilterFactory(null);

        List<GroupByVisitor> groupByVisitors = functions.stream().map(function -> new 
                GroupByVisitorBuilder()
                .withAggregateAttribute(function == AggregationFunction.SumArea ?
                        factory.function("area2", factory.property(features.getSchema()
                                .getDescriptor(aggAttribute).getLocalName())) :
                        factory.property(features.getSchema().getDescriptor(aggAttribute)
                                .getLocalName()))
        ...
        return new Results(aggAttribute, functions, rawGroupByAttributes, mergeResults(results, 
                rawGroupByAttributes.size()));
    }

    /**
     * Helper method that merge all group by visitors results in a tabular format. Each line of 
     * the table is composed
     * of the group by attributes values and the aggregation functions results.
     */
    private List<Object[]> mergeResults(List<Map<List<Object>, Object>> results, int 
        ...
        // the group by attributes values are equal for all the visitors so we use the first 
        // visitor result to grab all the group by values
        for (List<Object> groupByAttributes : results.get(0).keySet()) {
            ...
            // we extract from each group by visitor result the aggregation function result and 
            // add it to out table line
    ...
    public static final class Results {
        ...
        // this values are used by output formats that want to add more meta information (the 
        // JSON tabular output format for example)
        ...
        public Results(String aggregateAttribute, Set<AggregationFunction> functions, 
                       List<String> groupByAttributes, List<Object[]> groupByResult) {
