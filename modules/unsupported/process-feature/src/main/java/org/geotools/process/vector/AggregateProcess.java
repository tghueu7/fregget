...
public class AggregateProcess implements VectorProcess {
  ...
  private List<Object[]> mergeResults(
    ...
    // the size of each line is the number of the group by attributes plus the number of aggregation
    // functions
    ...
    for (List<Object> groupByAttributes : results.get(0).keySet()) {
      ...
      // we extract from each group by visitor result the aggregation function result and add it to
      // out table line
