...
public class BarnesSurfaceProcess implements VectorProcess {
   ...
   * stored in an image format, in a YX grid with 0 being the top.
   *
  ...
  private Filter expandBBox(Filter filter, double distance) {
    return (Filter)
        filter.accept(new BBOXExpandingFilterVisitor(distance, distance, distance, distance), null);
  }
    ...
    try {
      while (obsIt.hasNext()) {
        ...
        try {
          ...
          // throw new ProcessException("Expression " + attrExpr + " failed to evaluate to a numeric
          // value", e);
        }
      }
    } finally {
