 ...
 * with cell values in the same domain as the input observation field specified by 
 * <code>valueAttr</code>.
 * <p>
...
@DescribeProcess(title = "BarnesSurface", description = "Uses Barnes Analysis to compute an " +
        "interpolated surface over a set of irregular data points.")
public class BarnesSurfaceProcess implements VectorProcess {
    ...
    public GridCoverage2D execute(
            ...
            @DescribeParameter(name = "valueAttr", description = "Name of attribute containing " +
                    "the data value to be interpolated") String valueAttr,
            @DescribeParameter(name = "dataLimit", description = "Limit for the number of input " +
                    "features processed", min = 0, max = 1) Integer argDataLimit,

            // process parameters
            @DescribeParameter(name = "scale", description = "Length scale for the interpolation," +
                    " in units of the source data CRS", min = 1, max = 1) Double argScale,
            @DescribeParameter(name = "convergence", description = "Convergence factor for " +
                    "refinement (between 0 and 1, default 0.3)", min = 0, max = 1, defaultValue =
                    "0.3") Double argConvergence,
            @DescribeParameter(name = "passes", description = "Number of passes to compute " +
                    "(default = 2)", min = 0, max = 1) Integer argPasses,
            @DescribeParameter(name = "minObservations", description = "Minimum number of " +
                    "observations required to support a grid cell (default = 2)", min = 0, max = 
                    1, defaultValue = "2") Integer argMinObsCount,
            @DescribeParameter(name = "maxObservationDistance", description = "Maximum distance " +
                    "to an observation for it to support a grid cell, in units of the source CRS " +
                    "(default = 0, meaning all observations used)", defaultValue = "0", min = 0, 
                    max = 1) Double argMaxObsDistance,
            @DescribeParameter(name = "noDataValue", description = "Value to use for NO_DATA " +
                    "cells (default = -999)", defaultValue = "-999", min = 0, max = 1) Double 
                    argNoDataValue,
            @DescribeParameter(name = "pixelsPerCell", description = "Resolution of the computed " +
                    "grid in pixels per grid cell (default = 1)", defaultValue = "1", min = 0, 
                    max = 1) Integer argPixelsPerCell,

            // query modification parameters
            @DescribeParameter(name = "queryBuffer", description = "Distance to expand the query " +
                    "envelope by, in units of the source CRS (larger values provide a more stable" +
                    " surface)", min = 0, max = 1) Double argQueryBuffer,

            // output image parameters
            @DescribeParameter(name = "outputBBOX", description = "Bounding box for output") 
                    ReferencedEnvelope outputEnv,
            @DescribeParameter(name = "outputWidth", description = "Width of the output raster in" +
                    " pixels") Integer outputWidth,
            @DescribeParameter(name = "outputHeight", description = "Height of the output raster " +
                    "in pixels") Integer outputHeight,
        ...
        float[][] barnesGrid = createBarnesGrid(pts, dstLengthScale, convergenceFactor, passes, 
                minObsCount, dstMaxObsDistance, noDataValue, outputEnv, gridWidth, gridHeight);
        ...
        GridCoverageFactory gcf = CoverageFactoryFinder.getGridCoverageFactory(GeoTools
                .getDefaultHints());
    ...
    public Query invertQuery(
            @DescribeParameter(name = "queryBuffer", description = "The distance by which to " +
                    "expand the query window", min = 0, max = 1) Double argQueryBuffer,
    ...
    public static Coordinate[] extractPoints(SimpleFeatureCollection obsPoints, String attrName, 
                                             MathTransform trans, int dataLimit) throws 
            CQLException {
        ...
        try {
            while (obsIt.hasNext()) {
                ...
                try {
                    ...
                    //throw new ProcessException("Expression " + attrExpr + " failed to evaluate 
                    // to a numeric value", e);
                }
            }
        } finally {
            obsIt.close();
