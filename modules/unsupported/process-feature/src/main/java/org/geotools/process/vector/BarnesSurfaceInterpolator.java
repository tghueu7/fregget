...
public class BarnesSurfaceInterpolator {
   ...
   *
   * <p>Using these parameters is optional, but recommended, since estimating grid points which are
   * far from any observations can produce unrealistic surfaces.
   */
  ...
  /**
   * Sets the length scale for the interpolation weighting function. The length scale is determined
   * from the distance between the observation points, as well as the scale of the phenomena which
   * is being measured.
   *
   * <p>
   *
   ...
   * observation. Empirically determined; a reasonable starting point is between 1.5 and 2 times the
   * Length scale. If the value is 0 (which is the default), all grid points are considered to be
  ...
  /**
   * Sets the NO_DATA value used to indicate that a grid cell was not computed. This value should be
   * distinct from any potential data value.
   *
  ...
  private double weight(double dist, double lengthScale, double convergenceFactor) {
     ...
     *
     * <p>The effective radius grows as a log function of the cutoff weight, so even for very small
     * cutoff weight values, the effective radius is only a few times the size of the influence
     * radius.
     *
     * <p>Also, dropping observation terms from the estimate results in very drastic (discontinuous)
     * changes at distances around the effective radius. (Probably because beyond that distance
     * there are very few terms (maybe only 2) contributing to the estimate, so there is no
  ...
  /**
   * Computes effective radius which is determined by the specified cutoff weight and the radius of
   * the decay function.
   *
   * @param cutoffWeight
   * @param radius
