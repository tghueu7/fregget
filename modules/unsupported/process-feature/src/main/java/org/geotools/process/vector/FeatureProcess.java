...
@DescribeProcess(title = "Feature from Geometry", description = "Converts a geometry into a " +
        "feature collection.")
/**
 *
 *
...
public class FeatureProcess implements VectorProcess {
    ...
    public SimpleFeatureCollection execute(
            ...
            @DescribeParameter(name = "crs", description = "Coordinate reference system of the " +
                    "input geometry (if not provided in the geometry)") CoordinateReferenceSystem
                    crs,
            @DescribeParameter(name = "typeName", description = "Feauturetype name for the " +
                    "feature collection", min = 1) String name) {
        // get the crs
        if (crs == null) {
            try {
