...
public class GridFeatureCacheTest extends AbstractFeatureCacheTest {
    ...
    public void testEviction() throws IOException, FeatureCacheException {
        ...
        for (int i = 0; i < 11; i++) {
            for (int j = 0; j < 11; j++) {
                ...
                SimpleFeatureCollection control = ds.getFeatureSource(dataset.getSchema().getName
                        ()).getFeatures(f);
                SimpleFeatureCollection c = cache.getFeatures(f);

                assertEquals(control.size(), c.size());

                if (!testEviction_holistic && (cache.tracker.getEvictions() > 10)) { // wait to 
                    // generate a fair amount of evictions,
    ...
    public void testEviction2() throws IOException, FeatureCacheException {
        ...
        FeatureCache cache = new GridFeatureCache(ds.getFeatureSource(featureType.getTypeName()),
                4, 4, MemoryStorage.createInstance());
    ...
    public void testPut() throws CacheOversizedException {
        ...
        FeatureCollectingVisitor v = new FeatureCollectingVisitor((SimpleFeatureType) dataset
                .getSchema());
