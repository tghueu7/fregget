...
public class StreamingGridFeatureCache extends GridFeatureCache {
    ...
    public StreamingGridFeatureCache(SimpleFeatureSource fs, int indexcapacity, int capacity, 
                                     Storage store) throws FeatureCacheException {
        super(fs, indexcapacity, capacity, store);
    }

    public StreamingGridFeatureCache(SimpleFeatureSource fs, ReferencedEnvelope env, int 
            indexcapacity, int capacity, Storage store) {
    ...
    public SimpleFeatureCollection get(Envelope e) throws IOException {
        Filter f = ff.bbox(getSchema().getGeometryDescriptor().getLocalName(), e.getMinX(), e
                .getMinY(), e.getMaxX(), e.getMaxY(), getSchema().getCoordinateReferenceSystem()
                .toString());
    ...
    private BBOX createFilterFromBounds() throws IOException {
        ...
        BBOX myfilter = ff.bbox(getSchema().getGeometryDescriptor().getLocalName(), env.getMinX()
                , env.getMinY(), env.getMaxX(), env.getMaxY(), getSchema()
    ...
    private SimpleFeatureCollection generateFeatureCollection(BBOX envFilter, Filter postFilter) {
        GridCachingFeatureCollection collection = new GridCachingFeatureCollection(envFilter, 
                postFilter, this, this.fs, true);
        return collection;
    }
    ...
    public void put(FeatureCollection fc, Envelope e) throws CacheOversizedException {
        throw new UnsupportedOperationException("Streaming feature cache does not put entire " +
                "feature collections.");
    }
    ...
    public void remove(Envelope e) {
        ...
        try {
            ...
            this.tracker.getStatistics().addToDataCounter(-v.getDataCount());    //update stats 
            // to remove all data
