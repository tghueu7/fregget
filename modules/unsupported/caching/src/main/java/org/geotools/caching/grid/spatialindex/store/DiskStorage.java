...
public class DiskStorage implements Storage {
    ...
    public static Storage createInstance(Properties pset) {
        try {
            ...
            logger.log(Level.WARNING, "DiskStorage : error occured when creating new instance : "
                    + e.getMessage(), e);
            return null;
     ...
     */
    private Node readNode(byte[] data, NodeIdentifier id) throws IOException, 
        ...
        try {
            ...
            try {
                ...
                for (Iterator<FeatureType> iterator = featureTypes.iterator(); iterator.hasNext()
                    ...
                    oos.writeObject(type.getName().getNamespaceURI() + "." + type.getName()
                            .getLocalPart());
                    oos.writeObject(rep);
                }
            } finally {
