...
public class BBoxFilterSplitter implements FilterVisitor {
    ...
    private static final Envelope UNIVERSE_ENVELOPE = new Envelope(Double.NEGATIVE_INFINITY, 
    ...
    public Object visit(BBOX f, Object arg1) {
        if (geom == null) {
            ...
            String newgeom = f.getExpression1() instanceof PropertyName ? ((PropertyName) f
                    .getExpression1()).getPropertyName() : null;
            String newsrs = f.getSRS();

            if ((geom != newgeom) || srs != srs) {
                throw new UnsupportedOperationException(
                        "This splitter can not be used against a filter where different BBOX " +
                                "filters refer to different Geometry attributes.");
            }
        }
    ...
    public Filter getFilterPre() {
        ...
        if (e == null || e.isNull()) {
            ...
            Filter myfilter = filterFactory.bbox(geom, e.getMinX(), e.getMinY(), e.getMaxX(), e
                    .getMaxY(), srs);
            return myfilter;
        }
    }
