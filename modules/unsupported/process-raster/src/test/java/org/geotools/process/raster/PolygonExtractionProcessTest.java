...
public class PolygonExtractionProcessTest {
  ...
  private GridCoverage2D buildSmallCoverage() {
    ...
    GridCoverage2D cov =
        covFactory.create(
            "coverage", DATA, new ReferencedEnvelope(0, DATA[0].length, 0, DATA.length, null));
    return cov;
  }
  ...
  public void ignoreInteriorBoundariesBetweenRegions() throws Exception {
    ...
    GridCoverage2D cov =
        covFactory.create("coverage", DATA, new ReferencedEnvelope(0, width, 0, height, null));
  ...
  /**
   * This test uses an image that caused the original raster to vector code to fail. It is kept here
  ...
  public void testCallViaProcessFactory() {
    ...
    inputs.put(
        "ranges",
        Arrays.asList(new Range<Integer>(0, true, 1, true), new Range<Integer>(2, true, 3, true)));
