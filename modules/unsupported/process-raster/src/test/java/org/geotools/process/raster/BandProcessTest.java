...
public class BandProcessTest {
  ...
  public static void setup()
    ...
    // Third file selection (This file is similar to the first one but it is translated by 1 on the
    // X direction)
  ...
  public void testROI() throws IOException, MismatchedDimensionException, TransformException {
    ...
    // Ensure that the images are equals on the cropped selection (The new images contains no data
  ...
  private Geometry createGeometry(GridCoverage2D coverage) {
    ...
    for (int i = 0; i < coordinates.length; i++) {
      ...
      switch (i) {
        ...
        case 1:
          coordinates[i] =
              new Coordinate(envelope.getMinX(), envelope.getMinY() + envelope.getHeight() / 2);
          break;
        case 2:
          coordinates[i] =
              new Coordinate(
        ...
        case 3:
          coordinates[i] =
              new Coordinate(envelope.getMinX() + envelope.getWidth() / 2, envelope.getMinY());
          break;
      }
    }
  ...
  private void ensureNoDataOutside(GridCoverage2D coverage, Geometry geom)
    ...
    for (int tx = minTileX; tx < maxTileX; tx++) {
      for (int ty = minTileY; ty < maxTileY; ty++) {
        ...
        int maxx =
            Math.min(
                minx + tile.getWidth(), img.getMinX() + img.getWidth() - tx * img.getTileWidth());
        int maxy =
            Math.min(
                miny + tile.getHeight(),
