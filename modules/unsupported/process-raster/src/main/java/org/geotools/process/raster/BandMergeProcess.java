 ...
 * <li>an optional String indicating the policy for choosing the Grid To World transformation
 * (from those of all the Coverages) to use for the final
 * coverage. The available values are: FIRST(default), for selecting the first coverage; LAST, 
 * for the last coverage; INDEX, for selecting the
 ...
 * for choosing the coverage at the "Index" position.</li>
 * </ul>
 * <p>
 * The output of this process is a {@link GridCoverage2D} object which contains all the input 
 * Coverages, each one stored as a Band (or multiple bands
 * if the coverage is multibanded). This process can be used also for merging coverages which are
 * not aligned and with different resolutions.
 *
 * @author Nicola Lagomarsini, GeoSolutions S.A.S.
 */
@DescribeProcess(title = "Merge Coverages", description = "Returns a raster generated by the " +
        "merge of the input raster bands. Source rasters must have the same CRS.")
public class BandMergeProcess implements RasterProcess {
    ...
    public GridCoverage2D execute(
            @DescribeParameter(name = "coverages", description = "Coverage List", min = 1, 
                    collectionType = GridCoverage2D.class) Collection<GridCoverage2D> coverages,
            @DescribeParameter(name = "roi", description = "Geometry to use as ROI", min = 0) 
                    Geometry roi,
            @DescribeParameter(name = "transformChoice", description = "Choice on which Coverage " +
                    "G2W transform to use", min = 0) String transformChoice,
            @DescribeParameter(name = "index", description = "Index used by the transformChoice " +
                    "parameter", min = 0) Integer index)
