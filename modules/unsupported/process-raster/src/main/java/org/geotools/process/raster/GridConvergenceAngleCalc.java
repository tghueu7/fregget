 ...
 * Reference System. The Grid Convergence Angle at any
 * point on a projected 2D map is the difference between "up" or "grid north" on the map at that 
 * point and True North. Since the map is projected, the
 * Grid Convergence Angle can change at each point on the map; True North can be in a different 
 * Cartesian direction on the flat map for every point.
 * One example impact of this is that vectors cannot be accurately drawn on the screen by simply 
 * rotating them a certain amount in "screen degrees."
 * This class, then, is used to estimate the Grid Convergence Angle at those points. Though the 
 * underlying meaning is the same, different mapping
 * conventions define the angle's direction (+/-) and beginning point / ending point differently.
 * Therefore, for the purposes of this code, the Grid
 * Convergence Angle is defined as the angle (C) FROM true north TO grid north with 0 deg up and 
 * angles increasing positively clockwise. So, for the
 * example below, C would be approximately -33 deg, since the angle FROM true north TO grid north
 ...
 * between a line extending toward grid north and one
 * extending toward true north THAT HAVE BEEN PROJECTED into the map projection, but suggests the
 * difference between this angle and the way the angle
...
final class GridConvergenceAngleCalc {
    ...
    /**
     * Estimates the grid convergence angle at a position within a Coordinate Reference System. 
     * The angle returned is as described in the
     * documentation for the class. The Coordinate Reference System of the supplied position must
     * be the same as was used when constructing the
     * calculator, because using anything else would not make sense as convergence angle depends 
     * on projection.
     *
