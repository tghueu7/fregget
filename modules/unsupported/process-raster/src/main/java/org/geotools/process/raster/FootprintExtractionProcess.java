...
public class FootprintExtractionProcess implements RasterProcess {
   ...
   *     which luminance values should be excluded from the search. This allows you, as an instance,
   *     to exclude "Dark pixels / Almost black pixels" from the results.
   * @param thresholdArea An optional {@code thresholdArea} parameter is supported to exclude small
   *     polygons from the final result. In case a polygon has an area (in pixels) smaller than the
   *     provided value it will not be included in the footprint. Default Threshold Area is {@link
   *     MarchingSquaresVectorizer#DEFAULT_THRESHOLD_AREA}
   ...
   * @param removeCollinear specifies whether the collinear vertices of the retrieved polygon should
   *     be removed (Default is true)
   * @param forceValid specifies whether polygons should be forced to be valid (also removing holes)
   * @param imageLoadingType specifies the type of imageLoading (DEFERRED vs IMMEDIATE). Default is
   *     {@link ImageLoadingType#getDefault()}.
