...
public class FootprintExtractionProcess implements RasterProcess {
     ...
     * @param exclusionRanges            An optional {@code exclusionRanges} parameter is 
     *                                   supported to define
     *                                   which luminance values should be excluded from the 
     *                                   search. This allows you, as an
     *                                   instance, to exclude "Dark pixels / Almost black pixels"
     *                                   from the results.
     * @param thresholdArea              An optional {@code thresholdArea} parameter is supported
     *                                  to exclude
     *                                   small polygons from the final result. In case a polygon 
     *                                   has an area (in pixels)
     *                                   smaller than the provided value it will not be included 
     *                                   in the footprint. Default
     *                                   Threshold Area is 
     *                                   {@link MarchingSquaresVectorizer#DEFAULT_THRESHOLD_AREA}
     * @param computeSimplifiedFootprint An optional {@code computeSimplifiedFootprint}
     *                                   parameter is supported to return a simplified version of
     *                                   the biggest polygon.
     * @param simplifierFactor           the simplifier factor to be applied to compute the 
     *                                   simplified
     ...
     * @param forceValid                 specifies whether polygons should be forced to be valid 
     *                                   (also removing holes)
    ...
    public SimpleFeatureCollection execute(
            @DescribeParameter(name = "data", description = "Source raster") GridCoverage2D 
                    coverage,
            @DescribeParameter(name = "exclusionRanges", description = "the ranges of luminance " +
                    "values to be excluded by the computation.", min = 0) List<Range<Integer>> 
                    exclusionRanges,
            @DescribeParameter(name = "thresholdArea", description = "Indicates the minimum area " +
                    "of a polygon to be included in the final result", min = 0) Double 
                    thresholdArea,
            @DescribeParameter(name = "computeSimplifiedFootprint", description = "Indicates " +
                    "whether the simplified footprint should be computed", min = 0) Boolean 
                    computeSimplifiedFootprint,
            @DescribeParameter(name = "simplifierFactor", description = "Indicates the simplifier" +
                    " factor to be applied when computing the simplified footprint", min = 0) 
                    Double simplifierFactor,
            @DescribeParameter(name = "removeCollinear", description = "Indicates whether remove " +
                    "collinear point should be applied", min = 0) Boolean removeCollinear,
            @DescribeParameter(name = "forceValid", description = "Indicates whether polygon " +
                    "should be forced to be valid, also removing holes", min = 0) Boolean 
                    forceValid,
            @DescribeParameter(name = "loadingType", description = "Indicates which type of " +
                    "imageLoading should be performed (DEFERRED vs IMMEDIATE)", min = 0) 
        ...
        MarchingSquaresVectorizer vectorizer = new MarchingSquaresVectorizer(coverage, null, 
                thresholdArea, simplifierFactor, imageLoadingType, exclusionRanges);
