...
public final class MarchingSquaresVectorizer {
  ...
  static class ScanInfo {
    /**
     * A trigger which is internally enabled when no polygons having area greater than the threshold
     * have been found. In that case, the algorithm will return the first polygon found.
     */
   ...
   * "Dark" means having a luminance between 0 and a reference value (As an instance: 10) as well as
   * "white pixels" such as clouds (adding a range like 254, 255).
   */
  ...
  /**
   * When set to {@code true} (the default) will perform extra checks on the output polygons to make
   * sure they are valid geometries
   *
   * @param forceValid
   */
  ...
  /**
   * If validation is requested, scan the geometries and build valid polygons (in case they aren't)
   * by also removing holes.
   *
   * @param geometriesList
   * @return
   */
  ...
  private void identifyGeometries(
                  ...
                  if ((col >= minX) && (col <= maxX)) {
                    ...
                    if (!bitSet.get(col - minX, row - minY) && !Double.isNaN(value)) {
                      if (areEqual(value, D_VALUE)) {
                        Polygon polygon =
                            identifyPerimeter(iter, col, row, awtPolygon, sampleDataType, scanInfo);
                        if (polygon != null) {
                          if (removeCollinear) {
                            if (LOGGER.isLoggable(Level.FINE)) {
    ...
    if (!scanInfo.fullyCovered
      ...
      Polygon polygon =
          identifyPerimeter(
              iter, scanInfo.refColumn, scanInfo.refRow, awtPolygon, sampleDataType, scanInfo);
      geometriesList.add(polygon);
    }
  }
    ...
    if (dataType != DataBuffer.TYPE_BYTE) {
      ...
      switch (dataType) {
        case DataBuffer.TYPE_USHORT:
          inputRI =
              worker
                  .lookup(createLookupTableUShort(exclusionLuminanceRanges, dataType))
                  .getRenderedImage();
          break;
        case DataBuffer.TYPE_SHORT:
          ...
          inputRI =
              worker
                  .lookup(createLookupTableByte(exclusionLuminanceRanges, dataType))
                  .getRenderedImage();
          break;
