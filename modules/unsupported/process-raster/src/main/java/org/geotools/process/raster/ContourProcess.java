...
public class ContourProcess implements RasterProcess {
   ...
   * feature collection. You can control which contours are generated either by providing a list of
   * values via the {@code levels} argument, or by specifying the interval between contour values
   * via the {@code interval} argument. In the interval case, the resulting contour values will be
   * integer multiples of the specified interval. If both {@code levels} and {@code interval} are
   * supplied the {@code interval} argument is ignored.
   *
   ...
   * @param simplify whether to simplify contour lines by removing co-linear vertices; default is to
   *     simplify
   * @param smooth whether to apply Bezier smooth to the contours; default is no smoothing
   * @param roi an optional polygonal {@code Geometry} to define the region of interest within which
