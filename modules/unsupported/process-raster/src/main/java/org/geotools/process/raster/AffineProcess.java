 ...
 * background values in the areas which does not
 * represent valid data.
 *
 * @author Nicola Lagomarsini - GeoSolutions S.A.S.
 */
@DescribeProcess(title = "Transformed Coverage", description = "Returns the result of an Affine " +
        "transformation on the input raster.")
public class AffineProcess implements RasterProcess {
    ...
    public GridCoverage2D execute(
            ...
            @DescribeParameter(name = "scalex", description = "Scale parameter for the X " +
                    "direction", min = 0) Double scaleX,
            @DescribeParameter(name = "scaley", description = "Scale parameter for the Y " +
                    "direction", min = 0) Double scaleY,
            @DescribeParameter(name = "shearx", description = "Shear parameter for the X " +
                    "direction", min = 0) Double shearX,
            @DescribeParameter(name = "sheary", description = "Shear parameter for the Y " +
                    "direction", min = 0) Double shearY,
            @DescribeParameter(name = "translatex", description = "Offset parameter for the X " +
                    "direction", min = 0) Double translateX,
            @DescribeParameter(name = "translatey", description = "Offset parameter for the Y " +
                    "direction", min = 0) Double translateY,
            @DescribeParameter(name = "nodata", description = "Output coverage nodata values", 
                    min = 0) double[] nodata,
            @DescribeParameter(name = "interpolation", description = "Interpolation function to " +
                    "use.  Values are NEAREST, BILINEAR, BICUBIC2, BICUBIC", min = 0) 
