...
public class RasterAsPointCollectionProcess implements RasterProcess {
  ...
  public SimpleFeatureCollection execute(
      ...
      @DescribeParameter(name = "scale", description = "scale", min = 0, defaultValue = "1.0f")
          Float scaleFactor,
      @DescribeParameter(
            name = "interpolation",
    ...
    if (scaleFactor != null
      ...
      // Selection of the ScaleFactors in order to check if the final Raster has almost 1 pixel for
      // Height and Width
      ...
      gc2d =
          new AffineProcess().execute(gc2d, scaleX, scaleY, null, null, null, null, null, interp);
    }
  ...
  private static final class RasterAsPointFeatureCollection extends BaseSimpleFeatureCollection {
    ...
    private void emisphereManagement(CoordinateReferenceSystem coverageCRS) throws IOException {
      ...
      if (emisphere) {
        // If the Coverage CRS is already in WGS84 then is is easy to get the Hemisphere from the Y
        // direction value,
          ...
          try {
            ...
            if (northDimension < 0) {
              final IOException ioe =
                  new IOException(
                      "Unable to find nort dimension in the coverage CRS+ " + coverageCRS.toWKT());
              throw ioe;
            }
