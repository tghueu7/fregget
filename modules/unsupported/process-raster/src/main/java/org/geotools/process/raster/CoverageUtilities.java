...
public class CoverageUtilities {
     ...
     * @param mt2d an {@link AffineTransform} that maps from raster to model space. This is 
     *             already referred to the pixel corner.
     * @return a {@link ROI} suitable for using with JAI.
     * @throws ProcessException in case there are problems with ivnerting the provided 
     * {@link AffineTransform}. Very unlikely to happen.
     */
     ...
     * of feature points, mapping the centre of each pixel as a point plus all the bands as 
     * attributes.
     * <p>
     * <p>
     ...
     * coverage
     */
    public static SimpleFeatureType createFeatureType(final GridCoverage2D gc2d, final Class<? 
            extends Geometry> geometryClass) {
...
//		ftBuilder.setCRS(DefaultEngineeringCRS.GENERIC_2D);
    ...
    public static RangeLookupTable getRangeLookupTable(
            ...
            final Number noDataValue) {
        return getRangeLookupTable(classificationRanges, outputPixelValues, noDataValue, 
                noDataValue.getClass());
    }

    public static RangeLookupTable getRangeLookupTable(List<Range> classificationRanges, final 
    int[] outputPixelValues,
                                                       final Number noDataValue, final Class<? 
        ...
        final boolean useCustomOutputPixelValues = outputPixelValues != null && outputPixelValues
                .length == size;
        ...
        rltBuilder.add(new Range(getClassMinimum(widestClass), true, getClassMaximum(widestClass)
                , true), noDataValue);
    ...
    public static it.geosolutions.jaiext.rlookup.RangeLookupTable getRangeLookupTableJAIEXT(
        ...
        final it.geosolutions.jaiext.rlookup.RangeLookupTable.Builder rltBuilder = new it
                .geosolutions.jaiext.rlookup.RangeLookupTable.Builder();
        final int size = classificationRanges.size();
    ...
    private static Number getClassMinimum(Class<? extends Number> numberClass) {
        ...
        throw new UnsupportedOperationException("Class " + numberClass + " can't be used in a " +
                "value Range");
    }
     ...
     * @return the {@link HashMap map} of parameters. ( {@link #NORTH} and the other static vars 
