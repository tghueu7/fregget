...
public class RangeLookupProcess implements RasterProcess {
  ...
  public GridCoverage2D execute(
    ...
    //
    // Check the number of ranges we have in order to decide which type we can use for the output
    // values.
    ...
    if (JAIExt.isJAIExtOperation("RLookup")) {
      ...
      switch (transferType) {
        ...
        case DataBuffer.TYPE_INT:
          lookupTable =
              CoverageUtilities.getRangeLookupTable(classificationRanges, outputPixelValues, nd);
          break;
        default:
          throw new IllegalArgumentException(
              org.geotools.resources.i18n.Errors.format(
                  ErrorKeys.ILLEGAL_ARGUMENT_$2, "classification ranges size", size));
      }
    }
