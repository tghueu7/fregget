...
@DescribeProcess(title = "Convolve Coverage", description = "Returns a convoluted version of a " +
        "given raster")
public class ConvolveCoverageProcess implements RasterProcess {
    ...
    public GridCoverage2D execute(
            ...
            @DescribeParameter(name = "kernel", description = "Convolution kernel", min = 0) 
                    KernelJAI kernel,
            @DescribeParameter(name = "kernelRadius", description = "Radius for a circular " +
                    "kernel", min = 0) Integer kernelRadius,
            @DescribeParameter(name = "kernelWidth", description = "Width for rectangular " +
                    "kernel", min = 0) Integer kernelWidth,
            @DescribeParameter(name = "kernelHeight", description = "Height for rectangular " +
                    "kernel", min = 0) Integer kernelHeight
    ) throws IOException {
