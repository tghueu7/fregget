...
class SFSFeatureSource extends ContentFeatureSource implements SimpleFeatureSource {
  ...
  /**
   * This method get the Bound of the Layer -- using /mode=bounds portions of code has been borrowed
   * from
    ...
    if ((postFilter != null) && (postFilter != Filter.INCLUDE) || preFilter == Filter.EXCLUDE) {
      ...
      String bboxString =
          ods.resourceToString(
              "data/" + layer.getTypeName().getLocalPart(), "mode=bounds&" + strQuery);
      JSONArray bbox;
      try {
        bbox = (JSONArray) new JSONParser().parse(bboxString);
      } catch (org.json.simple.parser.ParseException e) {
        throw (IOException)
            new IOException("Failed to parse the bbox JSON array:" + bboxString).initCause(e);
      }
  ...
  protected SimpleFeatureType buildFeatureType() throws IOException, MalformedURLException {
    ...
    while (itr.hasNext()) {
      ...
      for (Map.Entry<String, String> entry : tmpMap.entrySet()) {
        ...
        // LOGGER.log(Level.INFO, "{0} - {1} - {2}", new Object[]{entry.getKey(), entry.getValue(),
        // OpenDataStoreUtil.getClass(entry.getValue())});
  ...
  public SimpleFeature getFeatureWithID(String fnID)
    ...
    /* Get the layer describing URL*/
    String jsonString =
        ods.resourceToString("data/" + contentEntry.getName().getLocalPart() + "/" + fnID, null);
  ...
  Filter[] splitFilter(Filter fnOriginalFilter) {
    ...
    if (fnOriginalFilter != null) {
      ...
      PostPreProcessFilterSplittingVisitor splitter =
          new PostPreProcessFilterSplittingVisitor(ods.ODS_FILTER_CAPABILITIES, getSchema(), null);
