...
class SFSFeatureSource extends ContentFeatureSource implements SimpleFeatureSource {
    ...
    static final Logger LOGGER = org.geotools.util.logging.Logging.getLogger("org.geotools.data" +
     ...
     * https://svn.osgeo.org/geotools/trunk/modules/library/jdbc/src/main/java/org/geotools/jdbc
     * /JDBCFeatureSource.java
     *
     * @param fnQuery
    ...
    public ReferencedEnvelope getBoundsInternal(Query fnQuery) throws IOException {
        ...
        if ((postFilter != null) && (postFilter != Filter.INCLUDE) || preFilter == Filter.EXCLUDE) {
            ...
            /* Create the URL */
            String bboxString = ods.resourceToString("data/" + layer.getTypeName().getLocalPart()
                    , "mode=bounds&" + strQuery);
            JSONArray bbox;
            try {
                bbox = (JSONArray) new JSONParser().parse(bboxString);
            } catch (org.json.simple.parser.ParseException e) {
                throw (IOException) new IOException("Failed to parse the bbox JSON array:" + 
                        bboxString).initCause(e);
            }
            ...
            if (bbox.size() == 4) {
                ...
                throw new IOException("The returned bound was not of size 4 but of size: " + bbox
                        .size());
            }
    ...
    protected int getCountInternal(Query fnQuery) throws IOException {
        ...
        if ((postFilter != null) && (postFilter != Filter.INCLUDE)) {
            ...
            /* Create the URL */
            String strCount = ods.resourceToString("data/" + layer.getTypeName().getLocalPart(), 
                    "mode=count&" + strQuery);
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, "Number format Exception in getCountInternal : " +
                        "FeatureSource -- getCount --" + nfe.getMessage(), nfe);
                return 0;
            }
        }
    ...
    protected FeatureReader<SimpleFeatureType, SimpleFeature> getReaderInternal(Query fnQuery) 
        ...
        if (fnQuery.getPropertyNames() == Query.ALL_NAMES) {
            ...
            returnedSchema = SimpleFeatureTypeBuilder.retype(getSchema(), fnQuery
                    .getPropertyNames());
            ...
            if (extraAttributes == null || extraAttributes.length == 0) {
                ...
                List<String> allAttributes = new ArrayList<String>(Arrays.asList(fnQuery
                        .getPropertyNames()));
    ...
    protected SimpleFeatureType buildFeatureType() throws IOException, MalformedURLException {
        ...
        String jsonString = ods.resourceToString("describe/" + contentEntry.getName()
                .getLocalPart(), null);
        ...
        try {
            ...
            LOGGER.log(Level.SEVERE, "parse Exception : FeatureSource -- buildFeatureType --" + 
                    pe.getMessage(), pe);
        }
        ...
        while (itr.hasNext()) {
            ...
            for (Map.Entry<String, String> entry : tmpMap.entrySet()) {
                ...
                //LOGGER.log(Level.INFO, "{0} - {1} - {2}", new Object[]{entry.getKey(), entry
                // .getValue(), OpenDataStoreUtil.getClass(entry.getValue())});
     ...
     */
    public SimpleFeature getFeatureWithID(String fnID) throws MalformedURLException, IOException,
        ...
        /* Get the layer describing URL*/
        String jsonString = ods.resourceToString("data/" + contentEntry.getName().getLocalPart() 
                + "/" + fnID, null);

        FeatureJSON fjson = new FeatureJSON();

        SimpleFeature feature = fjson.readFeature(new StringReader(SFSDataStoreUtil.strip
                (jsonString)));
    ...
    Filter[] splitFilter(Filter fnOriginalFilter) {
        ...
        if (fnOriginalFilter != null) {
            ...
            PostPreProcessFilterSplittingVisitor splitter = new 
                    PostPreProcessFilterSplittingVisitor(ods.ODS_FILTER_CAPABILITIES, getSchema()
                    , null);
