...
class SFSDataStoreUtil {
     ...
     * @return String
     */
    public static String encodeQuery(Query fnQuery, SimpleFeatureType targetType) throws 
        ...
        if (!fnQuery.retrieveAllProperties() && fnQuery.getPropertyNames().length > 0) {
            ...
            for (int i = 0; i < fnQuery.getPropertyNames().length; i++) {
                if (targetType.getDescriptor(fnQuery.getPropertyNames()[i]) instanceof 
                        GeometryDescriptor) {
                    no_geom = false;
                }

                urlBuilder.append(URLEncoder.encode(fnQuery.getPropertyNames()[i], 
                        DEFAULT_ENCODING));
        ...
        if (fnQuery.getSortBy() != null && fnQuery.getSortBy().length > 0) {
            ...
            urlBuilder.append(URLEncoder.encode(fnQuery.getSortBy()[0].getPropertyName()
                    .getPropertyName(), DEFAULT_ENCODING));
     ...
     */
    public static CoordinateReferenceSystem decodeXY(String srsName) throws 
