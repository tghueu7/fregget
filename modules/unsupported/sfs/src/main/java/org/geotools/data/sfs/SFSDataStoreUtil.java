...
class SFSDataStoreUtil {
   ...
   * get coordinate Array As per its documentation "In this implementation, Coordinates returned by
   * #toArray and #getCoordinate are live -- modifications to them are actually changing the
  ...
  /**
   * This method parses the query and creates a string which can be appended to the baseURL and then
   * execute it to get response Pulled from GeoRest DataStore
   *
   * @param fnQuery
   * @return String
   */
  public static String encodeQuery(Query fnQuery, SimpleFeatureType targetType)
    ...
    if (fnQuery.getSortBy() != null && fnQuery.getSortBy().length > 0) {
      ...
      urlBuilder.append(
          URLEncoder.encode(
              fnQuery.getSortBy()[0].getPropertyName().getPropertyName(), DEFAULT_ENCODING));
  ...
  /**
   * This method returns the class, which is required to build the feature type if input string type
   * in not among String, int, double, boolean, timestamp and geometry objects then default string
   * is returned
   *
   * @param strObj
   * @return Class
