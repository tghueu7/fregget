...
public class FeatureJSONTest extends GeoJSONTestSupport {
    ...
    public void testFeatureWithGeometryCollectionRead2() throws Exception {
        String json = strip("{" +
                ...
                "         {" +
                "            'type':'Polygon'," +
                "            'coordinates':[[[-28.1107, 142.998], [-28.1107, 148.623], [-30.2591," +
                " 148.623], [-30.2591, 142.998], [-28.1107, 142.998]]]" +
                "         }," +
                "         {" +
                "            'type':'Polygon'," +
                "            'coordinates':[[[-27.1765, 142.998], [-25.6811, 146.4258], " +
                "[-27.1765, 148.5352], [-27.1765, 142.998]]]" +
                "         }" +
                "     ]" +
                "   }," +
                "   'properties':{" +
        ...
        WKTReader wkt = new WKTReader();
        assertTrue(wkt.read("POLYGON ((-28.1107 142.998, -28.1107 148.623, -30.2591 148.623, " +
                "-30.2591 142.998, -28.1107 142.998))").equals(gc.getGeometryN(0)));
        assertTrue(wkt.read("POLYGON((-27.1765 142.998, -25.6811 146.4258, -27.1765 148.5352, " +
                "-27.1765 142.998))").equals(gc.getGeometryN(1)));
    ...
    public void testFeatureWithRegularGeometryAttributeRead() throws Exception {
        ...
        assertTrue(new GeometryFactory().createLineString(new Coordinate[]{
                new Coordinate(1.1, 1.2), new Coordinate(1.3, 1.4)}).equals((LineString) f
                .getAttribute("otherGeometry")));
    ...
    public void testFeatureWithGeometryAfterPropertiesRead() throws Exception {
        SimpleFeature f1 = feature(1);
        SimpleFeature f2 = fjson.readFeature(reader(strip(featureTextWithGeometryAfterProperties
                (1))));
        assertEqualsLax(f1, f2);
    ...
    public void testFeatureCollectionWithMissingAttributeRead() throws Exception {
        ...
        SimpleFeatureCollection fcol = (SimpleFeatureCollection) fjson.readFeatureCollection(
                (strip(collectionText)));
    ...
    public void testFeatureCollectionWithTypePostFeaturesRead() throws Exception {
        String json = strip("{ " +
                "  'features' : [{ " + "     'geometry' : { 'coordinates' : [ 17.633333, 59.85 ]," +
                " 'type' : 'Point' }," +
                "     'type' : 'Feature'," +
        ...
        assertTrue(new WKTReader().read("POINT (17.633333 59.85)").equals((Geometry) f
                .getDefaultGeometry()));
    ...
    public void testFeatureCollectionConflictingButInterchangeableTypesSchemaRead() throws 
            Exception {
        String json = strip(
                "{" +
                        "  'type': 'FeatureCollection'," +
    ...
    String featureText(int val, boolean missingAttribute, boolean nullAttribute) {
        if (missingAttribute && nullAttribute) {
            throw new IllegalArgumentException("For tests, use only one of either " +
                    "missingAttribute or nullAttribute");
        }
        String text =
                "{" +
                        "  'type': 'Feature'," +
    ...
    String collectionText(boolean withBounds, boolean withCRS, boolean crsAfter, boolean 
            missingFirstFeatureAttribute, boolean nullFirstFeatureAttribute) {
        return collectionText(withBounds, withCRS, crsAfter, missingFirstFeatureAttribute, 
                nullFirstFeatureAttribute, false);
    }

    String collectionText(boolean withBounds, boolean withCRS, boolean crsAfter, boolean 
            missingFirstFeatureAttribute, boolean nullFirstFeatureAttribute, boolean 
            nullAttributeAllFeatures) {
    ...
    public void testKeyOrderInFeatureCollectionParsing() throws Exception {
        ...
        /* input2 tests parsing when "type" in a geometry of the geom collection occurs after 
        "coordinates". */
        String input2 = "{" +
                " \"type\": \"FeatureCollection\"," +
                " \"features\": [{" +
        ...
        /* input3 is similar to input 2 but also tests parsing when "type" for the feature 
        collection occurs after
         * "features".
         */
        String input3 = "{" +
                " \"features\": [{" +
     ...
     */
    private final void testKeyOrderInFeatureCollectionParsing_VerifyContents(FeatureCollection 
                                                                                     featureCollection, Point expectedLastPoint) {
    ...
    public void testParseCrsAttribute() throws Exception {
        String json = strip("{\n" +
                ...
                "\"/var/data/sentinel2/2017/04/23" +
                "/S2A_OPER_MSI_L1C_TL_SGS__20180101T000000_A006640_T32TPP_N02.04.zip\",\n" +
                "    \"thumbnailURL\" : null,\n" +
                "    \"quicklookURL\" : null,\n" +
                "    \"crs\" : \"EPSG:32632\",\n" +
                "  }\n" +
