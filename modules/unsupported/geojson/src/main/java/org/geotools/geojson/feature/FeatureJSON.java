...
public class FeatureJSON {
   ...
   * array containing elements of the bounding box (in x1,y1,x2,y2 order) of the feature collection.
   */
  ...
  /**
   * Reads the {@link SimpleFeatureType} of a GeoJSON feature collection. In the worst case, it will
   * parse all features searching for attributes not present in previous features.
   *
   * @param input The input. See {@link GeoJSONUtil#toReader(Object)} for details.
   * @param nullValuesEncoded if the input has null values encoded. If this flag is set to true and
   *     the GeoJSON doesn't actually encode null values, only the first feature attributes will be
   *     discovered.
