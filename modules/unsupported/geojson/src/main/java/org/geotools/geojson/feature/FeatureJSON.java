...
public class FeatureJSON {
     ...
     * @param output   The output. See {@link GeoJSONUtil#toWriter(Object)} for details.
     */
    public void writeFeatureCollection(FeatureCollection features, Object output) throws 
            IOException {
        ...
        if (features.getSchema().getGeometryDescriptor() != null) {
            ...
            final CoordinateReferenceSystem crs = bounds != null ? bounds
    ...
    private boolean isStandardCRS(CoordinateReferenceSystem crs) {
        ...
        try {
            CoordinateReferenceSystem standardCRS = CRS.decode("EPSG:4326"); // Consider CRS:84 
            // due to axis order 
     ...
     */
    public void writeFeatureCollection(FeatureCollection features, OutputStream output) throws 
            IOException {
     ...
     * @throws IOException In the event of a parsing error or if the input json is invalid.
     */
    public SimpleFeatureType readFeatureCollectionSchema(InputStream input, boolean 
            nullValuesEncoded) throws IOException {
