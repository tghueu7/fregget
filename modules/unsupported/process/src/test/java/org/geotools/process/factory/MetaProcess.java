...
public class MetaProcess {
    ...
    @DescribeResult(name = "value", description = "the value provided as input", meta = 
            {"mimeTypes=application/shapefile,application/json"})
    public byte[] execute(
            @DescribeParameter(name = "extension", meta = {Parameter.EXT + "=shp"}) String ext,
            @DescribeParameter(name = "password", meta = {Parameter.IS_PASSWORD + "=true"}) 
                    String pwd) {
        return null;
    }
