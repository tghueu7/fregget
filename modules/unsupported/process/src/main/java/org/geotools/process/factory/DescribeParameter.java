...
public @interface DescribeParameter {
  ...
  /**
   * Minimum number of occurrences for the parameter. A value of 0 means the parameter is optional,
   * a value greater than one makes sense only if the parameter is a collection, in which case the
   * number of items of the collections will be compared against the minimum value.
   *
   * @return
   */
  int min() default -1;
  ...
  /**
   * The default value for the parameter in case it's not specified. The string value will be first
   * interpreted as a reference to a constant defined with the following two syntaxes:
   *
   * <ul>
   ...
   * reference to a constant for String values too (if {@link Converters} were to be used right away
   * the value of the default itself would be used as the default value).
   *
   ...
   * metadata map. Each string should be in the key=value form, if not, the string will be taken as
