...
public class ProgressTask implements Runnable, Progress {
   ...
   * normally or via cancellation). The default implementation does nothing. Subclasses may override
   * this method to invoke completion callbacks. You can query status inside the implementation of
   * this method to determine whether this task has been canceled.
   */
  protected void done() {}
  ...
  /**
   * Causes this ProgressTask to report an ExecutionException with the given throwable as its cause,
   * unless this ProgressTask has already been set or has been canceled.
   *
   ...
   * initial state, failing to do so if the computation encounters an exception or is canceled. This
   * is designed for use with processes that execute more than once.
   *
  ...
  private final class Synchronizer extends AbstractQueuedSynchronizer implements ProgressListener {
    ...
    /**
     * Implements AQS base release to always signal after setting final done status by nulling the
     * runningThread.
     */
    @Override
