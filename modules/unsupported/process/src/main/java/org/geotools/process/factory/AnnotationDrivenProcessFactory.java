...
public abstract class AnnotationDrivenProcessFactory implements ProcessFactory {
  ...
  private Object getConstantValue(String path, Class<?> holder, Class<?> target) {
    ...
    if ((field.getModifiers() & (Modifier.FINAL | Modifier.STATIC)) == 0) {
      if (LOGGER.isLoggable(Level.FINE)) {
        LOGGER.log(
            Level.FINE, "Field " + path + " found in class " + holder + ", but it's not a costant");
      }
      return null;
    }
   ...
   *
   * <p>Used to look up the method to use for "invertGridGeometry"; if a specific method name is not
   ...
   * {@link #execute(Map, ProgressListener)} is called the method is invoked to produce a result.
   * The mapping from the process parameters to the method parameters is determined by the {@link
   * DescribeParameter} annotations on the method parameters.
   *
   ...
   * number of parameters preceding them. These parameters must be a subset of the parameters of the
   * given execution method, and they use the same annotation to describe them.
   */
