...
public class MBObjectParserTest {
    ...
    public void testColorFromFunction() throws ParseException {
        JSONObject object = MapboxTestUtils.object("{'expr': {'property':'color'," +
                "'type':'identity'}}");
    ...
    public void testStringFromFunction() throws ParseException {
        JSONObject object = MapboxTestUtils.object("{'expr': {'base': 1, 'stops': [[0, " +
                "'{name-en}']]}}");
    ...
    public void testBoolFromFunction() throws ParseException {
        JSONObject object = MapboxTestUtils.object("{'expr': { 'base': 1, 'stops': [ [ 0, true ]," +
                " [ 10, true ], [ 22, true ] ] }}");
    ...
    public void testEnumToExpression() throws ParseException {
        ...
        JSONObject object = MapboxTestUtils.object("{'text-pitch-alignment': 'map'}"); // 
        // viewport, auto
        Expression expression = parser.enumToExpression(object, "text-pitch-alignment", 
                SymbolMBLayer.Alignment.class, SymbolMBLayer.Alignment.AUTO);
        assertEquals("Enum correctly parsed", "map", expression.evaluate(null, String.class));

        // Test for line-cap
        object = MapboxTestUtils.object("{'line-cap': 'round'}");
        expression = parser.enumToExpression(object, "line-cap", LineMBLayer.LineCap.class, 
                LineMBLayer.LineCap.BUTT);
        assertEquals("round", expression.evaluate(null, String.class));

        // Test for line-join
        object = MapboxTestUtils.object("{'line-join': 'bevel'}");
        expression = parser.enumToExpression(object, "line-join", LineMBLayer.LineJoin.class, 
     ...
     * property value is invalid for the enumeration (should
     * fallback to default).
     */
    @Test
