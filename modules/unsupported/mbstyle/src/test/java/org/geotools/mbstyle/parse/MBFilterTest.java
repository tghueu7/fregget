...
public class MBFilterTest {
    ...
    public void mixed() throws ParseException {
        JSONArray json = array(
                "['all',['==', 'class', 'street_limited'],['>=', 'admin_level', 3],['!in', " +
                        "'$type', 'Polygon']]"
        );
        ...
        assertEquals("class = 'street_limited' AND admin_level >= 3 AND NOT ((dimension(geometry" +
                "()) = 2 AND NOT (isCoverage() = true)))", ECQL.toCQL(filter));
    }
    ...
    public void type() throws ParseException {
        ...
        filter = mbfilter.filter();
        assertEquals("(dimension(geometry()) = 2 AND NOT (isCoverage() = true))", ECQL.toCQL
                (filter));
