...
public class MapboxSpriteTest {
  ...
  /**
   * Perform a perceptual test verifying that icons can be correctly retrieved by name from a sprite
   * sheet.
   */
  @Test
  ...
  /**
   * Append a base url with #{iconName}. This parameter is used by the {@link SpriteGraphicFactory}
   * to pull the correct icon from the spritesheet.
   */
  ...
  File file(String name) throws IOException {
    ...
    return new File(
        "src/test/resources/org/geotools/mbstyle/sprite/test-data/rendered/" + name + ".png");
  }
}
