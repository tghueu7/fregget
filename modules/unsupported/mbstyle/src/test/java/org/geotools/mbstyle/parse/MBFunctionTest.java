...
public class MBFunctionTest {
  ...
  public void colorIdentityTest() throws Exception {
    ...
    SimpleFeature feature2 =
        DataUtilities.createFeature(SAMPLE, "measure1=A|50.0|POINT(0,0)|#0000FF|rgb(0,0,255)");
  ...
  /**
   * Assert that the an exponential color function correctly falls back to the default value if the
   * input property is not numeric.
   */
  @Test
  ...
  public void propertyColorStopsBase() throws Exception {
    ...
    // So the interpolation on the halfway input value should be less than the halfway output value,
    // which would be (128, 0 , 128).
  ...
  /**
   * Tests that a MapBox exponential zoom function (outputting a color) correctly interpolates color
   * values for zoom levels between stops, when the exponential base is > 1.
   *
   * @throws Exception
   */
  @Test
  public void zoomColorStopsExponentialInterpolationTest() throws Exception {
    ...
    assertTrue(
        "Color has no red, but should be interpolated mix of red and green", result.getRed() > 0);
    assertTrue(
        "Color has full green, but should be interpolated mix of red and green",
        result.getGreen() < 255);

    // We are interpolating at the halfway point, so the linear interpolation would have been (128,
    // 128, 0).
    assertTrue(
  ...
  /**
   * Tests that a MapBox exponential zoom function (outputting a color) correctly interpolates color
   * values for zoom levels between stops, when the exponential base is == 1 (linear).
   *
   * @throws Exception
   */
  @Test
  ...
  public void colorIntervalFunctionTest() throws Exception {
    ...
    assertEquals(
        "Function type is \"interval\"", MBFunction.FunctionType.INTERVAL, function.getType());
    ...
    // Test each interval
    assertThat(
        outputExpression, evaluatesTo(features.apply(-900L), Color.class, equalTo(Color.BLACK)));
    assertThat(
        outputExpression, evaluatesTo(features.apply(-20L), Color.class, equalTo(Color.GREEN)));
    assertThat(
        outputExpression, evaluatesTo(features.apply(10L), Color.class, equalTo(Color.BLUE)));
    assertThat(
        outputExpression, evaluatesTo(features.apply(500L), Color.class, equalTo(Color.WHITE)));

    // Test at stops
    assertThat(
        outputExpression, evaluatesTo(features.apply(-1000L), Color.class, equalTo(Color.BLACK)));
    assertThat(
        outputExpression, evaluatesTo(features.apply(-30L), Color.class, equalTo(Color.GREEN)));
    assertThat(outputExpression, evaluatesTo(features.apply(0L), Color.class, equalTo(Color.BLUE)));
    assertThat(
        outputExpression, evaluatesTo(features.apply(100L), Color.class, equalTo(Color.WHITE)));
  }
  ...
  public void enumIntervalFunctionTest() throws Exception {
    ...
    assertThat(
        outputExpression, evaluatesTo(features.apply(-10000L), String.class, any(String.class)));

    // Test each interval
    assertThat(
        outputExpression, evaluatesTo(features.apply(-900L), String.class, equalTo("interval")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(-20L), String.class, equalTo("categorical")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(10L), String.class, equalTo("exponential")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(500L), String.class, equalTo("identity")));

    // Test at stops
    assertThat(
        outputExpression, evaluatesTo(features.apply(-1000L), String.class, equalTo("interval")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(-30L), String.class, equalTo("categorical")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(0L), String.class, equalTo("exponential")));
    assertThat(
        outputExpression, evaluatesTo(features.apply(100L), String.class, equalTo("identity")));
  }
  ...
  public void zoomStopsNumericInterpolationTest() throws Exception {
    ...
    assertTrue(
        "Interpolated value should be below midpoint (for base > 1)", result.doubleValue() < 36);
    ...
    assertTrue("Interpolated value should be above lower stop", result.doubleValue() > 24);
    assertTrue(
        "Interpolated value should be above midpoint (for base < 1)", result.doubleValue() > 36);
    ...
    assertTrue("Interpolated value should be above lower stop", result.doubleValue() > 24);
    assertEquals(
        "Interpolated value should = midpoint (for base = 1)", result.doubleValue(), 36.0, .0001);
  }
  ...
  public void numericIdentityFunctionTest() throws Exception {
    ...
    assertTrue(
        ...
        EnumSet.of(MBFunction.FunctionCategory.PROPERTY).equals(function.category()));
    assertEquals(
        "Function type is \"Identity\"", MBFunction.FunctionType.IDENTITY, function.getType());
  ...
  /**
   * Assert that {@link MBFunction} uses the correct default function type when the requested return
   * class is String.
   */
  @Test
  ...
  public void colorDefaultFunctionTest() throws Exception {
    JSONObject json =
        MapboxTestUtils.object("{'property':'temperature','stops':[[0,'blue'],[100,'red']]}");
    MBFunction function = new MBFunction(json);
