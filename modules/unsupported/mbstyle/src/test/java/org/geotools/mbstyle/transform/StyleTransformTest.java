...
public class StyleTransformTest {
  ...
  public void testFillExtrusion() throws IOException, ParseException {
    ...
    assertEquals(
        "Height", ((FilterFunction_isometric) sidesGeometry).getParameters().get(1).toString());
  ...
  public void testCircle() throws IOException, ParseException {
    ...
    assertEquals(Double.valueOf(.5), m.getFill().getOpacity().evaluate(null, Double.class), .0001);
    ...
    assertEquals(
        Double.valueOf(.75), m.getStroke().getOpacity().evaluate(null, Double.class), .0001);
  }
  ...
  public void testBackgroundColor() throws IOException, ParseException {
    ...
    assertEquals(
        Double.valueOf(.45), psym.getFill().getOpacity().evaluate(null, Double.class), .0001);
  ...
  /**
   * MapBox symbol-avoid-edges defaults to false, If true, the symbols will not cross tile edges to
   * avoid mutual collisions. This concept is represented by using the Partials option in GeoTools.
   * The partials options instructs the renderer to render labels that cross the map extent, which
   * are normally not painted since there is no guarantee that a map put on the side of the current
   * one (tiled rendering) will contain the other half of the label. By enabling “partials” the
   * style editor takes responsibility for the other half being there (maybe because the label
   * points have been placed by hand and are assured not to conflict with each other, at all zoom
   * levels).
   *
   * <p>Based upon the above if symbol-avoid-edges is true we do not need to add the partials option
   * as the renderer will do this by default. But if symbol-avoid-edges is missing or set to false,
   * then we do need to add the partials option set to true.
   *
   * @throws IOException
