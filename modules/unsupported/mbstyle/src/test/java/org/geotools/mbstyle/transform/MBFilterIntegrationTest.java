...
public class MBFilterIntegrationTest {
  ...
  /**
   * Create a style with three layers, each of them with a filter requiring a '$type' (or a list of
   * them).
   *
  ...
  public void testSemanticTypes() throws IOException, ParseException {
    ...
    assertTrue(
        semanticTypes.contains(SemanticType.POINT) && semanticTypes.contains(SemanticType.POLYGON));
  }
}
