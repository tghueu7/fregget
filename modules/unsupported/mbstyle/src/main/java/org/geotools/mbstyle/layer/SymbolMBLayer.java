...
public class SymbolMBLayer extends MBLayer {
  ...
  public enum TextAnchor {
     ...
     *
     * <p>One of center, left, right, top, bottom, top-left, top-right, bottom-left, bottom-right.
     * Defaults to center.
     *
   ...
   *
   * <p>(Optional) Defaults to false. If true, the symbols will not cross tile edges to avoid mutual
   * collisions. Recommended in layers that don't have enough padding in the vector tile to prevent
   ...
   *
   * <p>(Optional) One of none, width, height, both. Defaults to none. Requires icon-image. Requires
   * text-field. Scales the icon to fit around the associated text.
   *
   * @return How the icon should be scaled to fit the associated text
   */
  ...
  /**
   * Converts {@link #getTextPitchAlignment()} to a GeoTools expression. Returns an expression that
   * evaluates to one of "map", "viewport", or "auto".
   */
   ...
   * anchor.
   *
   * <p>{@link TextAnchor#BOTTOM_RIGHT} The bottom right corner of the text is placed closest to the
   * anchor.
   *
   ...
   * Requires text-field. Requires text-rotation-alignment = map. Requires symbol-placement = line.
   *
   * <p>If true, the text may be flipped vertically to prevent it from being rendered upside-down.
   *
   * @return Boolean
  ...
  /**
   * Converts {@link #getIconTranslateAnchor()} to a GeoTools expression. Returns an expression that
   * evaluates to one of "map", "viewport".
   *
  ...
  /**
   * Converts {@link #getTextTranslateAnchor()} to a GeoTools expression. Returns an expression that
   * evaluates to one of "map", "viewport".
   *
   ...
   * </ul>
   *
   * @param styleContext The MBStyle to which this layer belongs, used as a context for things like
   *     resolving sprite and glyph names to full urls.
  ...
  public List<FeatureTypeStyle> transformInternal(MBStyle styleContext) {
    ...
    Font font =
        sb.createFont(ff.literal(""), ff.literal("normal"), ff.literal("normal"), textSize());
    ...
    Number symbolSpacing =
        transformer.requireLiteral(symbolSpacing(), Number.class, 250, "symbol-spacing", getId());
    ...
    String textFitVal =
        transformer
            .requireLiteral(iconTextFit(), String.class, "none", "icon-text-fit", getId())
            .trim();
    ...
    // The partials
    // options instructs the renderer to render labels that cross the map extent, which are normally
    // not painted
    // since there is no guarantee that a map put on the side of the current one (tiled rendering)
    // will contain
    // the other half of the label. By enabling “partials” the style editor takes responsibility for
    // the other
    // half being there (maybe because the label points have been placed by hand and are assured not
    // to conflict
    // with each other, at all zoom levels).
    //
    // Based upon the above if symbol-avoid-edges is true we do not need
    // to add the partials option as the renderer will do this by default. But if symbol-avoid-edges
    // is missing or
    ...
    if (getIconTextFitPadding() != null && !getIconTextFitPadding().isEmpty()) {
      symbolizer.getOptions().put("graphic-margin", String.valueOf(getIconTextFitPadding().get(0)));
    } else {
    ...
    // Only supported when text-max-width and text-size are not functions (because vendor options
    // don't take expressions)
    if (hasTextMaxWidth()) {
      double textMaxWidth =
          transformer.requireLiteral(textMaxWidth(), Double.class, 10.0, "text-max-width", getId());
      double textSize =
          transformer.requireLiteral(
              textSize(),
              Double.class,
    ...
    if (hasIconImage()) {
      ...
      // By default - or icon-ignore-placement: false, MapBox prevents symbols from being visible if
      // they collide
      ...
      // Check to see that hasTextField() is true check to see if IconPadding is greater to put to
      // spaceAround
      if (!hasTextField()
    ...
    // that
    // textIgnorePlacement is either null or false, if so add it.  If textIgnorePlacement is true,
    ...
    Rule rule =
        sf.rule(getId(), null, null, 0.0, Double.POSITIVE_INFINITY, symbolizers, filter.filter());
    ...
    return Collections.singletonList(
        sf.featureTypeStyle(
            getId(),
            sf.description(
                Text.text("MBStyle " + getId()), Text.text("Generated for " + getSourceLayer())),
            null, // (unused)
            Collections.emptySet(),
            filter.semanticTypeIdentifiers(), // we only expect this to be applied to polygons
            rules));
  }
  ...
  private Graphic getGraphic(MBStyleTransformer transformer, MBStyle styleContext) {
    ...
    if (iconExpression instanceof Literal) {
      iconExpression =
          transformer.cqlExpressionFromTokens(iconExpression.evaluate(null, String.class));
    }
    ...
    String spriteSheetLocation =
        styleContext.getSprite() == null ? "" : styleContext.getSprite().trim().toLowerCase();
    ...
    if (gs instanceof Mark) {
      ...
      // If it is an ExternalGraphic from a sprite sheet, the absolute size of the icon is unknown
      // at this point.
