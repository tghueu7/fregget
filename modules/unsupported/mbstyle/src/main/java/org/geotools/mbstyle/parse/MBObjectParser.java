...
public class MBObjectParser {
   ...
   * @param json The JSONObject in which to lookup the provided tag and return a JSONObject
   * @param tag The tag to look up in the provided JSONObject
   * @param fallback The JSONObject to return if the provided json does not contain a JSONObject for
   *     that tag.
  ...
  /**
   * Look up a Double in the provided {@link JSONObject} at the provided 'tag', or thrown an {@link
   * MBFormatException}.
   *
  ...
  /**
   * Look up a Boolean in the provided {@link JSONObject} at the provided 'tag', or thrown an {@link
   * MBFormatException}.
   *
  ...
  public <T> T require(Class<T> type, JSONObject json, String tag) {
    ...
    if (json.containsKey(tag) && type.isInstance(json.get(tag))) {
      ...
      throw new MBFormatException(
          context.getSimpleName() + " requires \"" + tag + "\" " + type.getSimpleName() + " field");
    }
  }
  ...
  public <T extends Enum<?>> T getEnum(
    ...
    if (value == null) {
      ...
      throw new MBFormatException(
          "\"" + tag + "\" contains invalid value for enumeration " + enumeration.getSimpleName());
    } else {
      throw new MBFormatException(
          "Conversion of \""
              + tag
   ...
   * that tag is not found. Throws an exception if that tag is something other than an array, or if
   * its contents cannot be cast to type.
   *
  ...
  //
  /**
   * Convert the provided object to a numeric Expression (or function), with a fallback value if the
   * object is null.
   *
   ...
   * @throws MBFormatException
   */
  public Expression number(JSONObject json, String tag, Number fallback) throws MBFormatException {
    if (json == null) {
      return ff.literal(fallback);
    }
    Object obj = json.get(tag);
  ...
  /**
   * Convert the provided object to a string Expression (or function), with a fallback value if the
   * object is null.
   *
   * @param context The json context of the object, used for error messages.
   * @param obj The object to convert
   ...
   * @throws MBFormatException
   */
  public Expression string(JSONObject json, String tag, String fallback) throws MBFormatException {
    if (json == null) {
      return fallback == null ? null : ff.literal(fallback);
    }
  ...
  public Expression bool(JSONObject json, String tag, boolean fallback) throws MBFormatException {
    ...
    if (obj instanceof String) {
      ...
      throw new UnsupportedOperationException("\"" + tag + "\": boolean from Number not supported");
    } else if (obj instanceof Boolean) {
