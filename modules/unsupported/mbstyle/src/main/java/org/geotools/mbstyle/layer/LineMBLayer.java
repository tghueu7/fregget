...
public class LineMBLayer extends MBLayer {
   ...
   *
   * <p>Round - A join with a rounded end which is drawn beyond the endpoint of the line at a radius
   * of one-half of the line's width and centered on the endpoint of the line.
   *
   ...
   *
   * <p>(Optional) Draws a line casing outside of a line's actual path. Value indicates the width of
   * the inner gap.
   *
   * <p>Units in pixels. Defaults to 0.
   *
   ...
   *
   * <p>(Optional) The line's offset. For linear features, a positive value offsets the line to the
   * right, relative to the direction of the line, and a negative value to the left. For polygon
   * features, a positive value results in an inset, and a negative value results in an outset.
   *
   * <p>Units in pixels. Defaults to 0.
  ...
  /**
   * (Optional) Specifies the lengths of the alternating dashes and gaps that form the dash pattern.
   * The lengths are later scaled by the line width. To convert a dash length to pixels, multiply
   * the length by the current line width.
   *
  ...
  /**
   * (Optional) Name of image in sprite to use for drawing image lines. For seamless patterns, image
