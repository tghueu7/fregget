...
/**
 * Wrapper around a {@link JSONObject} holding a Mapbox GeoJSON source. Data must be provided via
 * a "data" property, whose value can be a URL or
 * inline GeoJSON.
 *
 * @see {@link MBSource}
 * @see <a href="https://www.mapbox.com/mapbox-gl-js/style-spec/#sources-geojson">https://www
 * .mapbox.com/mapbox-gl-js/style-spec/#sources-geojson</a>
 */
public class GeoJsonMBSource extends MBSource {
    ...
    /**
     * (Optional) Defaults to 18. Maximum zoom level at which to create vector tiles (higher 
     * means greater detail at high zoom levels).
     *
     ...
     * buffer. A value of 512 produces a buffer as wide as
     * the tile itself. Larger values produce fewer rendering artifacts near tile edges and 
     * slower performance.
     *
    ...
    /**
     * (Optional) Defaults to 0.375. Douglas-Peucker simplification tolerance (higher means 
     * simpler geometries and faster performance).
     *
    ...
    /**
     * (Optional) Defaults to false. If the data is a collection of point features, setting this 
     * to true clusters the points by radius into groups.
     *
    ...
    /**
     * (Optional)  Max zoom on which to cluster points if clustering is enabled. Defaults to one 
     * zoom less than maxzoom (so that last zoom features are not
     * clustered).
     *
