...
public class FillMBLayer extends MBLayer {
  ...
  /**
   * (Optional) The opacity of the entire fill layer. In contrast to the fill-color, this value will
   * also affect the 1px stroke around the fill, if the stroke is used.
   *
   * <p>Defaults to 1.
   *
   ...
   * with an alpha component and the color's opacity will not affect the opacity of the 1px stroke,
   * if it is used.
   *
  ...
  public List<FeatureTypeStyle> transformInternal(MBStyle styleContext) {
    ...
    if (hasFillPattern()) {
      ...
      GraphicFill gf =
          sf.graphicFill(
              Arrays.asList(eg), fillOpacity(), null, null, null, fillTranslateDisplacement());
    ...
    // How do other style transformers set a null legend? SLD/SE difference - fix setLegend(null) to
    // empty list.
