...
public class CircleMBLayer extends MBLayer {
  ...
  /**
   * (Optional) Amount to blur the circle. 1 blurs the circle such that only the centerpoint is full
   ...
   *
   * <p>{@link CirclePitchScale#MAP}: Circles are scaled according to their apparent distance to the
   * camera. {@link CirclePitchScale#VIEWPORT}: Circles are not scaled.
   *
  ...
  public List<FeatureTypeStyle> transformInternal(MBStyle styleContext) {
    ...
    Rule rule =
        sf.rule(
            getId(), null, null, 0.0, Double.POSITIVE_INFINITY, Arrays.asList(ps), filter.filter());
