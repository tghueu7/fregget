  ...
  public static List<Exception> validate(Reader reader) {
    ...
    for (MBLayer layer : layers) {
      ...
      try {
        ...
        problems.add(
            (MBFormatException)
                new MBFormatException("Layer " + layer.getId() + ":" + invalid.getMessage())
                    .initCause(invalid));
        return problems;
      }
    }
    if (!hasVisibleLayer) {
      ...
      try {
                ...
                for (Style layerStyle : named.styles()) {
                  if (layerStyle instanceof FeatureTypeStyle) {
                    ...
                    if (fts.rules().isEmpty()) {
                      problems.add(
                          new MBFormatException(
                              "Generated feature type style " + fts.getName() + " is empty."));
                    }
                  }
                }
              }
        ...
        problems.add(
            (MBFormatException)
                new MBFormatException("Style " + style.getName() + ":" + problem.getMessage())
                    .initCause(problem));
      }
    }
    return problems;
