...
public class MapBoxStyle {
     ...
     * @return generated style
     */
    public static StyledLayerDescriptor parse(InputStream stream) throws IOException, 
            ParseException {
        MBStyleParser parser = new MBStyleParser();
        ...
        if (!hasVisibleLayer) {
            ...
            try {
                ...
                if (sld.getStyledLayers().length == 0) {
                    problems.add(new MBFormatException("Generated sld for " + sld.getName() + " " +
                            "is empty."));
                } else {
                    for (StyledLayer layer : sld.getStyledLayers()) {
                        if (layer instanceof NamedLayer) {
                            ...
                            if (named.styles().isEmpty()) {
                                problems.add(new MBFormatException("Generated named layer for " +
                                        named.getName() + " is empty."));
                            } else {
                                for (Style layerStyle : named.styles()) {
                                    if (layerStyle instanceof FeatureTypeStyle) {
                                        ...
                                        if (fts.rules().isEmpty()) {
                                            problems.add(new MBFormatException("Generated feature" +
                                                    " type style " + fts.getName() + " is empty."));
                                        }
                                    }
                                }
                            }
                ...
                problems.add((MBFormatException) new MBFormatException(
                        "Style " + style.getName() + ":" + problem.getMessage()).initCause
                        (problem));
            }
        }
        return problems;
