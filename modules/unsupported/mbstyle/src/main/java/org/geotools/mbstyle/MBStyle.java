...
public class MBStyle {
     ...
     * rendering. Properties should be prefixed to avoid
     * collisions, like 'mapbox:'.
     *
     * @return {@link JSONObject} containing the metadata, or an empty JSON object the style has 
     * no metadata.
     */
    ...
    /**
     * (Optional) Default map center in longitude and latitude. The style center will be used 
     * only if the map has not been positioned by other means (e.g. map options or user 
     * interaction).
     *
    ...
    /**
     * (Optional) Default bearing, in degrees clockwise from true north. The style bearing will 
     * be used only if the map has not been positioned by
     ...
     * straight down at the map, while a greater value like 60
     * looks ahead towards the horizon. The style pitch will be used only if the map has not been
     * positioned by other means (e.g. map options or user
     * interaction).
     *
    ...
    /**
     * A base URL for retrieving the sprite image and metadata. The extensions .png, .json and 
     * scale factor @2x.png will be automatically appended.
     * This property is required if any layer uses the background-pattern, fill-pattern, 
     * line-pattern, fill-extrusion-pattern, or icon-image
     * properties.
     *
    ...
    /**
     * (Optional) A URL template for loading signed-distance-field glyph sets in PBF format. The 
     * URL must include {fontstack} and {range} tokens. This
    ...
    public StyledLayerDescriptor transform() {
        ...
        for (MBLayer layer : layers) {
            ...
            if (layer.visibility()) {
                // check for property and zoom functions, if true we will have a layer for each 
                // one that
                ...
                if (mbObjectStops.ls.zoomStops || mbObjectStops.ls.zoomPropertyStops) {
                    ...
                    for (MBLayer l : mbObjectStops.layersForStop) {
                        ...
                        long[] rangeForStopLevel = mbObjectStops.getRangeForStop(s, mbObjectStops
                                .ranges);
                        Double maxScaleDenominator = MBObjectStops.zoomLevelToScaleDenominator
                                (rangeForStopLevel[0]);
                        Double minScaleDenominator = null;
                        if (rangeForStopLevel[1] != -1) {
                            minScaleDenominator = MBObjectStops.zoomLevelToScaleDenominator
                                    (rangeForStopLevel[1]);
                        }

                        featureTypeStyles.addAll(l.transform(this, minScaleDenominator, 
                                maxScaleDenominator));
                        i++;
                    }
                } else {
                    featureTypeStyles.addAll(layer.transform(this, layerMinScaleDenominator, 
                            layerMaxScaleDenominator));
                }
            }
            ...
            if (!featureTypeStyles.isEmpty()) {
                ...
                if (layer instanceof BackgroundMBLayer) {
                    ...
                    //Background does not use a source; construct a user later with a world 
                    // extent inline feature
                    ...
                    try {
                        ...
                        fc.add(SimpleFeatureBuilder.build(
                                featureType,
                                new Object[]{jtsFactory.toGeometry(new ReferencedEnvelope(CRS
                                        .getEnvelope(crs)))},
                                "background"));
                    ...
                    //Append to existing namedlayer if source name is the same, otherwise create 
                    // a new one
