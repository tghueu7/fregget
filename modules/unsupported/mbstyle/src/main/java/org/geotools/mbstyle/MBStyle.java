...
public class MBStyle {
  ...
  /**
   * (Optional) Default map center in longitude and latitude. The style center will be used only if
   * the map has not been positioned by other means (e.g. map options or user interaction).
   *
  ...
  /**
   * (Optional) Default pitch, in degrees. Zero is perpendicular to the surface, for a look straight
   * down at the map, while a greater value like 60 looks ahead towards the horizon. The style pitch
   * will be used only if the map has not been positioned by other means (e.g. map options or user
   * interaction).
   *
   ...
   * factor @2x.png will be automatically appended. This property is required if any layer uses the
   * background-pattern, fill-pattern, line-pattern, fill-extrusion-pattern, or icon-image
   * properties.
   *
    ...
    for (MBLayer layer : layers) {
      ...
      if (layer.visibility()) {
        ...
        if (mbObjectStops.ls.zoomStops || mbObjectStops.ls.zoomPropertyStops) {
          ...
          featureTypeStyles.addAll(
              layer.transform(this, layerMinScaleDenominator, layerMaxScaleDenominator));
        }
      }
      ...
      if (!featureTypeStyles.isEmpty()) {
        ...
        if (layer instanceof BackgroundMBLayer) {
          ...
          // Background does not use a source; construct a user later with a world extent inline
          // feature
          ...
          if (sourceLayer == null) {
            // If source-layer is not set, assume the source just has one layer which shares its
            // name
