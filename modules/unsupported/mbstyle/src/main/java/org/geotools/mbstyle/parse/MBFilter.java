 ...
 * </ul>
 * <p>
 * A key must be a string that identifies a feature property, or one of the following special 
 * keys:</p>
 * <ul>
 * <li>"$type": the feature type. This key may be used with the "==",  "!=", "in", and "!in" 
 * operators. Possible values are  "Point", "LineString", and "Polygon".</li>
 * <li>"$id": the feature identifier. This key may be used with the "==",  "!=", "has", "!has", 
 * "in", and "!in" operators.</li>
 * </ul>
 *
 * @see Filter
 * @see MBFunction
            ...
            if ("in".equals(operator) || "==".equals(operator)) {
                ...
                for (Object type : types) {
                    if (type instanceof String) {
                        ...
                        throw new MBFormatException("[\"in\",\"$type\", ...] limited to Point, " +
                                "LineString, Polygon: " + type);
                    }
                }
                if ("==".equals(operator) && types.size() != 1) {
                    throw new MBFormatException("[\"==\",\"$type\", ...] limited one geometry " +
                            "type, to test more than one use \"in\" operator.");
                }
            ...
            for (int i = 1; i < json.size(); i++) {
                ...
                if (types.isEmpty()) {
                    ...
                    if (semanticTypes.isEmpty()) {
                        ...
                        throw new MBFormatException("Only one \"all\" alternative may be a $type " +
                                "filter");
                    }
                }
            }
    ...
    public Filter filter() {
        ...
        if (("==".equals(operator) || "!=".equals(operator) ||
            ...
            for (Object type : types) {
                ...
                if (typeFilter == null) {
                    throw new MBFormatException("\"$type\" limited to Point, LineString, Polygon:" +
                            " " + type);
                }
