...
public abstract class MBLayer {
  ...
  /**
   * Arbitrary properties useful to track with the layer, but do not influence rendering. Properties
   * should be prefixed to avoid collisions, like 'mapbox:' and 'gt:`.
   *
   ...
   *
   * @return References another layer to copy type, source, source-layer, minzoom, maxzoom, filter,
   *     and layout properties from.
   */
  ...
  /**
   * A MBFilter wrapping optional json specifying conditions on source features. Only features that
   * match the filter are displayed. This is available as a GeoTools {@link Filter} via {@link
   * #filter()}.
   *
  ...
  /**
   * The "filter" as a GeoTools {@link Filter} suitable for feature selection, as defined by {@link
   * #getFilter()}.
   *
   ...
   *
   * <p><em>Layout properties</em> appear in the layer's "layout" object. They are applied early in
   ...
   * property, and should do so where possible. This will decrease processing time and allow the two
   * layers will share GPU memory and other resources associated with the layer.
   *
