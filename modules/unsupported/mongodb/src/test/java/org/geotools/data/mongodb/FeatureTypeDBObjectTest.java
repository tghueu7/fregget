...
public class FeatureTypeDBObjectTest {
    ...
    @Test
    public void testRoundTripConversion() throws FileNotFoundException, IOException, 
            FactoryException {
    ...
    static void compareFeatureTypes(SimpleFeatureType left, SimpleFeatureType right, boolean 
            strictGeometryClass) {
        ...
        assertTrue("CRS are equal", CRS.equalsIgnoreMetadata(right.getCoordinateReferenceSystem()
                , left.getCoordinateReferenceSystem()));
        ...
        if (strictGeometryClass) {
            ...
            assertThat(Geometry.class.isAssignableFrom(right.getGeometryDescriptor().getType()
                    .getBinding()), is(equalTo(true)));
            assertThat(Geometry.class.isAssignableFrom(left.getGeometryDescriptor().getType()
                    .getBinding()), is(equalTo(true)));
        }
        ...
        for (AttributeDescriptor rad : right.getAttributeDescriptors()) {
            ...
            if (!radName.equals(rgdName)) {
                assertThat(rad.getType().getBinding().getSimpleName(), is(equalTo(oad.getType()
