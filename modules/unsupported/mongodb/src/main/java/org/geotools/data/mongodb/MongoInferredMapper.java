...
public class MongoInferredMapper extends AbstractCollectionMapper {
    ...
    public SimpleFeatureType buildFeatureType(Name name, DBCollection collection) {
        ...
        if (indexedGeometries.size() > 1) {
            LOG.log(Level.WARNING, "More than one indexed geometry field found for type {0}, " +
                            "selecting {1} (first one encountered with index search of collection" +
                            " {2})",
        ...
        for (Map.Entry<String, Class> mappedField : mappedFields.entrySet()) {
            ...
            LOG.log(Level.INFO, "building type \"{0}\": mapping field \"{1}\" with binding {2} " +
                            "from collection {3}",
        ...
        for (String field : indexedFields) {
            ...
            LOG.log(Level.INFO, "building type \"{0}\": mapping indexed field \"{1}\" with " +
                            "default binding, {2}, from collection {3}",
