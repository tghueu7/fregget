...
public final class MongoComplexUtilities {
  ...
  public static Feature extractFeature(Object feature, String jsonPath) {
    ...
    Object mongoFeature =
        ((Feature) feature).getUserData().get(AbstractCollectionMapper.MONGO_OBJECT_FEATURE_KEY);
  ...
  /**
   * Will extract from the mongo db object the value that correspond to the given json path. If the
   * path contain a nested list of values an exception will be throw.
   */
  ...
  private static final class MongoObjectWalker {
    ...
    private Object next(List basicDBList) {
      ...
      if (rawCollectionIndex == null && basicDBList.size() == 1) {
        // this collection only has a single element and there is not index information for this
        // collection
  ...
  /**
   * Will extract from the mongo db object all the values that correspond to the given json path. If
   * the path contains nested collections the values from all the branches will be merged.
   */
  ...
  private static List<Object> getValuesHelper(
    ...
    if (object instanceof List) {
      if (finalPath) {
        // we reached the end of the json path and we have a list, so let's add all the elements of
        // the list
