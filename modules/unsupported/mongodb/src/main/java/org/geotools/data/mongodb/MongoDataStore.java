...
public class MongoDataStore extends ContentDataStore {
    ...
    public MongoDataStore(String dataStoreURI, String schemaStoreURI, boolean 
        ...
        dataStoreDB = createDB(dataStoreClient, dataStoreClientURI.getDatabase(), 
                !createDatabaseIfNeeded);
        if (dataStoreDB == null) {
            dataStoreClient.close(); // This smells bad...
            throw new IllegalArgumentException("Unknown mongodb database, \"" + 
                    dataStoreClientURI.getDatabase() + "\"");
        }

        schemaStore = createSchemaStore(schemaStoreURI);
        if (schemaStore == null) {
            dataStoreClient.close(); // This smells bad too...
            throw new IllegalArgumentException("Unable to initialize schema store with URI \"" + 
                    schemaStoreURI + "\"");
        }
    ...
    final MongoClientURI createMongoClientURI(String dataStoreURI) {
        ...
        if (!dataStoreURI.startsWith("mongodb://")) {
            throw new IllegalArgumentException("incorrect scheme for URI, expected to begin with " +
                    "\"mongodb://\", found URI of \"" + dataStoreURI + "\"");
        }
    ...
    private MongoSchemaStore createSchemaStore(String schemaStoreURI) {
        if (schemaStoreURI.startsWith("file:")) {
            try {
                ...
                LOGGER.log(Level.SEVERE, "Unable to create file-based schema store with URI \"" +
                        schemaStoreURI + "\"", e);
            } catch (IOException e) {
                LOGGER.log(Level.SEVERE, "Unable to create file-based schema store with URI \"" +
                        schemaStoreURI + "\"", e);
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, "Unable to create mongodb-based schema store with URI " +
                        "\"" + schemaStoreURI + "\"", e);
            }
        } else {
            try {
...
//        capabilities.addAll(FilterCapabilities.LOGICAL_OPENGIS);
    ...
    public void createSchema(SimpleFeatureType incoming) throws IOException {
        ...
        if (!CRS.equalsIgnoreMetadata(incomingCRS, DefaultGeographicCRS.WGS84)) {
            throw new IllegalArgumentException("Unsupported coordinate reference system, only " +
                    "WGS84 supported");
        }
        ...
        dataStoreDB.createCollection(incoming.getTypeName(), new BasicDBObject()).createIndex(new
                BasicDBObject(geometryMapping, "2dsphere"));
    ...
    protected List<Name> createTypeNames() throws IOException {
        ...
        for (String candidateTypeName : getSchemaStore().typeNames()) {
            try {
                SimpleFeatureType candidateSchema = getSchemaStore().retrieveSchema(name
                        (candidateTypeName));
                ...
                if (collectionNames.contains(candidateCollectionName)) {
                    ...
                    String geometryMapping = (String) candidateSchema.getDescriptor(geometryName)
                            .getUserData().get(KEY_mapping);
                    if (geometryMapping != null) {
                        DBCollection collection = dataStoreDB.getCollection
                        ...
                        if (geometryIndices.contains(geometryMapping)) {
                            ...
                            LOGGER.log(Level.WARNING, "Ignoring type \"{0}\", the geometry " +
                                            "attribute, \"{1}\", is mapped to document key " +
                                            "\"{2}\" but it is not spatialy indexed in collection" +
                                            " {3}",
                                    new Object[]{name(candidateTypeName), geometryName, 
                                            geometryMapping, collection.getFullName()});
                        }
                    } else {
                        LOGGER.log(Level.WARNING, "Ignoring type \"{0}\", the geometry attribute " +
                                        "\"{1}\" is not mapped to a document key",
                                new Object[]{name(candidateTypeName), geometryName});
                    }
                } else {
                    LOGGER.log(Level.WARNING, "Ignoring type \"{0}\", the collection it maps " +
                                    "\"{1}.{2}\" does not exist",
                            new Object[]{name(candidateTypeName), dataStoreDB.getName(), 
                                    candidateCollectionName});
                }
            } catch (IOException e) {
                LOGGER.log(Level.WARNING, "Ignoring type \"{0}\", an exception was thrown while " +
                                "attempting to retrieve the schema: {1}",
        ...
        for (String collectionName : collectionsToCheck) {
            ...
            if (!collectionName.startsWith("system.")) {
                ...
                if (!geometryIndexSet.isEmpty()) {
                    ...
                    LOGGER.log(Level.INFO, "Ignoring collection \"{0}\", unable to find key with " +
                                    "spatial index",
    ...
    protected ContentState createContentState(ContentEntry entry) {
        ...
        try {
            ...
            LOGGER.log(Level.WARNING, "Exception thrown while attempting to retrieve the schema " +
                            "for {0}: {1}",
