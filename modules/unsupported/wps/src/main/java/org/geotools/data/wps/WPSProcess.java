...
public class WPSProcess extends AbstractProcess {
   ...
   * execute the process. Instead it builds a request to send to the server to execute the process.
   * The response is parsed and returned as a map just like a process would return locally. The
   * inputs are not validated, they are just built and sent to the server in a request. If they
   * result in a bad request, the request system will handle the returned error.
   *
