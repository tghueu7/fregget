...
public abstract class FilteredPowerSetBuilder<T, R> {
  ...
  /**
   * Recursively builds all possible signatures in the domain (will stop immediately if a signature
   * is not accepted, or builds on top of a already rejected signature)
   *
   * @param s
   * @param k
   * @param n
   ...
   * generate a valid combination when grown with more objects (thus, not a candidate for returning
