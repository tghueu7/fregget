...
class DomainCoverage {
  ...
  class SLDSelector {
    ...
    /**
     * Returns a list of scale dependent filters that represent the difference (the uncovered area)
     * between this {@link SLDSelector} and then specified rule
     *
     * @param rule
     * @return
     */
  ...
  /**
   * The set of selectors generated so far. We can get several repeated selectors due to conditional
   * pseudo-classes, but only the first one will be not covered
   */
  ...
  public List<CssRule> addRule(CssRule rule) {
    ...
    if (elements.isEmpty()) {
      ...
      if (!reducedCoverage.isEmpty()) {
        ...
        for (SLDSelector rc : reducedCoverage) {
          derivedRules.add(
              new CssRule(rc.toSelector(simplifier), rule.getProperties(), rule.getComment()));
        }
  ...
  /**
   * Simplifies a filter via the simplifying filter visitor, taking into account the target feature
   * type
   *
   * @param filter
   * @return
