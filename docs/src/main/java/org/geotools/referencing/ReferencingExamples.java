...
public class ReferencingExamples {
   ...
   *       codes. This file does not derive directly from the official EPSG database, so its should
   *       be used with caution. It provides a very simple method of creating a new authority
   *       factory and named objects.
   * </ul>
   *
   * The specific authority factory returned by getCRSAuthorityFactory is dependent on the different
   * factories on your classpath (ie WKT or Access or HSQL) and the hints you provide. By default
   * the "better" authority factory should be used if more than one is available.
   *
  ...
  /**
   * Creates a NAD 27 geographic CRS. Notice that the datum factory automatically adds aliase names
   * to the datum (because "North American Datum 1927" has an entry in http://svn.geotools.org
   * /geotools/trunk/gt/module/referencing/src/org/geotools/referencing/factory
   * /DatumAliasesTable.txt ). Also notice that toWGS84 information (used in a datum transform) was
